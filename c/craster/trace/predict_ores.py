...
Then checks the prediction against BOTH worlds:
  ...
  - the craster dump (world_dump CRWD)
...
def main():
    ...
    # craster dump: one tile per target chunk (small 4x4 window centered-ish)
    ...
    for (cx, cz) in targets:
        ...
        cmd = [dump_bin, "--seed", str(args.seed), "--cx0", str(cx - 1), "--cz0", str(cz - 1),
               "--ncx", "4", "--ncz", "4", "--out", out]
        rr = subprocess.run(cmd, cwd=CRASTER, capture_output=True, text=True)
        if rr.returncode != 0:
            sys.exit(f"world_dump failed: {rr.stderr}")
        _, cch = read_craster(out)
        cras = {}
        ...
        print(f"  {'ore':10s} {'java hit':>12s} {'craster hit':>14s}")
