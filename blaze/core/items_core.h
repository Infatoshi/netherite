...
typedef struct ICStack {
    ...
    i32 count;
    i32 meta;
    /* ItemStack NBT fields needed by ContainerRepair. custom_name is a
     * runtime-local interned-string id (0 means no display.Name); keeping the
     * text cold avoids adding 32 bytes to every CPU/CUDA inventory stack. */
    i32 repair_cost;
    i32 custom_name;
    /* Runtime-local handle for the complete ItemStack tag compound. Zero is
     * no tag. The pointed-to bytes stay in a cold runtime intern table, so
     * split/copy/merge paths carry arbitrary NBT without putting pointers in
     * CPU/CUDA state. */
    i32 tag_id;
    i32 n_enchants;
...
MC_HD static inline ICStack ic_empty(void) {
    ICStack s;
    int i;
    s.item = IC_AIR; s.count = 0; s.meta = 0;
    s.repair_cost = 0; s.custom_name = 0; s.tag_id = 0; s.n_enchants = 0;
...
/* Item.getContainerItem for the vanilla items that own one in 1.11.2.
 * The returned stack is a fresh single item, as CraftingManager's default
 * getRemainingItems path calls ForgeHooks.getContainerItem per occupied cell. */
MC_HD static inline ICStack ic_crafting_container_item(const ICStack *s) {
    if (!s || s->item == IC_AIR || s->count <= 0) return ic_empty();
    if (s->item == IC_WATER_BUCKET || s->item == IC_LAVA_BUCKET
            || s->item == 335) /* milk bucket */
        return ic_mk(IC_BUCKET, 1, 0);
    if (s->item == 437) /* dragon's breath */
        return ic_mk(374, 1, 0); /* glass bottle */
    return ic_empty();
}
...
/* EnchantmentHelper.getEnchantmentLevel: NBT list order is observable for
 * malformed duplicate ids, so return the first matching entry rather than
 * taking a maximum.  Equipment helpers select the appropriate stack before
 * calling this compact representation. */
MC_HD static inline int ic_enchantment_level(const ICStack *stack, int id) {
    int i;
    if (!stack || stack->item == IC_AIR || stack->count <= 0) return 0;
    for (i = 0; i < stack->n_enchants && i < IC_MAX_ENCHANTS; ++i)
...
/* ItemStack.areItemsEqual + represented areItemStackTagsEqual fields. */
MC_HD static inline int ic_stack_equal(const ICStack *a, const ICStack *b) {
    ...
    if (a->repair_cost != b->repair_cost || a->custom_name != b->custom_name
            || a->tag_id != b->tag_id)
        return 0;
