/* cuenv_snapshot.c - host-side .bsnp loader (format: cuenv_snapshot.h; the
 ...
 * regions containing liquids (ids 8-11) - fluids are not simulated in cuenv. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "cuenv_snapshot.h"
...
int cuenv_snapshot_load(const char *path, CuSnapshot *out,
    ...
    if (out->head.n_items > CUENV_SNAP_MAX_ITEMS || out->head.rnx <= 0 ||
     ...
     * xy_off NULL - consumers fall back to the full scan. CUENV_NO_ORE_XY=1
     * forces the fallback (legacy full-scan A/B; load-time only, no tick
     * cost). */
    out->xy_off = NULL;
    if (out->ncoal && !(getenv("CUENV_NO_ORE_XY") &&
                        atoi(getenv("CUENV_NO_ORE_XY")))) {
        long ncell = (long)out->head.rnx * out->head.rny;
        out->xy_off = (int *)malloc(((size_t)ncell + 1) * sizeof *out->xy_off);
        if (out->xy_off &&
            !cuenv_build_ore_xy(out->coal, (int)out->ncoal,
    ...
    out->cont = (int *)malloc((size_t)CUENV_SNAP_MAX_CONT * 3 *
                              sizeof *out->cont);
    out->ncont = out->cont
        ? cuenv_build_containers(out->cells, out->head.rx0, out->head.ry0,
