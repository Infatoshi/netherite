/* cuenv_cuda.cu - CUDA driver over cuenv_core.h, exporting the SAME C ABI as
 * cuenv_cpu.c so Python picks a .so at load time. One env per thread for the
 ...
 *   k_tick  : cuenv_decision_begin + `repeat` cuenv_decision_subtick per env
 ...
 *             the kernel comment; CUENV_LEGACY_RECENTER=1 selects the old
 *             serial-recenter kernel at create for A/B)
 *   k_obs   : cuenv_render_cam_pixel for envs whose decision frame is fresh,
 *             then copies the persisted frame into the caller's tensors
 *   k_final : cuenv_decision_finalize - deferred crosshair/+10 reward terms
 ...
 *
 * Action ABI (mirrors cuenv_cpu.c): cuenv_step takes double actions[n][12] =
 * the FULL raw action vector in cuenv_tick_raw order {forward,strafe,dyaw,
 ...
 * are expanded to this layout in cuenv.py (bit-identical decode).
 *
 * Pointer convention: cuenv_step's actions and all outputs are DEVICE
 ...
 * are allocated ONCE at the FIRST cuenv_load_snapshots (dims unknown at
 ...
 * cuenv_create/cuenv_load_snapshots; kernels only mutate bytes. The per-env
 ...
 * cuenv_create/cuenv_load_snapshots fail gracefully (NULL / -1) with a GB
 * estimate if any cudaMalloc fails.
 *
 * Build: nvcc -O2 --fmad=false (mc-sim determinism flags; NEVER fast-math),
 * Makefile target cuenv_cuda_so. Optional kernel timing: set CUENV_KTIME=1
...
typedef struct {
    RlSnapHead head;
    RlSnapItem items[CUENV_SNAP_MAX_ITEMS];
    ...
    const int *xy_off;           /* device, rnx*rny+1 CSR (ix,iy)->coal-range
                                  * offsets (cuenv_build_ore_xy); NULL =
...
typedef struct {
    ...
    cudaStream_t stream;
    CuEnv *d_envs;
    CuEnv *h_envs;               /* host staging mirror (pool pointers) */
    McSinTable *d_st;
    int *d_assign;
    int *h_assign;
    ...
    Chunk *d_window;
    CuCand *d_cand;
    int *d_cont;                 /* per-env CUENV_SNAP_MAX_CONT container cells */
    McAABB *d_aabb;
    CRRecipe *d_recipes;         /* crf_build once at create, uploaded once */
    int nrecipes;
    /* snapshot cache */
    CuSnapDev h_snaps[CUENV_MAX_SNAPS];   /* mirrors d_snaps; .cells/.coal are
                                           * device pointers */
    CuSnapDev *d_snaps;
    int nsnaps;
    int has_liquid[CUENV_MAX_SNAPS];
    ...
    int legacy_recenter;  /* CUENV_LEGACY_RECENTER=1 at create: A/B fallback
                           * to the serial-recenter k_tick_legacy (host-side
                           * launch pick; zero tick cost) */
    int warp_tick;        /* CUENV_WARP_TICK (default 1): one env per WARP -
                           * 32x resident warps for the latency-bound serial
                           * chains + warp-parallel coal sweep. 0 = flat
                           * one-env-per-thread k_tick. */
    /* optional kernel timing (CUENV_KTIME=1) */
    int ktime;
    cudaEvent_t ev[4];
    double ms_tick, ms_obs, ms_final;
    long nsteps;
    /* optional k_tick stage cycle counters (CUENV_STAGE_TIME=1):
    ...
    /* optional op-trace activity counters (CUENV_OP_TRACE=1): device pool of
...
__global__ void k_reset_scalar(CuEnv *envs, const int *active, int nactive,
    ...
    cuenv_reset_scalar(&envs[i], &s->head, s->items, s->coal, s->ncoal,
...
__global__ void k_reset_bulk(CuEnv *envs, const int *active,
    ...
    cuenv_reset_bulk(&envs[i], snaps[assign[i]].cells, (long)(gi % bulk));
}
 ...
 * cuenv_decision_ticks: same recenter sequence point, same fill values
...
__global__ void k_tick(CuEnv *envs, int n, const McSinTable *st,
    ...
    int valid = i < n;
    CuEnv *e = &envs[valid ? i : 0];
    int exec = 0;
    unsigned long long t0 = 0, t1 = 0;
    unsigned long long c_begin = 0, c_re = 0, c_sub = 0;
    if (stage_cycles) t0 = cu_clk64();
    if (valid)
        exec = cuenv_decision_begin(e, st,
                                    actions + (size_t)i * CUENV_ACT_HEADS,
                                    recipes, nrecipes);
    if (stage_cycles) {
        t1 = cu_clk64();
        c_begin = t1 - t0;
...
/* ---- warp-per-env tick (CUENV_WARP_TICK, default ON) ----
 *
...
__device__ __forceinline__ int cu_k2(const CuEnv *e, int wx, int wy, int wz) {
    /* region-local pack, monotonic in world (x,y,z) lex order */
    return ((wx - e->rx0) << 16) | ((wy - e->ry0) << 8) | (wz - e->rz0);
}

__device__ void cu_coal_warp(CuEnv *env, int lane,
    ...
    if (fell) {
        ...
        if (lane == 0) {
             ...
             * CU_OP_COAL_CALL: cuenv_coal_list would re-count it) */
            (void)cuenv_coal_sweep(env, fx, fy, fz, pwx, pwz, y0, y1,
                                   coal_now);
            have = cuenv_nearest_coal(coal_now, (double)fx, (double)fy,
    ...
    {   /* nearest: argmin by (d, rank), winner evaluates the angles */
        ...
        if (lane == win && k > 0) {
            ...
            ry = cuenv_wrap180(atan2(-dx, dz) * (180.0 / CU_DEC_PI) -
                               (double)env->pl.yaw);
...
__global__ void k_tick_warp(CuEnv *envs, int n, const McSinTable *st,
    ...
    CuEnv *e = &envs[w];
    const double *a = actions + (size_t)w * CUENV_ACT_HEADS;
    int exec = 0;
    unsigned long long t0 = 0, t1 = 0;
    unsigned long long c_begin = 0, c_re = 0, c_sub = 0;
    if (stage_cycles && lane == 0) t0 = cu_clk64();
    if (lane == 0)
        exec = cuenv_decision_begin(e, st, a, recipes, nrecipes);
...
/* Pre-cooperative kernel, selectable at CREATE time (CUENV_LEGACY_RECENTER=1)
 * for A/B benching; zero cost on the default path (host-side launch pick). */
__global__ void k_tick_legacy(CuEnv *envs, int n, const McSinTable *st,
                              const double *actions, int repeat,
                              McAABB *aabb_pool, const CRRecipe *recipes,
                              int nrecipes, double atk_gate) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    cuenv_decision_ticks(&envs[i], st,
                         actions + (size_t)i * CUENV_ACT_HEADS, repeat,
                         aabb_pool + (size_t)i * PSV_MAX_BLOCKS, 0, atk_gate,
                         recipes, nrecipes);
}

__global__ void k_obs(CuEnv *envs, int n, const McSinTable *st,
                      unsigned short *cam, unsigned char *depth,
                      unsigned char *edge) {
    int gi = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    CuEnv *e = &envs[i];
    if (e->dec_cam_fresh)
        cuenv_render_cam_pixel(e, st, pix);
...
__global__ void k_final(CuEnv *envs, int n, const McSinTable *st,
                        float *scal, float *rew,
                        unsigned char *done, float *pose, double atk_gate,
                        int *status) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    cuenv_decision_finalize(&envs[i], st,
                            scal ? scal + (size_t)i * 6 : NULL,
                            rew ? rew + i : NULL,
                            done ? done + i : NULL,
                            pose ? pose + (size_t)i * 5 : NULL,
                            atk_gate);
    if (status) cuenv_fill_status(&envs[i], status + (size_t)i * CU_STATUS_K);
}

/* verify helpers: single-thread, lockstep with the CPU driver's semantics */
__global__ void k_emit(CuEnv *envs, int env, const McSinTable *st,
                       int want_cam, CuBinObs *out) {
    if (threadIdx.x || blockIdx.x) return;
    cuenv_emit_bolr(&envs[env], st, out, want_cam);
}
 ...
 * driver's cuenv_tick_raw: craft, then interact, then smelt, then the tick
 * - same thread, same order. */
__global__ void k_tick_raw(CuEnv *envs, int env, int n, const McSinTable *st,
    ...
    if ((int)ra.a[10] >= 0)
        (void)cuenv_do_craft(&envs[i], (int)ra.a[10], recipes, nrecipes);
    if ((int)ra.a[11])
        (void)cuenv_do_interact(&envs[i]);
    if ((int)ra.a[12])
        (void)cuenv_do_smelt(&envs[i]);
    cuenv_runtime_tick(&envs[i], st, act,
                       aabb_pool + (size_t)i * PSV_MAX_BLOCKS);
    if (out && env >= 0)
        cuenv_emit_bolr(&envs[i], st, out, want_cam);
}

/* =================== host driver =================== */

static int cu_ck(cudaError_t e, const char *what) {
    if (e == cudaSuccess) return 0;
    fprintf(stderr, "cuenv_cuda: %s: %s\n", what, cudaGetErrorString(e));
    return -1;
}
...
void *cuenv_create(int device, int n) {
    ...
    v->device = device;
    v->success_item = 263;
    v->ktime = getenv("CUENV_KTIME") && atoi(getenv("CUENV_KTIME"));
    v->stage_time = getenv("CUENV_STAGE_TIME") &&
                    atoi(getenv("CUENV_STAGE_TIME"));
    v->legacy_recenter = getenv("CUENV_LEGACY_RECENTER") &&
                         atoi(getenv("CUENV_LEGACY_RECENTER"));
    v->warp_tick = getenv("CUENV_WARP_TICK")
                       ? atoi(getenv("CUENV_WARP_TICK")) : 1;
    v->op_trace = getenv("CUENV_OP_TRACE") && atoi(getenv("CUENV_OP_TRACE"));
    v->h_assign = (int *)calloc((size_t)n, sizeof *v->h_assign);
    v->h_active = (int *)calloc((size_t)n, sizeof *v->h_active);
    v->h_envs = (CuEnv *)calloc((size_t)n, sizeof *v->h_envs);
    ...
    if (cudaStreamCreate(&v->stream) != cudaSuccess ||
        cudaMalloc(&v->d_envs, (size_t)n * sizeof(CuEnv)) != cudaSuccess ||
        ...
        cudaMalloc(&v->d_cont,
                   (size_t)n * CUENV_SNAP_MAX_CONT * 3 *
        ...
        cudaMalloc(&v->d_snaps,
                   (size_t)CUENV_MAX_SNAPS * sizeof(CuSnapDev)) != cudaSuccess ||
        cudaMalloc(&v->d_obs, sizeof(CuBinObs)) != cudaSuccess) {
        fprintf(stderr, "cuenv_cuda: cudaMalloc failed for n=%d fixed pools "
    ...
    {   /* upload the LUT sin table + the crf recipe table (built once) */
        ...
        if (!h_st || !h_rec) {
            free(h_st); free(h_rec); cuenv_destroy(v);
            return NULL;
        }
    ...
    if (v->op_trace) {
        ...
        if (cudaMalloc(&v->d_ops, nb) != cudaSuccess) {
            fprintf(stderr, "cuenv_cuda: op-trace counter alloc failed\n");
            cuenv_destroy(v);
            return NULL;
        }
    ...
    for (i = 0; i < n; ++i) {
        ...
        e->cont = v->d_cont + (size_t)i * CUENV_SNAP_MAX_CONT * 3;
        e->ops = v->d_ops ? v->d_ops + (size_t)i * CU_OP_N : NULL;
    }
    if (cu_ck(cudaMemcpy(v->d_envs, v->h_envs, (size_t)n * sizeof(CuEnv),
    ...
    if (v->stage_time) {
        if (cudaMalloc(&v->d_stage_cycles, 3 * sizeof(unsigned long long)) !=
            cudaSuccess) {
            fprintf(stderr, "cuenv_cuda: stage_time counter alloc failed\n");
            cuenv_destroy(v);
            return NULL;
        }
...
void cuenv_destroy(void *vh) {
    ...
    if (v->ktime && v->nsteps) {
        fprintf(stderr,
                "cuenv_cuda ktime: %ld steps  k_tick %.1f ms (%.3f ms/step)  "
    ...
    if (v->stage_time && v->d_stage_cycles) {
        ...
        if (sum) {
            fprintf(stderr,
                    "cuenv_cuda stage_time (thread-cycle share of k_tick):  "
...
static int cu_alloc_region_pools(CuVecCu *v, int rnx, int rny, int rnz,
    ...
    if (cudaMemcpy(v->d_envs, v->h_envs, (size_t)v->n * sizeof(CuEnv),
...
/* cuenv_step + an optional int32[n][CU_STATUS_K] status readout (device
 * pointer; the 9 rl_inv_ids counts, hotbar_sel, held item id, container).
 * status == NULL is the legacy cuenv_step. */
int cuenv_step_full(void *vh, const double *actions, int repeat,
                    unsigned short *cam, unsigned char *depth,
                    unsigned char *edge, float *scal, float *rew,
                    unsigned char *done, float *pose, int *status) {
    ...
    if (cu_ck(cudaStreamSynchronize(v->stream), "cuenv_step")) return -1;
    if (v->ktime) {
        float ms;
...
int cuenv_capture(void *vh, int env, int slot) {
    ...
    (void)cuenv_capture_head(&he, &d->head, d->items);
    if (!d->cells) {
        u16 *cells = NULL;
    ...
    {   /* container list: the env's LIVE device list is exactly the captured
        ...
        if (!d->cont) {
            int *cn = NULL;
            if (cu_ck(cudaMalloc(&cn, (size_t)CUENV_SNAP_MAX_CONT * 3 *
                                          sizeof(int)),
                      "capture cont alloc"))
                return -1;
            d->cont = cn;
 ...
 * thread per env; no obs - use cuenv_emit per lane). Mirrors the CPU
 ...
 * Returns -1 when tracing is off (CUENV_OP_TRACE unset at create). */
