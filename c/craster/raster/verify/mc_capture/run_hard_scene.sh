...
# full craster stack (worldgen → mesh → shade → software raster) and diffs against
...
#   seed 0, eye (8.3, 95.0, 40.5), craster yaw 0 / pitch -35, 854x480 FOV 77
#
# Gates:
...
#
# Usage (from c/craster):
#   bash raster/verify/mc_capture/run_hard_scene.sh
...
# craster yaw/pitch degrees
print(f"YAW={c.get('craster_yaw_deg',0.0)}")
print(f"PITCH={c.get('craster_pitch_deg',-35.0)}")
...
PY
  ...
  echo "    camera.json: eye ($EYE_X,$EYE_Y,$EYE_Z) craster_yaw=$YAW pitch=$PITCH fov_eff=$FOV"
fi
...
export CRASTER_SPAWN_CX="${CRASTER_SPAWN_CX:-2}"
export CRASTER_SPAWN_CZ="${CRASTER_SPAWN_CZ:-11}"
if [ -z "${CRASTER_PREP_LIST:-}" ]; then
  for cand in \
      "$REPORT_DIR/wv_prep_list.txt" \
      /tmp/frustum_wv2/wv_prep_list.txt \
      trace/out/wv_prep_list.txt; do
    if [ -s "$cand" ]; then
      export CRASTER_PREP_LIST="$cand"
      break
    fi
  done
fi
echo "    prepare: spawn=($CRASTER_SPAWN_CX,$CRASTER_SPAWN_CZ) prep_list=${CRASTER_PREP_LIST:-derived}"
echo
...
gcc "${FLAGS[@]}" raster/verify/mc_capture/game_candidate.c \
...
SMOOTH_PNG=$(CRASTER_SMOOTH=1 render_variant smooth CRASTER_SMOOTH=1)
# terrain fog
FOG_PNG=$(CRASTER_FOG=1 render_variant fog CRASTER_FOG=1)
# fog off escape (pre-default baseline)
NOFOG_PNG=$(CRASTER_FOG=0 render_variant nofog CRASTER_FOG=0)
# gamma 2.0 (MC-ish curve; often worsens crop when mis-applied)
GAMMA_PNG=$(CRASTER_GAMMA=2.0 render_variant gamma2 CRASTER_GAMMA=2.0)
# smooth + fog (interaction)
BOTH_PNG=$(CRASTER_SMOOTH=1 CRASTER_FOG=1 render_variant smooth_fog CRASTER_SMOOTH=1 CRASTER_FOG=1)
...
cp -f "$REPORT_DIR/cand_default.png" "$OUT/hard_scene_craster.png" 2>/dev/null || true
