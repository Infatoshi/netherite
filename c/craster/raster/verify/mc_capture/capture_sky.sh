...
# that craster's arbitrary-pose seed->raster path (game_candidate.c) is diffed
...
#     tick chunks in, NO fluid() flood (craster's world is clean),
...
#
# Outputs (this dir): mc_sky_<i>.png + pose_<i>.json for each pose i (0-based).
# pose_0 is the frozen rung-4 pose, so mc_sky_0.png == the rung-4 golden.
#
...
OUTDIR=/home/infatoshi/dev/minecraft/mc-1.11.2-env/c/craster/raster/verify/mc_capture
...
FOV=70            # MC "Normal" FOV default == craster fov_deg
...
# faces -Z, POSITIVE pitch looks DOWN. The MATCHING craster pose (used by
# run_game_verify.sh) is craster_yaw = 180 - MC_yaw, craster_pitch = -MC_pitch.
# SKY poses: high eye (y=140, above all seed-0 terrain) looking UP (negative MC
# pitch) so the frame is sky-dominant. These are the goldens for the sky rung
# (gradient + clouds + sun); terrain only intrudes at the -10 near-horizon pose.
#   0: zenith (straight up)   1: up 60   2: up 30   3: near-horizon up 10
POSES=(
...
# over poses (tp + tick + write /tmp/qrl_sky_<i>.json). It signals us per pose
# via a ready file so we can grab the window between teleports.
cd "$ENVDIR"
rm -f /tmp/qrl_sky_*.json /tmp/qrl_skygrab_ready_* /tmp/qrl_skygrab_done_* /tmp/qrl_sky_all_done
POSES_STR="${POSES[*]}"
...
for i in range(n):
    ...
    obs = e.obs()
    with open("/tmp/qrl_sky_%d.json" % i, "w") as f:
    ...
    open("/tmp/qrl_skygrab_ready_%d" % i, "w").close()
    for _ in range(600):
        if os.path.exists("/tmp/qrl_skygrab_done_%d" % i):
            break
        time.sleep(0.1)
open("/tmp/qrl_sky_all_done", "w").close()
e.close()
PY
QRL_PID=$!
...
for ((i=0; i<NP; i++)); do
    ...
    for _ in $(seq 1 1200); do
        [ -f "/tmp/qrl_skygrab_ready_$i" ] && { ready=1; break; }
...
p.update({"fov": fov, "width": w, "height": h, "pose_index": idx,
          "notes": ("REAL MC 1.11.2 client frame, llvmpipe software GL, seed 0 "
                    ...
                    "mc_yaw/mc_pitch are the teleport heading; the matching craster pose "
                    "is craster_yaw=180-mc_yaw, craster_pitch=-mc_pitch. COARSE golden.")})
json.dump(p, open(out, "w"), indent=2)
print("[capture] wrote", out)
PY
    # tell the python side to advance to the next pose
    touch "/tmp/qrl_skygrab_done_$i"
    log "pose $i captured."
done

# --- 5. wait for python to finish, stop the game ---
for _ in $(seq 1 300); do [ -f /tmp/qrl_sky_all_done ] && break; sleep 1; done
...
log "done. captured $NP sky poses -> $OUTDIR/mc_sky_<i>.png + sky_<i>.json"
