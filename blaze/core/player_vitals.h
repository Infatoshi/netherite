...
/* FoodStats.onUpdate (40-102), gamerule-driven naturalRegeneration. Difficulty
 * uses EnumDifficulty ids: 0 peaceful, 1 easy, 2 normal, 3 hard. */
MC_HD static inline void pv_on_update_difficulty_gr(
        PvStats *s, const McGameRules *gr, i32 difficulty) {
    ...
    if (flag && s->saturation > 0.0f && pv_should_heal(s) && s->foodLevel >= 20) {
        ...
        ++s->foodTimer;
        if (s->foodTimer >= 80) {
            if (s->health > 10.0f || difficulty == 3
                    || (s->health > 1.0f && difficulty == 2))
                pv_attack(s, 1.0f);
            s->foodTimer = 0;
        }
    } else {
...
/* Normal-difficulty compatibility wrapper retained for CPU/CUDA curricula and
 * focused oracles whose input contract pins the vanilla default. */
MC_HD static inline void pv_on_update_gr(PvStats *s, const McGameRules *gr) {
    pv_on_update_difficulty_gr(s, gr, 2);
}
