"""Look-at-nearest-tree REINFORCE on N parallel craster --rl envs.
...
craster script (type "action" events) so any episode replays deterministically
with rendering for the mp4s (same seed, same tick order as rl mode).

Run (anvil):
  cd c/craster && uv run --no-project --with numpy,matplotlib python rl/look_at_tree.py
"""
import json
import math
import os
...
def main():
    ...
    envs = [CrasterEnv(seed) for seed in SEEDS]
    ...
    # persist: curve, env-0 action stream as a craster script, episode marks
    ...
    plt.title("REINFORCE: look at nearest tree (16 craster envs, 9 look actions)")
