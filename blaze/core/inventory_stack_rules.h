...
/* EntityLiving.getSlotForItemStack for the right-click self-equip items in
 * 1.11.2. ItemArmor and ItemElytra consume the action even when the target
 * slot is occupied; only an empty slot transfers one complete tagged item. */
MC_HD static inline int isr_right_click_equipment_slot(i32 item) {
    if (item == ISR_ELYTRA_ITEM) return ISR_ARMOR_CHEST;
    switch (item) {
    case 298: case 302: case 306: case 310: case 314: return ISR_ARMOR_HEAD;
    case 299: case 303: case 307: case 311: case 315: return ISR_ARMOR_CHEST;
    case 300: case 304: case 308: case 312: case 316: return ISR_ARMOR_LEGS;
    case 301: case 305: case 309: case 313: case 317: return ISR_ARMOR_FEET;
    default: return -1;
    }
}

/* Complete vanilla 1.11.2 Item.REGISTRY census. Limits are only 1, 16, 64. */
MC_HD static inline i32 isr_max_stack_size(i32 item, i32 meta) {
    (void)meta;
    if ((item >= 219 && item <= 234)
            || (item >= 256 && item <= 259) || item == 261
            || (item >= 267 && item <= 279)
            || (item >= 282 && item <= 286)
            || (item >= 290 && item <= 294)
            || (item >= 298 && item <= 317)
            || (item >= 326 && item <= 329)
            || item == 333 || item == 335 || item == 342 || item == 343
            || item == 346 || item == 354 || item == 355 || item == 359
            || item == 373 || item == 386 || item == 398 || item == 403
            || item == 407 || item == 408 || item == 413
            || (item >= 417 && item <= 419) || item == 422 || item == 436
            || item == 438 || (item >= 441 && item <= 449)
            || (item >= 2256 && item <= 2267))
        return 1;
    if (item == 323 || item == 325 || item == 332 || item == 344
            || item == 368 || item == 387 || item == 416 || item == 425)
        return 16;
    return ISR_INV_LIMIT;
}

MC_HD static inline int isr_is_damageable(i32 item) {
    return (item >= 256 && item <= 259) || item == 261
        || (item >= 267 && item <= 279)
        || (item >= 283 && item <= 286)
        || (item >= 290 && item <= 294)
        || (item >= 298 && item <= 317)
        || item == 346 || item == 359 || item == 398
        || item == 442 || item == 443;
}

MC_HD static inline int isr_has_subtypes(i32 item) {
    return item == 1 || item == 3 || item == 5 || item == 6 || item == 12
        || item == 17 || item == 18 || item == 19 || item == 24
        || item == 31 || item == 35 || item == 37 || item == 38
        || item == 44 || item == 95 || item == 97 || item == 98
        || item == 126 || item == 139 || item == 145 || item == 155
        || item == 159 || item == 160 || item == 161 || item == 162
        || item == 168 || item == 171 || item == 175 || item == 179
        || item == 182 || item == 205 || item == 263 || item == 322
        || item == 349 || item == 350 || item == 351 || item == 358
        || item == 397 || item == 425;
}
...
MC_HD static inline int isr_right_click_equip(IsrInv *inv, int held_slot) {
    ICStack held = isr_get_stack(inv, held_slot);
    int equipment_slot = isr_right_click_equipment_slot(held.item);
    ICStack equipped;
    if (equipment_slot < 0) return 0;
    equipped = isr_get_stack(inv, equipment_slot);
    if (isr_is_empty(&equipped)) {
        equipped = ic_with_count(&held, 1);
        isr_set_stack(inv, equipment_slot, equipped);
        (void)isr_decr_stack_size(inv, held_slot, 1);
    }
    return 1;
}
...
MC_HD static inline i32 isr_store_partial_item_stack(IsrInv *inv, ICStack *incoming) {
    ...
    {
        i32 k = i;
        /* Java constructs a zero-count copy before asking its max size.
         * ItemStack.getItem() then observes AIR, whose limit is 64.  This
         * permits a synthetic overstack (for example 17 pearls) to remain
         * intact when first inserted into an empty player slot. */
        i32 max = created_empty
            ? isr_inv_stack_limit()
            : isr_max_stack_size(slot.item, slot.meta);
