# trace - oracle-vs-craster trace/replay verification flywheel

> Verification entry point: `c/craster/VERIFY.md`. This dir implements the
> computer loop (checkpoints/trajectory/spawns) and the human tape replay
> (`replay_tape.py`).

Start the REAL Java Minecraft 1.11.2 oracle and the craster C game at the EXACT
...
`craster_game` if missing. `--wipe` deletes `saves/qrl_<seed>` and restarts the
...
- **checkpoints** (`checkpoints.py` + `checkpoints.json`): for each named pose
  ...
  render the identical pose in craster headless (`--script` with
...
- **trajectory** (`trajectory.py`): both games replay the SAME 200-tick input
  ...
  qrl yaw steps -> craster `dyaw:15`, forward x60, jumps at t=130/140/150,
  stand). Oracle obs recorded per tick; craster `--state-out` per tick. Outputs
...
- **spawns** (`spawns.py`): frozen midnight, `doMobSpawning` on, player parked
  ...
  craster is NOT asked to reproduce spawns yet.
...
authoritative (re-read from obs after pinning, then used for the craster
...
- Pose passthrough: qrl obs / `tp` / craster `set_pose` all use FEET coords +
  MC yaw/pitch; the camera eye is 1.62 above feet on both sides. The
  MC-degrees -> craster-camera conversion lives in `game/view.h`
  (`craster_yaw = 180 - mc_yaw`); `game/frame_capture.c` now uses it (it
...
- Kill worldgen animals (`killentities`) before captures: craster renders no
...
- MC options pinned by earlier capture work and relied on here (run/options.txt):
  `renderDistance:8` (== craster `--view-distance 8`), `fancyGraphics:false`,
...
- Known irreducible-for-now diff sources (measured in report/checkpoints.md):
  craster headless draws the hand but no hotbar/hearts HUD and no crosshair;
  MC night applies its darkened lightmap to terrain while craster night terrain
  stays near day-bright (biggest terrain divergence, mean ~49/ch); under-canopy
  smooth lighting differs; craster sky is a flat gradient without MC's horizon
  band.
