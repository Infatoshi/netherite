# Checkpoint pixel parity: REAL MC 1.11.2 vs craster

Same seed-0 world, same pose (feet + MC yaw/pitch), same 854x480 FOV-70 camera. Oracle = qrl in-process frame grab (llvmpipe SW GL); craster = headless `--script` render, frame after 2 ticks. Diff = abs per-channel over RGB; `%diff` = fraction of pixels with any channel delta > 0. Regions: whole frame; terrain crop (rows 14-86%, cols 9-91%: excludes sky top + HUD strip); HUD strip (bottom 10%).
