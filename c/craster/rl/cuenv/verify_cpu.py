#!/usr/bin/env python3
"""M1 fidelity gate: real craster_game (--rl-bin --snapshot-in) vs batch-of-1
CPU cuenv, stepped in lockstep with identical action streams; EVERY tick's
...
cuenv does not reproduce; the trainer never reads them.

Usage (anvil):
  cd c/craster && uv run --no-project --with numpy python rl/cuenv/verify_cpu.py \
      [--seeds 14,16,...] [--ticks 1000] [--stage 6.0] [--episodes FILE.jsonl]

Default: the 8 training seeds x 1000 seeded pseudo-random actions
(attack-heavy; same xorshift32 as cuenv_verify.c). --episodes replays real
...
craft/interact/use are simulated in cuenv, nothing is stripped.
"""
import argparse
import ctypes
import json
...
CRASTER = os.path.dirname(RL)
BIN = os.path.join(CRASTER, "craster_game")
SO = os.path.join(RL, "cuenv", "cuenv_cpu.so")
...
class Rng:
    """xorshift32, kept in sync with cuenv_verify.c."""
...
class CuEnv1:
    def __init__(self, snap):
        ...
        self.h = self.lib.cuenv_create(0, 1)
        assert self.h
        err = ctypes.create_string_buffer(256)
        paths = (ctypes.c_char_p * 1)(snap.encode())
        r = self.lib.cuenv_load_snapshots(ctypes.c_void_p(self.h), paths, 1,
                                          err, 256)
        if r < 0:
            raise RuntimeError(f"load_snapshots: {err.value.decode()}")
        self.liquid = self.lib.cuenv_snapshot_has_liquid(
            ctypes.c_void_p(self.h), 0)
        assign = (ctypes.c_int * 1)(0)
        assert self.lib.cuenv_assign(ctypes.c_void_p(self.h), assign) == 0
        assert self.lib.cuenv_reset(ctypes.c_void_p(self.h), None) == 0
        assert self.lib.cuenv_obs_size() == BIN_SIZE, \
            f"CuBinObs {self.lib.cuenv_obs_size()} != RlBinObs {BIN_SIZE}"
        self.buf = ctypes.create_string_buffer(BIN_SIZE)

    def emit(self, want_cam=1):
        assert self.lib.cuenv_emit(ctypes.c_void_p(self.h), 0, want_cam,
                                   self.buf) == 0
        return self.buf.raw
    ...
    def step(self, act):
        ...
        r = self.lib.cuenv_tick_raw(ctypes.c_void_p(self.h), 0, a,
                                    act.get("cam", 1), self.buf)
        assert r == 0, "cuenv_tick_raw failed"
        return self.buf.raw

    def debug(self):
        out = (ctypes.c_double * 21)()
        self.lib.cuenv_debug_state(ctypes.c_void_p(self.h), 0, out, 21)
        return list(out)

    def close(self):
        self.lib.cuenv_destroy(ctypes.c_void_p(self.h))
...
def run_seed(seed, snap, actions, label, show_final_inv=False):
    ...
    try:
        ...
        if not gated_equal(a_rec, b_rec):
            ...
            print(f"    cuenv: {fmt_field(b_rec, f)}")
            ok = False
        for t, act in enumerate(actions):
            ...
            if not gated_equal(a_rec, b_rec):
                ...
                print(f"    cuenv: {fmt_field(b_rec, f)}")
                d = cu.debug()
                print(f"    cuenv state: pos=({d[0]:.17g},{d[1]:.17g},"
...
def main():
    ...
    if args.chain:
        ...
        # simulated in cuenv; only a stray "snapshot" key would be protocol-
    ...
    if args.episodes and os.path.exists(args.episodes):
        with open(args.episodes) as f:
            for li, line in enumerate(f):
                ...
                # (craft/interact/snapshot): cuenv does not implement them, so
                ...
                # real env and cuenv share the want_cam semantics.
                seed = seeds[0]
                snap = os.path.join(SNAPS, f"s{seed}_d{args.stage}.bsnp")
                total += 1
