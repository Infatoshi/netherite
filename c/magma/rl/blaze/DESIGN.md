...
Produced 2026-07-14 by a design pass over magma + mc-sim. Code is ground truth; file:line
...
- Dig targeting uses `gm_raycast_sel_reach` (c/magma/game/sel_box.c:250-292): fixed-step DDA (`PSV_RAY_DT=0.05`, reach `PSV_REACH=5.0`), LUT trig, with per-block selection boxes (`gm_sel_box`, sel_box.c:34). This file is magma-side (not MC_HD), but pure deterministic math.
...
- `c/magma/trace/world_dump.c` exists but is a worldgen-verification tool (fresh world from seed, "CRWD"/"CRWS" format) — it cannot capture post-prefix mutations (mined tunnel, placed table) or player/inventory state. **A new state-dump in rl_mode.c is the right path.**
...
3. Then CPU blaze vs CUDA blaze, batch of 4096 (mixed seeds/stages), same actions: **bitwise-identical** dumps (the standard mc-sim oracle bar, SPEC.md:21).
...
- `c/magma/Makefile`: targets `rl/blaze/blaze.so` (`$(NVCC) -O2 --fmad=false -arch=sm_120 -Icore -I. -I$(MCSIM)/core --shared -Xcompiler -fPIC`) and `rl/blaze/blaze_verify` (CC path with `-ffp-contract=off`). Take `SM` as an override var.
...
7. **Float discipline**: `-ffp-contract=off` (CC) / `--fmad=false` (nvcc) are mandatory (mc-sim/SPEC.md:46-47,139,156-157; magma/Makefile:7-8). Use `MC_NOINLINE` on any large ported function to keep nvcc compile times sane (mc.h:11-16).
