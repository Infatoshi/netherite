#!/usr/bin/env python3
"""M2 gate: CPU cuenv vs CUDA cuenv, identical snapshots and action streams.
...
craft:N + interact) via the raw-tick path (cuenv_tick_raw env=-1 broadcast).
EVERY tick, EVERY lane's full BOLR record must match the batch-of-1 CPU
cuenv record byte-for-byte (the CPU env is itself byte-exact vs the real
...
Usage (anvil, GPU0 must be idle - check nvidia-smi first):
  cd c/craster && uv run --no-project --with numpy,torch python \
      rl/cuenv/verify_cuda.py [--big|--chain|--mixed] \
...
from cuenv import VecCuEnv, CPU_SO, CUDA_SO, NPIX  # noqa: E402
...
class ActStream:
    ...
    cuenv_verify.c's generator shape: 5 draws per decision)."""
...
def make_envs(n_cpu, n_cuda, paths, cpu_assign, cuda_assign, device):
    cpu = VecCuEnv(n_cpu, device=0, so_path=CPU_SO)
    cuda = VecCuEnv(n_cuda, device=device, so_path=CUDA_SO)
...
def _raw_abi(env):
    """Declare the raw-tick/emit verify-helper ABI on a VecCuEnv's lib."""
    env.lib.cuenv_tick_raw.argtypes = [
        ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_double),
        ctypes.c_int, ctypes.c_void_p]
    env.lib.cuenv_emit.argtypes = [ctypes.c_void_p, ctypes.c_int,
                                   ctypes.c_int, ctypes.c_void_p]
    env.lib.cuenv_obs_size.restype = ctypes.c_int
    assert env.lib.cuenv_obs_size() == BIN_SIZE
...
def run_chain(args, iron=False):
    ...
    cpu = VecCuEnv(1, device=0, so_path=CPU_SO)
    cuda = VecCuEnv(nl, device=args.device, so_path=CUDA_SO)
    ...
    def lanes_match(t, want):
        ...
        for lane in range(nl):
            assert cuda.lib.cuenv_emit(cuda.h, lane, want, buf_g) == 0
    ...
    assert cpu.lib.cuenv_emit(cpu.h, 0, 1, buf_c) == 0
    if lanes_match("initial", 1):
        for t, act in enumerate(acts):
            want = act.get("cam", 1)
            a = _raw_act(act)
            assert cpu.lib.cuenv_tick_raw(cpu.h, 0, a, want, buf_c) == 0
            assert cuda.lib.cuenv_tick_raw(cuda.h, -1, a, 0, None) == 0
...
def dump_op_trace(env, ops0, args):
    ...
    from cuenv import OP_NAMES
    ops = env.op_trace()
    if ops is None:
        print("op-trace: unavailable (CUENV_OP_TRACE was not set at create)")
        return
...
def run_bench(args):
    ...
    env = VecCuEnv(n, device=args.device, so_path=CUDA_SO)
...
def main():
    ...
    ap.add_argument("--op-trace", action="store_true",
                    help="bench only: per-env op activity counters -> "
                         ...
                         "(implies CUENV_OP_TRACE=1)")
    args = ap.parse_args()
    if args.ktime:
        os.environ["CUENV_KTIME"] = "1"
    if os.environ.get("CUENV_OP_TRACE", "0") not in ("", "0"):
        args.op_trace = True
    if args.op_trace:
        os.environ["CUENV_OP_TRACE"] = "1"
    if args.bench:
        ok = run_bench(args)
    elif args.iron:
