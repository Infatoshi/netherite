...
OUT="$ROOT/c/render-opt/dropin/composed"
echo "$MODE" > "$ROOT/c/render-opt/dropin/qsin_mode.txt"
echo "$MODE" > "$ROOT/c/render-opt/dropin/lightmap/qlm_mode.txt"
echo "$MODE" > "$ROOT/c/render-opt/dropin/biome/qbiome_mode.txt"
...
for i in $(seq 1 300); do grep -q "\[qrl\] listening" runclient.log 2>/dev/null && { echo "qrl up ${i}s"; break; }; sleep 1; done
grep -q "\[qrl\] listening" runclient.log 2>/dev/null || { echo "ERROR no qrl"; exit 2; }
cd "$ROOT/java" && uv run --no-project python "$ROOT/c/render-opt/dropin/biome/scene_grass.py" 2>&1 | tail -6
sleep 7
