...
#define POPMC_IGLOO_CHESTS_MAX 1
#define POPMC_IGLOO_RESIDENTS_MAX 2
typedef struct {
    int wx, wz;
    unsigned short y;
...
typedef struct {
    ...
    DungeonChestSite igloo_chests[POPMC_IGLOO_CHESTS_MAX];
    IglooResidentSite igloo_residents[POPMC_IGLOO_RESIDENTS_MAX];
    ...
    int n_igloo_chests, n_igloo_residents;
...
static void igloo_capture_event(int baseCx, int baseCz,
                                const IgIgloo *igloo) {
    DungeonCapture *capture;
    if (!igloo || g_dungeon_capture_depth <= 0) return;
    capture = &g_dungeon_capture[g_dungeon_capture_depth - 1];
    if (capture->bcx != baseCx || capture->bcz != baseCz) return;
    if (igloo->chest_placed) {
        DungeonChestSite *site;
        if (capture->n_igloo_chests >= POPMC_IGLOO_CHESTS_MAX) {
            fprintf(stderr, "[populate_mc] FATAL: igloo chest cap exceeded at (%d,%d)\n",
                    baseCx, baseCz);
            abort();
        }
        site = &capture->igloo_chests[capture->n_igloo_chests++];
        site->wx = igloo->chest_x;
        site->wz = igloo->chest_z;
        site->y = (unsigned short)igloo->chest_y;
        site->facing_meta = (unsigned char)igloo->chest_facing;
        site->loot_seed = (long long)igloo->chest_loot_seed;
    }
    for (int i = 0; i < igloo->entity_count; ++i) {
        IglooResidentSite *site;
        if (capture->n_igloo_residents >= POPMC_IGLOO_RESIDENTS_MAX) {
            fprintf(stderr, "[populate_mc] FATAL: igloo resident cap exceeded at (%d,%d)\n",
                    baseCx, baseCz);
            abort();
        }
        site = &capture->igloo_residents[capture->n_igloo_residents++];
        site->x = igloo->entities[i].x;
        site->y = igloo->entities[i].y;
        site->z = igloo->entities[i].z;
        site->vx = igloo->entities[i].vx;
        site->vy = igloo->entities[i].vy;
        site->vz = igloo->entities[i].vz;
        site->health = igloo->entities[i].health;
        site->yaw = igloo->entities[i].yaw;
        site->pitch = igloo->entities[i].pitch;
        site->conversion_time = igloo->entities[i].conversion_time;
        site->fire = igloo->entities[i].fire;
        site->air = igloo->entities[i].air;
        site->profession = igloo->entities[i].profession;
        site->kind = (unsigned char)igloo->entities[i].kind;
        site->persistence = igloo->entities[i].persistence;
        site->on_ground = igloo->entities[i].on_ground;
    }
}
...
static Window *build_window(long long seed, int bcx, int bcz) {
    ...
    JavaRandom r;
    {
        ...
        owr_igloo_hook = igloo_capture_event;
        ...
        owr_igloo_hook = g_dungeon_capture_depth > 1
            ? igloo_capture_event : 0;
    ...
    win->animal_biome = g_bio[16 * W_Z + 16];
    win->animal_seed48 = r.seed;
    ...
    win->n_igloo_chests = dungeon_capture->n_igloo_chests;
    memcpy(win->igloo_chests, dungeon_capture->igloo_chests,
           (size_t)dungeon_capture->n_igloo_chests
               * sizeof win->igloo_chests[0]);
    win->n_igloo_residents = dungeon_capture->n_igloo_residents;
    memcpy(win->igloo_residents, dungeon_capture->igloo_residents,
           (size_t)dungeon_capture->n_igloo_residents
               * sizeof win->igloo_residents[0]);
...
static void apply_dungeon_meta(const Window *win, int cx, int cz,
    ...
    for (int i = 0; i < win->n_igloo_chests; ++i) {
        const DungeonChestSite *s = &win->igloo_chests[i];
        int index;
        if (s->wx < bx || s->wx >= bx + 16 ||
            s->wz < bz || s->wz >= bz + 16) continue;
        index = CB_INDEX_LOCAL(s->wx - bx, s->y, s->wz - bz);
...
int popmc_igloo_chest_info(long long seed, int x, int y, int z,
                           long long *loot_seed, int *facing_meta) {
    const DungeonChestSite *best = NULL;
    long best_seq = -1;
    if (!g_slots || y < 0 || y >= W_Y) return 0;
    for (int wi = 0; wi < g_owr_slots; ++wi) {
        const Window *w = &g_slots[wi];
        if (!w->valid || w->seed != seed || w->seq < best_seq) continue;
        for (int i = 0; i < w->n_igloo_chests; ++i) {
            const DungeonChestSite *site = &w->igloo_chests[i];
            if (site->wx == x && (int)site->y == y && site->wz == z) {
                best = site;
                best_seq = w->seq;
            }
...
int popmc_igloo_residents(long long seed,
                          int min_x, int min_z, int max_x, int max_z,
                          PopmcIglooResident *out, int capacity) {
    int count = 0;
    if (!g_slots || !out || capacity <= 0) return 0;
    for (int wi = 0; wi < g_owr_slots && count < capacity; ++wi) {
        const Window *w = &g_slots[wi];
        if (!w->valid || w->seed != seed) continue;
        for (int i = 0; i < w->n_igloo_residents && count < capacity; ++i) {
            const IglooResidentSite *site = &w->igloo_residents[i];
            int duplicate = 0;
            if (site->x < min_x || site->x > max_x
                    || site->z < min_z || site->z > max_z) continue;
            for (int j = 0; j < count; ++j)
                if (out[j].x == site->x && out[j].y == site->y
                        && out[j].z == site->z
                        && out[j].kind == (int)site->kind) {
                    duplicate = 1;
                    break;
                }
            ...
            out[count].y = site->y;
            out[count].z = site->z;
            out[count].vx = site->vx;
            out[count].vy = site->vy;
            out[count].vz = site->vz;
            out[count].health = site->health;
            out[count].yaw = site->yaw;
            out[count].pitch = site->pitch;
            out[count].conversion_time = site->conversion_time;
            out[count].fire = site->fire;
            out[count].air = site->air;
            out[count].profession = site->profession;
            out[count].kind = (int)site->kind;
            out[count].persistence = (int)site->persistence;
            out[count].on_ground = (int)site->on_ground;
            ++count;
        }
    }
    return count;
...
int popmc_swamp_witches(long long seed,
                        int min_x, int min_z, int max_x, int max_z,
                        PopmcSwampWitch *out, int capacity) {
    int count = 0;
    if (!g_slots || !out || capacity <= 0) return 0;
    for (int wi = 0; wi < g_owr_slots && count < capacity; ++wi) {
        const Window *w = &g_slots[wi];
        if (!w->valid || w->seed != seed) continue;
        for (int i = 0; i < w->n_swamp_witches && count < capacity; ++i) {
            const SwampSite *site = &w->swamp_witches[i];
            double x = (double)site->wx + 0.5;
            double y = (double)site->y;
            double z = (double)site->wz + 0.5;
            int duplicate = 0;
            if (site->wx < min_x || site->wx > max_x
                    || site->wz < min_z || site->wz > max_z) continue;
            for (int j = 0; j < count; ++j)
                if (out[j].x == x && out[j].y == y && out[j].z == z) {
                    duplicate = 1;
                    break;
                }
            if (duplicate) continue;
            ...
            out[count].vy = 0.0;
            out[count].vz = 0.0;
            out[count].health = 26.0F;
            out[count].yaw = 0.0F;
            out[count].pitch = 0.0F;
            out[count].fire = -1;
            out[count].air = 300;
            out[count].persistence = 1;
            out[count].on_ground = 0;
            ++count;
        }
    }
    return count;
...
int popmc_worldgen_spawn_batches(
        long long seed, long after_seq,
        PopmcWorldgenSpawnBatch *out, int capacity) {
    int count = 0;
    if (!out || capacity <= 0 || !g_slots) return 0;
    for (int slot = 0; slot < g_owr_slots; ++slot) {
        const Window *win = &g_slots[slot];
        if (!win->valid || win->seed != seed || win->seq <= after_seq)
            continue;
        if (count >= capacity) break;
        out[count++] = (PopmcWorldgenSpawnBatch){
            win->seq, win->bcx, win->bcz,
            win->animal_biome, win->animal_seed48
        };
    }
    for (int index = 1; index < count; ++index) {
        PopmcWorldgenSpawnBatch value = out[index];
        int before = index - 1;
        while (before >= 0 && out[before].seq > value.seq) {
            out[before + 1] = out[before];
            --before;
        }
        out[before + 1] = value;
    }
...
/* ========================= live sapling generation =========================
 *
 * BlockSapling uses the same WorldGen* implementations as decoration, but with
 * two observable differences: a newly constructed big oak keeps the default
 * four-layer leaf distance, and jungle cocoa must retain legacy facing/age
 * metadata. Keep those adaptations here while reusing every other verified
 * population generator directly. The fixed scratch is allocated only on the
 * first stage-one bonemeal firing. */

#define SAP_RAW_STATE_FLAG 0x2000

static u16 *g_sap_blocks;
static u16 *g_sap_before;
static McSinTable *g_sap_st;
static FoliageCoord *g_sap_foliage;
static u8 *g_sap_leaf_flags;
...
static int sapling_scratch_init(void) {
    if (g_sap_blocks && g_sap_before && g_sap_st && g_sap_foliage
            && g_sap_leaf_flags)
        return 1;
    free(g_sap_blocks);
    free(g_sap_before);
    free(g_sap_st);
    free(g_sap_foliage);
    free(g_sap_leaf_flags);
    g_sap_blocks = NULL;
    g_sap_before = NULL;
    g_sap_st = NULL;
    g_sap_foliage = NULL;
    g_sap_leaf_flags = NULL;
    g_sap_blocks = (u16 *)malloc(sizeof(u16) * W_N);
    g_sap_before = (u16 *)malloc(sizeof(u16) * W_N);
    g_sap_st = (McSinTable *)malloc(sizeof(McSinTable));
    g_sap_foliage = (FoliageCoord *)malloc(
        sizeof(FoliageCoord) * BT_MAX_FOLIAGE);
    g_sap_leaf_flags = (u8 *)malloc(W_N);
    if (!g_sap_blocks || !g_sap_before || !g_sap_st || !g_sap_foliage
            || !g_sap_leaf_flags) {
        free(g_sap_blocks);
        free(g_sap_before);
    ...
    mc_sin_table_init(g_sap_st);
    return 1;
}

static int sapling_leaf_block_group(int block, int index) {
    if (!pb_isLeaves(block)) return -1;
    if (!(g_sap_leaf_flags[index] & SAP_LEAF_WRITTEN)) {
        int id = g_sap_before[index] >> 4;
        if (id == 18) return 0;
        if (id == 161) return 1;
    }
    return block == PB_LEAVES_ACACIA || block == PB_LEAVES_DARKOAK;
}

static void sapling_before_write(
        World *w, int x, int y, int z, int oldState, int newState) {
    int index = w_index(x, y, z);
    int oldLeafGroup = sapling_leaf_block_group(oldState, index);
    int newLeafGroup = pb_isLeaves(newState)
        ? (newState == PB_LEAVES_ACACIA
            || newState == PB_LEAVES_DARKOAK) : -1;
    if (newLeafGroup >= 0)
        g_sap_leaf_flags[index] = SAP_LEAF_WRITTEN;
    if (oldLeafGroup < 0 || oldLeafGroup == newLeafGroup) return;

    /* Chunk.setBlockState installs the replacement before BlockLeaves.breakBlock
     * visits the 3x3x3 neighborhood. Model that center cell explicitly because
     * the compact populate world still contains oldState until this hook exits. */
    for (int dx = -1; dx <= 1; ++dx)
        for (int dy = -1; dy <= 1; ++dy)
            for (int dz = -1; dz <= 1; ++dz) {
                int nx = x + dx, ny = y + dy, nz = z + dz;
                int neighbor;
                if (!w_inb(nx, ny, nz)) continue;
                neighbor = dx == 0 && dy == 0 && dz == 0
                    ? newState : w_get(w, nx, ny, nz);
                if (pb_isLeaves(neighbor))
                    g_sap_leaf_flags[w_index(nx, ny, nz)] |= SAP_LEAF_DECAY;
            }
}

static int sapling_pb_from_state(
        u16 state, int log_marker, int leaf_marker) {
    int id = state >> 4, meta = state & 15;
    switch (id) {
    case 0: return PB_AIR;
    case 2: return PB_GRASS;
    case 3: return meta == 1 ? PB_COARSE_DIRT
                 : meta == 2 ? PB_PODZOL : PB_DIRT;
    case 6: return log_marker;
    case 17: case 162: return log_marker;
    case 18: case 161: return leaf_marker;
    case 31: return meta == 2 ? PB_FERN : PB_TALLGRASS;
    case 32: return PB_DEADBUSH;
    case 106:
        if (meta & 8) return PB_VINE_BASE;
        if (meta & 2) return PB_VINE_BASE + 1;
        if (meta & 1) return PB_VINE_BASE + 2;
        if (meta & 4) return PB_VINE_BASE + 3;
        return PB_VINE_BASE;
    case 175:
        return meta >= 8 ? PB_DPLANT_UPPER
                         : PB_DPLANT_LOWER_BASE + (meta % 6);
    default:
        return 0x4000 | state;
    }
}

static int sapling_state_from_pb(int block, u16 *state) {
    if (block & SAP_RAW_STATE_FLAG) {
        *state = (u16)(block & (SAP_RAW_STATE_FLAG - 1));
        return 1;
    }
    if (block & 0x4000) {
        *state = (u16)(block & 0x3fff);
        return 1;
    }
    switch (block) {
    case PB_AIR: *state = 0; return 1;
    case PB_DIRT: *state = (u16)(3 << 4); return 1;
    case PB_PODZOL: *state = (u16)((3 << 4) | 2); return 1;
    case PB_LOG_OAK: *state = (u16)(17 << 4); return 1;
    case PB_LOG_BIRCH: *state = (u16)((17 << 4) | 2); return 1;
    case PB_LOG_SPRUCE: *state = (u16)((17 << 4) | 1); return 1;
    case PB_LOG_OAK_X: *state = (u16)((17 << 4) | 4); return 1;
    case PB_LOG_OAK_Z: *state = (u16)((17 << 4) | 8); return 1;
    case PB_LOG_JUNGLE: *state = (u16)((17 << 4) | 3); return 1;
    case PB_LOG_ACACIA: *state = (u16)(162 << 4); return 1;
    case PB_LOG_DARKOAK: *state = (u16)((162 << 4) | 1); return 1;
    case PB_LEAVES_OAK: *state = (u16)(18 << 4); return 1;
    case PB_LEAVES_BIRCH: *state = (u16)((18 << 4) | 2); return 1;
    case PB_LEAVES_SPRUCE: *state = (u16)((18 << 4) | 1); return 1;
    case PB_LEAVES_JUNGLE: *state = (u16)((18 << 4) | 3); return 1;
    case PB_LEAVES_ACACIA: *state = (u16)(161 << 4); return 1;
    case PB_LEAVES_DARKOAK: *state = (u16)((161 << 4) | 1); return 1;
    case PB_VINE_BASE: *state = (u16)((106 << 4) | 8); return 1;
    case PB_VINE_BASE + 1: *state = (u16)((106 << 4) | 2); return 1;
    case PB_VINE_BASE + 2: *state = (u16)((106 << 4) | 1); return 1;
    case PB_VINE_BASE + 3: *state = (u16)((106 << 4) | 4); return 1;
    default: return 0;
    }
}

/* WorldGenTrees for a single jungle sapling. Biome jungle decoration enables
 * vines/cocoa, but BlockSapling constructs this generator with vinesGrow=false. */
static int sapling_jungle_tree(
        World *w, JavaRandom *r, int posX, int posY, int posZ,
        int minTreeHeight) {
    int i = jrand_int_bound(r, 3) + minTreeHeight;
    int flag = 1;
    if (posY < 1 || posY + i + 1 > W_Y) return 0;
    for (int j = posY; j <= posY + 1 + i; ++j) {
        int k = j == posY ? 0 : j >= posY + 1 + i - 2 ? 2 : 1;
        for (int l = posX - k; l <= posX + k && flag; ++l)
            for (int i1 = posZ - k; i1 <= posZ + k && flag; ++i1)
                if (j < 0 || j >= W_Y
                        || !wg_isReplaceableTree(w, l, j, i1))
                    flag = 0;
    }
    if (!flag || !pb_canSustainSapling(
            w_get(w, posX, posY - 1, posZ))
            || posY >= W_Y - i - 1)
        return 0;
    wg_onPlantGrow(w, posX, posY - 1, posZ);
    for (int i3 = posY - 3 + i; i3 <= posY + i; ++i3) {
        int i4 = i3 - (posY + i), j1 = 1 - i4 / 2;
        for (int k1 = posX - j1; k1 <= posX + j1; ++k1) {
            ...
            for (int i2 = posZ - j1; i2 <= posZ + j1; ++i2) {
                ...
                int al1 = l1 < 0 ? -l1 : l1;
                int aj2 = j2 < 0 ? -j2 : j2;
                int place = al1 != j1 || aj2 != j1;
                if (!place)
                    place = jrand_int_bound(r, 2) != 0 && i4 != 0;
                if (place) {
                    int current = w_get(w, k1, i3, i2);
                    if (pb_isAir(current) || pb_isLeaves(current)
                            || pb_isMaterialVine(current))
    ...
    for (int j3 = 0; j3 < i; ++j3) {
        int current = w_get(w, posX, posY + j3, posZ);
        if (pb_isAir(current) || pb_isLeaves(current)
                || pb_isMaterialVine(current))
...
/* A sapling constructs a fresh WorldGenBigTree and therefore retains the
 * default leafDistanceLimit=4. Decoration calls setDecorationDefaults(), which
 * is why wg_bigtree itself correctly uses five instead. */
static int sapling_big_oak(
        World *w, JavaRandom *mainr, int x, int y, int z,
        FoliageCoord *fol) {
    const int leafDistanceLimit = 4;
    JavaRandom random;
    jrand_set(&random, jrand_long(mainr));
    int heightLimit = 5 + jrand_int_bound(&random, 12);
    if (!pb_canSustainSapling(w_get(w, x, y - 1, z))) return 0;
    int obstruction = bt_checkBlockLine(
        w, x, y, z, x, y + heightLimit - 1, z);
    if (obstruction != -1) {
    ...
    int perLayer = (int)(1.382
        + pow((double)heightLimit / 13.0, 2.0));
    if (perLayer < 1) perLayer = 1;
    int trunkTop = y + height;
    int layer = heightLimit - leafDistanceLimit;
    int count = 0;
    fol[count].x = x;
    fol[count].y = y + layer;
    fol[count].z = z;
    fol[count].branchBase = trunkTop;
    ++count;
    for (; layer >= 0; --layer) {
        float size = bt_layerSize(heightLimit, layer);
        if (size < 0.0F) continue;
        for (int attempt = 0; attempt < perLayer; ++attempt) {
            double distance = (double)size
                * ((double)jrand_float(&random) + 0.328);
            double angle = (double)(jrand_float(&random) * 2.0F) * MC_PI;
            int bx = x + mc_floor(distance * sin(angle) + 0.5);
            int by = y + layer - 1;
            int bz = z + mc_floor(distance * cos(angle) + 0.5);
            if (bt_checkBlockLine(
                    w, bx, by, bz, bx, by + leafDistanceLimit, bz) != -1)
                continue;
            int dx = x - bx, dz = z - bz;
            double branch = (double)by
                - sqrt((double)(dx * dx + dz * dz)) * 0.381;
            int branchY = branch > (double)trunkTop
                ? trunkTop : (int)branch;
            if (bt_checkBlockLine(
                    w, x, branchY, z, bx, by, bz) == -1
                    && count < BT_MAX_FOLIAGE) {
                fol[count].x = bx;
                fol[count].y = by;
                fol[count].z = bz;
                fol[count].branchBase = branchY;
                ++count;
            }
        }
    }
    for (int n = 0; n < count; ++n)
        for (int dy = 0; dy < leafDistanceLimit; ++dy)
            bt_crosSection(
                w, fol[n].x, fol[n].y + dy, fol[n].z,
                bt_leafSize(leafDistanceLimit, dy), PB_LEAVES_OAK);
    bt_limb(w, x, y, z, x, y + height, z);
    for (int n = 0; n < count; ++n) {
        int branchY = fol[n].branchBase;
        if (!(x == fol[n].x && branchY == fol[n].y
                && z == fol[n].z)
                && (double)(branchY - y) >= (double)heightLimit * 0.2)
            bt_limb(w, x, branchY, z,
                    fol[n].x, fol[n].y, fol[n].z);
    }
    return 1;
}

static int sapling_type_at(int lx, int y, int lz, int type) {
    u16 state;
    if (!w_inb(lx, y, lz)) return 0;
    state = g_sap_before[w_index(lx, y, lz)];
    return (state >> 4) == 6 && (state & 7) == type;
}

int popmc_generate_sapling(
        PopmcSaplingGetState get_state, PopmcSaplingSetState set_state,
        void *opaque, uint64_t *seed48, int x, int y, int z, int type,
        uint16_t source_state) {
    const int center = 16;
    int logMarker, leafMarker, anchorX = center, anchorZ = center;
    int twoByTwo = 0, generated = 0, changes = 0;
    int baseX = x - center, baseZ = z - center;
    World world;
    JavaRandom random;
    u16 sourceState;
    if (!get_state || !set_state || !seed48 || type < 0 || type > 5
            || y < 1 || y >= W_Y || !sapling_scratch_init())
        return -1;
    logMarker = type == 0 ? PB_LOG_SPRUCE : PB_LOG_OAK;
    leafMarker = type == 0 ? PB_LEAVES_SPRUCE : PB_LEAVES_OAK;
    for (int lx = 0; lx < W_X; ++lx)
        for (int lz = 0; lz < W_Z; ++lz)
            for (int by = 0; by < W_Y; ++by) {
                int index = w_index(lx, by, lz);
                u16 state = get_state(
                    opaque, baseX + lx, by, baseZ + lz);
                g_sap_before[index] = state;
                g_sap_leaf_flags[index] =
                    ((state >> 4) == 18 || (state >> 4) == 161)
                        && (state & 8) ? SAP_LEAF_DECAY : 0;
                g_sap_blocks[index] = (u16)sapling_pb_from_state(
                    state, logMarker, leafMarker);
            }
    sourceState = source_state;
    u16 liveSource = g_sap_before[w_index(center, y, center)];
    if ((sourceState >> 4) != 6 || (sourceState & 7) != type
            || (liveSource != sourceState && liveSource != 0))
        return -1;
    memset(&world, 0, sizeof world);
    world.blocks = g_sap_blocks;
    world.st = g_sap_st;
    jrand_set_seed48(&random, *seed48);

    /* BlockSapling constructs this oak candidate before its type switch, so
     * every stage-one type consumes nextInt(10), even dark-oak failure. */
    int bigOak = jrand_int_bound(&random, 10) == 0;
    if (type == 1 || type == 3 || type == 5) {
        for (int ox = 0; ox >= -1 && !twoByTwo; --ox)
            for (int oz = 0; oz >= -1; --oz)
                if (sapling_type_at(center + ox, y, center + oz, type)
                        && sapling_type_at(
                            center + ox + 1, y, center + oz, type)
                        && sapling_type_at(
                            center + ox, y, center + oz + 1, type)
                        && sapling_type_at(
                            center + ox + 1, y, center + oz + 1, type)) {
                    anchorX = center + ox;
                    anchorZ = center + oz;
                    twoByTwo = 1;
                    break;
    ...
    g_w_before_write = sapling_before_write;
    if (twoByTwo) {
        for (int dx = 0; dx <= 1; ++dx)
            for (int dz = 0; dz <= 1; ++dz)
                w_set(&world, anchorX + dx, y, anchorZ + dz, PB_AIR);
    } else {
        w_set(&world, center, y, center, PB_AIR);
    }
    ...
    case 0:
        generated = bigOak
            ? sapling_big_oak(
                &world, &random, center, y, center, g_sap_foliage)
            : wg_trees(
                &world, &random, center, y, center,
                PB_LOG_OAK, PB_LEAVES_OAK);
        break;
    case 1:
        if (twoByTwo) {
            int useBaseHeight = jrand_next(&random, 1) != 0;
            generated = wg_mega_pine(
                &world, &random, anchorX, y, anchorZ, useBaseHeight);
        } else {
            generated = wg_taiga2(&world, &random, center, y, center);
        }
        break;
    case 2:
        generated = wg_birch_ex(&world, &random, center, y, center, 0);
        break;
    case 3:
        if (twoByTwo)
            generated = wg_mega_jungle(
                &world, &random, anchorX, y, anchorZ);
        else {
            int minHeight = 4 + jrand_int_bound(&random, 7);
            generated = sapling_jungle_tree(
                &world, &random, center, y, center, minHeight);
        }
        break;
    case 4:
        generated = wg_savannatree(
            &world, &random, center, y, center);
        break;
    case 5:
        generated = pls_wg_canopy(
            &world, &random, anchorX, y, anchorZ);
        break;
    }
    if (!generated) {
        int restored = SAP_RAW_STATE_FLAG | sourceState;
        if (twoByTwo) {
            for (int dx = 0; dx <= 1; ++dx)
                for (int dz = 0; dz <= 1; ++dz)
                    w_set(&world, anchorX + dx, y, anchorZ + dz, restored);
        } else {
            w_set(&world, center, y, center, restored);
        }
    }
    g_w_before_write = 0;
    *seed48 = random.seed;
    for (int lx = 0; lx < W_X; ++lx)
        for (int lz = 0; lz < W_Z; ++lz)
            for (int by = 0; by < W_Y; ++by) {
                int index = w_index(lx, by, lz);
                int beforePb = sapling_pb_from_state(
                    g_sap_before[index], logMarker, leafMarker);
                u16 finalState;
                if (pb_isLeaves(g_sap_blocks[index])
                        && !(g_sap_leaf_flags[index] & SAP_LEAF_WRITTEN)) {
                    finalState = g_sap_before[index];
                } else {
                    if (g_sap_blocks[index] == beforePb
                            && !pb_isLeaves(g_sap_blocks[index]))
                        continue;
                    if (!sapling_state_from_pb(
                            g_sap_blocks[index], &finalState))
                        return -1;
                }
                if (pb_isLeaves(g_sap_blocks[index])
                        && (g_sap_leaf_flags[index] & SAP_LEAF_DECAY))
                    finalState |= 8;
                if (finalState == g_sap_before[index])
                    continue;
                set_state(opaque, baseX + lx, by, baseZ + lz, finalState);
                ++changes;
            }
    return changes;
}
