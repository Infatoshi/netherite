 ...
 * (B) STICK (280): emits 36 verts (generated plate); all UV inside stick sprite;
 *     geometry spans a non-zero AABB in eye space; swing moves verts.
 * (C) WOOD SHOVEL (269): emits 36 verts; UV inside wood_shovel sprite.
...
int main(void) {
    ...
    /* ---- (A) empty ---- */
    {
        int n = gm_hand_emit_held(0, 0, 0.0f, 0.0f, out, 64);
    ...
    /* ---- (B) stick ---- */
    {
        ...
        int n = gm_hand_emit_held(280, 0, 0.0f, 0.0f, out, 64);
        CHECK(n == 36, "stick emits 36 verts");
        ...
        CrVertex out2[64];
        int n2 = gm_hand_emit_held(280, 0, 0.5f, 0.0f, out2, 64);
        CHECK(n2 == 36, "stick swing emits 36 verts");
        float d = 0.0f;
        for (int i = 0; i < n; ++i) {
    ...
    /* ---- (C) wood shovel ---- */
    {
        int n = gm_hand_emit_held(269, 0, 0.0f, 0.0f, out, 64);
        CHECK(n == 36, "wood shovel emits 36 verts");
    ...
    /* ---- (D) dirt block ---- */
    {
        ...
        int n = gm_hand_emit_held(3, 0, 0.0f, 0.0f, out, 64);
