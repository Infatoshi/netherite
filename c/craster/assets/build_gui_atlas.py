...
Same zipfile pattern as build_hud_atlas.py. Pulls from the client jar:
  - the three 176x166 container panels (inventory / crafting_table / furnace)
...
(item id -> sprite index) table GUI_ITEM_ICONS. Dumps /tmp/craster_gui_*.png.
"""
import io
import os
import zipfile
...
def main():
    ...
    with zipfile.ZipFile(JAR) as zf:
        ...
        def add(sym, img, dump=True):
            ...
            if dump:
                img.save("/tmp/craster_gui_%s.png" % sym.lower())
    ...
    with open(OUT_H, "w") as f:
        ...
        f.write("#ifndef CRASTER_GUI_ATLAS_H\n#define CRASTER_GUI_ATLAS_H\n\n")
        f.write("#define GUI_SPRITE_COUNT %d\n" % len(table))
        f.write("#define GUI_ITEM_ICON_COUNT %d\n\n" % len(icons))
        for i, (sym, _, _, _) in enumerate(table[:len(SPRITES)]):
        ...
        f.write("};\n\n")
        f.write("#endif /* CRASTER_GUI_ATLAS_H */\n")
