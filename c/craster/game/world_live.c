...
/* CRASTER_DEBUG_CAPS: worst-case single-chunk mesh maxima, for fixed-pool sizing. */
...
static WlSlot *wl_ensure_mesh(GmWorld *w, int cx, int cz) {
    ...
    for (int l = 0; l < 4; ++l) {
        ...
        if (off + nl > w->caps->max_verts_per_chunk) {
            fprintf(stderr,
                ...
                "(raise max_verts_per_chunk in craster.conf / caps.h)\n",
    ...
    if (g_caps_on < 0) g_caps_on = getenv("CRASTER_DEBUG_CAPS") != NULL;
    if (g_caps_on) {
        int tot = 0;
        for (int l = 0; l < 4; ++l) {
...
void gm_world_mesh_view(GmWorld *w, const CrCamera *cam, int fb_w, int fb_h,
     ...
     * lock when the env is unset). CRASTER_VIEW_RADIUS lowers it at runtime for smooth
     * interactive FPS (fewer chunks meshed/rasterized); clamped to [1, SCN_VIEW_RADIUS]. */
    int R = SCN_VIEW_RADIUS;
    { const char *e = getenv("CRASTER_VIEW_RADIUS");
    ...
    /* CRASTER_DEBUG_VERTS: prove far-chunk decoration reaches the mesh - tally the
    ...
    const int dbg_verts = getenv("CRASTER_DEBUG_VERTS") != NULL;
