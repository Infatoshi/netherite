...
def gui_slot_id(gui, index, slot_count=None):
    ...
    if gui == "GuiInventory":
        ...
        # ContainerChest large: 0..53 chest, 54..80 main, 81..89 hotbar.
        if slot_count == 90:
            if 0 <= index <= 26:
                return 53 + index
            if 27 <= index <= 53:
    ...
    with open(snapshot_patch) as sf:
        for line in sf:
            ...
            if event_type not in (
                    "snapshot_block", "set_spawner_state",
                    "add_spawner_potential"):
                continue
            ...
            for tick, target_dim, cx, cz, radius in arrivals:
                if (dimension == target_dim and abs(bx - cx) <= radius
                    ...
                    identity = json.dumps(event, sort_keys=True,
                                          separators=(",", ":"))
                    if identity in seen[tick]:
                        continue
                    seen[tick].add(identity)
...
def tape_to_script(header, ticks, script_path, tape_path=None,
    ...
    with open(script_path, "w") as f:
            ...
            with open(snapshot_patch) as sf:
                for line in sf:
                    if line.strip():
                        ...
                        # Position-keyed TileEntities are installed by the
                        # arrival pass after set_dimension. Applying every
                        # dimension's spawners here would either target the
                        # wrong world or reject before the tape enters it.
                        if (event.get("type") not in (
                                "set_spawner_state",
                                "add_spawner_potential")
                                and not post_capture_spread(event)):
        ...
        for row_index, row in enumerate(ticks):
            ...
            # Explosion-class constructors. Legacy seven-field rows retain the
            # spawn-call approximation. V1 rows append the returned Java
            # Particle's constructor state, including private LARGE/HUGE clocks,
            # and therefore bypass all native visual RNG.
            for particle in row.get("pcl", []):
                if len(particle) not in (7, 8):
                    raise ValueError("pcl entry must have 7 legacy fields or V1 state")
                particle_id, x, y, z, vx, vy, vz = particle[:7]
                if len(particle) == 7:
                    event = {"tick": t, "type": "spawn_particle",
                             "id": int(particle_id),
                             "x": x, "y": y, "z": z,
                             "vx": vx, "vy": vy, "vz": vz}
                else:
                    ...
                    if not isinstance(state, dict) or state.get("v") != 1:
                        raise ValueError("unsupported pcl constructor-state version")
                    prev = state["prev"]
                    pos = state["pos"]
                    motion = state["motion"]
                    rgba = state["rgba"]
                    if int(particle_id) == 0:
                        ...
                        scale = state["scale"]
                    elif int(particle_id) == 1:
                        age = state["life"]
                        max_age = state["life_time"]
                        scale = state["size"]
                    elif int(particle_id) == 2:
                        age = state["time"]
                        max_age = 8
                        scale = state["scale"]
                    elif int(particle_id) in (11, 15, 34, 48):
                        age = state["age"]
                        max_age = state["max_age"]
                        scale = state["scale"]
                    else:
                        raise ValueError("pcl constructor state has invalid id")
                    event = {
                        "tick": t, "type": "spawn_particle_state",
                        "id": int(particle_id),
                        "prev_x": prev[0], "prev_y": prev[1], "prev_z": prev[2],
                        "x": pos[0], "y": pos[1], "z": pos[2],
                        "vx": motion[0], "vy": motion[1], "vz": motion[2],
                        "age": int(age), "max_age": int(max_age),
                        "on_ground": 1 if state["ground"] else 0,
                        "scale": scale,
                        "color_r": rgba[0], "color_g": rgba[1],
                        "color_b": rgba[2],
                        "tex": int(state["tex"][1]) * 16
                               + int(state["tex"][0]),
                        "tex_base": int(state.get("base_tex", 0)),
                    }
                f.write(json.dumps(event) + "\n")
            if "ppos" in row:
            ...
            # render-side; post-2026-07-12 rows append the exact vanilla pose,
                    # flags (including Entity.isGlowing), sheep state, or
                    # EntityItem stack/bob phase. Old 7-field
            ...
            for e in row.get("ents", []):
                ...
                if e[1] == "EntityWitherSkull" and len(e) >= 10:
                    view.update(pitch=e[7], wither_skull_invulnerable=int(e[8]),
                                ticks_existed=int(e[9]))
                elif e[1] == "EntityLlamaSpit" and len(e) >= 8:
                    view["pitch"] = e[7]
                elif "Arrow" in e[1] and len(e) >= 8:
                    ...
                    if e[1] == "EntityArmorStand" and len(e) >= 41:
                        view.update(
                            stand_flags=int(e[14]),
                            stand_mainhand=int(e[15]),
                            stand_offhand=int(e[16]),
                            armor_feet=int(e[17]),
                            armor_legs=int(e[18]),
                            armor_chest=int(e[19]),
                            armor_head=int(e[20]),
                            ticks_existed=int(e[21]),
                            stand_punch_time=e[22],
                            stand_pose_valid=1,
                        )
                        for part in range(6):
                            for axis, name in enumerate(("x", "y", "z")):
                                view[f"stand_pose_{part}_{name}"] = (
                                    e[23 + part * 3 + axis])
                        if len(e) >= 45:
                            view["armor_color_valid"] = 15
                            for slot in range(4):
                                view[f"armor_color_{slot}"] = int(e[41 + slot])
                        if len(e) >= 51:
                            view.update(
                                stand_mainhand_meta=int(e[45]),
                                stand_offhand_meta=int(e[46]),
                                armor_feet_meta=int(e[47]),
                                armor_legs_meta=int(e[48]),
                                armor_chest_meta=int(e[49]),
                                armor_head_meta=int(e[50]),
                            )
                    elif e[1] == "EntitySheep" and len(e) >= 17:
                        view.update(sheared=e[14], fleece=e[15], graze_y=e[16])
                        if len(e) >= 18:
                            view["graze_x"] = e[17]
                    elif e[1] == "EntityLlama" and len(e) >= 17:
                        view.update(item=int(e[14]),
                                    item_meta=int(e[15]) + 1,
                                    count=int(e[16]))
                    elif e[1] == "EntityBat" and len(e) >= 15:
                        view["ticks_existed"] = int(e[14])
                    elif e[1] == "EntityWither" and len(e) >= 20:
                        view.update(
                            wither_invul=int(e[14]),
                            ticks_existed=int(e[15]),
                            wither_head_yaw_0=e[16],
                            wither_head_pitch_0=e[17],
                            wither_head_yaw_1=e[18],
                            wither_head_pitch_1=e[19])
            ...
            if "gui" in row:
                ...
                gui = row["gui"]
                gui_slots = row.get("gslots", [])
                render_gui = "GuiLargeChest" \
                    if gui == "GuiChest" and len(gui_slots) == 90 else gui
                f.write(json.dumps({"tick": t, "type": "gui_view",
                                    "gui": render_gui, "mx": mx, "my": my})
                        + "\n")
                ...
                for index, stack in enumerate(gui_slots):
                    slot = gui_slot_id(gui, index, len(gui_slots))
                    if slot is not None:
                        f.write(json.dumps({"tick": t, "type": "gui_slot_view",
                                            "slot": slot, **tape_stack(stack)})
                                + "\n")
                if "gcur" in row and gui_slot_id(
                        gui, 0, len(gui_slots)) is not None:
                ...
                if gui == "GuiMerchant" and "gmrec" in row:
                    selected, offers, disabled, buy_a, buy_b, sell = \
                        row["gmrec"]
                    a = tape_stack(buy_a)
                    b = tape_stack(buy_b)
                    s = tape_stack(sell)
                    f.write(json.dumps({
                        "tick": t, "type": "gui_merchant_view",
                        "selected": int(selected), "offers": int(offers),
                        "disabled": int(disabled),
                        "a_item": a["item"], "a_count": a["count"],
                        "a_meta": a["meta"],
                        "b_item": b["item"], "b_count": b["count"],
                        "b_meta": b["meta"],
                        "sell_item": s["item"], "sell_count": s["count"],
                        "sell_meta": s["meta"],
                    }) + "\n")
...
def main():
    ...
    private_version = int(header.get("recorder_private_v", 0))
    if private_version > 1:
        raise SystemExit(f"[tape] unsupported recorder_private_v={private_version}")
    if "render_partial_ticks" in header \
            and float(header["render_partial_ticks"]) != 1.0:
        raise SystemExit("[tape] native replay currently requires "
                         "render_partial_ticks=1.0")
    for row in ticks:
        if "frame" in row and "render_partial" in row \
                and float(row["render_partial"]) != 1.0:
            raise SystemExit(
                f"[tape] tick {row['t']} has unsupported render_partial")
    ...
    if frame_ticks and not os.path.exists(frame_ticks[0][1]):
        ...
        if os.path.isdir(fdir):
            ...
            print("[tape] golden frames unavailable; continuing state-only "
                  "because --no-gate was requested")
            frame_ticks = []
        else:
    ...
    try:
        ...
        replay_set = []
        replay_env = {}
        snapshot_cache = (Path(args.tape).resolve().with_suffix(
            Path(args.tape).suffix + ".snapshot_patch.jsonl"))
        snapshot_payloads = snapshot_cache.with_suffix(
            snapshot_cache.suffix + ".payloads")
        if snapshot_payloads.is_dir():
            replay_env["MAGMA_CAPSULE_DIR"] = str(snapshot_payloads)
