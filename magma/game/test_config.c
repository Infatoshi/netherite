...
int main(void) {
          ...
          "ticks with legacy frames rejected");
    {
        ...
        CHECK(parse(&c, 4, modes, err) == 0
              && c.game_mode == GM_MODE_SURVIVAL,
              "survival launch profile is accepted");
    }
    for (int mode = 1; mode <= 3; ++mode) {
        char value[2] = {(char)('0' + mode), 0};
        char *mode_argv[] = {"game", "--gamemode", value};
        CHECK(parse(&c, 3, mode_argv, err) == 0
              && c.game_mode == (GmGameMode)mode,
              "numeric creative/adventure/spectator profile is accepted");
    }
    {
        char *bad_mode[] = {"game", "--gamemode", "builder"};
        CHECK(parse(&c, 3, bad_mode, err) == 2
              && strstr(err, "invalid gamemode"),
              "unknown game mode is rejected");
    }
