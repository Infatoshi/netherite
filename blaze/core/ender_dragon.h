...
/* BiomeEndDecorator.SpikeCacheLoader. Centers are the exact integer casts of
 * the ten Math.cos/Math.sin circle positions. Radius, height, and cages come
 * from the world-seed-derived 16-bit cache key and Collections.shuffle. */
MC_HD static inline void ed_pillar_spec(
        u64 world_seed, int idx, int *cx, int *cz,
        int *radius, int *height, int *guarded) {
    static const int px[ED_NUM_CRYSTALS] = {
        42, 33, 12, -12, -33, -42, -33, -12, 12, 33
    };
    static const int pz[ED_NUM_CRYSTALS] = {
        0, 24, 39, 39, 24, 0, -24, -39, -39, -24
    };
    int order[ED_NUM_CRYSTALS];
    JavaRandom seed_random, shuffle_random;
    jrand_set(&seed_random, (i64)world_seed);
    i64 key = jrand_long(&seed_random) & 65535;
    jrand_set(&shuffle_random, key);
    for (int i = 0; i < ED_NUM_CRYSTALS; ++i) order[i] = i;
    for (int i = ED_NUM_CRYSTALS; i > 1; --i) {
        int at = jrand_int_bound(&shuffle_random, i);
        int value = order[i - 1];
        order[i - 1] = order[at];
        order[at] = value;
    }
    ...
    if (cx) *cx = px[idx];
    if (cz) *cz = pz[idx];
    if (radius) *radius = 2 + rank / 3;
    if (height) *height = 76 + rank * 3;
    if (guarded) *guarded = rank == 1 || rank == 2;
}

MC_HD static inline void ed_pillar_crystal_pos(
        u64 world_seed, int idx, double *cx, double *cy, double *cz) {
    int x, z, height;
    ed_pillar_spec(world_seed, idx, &x, &z, NULL, &height, NULL);
    *cx = (double)x + 0.5;
    *cy = (double)(height + 1);
    *cz = (double)z + 0.5;
}
...
MC_HD static inline void ed_init(EdArena *a, u64 seed) {
    ...
    for (int i = 0; i < ED_NUM_CRYSTALS; ++i) {
        ed_pillar_crystal_pos(
            seed, i, &a->crystals[i].x, &a->crystals[i].y,
            &a->crystals[i].z);
        a->crystals[i].alive = 1;
        a->crystals[i].invulnerable = 0;
        a->crystals[i].has_beam = 0;
        a->crystals[i].beam_x = 0;
        a->crystals[i].beam_y = 0;
        a->crystals[i].beam_z = 0;
    }
