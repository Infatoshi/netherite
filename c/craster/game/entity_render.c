 ...
 * the current tick's values, so craster rebuilds the ring here: one push per
...
/* ---- geometry-oracle dump (CRASTER_GEOM_DUMP=path) ----------------------
...
static void geom_log(const char *lbl, float rpx, float rpy, float rpz,
    ...
    if (!er_geom_checked) {
        er_geom_checked = 1;
        const char *p = getenv("CRASTER_GEOM_DUMP");
 ...
 * TAKEOFF(4) divide by the distance to the exit-podium top; craster has no
...
static int emit_dragon(const GmEntityView *ent, CrVertex *out, int cap) {
     ...
     * alpha > deathTicks/200 (alpha-test pass + depth EQUAL repaint). craster
