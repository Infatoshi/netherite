/* game_candidate - render the seed-0 view-distance world through craster at an
 ...
 *
 * Convention: yaw/pitch here are CRASTER convention (core/math.c) in DEGREES.
 ...
 * this convention via craster_yaw = 180 - MC_yaw, craster_pitch = -MC_pitch, so a
 ...
 *
 * Default: single winding (matches game_main). Opt-in dual via CRASTER_DUAL_WIND=1
...
#include "game/sky.h"
#include "raster/verify/mc_capture/pose_scene.h"
...
static void render_layer(CrFramebuffer *fb, const PoseScene *s, int W, int H,
     ...
     * game_main never dual-winds. Default OFF; CRASTER_DUAL_WIND=1 restores the
     * old GL-parity "both windings" path. CRASTER_NO_DUAL_WIND=1 still forces off. */
    int dual = 0;
    {
        const char *s = getenv("CRASTER_DUAL_WIND");
        if (s && atoi(s) != 0) dual = 1;
        s = getenv("CRASTER_NO_DUAL_WIND");
...
int main(int argc, char **argv) {
    ...
    if (!getenv("CRASTER_TEXEL_BIAS") && !getenv("CRASTER_TEXEL_BIAS_V"))
        setenv("CRASTER_TEXEL_BIAS_V", "-0.32", 1);
    ...
    /* MC-convention yaw/pitch that reproduces this craster pose (for the operator
     * / capture side): craster_yaw = 180 - MC_yaw, craster_pitch = -MC_pitch. */
    float mc_yaw   = 180.0f - yaw_deg;
    float mc_pitch = -pitch_deg;
    printf("POSE seed=%lld x=%.4f y=%.4f z=%.4f "
           "craster_yaw_deg=%.4f craster_pitch_deg=%.4f "
     ...
     * toward the same color before src-over blend. DEFAULT ON (CRASTER_FOG=0 off).
