...
enum { FEATURE_DESERT, FEATURE_JUNGLE, FEATURE_SWAMP, FEATURE_IGLOO };

static int find_start(i64 seed, CpScratch *scratch, int feature, int selected,
    ...
    for (int rx = -radius; rx <= radius; ++rx)
        for (int rz = -radius; rz <= radius; ++rz) {
            ...
            if (((feature == FEATURE_DESERT
                    ...
                    || (feature == FEATURE_IGLOO
                        && (biome == B_ICE_PLAINS || biome == B_COLD_TAIGA)))
...
int main(void) {
    ...
    int swamp_start_cx = 0, swamp_start_cz = 0;
    CHECK(scratch && primer && sin_table && blocks && meta, "scratch allocation");
    if (fail) return 1;
    mc_sin_table_init(sin_table);
    CHECK(find_start(seed, scratch, FEATURE_DESERT, 0, &start_cx, &start_cz),
    ...
    CHECK(find_start(seed, scratch, FEATURE_JUNGLE, 0, &start_cx, &start_cz),
    ...
    CHECK(find_start(seed, scratch, FEATURE_SWAMP, 0, &start_cx, &start_cz),
          "seed 0 swamp hut within searched regions");
    swamp_start_cx = start_cx;
    swamp_start_cz = start_cz;
    memset(meta, 0, 4 * 65536 * sizeof *meta);
    for (int dx = 0; dx < 2; ++dx)
        for (int dz = 0; dz < 2; ++dz) {
            int ci = dx * 2 + dz;
    ...
    {
        u16 *igloo_blocks = (u16 *)malloc(9 * 65536 * sizeof *igloo_blocks);
        u8 *igloo_meta = (u8 *)calloc(9 * 65536, sizeof *igloo_meta);
        PopmcIglooResident residents[2];
        int found_basement = 0, igloo_chests = 0, resident_count = 0;
        CHECK(igloo_blocks && igloo_meta, "igloo chunk scratch allocation");
        for (int selected = 0; selected < 16 && !found_basement; ++selected) {
            CHECK(find_start(seed, scratch, FEATURE_IGLOO, selected,
                             &start_cx, &start_cz),
                  "seed 0 igloo within searched regions");
            if (fail) break;
            for (int dx = -1; dx <= 1; ++dx)
                for (int dz = -1; dz <= 1; ++dz) {
                    ...
                    int cx = start_cx + dx, cz = start_cz + dz;
                    st_run_features(primer, scratch, sin_table, seed, cx, cz, -1);
                    memcpy(igloo_blocks + ci * 65536, primer->data,
                           sizeof primer->data);
                    popmc_decorate_chunk_meta(seed, cx, cz,
                        igloo_blocks + ci * 65536,
                        igloo_meta + ci * 65536);
                }
            resident_count = popmc_igloo_residents(
                seed, (start_cx - 1) * 16, (start_cz - 1) * 16,
                (start_cx + 2) * 16 - 1, (start_cz + 2) * 16 - 1,
                residents, 2);
            found_basement = resident_count == 2;
        }
        CHECK(found_basement, "natural seed-0 basement igloo found");
        if (found_basement) {
            int kinds = 0;
            for (int i = 0; i < resident_count; ++i)
                kinds |= 1 << residents[i].kind;
            CHECK(kinds == ((1 << POPMC_IGLOO_RESIDENT_VILLAGER)
                          | (1 << POPMC_IGLOO_RESIDENT_ZOMBIE_VILLAGER)),
                  "live igloo retains villager and zombie-villager sites");
            for (int dx = -1; dx <= 1; ++dx)
                for (int dz = -1; dz <= 1; ++dz) {
                    int ci = (dx + 1) * 3 + dz + 1;
                    int cx = start_cx + dx, cz = start_cz + dz;
                    for (int lx = 0; lx < 16; ++lx)
                        for (int lz = 0; lz < 16; ++lz)
                            for (int y = 0; y < 256; ++y) {
                                ...
                                int facing = -1;
                                if (popmc_igloo_chest_info(
                                            seed, cx * 16 + lx, y,
                                            cz * 16 + lz, NULL, &facing)) {
                                    CHECK(igloo_blocks[ci * 65536 + index]
                                              == PB_CHEST,
                                          "cached igloo chest is present in chunk");
                                    CHECK(igloo_meta[ci * 65536 + index] == facing,
                                          "igloo chest facing metadata retained");
                                    ++igloo_chests;
                                }
                            }
                }
            CHECK(igloo_chests >= 1,
                  "live basement igloo has a deferred-loot chest");
        }
        free(igloo_meta);
        free(igloo_blocks);
    }
    ...
    if (fail) return 1;
    printf("scattered_live: PASS desert, jungle, swamp, and igloo; "
           "swamp start=(%d,%d), igloo start=(%d,%d) facing=%d\n",
           swamp_start_cx, swamp_start_cz, start_cx, start_cz,
           owr_sd_facing(seed, start_cx, start_cz));
    return 0;
}
