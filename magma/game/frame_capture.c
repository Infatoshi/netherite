...
struct GmFrameCapture {
    ...
    GmEntityView *entity_views;
    int entity_views_cap;
    GmLeashRenderView *leash_views;
    int leash_views_cap;
    int *projectile_types;
    int projectile_types_cap;
    GmLightningView *lightning_views;
    int lightning_views_cap;
    CrVertex *beam_verts;
    int beam_verts_cap;
    CrVertex *wither_verts;
    int wither_verts_cap;
    CrVertex *structure_verts[2];
    size_t structure_verts_cap[2];
    ...
    GmMeshRun *runs;        /* 4 layer-major banks of section-run records */
    int *gsrc, *gcnt;       /* gather entry scratch (backend bounded) */
    ...
    int boss_fog_override;  /* -1 normal latch; 0/1 exact oracle frame state */
    float boss_frac_override; /* <0 live entity/BossInfo fraction */
    ...
    CrVertex *outline_verts;    /* host-only outline pass, never GPU-uploaded */
    CrRgba *outline_work;
    float *outline_depth;
    CrRgba *outline_overlay[2]; /* pipeline parity: final post-shader RGBA */
    int outline_active[2];
    int pend_outline_active, pend_outline_set;
    ...
    float fog_c1;
    int fog_c1_init;
    float boss_color_modifier;
    float boss_color_modifier_prev;
    char dir[1024];
};

static int frame_entity_views_reserve(GmFrameCapture *c, int needed) {
    GmEntityView *grown;
    int cap;
    if (!c || needed < 0) return 0;
    if (needed <= c->entity_views_cap) return 1;
    cap = c->entity_views_cap > 0 ? c->entity_views_cap : 512;
    while (cap < needed) {
        if (cap > INT_MAX / 2) return 0;
        cap *= 2;
    }
    grown = (GmEntityView *)realloc(
        c->entity_views, (size_t)cap * sizeof *grown);
    if (!grown) return 0;
    c->entity_views = grown;
    c->entity_views_cap = cap;
    return 1;
}

static int frame_leash_views_reserve(GmFrameCapture *c, int needed) {
    GmLeashRenderView *grown;
    int cap;
    if (!c || needed < 0) return 0;
    if (needed <= c->leash_views_cap) return 1;
    cap = c->leash_views_cap > 0 ? c->leash_views_cap : 128;
    while (cap < needed) {
        if (cap > INT_MAX / 2) return 0;
        cap *= 2;
    }
    grown = (GmLeashRenderView *)realloc(
        c->leash_views, (size_t)cap * sizeof *grown);
    if (!grown) return 0;
    c->leash_views = grown;
    c->leash_views_cap = cap;
    return 1;
}

static int frame_beam_verts_reserve(GmFrameCapture *c, int needed) {
    CrVertex *grown;
    int cap;
    if (!c || needed < 0) return 0;
    if (needed <= c->beam_verts_cap) return 1;
    cap = c->beam_verts_cap > 0 ? c->beam_verts_cap : 2048;
    while (cap < needed) {
        if (cap > INT_MAX / 2) return 0;
        cap *= 2;
    }
    grown = (CrVertex *)realloc(
        c->beam_verts, (size_t)cap * sizeof *grown);
    if (!grown) return 0;
    c->beam_verts = grown;
    c->beam_verts_cap = cap;
    return 1;
}

static int frame_projectile_types_reserve(GmFrameCapture *c, int needed) {
    int cap;
    int *grown;
    if (needed <= c->projectile_types_cap) return 1;
    cap = c->projectile_types_cap > 0 ? c->projectile_types_cap : 32;
    while (cap < needed) {
        if (cap > INT_MAX / 2) return 0;
        cap *= 2;
    }
    grown = (int *)realloc(c->projectile_types, (size_t)cap * sizeof *grown);
    if (!grown) return 0;
    c->projectile_types = grown;
    c->projectile_types_cap = cap;
    return 1;
}

static int frame_lightning_views_reserve(GmFrameCapture *c, int needed) {
    int cap;
    GmLightningView *grown;
    if (needed <= c->lightning_views_cap) return 1;
    cap = c->lightning_views_cap > 0 ? c->lightning_views_cap : 8;
    while (cap < needed) {
        if (cap > INT_MAX / 2) return 0;
        cap *= 2;
    }
    grown = (GmLightningView *)realloc(
        c->lightning_views, (size_t)cap * sizeof *grown);
    if (!grown) return 0;
    c->lightning_views = grown;
    c->lightning_views_cap = cap;
    return 1;
}

static int frame_wither_verts_reserve(GmFrameCapture *c, int needed) {
    int cap;
    CrVertex *grown;
    if (needed <= c->wither_verts_cap) return 1;
    cap = c->wither_verts_cap > 0 ? c->wither_verts_cap : 5184;
    while (cap < needed) {
        if (cap > INT_MAX / 2) return 0;
        cap *= 2;
    }
    grown = (CrVertex *)realloc(
        c->wither_verts, (size_t)cap * sizeof *grown);
    if (!grown) return 0;
    c->wither_verts = grown;
    c->wither_verts_cap = cap;
    return 1;
}
...
void gm_frame_capture_set_boss_fog(GmFrameCapture *c, int boss_fog) {
    if (c) c->boss_fog_override = boss_fog < 0 ? -1 : !!boss_fog;
}

void gm_frame_capture_set_boss_fraction(GmFrameCapture *c,
                                        float boss_fraction) {
    if (!c) return;
    c->boss_frac_override = boss_fraction < 0.0f ? -1.0f
        : boss_fraction > 1.0f ? 1.0f : boss_fraction;
}
...
void gm_frame_lightmap_fill_view(
    ...
    gm_frame_lightmap_fill_view_boss(
        st, world_time, rain_strength, thunder_strength, night_vision,
        0.0f, lut);
}

void gm_frame_lightmap_fill_view_boss(
        const McSinTable *st, long long world_time, float rain_strength,
        float thunder_strength, float night_vision,
        float boss_color_modifier, CrRgba lut[256]) {
    ...
    for (int sl = 0; sl < 16; ++sl)
        for (int bl = 0; bl < 16; ++bl)
            lut[sl * 16 + bl] =
                cr_lightmap_rgba8(cr_lightmap_rgb_boss_night_vision(
                    0, sl, bl, sun, 0.0f, 0.0f,
                    boss_color_modifier, night_vision));
}
...
static int frame_wither_boss_active(const GmRuntime *r) {
    for (int i = 0; i < r->withers_cap; ++i)
        if (r->withers[i].active && r->withers[i].dimension == r->dimension)
            return 1;
    for (int i = 0; i < r->nghost_views; ++i)
        if (r->ghost_views[i].type == GM_VIEW_WITHER)
            return 1;
    return 0;
}

static void frame_boss_color_step(GmFrameCapture *c, const GmRuntime *r) {
    c->boss_color_modifier_prev = c->boss_color_modifier;
    if (frame_wither_boss_active(r)) {
        c->boss_color_modifier += 0.05f;
        if (c->boss_color_modifier > 1.0f) c->boss_color_modifier = 1.0f;
    } else {
        c->boss_color_modifier -= 0.0125f;
        if (c->boss_color_modifier < 0.0f) c->boss_color_modifier = 0.0f;
    }
}
...
void gm_frame_entities_light(GmEntityView *ents, int n, GmWorld *world,
    ...
    gm_frame_entities_light_boss(ents,n,world,dimension,0.0f,lm);
}
...
void gm_frame_entities_light_boss(
    ...
    for(int i=0;i<n;++i){
        ...
        if(lm){
            ...
            CrLightmapRgb c3=cr_lightmap_rgb_boss_night_vision(
                dimension,sky,bl,cr_dimension_sun_brightness(dimension),
                0.f,0.f,boss_color_modifier,0.f);
            ents[i].lm_lit=2;
...
void gm_frame_prepare_minecarts(GmEntityView *ents, int n, GmWorld *world) {
    for (int i = 0; i < n; ++i) {
        ...
        if (type != GM_VIEW_MINECART_EMPTY &&
            ...
            type != GM_VIEW_MINECART_SPAWNER &&
            type != GM_VIEW_MINECART_COMMAND)
            continue;
...
static CrCamera camera_for(const GmPlayerView *v, int w, int h) {
    ...
    c.fov_deg = cr_cfg()->fov_setting
        * (v->fov_mult > 0.01f ? v->fov_mult : 1.0f);
    ...
    if (cr_cfg()->third_person_camera) {
        /* EntityRenderer.orientCamera third-person rear view. The source F5
         * camera remains aimed along the player's look vector and moves four
         * blocks backward. This is render-only: inverse-control checkpoints
         * and physics remain ordinary first-person game state. Vanilla also
         * ray-clips this distance; the unobstructed transform is the useful
         * baseline and prevents a first-person/F5 registration lie. */
        const float cp = cosf(c.pitch), sp = sinf(c.pitch);
        const CrVec3 forward = {
            -sinf(c.yaw) * cp, sp, -cosf(c.yaw) * cp
        };
        c.pos.x -= 4.0f * forward.x;
        c.pos.y -= 4.0f * forward.y;
        c.pos.z -= 4.0f * forward.z;
    }
    return c;
}
...
/* RenderLivingBase disables GL_CULL_FACE. Transform one model triangle at a
 * time and reverse only a culled winding, preserving the Java ModelBox draw
 * order while feeding the front-winding-only software rasterizer. */
static int frame_transform_entity_nocull(
        const CrVertex *verts, int nv, const CrCamera *cam,
        int w, int h, CrScreenTri *out, int max_out) {
    int count = 0;
    for (int i = 0; i + 2 < nv && count < max_out; i += 3) {
        int nt = cr_transform(
            verts + i, 3, NULL, 0, cam, w, h,
            out + count, max_out - count);
        if (nt == 0) {
            CrVertex reverse[3] = {verts[i], verts[i + 2], verts[i + 1]};
            nt = cr_transform(
                reverse, 3, NULL, 0, cam, w, h,
                out + count, max_out - count);
        }
        count += nt;
    }
...
static int frame_build_entity_outline(
        GmFrameCapture *c, const CrCamera *cam,
        const GmEntityView *ents, int n, int set) {
    CrFramebuffer mask;
    CrTexture atlas;
    CrShadeCtx shade = {0};
    int nv, nt;
    if (!c || !cam || !ents || n <= 0 || set < 0 || set > 1)
        return 0;
    nv = gm_entities_glowing_emit(
        ents, n, c->outline_verts, c->max_entity_verts);
    if (nv <= 0) return 0;
    mask = (CrFramebuffer){
        c->fb.w, c->fb.h, c->post_scratch, c->outline_depth};
    cr_fb_clear(&mask, (CrRgba){0, 0, 0, 0});
    nt = frame_transform_entity_nocull(
        c->outline_verts, nv, cam, mask.w, mask.h,
        c->tris, c->max_tris);
    if (nt <= 0) return 0;
    atlas = gm_entity_atlas();
    shade.atlas = &atlas;
    shade.alpha_test = 1;
    shade.alpha_ref = 0.1f;
    shade.layer = CR_LAYER_CUTOUT;
    shade.sample_mode = 1;
    shade.depth_d24 = 1;
    shade.subpixel_bits = 8;
    shade.depth_always = 1;
    cr_raster_cpu(&mask, c->tris, nt, &shade);
    nv = gm_entities_glowing_slime_gel_emit(
        ents, n, c->outline_verts, c->max_entity_verts);
    if (nv > 0) {
        nt = frame_transform_entity_nocull(
            c->outline_verts, nv, cam, mask.w, mask.h,
            c->tris, c->max_tris);
        if (nt > 0) {
            CrShadeCtx gel = shade;
            gel.layer = CR_LAYER_TRANSLUCENT;
            gel.blend = 6;
            cr_raster_cpu(&mask, c->tris, nt, &gel);
        }
    }
    gm_overlay_entity_outline_build(
        mask.w, mask.h, c->post_scratch, c->outline_work);
    memcpy(c->outline_overlay[set], c->outline_work,
           (size_t)mask.w * (size_t)mask.h * sizeof *c->outline_work);
    return 1;
}

static void render_structures(
        GmFrameCapture *c, GmRuntime *r,
        const CrCamera *cam, const CrTexture *atlas, CrRgba fog) {
    int set = c->ent_set;
    size_t required = gm_overlay_structure_vertices_required(
        r, cam);
    int nv;
    if (!required || required > (size_t)INT_MAX) return;
    if (required > c->structure_verts_cap[set]) {
        CrVertex *grown = realloc(
            c->structure_verts[set], required * sizeof *grown);
        if (!grown) return;
        c->structure_verts[set] = grown;
        c->structure_verts_cap[set] = required;
    }
    nv = gm_overlay_emit_structures(
        c->structure_verts[set], (int)c->structure_verts_cap[set], r,
        cam, c->fb.w, c->fb.h);
    if (nv > 0) {
        CrShadeCtx shade = {0};
        shade.atlas = atlas;
        ...
        shade.depth_d24 = 1;
        render_layer(c, cam, c->structure_verts[set], nv, &shade);
    }
}
...
CrRgba gm_frame_clear_color(
    ...
    if (dimension == -1) {
        float r = 0.20f * fog_c1, g = 0.03f * fog_c1, b = 0.03f * fog_c1;
        gm_void_blindness_rgb(&r, &g, &b, blindness_duration,
                              feet_y, void_fog_y_factor);
        r *= 1.0f - 0.3f * boss_color_modifier;
        g *= 1.0f - 0.4f * boss_color_modifier;
        b *= 1.0f - 0.4f * boss_color_modifier;
        ...
        float r = 0.09411765f * (1.0f - blend) * fog_c1;
        float g = 0.07529412f * (1.0f - blend) * fog_c1;
        float b = 0.09411765f * (1.0f - blend) * fog_c1;
        gm_void_blindness_rgb(&r, &g, &b, blindness_duration,
                              feet_y, void_fog_y_factor);
        r *= 1.0f - 0.3f * boss_color_modifier;
        g *= 1.0f - 0.4f * boss_color_modifier;
...
static void terrain_shades(const CrTexture *atlas, CrRgba fog, int dimension,
    ...
    gm_frame_world_fog_params(dimension, boss_fog, &enabled,
                              &fog_start, &fog_end);
    /* Both oracle launch profiles pin mipmapLevels:0, so terrain_mips stays
     * off by default. Video recovery may enable the ordinary client mip path;
     * the vanilla layer NAME only selects alpha handling, while the sampler
     * state follows mipmapLevels. */
...
#undef TSH
    for (int i = 0; i < 4; ++i) {
        shade[i] = s[i];
        shade[i].lightmap = lm;
        shade[i].use_mips = cr_cfg()->terrain_mips;
    }
...
static void render_world_dev_layers(GmFrameCapture *c, const CrCamera *cam,
                                    const int nruns[4], const int nv[4], int nch,
    ...
    int merged_entries = nruns[CR_LAYER_SOLID]
        + nruns[CR_LAYER_CUTOUT_MIPPED] + nruns[CR_LAYER_CUTOUT];
    if (c->merge_layers && first_layer == CR_LAYER_SOLID &&
        end_layer == CR_LAYER_TRANSLUCENT &&
        merged_entries <= GM_GATHER_MAX_ENTRIES) {
        ...
        for (int layer = first_layer; layer < end_layer; ++layer) {
            const GmMeshRun *bank = c->runs
                + (size_t)layer * (size_t)c->runs_stride;
            for (int i = 0; i < nruns[layer]; ++i) {
                c->gsrc[ne] = bank[i].slot * c->slab_cap + bank[i].off;
                c->gcnt[ne] = bank[i].n;
                ++ne;
            }
        }
    ...
    for (int layer = first_layer; layer < end_layer; ++layer) {
        ...
        if (nruns[layer] > 0 && nv[layer] >= 3)
            cr_raster_cuda_render_gather(&c->fb, c->gsrc, c->gcnt,
                                         nruns[layer],
...
GmFrameCapture *gm_frame_capture_open(const GmConfig *cfg, char *err, int err_cap) {
    ...
    c->boss_fog_override=-1;
    c->boss_frac_override=-1.0f;
    ...
    c->outline_verts=malloc(
        (size_t)c->max_entity_verts*sizeof *c->outline_verts);
    c->outline_work=malloc(
        (size_t)cfg->width*(size_t)cfg->height*sizeof *c->outline_work);
    c->outline_depth=malloc(
        (size_t)cfg->width*(size_t)cfg->height*sizeof *c->outline_depth);
    for(int i=0;i<2;++i)c->outline_overlay[i]=malloc(
        (size_t)cfg->width*(size_t)cfg->height*sizeof *c->outline_overlay[i]);
    if(!c->fb.color||!c->tris||!c->entity_verts[0]||!c->entity_verts[1]||
       !c->entity_verts[2]||!c->entity_verts[3]||!c->entity_verts[4]||
       !c->entity_verts[5]||!c->entity_verts[6]||!c->entity_verts[7]||
       !c->weather_verts[0]||
       !c->weather_verts[1]||!c->weather_verts[2]||!c->weather_verts[3]||
       !c->ppm_buf||!c->post_scratch||!c->outline_verts||
       !c->outline_work||!c->outline_depth||!c->outline_overlay[0]||
       !c->outline_overlay[1]){set_error(err,err_cap,"frame capture allocation failed");gm_frame_capture_close(c);return NULL;}
    if(npy){
    ...
    if(c->use_cuda){
        ...
        if(stride<=GM_GATHER_MAX_ENTRIES&&
           cr_raster_cuda_slab_pool&&cr_raster_cuda_slab_sync&&
           ...
           cr_raster_cuda_slab_pool(caps->mesh_slots,caps->max_verts_per_chunk)){
            ...
            c->gsrc=malloc((size_t)GM_GATHER_MAX_ENTRIES*sizeof *c->gsrc);
            c->gcnt=malloc((size_t)GM_GATHER_MAX_ENTRIES*sizeof *c->gcnt);
...
static int finish_pending(GmFrameCapture *c) {
    ...
    if (c->pend_outline_active)
        gm_overlay_entity_outline_composite(
            &pfb, c->outline_overlay[c->pend_outline_set]);
    ...
    {
        int nausea = gm_potion_view_duration(&c->pend_v, 9) > 0;
        float portal = c->pend_v.portal_prev
            + (c->pend_v.portal - c->pend_v.portal_prev);
        gm_overlay_portal_warp(
            &pfb, c->post_scratch, portal, c->pend_v.portal_phase,
            1.0F, nausea, c->pend_uwfov);
    }
    ...
    if (c->pend_v.portal > 0.0f && !hud_hidden()
            && gm_potion_view_duration(&c->pend_v, 9) <= 0) {
...
int gm_frame_capture_write(GmFrameCapture *c, GmRuntime *r,
    ...
    GmUnderwater uw;gm_uw_eval(
        r->world,r->dimension,&v,c->fog_c1,night_vision,
        c->boss_color_modifier,blindness,
    ...
    gm_sky_set_boss_color_modifier(c->boss_color_modifier);
    ...
    CrRgba clear=gm_frame_clear_color(
        day,r->dimension,c->fog_c1,night_vision,c->boss_color_modifier,
        blindness,(double)v.y,void_fog_y_factor,&uw);
    ...
    const CrRgba *lm=build_lightmap_lut(
        c,r,night_vision,c->boss_color_modifier);
     ...
     * rotation the RenderHelper item lights are anchored under. */
    {
        ...
        if(lm){
            ...
            CrLightmapRgb hc3=cr_lightmap_rgb_boss_night_vision(
                r->dimension,hsky,hblk,
                cr_dimension_sun_brightness(r->dimension),0.f,0.f,
                c->boss_color_modifier,night_vision);
    ...
    enum { FC_FIXED_ENTS = GM_RUNTIME_GHOST_VIEWS + 1 };
    int living_views = gm_mobs_living_count(&r->mobs);
    size_t fc_ents_needed = (size_t)gm_live_entity_slot_count(&r->entities)
        + (size_t)r->end_crystals_cap + (size_t)r->falling_blocks_cap
        + (size_t)r->fireworks_cap + (size_t)r->projectiles_cap
        + (size_t)r->withers_cap + (size_t)r->shulkers_cap
        + (size_t)r->armor_stands_cap + (size_t)living_views + FC_FIXED_ENTS;
    if (fc_ents_needed > (size_t)(INT_MAX / 96)
            || fc_ents_needed > INT_MAX
            || !frame_entity_views_reserve(c, (int)fc_ents_needed)
            || !frame_leash_views_reserve(c, living_views)
            || !frame_projectile_types_reserve(c, r->projectiles_cap)
            || !frame_lightning_views_reserve(c, r->lightning_cap)
            || r->withers_cap > INT_MAX / (18 * 36)
            || !frame_wither_verts_reserve(
                c, r->withers_cap * 18 * 36)
            || !frame_beam_verts_reserve(c, (int)fc_ents_needed * 96)) {
        set_error(err, err_cap, "frame entity-view allocation failed");
        return 0;
    }
    int fc_ents = (int)fc_ents_needed;
    GmEntityView *ents=c->entity_views;
    int n=gm_dragon_fill_views(&r->dragon,ents,fc_ents);
    n+=gm_runtime_end_crystal_views(r,ents+n,fc_ents-n);
    n+=gm_mobs_fill_views(&r->mobs,ents+n,fc_ents-n);
    n+=gm_runtime_shulker_views(r,ents+n,fc_ents-n);
    n+=gm_runtime_wither_views(r,ents+n,fc_ents-n);
    n+=gm_runtime_armor_stand_views(r,ents+n,fc_ents-n);
    int n_proj0=n;
    n+=gm_runtime_projectile_views(r,ents+n,fc_ents-n);
    /* Dense list of active projectile types in the same order as views. */
    {
        int *ptypes=c->projectile_types, npt=0;
    ...
    if(c->boss_fog_override>=0)c->boss_latch=c->boss_fog_override;
    ...
    {
        int wither_seen=0;
        float wither_frac=0.0f;
        for(int i=0;i<n;++i)if(ents[i].type==GM_VIEW_WITHER){
            wither_seen=1;
            wither_frac=ents[i].health/300.0f;
            break;
        }
        gm_hud_set_boss(
            !cr_cfg()->strip_overlays&&(wither_seen||c->boss_latch),
            c->boss_frac_override>=0.0f?c->boss_frac_override:
                (wither_seen?wither_frac:c->boss_frac),
            wither_seen?"Wither":"Ender Dragon");
    }
    GmMeshView mv;
    int dev_nruns[4] = {0,0,0,0}, dev_nv[4] = {0,0,0,0};
    int dev_nch = -1, dev_frame_mesh = 0;
    if(c->dev_mesh){
        ...
        dev_nch=gm_world_mesh_runs(r->world,&cam,c->fb.w,c->fb.h,
                                   ...
                                   dev_nruns,dev_nv,NULL,NULL);
        if(dev_nch>=0){
            dev_frame_mesh=1;
            render_world_dev_layers(c,&cam,dev_nruns,dev_nv,dev_nch,
                                    &atlas,clear,r->dimension,c->boss_latch,
                                    lm,&uw,CR_LAYER_SOLID,
    ...
    gm_frame_entities_light_boss(
        ents,n,r->world,r->dimension,c->boss_color_modifier,lm);
    GmPaintingView painting_views[GM_RUNTIME_PAINTINGS_MAX];
    GmLeashKnotView knot_views[GM_RUNTIME_LEASH_KNOTS_MAX];
    GmItemFrameRenderView frame_views[GM_RUNTIME_ITEM_FRAMES_MAX];
    GmLeashRenderView *leash_views = c->leash_views;
    int npaint=0,nknot=0,nframe=0,nleash=0;
    for(int i=0;i<gm_runtime_painting_count(r)&&npaint<GM_RUNTIME_PAINTINGS_MAX;++i){
        GmRuntimePainting p;if(!gm_runtime_painting_get(r,i,&p))continue;
        GmPaintingView *pv=&painting_views[npaint++];memset(pv,0,sizeof *pv);
        pv->x=(float)p.x;pv->y=(float)p.y;pv->z=(float)p.z;
        pv->facing=p.facing;pv->art=p.art;
        int cells[16][3],nc=gm_painting_light_cells(pv,cells,16);
        for(int j=0;j<nc;++j){
            pv->lm_light[j]=(float)gm_world_sky_light(
                r->world,cells[j][0],cells[j][1],cells[j][2]);
            pv->lm_blk[j]=(float)gm_world_block_light(
                r->world,cells[j][0],cells[j][1],cells[j][2]);
        }
    }
    for(int i=0;i<gm_runtime_leash_knot_count(r)&&nknot<GM_RUNTIME_LEASH_KNOTS_MAX;++i){
        GmRuntimeLeashKnot k;if(!gm_runtime_leash_knot_get(r,i,&k))continue;
        int lx=(int)floor(k.x),ly=(int)floor(k.y-.0625),lz=(int)floor(k.z);
        knot_views[nknot++]=(GmLeashKnotView){
            (float)k.x,(float)k.y,(float)k.z,
            (float)gm_world_sky_light(r->world,lx,ly,lz),
            (float)gm_world_block_light(r->world,lx,ly,lz)};
    }
    for(int i=0;i<gm_runtime_item_frame_count(r)&&nframe<GM_RUNTIME_ITEM_FRAMES_MAX;++i){
        const GmRuntimeItemFrame *f=gm_runtime_item_frame_ref(r,i);if(!f)continue;
        int lx=(int)floor(f->x),ly=(int)floor(f->y+.425),lz=(int)floor(f->z);
        frame_views[nframe++]=(GmItemFrameRenderView){
            (float)f->hanging_x+.5f,(float)f->hanging_y+.5f,
            (float)f->hanging_z+.5f,f->facing,f->item,f->meta,f->rotation,
            (float)gm_world_sky_light(r->world,lx,ly,lz),
            (float)gm_world_block_light(r->world,lx,ly,lz),
            f->map_colors_present?f->map_colors:NULL,
            f->map_decoration_present,f->map_decoration_type,
            f->map_decoration_x,f->map_decoration_z,
            f->map_decoration_rotation};
    }
    for(int i=0;i<n&&nleash<c->leash_views_cap;++i){
        GmLivingLeashState ls;
        if(!gm_mobs_get_living_leash_state(&r->mobs,ents[i].ent_id,&ls)
                ||!ls.leashed||ls.holder_kind<1||ls.holder_kind>3)
            continue;
        GmLeashRenderView q={0};
        q.x=ents[i].x;q.y=ents[i].y;q.z=ents[i].z;q.yaw=ents[i].yaw;
        gm_entity_size(ents[i].type,&q.width,&q.height);
        if(ls.holder_kind==1){
            q.holder_x=v.x;q.holder_y=v.y;q.holder_z=v.z;
            q.holder_yaw=v.yaw;q.holder_pitch=v.pitch;q.holder_eye=v.eye_height;
        }else if(ls.holder_kind==2){
            int holder=-1;for(int j=0;j<n;++j)if(ents[j].ent_id==ls.holder_eid){holder=j;break;}
            if(holder<0)continue;
            q.holder_x=ents[holder].x;q.holder_y=ents[holder].y;
            q.holder_z=ents[holder].z;q.holder_yaw=ents[holder].yaw;
            q.holder_pitch=ents[holder].pitch;
            q.holder_eye=gm_entity_eye_y(ents[holder].type);
        }else{
            q.holder_x=(float)ls.holder_x;q.holder_y=(float)ls.holder_y;
            q.holder_z=(float)ls.holder_z;q.holder_eye=-.0625f;
            q.holder_hanging=1;
        }
        leash_views[nleash++]=q;
    }
    int nchest=0;
    for(int i=0;i<r->chests_cap;++i){
        const GmRuntimeChest *tile=&r->chests[i];
        if(!tile->active)continue;
        int block=gm_world_block(r->world,tile->wx,tile->wy,tile->wz);
        if(block!=54&&block!=146)continue;
        if(gm_world_block(r->world,tile->wx-1,tile->wy,tile->wz)==block
                ||gm_world_block(r->world,tile->wx,tile->wy,tile->wz-1)==block)
            continue;
        ++nchest;
    }
    int nender=0;
    for(int i=0;i<r->ender_chests_cap;++i)
        if(r->ender_chests[i].active
                &&r->ender_chests[i].dimension==r->dimension
                &&gm_world_block(r->world,r->ender_chests[i].wx,
                                  r->ender_chests[i].wy,
                                  r->ender_chests[i].wz)==130)
            ++nender;
    int nshulker_box=0;
    for(int i=0;i<r->static_containers_cap;++i){
        const GmRuntimeStaticContainer *tile=&r->static_containers[i];
        if(tile->active&&tile->dimension==r->dimension
                &&tile->block>=219&&tile->block<=234
                &&gm_world_block(r->world,tile->wx,tile->wy,tile->wz)
                    ==tile->block)
            ++nshulker_box;
    }
    int nbeacon=0;
    for(int i=0;i<r->static_containers_cap;++i){
        const GmRuntimeStaticContainer *tile=&r->static_containers[i];
        if(tile->active&&tile->dimension==r->dimension&&tile->block==138
                &&gm_world_block(r->world,tile->wx,tile->wy,tile->wz)==138)
            ++nbeacon;
    }
    int nspawner=0;
    for(int i=0;i<r->mobs.spawners_cap;++i){
        const GmSpawnerTE *tile=&r->mobs.spawners[i];
        if(tile->active&&tile->dimension==r->dimension
                &&gm_world_block(r->world,tile->x,tile->y,tile->z)==52)
            ++nspawner;
    }
    if(n>0||npaint>0||nknot>0||nframe>0||nchest>0||nender>0
            ||nshulker_box>0||nbeacon>0||nspawner>0){
        ...
        nv+=gm_paintings_emit(painting_views,npaint,eb[0]+nv,
                              c->max_entity_verts-nv);
        nv+=gm_leash_knots_emit(knot_views,nknot,eb[0]+nv,
                                c->max_entity_verts-nv);
        for(int i=0;i<r->mobs.spawners_cap;++i){
            const GmSpawnerTE *tile=&r->mobs.spawners[i];
            if(!tile->active||tile->dimension!=r->dimension
                    ||gm_world_block(
                        r->world,tile->x,tile->y,tile->z)!=52)
                continue;
            GmSpawnerView spawner={
                tile->x,tile->y,tile->z,tile->entity_type,
                (float)tile->mob_rotation,1.0f};
            nv+=gm_spawner_miniatures_emit(
                &spawner,1,eb[0]+nv,c->max_entity_verts-nv);
        }
        for(int i=0;i<r->chests_cap;++i){
            const GmRuntimeChest *tile=&r->chests[i];
            if(!tile->active)continue;
            ...
            if(gm_world_block(r->world,tile->wx-1,tile->wy,tile->wz)==block
                    ||gm_world_block(r->world,tile->wx,tile->wy,tile->wz-1)==block)
                continue;
            int xpos=gm_world_block(
                r->world,tile->wx+1,tile->wy,tile->wz)==block;
            int zpos=gm_world_block(
                r->world,tile->wx,tile->wy,tile->wz+1)==block;
            int large=xpos||zpos;
            int trapped=block==146;
            GmChestRenderView chest={
                tile->wx,tile->wy,tile->wz,
                gm_world_meta(r->world,tile->wx,tile->wy,tile->wz),
                tile->state.te.prev_lid_angle,tile->state.te.lid_angle,
                (float)gm_world_sky_light(
                    r->world,tile->wx,tile->wy,tile->wz),
                (float)gm_world_block_light(
                    r->world,tile->wx,tile->wy,tile->wz),
                large
                    ?(trapped?GM_CHEST_TEXTURE_TRAPPED_DOUBLE
                              :GM_CHEST_TEXTURE_NORMAL_DOUBLE)
                    :(trapped?GM_CHEST_TEXTURE_TRAPPED
                              :GM_CHEST_TEXTURE_NORMAL),
                large,
                large&&xpos&&gm_world_meta(
                    r->world,tile->wx,tile->wy,tile->wz)==2?1:0,
                large&&zpos&&gm_world_meta(
                    r->world,tile->wx,tile->wy,tile->wz)==5?1:0};
            nv+=gm_chests_emit(
                &chest,1,1.0f,eb[0]+nv,c->max_entity_verts-nv);
        }
        for(int i=0;i<r->ender_chests_cap;++i){
            const GmRuntimeEnderChest *tile=&r->ender_chests[i];
            if(!tile->active||tile->dimension!=r->dimension
                    ||gm_world_block(r->world,tile->wx,tile->wy,tile->wz)!=130)
                continue;
            GmEnderChestRenderView chest={
                tile->wx,tile->wy,tile->wz,
                gm_world_meta(r->world,tile->wx,tile->wy,tile->wz),
                tile->prev_lid_angle,tile->lid_angle,
                (float)gm_world_sky_light(
                    r->world,tile->wx,tile->wy,tile->wz),
                (float)gm_world_block_light(
                    r->world,tile->wx,tile->wy,tile->wz)};
            nv+=gm_ender_chests_emit(
                &chest,1,1.0f,eb[0]+nv,c->max_entity_verts-nv);
        }
        for(int i=0;i<r->static_containers_cap;++i){
            const GmRuntimeStaticContainer *tile=&r->static_containers[i];
            if(!tile->active||tile->dimension!=r->dimension
                    ||tile->block<219||tile->block>234
                    ||gm_world_block(
                        r->world,tile->wx,tile->wy,tile->wz)!=tile->block)
                continue;
            GmShulkerBoxRenderView box={
                tile->wx,tile->wy,tile->wz,
                gm_world_meta(r->world,tile->wx,tile->wy,tile->wz)%6,
                tile->block-219,
                tile->shulker_progress_old,tile->shulker_progress,
                (float)gm_world_sky_light(
                    r->world,tile->wx,tile->wy,tile->wz),
                (float)gm_world_block_light(
                    r->world,tile->wx,tile->wy,tile->wz)};
            nv+=gm_shulker_boxes_emit(
                &box,1,1.0f,eb[0]+nv,c->max_entity_verts-nv);
        }
        ...
        CrShadeCtx sh={0};
        sh.atlas=&ea; sh.fog_color=clear; sh.alpha_test=1;sh.alpha_ref=0.1f;
        ...
        if(nleash>0){
            int nl=gm_living_leashes_emit(leash_views,nleash,eb[0]+nv,
                                          c->max_entity_verts-nv);
            if(nl>0){
                CrShadeCtx line={0};line.fog_color=clear;line.untextured=1;
        ...
        {
            CrVertex *wither_ov=c->wither_verts;
            int nw=gm_wither_aura_emit(
                ents,n,wither_ov,c->wither_verts_cap);
            if(nw>0){
                CrTexture wt=gm_wither_armor_texture();
                CrShadeCtx aura={0};
                aura.atlas=&wt;aura.fog_color=(CrRgba){0,0,0,255};
                aura.alpha_test=1;aura.alpha_ref=0.1f;
                aura.layer=CR_LAYER_TRANSLUCENT;aura.blend=5;
                aura.depth_lequal=1;aura.repeat_uv=1;aura.sample_mode=1;
                aura.lightmap=lm;
                aura.depth_d24=1;
                aura.depth_bias_d24=4;
                gm_frame_world_fog_params(
                    r->dimension,c->boss_latch,&aura.enable_fog,
                    &aura.fog_start,&aura.fog_end);
                gm_view_fog_apply(&aura,&uw,clear);
                render_layer(c,&cam,wither_ov,nw,&aura);
            }
        }
        {
        ...
        /* TileEntityBeaconRenderer runs after ordinary entities in render pass
         * zero. Keep core/glow in disjoint tails of the rotating entity buffer:
         * CUDA uploads may remain in flight until frame_end. */
        if(nbeacon>0&&nv<c->max_entity_verts-96){
            int available=c->max_entity_verts-nv;
            int each=available/2;
            CrVertex *inner=eb[0]+nv;
            CrVertex *glow=inner+each;
            int ni=0,ng=0;
            for(int i=0;i<r->static_containers_cap;++i){
                GmRuntimeStaticContainer *tile=&r->static_containers[i];
                GmBeaconBeamSegmentView segments[GM_BEACON_SEGMENTS_MAX];
                ...
                if(!tile->active||tile->dimension!=r->dimension
                        ||tile->block!=138
                        ||gm_world_block(
                            r->world,tile->wx,tile->wy,tile->wz)!=138)
                    continue;
                segment_count=tile->beacon_segment_count;
                if(segment_count<0)segment_count=0;
                if(segment_count>GM_BEACON_SEGMENTS_MAX)
                    segment_count=GM_BEACON_SEGMENTS_MAX;
                for(int s=0;s<segment_count;++s){
                    segments[s]=(GmBeaconBeamSegmentView){
                        tile->beacon_segments[s].red,
                        tile->beacon_segments[s].green,
                        tile->beacon_segments[s].blue,
                        tile->beacon_segments[s].height};
                }
                view=(GmBeaconRenderView){
                    tile->wx,tile->wy,tile->wz,
                    gm_runtime_beacon_should_render(
                        tile,r->clock.total_time),
                    (double)r->clock.total_time,
                    segment_count,segments};
                int one_inner=gm_beacon_beams_emit(
                    &view,1,1.0f,
                    inner+ni,each-ni,glow+ng,each-ng,&one_glow);
                ni+=one_inner;
                ng+=one_glow;
                if(each-ni<48||each-ng<48)break;
            }
            if(ni>0||ng>0){
                CrTexture bt=gm_beacon_beam_texture();
                if(ni>0){
                    CrShadeCtx beam={0};
                    ...
                    beam.depth_d24=1;
                    render_layer(c,&cam,inner,ni,&beam);
                }
                if(ng>0){
                    CrShadeCtx beam={0};
                    beam.atlas=&bt;beam.fog_color=clear;
                    beam.alpha_test=1;beam.alpha_ref=0.1f;
                    beam.layer=CR_LAYER_TRANSLUCENT;beam.blend=1;
                    beam.sample_mode=1;beam.repeat_uv=1;
                    beam.depth_d24=1;
                    render_layer(c,&cam,glow,ng,&beam);
                }
            }
        }
        ...
        {
            CrVertex *beam_ov=c->beam_verts;
            int nb=gm_crystal_beams_emit(
                ents,n,beam_ov,c->beam_verts_cap);
            if(nb>0){
            ...
            int ng=gm_guardian_beams_emit(
                ents,n,beam_ov,c->beam_verts_cap);
            if(ng>0){
                CrTexture gt=gm_guardian_beam_texture();
                CrShadeCtx beam={0};
                beam.atlas=&gt;beam.fog_color=clear;
                beam.alpha_test=1;beam.alpha_ref=0.1f;
                beam.layer=CR_LAYER_CUTOUT;
                beam.sample_mode=1;beam.repeat_uv=1;
                gm_view_fog_apply(&beam,&uw,clear);
                render_layer(c,&cam,beam_ov,ng,&beam);
            }
        }
        /* RenderXPOrb never enables blending. Its vertex alpha is carried to
         * the framebuffer but RGB replaces the destination after alpha test. */
        {
         ...
         * async CUDA uploads of eb[*] are not overwritten mid-flight. */
        {
            ...
            if(ng>0){
                ...
                /* LayerSlimeGel renders inside RenderLivingBase without
                 * changing the standard item lights or the entity lightmap.
                 * Leaving the lightmap detached made the 0..15 sky level act
                 * as a scalar and saturated the translucent shell to white. */
                gel.lightmap=lm; gel.entity_brightness=1;
                gel.sample_mode=1; gel.depth_d24=1; gel.subpixel_bits=8;
            ...
            int ngb=gm_guardian_beams_emit(
                ents,n,beam_ov,c->beam_verts_cap);
            if(ngb>0){
                CrTexture gt=gm_guardian_beam_texture();
                CrShadeCtx bm={0};
                bm.atlas=&gt; bm.fog_color=clear;
                bm.alpha_test=1; bm.alpha_ref=0.1f;
                bm.layer=CR_LAYER_CUTOUT;
                bm.sample_mode=1; bm.repeat_uv=1;
                gm_frame_world_fog_params(r->dimension,c->boss_latch,
                                          &bm.enable_fog,
                                          &bm.fog_start,&bm.fog_end);
                if(uw.fluid){ bm.enable_fog=1; bm.fog_exp_density=uw.density;
                              bm.fog_color=uw.fog_rgba; }
                render_layer(c,&cam,beam_ov,ngb,&bm);
            }
        }
        /* dropped items + falling blocks: block cubes/plants on the TERRAIN
         * atlas, then non-block items on the item atlas. */
        nv=gm_item_frames_emit(frame_views,nframe,eb[1],c->max_entity_verts);
        nv+=gm_item_frame_block_items_emit(frame_views,nframe,eb[1]+nv,
                                           c->max_entity_verts-nv);
        if(nv>0){
            CrShadeCtx frame_sh={0};frame_sh.atlas=&atlas;
            frame_sh.fog_color=clear;frame_sh.alpha_test=1;
            frame_sh.alpha_ref=.1f;frame_sh.layer=CR_LAYER_CUTOUT;
            frame_sh.lightmap=lm;frame_sh.entity_brightness=1;
            gm_view_fog_apply(&frame_sh,&uw,clear);
            render_layer(c,&cam,eb[1],nv,&frame_sh);
        }
        int item_start=nv;
        nv+=gm_items_emit(ents,n,eb[1]+nv,c->max_entity_verts-nv);
        nv+=gm_held_blocks_emit(ents,n,eb[1]+nv,c->max_entity_verts-nv);
        nv+=gm_mooshroom_mushrooms_emit(
            ents,n,eb[1]+nv,c->max_entity_verts-nv);
        nv+=gm_falling_blocks_emit(ents,n,eb[1]+nv,c->max_entity_verts-nv);
        nv+=gm_minecart_contents_emit(
            ents,n,1.0f,eb[1]+nv,c->max_entity_verts-nv);
        if(nv>item_start){
            CrShadeCtx ish={0};
            ish.atlas=&atlas; ish.fog_color=clear; ish.alpha_test=1;
            ish.layer=CR_LAYER_CUTOUT;
            gm_view_fog_apply(&ish,&uw,clear);
            render_layer(c,&cam,eb[1]+item_start,nv-item_start,&ish);
        }
        nv=gm_minecart_tnt_flash_emit(
            ents,n,1.0f,eb[1],c->max_entity_verts);
        if(nv>0){
            CrShadeCtx flash={0};flash.untextured=1;
            flash.fog_color=clear;flash.layer=CR_LAYER_TRANSLUCENT;
            flash.blend=5;flash.depth_lequal=1;
            gm_view_fog_apply(&flash,&uw,clear);
            render_layer(c,&cam,eb[1],nv,&flash);
        }
        nv=gm_item_frame_flat_items_emit(frame_views,nframe,eb[2],
                                         c->max_entity_verts);
        if(nv>0){
            CrTexture ia=gm_item_atlas();CrShadeCtx frame_item={0};
            frame_item.atlas=&ia;frame_item.fog_color=clear;
            frame_item.alpha_test=1;frame_item.alpha_ref=.1f;
            frame_item.layer=CR_LAYER_CUTOUT;frame_item.lightmap=lm;
            frame_item.entity_brightness=1;
            gm_view_fog_apply(&frame_item,&uw,clear);
            render_layer(c,&cam,eb[2],nv,&frame_item);
        }
        CrRgba map_rgba[128*128];
        for(int i=0;i<nframe;++i){
            CrVertex map_quad[6];
            int nm=gm_item_frame_map_plane_emit(&frame_views[i],map_quad,6);
            if(nm>0&&gm_item_frame_map_rgba(
                    frame_views[i].map_colors,map_rgba)){
                CrTexture mt={128,128,map_rgba,0,0,{0},{0},{0}};
                CrShadeCtx msh={0};msh.atlas=&mt;msh.fog_color=clear;
                msh.layer=CR_LAYER_TRANSLUCENT;msh.blend=4;msh.lightmap=lm;
                gm_view_fog_apply(&msh,&uw,clear);
                render_layer(c,&cam,map_quad,nm,&msh);
            }
            nm=gm_item_frame_map_icon_emit(&frame_views[i],map_quad,6);
            if(nm>0){
                CrTexture ea=gm_entity_atlas();CrShadeCtx ish={0};
                ish.atlas=&ea;ish.fog_color=clear;ish.alpha_test=1;
                ish.alpha_ref=.1f;ish.layer=CR_LAYER_CUTOUT;
                ish.lightmap=lm;
                gm_view_fog_apply(&ish,&uw,clear);
                render_layer(c,&cam,map_quad,nm,&ish);
            }
        }
        item_start=nv;
        nv+=gm_items_emit_flat(ents,n,eb[2]+nv,c->max_entity_verts-nv);
        ...
        if(nv>item_start){
            ...
            render_layer(c,&cam,eb[2]+item_start,nv-item_start,&fsh);
        }
    ...
    render_structures(c, r, &cam, &atlas, clear);
    ...
    if(c->particles){
        ...
        nv=gm_particles_live_emit_water(
            ...
            GM_PARTICLES_LIVE_CAP*6);
        nv+=gm_particles_live_emit_spell(
            c->particles,1.0f,v.yaw,v.pitch,pclw_ov+nv,
            GM_PARTICLES_LIVE_CAP*6-nv);
        nv+=gm_particles_live_emit_smoke(
            c->particles,1.0f,v.yaw,v.pitch,pclw_ov+nv,
            GM_PARTICLES_LIVE_CAP*6-nv);
        nv+=gm_particles_live_emit_heart(
            c->particles,1.0f,v.yaw,v.pitch,pclw_ov+nv,
            GM_PARTICLES_LIVE_CAP*6-nv);
        nv+=gm_particles_live_emit_portal(
            c->particles,1.0f,v.yaw,v.pitch,pclw_ov+nv,
            GM_PARTICLES_LIVE_CAP*6-nv);
        nv+=gm_particles_live_emit_combat(
            c->particles,0,1.0f,v.yaw,v.pitch,pclw_ov+nv,
            GM_PARTICLES_LIVE_CAP*6-nv);
        if(nv>0){
            CrShadeCtx ps={0};ps.atlas=&ea;ps.fog_color=clear;
            ps.alpha_test=1;ps.alpha_ref=0.003921569f;
    ...
    {
        int dx=0,dy=0,dz=0,dface=-1;float dmg=0.1f;
        int exact_dig = gm_recorded_dig_particles_count() > 0;
        int live_dig = gm_player_dig_state_ex(&dx,&dy,&dz,&dmg,&dface)
                    && dmg>0.0f;
        if(live_dig || exact_dig){
            ...
            int source_block = exact_dig
                ? gm_recorded_dig_particles_source()
                : gm_world_block(r->world,wx,dy,wz);
            int source_meta = exact_dig ? 0
                : (gm_world_meta(r->world,wx,dy,wz)&15);
            int bid=gm_state_to_model_key(gm_pack_state(
                source_block,source_meta));
            ...
            CrLightmapRgb dlm=cr_lightmap_rgb_boss_night_vision(
                r->dimension,dsky,dblk,
                cr_dimension_sun_brightness(r->dimension),0.f,0.f,
                c->boss_color_modifier,night_vision);
            int nd = exact_dig
                ? gm_recorded_dig_particles_emit(
                    bid, v.yaw, v.pitch, dlm.r, dlm.g, dlm.b,
                    dig_ov, 1024)
                : gm_block_break_particles_emit(
    ...
    if(c->particles){
        ...
        int nv=gm_particles_live_emit_recorded(
            ...
            GM_PARTICLES_LIVE_CAP*6);
        nv+=gm_particles_live_emit_combat(
            c->particles,3,1.0f,v.yaw,v.pitch,pcl3_ov+nv,
            GM_PARTICLES_LIVE_CAP*6-nv);
        if(nv>0){
            CrTexture ea=gm_entity_atlas();
            CrShadeCtx ps={0};ps.atlas=&ea;ps.fog_color=clear;
    ...
    if(dev_frame_mesh)
        render_world_dev_layers(c,&cam,dev_nruns,dev_nv,dev_nch,&atlas,clear,
    ...
    {
        GmLightningView *bolts=c->lightning_views;
        int bolt_count=gm_runtime_lightning_views(
            r,bolts,c->lightning_views_cap);
    ...
    c->outline_active[c->ent_set] = frame_build_entity_outline(
        c, &cam, ents, n, c->ent_set);
    ...
    if(c->use_cuda){
        if(c->pend_color&&cr_raster_cuda_frame_end_async){
            ...
            if(!force_sync&&cr_raster_cuda_frame_end_async(&c->fb,c->pend_color)){
                ...
                c->pend_tick=r->tick;
                c->pend_outline_active=c->outline_active[c->ent_set];
                c->pend_outline_set=c->ent_set;
    ...
    if(c->outline_active[c->ent_set])
        gm_overlay_entity_outline_composite(
            &c->fb,c->outline_overlay[c->ent_set]);
    ...
    if(v.loading==2){
        ...
        if(!v.dead&&!v.riding_boat&&!cr_cfg()->no_hand
                &&!cr_cfg()->third_person_camera&&!hand_hidden(r->tick))
            gm_hand_draw(&c->fb,&v,c->hand_bob);
        {
            int nausea = gm_potion_view_duration(&v, 9) > 0;
            float portal = v.portal_prev + (v.portal - v.portal_prev);
            gm_overlay_portal_warp(
                &c->fb, c->post_scratch, portal, v.portal_phase,
                1.0F, nausea, cam.fov_deg);
        }
...
void gm_frame_capture_close(GmFrameCapture *c) {
    ...
    free(c->entity_views);
    free(c->leash_views);
    free(c->projectile_types);
    free(c->lightning_views);
    free(c->beam_verts);
    free(c->wither_verts);
    for(int i=0;i<GM_FC_ENT_BUFS;++i)free(c->entity_verts[i]);
    for(int i=0;i<2;++i)free(c->structure_verts[i]);
    for(int i=0;i<4;++i)free(c->weather_verts[i]);
    free(c->chunks);free(c->runs);free(c->gsrc);free(c->gcnt);free(c->pend_color);
    free(c->pend_depth);free(c->post_scratch);free(c->outline_verts);
    free(c->outline_work);free(c->outline_depth);
    for(int i=0;i<2;++i)free(c->outline_overlay[i]);
