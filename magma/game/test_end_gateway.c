...
int main(void) {
    ...
    for (int extra = 1; extra <= GM_RUNTIME_END_GATEWAYS; ++extra)
        CHECK(gm_runtime_spawn_end_gateway(
                  &r, 100 + extra * 4, 200, 100,
                  1, 0, 80, 0, 1),
              "End-gateway store grows beyond hot capacity");
    CHECK(r.end_gateway_count == GM_RUNTIME_END_GATEWAYS + 1
              && r.end_gateways_cap > GM_RUNTIME_END_GATEWAYS
              && gm_runtime_end_gateway_get(
                  &r, GM_RUNTIME_END_GATEWAYS, &gateway_state)
              && gateway_state.x == 100 + GM_RUNTIME_END_GATEWAYS * 4,
          "grown End-gateway store preserves insertion payload");
    CHECK(gm_runtime_write_checkpoint(
              &r, "end-gateway-capacity-checkpoint.bin")
              && gm_runtime_load_checkpoint(
                  &r, "end-gateway-capacity-checkpoint.bin")
              && r.end_gateway_count == GM_RUNTIME_END_GATEWAYS + 1
              && r.end_gateways_cap > GM_RUNTIME_END_GATEWAYS,
          "grown End-gateway store survives checkpoint reload");
    (void)remove("end-gateway-capacity-checkpoint.bin");
