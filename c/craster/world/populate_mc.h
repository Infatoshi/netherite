/* world/populate_mc.h - decoration overlay for the craster world (VIEW-DISTANCE).
 *
 * mc-sim's cp_provide_chunk (used by world/light.c) is base terrain ONLY: it
 * omits populate()/decoration, so the raw chunks have no trees, foliage, or
 * plants. This module runs mc-sim's faithful, position-parametrized populate
 ...
 *
 * The block ids overlaid are mc-sim PB_* codes (0..20 == CB_*, 21+ are feature
 * blocks), which the craster block-model table (assets/blockmodels.c) understands.
 */
#ifndef CRASTER_WORLD_POPULATE_MC_H
#define CRASTER_WORLD_POPULATE_MC_H
#ifdef __cplusplus
extern "C" {
#endif
 ...
 * region (owr_d_min via cr_caps_override / craster.conf) or windows get evicted and
...
/* Light opacity / emission for a decorated (PB_*) block id, faithful to mc-sim. */
...
#endif
#endif /* CRASTER_WORLD_POPULATE_MC_H */
