...
MC_HD static inline int ita_is_sword(i32 item_id) {
    return item_id == 268 || item_id == 272 || item_id == 267
...
MC_HD static inline int ita_pickaxe_can_harvest(int mat, int block_id) {
    ...
    /* ItemPickaxe.canHarvestBlock: the base set is every initialized block
     * whose default-state material is ROCK, IRON, or ANVIL, with the explicit
     * ore/block harvest-level exceptions applied before that material test. */
    if (block_id == 49) return hl == 3; /* obsidian */
    if (block_id == 14 || block_id == 41 || block_id == 56 || block_id == 57
            || block_id == 73 || block_id == 74 || block_id == 129
            || block_id == 133)
        return hl >= 2;
    if (block_id == 15 || block_id == 21 || block_id == 22 || block_id == 42)
        return hl >= 1;
    switch (block_id) {
    case 1: case 4: case 7: case 16: case 23: case 24: case 43: case 44:
    case 45: case 48: case 52: case 61: case 62: case 67: case 70: case 71:
    case 87: case 98: case 101: case 108: case 109: case 112: case 113:
    case 114: case 116: case 117: case 118: case 120: case 121: case 128:
    case 130: case 137: case 139: case 145: case 147: case 148: case 152:
    case 153: case 154: case 155: case 156: case 158: case 159: case 167:
    case 168: case 172: case 173: case 179: case 180: case 181: case 182:
    case 201: case 202: case 203: case 204: case 205: case 206: case 210:
    case 211: case 213: case 215: case 216: case 218: case 219: case 220:
    case 221: case 222: case 223: case 224: case 225: case 226: case 227:
...
/* Callback-exact finite classifiers used by the registry gate and live path. */
MC_HD static inline int ita_can_harvest_block(i32 item_id, int block_id) {
    int mat = ita_tool_material(item_id);
    if (ita_is_pickaxe(item_id)) return ita_pickaxe_can_harvest(mat, block_id);
    if (ita_is_shovel(item_id)) return block_id == 78 || block_id == 80;
    if (ita_is_sword(item_id)) return block_id == 30;
    if (item_id == 359)
        return block_id == 30 || block_id == 55 || block_id == 132;
    return 0;
}

/* tool_class: 1 pickaxe, 2 axe, 3 shovel. Forge ItemTool returns its material
 * level only for the class installed by that concrete constructor. */
MC_HD static inline int ita_tool_harvest_level_exact(
        i32 item_id, int tool_class) {
    int own = ita_is_pickaxe(item_id) ? 1 : ita_is_axe(item_id) ? 2
        : ita_is_shovel(item_id) ? 3 : 0;
    int mat = ita_tool_material(item_id);
    return own && own == tool_class && mat >= 0
        ? ita_tool_harvest_level_mat(mat) : -1;
}

MC_HD static inline int ita_hit_entity_wear(i32 item_id) {
    if (ita_is_sword(item_id) || (item_id >= 290 && item_id <= 294)) return 1;
    if (ita_is_pickaxe(item_id) || ita_is_shovel(item_id)
            || ita_is_axe(item_id)) return 2;
    return 0;
}

MC_HD static inline int ita_destroy_block_wear(i32 item_id, int hardness_nonzero) {
    if (item_id == 359) return 1;
    if (!hardness_nonzero) return 0;
    if (ita_is_sword(item_id)) return 2;
    if (ita_is_pickaxe(item_id) || ita_is_shovel(item_id)
            || ita_is_axe(item_id)) return 1;
    return 0;
}
...
MC_HD static inline int ita_stack_max_damage(const ITAStack *s) {
    ...
    if (s->item == 261) return 384; /* Items.BOW maxDamage */
...
MC_HD static inline int ita_block_destroy_damage(const ITAStack *s, int block_id) {
    BptProps p = mc_bpt_props(block_id);
    if (s->item == 359)
        return 1;
    if (p.hardness == 0.0f)
        return 0;
    if (ita_is_sword(s->item))
        return 2;
    if (ita_is_pickaxe(s->item) || ita_is_shovel(s->item)
            || ita_is_axe(s->item))
        return 1;
    return 0;
}
...
MC_HD static inline void ita_on_block_destroyed(ITAStack *s, int block_id) {
    s->damage += ita_block_destroy_damage(s, block_id);
}
