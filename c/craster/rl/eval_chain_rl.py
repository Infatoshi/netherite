"""Evaluate the learned FULL-CHAIN policy on the REAL craster env, cold
...
Feature pipeline mirrors rl/cuenv/ppo_chain_cu.py exactly, computed from the
...
Run (anvil):
  cd c/craster && CHAIN_NET=chain_net_cu.pt uv run --no-project \
...
sys.path.insert(0, os.path.join(HERE, "cuenv"))

from look_at_tree import CrasterEnv, EYE, wrap180            # noqa: E402
from ppo_chain_cu import (ChainPolicy, build_frame, build_scal,  # noqa
                          obs_float, stage_of_best, NPLANES, STACK,
                          REPEAT, HEADS, YAWS, PITCHES, FWD,
                          IX_TORCH, MILE_NAMES, N_STAGES)
...
def nearest_coal_scal(obs):
    """The 6 env scalars, exactly as cuenv_decision_finalize computes them."""
    best = None
    ...
    try:
        ...
        for dec in range(ep_dec):
            ...
            if int(status[0, IX_TORCH]) >= 1:
                return N_STAGES, list(env.actions)
            if env.obs["dead"]:
                break
        return int(stage_of_best(best[:, :9])[0]), list(env.actions)
    finally:
        env.close()
...
def main():
    ...
    for seed in seeds:
        ...
        for att in range(TRIES):
            reached, actions = run_episode(seed, net, seed * 100 + att)
