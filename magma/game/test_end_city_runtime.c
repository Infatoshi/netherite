...
int main(void) {
    ...
    char err[256];
    int purpur = 0, end_bricks = 0, found_elytra = 0, sentries = 0;
    ...
    sentries = gm_runtime_shulker_count(&r);
    CHECK(sentries == 36,
          "all 36 End City Sentry markers create live attached shulkers");
    ...
    printf("end_city_runtime: PASS (%d purpur-family, %d end-brick cells, "
           "%d sentries, elytra collectible)\n",
           purpur, end_bricks, sentries);
    return 0;
}
