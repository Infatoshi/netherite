/* cuenv_core.h - batched-env core for the mine-coal learned stage: one
 * MC_HD single source that reproduces the REAL env (craster_game --rl-bin
 * restored from a .bsnp snapshot) tick-for-tick, bit-for-bit, over the
 * snapshot's 64x128x64 region. CPU reference driver: cuenv_cpu.c; the CUDA
 * driver (M2) compiles this same header under nvcc.
 *
 * Fidelity contract (see rl/cuenv/DESIGN.md Part 3): every function here is
...
#include "cuenv_snapshot.h"     /* RlSnapHead/RlSnapItem (.bsnp format) */
...
/* ---- op-trace activity counters (CUENV_OP_TRACE=1 at driver create) ----
 ...
 * pre-tick primitives - cuenv_do_craft / cuenv_do_interact) ---- */
typedef struct {
...
typedef struct {
    ...
    const int *ore_xy;           /* snapshot CSR (ix,iy)->ore-range offsets
                                  * (rnx*rny+1; cuenv_build_ore_xy), NULL =
    ...
    int   *cont;                 /* pooled container list (wx,wy,wz x n_cont;
                                  ...
                                  * - cuenv_do_interact iterates it instead of
    ...
    /* per-decision scratch (cuenv_decision_ticks -> cuenv_decision_finalize;
...
MC_HD static inline long cu_region_idx(const CuEnv *e, int wx, int wy, int wz) {
...
MC_HD static inline int cu_world_block(const CuEnv *e, int wx, int wy, int wz) {
    long i = cu_region_idx(e, wx, wy, wz);
    CU_OP(e, CU_OP_WORLD_LOAD);
    return i < 0 ? 0 : mc_state_id(e->cells[i]);
}
MC_HD static inline int cu_world_meta(const CuEnv *e, int wx, int wy, int wz) {
    long i = cu_region_idx(e, wx, wy, wz);
    CU_OP(e, CU_OP_WORLD_LOAD);
...
MC_HD static inline void cu_cont_edit(CuEnv *e, int wx, int wy, int wz,
    ...
    if (e->n_cont >= CUENV_SNAP_MAX_CONT) { e->n_cont = -1; return; }
...
MC_HD static inline void cu_world_set_state(CuEnv *e, int wx, int wy, int wz,
                                            int id, int meta) {
    long i = cu_region_idx(e, wx, wy, wz);
    if (i >= 0) {
...
MC_HD static inline void cu_fill_chunk_lanes(CuEnv *e, Chunk *ch, int cx, int cz,
...
MC_HD static inline int cu_recenter_pose(CuEnv *e, int *odcx, int *odcz) {
 ...
 * dcz, 0, 1). */
MC_HD static inline void cu_recenter_fill(CuEnv *e, int nccx, int nccz,
                                          int dcx, int dcz,
                                          int lane, int nlanes) {
...
MC_HD static inline void cu_recenter(CuEnv *e) {
    int dcx, dcz;
 ...
 * Ported exactly, statics -> CuEnv fields. Dropped as inert for this action
...
MC_HD static inline void cuenv_player_tick(CuEnv *env, const McSinTable *st,
                ...
                if (r >= 0) {
                    ...
                    if (cu_ib_is_interactable(hit_id)) {
                        ...
                        if ((held.item == 326 || held.item == 327) &&
                             ...
                             * runtime-side follower - see cuenv_runtime_tick) */
...
MC_HD static inline int cu_spawn_item(CuEnv *env, double x, double y, double z,
                                      int item, int count, int meta,
                                      int pickup_delay) {
    int i;
...
MC_HD static inline int cu_item_solid_at(const CuEnv *env, int x, int y, int z) {
...
MC_HD static inline void cu_live_tick_player(CuEnv *env) {
    int i;
    double px, py, pz;
    for (i = 0; i < CU_MAX_ITEMS; ++i) {
        CuItem *e = &env->items[i];
...
MC_HD static inline void cu_break_unsupported_plants(CuEnv *env, int wx, int wy, int wz) {
    int y;
    for (y = wy + 1; y < 255; ++y) {
...
MC_HD static inline int cuenv_do_craft(CuEnv *env, int which,
...
MC_HD static inline int cuenv_do_interact(CuEnv *env) {
     ...
     * subtraction - replicate the float add. */
    {
        ...
        if (id == 58) {
            /* gm_container_close: no-op here (cuenv carries no craft grid or
...
MC_HD static inline int cuenv_do_smelt(CuEnv *env) {
    int did = 0, i;
    CuFurnace *f;
 ...
 * cuenv_runtime_tick below, or warp-cooperatively in the CUDA k_tick). */
MC_HD static inline void cuenv_runtime_tick_nr(CuEnv *env, const McSinTable *st,
     ...
     * 290-298): cuenv's window is updated in place on every edit,
     * value-identical to the gen-triggered refill. */

    cuenv_player_tick(env, st, act, edits, &n, CU_MAX_EDITS, blocks);
...
MC_HD static inline void cuenv_runtime_tick(CuEnv *env, const McSinTable *st,
                                            CuAction act, McAABB *blocks) {
    if (!env->dead) cu_recenter(env);
    cuenv_runtime_tick_nr(env, st, act, blocks);
}
...
/* Candidate-cache maintenance, split out of cuenv_coal_list so the CUDA
...
MC_HD static inline void cuenv_coal_cache_sync(CuEnv *env, int pwx, int pwy,
    ...
    if (!env->cand_valid || env->cand_pwx != pwx || env->cand_pwy != pwy ||
        ...
        if (env->ore_xy) {
             ...
             * ordered (lex region x,y,z; verified by cuenv_build_ore_xy), so
 ...
 * full ore list on cache overflow). Split from cuenv_coal_list so the CUDA
 * warp path can reuse it as its exact fallback without re-counting the
 * CU_OP_COAL_CALL of the composed function. */
MC_HD static inline int cuenv_coal_sweep(CuEnv *env, float fx, float fy,
...
MC_HD static inline int cuenv_coal_list(CuEnv *env, int out[CU_NCOAL][3]) {
    float fx = (float)(env->pl.ent.posX + (double)env->ox);
    float fy = (float)(env->pl.ent.posY);
    float fz = (float)(env->pl.ent.posZ + (double)env->oz);
    int pwx = (int)floor((double)fx);
    ...
    if (y0 < 0)   y0 = 0;
    if (y1 > 255) y1 = 255;
    cuenv_coal_cache_sync(env, pwx, pwy, pwz, y0, y1);
    return cuenv_coal_sweep(env, fx, fy, fz, pwx, pwz, y0, y1, out);
}

/* total count of item id across the 36 main slots (rl_mode.c rl_inv_count) */
MC_HD static inline int cuenv_inv_count(const CuEnv *env, int item) {
    int i, n = 0;
 ...
 * cuenv rays that leave the 64x128x64 snapshot region read air - the design
...
MC_HD static inline void cuenv_render_cam_pixel(CuEnv *env, const McSinTable *st,
                                                int pix) {
    OcRegion reg;
...
MC_HD static inline void cuenv_render_cam(CuEnv *env, const McSinTable *st) {
    int pix;
    for (pix = 0; pix < CU_NPIX; ++pix)
        cuenv_render_cam_pixel(env, st, pix);
}
...
MC_HD static inline void cuenv_emit_bolr(CuEnv *env, const McSinTable *st,
    ...
    for (i = 0; i < 9; ++i) {
        ...
        o->inv_counts[i] = cuenv_inv_count(env, inv_ids[i]);
    }
    o->container = env->container;
    (void)cuenv_coal_list(env, o->coal);
    if (want_cam)
        cuenv_render_cam(env, st);
...
MC_HD static inline double cuenv_wrap180(double a) {
...
MC_HD static inline int cuenv_nearest_coal(const int coal[CU_NCOAL][3],
    ...
    for (i = 0; i < CU_NCOAL; ++i) {
        ...
        if (!have || d < bd) {
            ...
            *ry = cuenv_wrap180(atan2(-dx, dz) * (180.0 / CU_DEC_PI) - yaw);
 ...
 * layout as cuenv_tick_raw: {forward, strafe, dyaw(deg), dpitch(deg), jump,
 ...
 * actions are expanded to this layout by the Python wrapper (cuenv.py) with
...
MC_HD static inline int cuenv_decision_begin(CuEnv *e, const McSinTable *st,
                                             const double *a,
    ...
    if ((int)a[10] >= 0)
        (void)cuenv_do_craft(e, (int)a[10], recipes, nrecipes);
    if ((int)a[11])
        (void)cuenv_do_interact(e);
    if ((int)a[12])
        (void)cuenv_do_smelt(e);

    if (!e->have_prev_dist) {
        int coal_now[CU_NCOAL][3];
        double ry = 0.0, rp = 0.0;
        (void)cuenv_coal_list(e, coal_now);
        e->have_prev_dist = cuenv_nearest_coal(
            coal_now,
 ...
 * between the two; cuenv_decision_subtick composes them statement-for-
 * statement identically to the pre-split function. */
MC_HD static inline void cuenv_subtick_phys(CuEnv *e, const McSinTable *st,
    ...
    cuenv_runtime_tick_nr(e, st, act, blocks);
    if (rep == repeat - 1) {
        e->dec_cam_fresh = 1;
        if (render_cam_inline) cuenv_render_cam(e, st);
    }
...
MC_HD static inline void cuenv_subtick_post(CuEnv *e, int rep, int repeat,
    ...
    int plus10 = 0, is_last;
    {
        ...
        if (e->success_item > 0 &&
            cuenv_inv_count(e, e->success_item) > e->base_success) {
            plus10 = 1;
            e->done = 1;
        }
        if (e->dead) e->done = 2;
 ...
 * (serially in cuenv_decision_ticks, warp-cooperatively in the CUDA k_tick).
 * Caller contract: only while !e->done, after cuenv_decision_begin ran. */
MC_HD static inline void cuenv_decision_subtick(CuEnv *e, const McSinTable *st,
                                                const double *a, int rep,
                                                int repeat, McAABB *blocks,
                                                int render_cam_inline,
                                                double atk_gate) {
    ...
    int have_nc;
    cuenv_subtick_phys(e, st, a, rep, repeat, blocks, render_cam_inline,
                       &fx, &fy, &fz);
    (void)cuenv_coal_list(e, coal_now);
    have_nc = cuenv_nearest_coal(coal_now, fx, fy, fz,
                                 (double)e->pl.yaw, (double)e->pl.pitch,
                                 &ry, &rp, &dist);
    cuenv_subtick_post(e, rep, repeat, atk_gate, have_nc, ry, rp, dist);
}
 ...
 * used to open cuenv_runtime_tick now sits between the done and dead gates
 * at the same sequence point). */
MC_HD static inline void cuenv_decision_ticks(CuEnv *e, const McSinTable *st,
                                              ...
                                              int nrecipes) {
    int rep;
    if (!cuenv_decision_begin(e, st, a, recipes, nrecipes)) return;
    for (rep = 0; rep < repeat && !e->done; ++rep) {
        if (!e->dead) cu_recenter(e);
        cuenv_decision_subtick(e, st, a, rep, repeat, blocks,
...
MC_HD static inline void cuenv_decision_finalize(CuEnv *e, const McSinTable *st,
 ...
 * do not carry. Consumed by the cuenv_step_full drivers; NOT part of the
...
MC_HD static inline void cuenv_fill_status(const CuEnv *e, int *out) {
    static const int inv_ids[9] = CU_INV_IDS;
    int i, sel = e->pl.inv.current_item;
    for (i = 0; i < 9; ++i) out[i] = cuenv_inv_count(e, inv_ids[i]);
    if (sel < 0) sel = 0;
    if (sel > 8) sel = 8;
    out[9] = sel;
    {
    ...
    out[13] = cuenv_inv_count(e, 61);
    out[14] = cuenv_inv_count(e, 15);
    out[15] = cuenv_inv_count(e, 265);
    out[16] = cuenv_inv_count(e, 257);
}

/* =================== live env -> snapshot capture ========================= */

/* Inverse of cuenv_reset_scalar: serialize a live env's scalar state into an
 * RlSnapHead + item list (the region cells are copied separately by the
 * driver). Round-trip contract: cuenv_reset_scalar(cuenv_capture_head(e))
 ...
 * world_dirty is 0 (cuenv has no scan cache). Returns n_items written. */
MC_HD static inline int cuenv_capture_head(const CuEnv *e, RlSnapHead *h,
                                           RlSnapItem *items) {
    int i, n = 0;
    memset(h, 0, sizeof *h);
    ...
    for (i = 0; i < CU_MAX_ITEMS && n < CUENV_SNAP_MAX_ITEMS; ++i) {
...
/* Raw sim state for divergence bisecting (cuenv_debug_state ABI): doubles
 ...
 * a device CuEnv. */
MC_HD static inline int cuenv_debug_fill(const CuEnv *e, double *out) {
    int k = 0;
 ...
 * masked k_reset cost >100 ms). cuenv_reset_from_snapshot below runs both
...
MC_HD static inline long cu_reset_bulk_count(const CuEnv *env) {
    return env->rvol + (long)PSV_NCHUNKS * MC_CHUNK_VOL + CU_NPIX;
}

MC_HD static inline void cuenv_reset_bulk(CuEnv *env, const u16 *cells_src,
                                          long idx) {
    if (idx < env->rvol) {
        u16 s = cells_src[idx];
        env->cells[idx] = s;
 ...
 * field-for-field; world_dirty is ignored (cuenv has no scan cache - coal
...
MC_HD static inline void cuenv_reset_scalar(CuEnv *env, const RlSnapHead *h,
    ...
    env->base_coal = cuenv_inv_count(env, 263);
    env->success_item = success_item;
    env->base_success =
        success_item > 0 ? cuenv_inv_count(env, success_item) : 0;
...
MC_HD static inline void cuenv_reset_from_snapshot(CuEnv *env, const RlSnapHead *h,
                                                   const RlSnapItem *items,
                                                   const u16 *cells_src,
                                                   const int *ore, int nore,
                                                   const int *ore_xy,
                                                   ...
                                                   int success_item) {
    long i, nbulk;
    cuenv_reset_scalar(env, h, items, ore, nore, ore_xy, cont, ncont,
                       success_item);
    nbulk = cu_reset_bulk_count(env);
    for (i = 0; i < nbulk; ++i) cuenv_reset_bulk(env, cells_src, i);
}
