...
def test_portal_transit_reloads_arrival_dimension_snapshot(tmp_path: Path):
    ...
    patch.write_text(
        ...
        '{"tick":0,"type":"set_spawner_state","dim":-1,'
        '"x":5,"y":22,"z":-5,"entity":7,"delay":20,'
        '"min_delay":200,"max_delay":800,"spawn_count":4,'
        '"max_nearby":6,"activate_range":0,"spawn_range":4,'
        '"spawn_nbt_file":"/fixture/blaze.nbt",'
        '"default_entity_nbt":true}\n'
        '{"tick":0,"type":"add_spawner_potential","dim":-1,'
        '"x":5,"y":22,"z":-5,"entity":7,"weight":1,'
        '"entity_nbt_file":"/fixture/blaze.nbt",'
        '"default_entity_nbt":true}\n'
    ...
    spawners = [event for event in events[133]
                if event["type"] in {
                    "set_spawner_state", "add_spawner_potential"}]
    assert [event["type"] for event in spawners] == [
        "set_spawner_state", "add_spawner_potential"]
    assert all(event["tick"] == 133 and event["dim"] == -1
               for event in spawners)
...
def test_gui_container_slots_cursor_and_furnace_progress_are_mapped(tmp_path: Path):
    ...
    large_chest = [0] * 90
    large_chest[0] = [264, 0, 5]
    large_chest[27] = [297, 0, 2]
    large_chest[54] = [4, 0, 16]
    large_chest[81] = [280, 0, 1]
    brewing = [0] * 41
    brewing[0] = [373, 1, 1]
    brewing[3] = [372, 0, 2]
    brewing[4] = [377, 0, 1]
    ...
    ticks = [
        ...
        {"t": 3, "in": empty_input, "x": 0.5, "y": 70.0, "z": 0.5,
         ...
         "gprop": [200, 19]},
        {"t": 4, "in": empty_input, "x": 0.5, "y": 70.0, "z": 0.5,
         "yaw": 0.0, "pitch": 0.0, "ents": [],
         "gui": "GuiMerchant", "gslots": merchant, "gcur": 0,
         "gmrec": [1, 4, 0, [340, 0, 1], [388, 0, 5],
                    [264, 0, 1]]},
        {"t": 5, "in": empty_input, "x": 0.5, "y": 70.0, "z": 0.5,
         "yaw": 0.0, "pitch": 0.0, "ents": [], "gui": "GuiChest",
         "gslots": large_chest, "gcur": 0},
    ]
    script = tmp_path / "events.jsonl"
    replay_tape.tape_to_script(header, ticks, str(script))
    events = [json.loads(line) for line in script.read_text().splitlines()]
    ...
    tick5 = [event for event in events if event["tick"] == 5]
    assert any(event["type"] == "gui_view"
               and event["gui"] == "GuiLargeChest" for event in tick5)
    slots5 = {event["slot"]: event for event in tick5
              if event["type"] == "gui_slot_view"}
    assert slots5[53]["item"] == 264
    assert slots5[115]["item"] == 297
    assert slots5[9]["item"] == 4
    assert slots5[0]["item"] == 280
    ...
    tick4 = [event for event in events if event["tick"] == 4]
    slots4 = {event["slot"]: event for event in tick4
              if event["type"] == "gui_slot_view"}
    assert [slots4[slot]["item"] for slot in (93, 94, 95)] \
        == [340, 388, 264]
    assert next(event for event in tick4
                if event["type"] == "gui_merchant_view") == {
                    "tick": 4, "type": "gui_merchant_view",
                    "selected": 1, "offers": 4, "disabled": 0,
                    "a_item": 340, "a_count": 1, "a_meta": 0,
                    "b_item": 388, "b_count": 5, "b_meta": 0,
                    "sell_item": 264, "sell_count": 1, "sell_meta": 0,
                }
...
def test_gui_chest_slot_id_mapping():
    ...
    assert replay_tape.gui_slot_id("GuiChest", 0, 90) == 53
    assert replay_tape.gui_slot_id("GuiChest", 26, 90) == 79
    assert replay_tape.gui_slot_id("GuiChest", 27, 90) == 115
    assert replay_tape.gui_slot_id("GuiChest", 53, 90) == 141
    assert replay_tape.gui_slot_id("GuiChest", 54, 90) == 9
    assert replay_tape.gui_slot_id("GuiChest", 80, 90) == 35
    assert replay_tape.gui_slot_id("GuiChest", 81, 90) == 0
    assert replay_tape.gui_slot_id("GuiChest", 89, 90) == 8
    assert replay_tape.gui_slot_id("GuiChest", 90, 90) is None
...
def test_gui_merchant_slot_id_mapping():
    assert replay_tape.gui_slot_id("GuiMerchant", 0) == 93
    assert replay_tape.gui_slot_id("GuiMerchant", 2) == 95
    assert replay_tape.gui_slot_id("GuiMerchant", 3) == 9
    assert replay_tape.gui_slot_id("GuiMerchant", 29) == 35
    assert replay_tape.gui_slot_id("GuiMerchant", 30) == 0
    assert replay_tape.gui_slot_id("GuiMerchant", 38) == 8
    assert replay_tape.gui_slot_id("GuiMerchant", 39) is None
...
def test_llama_and_spit_tape_map_complete_render_state(tmp_path: Path):
    header = {
        "header": 1, "seed": 0, "world_time": 6000,
        "x": 0.5, "y": 70.0, "z": 0.5, "yaw": 0.0, "pitch": 0.0,
        "hp": 20.0, "food": 20, "dim": 0,
    ...
    llama = [
        21, "EntityLlama", 3.0, 65.0, 4.0, 12.0, 15.0,
        35.0, -8.0, 0.25, 3, 0, 10.0, 0,
        3, 15, 1,
    ]
    spit = [22, "EntityLlamaSpit", 3.5, 66.2, 4.5, 72.0, -1.0, -14.0]
    ticks = [{
        "t": 0, "in": {"f": 0, "s": 0},
        "x": 0.5, "y": 70.0, "z": 0.5,
        "yaw": 0.0, "pitch": 0.0, "ents": [llama, spit],
    }]
    script = tmp_path / "llama_events.jsonl"
    replay_tape.tape_to_script(header, ticks, str(script))
    events = [json.loads(line) for line in script.read_text().splitlines()]
    llama_view = next(e for e in events if e.get("type") == "ent_view"
                      and e.get("ent") == "EntityLlama")
    assert llama_view["item"] == 3
    assert llama_view["item_meta"] == 16
    assert llama_view["count"] == 1
    assert llama_view["body_yaw"] == 10.0
    assert llama_view["head_yaw"] == 35.0
    spit_view = next(e for e in events if e.get("type") == "ent_view"
                     and e.get("ent") == "EntityLlamaSpit")
    assert spit_view["yaw"] == 72.0
    assert spit_view["pitch"] == -14.0


def test_bat_tape_maps_hanging_flag_and_animation_age(tmp_path: Path):
    header = {
        "header": 1, "seed": 0, "world_time": 6000,
        "x": 0.5, "y": 70.0, "z": 0.5, "yaw": 0.0, "pitch": 0.0,
        "hp": 20.0, "food": 20, "dim": 0,
    ...
    bat = [
        23, "EntityBat", 3.0, 65.0, 4.0, 180.0, 6.0,
        180.0, 0.0, 0.0, 0, 0, 180.0, 524288, 40,
    ]
    ticks = [{
        "t": 0, "in": {"f": 0, "s": 0},
        "x": 0.5, "y": 70.0, "z": 0.5,
        "yaw": 0.0, "pitch": 0.0, "ents": [bat],
    }]
    script = tmp_path / "bat_events.jsonl"
    replay_tape.tape_to_script(header, ticks, str(script))
    events = [json.loads(line) for line in script.read_text().splitlines()]
    view = next(event for event in events
                if event.get("type") == "ent_view"
                and event.get("ent") == "EntityBat")
    assert view["flags"] == 524288
    assert view["ticks_existed"] == 40


def test_entity_armor_stand_tape_maps_live_pose_and_layers(tmp_path: Path):
    header = {
        "header": 1, "seed": 0, "world_time": 6000,
        "x": 0.5, "y": 70.0, "z": 0.5,
    ...
    pose = [float(value) for value in range(1, 19)]
    colors = [0x102030, 0x405060, 0x708090, 0xa0b0c0]
    metadata = [2, 3, 4, 5, 6, 1]
    stand = [
        19, "EntityArmorStand", 2.5, 70.0, 3.5, 27.0, 20.0,
        27.0, -4.0, 0.0, 0, 0, 27.0, 8,
        7, 276, 261, 309, 308, 307, 306, 43, 2.25,
        *pose,
        *colors,
        *metadata,
    ]
    ...
    script = tmp_path / "stand_events.jsonl"
    replay_tape.tape_to_script(header, ticks, str(script))
    events = [json.loads(line) for line in script.read_text().splitlines()]
    view = next(event for event in events
                if event.get("type") == "ent_view"
                and event.get("ent") == "EntityArmorStand")
    assert view["stand_flags"] == 7
    assert (view["stand_mainhand"], view["stand_offhand"]) == (276, 261)
    assert (view["armor_feet"], view["armor_legs"],
            view["armor_chest"], view["armor_head"]) == (309, 308, 307, 306)
    assert view["ticks_existed"] == 43
    assert view["stand_punch_time"] == 2.25
    assert view["stand_pose_valid"] == 1
    assert [view[f"stand_pose_{part}_{axis}"]
            for part in range(6) for axis in ("x", "y", "z")] == pose
    assert view["armor_color_valid"] == 15
    assert [view[f"armor_color_{slot}"] for slot in range(4)] == colors
    assert (view["stand_mainhand_meta"], view["stand_offhand_meta"],
            view["armor_feet_meta"], view["armor_legs_meta"],
            view["armor_chest_meta"], view["armor_head_meta"]) == tuple(metadata)
...
def test_recorded_particle_constructor_state_maps_without_rng(tmp_path):
    header = {
        "header": 1, "seed": 0, "world": "qrl_0", "world_time": 6000,
        "x": 0.5, "y": 70.0, "z": 0.5, "yaw": 0.0, "pitch": 0.0,
        "hp": 20.0, "food": 20, "dim": 0,
    ...
    base_state = {
        ...
        "prev": [1.0, 2.0, 3.0],
        "pos": [1.25, 2.5, 3.75],
        "motion": [-0.03125, 0.0625, 0.125],
        "age": 0, "max_age": 29,
        "ground": True, "collide": True,
        "scale": 4.25, "gravity": 0.0,
        "rgba": [0.8125, 0.8125, 0.8125, 1.0],
        "tex": [0, 0], "jitter": [1.0, 2.0],
    }
    tick = {
        "t": 4, "in": {"f": 0, "s": 0},
        "x": 0.5, "y": 70.0, "z": 0.5,
        ...
        "ents": [],
        "pcl": [
            [0, 9.0, 9.0, 9.0, 8.0, 8.0, 8.0, base_state],
            [1, 4.0, 5.0, 6.0, 0.75, 0.0, 0.0,
             {**base_state, "life": 3, "life_time": 9, "size": 0.625}],
            [2, 7.0, 8.0, 9.0, 0.0, 0.0, 0.0,
             {**base_state, "time": 2}],
            [11, 3.0, 4.0, 5.0, 0.01, 0.02, 0.03,
             {**base_state, "max_age": 17,
              "rgba": [0.2, 0.2, 0.2, 1.0]}],
            [15, 2.0, 3.0, 4.0, 0.25, 0.5, 0.75,
             {**base_state, "base_tex": 128,
              "rgba": [0.25, 0.5, 0.75, 1.0]}],
            [34, 5.0, 6.0, 7.0, 0.0, 0.1, 0.0,
             {**base_state, "max_age": 16, "scale": 2.5,
              "rgba": [1.0, 1.0, 1.0, 1.0], "tex": [0, 5]}],
            [48, 6.0, 7.0, 8.0, 0.2, 0.3, 0.4,
             {**base_state, "max_age": 13, "scale": 1.75,
              "rgba": [0.6, 0.6, 0.6, 1.0], "tex": [3, 1]}],
        ],
    }
    script = tmp_path / "events.jsonl"
    replay_tape.tape_to_script(header, [tick], str(script))
    events = [json.loads(line) for line in script.read_text().splitlines()]
    particles = [e for e in events if e["type"] == "spawn_particle_state"]
    assert particles == [
        {"tick": 4, "type": "spawn_particle_state", "id": 0,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 0, "max_age": 29, "on_ground": 1,
         "scale": 4.25, "color_r": 0.8125, "color_g": 0.8125,
         "color_b": 0.8125, "tex": 0, "tex_base": 0},
        {"tick": 4, "type": "spawn_particle_state", "id": 1,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 3, "max_age": 9, "on_ground": 1,
         "scale": 0.625, "color_r": 0.8125, "color_g": 0.8125,
         "color_b": 0.8125, "tex": 0, "tex_base": 0},
        {"tick": 4, "type": "spawn_particle_state", "id": 2,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 2, "max_age": 8, "on_ground": 1,
         "scale": 4.25, "color_r": 0.8125, "color_g": 0.8125,
         "color_b": 0.8125, "tex": 0, "tex_base": 0},
        {"tick": 4, "type": "spawn_particle_state", "id": 11,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 0, "max_age": 17, "on_ground": 1,
         "scale": 4.25, "color_r": 0.2, "color_g": 0.2,
         "color_b": 0.2, "tex": 0, "tex_base": 0},
        {"tick": 4, "type": "spawn_particle_state", "id": 15,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 0, "max_age": 29, "on_ground": 1,
         "scale": 4.25, "color_r": 0.25, "color_g": 0.5,
         "color_b": 0.75, "tex": 0, "tex_base": 128},
        {"tick": 4, "type": "spawn_particle_state", "id": 34,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 0, "max_age": 16, "on_ground": 1,
         "scale": 2.5, "color_r": 1.0, "color_g": 1.0,
         "color_b": 1.0, "tex": 80, "tex_base": 0},
        {"tick": 4, "type": "spawn_particle_state", "id": 48,
         "prev_x": 1.0, "prev_y": 2.0, "prev_z": 3.0,
         "x": 1.25, "y": 2.5, "z": 3.75,
         "vx": -0.03125, "vy": 0.0625, "vz": 0.125,
         "age": 0, "max_age": 13, "on_ground": 1,
         "scale": 1.75, "color_r": 0.6, "color_g": 0.6,
         "color_b": 0.6, "tex": 19, "tex_base": 0},
    ]
