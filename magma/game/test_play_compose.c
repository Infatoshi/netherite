...
static void test_raycast_starts_inside_block(void) {
    McSinTable st; mc_sin_table_init(&st);
    Chunk win[PSV_NCHUNKS];
    fill_flat(win, 68, BLK_STONE);
    mc_set(&win[4], 8, 71, 8, mc_state(BLK_SAND, 0));
    PsvPlayer pl; psv_player_init(&pl);
    pl.ent.posX = 8.5; pl.ent.posY = 70.0; pl.ent.posZ = 8.5;
    pl.ent.box = psv_player_box(pl.ent.posX, pl.ent.posY, pl.ent.posZ);
    pl.yaw = 0.0f; pl.pitch = 45.0f;
    int hx, hy, hz, ax, ay, az;
    int hit = gm_raycast_sel_reach(win, &st, &pl, PSV_REACH,
                                   &hx, &hy, &hz, &ax, &ay, &az);
    CHECK(hit == 0 && hx == 8 && hy == 71 && hz == 8,
          "selection ray tests a solid starting eye cell");
    mc_set(&win[4], 8, 71, 8, mc_state(BLK_AIR, 0));
    mc_set(&win[4], 8, 71, 9, mc_state(BLK_STONE, 0));
    pl.yaw = 0.0f; pl.pitch = 0.0f;
    hit = gm_raycast_sel_reach(win, &st, &pl, PSV_REACH,
                               &hx, &hy, &hz, &ax, &ay, &az);
    CHECK(hit == 1 && hx == 8 && hy == 71 && hz == 9,
          "selection ray preserves normal adjacent-cell hit semantics");
}
...
int main(void) {
    ...
    test_raycast_starts_inside_block();
