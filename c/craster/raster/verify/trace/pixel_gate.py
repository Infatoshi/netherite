...
Classes (conservative predicates, keyed to OPEN_DIVERGENCES numbers):
  ...
  particles #40/#48  oracle-only brightness (additive particles craster
                     doesn't draw); craster-brighter clusters NEVER match
...
def transit_ticks(rows, pad=40):
    ...
    a few frames apart from craster (chunk-stream timing is not simulated);
