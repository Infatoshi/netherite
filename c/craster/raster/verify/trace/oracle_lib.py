...
the hard-won prior art in raster/verify/mc_capture/ (capture.sh /
...
Conventions (pixel-verified, see c/craster/game/view.h):
  - qrl obs / MC tp use FEET coords and MC yaw/pitch (yaw 180 faces -Z,
    positive pitch looks DOWN). craster's script set_pose takes the SAME
...
def craster_root():
    """raster/verify/trace/oracle_lib.py -> c/craster (works in any worktree)."""
    return os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))


def repo_root():
    return os.path.abspath(os.path.join(craster_root(), "..", ".."))


def diff_frame_py():
    return os.path.join(repo_root(), "c", "render-opt", "wholeframe", "diff_frame.py")


def run_craster_script(script_path, ticks, frames_dir, state_out, w=854, h=480,
                       seed=0, extra_env=None, timeout=600,
                       frame_every=1, frame_offset=0, mobs=True,
                       backend="cpu", daylight=True):
    """Run the craster game headlessly on a JSONL event script.

    backend="cuda" uses the craster_game_cuda binary (raster stage on the GPU,
    ...
    """
    croot = craster_root()
    binary = "craster_game_cuda" if backend == "cuda" else "craster_game"
    ...
    if r.returncode != 0:
        ...
        sys.stderr.write(r.stderr.decode(errors="replace"))
        raise RuntimeError(f"craster_game failed (rc={r.returncode})")
    return r
...
def read_ppm(path):
    """Binary P6 PPM -> (h,w,3) uint8. craster's --frames-out format; reading
...
def side_by_side(mc_png, craster_png, out_png, scale=0.5):
    """Small side-by-side (MC | craster) for the committed report."""
    from PIL import Image
    a = Image.open(mc_png).convert("RGB")
    b = Image.open(craster_png).convert("RGB")
    w, h = a.size
...
def heatmap(mc_png, craster_png, out_png, scale=0.5):
    import numpy as np
    from PIL import Image
    a = np.asarray(Image.open(mc_png).convert("RGB")).astype(np.int16)
    b = np.asarray(Image.open(craster_png).convert("RGB")).astype(np.int16)
