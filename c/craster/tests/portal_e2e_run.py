...
CRASTER = REPO / "c" / "craster"
MCSIM = REPO / "c" / "mc-sim" / "core"
...
def build_dim_mesh() -> Path:
    ...
    objs = []
    flags = ["-O2", "-ffp-contract=off", "-Wall", "-Icore", "-I.", f"-I{MCSIM}"]
    units = [
    ...
    for u in units:
        ...
        subprocess.run(
            ...
            cwd=str(CRASTER), check=True,
        )
        objs.append(str(o))
    subprocess.run(
        ["gcc", *flags,
         str(CRASTER / "raster/verify/mc_capture/dim_mesh_candidate.c"),
         *objs, "-o", str(out), "-lm"],
        cwd=str(CRASTER), check=True,
    )
    return out
...
def project_aabb_mask(cam: dict, bounds: tuple[float, ...], pad: int = 2) -> np.ndarray:
    """Project a world AABB with craster's exact first-person camera convention."""
    x0, y0, z0, x1, y1, z1 = bounds
    yaw = math.radians(float(cam["craster_yaw_deg"]))
    pitch = math.radians(float(cam["craster_pitch_deg"]))
...
def render_c(
    ...
    # craster yaw/pitch from camera dump
    cyaw = float(cam.get("craster_yaw_deg", 180.0 - float(cam["yaw"])))
    cpitch = float(cam.get("craster_pitch_deg", -float(cam["pitch"])))
...
def compare_dimension_views(
    ...
    for view in views:
        ...
        required = ("eye_x", "eye_y", "eye_z", "craster_yaw_deg",
                    "craster_pitch_deg", "fov_effective")
