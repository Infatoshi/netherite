...
void gm_sel_box(const GmSelIn *in, float b[6]) {
    ...
    case 29: case 33:                                /* piston bases */
        ...
        break;
    case 44: case 126: case 182: case 205:           /* slabs: top bit 8 */
...
int gm_raycast_sel_reach_distance(
    ...
    /* World.rayTraceBlocks tests the starting BlockPos before its DDA loop.
     * This matters when falling sand enters the player's eye cell: vanilla
     * can still select and mine that block from inside its collision box. */
    {
        int id = psv_get_block(win, lastx, lasty, lastz);
        if (!ray_transparent(id)) {
            float b[6];
            int nx, ny, nz;
            gm_sel_box_at(win, lastx, lasty, lastz, b);
            double th = ray_box_hit(
                ex, ey, ez, dx, dy, dz,
                lastx + (double)b[0], lasty + (double)b[1],
                lastz + (double)b[2], lastx + (double)b[3],
                lasty + (double)b[4], lastz + (double)b[5],
                &nx, &ny, &nz);
            if (th >= 0.0 && th <= reach) {
                *hx = lastx; *hy = lasty; *hz = lastz;
                *ax = lastx + nx; *ay = lasty + ny; *az = lastz + nz;
                if (distance)
        ...
        if (!ray_transparent(id)) {
            ...
            if (th >= 0.0 && th <= reach) {
                ...
                *ax = bx + nx; *ay = by + ny; *az = bz + nz;
                if (distance)
                    *distance = th * sqrt(dx * dx + dy * dy + dz * dz);
                /* Zero is reserved for a hit in the starting eye cell.
                 * Reaching even the immediately adjacent cell is the normal
                 * World.rayTraceBlocks path and must remain a usable hit. */
                return 1;
            }
        }
