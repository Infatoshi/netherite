#!/usr/bin/env bash
# run_trace.sh - one entry point for the oracle-vs-craster trace/replay flywheel.
#
...
# --- craster build (worktree-local) -----------------------------------------
[ -x "$CRASTER/craster_game" ] || { log "building craster_game ..."; make -C "$CRASTER" game >/dev/null || fail "craster_game build failed"; }
