...
MC_HD static inline void shl_table_get(int table_id, ShlTable *t) {
    ...
    } else if (table_id == SHL_IGLOO_CHEST) {
        /* assets/minecraft/loot_tables/chests/igloo_chest.json */
        ShlPool *supplies = &t->pools[0];
        ShlPool *apple = &t->pools[1];
        t->n_pools = 2;

        supplies->rolls.min = 2.0f; supplies->rolls.max = 8.0f;
        supplies->bonus_rolls.min = 0.0f;
        supplies->bonus_rolls.max = 0.0f;
        supplies->n_entries = 7;
        shl_entry_count(&supplies->entries[0], SHL_APPLE, 15, 1, 3);
        shl_entry_count(&supplies->entries[1], SHL_COAL, 15, 1, 4);
        shl_entry_count(&supplies->entries[2], SHL_GOLD_NUGGET, 10, 1, 3);
        shl_entry_one(&supplies->entries[3], SHL_STONE_AXE, 2);
        shl_entry_one(&supplies->entries[4], SHL_ROTTEN_FLESH, 10);
        shl_entry_one(&supplies->entries[5], SHL_EMERALD, 1);
        shl_entry_count(&supplies->entries[6], SHL_WHEAT, 10, 2, 3);

        apple->rolls.min = 1.0f; apple->rolls.max = 1.0f;
        apple->bonus_rolls.min = 0.0f; apple->bonus_rolls.max = 0.0f;
        apple->n_entries = 1;
        shl_entry_one(&apple->entries[0], SHL_GOLDEN_APPLE, 1);
    } else if (table_id == SHL_WOODLAND_MANSION) {
        /* assets/minecraft/loot_tables/chests/woodland_mansion.json */
        ShlPool *treasure = &t->pools[0];
        ShlPool *supplies = &t->pools[1];
        ShlPool *bones = &t->pools[2];
        t->n_pools = 3;

        treasure->rolls.min = 1.0f; treasure->rolls.max = 3.0f;
        treasure->bonus_rolls.min = 0.0f;
        treasure->bonus_rolls.max = 0.0f;
        treasure->n_entries = 10;
        shl_entry_one(&treasure->entries[0], SHL_LEAD, 20);
        shl_entry_one(&treasure->entries[1], SHL_GOLDEN_APPLE, 15);
        shl_entry_one_meta(&treasure->entries[2], SHL_GOLDEN_APPLE, 1, 2);
        shl_entry_one(&treasure->entries[3], SHL_RECORD_13, 15);
        shl_entry_one(&treasure->entries[4], SHL_RECORD_CAT, 15);
        shl_entry_one(&treasure->entries[5], SHL_NAME_TAG, 20);
        shl_entry_one(&treasure->entries[6], SHL_CHAIN_CHESTPLATE, 10);
        shl_entry_one(&treasure->entries[7], SHL_DIAMOND_HOE, 15);
        shl_entry_one(&treasure->entries[8], SHL_DIAMOND_CHESTPLATE, 5);
        shl_entry_enchant_random_book(&treasure->entries[9], 10);

        supplies->rolls.min = 1.0f; supplies->rolls.max = 4.0f;
        supplies->bonus_rolls.min = 0.0f;
        supplies->bonus_rolls.max = 0.0f;
        supplies->n_entries = 10;
        shl_entry_count(&supplies->entries[0], SHL_IRON_INGOT, 10, 1, 4);
        shl_entry_count(&supplies->entries[1], SHL_GOLD_INGOT, 5, 1, 4);
        shl_entry_one(&supplies->entries[2], SHL_BREAD, 20);
        shl_entry_count(&supplies->entries[3], SHL_WHEAT, 20, 1, 4);
        shl_entry_one(&supplies->entries[4], SHL_BUCKET, 10);
        shl_entry_count(&supplies->entries[5], SHL_REDSTONE, 15, 1, 4);
        shl_entry_count(&supplies->entries[6], SHL_COAL, 15, 1, 4);
        shl_entry_count(&supplies->entries[7], SHL_MELON_SEEDS, 10, 2, 4);
        shl_entry_count(&supplies->entries[8], SHL_PUMPKIN_SEEDS, 10, 2, 4);
        shl_entry_count(&supplies->entries[9], SHL_BEETROOT_SEEDS, 10, 2, 4);

        bones->rolls.min = 3.0f; bones->rolls.max = 3.0f;
        bones->bonus_rolls.min = 0.0f; bones->bonus_rolls.max = 0.0f;
        bones->n_entries = 4;
        shl_entry_count(&bones->entries[0], SHL_BONE, 10, 1, 8);
        shl_entry_count(&bones->entries[1], SHL_GUNPOWDER, 10, 1, 8);
...
MC_HD static inline void shl_fill_inventory_table(
        TecStack *slots, int slot_count, const ShlTable *table, i64 loot_seed,
        const LtContext *context) {
    LtContext zero;
    const LtContext *ctx = context;
    JavaRandom rng;
    ...
    if (!slots || slot_count <= 0 || slot_count > TEC_SLOTS || !table)
        return;

    if (!ctx) {
        zero.luck = 0.0f;
        zero.has_killer = 0;
        zero.looting_level = 0;
        ctx = &zero;
    }
...
MC_HD static inline void shl_fill_inventory_context(
        TecStack *slots, int slot_count, int table_id, i64 loot_seed,
        const LtContext *context) {
    ShlTable table;
    if (table_id < 0 || table_id >= SHL_NUM_TABLES) return;
    shl_table_get(table_id, &table);
    shl_fill_inventory_table(
        slots, slot_count, &table, loot_seed, context);
}

MC_HD static inline void shl_fill_inventory(TecStack *slots, int slot_count,
                                            int table_id, i64 loot_seed) {
    shl_fill_inventory_context(
        slots, slot_count, table_id, loot_seed, (const LtContext *)0);
}

MC_HD static inline void shl_fill_inventory_luck(
        TecStack *slots, int slot_count, int table_id, i64 loot_seed,
        float luck) {
    LtContext context;
    context.luck = luck;
    ...
    shl_fill_inventory_context(
        slots, slot_count, table_id, loot_seed, &context);
}
...
MC_HD static inline void shl_fill_chest_luck(
        TeChest *chest, int table_id, i64 loot_seed, float luck) {
    if (!chest) return;
    shl_fill_inventory_luck(
        chest->slots, TEC_SLOTS, table_id, loot_seed, luck);
}
