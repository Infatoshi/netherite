...
`docs/COMPLETENESS.md` is the stricter registry-complete arbitrary-save audit;
it is authoritative when a bounded promotion here could be mistaken for
universal feature completion.

Latest living checkpoint: the item-environment baseline is joined by exact
expanded minecarts, initialized villager-economy persistence, an interactive
merchant container/UI, wolf and ocelot tame/sit/follow/teleport behavior,
dragon resummoning and End spikes, live End-city shulkers and guided bullets,
fresh-NBT NoAI continuation for all 34 plain native living classes, exact
fresh-NBT active-villager continuation, and
complete woodland mansion graph/template placement with live base illagers.
Five real-Java graph
fixtures, all 73 owned templates and transformed block states, mansion/illager
loot, resident materialization, Evoker attack/summon/Wololo spells, fangs,
bounded Vex behavior, mob geometry, and focused runtime gates pass. Scalar
throughput remains above the frozen 3,858.9 floor and 4,062 baseline under the
current shared-host load. Ocean monuments now add Java-identical spacing and
biome viability, the complete room graph and clipped block volume, streaming
placement, and all three persistent elders. Guardian and Elder Guardian
attributes, laser timing/damage, defensive thorns, loot RNG, sounds, animated
model, beam, and mining-fatigue route are live under focused Java/native gates.
All 81 registered entity rows and all 24 registered tile rows are now
`live_bounded`. The final tile promotion adds coordinate-free rich sign and
banner NBT, exact support-loss drops, signed skull profiles and piston drops,
and bounded command-block `Searge` plus the complete vanilla `time`
family with exact special/timestamped output, world-time effect, comparator
update, loaded order, state-capsule continuation, and native checkpoints. A
mixed command/sign save boundary is exact across all 30 observed state
features, blocks, and light.
Explicit-duration `weather clear`, `rain`, and `thunder` now share that strict
boundary, including timers, persisted flags, the 0.01-per-tick visual-strength
ramp, and translation output. Their fork exposed and fixed the oracle capsule's
prior use of a strength-threshold weather view in place of `WorldInfo` flags.
All eleven command/weather cases now execute the trigger independently from
the same unexecuted pre-tick capsule. Java's wall-clock output field is an
explicit replay input. This stronger boundary found and fixed empty command
output normalization and a private weather-clock synchronization bug that
previously reset native `time set/add` on the following tick.
Default-duration weather now captures the Java constructor-RNG result as an
explicit external fork input. Clear, rain, and thunder each pass the same
30-family state, block, and light boundary; native standalone execution draws
the vanilla 300-899 second duration range without perturbing world RNG.
The honest effort-weighted full-replica estimate remains roughly 98%; the last
program is arbitrary-world promotion beyond the bounded registry slices plus
broad state/pixel equivalence. The finalization audit deliberately did
not round this to 100: strict focused pixel gates still expose nonzero hand,
portal, underwater, preview, and entity residuals. This percentage measures
completed, directly checked roadmap effort, not universal equivalence over
arbitrary saves and inputs. GPU 1 was not executed while shared.
Every locally runnable quick-sweep stage passes, with only the two
...
and one is `QUEUED`. The later 2026-08-10 checkpoint supersedes this historical
snapshot with an effort-weighted estimate of approximately 83%. Oracle case
count measures regression depth, not global feature completion.
...
| 8 | O-02 | ACTIVE | Versioned state capsule/load contract. Player, inventory, block cuboid, represented entities, dimension, time/weather, and relevant RNG cursors restore on both sides. Ordinary tagless items restore exact loaded order and environment state. Exact NoAI restore now covers plain living classes, pigs, tameable wolves/ocelots, villagers with either lazy or initialized bounded economies, and shulkers, including private RNG/Gaussian and common living-base state | general task/path state, arbitrary tagged/custom item NBT and hooks, arbitrary cross-feature loaded order, remaining entity classes, and exhaustive capsule translation remain | 1-2 wk |
...
| 18 | R-05 | ACTIVE | Automation and rails. Live five-slot hoppers now push, pull, capture item entities, respect power and exact eight-tick cooldowns, chain into adjacent inventories, honor brewing stands' exact top/side/bottom slot filters including the ingredient glass-bottle exception, obey furnace top-input/side-fuel/bottom-output rules including bottom water-bucket extraction, traverse ordinary/trapped double chests in canonical west/north-first order while ignoring player-facing solid-block obstruction above either half, respect that exact lower-face side-solid obstruction separately for player use of every single/double chest half, and insert/extract shulker boxes while rejecting nested shulkers. Droppers insert into facing inventories, and both droppers and dispensers use exact multi-slot reservoir selection with a shared injectible 48-bit cursor. Nineteen dispenser behavior classes now match Java position, motion, events, item mutation, and stable RNG cursor: metadata/damage-preserving default ejection for ordinary untagged items through air or solid targets, arrows, splash/lingering potions, fire charges, fireworks, all six boat wood types on water/air-over-water plus blocked fallback, all six minecart item kinds on flat/ascending/lower rails plus blocked fallback, all 16 untagged shulker-box colors with support-dependent facing/failure, empty-target fallback and represented-player equip/occupied-slot fallback for all armor items/shield/elytra, non-pattern singleton/stacked represented-player and occupied/empty behavior for all skull types and pumpkins, filled water/lava buckets, singleton and stacked empty-bucket source pickup/slot insertion/full-dispenser ejection/default fallback, white-dye bonemeal for wheat/carrot/potato/beetroot, pumpkin/melon stems, cocoa, tall grass, ferns, both halves of clone-producing double plants, all six stage-zero and stage-one saplings through ordinary/big/2x2 dispatch, brown/red mushrooms, and grass spreading with every vanilla biome flower table, primed TNT, eggs, snowballs, experience bottles, and flint-and-steel air/failure/TNT paths. The playable main-hand and empty-main/offhand white-dye routes share the exact plant growth body and are locked for every sapling type, both stage-zero outcomes, ordinary/big trees, successful unobstructed 2x2 spruce/jungle/dark oak, required-layout failure, and obstruction rollback. Natural random ticks share that exact sapling stage/generator body behind Java's above-light and `nextInt(7)` gates for every species, with an isolated rank-zero real-selector proof covering selected cell, `updateLCG`, world RNG, and mutation. Invalid-support natural ticks additionally match drop-first teardown and exact item/cursor state for both stages and all six species, including generator-specific failure/restoration and dark-oak retained air. Rideable, TNT, and hopper minecarts follow straight, sloped, and curved rails; powered rails accelerate/brake, detector rails power, activator rails ignite TNT/disable hoppers, hopper carts capture items, and derailment, entity collision, and vehicle callbacks match the real-game fixture | tagged default-item stacks, tipped/spectral arrows, spawn eggs, mob/mixed-target equipment, populated-main-hand PASS arbitration before offhand bonemeal, general loaded-world random-tick section/chunk ordering, skull/pumpkin summon-pattern completion and tagged headwear, named/tagged shulker placement, boat-type capsule transport/non-oak textures, arbitrary Forge fluid-container variants, sitting-ocelot chest obstruction and broader container-persistence edges, automatic persistence of Java's unsaved static selector cursor, remaining powered-rail notification and comparator edges, furnace/chest/spawner/command-cart behavior, destruction/drops, broad save/load, rendering, and exhaustive comparator/rail notification edges | 6-12 wk |
...
| 21 | F-02 | ACTIVE | Enchanting bundle. Exact bookshelf scan, seeded offer recomputation, item/lapis slots, XP/lapis costs, option application/reseed, book conversion, and a playable live table UI are implemented. Live anvils cover repair, compatible combination, rename, level costs, XP deduction, output transfer, degradation, events, and container lifecycle; 30 real-Java/native cases lock the represented result | remaining enchanted-book and incompatible-item combinations, prior-work/repair-cost edges, runic text/glint, arbitrary NBT persistence, and real-Java UI pixel promotion | 4-8 wk |
| 22 | F-03 | ACTIVE | Brewing bundle. Five-slot live stands, exact fuel/timer and all 1.11.2 registry recipes, bottle filling, drinkable potion and milk completion, player effects, save/capsule state, break drops, GUI progress, comparator fullness, exact top/side/bottom hopper insertion and extraction, exact seeded splash/lingering launch, isolated player-thrown potion nearby-thrower suppression/release and returning direct-hit factor, player impact scaling, instant player/mob effects with undead reversal, water-fire extinguishing, water damage to blazes/endermen, exact event colors, per-target lingering-cloud lifecycle/reapplication, automatic Java-capsule scalar-state resume for bounded default thrown potions/clouds, exact active/waiting lingering-cloud particle call streams plus bounded ParticleSpell rendering, and bounded mob status-effect storage/combine/aging with live regeneration, poison, undead applicability, fire resistance, Speed, Slowness, Strength, Weakness, Jump Boost, post-immunity Resistance, periodic Wither death/drop, Health Boost, Absorption, Levitation, Water Breathing plus represented general-mob air/drowning, normal-survival Invisibility model/gel suppression with independent fire rendering, exact player Night Vision lightmap/fog rendering, exact player Blindness fog plus sprint-start suppression, and the represented critical/base-damage/knockback/Fire-Aspect/sword-sweep/player-attack sound/particle boundary including exact hurt/death audio for every represented living target are implemented. Equipment enchantments route Aqua Affinity, Depth Strider, Respiration, Frost Walker, and Thorns through the live product path; Mending consumes XP with exact equipped-item reservoir selection and repair/cooldown ordering | remaining mob world/render and non-brewing effect behaviors, statistics and player-target velocity acknowledgement, broader multi-entity throwable ignore ordering, full arbitrary NBT persistence for projectile/cloud state, custom potion NBT, unsaved ParticleSpell constructor entropy and cloud-particle pixel promotion, and remaining real-Java GUI/potion pixel promotion | 3-6 wk |
...
| 24 | G-02 | ACTIVE | Villages. Exact spacing candidates, recursive piece graphs, orientation/biome variants, ground alignment, roads, wells, farms/crops, houses, doors, blacksmith chest facing/seed/loot, and generated resident coordinates/professions feed live population. Normal residents materialize once into the bounded mob store, use profession skins and the exact nine-part villager model, and retain lazy private-RNG merchant state. All ordinary careers and tiers, including enchanted offers, match Java price rolls and recipe fields; matching, reversed two-input execution, use limits, pitch, XP, wealth, restocking, and willingness state are live. The interactive merchant container/UI preserves selected recipes, exact slot mapping, result pickup, repeated quick move, close cleanup, and age/distance lifecycle. An unopened NoAI villager now round-trips through the shared state capsule and continues for 20 exact ticks, including its otherwise hidden living-sound RNG consumption | villager task AI/navigation, village door/reputation state, breeding, golems, treasure-map edges, arbitrary chunk-load-order persistence, and pixel promotion | 8-16 wk |
| 25 | G-03 | ACTIVE | Scattered features, monuments, and woodland mansions. Desert pyramids, jungle temples, swamp huts, and igloos use exact four-facing 1.11.2 structure pieces in live population. Pyramid and temple chest/dispenser seeds, facing metadata, exact loot tables, generated dispenser realization, traps/puzzles, and the swamp empty-pot tile quirk are retained. Generated hut Witch sites materialize once into the bounded live mob store with a distinct type and bounded potion/combat/lifecycle behavior. Igloos match surface/basement selection, complete templates and tiles, deferred loot, resident state, and distinct zombie-villager rendering. Woodland mansions use Java-identical spacing/biome candidates, randomized grid and room graph, all 73 templates with exact mirror/rotation metadata, piece order, foundations, chest markers and loot, and persistent Mage/Warrior residents. Vindicator/Evoker base state, targeting, loot/audio/rendering, exact Evoker attack/summon/Wololo boundaries, fang damage/lifecycle, bounded Vex flight/charge/lifespan behavior, casting/aggressive arms, held weapons, and owned textures are live. Ocean monuments use exact spacing/biome candidates, room graph, four-facing clipped placement, shell/interior/treasure/sponge blocks, and three persistent elder sites. Guardian/Elder attributes, laser, thorns, loot, audio, model animation, beam, and mining fatigue are live | Ordinary Guardian natural-spawn pack and loaded-order parity; monument locate/save/load, arbitrary chunk-load-order and pixel promotion; arbitrary illager task/NBT continuation and pixels; exact Vex path/owner continuation and client particle/RNG details; Witch held-potion/status pixels and remaining lethal edges; active zombie-villager conversion; arbitrary template entity NBT/attributes | 8-16 wk |
| 26 | E-01 | ACTIVE | Outer End terrain plus exact chorus/island/gateway population primitives, bounded live chunk-discovery population, deterministic recursive End-city graphs, real template placement, ships, elytra frames, gateway travel, and exact End-city treasure realization are implemented. Live shulkers now cover attachment, peek armor/AABB, target cadence, guided bullets, damage/teleport, drops, sounds, rendering, and capsule state. Dragon resummoning covers crystal validation/consumption, the complete 604-tick ritual, exact ten End spikes, dragon replacement, and abort behavior | arbitrary chunk-load-order RNG/persistence, exhaustive population and shulker edges, exact chorus internal-face culling, dragon-ritual pixels, and End pixel promotion | 6-12 wk |
...
- F-03 now has a complete drinkable-potion vertical slice. The shared
  ...
  Cold JSONL fixtures and Java-authored neutral state capsules can resume a
  bounded default in-flight potion from exact age/pose/motion/thrower-ignore
  state or a default active cloud from every represented lifecycle scalar,
  including owner and player reapplication deadline. A locked real-Java gate
  captures both at a parked pre-tick boundary, translates that snapshot through
  the production capsule emitter, and matches the next native tick at double
  and float bit precision.
  Focused gates pass 378 Java/CPU brewing values, 162 Java/CPU crafting values,
  76 Java/CPU throwable/cloud/effect raw-state values, 389 live/runtime checks, all
  ...
  effect behaviors, cloud particles, custom potion/cloud state, full arbitrary
  NBT persistence, automation faces, and the real-Java potion/GUI pixel promotion
  remain open.
...
- W-01 lava now covers the natural 30-tick Overworld cadence and two
  ...
  fire ignition is also exact for static source lava in a bounded region
  backed by Java-registry `getCanBurn` and `blocksMovement` masks. Both
  positive outer values, the zero branch's three floor probes,
  first-moving-material abort, `doFireTick=false`, every resulting fire
  callback delay, and RNG cursor match across wood, leaves, cloth, TNT, vines,
  and carpet. Other reactions, general loaded-world random-tick iteration,
  broader fire weather, and broader lighting shapes remain queued.
- W-01 falling blocks now cover metadata-0 sand and gravel over a two-cell
  ...
  remain queued.
- W-01 random-tick callback parity covers wheat, carrots, potatoes, beetroot,
  pumpkin/melon stems, cocoa, nether wart, cactus, sugar cane, brown/red
  mushrooms, grass, mycelium, farmland, snow layers, full snow blocks, and
  Overworld ice, controlled frosted ice, and controlled chorus flowers on
  canonical valid support. The capsule and direct locked oracle restore the
  exact internal 48-bit `java.util.Random` seed. Magma matches the light gate,
  fertile-soil weighting, crop-layout penalty, float truncation, beetroot throttle, cocoa
  support/age roll, and mature-stem adjacent-fruit, four-direction,
  target-soil, obstruction, default-state, and RNG ordering. Unsupported
  canonical wheat, carrots, potatoes, and beetroot first construct their exact
  age-dependent item drop, publish air, and then continue `BlockCrops` with the
  stale input state. The locked outcomes cover both retained air and a
  successful age-one same-tick restoration, including beetroot's independent
  `nextInt(3)` throttle. The restored collisionless crop does not lift the new
  EntityItem on its first move. Unsupported canonical pumpkin and melon stems
  likewise run their age-gated `Block.RANDOM` seed-drop body, publish air, and
  continue with the stale stem age. The locked age-zero outcomes each emit one
  species seed and restore age one with exact World/Math/Block RNG and entity-ID
  cursors; the collisionless restored stem does not lift the new EntityItem.
  Unsupported stage-zero saplings likewise publish their species-metadata
  sapling drop and air before continuing `BlockSapling` with the stale input
  state. The locked outcomes cover retained air and a successful same-tick
  stage-one restoration. All six species are direct-tested with exact
  World/Math/Block RNG and entity-ID cursors, and the collisionless restored
  sapling does not lift its new EntityItem. Unsupported stage-one saplings are
  also exact for all six species. Their successful growth roll carries the
  stale source state into generation while retaining the actual air snapshot
  for 2x2 detection. Oak, spruce, birch, jungle, and acacia consume their
  generator-specific draws, reject the invalid soil, and restore stage one;
  dark oak consumes its candidate selection, finds no 2x2 after teardown, and
  remains air. Their item, raw-world, and global-cursor outcomes are locked.
  Unsupported canonical cocoa removes itself
  before constructing one or three brown-dye
  EntityItems, retains the old age for drop count, skips the growth roll, and
  matches World/Math/Block RNG plus entity-ID cursors. Supported nether wart
  retains its exact `nextInt(10)` roll. On unsupported soil it performs that
  growth roll first, then its trailing `BlockBush` callback drops the possibly
  updated age before publishing air. The locked age-zero growth branch emits
  one wart, while mature age three emits four separate wart stacks, with exact
  World/Math/Block RNG and entity-ID cursors. Cactus/reed age and extend
  deterministically, reset at age 15, and enforce the three-block height
  ceiling. Mushrooms retain
  their outer roll, bounded density census, four-step candidate walk, and exact
  placement cursor. Grass retains the dark decay gate and the bright
  four-attempt, twelve-draw dirt spread; mycelium shares those exact gates and
  cursor semantics while placing its own default state. Farmland retains the
  full 9x2x9 water search, open-sky rain predicate, moisture decrement,
  terminal dirt conversion, supported-plant preservation, hydration to seven,
  and no-RNG cursor. Its terminal conversion also reproduces the post-write
  lift for live items, both falling-block representations, represented living
  mobs, boats, XP orbs, projectiles, primed TNT, minecarts, fireworks, fish
  hooks, standalone end crystals, and area-effect clouds. Motion, timers, and
  both living buffers remain intact. The player, dragon arena, and
  unrepresented entity classes remain open. Snow layers preserve all eight
  metadata states through
  block light 11 and become air above that exact threshold without RNG. Full
  snow blocks preserve through stored block light 11; at 12 they become air
  and produce four separate snowball stacks with exact world-RNG offsets,
  Math.random constructor fields, entity IDs, and cursor advancement. Ice
  retains through block light eight; above that exact threshold it becomes
  flowing water and appends the exact priority-zero +5 liquid callback without
  consuming RNG. The ice proof uses the already-promoted flat-stone water
  basin, so that queued callback remains executable within the verified liquid
  subset. In the Nether, the same lit ordinary-ice callback instead publishes
  air with the exact synchronous notifications and no water callback or RNG
  use. Frosted ice consumes its exact outer `nextInt(3)`, preserves or ages
  metadata zero through three against the `8-age` combined-light threshold,
  and schedules the priority-zero successor at `20+nextInt(21)`. At age three,
  sparse face neighbors collapse through Java's old-block notification order;
  a supported dense ring instead retains those neighbors through notification
  and proves the later DOWN/UP/NORTH/SOUTH/WEST/EAST melt pass. In the Nether,
  its inherited `BlockIce.turnIntoWater` path publishes air for the mature
  target and any notification-collapsed sparse neighbors, while the supported
  dense neighborhood follows the explicit six-face pass; none of those melts
  schedules water. Exact block vectors, local queue entries and ranks, events,
  and RNG cursors match.
  Chorus flowers cover ages zero through five on end stone and rooted
  two-/three-deep stems. The exact bound-one RNG advance, depth gate,
  obstruction handling, randomized horizontal directions, vertical and
  lateral plant replacement, terminal death, and ordered 1033/1034 events all
  match over the 150-cell fixture. Vines cover the complete callback: the
  one-in-four gate, 9x3x9 density cutoff, all six directions, upward/downward
  face filtering, both rotated lateral attachments, diagonal and ceiling
  fallback spread, solid-face acquisition, and lower-vine merging match over a
  243-cell fixture. Forge event overrides, invalid-support random-tick teardown
  outside the sapling/crop/stem/wart/cocoa families, and general loaded-world
...
- W-01 skylight now transfers the exact saved Java nibble field at the cold
  ...
  boundary. A second locked gate covers leaf attenuation, water opacity,
  transparent glass, sequential stacked-water edits, opaque/water overhangs,
  exact static-to-flowing wake schedules, and a second local-x15 boundary.
  General multi-column overhang shapes, unloaded boundaries, and deferred gap
  relighting remain active W-01 coverage.
...
- `melee_cooldown_70`: implemented; fixed target and physical-click tape,
  ...
  and exact removal.
- `player_critical_locked`: implemented; 115 direct real-Java cases cover the
  complete represented critical predicate, raw target-health, motion, fire,
  sweep-neighbor health, player attack sounds, and hurt/death audio for every
  represented living target across slime/magma sizes 1, 2, and 4,
...
- `crop_random_tick`: controlled callbacks are implemented for saplings,
  wheat, carrots, potatoes, beetroot, pumpkin/melon stems, cocoa, nether wart,
  cactus, sugar
  cane, brown/red mushrooms, grass, mycelium, and farmland, including exact
  `java.util.Random` cursor consumption, deterministic plant-column
  aging/extension, and bounded mushroom density/walk placement; full
  loaded-chunk selection and invalid-support drop/item edges outside the
  sapling/crop/stem/wart/cocoa families remain. Unsupported canonical crops are exact
  through drop-first teardown and possible stale-state age-one restoration;
  unsupported pumpkin/melon stems are exact through age-gated seed drops,
  air-first teardown, stale-state regrowth, and all global cursors;
  unsupported nether wart is exact through growth-first state replacement,
  updated-age drop selection, one/four item construction, and final air;
  unsupported stage-zero saplings are exact through drop-first teardown,
  possible stale-state stage-one restoration, all six species, and all global
  cursors;
  unsupported stage-one saplings are exact through drop-first teardown,
  stale-state generator selection/failure, five restored species, dark-oak
  retained air, and all global cursors;
  unsupported cocoa is exact through air-first notification, one/three
  brown-dye item construction, and all global cursors.
...
- `grass_random_tick`: controlled decay and bright four-attempt spread are
  implemented with exact light/opacity gates, selected cells, and all twelve
  RNG draws; ordinary loaded-chunk selection remains.
- `mycelium_random_tick`: controlled decay and bright four-attempt spread are
  implemented with exact dirt-variant, light/opacity, selected-cell, and RNG
  outcomes; ordinary loaded-chunk selection remains.
- `farmland_random_tick`: controlled water/rain hydration, moisture decrement,
  supported-plant preservation, and terminal dirt conversion are implemented
  with an exact 162-state search/result volume and unchanged RNG cursor. The
  dirt-conversion callback also lifts intersecting live EntityItems and falling
  blocks to the new full-cube top without changing motion; loaded-chunk
  selection and lifting for the other represented entity stores remain.
- `vine_random_tick`: complete controlled callback implemented, including
  no-roll, exact density scan, all direction classes, face filtering, lateral
  spread fallbacks, solid-face acquisition, lower merge, and density
  suppression. Neighbor-driven support rechecks now prune unsupported faces,
  retain matching inherited faces, break metadata-zero states, and cascade
  through three-vine columns; loaded-chunk selection remains.
- `snow_layer_random_tick`: all eight metadata states retain at block light 11
  and melt above 11 with an unchanged RNG cursor; loaded-chunk selection
  remains.
- `leaf_decay`: implemented for the complete canonical callback and ordinary
  break lifecycle. The four-round face-connected 9-cube scan supports both log
  and leaf families, rejects diagonal-only and distance-five logs, preserves
  nondecayable leaves, and emits species-correct sapling/apple items with exact
  World, Math, Block, and entity-ID cursors. Log removal marks the surrounding
  9-cube, leaf removal marks the surrounding 3-cube, and decay sends the exact
  flag-three support notifications. General loaded-chunk selection and
  unloaded-boundary behavior remain open.
...
- `sky_light_column`: implemented for 13 parked mixed-opacity and sequential
  edit rows. Opaque stone, leaves, static/flowing water, and glass cover add,
  remove, stacked-column, under-overhang, and local-x15 chunk-edge outcomes.
  The gate compares 75,504 raw block/skylight scalar values, exact pre/post
  pending queues, and every controlled cursor. General multi-column overhang
  shapes, unloaded boundaries, and deferred gap relighting remain open.
- `static_lava_neighbor`: implemented for 58 parked lifecycle rows. Fifteen
  ordinary-neighbor rows show that static lava retains every level, ignores
  water below, and converts to flowing
  lava with the exact Overworld/End +30 or Nether +10 callback when mixing
  does not complete, and forms obsidian at level zero or cobblestone at levels
  one through four
  beside either water block. Mixing also matches nested static-water wakeups,
  the two World RNG draws for sound pitch, all 16 Math RNG draws for eight
  large-smoke positions, and the exact sound/particle event streams. The live
  renderer implements the vanilla large-smoke motion, scale ramp, lifetime,
  texture strip, and lightmap using deterministic private visual entropy.
  Forty-three direct dynamic rows additionally lock Nether/End decay one/two,
  cadence 10/30, queue order, and the four-step/two-step slope-search split on
  a shaped air/stone floor. Exact `nextInt(4)` slowdowns produce +40/+120
  schedules, and two neighboring lava sources retain Forge's default no-source
  result. Nether/End downward air flow produces falling level eight and exact
  child/source queues. Horizontal flow into fire replaces it with exact
  extinguish effects, queues, and final RNG cursors in both dimensions.
  Horizontal static/flowing water locks source-to-obsidian reentrancy, two
  ordered effect streams, static water's stale +5 queue, and child lava.
  Downward water contact locks stone, one effect stream, and the source
  requeue. An overlapping approximate-CA region is retired when an exact
  scheduled callback owns the cells; remote live-fluid regions remain active.
  Valid sapling, web, floor torch, snow layer, wall vine, and carpet targets
  additionally match exact replacement effects and queues. A supported ladder
  remains unchanged, locking Java's explicit `isBlocked` exception. Ordinary
  rail, an unpowered powered-rail state, unpowered redstone wire, tall grass,
  age-three wheat, dead bush, brown mushroom, and lit floor redstone torch now
  match too. Powered rail locks the break-hook center-before-child queue order;
  dead bush locks its sand support's +2 falling callback. A valid supported
  reed stays unchanged under the separate explicit liquid exception. Both
  flowers, red mushroom, pumpkin/melon stems, waterlily, nether wart, carrots,
  potatoes, and beetroot also match. Waterlily locks the support-water
  conversion and its exact +5 queue before the child/source lava callbacks.
  Remaining metadata, circuit/plant/material families, broader topology, and
  unloaded-boundary dispatch remain open.
- `plant_support_neighbor`: implemented for 65 parked flag-three neighbor
  rows. Nineteen rows cover exact support predicates and registered destroy
  payloads for saplings, tall grass, dead bush, flowers, mushrooms, crops,
  stems, waterlily, nether wart, reeds, cactus, and double plants. Thirty-six
  exhaustive metadata rows cover all eight snow layers, all sixteen carpet
  colors, and all twelve canonical cocoa states. Ten positive and negative
  controls lock snow support on stone, leaves, and a full snow layer versus
  ice, packed ice, and glass; carpet over static water; and cocoa on jungle
  versus oak logs. The gate compares 4,680 pre/post raw states, complete
  EntityItem descriptors, and World, Math, Block, and entity-ID cursors. It
  locks synchronous child teardown, callback order, exact drops, and cursor
  consumption without a world tick. Loaded-chunk selection, unloaded
  boundaries, and unrepresented support families remain open.
...
- `note_block_edges_and_interaction`: implemented with a 12-row parked
  real-Java/native comparator. It covers no-power, rising, stable-high and
  falling edges; blocked-above suppression; wood, sand, glass, iron, and
  default material instruments; note wraparound and blocked right-click
  tuning; and left-click playback. Tile pitch and prior-power state,
  sound/category/volume/pitch bits, NOTE particle payload bits, and unchanged
  World/Math RNG cursors are exact. Save-state capture and restore include the
  bounded note-tile pool. Live audio and the six-tick NOTE particle use owned
  assets and fixed storage. Broader pixel-scene promotion remains open.
...
- G-03 moved from `QUEUED` to `ACTIVE`. Exact four-facing desert pyramids,
  ...
  Igloos now also have exact template generation, loot, represented resident
  state, and distinct zombie-villager models/skins. Monuments, mansions,
  runtime witch lifecycle, active zombie-villager conversion, and complete
  generated block/entity pixel promotion remain open.
...
## 2026-08-08 player attack-particle slice

The represented attack outcome now exports the four Java combat-particle
classes through the existing allocation-free current-tick stream. The locked
real-Java fixture grew from 60 to 67 cases and matches critical id 9,
enchantment-critical id 10, sweep id 45, and damage-indicator id 44, including
event order, count, long-distance flag, attached target identity and geometry,
and every raw spawn argument bit. A one-health lethal sword target proves that
damage-indicator count follows clamped pre/post health loss and suppresses the
packet. Rejected and ordinary strong attacks retain an empty stream.

The interactive fixed pool implements the vanilla critical, magic-critical,
damage-indicator, and sweep constructors and updates. Attached critical
emitters use a fixed 16-slot pool, damage packets expand into the bounded
1,024-slot particle pool, and sweep uses the byte-exact `sweep.png` asset plus
its layer-3 fullbright quad. Both interactive and headless capture render
paths consume the same formulas. Java's constructor-only `Particle.rand`,
`Math.random()`, and emitter randomness come from unsaved wall-clock/global
state, so native uses a deterministic visual cursor and leaves exact
per-particle placement, color, and pixel output outside this gate.

Focused Java/native descriptor, runtime order, pool lifecycle, asset, render,
and clean product gates pass. The full aggregate passes in 6:45.89 at 450,356
KiB peak RSS with zero major faults or swap. The isolated CPU guard passes at
4,004 scalar steps/s against the 3,858.9 floor and is 1.4% below the 4,062
reference baseline. All 13 locally available quick-sweep stages pass; the two
snapshot-backed Blaze
stages skip because their `.bsnp` inputs are absent. The villager-trade and
asset-bootstrap helpers now use the pinned repo-local Gradle cache, eliminating
an unrelated legacy asset-download retry path. GPU 1 was not executed.

## 2026-08-08 player target hurt/death audio slice

The locked attack fixture now seeds target-local `Entity.rand` and covers 79
real-Java cases. Pig, cow, sheep, and chicken fresh hits match hurt versus
death identity, target coordinates, NEUTRAL category, raw pitch bits, and
cow's 0.4 volume override. Sprint knockback precedes the target sound, while
critical/strong/sweep follows it; a sweep's primary and neighbor sounds retain
entity iteration order. Rejected hits and accepted damage deltas inside the
existing hurt-resistant window correctly emit no target sound.

Native fresh-hit feedback uses the existing bounded mob event stream and the
runtime drains the attack-local rows at Java's `attackEntityFrom` seam. This
adds no allocation, no new idle scan, and no audio work to headless/RL paths.
The focused native runtime gate passes 15 cases; player-effect,
combat-particle, product, clean Java, and 146-event/469-variant OpenAL gates
also pass.

The exact-current full native aggregate passes in 5:34.62 at 450,204 KiB peak
RSS with zero major faults or swap. The stopped-oracle CPU guard passes at
4,420 steps/s against the 3,858.9 floor and exceeds the 4,062 baseline. All 13
locally available quick-sweep stages pass; only the two absent-snapshot Blaze
stages skip. GPU 1 remained shared and was not executed. Hostile and other
target sound families remain the next attack-audio expansion.

## 2026-08-08 all represented living target-audio slice

The locked player-attack fixture now covers 115 real-Java cases. Thirty-six
new rows pin hurt and death audio for zombies, pigmen, skeletons, wither
skeletons, creepers, spiders, cave spiders, endermen, blazes, ghasts,
silverfish, villagers, and slime/magma sizes 1, 2, and 4. They match sound
identity, target position, HOSTILE versus NEUTRAL category, private-RNG pitch
bits, ghast's 10.0 volume, slime/magma's `0.4 * size` volume and small/big
selection, plus the pigman's two pre-damage anger/sound-delay RNG draws.
Size-one slime and magma hurt use a weak nonlethal player hit; lethal cases
separately prove the death event.

Native fresh-hit feedback maps the complete represented living roster through
the existing fixed mob and runtime sound rings. The extended exact fixture
supports size 1/2/4 without changing ordinary spawn cost, and the corrected
spider max-health constant is 16 rather than 20. No allocation, idle scan, or
headless/RL playback work was added. The focused runtime passes 50 cases;
player-effect, combat-particle, Java, product, and 176-event/557-variant OpenAL
gates also pass.

The full native aggregate passes in 5:31.17 at 450,612 KiB peak RSS with zero
major faults or swap. The stopped-oracle CPU guard passes at 4,635 steps/s
against the frozen 3,858.9 floor and exceeds the 4,062 baseline. All 13
locally available quick-sweep stages pass; only the two absent-snapshot Blaze
stages skip. GPU 1 remained shared and was not executed. Thorns,
player-target velocity acknowledgement, statistics, and criteria remain the
open direct-attack effects.

## 2026-08-08 returning splash-potion player-collision slice

Player-thrown splash potions now model the missing `EntityThrowable` player
collision boundary. The fixed projectile state retains nearby-thrower
suppression, the exact ignore countdown 2, 1, 0 and release at -1, then selects
the closest returning player or represented mob segment hit. A direct player
hit uses splash factor 1.0 and feeds the existing hurt-immunity state.

A locked real-Java/native oracle matches all five transitions, impact removal,
Strong Harming health bits, `hurtTime=10`, and `hurtResistantTime=20`. The
native brewing/runtime suite passes 389 checks; Java and product builds pass.
The full aggregate passes in 5:21.18 at 450,764 KiB peak RSS with zero major
faults or swap. The stopped-oracle CPU guard passes at 4,750 steps/s against
the frozen 3,858.9 floor and exceeds the 4,062 baseline. The active-throwable
work is fixed-size and allocation-free. Broader multi-entity ignore ordering
remains open. All 13 locally available quick-sweep stages pass; only the two
absent-snapshot Blaze stages skip. GPU 1 was not executed.

## 2026-08-09 invalid-support crop random-tick slice

Unsupported wheat, carrots, potatoes, and beetroot now follow the exact
`BlockBush` plus `BlockCrops` ordering. They construct the age-dependent item
drop before replacing the cell with air, then continue the growth body with
the stale input state. A successful roll can therefore restore age one in the
same callback. Beetroot retains its earlier `nextInt(3)` throttle. The live
item integrator also treats every crop and stem as collisionless, so a crop
restored around its new drop does not snap that EntityItem to the block top.

Five locked real-Java cases pass all 27 state features, exact EntityItem and
World/Math/Block RNG cursors, and all 10,625 raw cells: wheat retained-air,
wheat regrowth, carrot regrowth, potato regrowth, and beetroot regrowth
(`trace/out/matrix_invalid_crop_family_final/summary.md`). The
focused runtime and randtick tests pass. The full product gate passes in
7:56.79 at 1,100,936 KiB peak RSS with zero swap, and the quick sweep passes
all 13 locally available stages with only the two known absent-snapshot skips.
The stopped-oracle CPU guard passes at 4,799 steps/s against the frozen 3,858.9
floor and exceeds the 4,062 baseline. GPU 1 was not executed.

## 2026-08-09 invalid-support stem random-tick slice

Unsupported pumpkin and melon stems now preserve Java's non-returning
`BlockBush` plus `BlockStem` callback order. The unsupported age-zero stem
uses static `Block.RANDOM` for three age-gated seed trials, publishes air and
its notifications, then continues the growth body with the stale age. With
the locked cursors, each species emits one seed EntityItem and restores its
age-one stem in the same callback. The new item then takes its collisionless
first physics step without being lifted by the restored stem.

Two locked real-Java/native cases pass all 27 state features, exact pumpkin or
melon seed entity state, exact World/Math/Block RNG and entity-ID cursors, and
all 10,625 raw cells
(`trace/out/matrix_invalid_stem_post_fix/summary.md`). A consolidated rerun of
those two plus the five crop cases passes 7/7 under the generalized cursor
checker (`trace/out/matrix_invalid_crop_stem_final/summary.md`). The focused
runtime and randtick tests pass. The full product gate passes from fresh spawn
through credits in 8:29.90 at 1,100,916 KiB peak RSS with zero major faults
and zero swap. The quick sweep passes all 13 locally available stages with
only the two known absent-snapshot skips. The stopped-oracle CPU guard passes
at 4,336 steps/s against the frozen 3,858.9 floor and exceeds the 4,062
baseline. GPU 1 was not executed.

## 2026-08-09 invalid-support nether-wart random-tick slice

Nether wart now preserves Java's distinct `BlockNetherWart` ordering on bad
support. Ages below three consume the growth `nextInt(10)` and may publish the
next age first. The trailing `BlockBush.updateTick` then selects drops from
that possibly updated state before replacing the cell with air. The locked
age-zero case therefore grows to age one, emits one wart, and ends as air; the
locked mature case skips growth, emits four separate wart stacks, and ends as
air.

Two locked real-Java/native cases pass all 27 state features, exact item
states and EIDs, exact World/Math/Block RNG cursors, and all 10,625 raw cells
(`trace/out/matrix_invalid_wart_post_fix/summary.md`). The generalized checker
also passes all nine crop, stem, and wart cases together
(`trace/out/matrix_invalid_growth_family_final/summary.md`). The focused
runtime and randtick gates pass; runtime takes 5:36.48 at 484,560 KiB peak RSS
with zero major faults and zero swap. The full product gate passes from fresh
spawn through credits in 7:45.83 at 1,100,268 KiB peak RSS, also with zero
major faults and zero swap. The quick sweep passes all 13 locally available
stages with only the two known absent-snapshot skips. The stopped-oracle CPU
guard passes at 4,730 steps/s against the frozen 3,858.9 floor and exceeds the
4,062 baseline. GPU 1 was not executed.

## 2026-08-09 invalid-support stage-zero sapling random-tick slice

Unsupported stage-zero saplings now preserve Java's non-returning
`BlockBush` plus `BlockSapling` callback order. They construct one sapling
EntityItem with the original species metadata, publish air and its
notifications, then continue the light and `nextInt(7)` growth body with the
stale state. The locked outcomes either retain air or restore that sapling at
stage one in the same callback. The item integrator now also classifies
saplings as collisionless, so the restored sapling cannot lift its new item on
the first physics step.

Two locked real-Java/native cases pass all 27 state features, exact item state
and EID, exact World/Math/Block RNG cursors, and all 10,625 raw cells
(`trace/out/matrix_invalid_sapling_final/summary.md`). The first corrected
matrix already matched the world and cursors but caught a 0.554504-block item
Y error; that evidence drove the collision classification fix. Direct runtime
coverage locks both outcomes for all six sapling species. The focused runtime
gate passes in 5:58.21 at 480,352 KiB peak RSS with zero major faults and zero
swap; randtick passes in 0.38 seconds at 51,884 KiB. The full product gate
passes from fresh spawn through credits in 8:00.87 at 1,100,900 KiB peak RSS
with one ordinary demand-page fault and zero swap. The quick sweep passes all
13 locally available stages in 57.17 seconds with only the two known
absent-snapshot skips. The stopped-oracle CPU guard passes at 4,868 steps/s
against the frozen 3,858.9 floor and exceeds the 4,062 baseline. GPU 1 was not
executed. Unsupported stage-one saplings were the next explicit open boundary.

## 2026-08-09 invalid-support stage-one sapling random-tick slice

Unsupported stage-one saplings now carry Java's stale input state across the
earlier `BlockBush` drop and air publication into `BlockSapling.generateTree`.
The generator receives that stale state for species and failure restoration,
but reads the actual post-teardown air snapshot when checking four possible
2x2 layouts. Oak, spruce, birch, jungle, and acacia therefore consume their
distinct candidate/height draws, reject stone support, and restore their
stage-one sapling. Dark oak consumes the common `nextInt(10)`, finds no 2x2,
returns before restoration, and leaves air. Every species creates exactly one
matching sapling EntityItem before that generator work.

Six locked real-Java/native cases pass all 27 state features, exact item state
and EID, exact World/Math/Block RNG cursors, and all 10,625 raw cells
(`trace/out/matrix_invalid_sapling_stage_one_final/summary.md`). A consolidated
run passes all eight stage-zero and stage-one sapling cases together
(`trace/out/matrix_invalid_sapling_all_stages_final/summary.md`). Direct runtime
coverage locks all six stage-one species in addition to the existing twelve
stage-zero outcomes. The focused runtime gate passes in 5:47.86 at 483,412 KiB
peak RSS with zero major faults and zero swap; randtick passes in 0.40 seconds
at 51,840 KiB. The full product gate passes from fresh spawn through credits
in 8:03.77 at 1,101,204 KiB peak RSS with zero major faults and zero swap. The
quick sweep passes all 13 locally available stages in 54.64 seconds with only
the two known absent-snapshot skips. The stopped-oracle CPU guard passes at
4,797 steps/s against the frozen 3,858.9 floor and exceeds the 4,062 baseline.
The added generator path runs only after an already-selected unsupported
stage-one sapling wins the 1-in-7 growth roll; it adds no idle scan or
allocation. GPU 1 was not executed.

## 2026-08-09 Nether ice vaporization slice

Ordinary ice and frosted ice now follow `WorldProvider.doesWaterVaporize()` in
the Nether. Lit ordinary ice publishes air through the ordinary synchronous
notification path without creating or scheduling water. Mature frosted ice
uses its inherited `BlockIce.turnIntoWater`, so the target and any sparse
neighbors removed by old-block notification also become air. A supported
dense neighborhood survives that notification and is handled by the later
six-face melt pass. No Nether melt creates a liquid callback.

Four new locked rows cover ordinary ice, isolated age-three frosted ice,
sparse adjacent age-three frosted ice, and the supported dense age-three
neighborhood. The complete parked real-Java/native gate passes 373/373 rows in
36.73 seconds at 30,252 KiB peak RSS, comparing final block state, the eleven-
cell frosted neighborhood where applicable, exact queue contents and ranks,
measured light, events, and RNG cursors. The focused random-tick gate passes in
0.46 seconds at 51,856 KiB. The exhaustive native runtime passes in 5:54.08 at
485,168 KiB peak RSS. The full product aggregate passes from launch through
the fresh-spawn-to-credits route in 8:00.87 at 1,100,912 KiB peak RSS, with
zero major faults and zero swap. The quick sweep passes all 13 locally
available stages in 55.35 seconds with only the two known absent-snapshot
skips. The stopped-oracle CPU guard passes at 4,897 steps/s against the frozen
3,858.9 floor and exceeds the 4,062 baseline. The production changes execute
only inside an already-selected ice melt callback and add no idle scan or
allocation. GPU 1 was not executed.

## 2026-08-09 farmland dirt-conversion entity-lift slice

`BlockFarmland.turnToDirt` now publishes default dirt and then lifts each
intersecting entity in the represented active stores to the new full-cube
collision top, matching Java's post-write query and `setPosition` order. The
exact set covers live EntityItems, the product and compatibility falling-block
stores, represented living mobs, boats, XP orbs, every represented projectile,
primed TNT, minecarts, fireworks, fish hooks, standalone end crystals, and
area-effect clouds. Motion, timers, history, and both living buffers remain
unchanged. A minimum Y already above the dirt box is not moved. Dry random
ticks and solid-above neighbor conversion share the same helper. At this
checkpoint, player, dragon-arena, and unrepresented entity classes remained
explicit open edges.

Twelve locked occupant rows extend the complete plant callback matrix to
385/385 exact real-Java/native outcomes in 35.42 seconds at 30,252 KiB peak
RSS, with zero major faults and zero swap. They compare final relative Y plus
the existing block, event, liquid-queue, RNG, and entity-ID outputs. Direct
native coverage additionally locks motion and timer preservation,
living-buffer synchronization, the actual product falling-block AABB, and
strict just-above nonintersection. The Java 8 build passes. The exhaustive
native runtime passes in 5:02.39 at 256,716 KiB peak RSS with zero major faults
and zero swap. The full product aggregate passes from launch through
fresh-spawn-to-credits in 7:51.65 at 1,101,204 KiB peak RSS, also with zero
major faults and zero swap. CPU throughput passes at 4,803 steps/s against the
frozen 3,858.9 floor and exceeds the 4,062 baseline. All 13 locally available
quick-sweep stages pass in 55.49 seconds at 716,960 KiB peak RSS with zero
swap; the two absent-snapshot Blaze stages skip. The entity scans run only
after farmland has already converted, add no idle-path work or allocation,
and GPU 1 was not executed.

The represented authoritative server player is now included. A thirteenth
locked occupant row moves the real parked `EntityPlayerMP` between chunk-list
sections without ticking it, proves Java's full-cube query lifts it to relative
Y 1.0, and restores its pose, bounding box, motion, gravity flag, interpolation
history, rotation history, and chunk membership. Magma moves only the matching
server pose at this callback boundary; client prediction remains independent.
The complete plant gate passes 386/386 rows in 35.14 seconds at 30,252 KiB
peak RSS with zero major faults and zero swap. Direct native coverage also
locks preserved player motion and strict just-above nonintersection. The
remaining represented-store edge is the dragon arena; unrepresented entity
classes remain explicit. The Java 8 build passes. The exhaustive native
runtime passes in 5:41.77 at 491,608 KiB peak RSS with zero major faults and
zero swap. The full product aggregate passes through fresh spawn and credits
in 7:37.70 at 1,100,920 KiB peak RSS, also with zero major faults and zero
swap. CPU throughput passes at 4,970 steps/s against the frozen 3,858.9 floor
and exceeds the 4,062 baseline. All 13 locally available quick-sweep stages
pass in 52.52 seconds at 716,792 KiB peak RSS with zero swap; the two known
absent-snapshot Blaze stages skip. GPU 1 was not executed.

The End dragon arena now completes the represented active-store set for this
callback. Two further locked rows stage a real untracked `EntityDragon` and
`EntityEnderCrystal` in Java's End without ticking them, then compare the
post-conversion position and all existing block, event, RNG, and entity-ID
outputs. The complete plant gate passes 388/388 rows in 34.95 seconds at
30,252 KiB peak RSS with zero major faults and zero swap. Direct native
coverage locks the dragon parent's exact 16-by-8 box, the crystal's exact
2-by-2 box, strict just-above nonintersection, and preservation of dragon
motion, head state, and target state. Only entity classes absent from the
product representation remain outside the lift contract. The Java 8 build
passes. The exhaustive native runtime passes in 5:42.22 at 492,568 KiB peak
RSS with zero major faults and zero swap. The full product aggregate passes
through fresh spawn and credits in 7:45.34 at 1,101,164 KiB peak RSS, also
with zero major faults and zero swap. CPU throughput passes at 4,917 steps/s
against the frozen 3,858.9 floor and exceeds the 4,062 baseline. All 13
locally available quick-sweep stages pass in 52.56 seconds at 714,060 KiB
peak RSS with zero swap; the two known absent-snapshot Blaze stages skip. The
arena checks remain conversion-callback-only and add no idle scan or
allocation. GPU 1 was not executed.

## 2026-08-09 grass-path solid-above callback slice

Grass paths now execute Java's shared `BlockGrassPath.updateBlockState` rule
from both `onBlockAdded` and `neighborChanged`. A path below a solid material
becomes default dirt through the ordinary flag-three notification boundary;
a non-solid material leaves it unchanged. Unlike farmland, this transition
does not relocate intersecting entities.

Three new locked cases cover solid-above placement, non-solid-above placement,
and an independent adjacent-block removal that triggers the existing path.
The complete plant/attachment callback gate passes 68/68 exact real-Java/native
rows in 13.83 seconds at 30,252 KiB peak RSS, comparing the raw 3-by-4-by-3
volume, item states, World/Math/Block RNG cursors, and entity-ID cursor. Direct
native coverage locks both entry points and cursor preservation. The Java 8
build passes. The exhaustive native runtime passes in 5:44.83 at 494,140 KiB
peak RSS with zero major faults and zero swap. The full product aggregate
passes through fresh spawn and credits in 7:39.64 at 1,100,904 KiB peak RSS,
also with zero major faults and zero swap. CPU throughput passes at 4,972
steps/s against the frozen 3,858.9 floor and exceeds the 4,062 baseline. All
13 locally available quick-sweep stages pass in 52.49 seconds at 716,812 KiB
peak RSS with zero swap; the two known absent-snapshot Blaze stages skip. The
callback adds no idle scan or allocation. GPU 1 was not executed.

## 2026-08-09 frosted-ice source-aware neighbor slice

Ordinary notifying block edits now retain the changed block's old identity for
neighbor callbacks. This closes `BlockFrostedIce.neighborChanged`: removing a
frost block collapses an adjacent frost state when fewer than two frost
neighbors remain, while an otherwise identical stone removal is ignored. The
collapse publishes flowing water and schedules its exact priority-zero update
five ticks later. A target with two remaining frost neighbors stays frozen.

The pre-fix oracle rejected the sparse row because Java produced block state
128 and schedule `[0,1,0,8,5,0,0]` while native retained state 3392 and no
schedule. After the fix, the complete plant/attachment callback gate passes
71/71 exact real-Java/native rows in 13.81 seconds at 30,252 KiB peak RSS. The
three new rows compare the raw 3-by-4-by-3 volume, exact pending queue and rank,
items, World/Math/Block RNG cursors, and entity-ID cursor. Direct native tests
also lock the source-type negative and cursor preservation. The Java 8 build
passes. The exhaustive native runtime passes in 5:21.97 at 493,684 KiB peak
RSS with zero major faults and zero swap. The full product aggregate passes
through fresh spawn and credits in 7:25.03 at 1,100,908 KiB peak RSS, also
with zero major faults and zero swap. CPU throughput passes at 4,996 steps/s
against the frozen 3,858.9 floor and exceeds the 4,062 baseline. All 13 locally
available quick-sweep stages pass in 52.21 seconds at 716,752 KiB peak RSS
with zero swap; the two known absent-snapshot Blaze stages skip. The source
identity is carried only through an already-triggered six-neighbor loop and
adds no idle scan or allocation. GPU 1 was not executed.

## 2026-08-09 dry-sponge bounded absorption slice

Dry sponge placement and ordinary neighbor callbacks now execute Java's exact
bounded `BlockSponge.absorb` breadth-first walk. The implementation visits
DOWN, UP, NORTH, SOUTH, WEST, and EAST; admits water through depth seven; tests
the over-64 stop only after completing a node; notifies every removed position
while the sponge is still dry; then publishes wet metadata and event
2001/data 9. It uses fixed local storage only when called, with no idle scan or
allocation.

Five new locked rows cover dry placement, a wet nonabsorbing control,
unrelated-neighbor triggering, adjacent-water addition, and a full 5-by-4-by-5
dense boundary. The dense row starts with 99 static-water cells and finishes
with 65 removed cells, 26 flowing boundary cells, eight retained static cells,
one wet sponge, and all 26 exact priority-zero +5 queue entries in rank order.
The pre-fix sparse row instead retained and woke the water, left the sponge dry,
and omitted the event. The oracle work also corrected the pending-update
fixture's positive-edge exclusion and drains fresh-client entity packets before
injecting process-global cursors.

The complete callback gate passes 76/76 exact real-Java/native rows in 15.73
seconds at 30,252 KiB peak RSS with zero major faults and zero swap. It compares
raw volumes, items, pending queue/rank, world events, World/Math/Block RNG
cursors, and the entity-ID cursor. The Java 8 build passes. The exhaustive
native runtime passes in 4:35.59 at 255,632 KiB peak RSS with zero major faults
and zero swap. The full product aggregate passes through fresh spawn and
credits in 7:25.99 at 1,101,272 KiB peak RSS, also with zero major faults and
zero swap. CPU throughput passes at 5,008 steps/s against the frozen 3,858.9
floor and exceeds the 4,062 baseline. All 13 locally available quick-sweep
stages pass in 52.14 seconds at 716,780 KiB peak RSS with zero major faults and
zero swap; the two known absent-snapshot Blaze stages skip. GPU 1 was not
executed.

## 2026-08-09 Nether-portal frame invalidation slice

Portal blocks now execute the bounded `BlockPortal.neighborChanged` frame
validation for both X and Z axes. The validator reproduces Java's downward
origin search, floor-edge scan, width 2-through-21 and height 3-through-21
bounds, side/top obsidian checks, and full-interior portal count. Invalid cells
become air and synchronously notify their neighbors, so one frame or interior
break collapses the complete connected portal without queued work or RNG.

Five new locked rows cover valid minimum X/Z frames under unrelated edits,
side-frame removal on both axes, and removal of one X-axis interior cell. The
pre-fix side-removal row left all six state-1441 cells in place; Java changed
all six to air. After the fix, the complete callback gate passes 81/81 exact
real-Java/native rows in 63.53 seconds at 30,252 KiB peak RSS with zero major
faults and zero swap. It compares the full 5-by-5-by-5 raw volume, pending
queue/rank, events, items, World/Math/Block RNG cursors, and entity-ID cursor.
The fixture now rejects and retries only a row contaminated by asynchronous
client entity packets before accepting the exact global cursor comparison.

The Java 8 build and dedicated minimum/maximum frame tests pass. The exhaustive
native runtime passes in 5:24.64 at 500,756 KiB peak RSS with zero major faults
and zero swap. The full product aggregate passes through fresh spawn and
credits in 7:28.61 at 1,101,072 KiB peak RSS, also with zero major faults and
zero swap. CPU throughput passes at 4,969 steps/s against the frozen 3,858.9
floor and exceeds the 4,062 baseline. All 13 locally available quick-sweep
stages pass in 52.99 seconds at 716,828 KiB peak RSS with zero swap; the two
known absent-snapshot Blaze stages skip. The validator runs only for an
already-triggered portal neighbor callback and scans at most the legal bounded
frame. GPU 1 was not executed.

## 2026-08-09 fire/cake/torch/ladder callback bundle

Four additional ordinary neighbor callback classes now execute their bounded
1.11.2 behavior. Fire without an opaque floor becomes air unless one of its
six face neighbors can catch fire. Canonical cake states zero through six
become air without a drop when the material below is not solid. Ordinary floor
and wall torches and north/east ladders test the exact attachment face, emit
their registered item before the flag-three air transition, and preserve the
complete EntityItem constructor state. Malformed cake metadata remains
unchanged under the existing atomic-rejection contract.

Eight new rows cover unsupported and flammable-retained fire, unsupported and
unrelated-edit cake, floor and wall torch drops, a north ladder drop, and an
east ladder retention control. The first pre-fix row had Java air while native
retained raw age-seven fire state 823; every queue, event, item, RNG, and EID
field otherwise matched. After the fix, the complete callback gate passes
89/89 exact real-Java/native rows in 68.56 seconds at 30,252 KiB peak RSS with
zero major faults and zero swap. The torch and ladder rows compare exact item
position/motion, yaw and hover-start bits, World/Math/Block RNG cursors, and
entity-ID allocation. The client-drain retry now catches only the fixture's
explicit EID-contamination error and rethrows every other oracle failure.

The Java 8 build passes. The exhaustive native runtime passes in 5:46.88 at
502,756 KiB peak RSS with zero major faults and zero swap. The full product
aggregate passes through fresh spawn and credits in 7:45.57 at 1,101,144 KiB
peak RSS, also with zero major faults and zero swap. CPU throughput passes at
4,875 steps/s against the frozen 3,858.9 floor and exceeds the 4,062 baseline.
All 13 locally available quick-sweep stages pass in 53.25 seconds at 716,728
KiB peak RSS with zero swap; the two known absent-snapshot Blaze stages skip.
All new work is entered only from an already-triggered neighbor callback and
uses fixed existing state. GPU 1 was not executed.

## 2026-08-09 standing/wall-sign callback slice

Standing and wall signs now execute their exact bounded support-loss callbacks.
Standing signs test whether the material below is solid. Wall metadata two
through five maps north, south, west, and east facings to the opposite
attachment cell. An unsupported sign emits item 323 before the flag-three air
transition; supported signs and unrelated edits retain their raw state.

Four new rows cover a standing sign drop and retention control plus a north
wall-sign drop and east wall-sign retention control. The first pre-fix row had
Java change raw state 1015 to air and emit item 323 while native retained state
1015 with no item or cursor advance. After the fix, the complete callback gate
passes 93/93 exact real-Java/native rows in 73.57 seconds at 30,252 KiB peak
RSS with zero major faults and zero swap. It compares the full 5-by-5-by-5 raw
volume, pending queue/rank, events, complete EntityItem state, World/Math/Block
RNG cursors, and entity-ID cursor.

The Java 8 build and direct native callback tests pass. The exhaustive native
runtime passes in 5:23.70 at 502,312 KiB peak RSS with zero major faults and
zero swap. The full product aggregate passes through fresh spawn and credits
in 7:31.25 at 1,100,092 KiB peak RSS, also with zero major faults and zero
swap. CPU throughput passes at 4,965 steps/s against the frozen 3,858.9 floor
and exceeds the 4,062 baseline. All 13 locally available quick-sweep stages
pass in 53.11 seconds at 716,824 KiB peak RSS with one major fault and zero
swap; the two known absent-snapshot Blaze stages skip. The callback adds no
idle scan or allocation. GPU 1 was not executed.

## 2026-08-09 default-banner/empty-flower-pot callback slice

Default standing and wall banners now execute the same material-solid support
rules as their sign counterparts and emit banner item 425 before air. Empty
flower pots use Java's separate fully-opaque support predicate, emit pot item
390, retire the native tile record, and reproduce the measured extra World RNG
float after the otherwise exact item construction. Valid support and unrelated
edits retain each block. Occupied pots keep using the previously verified tile
payload path; this direct callback gate covers the empty pot.

Six new rows cover standing and wall banner drop/retention pairs plus empty-pot
drop and retention. The first pre-fix row had Java change raw standing-banner
state 2823 to air and emit item 425 while native retained state 2823 with no
item or cursor advance. The first empty-pot candidate matched every raw block
and EntityItem field but stopped its World RNG at 138559663298432; Java advanced
once more to 158644051296651. After both fixes, the complete callback gate
passes 99/99 exact real-Java/native rows in 75.37 seconds at 30,252 KiB peak
RSS with zero major faults and zero swap. It compares the full 5-by-5-by-5 raw
volume, pending queue/rank, events, complete EntityItem state, all RNG cursors,
and the entity-ID cursor.

The Java 8 build and direct native callback tests pass. The exhaustive native
runtime passes in 5:22.24 at 503,008 KiB peak RSS with zero major faults and
zero swap. The full product aggregate passes through fresh spawn and credits
in 7:38.78 at 1,101,156 KiB peak RSS, also with zero major faults and zero
swap. CPU throughput passes at 4,982 steps/s against the frozen 3,858.9 floor
and exceeds the 4,062 baseline. All 13 locally available quick-sweep stages
pass in 53.84 seconds at 716,776 KiB peak RSS with eight major faults and zero
swap; the two known absent-snapshot Blaze stages skip. These callback-only
checks add no idle scan or allocation. GPU 1 was not executed.

## 2026-08-09 note-block behavior, audio, particle, and state slice

Note block 25 now owns a bounded tile pool containing pitch 0 through 24 and
the previous redstone-power state. Ordinary neighbor notification detects
only a rising edge, suppresses playback under an occupied ceiling, selects
the instrument from the authoritative block-material registry, and emits the
exact RECORDS sound plus NOTE particle descriptor. Right-click increments the
pitch modulo 25 before playback; left-click plays without tuning.

A 12-row parked comparator covers no power, every power edge, blocked playback,
four material-specific instruments plus the default harp, right-click wrap and
blocked tuning, and left-click playback. Java and native match tile state,
sound identity/category/volume/pitch bits, all NOTE particle payload bits, and
unchanged World and Math RNG cursors in 2.57 seconds at 30,252 KiB peak RSS.
The live sound manifest now owns 182 events and 563 variants. The fixed client
particle pool implements NOTE's constructor formula, color, texture, six-age
lifecycle, drag, and billboard scale. State capture, validation, script
restore, and both trace backends round-trip every represented note tile.

The Java 8 build, focused audio/particle/script/capsule gates, exhaustive native
runtime, and quick verification pyramid pass. The runtime takes 7:01.99 at
506,256 KiB peak RSS with zero major faults and zero swap. CPU throughput is
4,043 steps/s against the frozen 3,858.9 floor. The quick pyramid passes in
71.33 seconds at 716,848 KiB peak RSS; only its two undistributed Blaze
...
including the fresh-spawn-through-credits route. GPU 1 was not executed. Tile
lookup is entered only for note-block callbacks, interaction, and cold state
export, so the idle tick path adds no pool scan or allocation. A stable
Java/native block-and-particle pixel scene remains open.

## 2026-08-09 swamp-hut witch runtime slice

Retained swamp-hut witch sites now materialize once into the bounded live mob
store. The enumerator deduplicates cross-window sites and returns the exact
centered position, zero motion, 26 health, zero rotation, Fire -1, Air 300,
persistence-required, and pre-first-tick ground state. Runtime claims are fixed
capacity and permanent for the session. Their cold scan is keyed by the current
chunk and populate-cache build counter, so an unchanged tick performs only
three scalar memo comparisons.

Witches use a distinct live type with 26 health, 0.6 by 1.95 dimensions, 1.62
eye height, 0.25 movement speed, hostile routing, the existing exact 1.11.2
model/skin, and no incorrect generic melee substitution. Spawn consumes the
same follow-range Gaussian and five-percent left-hand roll as
`EntityLiving.onInitialSpawn`. A six-seed parked real-Java/shared-CPU oracle
matches the computed follow-range attribute, complete Gaussian cache and raw
48-bit cursor, and both hand branches. Potion selection, drinking and launch
remain explicitly open rather than approximated.

The Java comparator passes in 0.16 seconds at 30,252 KiB peak RSS. The focused
structure/runtime, entity-render, and mob-live gates pass. The Java 8 build and
the shared `sm_120` CUDA translation-unit compile pass; the latter takes 82.07
seconds at 3,185,280 KiB peak RSS. CPU throughput is 4,921 steps/s against the
3,858.9 floor and 4,062 baseline. The quick sweep passes all locally available
stages in 53.10 seconds at 716,776 KiB peak RSS; only its two undistributed
Blaze snapshot stages skip. The exhaustive product aggregate reaches credits
in 476.13 seconds at 1,100,928 KiB peak RSS with one major fault and zero swap.
GPU 1 was compiled for but not executed. Exact witch potion combat,
loot/sounds, capsule/NBT persistence, and pixel promotion remain open.

## 2026-08-09 Witch self-potion slice

The bounded live Witch now executes `EntityWitch.onLivingUpdate`'s complete
self-potion decision and drinking transition. The short-circuit order is water
breathing, fire resistance, healing, then target-distance swiftness, with the
same active-effect guards and direct `Entity.rand.nextFloat` calls. Starting a
drink retains its potion kind, timer 32, zero movement input, and exact sound
pitch draw. Completion clears held state, removes the movement penalty, and
queues the exact duration-3600 Water Breathing, Fire Resistance, or Speed
effect, or duration-1 Instant Health effect after the current tick's general
effect-aging pass. Instant Health applies four points and expires on the next
tick. The final rare roll emits entity status 15.

`magma/trace/test_witch_self_potion.py` compares ten parked real-Java rows to
the shared CPU transition. It covers every start choice, ordinary and rare
no-choice paths, all four completions, exact timer/held/effect/movement state,
and every direct private-RNG value and cursor. The live regression additionally
covers a normal runtime tick, all transitions, renderer-view held state, the
rare event, and next-tick healing. The same header compiles for
`sm_120 --fmad=false` in 0.50 seconds at 188,640 KiB without GPU execution.

The Java comparator passes in 0.16 seconds at 30,252 KiB. The combined
scattered/runtime gate passes in 3.26 seconds at 115,200 KiB, the mob aggregate
passes in 6.86 seconds at 172,000 KiB, and the 389-check brewing aggregate
passes. The stopped-oracle scalar guard passes at 4,682 steps/s against the
3,858.9 floor and 4,062 baseline. The hot transition is allocation-free and is
entered only for represented live Witches. Target-dependent thrown-potion
selection/launch, drink audio, actual held-potion geometry/tint/pose, client
status particles, loot, capsule/NBT persistence, and pixels remain open.

## 2026-08-09 Witch ranged splash-potion slice

The represented live Witch now executes the complete bounded
`attackEntityWithRangedAttack` decision. Horizontal distance uses the moving
target position, then the exact short-circuit order selects Slowness at eight
or more blocks, Poison for an eligible target at eight or more health,
Weakness inside three blocks on the private-RNG quarter roll, or Harming.
Matching target Slowness, Poison, or Weakness suppresses its branch. A drinking
Witch neither constructs a potion nor consumes the attack/sound RNG slice.

The live ranged task retains the independent visibility counter, holds inside
ten blocks after 20 visible ticks, waits at cooldown zero while occluded, and
resets to a 60-tick interval after an eligible attack call. A three-entry
per-Witch queue transports exact spawn and aim state to the existing fixed
projectile pool. Materialization gives the splash potion a reconciled entity
ID, living shooter, separately seeded projectile Random, exact three-Gaussian
`.75/8` heading and MathHelper rotation, then shares the already promoted
flight, collision, splash-effect, event-color, and billboard paths. The exact
hostile Witch throw sound and pitch enter the fixed sound stream.

`magma/trace/test_witch_ranged.py` passes nine parked real-Java/shared-CPU
rows in 0.19 seconds at 30,252 KiB. It compares the real Witch-selected potion,
relative spawn and aim bits, moving-distance boundary, active-effect guards,
all direct Witch RNG calls, throw sound, and a deterministically seeded real
EntityPotion's heading motion, rotation, Gaussian cache, and cursor. The
65-check live gate covers all four outcomes, effect suppression, cooldown
initialization, drinking suppression, occlusion wait/release, retained queue,
runtime projectile construction, first flight tick, and sound delivery.

The combined scattered/runtime gate passes in 3.31 seconds at 115,200 KiB,
the mob aggregate passes in 6.82 seconds at 172,320 KiB, and the 389-check
brewing aggregate passes. Java 8 builds in 3.64 seconds at 112,328 KiB. The
shared CUDA probe compiles for `sm_120 --fmad=false` in 0.55 seconds at
188,640 KiB without executing GPU 1. CPU throughput passes at 4,752 steps/s
against the 3,858.9 floor and 4,062 baseline. The audio-live gate passes for
182 represented events and 563 loaded variants. The full native product
aggregate reaches credits in 7:47.04 at 1,101,244 KiB with zero major faults
or swap. The clean quick pyramid passes in 58.52 seconds at 714,016 KiB with
zero major faults or swap; only the two undistributed `.bsnp` stages skip.

Open: the generated live projectile's private Random uses Netherite's
deterministic entity-ID hash unless an oracle capsule injects its Java cursor;
Java's ordinary `new Random()` seed is wall-clock dependent. Queue overflow is
a documented fixed-capacity boundary. Drink/hurt/death audio, Witch loot,
arbitrary Witch/projectile NBT and queue resume, held-potion/status particles,
and Witch/potion pixel promotion remain open.

## 2026-08-09 Witch lifecycle-audio and player-loot slice

Witch drink start, hurt, and death now enter the fixed audio stream with the
real event, category, scalar, position, and private-RNG pitch. The self-potion
oracle includes drink sound fields, and the expanded player-attack oracle
passes 117 cases including Witch hurt/death. The owned manifest covers 186
events and 576 weighted variants.

One pointer-free CPU/CUDA helper implements the exact 1.11.2 Witch loot table:
one to three weighted rolls over all seven entries, SetCount before Looting,
and zero-stack discard. Player-credited lethal damage calls it synchronously,
preserves separate stack construction, advances the shared Math and entity-ID
cursors exactly, and retains the dead Witch for its death-update window.
`doMobLoot=false` consumes no loot or constructor RNG. Fixed item capacity is
preflighted so failure rejects the complete damage boundary without partial
mutation.

The 13-row parked Java/shared-CPU comparator covers all entries, Looting zero
through three, empty and three-stack results, gamerule suppression, exact item
position/motion/yaw/hover/age/pickup fields, raw target and Math cursors, and
global IDs. The focused live gate adds the real held-Looting player route,
ordered status/death sound, and capacity atomicity. Scattered/runtime passes,
mob-live passes in 7.14 seconds at 172,760 KiB, Java 8 and audio-live pass, and
the shared `sm_120 --fmad=false` probe compiles without executing GPU 1. CPU
throughput passes at 4,842 steps/s against the 3,858.9 floor and 4,062
baseline. The full native aggregate reaches credits in 480.04 seconds at
1,100,120 KiB with zero major faults and zero swap. The quick pyramid passes
in 55.78 seconds at 716,824 KiB with one major fault and zero swap; only the
two undistributed `.bsnp` stages skip.

At this checkpoint, terminal XP was still open; the next section closes the
ordinary non-drinking player-credit case. Non-player lethal sources,
drinking/equipped XP, arbitrary NBT/load resume, held-potion/status rendering,
and pixel promotion remain open.

## 2026-08-09 ordinary Witch terminal-XP slice

The ordinary non-drinking, player-credited Witch death window now completes
the exact `EntityLivingBase.onDeathUpdate` tick-20 tail. Base experience five
splits greedily into three independent orbs with values `3,1,1`. Each orb owns
the real ID, position, motion, yaw, health, age, pickup, color, and target-color
constructor state from four `Math.random()` draws. `World.rand` is unchanged;
all three constructors precede the Witch's 20 terminal particle calls.

`magma/trace/test_witch_terminal_xp.py` passes seven parked
real-Java/shared-CPU cases over raw Witch, Math, and World seeds plus the
disabled gamerule. A fixture-only mixin captures the actual real-game
EntityXPOrb constructor objects without changing ordinary execution. The live
gate covers both controlled and ordinary product ticks, dynamic loaded-list
order, same-boundary orb updates, exact terminal particles, cursor commits,
retirement, and gamerule suppression. Scattered/runtime passes in 3.49 seconds
at 115,200 KiB and mob-live passes in 6.89 seconds at 172,748 KiB. CPU
throughput remains above the frozen floor at 4,794 steps/s versus 3,858.9. The
full native aggregate reaches credits in 492.81 seconds at 1,101,196 KiB with
zero major faults or swap. The quick pyramid passes in 53.70 seconds at
713,996 KiB with two major faults, zero swap, and only the two undistributed
`.bsnp` stages skipped. Java 8 and the shared `sm_120 --fmad=false` compile
probe pass; GPU 1 was not executed.

Open: non-player Witch lethal/loot routes, drinking or otherwise equipped
Witch equipment drops and experience bonus, arbitrary NBT/load resume,
held-potion/status rendering, and pixel promotion. Fixed XP capacity is an
explicit atomic boundary.

## 2026-08-09 equipped Witch death slice

The represented player-credited drinking-Witch death now preserves the full
ordering-sensitive boundary that was open above. After the ordinary Witch
loot table, `EntityLiving.dropEquipment` consumes its float subtraction at
float precision and may emit the still-held potion as a separate EntityItem.
The represented potion keeps the canonical PotionType registry metadata and
advances Math and global IDs only when emitted. At death tick 20 the same held
stack independently adds `1 + Witch.rand.nextInt(3)` to base XP five, producing
totals six, seven, or eight and greedy splits `3,3`, `7`, or `7,1`. Orb
constructors precede the exact 20-particle Gaussian/float tail.

`magma/trace/test_witch_equipped_death.py` passes eight parked
real-Java/shared-CPU cases. The matrix covers potion drop/no-drop, XP totals
6/7/8, zero through three ordinary loot stacks, Looting zero through three,
`doMobLoot=false`, exact EntityItem and EntityXPOrb fields, raw Witch/Math/World
cursors, global IDs, Gaussian cache, and removal state. The live regression
starts with an accepted lethal player hit, verifies the healing-potion drop,
continues the same entity to tick 20, and locks XP values 8 and 7, same-tick orb
updates, loaded-list order, exact particles, the ordinary product tick, and a
potion-only fixed-item-capacity rejection.

The combined scattered/runtime gate passes in 1.86 seconds at 82,120 KiB and
mob-live passes in 6.98 seconds at 172,728 KiB, both with zero major faults or
swap. Java 8 builds, and the shared header compiles for
`sm_120 --fmad=false` in 0.53 seconds at 188,640 KiB without GPU execution.
CPU throughput remains green at 4,615 steps/s against the frozen 3,858.9
floor. The quick pyramid passes in 55.91 seconds at 716,844 KiB with zero major
faults or swap; only the two undistributed `.bsnp` stages skip. The added work
is confined to lethal/terminal Witch branches and adds no unchanged-tick scan
or allocation.

Open: non-player Witch lethal/loot paths, arbitrary injected equipment and
Witch NBT/load resume, held-potion/status rendering, and Witch/potion pixels.
The fixed item and XP pools retain their documented atomic capacity boundary.

## 2026-08-09 ordinary drowning-killed Witch slice

The first non-player Witch death source is now promoted through the ordinary
product tick. A drowning pulse consumes the real 48 bubble floats, applies
fresh unblockable damage, orders status 2, the no-attacker Math yaw choice,
two death-sound floats, Looting-0 table generation, status 3, and the
same-boundary `onDeathUpdate` increment to `deathTime=1`. DROWN deliberately
skips `setBeenAttacked`. With `recentlyHit=0`, even a drinking Witch cannot
drop its held potion and cannot create terminal XP; table loot still runs.

`magma/trace/test_witch_drowning_death.py` passes seven parked
real-Java/shared-CPU cases spanning zero through three table stacks, raw
cursor extremes, exact EntityItem constructors, gamerule suppression,
feedback fields, global IDs, no equipment/orbs, removal, and final Gaussian
state. The focused live gate stages water plus air `-19`, proves the actual
ordinary product route reaches the exact death window and terminal tail, and
locks the fixed-item-pool atomic rejection.

Scattered/runtime passes in 3.41 seconds at 115,200 KiB and mob-live passes in
6.86 seconds at 172,860 KiB. The stopped-oracle CPU guard passes at 5,009
steps/s against the 3,858.9 floor and 4,062 baseline. The focused Java/native
comparison takes 0.19 seconds at 30,252 KiB. No work is added to unchanged air
ticks; the preview is restricted to a lethal ordinary Witch drowning pulse.
The quick pyramid passes in 53.37 seconds at 716,872 KiB with two major faults,
zero swap, and only the two undistributed `.bsnp` stages skipped.

Open: other non-player Witch lethal sources and attribution, arbitrary
equipment/NBT resume, held-potion/status rendering, and Witch/potion pixels.

## 2026-08-09 ordinary falling-anvil-killed Witch slice

The actual falling-block collision callback now handles ordinary Witches in
addition to the earlier controlled passive fixtures. A lethal anvil hit
preserves Java's fresh-hit `setBeenAttacked` draw, Math yaw, Witch death-sound
draws, Looting-0 table, exact item constructors, status ordering, and the
following ordinary mob tick to `deathTime=1`. No attacker means no held-potion
equipment drop and no terminal XP even though ordinary table loot remains
eligible.

`magma/trace/test_witch_source_death.py` contributes seven falling-anvil rows
to a 14-row parked real-Java/shared-CPU gate spanning zero through three
stacks, gamerule
suppression, exact item state, all cursors and IDs, death fields, no
equipment/orbs, and terminal particles. The focused live gate reaches the
boundary through the production collision function, verifies the subsequent
runtime order and no-credit tail, and locks fixed-item-capacity atomicity.

The focused comparison takes 0.20 seconds at 30,252 KiB and the live gate 0.02
seconds at 18,744 KiB. The full header-rebuilt runtime regression, including
the older controlled passive anvil fixtures, passes in 329.56 seconds at
505,780 KiB with zero major faults. Scattered/runtime passes in 3.38 seconds at
115,200 KiB, mob-live in 6.76 seconds at 172,688 KiB, and CPU throughput is
4,883 steps/s against the 3,858.9 floor and 4,062 baseline. The work is
restricted to actual falling-anvil collision callbacks, with no added
unchanged-tick scan or allocation. The quick pyramid passes in 52.45 seconds
at 717,012 KiB with two major faults, zero swap, and only the two
undistributed `.bsnp` stages skipped.

Open: other non-player Witch lethal sources and attribution, broader anvil
multi-target behavior, arbitrary equipment/NBT resume, held/status rendering,
and Witch/potion pixels.

## 2026-08-09 ordinary ON_FIRE-killed Witch slice

The ordinary product tick now runs a Witch's periodic ON_FIRE attack in the
real `Entity.onEntityUpdate` phase before air, combat-timer aging, death
progress, and living AI. A lethal fire-20 pulse shares the source-less damage
body with falling anvils, then decrements fire to 19 and reaches
`hurtTime=9`, `hurtResistantTime=19`, and `deathTime=1` in the same tick.
Fire Resistance suppresses the attack without freezing the fire counter.
No attacker credit means exact Looting-0 table loot but no held-potion drop or
terminal XP.

The combined 14-row `magma/trace/test_witch_source_death.py` gate passes both
falling-anvil and ON_FIRE sources over zero through three stacks, exact item
constructors, all Witch/Math/World cursors and IDs, feedback, fire/death state,
gamerule suppression, no equipment/orbs, and terminal particles. The burning
live gate additionally locks production-phase order, Fire Resistance, the
no-credit tail, and a retryable full-item-pool boundary.

The combined comparison passes in 0.20 seconds at 30,252 KiB and the focused
live gate in 0.01 seconds at 18,728 KiB. Scattered/runtime passes in 3.37
seconds at 115,200 KiB, mob-live in 6.75 seconds at 173,232 KiB, and CPU
throughput remains green at 4,741 steps/s against the 3,858.9 floor and 4,062
baseline. The ordinary mob array was already visited; dry and non-pulse Witch
ticks add only bounded scalar checks and no allocation. The header-rebuilt
...
major fault and only the two undistributed `.bsnp` stages skipped.

Open: other non-player Witch lethal sources and attribution, arbitrary
equipment/NBT resume, held/status rendering, and Witch/potion pixels.

## 2026-08-09 ordinary lava-killed Witch slice

The ordinary product base phase now probes the exact contracted Witch AABB for
lava immediately after periodic fire. `DamageSource.LAVA` applies raw damage
four, sets the fire floor to 300 even when Fire Resistance rejects damage, and
halves fall distance before air and living timer/death work. A fresh lethal
contact reaches `hurtTime=9`, `hurtResistantTime=19`, and `deathTime=1` in the
same product tick with Looting-0 table drops but no held potion or terminal XP.

The source comparator now passes 28 parked real-Java/shared-CPU rows: seven
each for falling anvil, ON_FIRE, direct lava, and a discriminating fire-20 plus
lava case. The latter proves ON_FIRE runs first: its nonlethal fresh hit plays
the Witch hurt sound, then LAVA applies only the immunity-window raw-damage
difference, kills without a second sound, and restores fire from 19 to 300.
Every source still locks zero through three stacks, gamerule suppression,
constructors, IDs, cursors, terminal particles, and no equipment/orbs.

The combined comparator passes in 0.46 seconds at 34,560 KiB. The focused live
gate passes in 0.02 seconds at 18,728 KiB and additionally locks actual-world
lava detection, Fire Resistance, direct and combined full-item-pool atomicity,
and product phase order. Scattered/runtime passes in 5.22 seconds at 180,272
KiB, mob-live in 6.78 seconds at 172,128 KiB, and CPU throughput remains green
at 5,024 steps/s against the 3,858.9 floor and 4,062 baseline. The full runtime
gate passes in 325.65 seconds at 506,028 KiB with zero major faults. The quick
pyramid passes in 56.66 seconds at 716,928 KiB with five major faults and only
the two undistributed `.bsnp` stages skipped. The existing ordinary-mob visit
adds a bounded allocation-free lava-box probe for Witches.

Open: block-collision IN_FIRE and other non-player Witch lethal sources,
arbitrary equipment/NBT resume, held/status rendering, and Witch/potion pixels.

## 2026-08-09 ordinary block-fire-contact Witch slice

The ordinary Witch move path now probes Java's contracted callback AABB for
FIRE and LAVA blocks. FIRE applies source-less raw-one `IN_FIRE` damage after
movement, respects Fire Resistance and the hurt-resistance window, and raises
a cold fire counter from -1 to 160. Wet extinguishing consumes the exact two
private floats before resetting the counter. The base tick now also owns
`ticksExisted`, previous-position state, and Witch's `nextInt(1000)` ambient
branch with its two-float pitch and hostile audio event. Lethal base work
short-circuits that ambient draw as Java does.
...
ON_FIRE-before-LAVA, direct IN_FIRE callback, and the full product-tick
IN_FIRE composition. They lock damage and timer state, zero through three
Looting-0 stacks, constructors, sound/status/particle order, all
Witch/Math/World cursors and IDs, no equipment/orbs, and gamerule suppression.
The focused live gate reaches actual NETHERRACK/FIRE blocks and additionally
locks the following dead tick, Fire Resistance, immunity rejection, ambient
ordering, and fixed-capacity atomic retry.

The comparator passes in 0.61 seconds at 30,252 KiB and the focused live gate
in 0.03 seconds at 18,728 KiB. Scattered/runtime passes in 1.72 seconds at
82,112 KiB, mob-live in 6.94 seconds at 172,168 KiB, and CPU throughput is
4,937 steps/s against the frozen 3,858.9 floor and 4,062 baseline. The full
runtime gate passes in 346.11 seconds at 505,888 KiB with zero major faults.
The quick pyramid passes in 53.55 seconds at 716,900 KiB; all available steps
pass and only the two undistributed `.bsnp` stages skip. The contact scan is
bounded, allocation-free, and runs inside the existing ordinary-mob visit.

Open: rain and mixed water/fire contact composition, other non-player lethal
sources, arbitrary equipment/NBT resume, held/status rendering, and
Witch/potion pixels.

## 2026-08-09 ordinary cactus-contact Witch slice

The ordinary Witch move tail now runs `Entity.doBlockCollisions`' cactus
callback before its flammable check. The callback coordinate scan uses the
entity AABB's exact 0.001 inset and remains distinct from cactus's 15/16-high,
1/16-horizontal collision box. Raw-one `DamageSource.CACTUS` enters the shared
source-less Witch damage/death path after base timers and living travel.

The source gate now passes 56 parked real-Java/shared-CPU rows: the prior six
families plus direct and full-product-tick cactus contact, seven cases each.
The full tick stages a valid cactus over sand at its collision-top boundary
and locks the ambient/status prefix, post-move death state, Looting-0 drops,
events, particles, constructors, IDs, and all RNG cursors. The focused product
gate additionally locks actual world-block detection and fixed-capacity
atomic rejection.

The 56-row comparator passes in 0.56 seconds at 30,252 KiB and the focused
live gate in 0.03 seconds at 18,752 KiB. Scattered/runtime passes in 3.39
seconds at 115,200 KiB, mob-live in 6.82 seconds at 173,124 KiB, and CPU
throughput is 5,000 steps/s against the frozen 3,858.9 floor and 4,062
baseline. The quick pyramid passes in 55.11 seconds at 716,848 KiB; every
available step passes and only the two undistributed `.bsnp` stages skip.

Open: rain and mixed water/fire contact composition, other non-player lethal
sources, arbitrary equipment/NBT resume, held/status rendering, and
Witch/potion pixels.

## 2026-08-09 ordinary still-water plus block-fire Witch slice

The ordinary Witch base phase now runs the exact still-water material probe
before periodic fire and lava. An overlap latches `inWater`, clears fall
distance, and extinguishes the entity before living travel. If the same AABB
then reaches FIRE during `Entity.move`, wetness suppresses the ignition tail
but not the raw-one IN_FIRE callback, so a fresh lethal hit ends at death time
zero, hurt timers 10/20, and fire zero.

The source comparator now passes 63 parked real-Java/shared-CPU rows. Seven
new full-product-tick rows span a WATER block and adjacent supported FIRE and
lock the base ambient/status prefix, exact no-credit table drops, constructors,
events, death state, terminal particles, IDs, fire/fall/water state, and every
RNG cursor. The actual-world focused gate locks the same composition through
the production block scans.

The 63-row comparator passes in 0.70 seconds at 30,252 KiB and the focused
live gate in 0.04 seconds at 18,764 KiB. Scattered/runtime passes in 3.46
seconds at 115,200 KiB, mob-live in 6.91 seconds at 171,816 KiB, and CPU
throughput is 4,955 steps/s against the frozen 3,858.9 floor and 4,062
baseline. Java 8 builds. The quick pyramid passes in 54.34 seconds at 716,928
KiB with zero failures, five major faults, no swap, and only the two
undistributed `.bsnp` stages skipped. The water scan is bounded and
allocation-free inside the existing mob visit.

Open: rain wetness, flowing-water acceleration and entry splash effects,
other non-player lethal sources, arbitrary equipment/NBT resume, held/status
rendering, and Witch/potion pixels.

## 2026-08-09 ordinary rain-wet block-fire Witch slice

Ordinary Witch wetness now includes Java's two precipitation checks at the
floored feet and full-height positions. Both use the live world clock's rain
strength, dimension and open-sky precipitation-height predicate. The base
living update extinguishes a rain-exposed Witch after the FIRE callback; a
solid roof over the same fixture rejects wetness and preserves fire 160.

The source comparator now passes 77 parked real-Java/shared-CPU rows. Fourteen
new full-product-tick rows cover open-rain and roofed-rain FIRE contact, seven
cases each, and lock exact damage/death state, no-credit Looting-0 drops,
constructors, events, particles, IDs, fire state, and all Witch/Math/World
cursors. The focused actual-world gate independently locks both sky-column
outcomes through production block and weather probes.

The 77-row comparator passes in 0.73 seconds at 30,252 KiB and the focused
live gate in 0.06 seconds at 18,728 KiB. Scattered/runtime passes in 3.43
seconds at 115,200 KiB, mob-live in 6.71 seconds at 171,880 KiB, and CPU
throughput is 4,960 steps/s against the frozen 3,858.9 floor and 4,062
baseline. Java 8 builds. A clean header-rebuilt `test-game` passes through
fresh-spawn-to-credits in 481.99 seconds at 1,100,948 KiB with one major fault
...
`.bsnp` stages skipped. The added checks are bounded, allocation-free, and
run only while visiting a represented ordinary Witch.

Open: flowing-water acceleration and entry splash effects, broader
precipitation-height and biome edges, other non-player lethal sources,
arbitrary equipment/NBT resume, held/status rendering, and Witch/potion
pixels.

## 2026-08-09 ordinary Witch flowing-water and entry slice

Ordinary Witch water handling now reuses the shared Java-verified
`handleMaterialAcceleration` kernel. A level-one flowing-water cell beside a
west source contributes the exact normalized +0.014 X acceleration before an
overlapping upper-body lava hit; water first clears fall distance, then lava
applies its fire floor and damage. The source comparator now passes 84 parked
real-Java/shared-CPU rows, adding seven complete flow-before-lava cases to the
prior source matrix.

The separate dry-to-water gate locks the non-first-update `resetHeight`
boundary across seven entity seeds. It bit-matches the HOSTILE splash event,
position, motion-scaled volume, two-float pitch, wet/fire/fall state, unchanged
motion, 26 ordered particle calls, and final entity RNG cursor. The product
retains those exact descriptors in four bounded batches, drains all 26 into
the current-tick runtime stream, and feeds the existing water-particle
constructor, lifecycle, and renderer. Constructor-private visual entropy is
deterministic but is not claimed as Java pixel identity.

The 84-row source comparator passes in 0.99 seconds at 34,560 KiB and the
seven-seed entry comparator in 0.29 seconds at 31,680 KiB. The expanded
focused live gate passes in 0.11 seconds at 20,216 KiB, the water renderer
gate in 0.11 seconds at 36,000 KiB, player movement runtime in 0.18 seconds
at 20,176 KiB, audio in 0.51 seconds at 63,360 KiB, scattered/runtime in 3.46
seconds at 115,200 KiB, and mob-live in 6.84 seconds at 172,964 KiB. Java 8
builds. A clean header-rebuilt `test-game` passes through fresh spawn to
credits in 484.66 seconds at 1,101,224 KiB with ten major faults and zero
swap. CPU throughput is 5,030 steps/s against the frozen 3,858.9 floor and
4,062 baseline. The quick pyramid passes in 55.36 seconds at 715,468 KiB with
zero major faults, zero swap, and only the two undistributed `.bsnp` stages
skipped. The reused current kernel and entry work are bounded and
allocation-free inside the existing Witch visit.

Open: mob splash pixel promotion, broader precipitation-height and biome
edges, other non-player lethal sources, arbitrary equipment/NBT resume,
held/status rendering, and Witch/potion pixels.

## 2026-08-09 ordinary Witch in-wall suffocation slice

The blockstate oracle schema is now `qrl.blockstate_props.v4`. A fresh real
1.11.2 registry capture records `Block.causesSuffocation` for all 4,096 legacy
ID/metadata inputs and pins SHA-256
`57911f7d3a42bd1e585d3cca75283fe500fbfcb2308c3f15127837a2c56433f6`.
The generated allocation-free lookup is shared by the first-person in-wall
overlay and the new ordinary Witch server path. It preserves stateful edges
such as extended versus retracted pistons and upper versus lower slabs without
a hand-maintained block list.

The Witch base phase tests Java's eight width-scaled eye samples in exact bit
order after water, fire, and lava and before air, wetness, and the Witch living
update. An accepted IN_WALL hit uses the existing exact source/death path.
The full tick also now retains Java's subtle post-base-death behavior: the
subclass Witch living update still advances the drinking/status RNG phase
after death progress, correcting the same boundary for drowning, anvil, fire,
and lava deaths.

The source comparator passes 91 real-Java/shared-CPU rows in 0.85 seconds at
30,252 KiB, including loot, constructors, feedback, terminal particles, and
all Witch/Math/World cursors. The state capsule and block-registry gates pass;
focused in-wall/fire passes in 0.14 seconds at 20,180 KiB, scattered/runtime
in 1.96 seconds at 82,104 KiB, mob-live in 6.73 seconds at 172,924 KiB, and
particles in 0.13 seconds at 36,000 KiB. Java 8 builds. A clean rebuilt
`test-game` reaches the final fresh-spawn-to-credits route and passes in about
489 seconds. CPU throughput passes at 4,748 steps/s against the 3,858.9 floor
and 4,062 baseline. The quick pyramid passes in 56.78 seconds at 717,036 KiB
with zero failures, zero major faults, zero swap, and only the two
undistributed `.bsnp` stages skipped.

Open: fall and other unpromoted non-player Witch sources, general-mob
suffocation, arbitrary equipment/NBT resume, held/status rendering, and
Witch/potion pixels.

## 2026-08-09 ordinary Witch stone/hay landing slice

The ordinary Witch move path now retains its pre-move fall distance through
the collision spine and runs Java's late `updateFallState` consequences after
a downward landing. Stone applies full fall damage; hay applies the exact
one-fifth multiplier. Jump Boost deduction, slime's zero multiplier, and the
represented block fall-sound families are encoded, while general-mob landing
callbacks remain open.

The source matrix expands from 91 to 105 parked real-Java/shared-CPU rows.
Seven stone and seven hay complete ticks lock the final position and raw motion,
ground/fall/death/timer state, Looting-0 items, constructors, IDs, feedback,
and all Witch/Math/World cursors. The exact sound sequence is hostile small
fall, hurt status, Witch death, death status, and stone or grass support fall.
The runtime also exports packet-shaped BLOCK_DUST descriptors with exact
counts 40/80 and state parameters 1/170.

The comparator passes in 0.88 seconds at 30,252 KiB. Focused live passes in
0.12 seconds at 20,160 KiB, audio in 0.37 seconds at 63,420 KiB, and mob-live
in 6.45 seconds at 172,336 KiB, all with no major faults or swap. Java 8
builds. CPU throughput passes at 4,942 steps/s against the frozen 3,858.9
floor and 4,062 baseline. The full game aggregate passes through fresh spawn
to credits in 478.45 seconds at 1,101,084 KiB with one major fault and zero
swap. The quick pyramid passes in 54.57 seconds at 716,848 KiB with zero
failures, zero major faults, zero swap, and only the two undistributed `.bsnp`
stages skipped. The inactive path adds one Witch type branch and no allocation
or scan.

Open: other non-player Witch sources and attribution, general-mob suffocation
and landing callbacks, arbitrary equipment/NBT resume, held/status rendering,
landing-particle pixel promotion, and Witch/potion pixels.

## 2026-08-09 ordinary Witch landing-completion slice

The ordinary Witch landing path now covers the main nonlethal block callbacks
beside the lethal stone/hay work. Seven-seed real-Java/shared-CPU rows lock a
Jump Boost II safe landing, non-sneaking living slime bounce, seeded farmland
trample/rejection, and gamerule-suppressed trample. Farmland uses the exact
15/16 collision AABB and still consumes World.rand when `mobGriefing=false`;
the block mutation and seed are staged around the bounded damage-capacity
boundary. Big stone falls are also promoted through the lethal source matrix.

The source matrix now passes 112 rows in 1.01 seconds at 30,252 KiB. The new
28-row landing matrix passes in 0.22 seconds at the same RSS, comparing raw
position/motion, effect state, events, BLOCK_DUST, support state, and entity
and world RNG. It found and closed a base-living ordering error: a visible
potion effect consumes server Entity.rand before the Witch ambient draw even
though the base WorldServer particle method emits no packet.

Focused and broad native gates, Java 8, the clean full game aggregate, and the
quick pyramid pass. CPU throughput is 4,104 steps/s against the frozen
3,858.9 floor and 4,062 baseline. The new work uses bounded contact lists and
the existing mob visit, with no heap allocation or loaded-world scan. GPU 1
was left untouched while shared.

Open: general-mob landing damage/audio/particles and non-slime callbacks,
sneaking/nonliving and mixed slime contacts, general-mob suffocation,
arbitrary equipment/NBT resume, held/status rendering, landing-particle pixel
promotion, and Witch/potion pixels.

## 2026-08-09 shared ordinary-living slime promotion

The earlier ordinary-Witch slime callback now feeds the common non-ridden
living travel path for every represented living type. A six-row parked
real-Java/shared-CPU matrix covers zombie, sheep, and Witch across full bounce
and low-speed walking damping, comparing raw position/motion bits,
collision/ground/fall state, support block/meta, and Java's exact 80-count
BLOCK_DUST descriptor. Focused product ticks independently exercise hostile
and passive dispatch, preserve Java's exact `0.01960000038146973`
post-gravity/drag bounce tail, and publish the same particle batch.

The implementation derives up to 32 slime contacts from the collision list
already resident in `move_mob`; it adds no world read, heap allocation, or
second world scan. The comparator passes in 0.15 seconds at 30,252 KiB,
mob-live in 6.74 seconds at 173,188 KiB, and the CPU guard at 4,991 steps/s
against the 3,858.9 floor and 4,062 baseline. Java 8 and the prior Witch
landing matrix pass. The quick pyramid passes in 54.00 seconds at 716,896 KiB
with zero failures and only the two undistributed `.bsnp` stages skipped; GPU
1 was not executed.

Open: general-mob fall damage/audio, landing-particle pixels,
sneaking/nonliving and mixed slime cases, other special landing callbacks,
and general-mob suffocation.

## 2026-08-09 ordinary-living nonlethal landing promotion

The common non-ridden living move path now promotes ordinary nonlethal
small/big stone falls, hay-reduced falls, and Jump Boost II damage avoidance
for zombie, zombie villager, skeleton, wither skeleton, creeper, spider, cave
spider, pigman, silverfish, sheep, pig, cow, and villager. The exact boundary
includes fall-distance damage, hostile versus generic small/big-fall audio,
type-specific hurt status/sound/volume/pitch, support-block audio, health and
immunity timers, effect state, entity and Math RNG, the 50/80-count BLOCK_DUST
descriptor, and creeper fuse advancement. Source-less FALL does not call the
attacker reaction path, so pigman anger and passive panic remain unchanged.

The real-Java/native matrix passes 58/58 rows: the six prior slime rows and 52
new landing rows. It found two useful class differences. Zombie villager owns
a distinct `entity.zombie_villager.hurt` event, now carried by the manifest and
runtime. Enderman's unblockable fall damage consumes a random teleport branch,
so Enderman remains excluded pending that exact special callback. Product
ticks independently cover the small stone path for all 13 types plus big
stone, hay, and Jump Boost representatives; the audio gate drains a passive
landing as generic fall, type hurt, then neutral support audio.

The 58-row comparator passes in 0.33 seconds at 30,252 KiB. The unchanged
28-row Witch landing matrix passes in 0.17 seconds at 30,252 KiB, mob-live in
8.73 seconds at 172,004 KiB, particles in 0.16 seconds at 37,440 KiB, audio in
0.52 seconds at 65,772 KiB, and scattered/runtime in 4.49 seconds at 115,200
KiB. Java 8 builds. CPU throughput is 4,946 steps/s against the 3,858.9 floor
and 4,062 baseline. The quick pyramid passes in 54.69 seconds at 714,340 KiB
with zero failures, seven major faults, zero swap, and only the two
undistributed `.bsnp` stages skipped. GPU 1 was not executed.

Open: lethal ordinary falls and type-specific death/drop progression,
Enderman teleport-on-damage, slime's generic fall callback, Chicken's special
no-damage path, flying/magma fall overrides, other block callbacks,
landing-particle pixels, and general-mob suffocation.

## 2026-08-10 survival, equipment, and ordinary-item checkpoint

The product now drives block harvesting from a complete real-1.11.2 registry
census. A 372-case Java/native matrix locks ordinary block drops and metadata,
a 132-case matrix locks tool damage, and 471 rows lock harvest XP. Player death
inventory, armor bypass and reduction, equipment enchantments, Mending, bow
release, arrow block/entity impact and pickup, Elytra wall damage, experience
bottles, zombie-villager curing, Slime/Magma terminal splitting, and represented
hostile/passive death loot and XP all have focused real-Java/native gates plus
live-product regression coverage.

Ordinary inventory and dropped-item behavior now share a generated census of
all 392 registered non-air Item IDs, their subtype policy, damageability, and
stack limits. Exact insertion is used by player pickup as well as the existing
automation paths. Twelve real-Java/native pickup cases cover empty, partial,
full, max-16 and max-1 stacks, damage, subtype and nonsubtype metadata,
offhand priority, and selected-slot independence. Fifteen real-Java/native
loaded-order merge cases cover distance, age/pickup-delay gates, stack limits,
metadata, damage, subtype policy, and the exact `ticksExisted % 25` scan.

The version-2 neutral capsule transports ordinary tagless item loaded order,
pose and motion, item/count/meta, age, pickup delay, lifespan, ground state,
and `ticksExisted`. A locked Java-capture/native-restore gate continues air,
ground, landing, and a two-entity merge boundary at raw double/float precision.
Tagged/custom NBT items are rejected rather than silently approximated.

The focused oracle battery passes more than 2,400 exact rows. The full native
runtime aggregate passes in 6:48.07 at 507,236 KiB peak RSS with zero major
faults and zero swap. With the Java oracle stopped, the CPU performance guard
passes at 4,674 scalar steps/s against the frozen 3,858.9 floor and the 4,062
original baseline. GPU 1 remained untouched while shared.
The quick verification pyramid passes all thirteen locally available stages in
1:09.08 at 716,832 KiB peak RSS; only the two undistributed `.bsnp` stages
skip.

This raised the strict effort-weighted full-replica estimate only into the
mid-70s. The high row count does not erase the remaining broad systems:
arbitrary entity/task NBT and loaded-list continuation, villager/village AI,
monuments/guardians, mansions/illagers, tameable pets, remaining redstone and
rail edges, ordinary item fire/lava/water/expiration lifecycle, and global
pixel equivalence remain open.

## 2026-08-10 ordinary-item environment and capsule checkpoint

Ordinary tagless `EntityItem` continuation now covers the previously open
environment lifecycle. A parked real-Java/native comparator passes 39 exact
ticks across four private-RNG seeds and locks flowing/still water entry, lava
damage/bounce/burn, periodic and collision fire, wet extinguish, cactus
contact, lifespan expiry, void retirement, and west/east/north/south/up
full-collision-cube push-out. It compares alive/health/fire/water/first-update,
age/pickup/ticks/ground/no-clip, raw pose/motion, and the 48-bit private cursor.

The version-2 capsule transports those hidden environment fields and admits
only proof-fenced worlds. Its Java-capture/native-restore gate now continues
air, ground, landing, water, fire, cactus, push-out, and loaded-order merge.
Custom/tagged stacks, Forge hooks, non-full-cube start-overlap variants, and
unbounded cross-class loaded order remain rejected or explicit backlog.

The first full aggregate exposed soul sand and controlled-tripwire regressions.
The repaired path uses soul sand's actual 7/8 collision shape for push-out and
preserves the documented stationary oracle-sidecar contract. The focused
runtime then passed in 357.69 seconds, the direct and capsule gates remained
exact, and a clean complete aggregate passed in 7:42.22 at 505,560 KiB peak
RSS with zero major faults and zero swap. The quick pyramid passed all 13
locally runnable stages in 1:07.91 at 715,596 KiB, with only the two
undistributed `.bsnp` stages skipped. With the Java oracle stopped, scalar
throughput passed at 4,983 steps/s against the frozen 3,858.9 floor and 4,062
baseline. GPU 1 was not executed while shared.

This closes the largest remaining ordinary-item lifecycle class and moves the
honest effort-weighted estimate into the upper 70s, roughly 77%. The systems
still large enough to dominate the remaining estimate are general entity/task
save continuation, villager/village AI, monuments/guardians,
mansions/illagers, tameable pets, remaining rails/redstone edges, and global
pixel equivalence.

## 2026-08-10 minecart, living-entity, and merchant checkpoint

The strict effort-weighted full-replica estimate is now approximately 83%,
with high uncertainty. This is a feature-and-correctness estimate, not a claim
that arbitrary saves or rendered frames are 83% exact.

- Minecarts now cover curves, derailment, entity collision and vehicle
  callbacks in addition to the prior straight/slope, powered, detector,
  activator, TNT, and hopper-cart paths. The focused gate compares real 1.11.2
  rail state and motion directly.
- Wolves and ocelots now have Java-locked tame, sit, follow, teleport, owner,
  combat, breeding, death/drop, rendering, capsule, and sitting-cat container
  obstruction behavior.
- End progression now includes exact dragon resummoning and spike/crystal
  regeneration. End-city shulkers include attachment/peek state, guided
  bullets, damage/teleport/death, drops, sounds, rendering, and continuation.
- The shared NoAI continuation path is exact for eleven living classes,
  including private RNG, living timers, motion, environmental callbacks, and
  sounds. Initialized villager economies restore exactly, ordinary career and
  tier offers are Java-locked, and the merchant container/UI handles matching,
  reversed two-input recipes, result pickup, quick move, close cleanup, and
  distance/age lifecycle.

The minecart, villager-trade, tameable, dragon-respawn, shulker, NoAI,
merchant, state-capsule, screen, and scripted-play gates pass. The fresh CPU
guard passes at 4,228 scalar steps/s against the frozen 3,858.9 floor and the
4,062 original baseline. The new live state remains fixed-capacity and
event-driven; no idle world scan or per-tick heap allocation was introduced.
The quick pyramid passes all 13 locally available stages in 1:00.86 at
716,944 KiB peak RSS with zero swap; only the two undistributed `.bsnp` stages
skip. The broad game aggregate reaches and passes dragon-live. Its final
survival-route test remains a known harness failure on both this tree and the
unchanged `ca21527` baseline: it guesses a gravel drop from coordinates even
though the live drop follows the runtime Java-RNG cursor.

The broad work still dominating the remaining estimate is arbitrary task and
NBT continuation, villager/village AI, ocean monuments/guardians, Evoker
spells, the remaining mob roster, exhaustive rail/redstone topologies, and
global pixel equivalence.

## 2026-08-10 Guardian and ocean-monument checkpoint

The honest effort-weighted full-replica estimate is now approximately 89%,
with high uncertainty. It measures implemented and directly checked feature
effort, not arbitrary-save or global pixel equivalence.

- Guardian and Elder Guardian are distinct live types with exact attributes,
  laser target/damage timing, stationary thorns, mining-fatigue delivery,
  loot RNG, water/land audio, animated model/skins, and textured beam.
- Ocean monuments use exact spacing and biome viability, randomized room
  graph, all four facings, complete chunk-clipped block placement, treasure
  and sponge variants, and three persistent elder sites. Five Java/native
  fixtures match complete volume hashes/censuses and elder positions.
- Nearby discovery is player-chunk driven and bounded; construction is a cold
  path with no idle heap work. The live seed-0 fixture places the full monument
  exactly once. ASAN leak detection is clean after closing temporary room-graph
  ownership.
- Focused gates and every locally runnable quick-sweep stage pass. Scalar CPU
  throughput is 4,438 steps/s against the 4,062 baseline and frozen 3,858.9
  floor. GPU 1 was not executed while shared.
- The broad aggregate passes all new and unchanged gates through `dragon_live`.
  Its last survival route has the same gravel-drop prediction failure at the
  pre-bundle `35aa47c` commit; the route harness remains explicitly open.

The remaining large classes are general entity/task NBT continuation,
villager/village AI, the remaining mob roster, ordinary Guardian natural-spawn
and loaded ordering, exhaustive rail/redstone topologies, and global pixel
equivalence. Monument locate/save/load, arbitrary chunk order, and Guardian
particle/pixel details are the residual edges of this promoted bundle.

## 2026-08-10 fresh-NBT continuation and legal-route checkpoint

The honest effort-weighted full-replica estimate is now approximately 95%,
with high uncertainty. This is a bounded feature-and-correctness estimate, not
a claim of arbitrary-save or global pixel equivalence.

- The Java oracle now saves each parked NoAI living entity with
  `writeToNBTAtomically`, discards the old object, reconstructs a fresh object
  through `EntityList.createEntityFromNBT`, and restores it to the original
  loaded order. This exercises constructor/transient reset behavior rather
  than serializing and reading back into the same object.
- The capsule/native gate covers all 23 plain types in the native living
  roster for 20 post-load ticks. It compares raw position and motion doubles,
  float state, timers, private RNG, exact six-bound AABBs, sheep/chicken state,
  and all three Slime/Magma squish fields. Pig, villager, wolf, and ocelot keep
  their more specialized exact capsule schemas, so every one of the 27 native
  living types has a declared continuation representation.
- Direct real-Java construction/tick comparison now covers 19 common NoAI
  classes. Reading the 1.11.2 source directly closed Ghast's two-stage drag,
  Witch self-potion RNG, and the null Slime/Magma ambient-sound RNG branch.
- The fresh survival route no longer predicts flint from block coordinates or
  injects fake drops. It mines live gravel until the runtime RNG yields flint,
  obtains food, wool, feathers, arrows, obsidian, and End access through legal
  represented gameplay, and passes end to end.
- The new continuation state uses fixed per-mob fields and existing bounded
  stores. There is no hot-path allocation or world scan. The CPU guard passes
  at 4,035 steps/s against the frozen 3,858.9 floor and 4,062 baseline under
  shared-host load.
...
The remaining tail is arbitrary AI task/path and target state across load,
villager reputation/doors/breeding/golems, Java mob species not represented by
the native roster, ordinary Guardian pack spawn and cross-chunk loaded order,
unbounded scheduled/random world-tick ordering, exhaustive rail/redstone
topologies, custom/tagged NBT, and global render/pixel promotion. Player-facing
disk save UI is still outside the product contract; the promoted save boundary
is the verification/import capsule.

## 2026-08-11 cross-class NBT and village-state checkpoint

The honest effort-weighted full-replica estimate is now approximately 98%,
with high uncertainty. This is roadmap coverage backed by bounded gates, not
a claim that every arbitrary Java save, input sequence, or rendered pixel is
already equivalent.

- The real server NBT boundary now covers XP orbs as well as every parked
  NoAI living class. It writes each object, removes it, reconstructs a fresh
  exact Java class through `EntityList.createEntityFromNBT`, restores entity
  IDs, and rebuilds the complete prior `loadedEntityList` order with fresh
  objects in the original slots.
- One permanent real-Java/capsule/native regression interleaves all 23 plain
  native living classes with 23 XP orbs and a non-reconstructed Item sentinel.
  Across 20 post-load ticks it matches raw position/motion bits, promoted
  living/orb/item fields, the complete 46-entity living/XP update order, and
  the Item's unchanged Java loaded-list slot. Natural spawning is disabled
  and restored around
  the fixture so the recorder's bounded entity census cannot be contaminated.
- The recorder now captures the real `VillageCollection` NBT surface:
  collection time, ordered villages, population/radius/golem counters,
  mating and stable clocks, center/helper sums, ordered wooden doors, and
  UUID-keyed reputation. The neutral capsule validates and restores that
  state instead of silently resetting it.
- The native runtime ports `Village.tick` from the 1.11.2 source, including
  door retirement/recentering, reputation clearing, mating state, 20/30-tick
  population counts, Java integer wrapping, and the shared World RNG cadence.
  Product admission is deliberately exact only below ten loaded villagers
  with zero golems, so the still-unrepresented iron-golem spawn branch cannot
  be reached. A direct Java/native kernel gate separately locks that branch's
  selection and RNG for the final promotion.
- The village-family oracle, capsule selftest, Java 8 build, and cross-class
  real-client regression pass. The quick pyramid passes every locally
  available stage and skips only the two undistributed `.bsnp` inputs. CPU
  throughput is 4,160 scalar steps/s against the frozen 3,858.9 floor and
  4,062 original baseline. Village storage is cold-allocated only for an
  imported nonempty collection; an ordinary tick adds one inactive-count
  branch and no scan or allocation. GPU 1 was untouched.
- A clean broad `make -C magma test-game` passes through the legal
  fresh-spawn-to-credits route in 11:05.45 at 1,100,928 KiB peak RSS, with
  zero major faults and zero swap.

One final program remains: arbitrary-world promotion. Its implementation side
is the unsupported species and task-AI tail, centered on active villager door,
mate, reputation, aggressor, and iron-golem lifecycle plus the remaining
passive/utility species. Its acceptance side is broad save/reload fuzzing and
state/pixel promotion across varied worlds, chunk orders, inputs, and long
tick windows. Existing bounded claims stay exact while this final program
turns them into a general equivalence claim.

## 2026-08-11 finalization audit

The finalization pass closes the bounded implementation checklist but does not
change the honest roadmap estimate from approximately 98%. It replaces the
former vague "last 2%" label with executable green and red boundaries:

- Giant Zombie joins the native living roster. A real fresh-object Java NBT
  gate now continues all 34 plain native living classes for 20 ticks, including
  private RNG, motion, timers, class fields, and exact AABBs. The permanent
  cross-class case also preserves 29 XP orbs and an Item sentinel in Java
  loaded-list order. Giant's null ambient-sound branch does not advance RNG.
- A fresh-NBT active villager continues for 20 exact real-Java/native ticks.
  Focused source-derived gates additionally lock willingness, mating/birth,
  doors, social play, farming/sharing, follow-golem and rose behavior. Iron
  Golem attributes/NBT/timers/attack RNG, village golem creation, and bounded
  siege lifecycle have direct Java/native or native runtime coverage. These
  are promoted slices, not a serialization of arbitrary pathfinder task stacks.
- Nine focused core HUD states are bit-exact: armor, absorption+armor, both
  hurt-flash phases, poisoned hunger, partial air, XP, durability, and the
  dragon boss bar. Both inside-block overlays are also full-frame exact. The
  death title, score, buttons, and source-gradient tint formula are exact; the
  full death world-underlay composition remains a soft diagnostic.
- Eight visible inventory interaction states and close behavior are exact,
  including pickup, place, split, single deposit, quick move, hotbar swap, and
  drop. Table, furnace, chest, and inventory chrome are exact. The inventory
  player preview remains an open sub-LSB shading gate at `0.011641/ch` and
  `0.009949/ch` on the two primary poses, with sparse held-out edge residuals.
- Real Java A/B capture validity passes for all 16 focused entity/particle
  states. The C gate correctly remains red for all 16. The three hand-use
  poses, portal composition, and underwater composition also remain strict
  residuals. These failures are retained as fail-closed regression tests; no
  tolerance was widened to manufacture a pass.
- The four-seed wrapper worldgen census passes its blessed exact counts and
  hashes. The complete locally runnable quick pyramid passes all 13 stages and
  skips only the two undistributed `.bsnp` inputs. The clean native game suite,
  Java 8 build, capsule selftest, villager/golem focused gates, and legal
  fresh-spawn-to-credits route pass. The clean full native suite takes 9:59.65
  at 1,101,216 KiB peak RSS with zero swap; the quick sweep takes 1:07.31 at
  715,556 KiB peak RSS.
- Scalar throughput is 4,850 steps/s against the original 4,062 baseline and
  frozen 3,858.9 floor. The new state is fixed-capacity or cold-allocated and
  adds no unbounded hot-tick scans. GPU 1 was not touched while co-tenanted.

The remaining acceptance program is therefore concrete: arbitrary task/path
and target NBT, remaining runtime species (principally the horse family and
Wither), cross-feature loaded/chunk/scheduled/random ordering, exhaustive
redstone/rail topologies, custom/tagged NBT, and zeroing the strict render
residual queue. Until those general gates pass, "full replica" and "universal
bit/pixel equivalence" remain intentionally unclaimed.
