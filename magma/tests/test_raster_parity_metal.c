...
int main(void) {
    ...
    struct { const char *name; const ScnVert *v; int nv; CrShadeCtx sh; int strict; } cases[] = {
        ...
        { "D24_BIAS",    SCN_TRANS_VERTS, SCN_TRANS_NVERTS,
          { &tex_trans, fog, 0,0, 0,0, CR_LAYER_TRANSLUCENT,    5, 0, 0 }, 1 },
    ...
    cases[3].sh.depth_d24 = 1;
    cases[3].sh.depth_bias_d24 = 4;
    cases[3].sh.subpixel_bits = 8;
    ...
    for (int i = 0; i < (int)(sizeof(cases)/sizeof(cases[0])); ++i) {
        ...
        int depth_writes = cases[i].sh.blend == 0 || cases[i].sh.blend == 5;
        int ok = (maxd == 0) && (!depth_writes || depth_bad == 0);
        /* Ordinary blend modes suppress depth writes; D24_BIAS intentionally
         * retains them and also exercises fixed subpixel coverage. */
        if (!ok) fail = 1;
