...
MOB_CLASS = {
    ...
    69: "EntityDonkey",
    70: "EntityMule",
    71: "EntitySkeletonHorse",
    72: "EntityZombieHorse",
}
SPAWNER_ENTITY_NAMES = {
    value: key for key, value in state_capsule.SPAWNER_ENTITY_TYPES.items()
}
...
def write_script(
        ...
        trigger_command_block_from_capsule=False,
        command_trigger_clock=None):
    ...
    if trigger_command_block_from_capsule:
        command_states = [
            row for row in events
            if row.get("type") == "set_command_block_state"
        ]
        if len(command_states) != 1:
            raise ValueError(
                "deferred command trigger requires exactly one command block "
                f"in the capsule, found {len(command_states)}"
            )
        source = command_states[0]
        fields = {
            key: source[key] for key in ("dim", "x", "y", "z")
        }
        if command_trigger_clock:
            fields.update(command_trigger_clock)
        events.append(event(0, "trigger_command_block", **fields))
...
def canonicalize(tick, raw):
    ...
    player = {
        "name": raw.get("player_name", "Player"),
        "x": raw.get("server_x") if server_position else raw["x"],
        "y": raw.get("server_y") if server_position else raw["y"],
        "z": raw.get("server_z") if server_position else raw["z"],
        "yaw": raw["yaw"],
        "pitch": raw["pitch"],
        "vx": raw.get("server_vx") if server_position else raw["vx"],
        "vy": raw.get("server_vy") if server_position else raw["vy"],
        "vz": raw.get("server_vz") if server_position else raw["vz"],
        ...
        "dead": raw["dead"],
        "creative": bool(raw.get("creative", False)),
        "game_mode": raw.get("game_mode", 0),
        "deaths": raw["deaths"],
        "dim": raw["dim"],
        "spawn_present": bool(raw.get("player_spawn_present", False)),
        "spawn_x": raw.get("player_spawn_x", 0),
        "spawn_y": raw.get("player_spawn_y", 0),
        "spawn_z": raw.get("player_spawn_z", 0),
        "spawn_forced": bool(raw.get("player_spawn_forced", False)),
        "trigger_qrl_present": bool(raw.get("trigger_qrl_present", False)),
        "trigger_qrl_score": raw.get("trigger_qrl_score", 0),
        "trigger_qrl_locked": bool(raw.get("trigger_qrl_locked", False)),
        "achievement_open_inventory": bool(raw.get(
            "stat_achievement_open_inventory", 0)),
        "potions": raw["potions"],
    }
    inventory = [
        {
        ...
        entities.append(
            {
                ...
                "age": entity.get("age"),
                "throwable_exact": entity.get("throwable_exact"),
                "potion_exact": entity.get("potion_exact"),
                "potion_item": entity.get("potion_item"),
                "potion_type": entity.get("potion_type"),
                "firework_exact": entity.get("firework_exact"),
                "firework_age": entity.get("firework_age"),
                "lifetime": entity.get("lifetime"),
                "ticks_existed": entity.get("ticks_existed"),
                "attached_player": entity.get("attached_player"),
                "flight": entity.get("flight"),
                "explosion_count": entity.get("explosion_count"),
                "large_blast": entity.get("large_blast"),
                "twinkle": entity.get("twinkle"),
                "firework_item_present": entity.get(
                    "firework_item_present"),
                "firework_item": entity.get("firework_item"),
                "firework_count": entity.get("firework_count"),
                "firework_meta": entity.get("firework_meta"),
                "prev_yaw": entity.get("prev_yaw"),
                "prev_pitch": entity.get("prev_pitch"),
                "player_thrower": entity.get("player_thrower"),
                "ignore_player": entity.get("ignore_player"),
                "ignore_player_time": entity.get("ignore_player_time"),
                "pearl_private_thrower": entity.get(
                    "pearl_private_thrower"),
                "portal_cooldown": entity.get("portal_cooldown"),
                ...
                "spawner_entity_id": SPAWNER_ENTITY_NAMES.get(
                    entity.get("spawner_entity")),
                "spawner_delay": entity.get("spawner_delay"),
                "spawner_min_delay": entity.get("spawner_min_delay"),
                "spawner_max_delay": entity.get("spawner_max_delay"),
                "spawner_spawn_count": entity.get(
                    "spawner_spawn_count"),
                "spawner_max_nearby": entity.get("spawner_max_nearby"),
                "spawner_activate_range": entity.get(
                    "spawner_activate_range"),
                "spawner_spawn_range": entity.get("spawner_spawn_range"),
                "spawner_spawn_data_nbt": (
                    nbt_codec.canonical_hex(
                        entity["spawner_spawn_data_nbt"])
                    if entity.get("spawner_spawn_data_nbt") is not None
                    else None),
                "spawner_default_entity_nbt": entity.get(
                    "spawner_default_entity_nbt"),
                "spawner_potentials": [
                    {
                        "weight": potential["weight"],
                        "entity_id": SPAWNER_ENTITY_NAMES.get(
                            potential.get("entity")),
                        "entity_nbt": nbt_codec.canonical_hex(
                            potential["entity_nbt"]),
                        "default_entity_nbt":
                            potential["default_entity_nbt"],
                    }
                    for potential in entity.get("spawner_potentials", [])
                ],
                ...
                **({field: entity.get(field) for field in (
                    "uuid_most", "uuid_least", "no_ai", "air", "fire",
                    "on_ground", "fall_distance", "in_water",
                    "ticks_existed", "base_living_sound_time",
                    "base_last_damage", "base_entity_seed48",
                    "base_entity_have_gaussian", "base_entity_gaussian",
                    ...
                    "bat_active_exact", "bat_hanging",
                    "bat_spawn_valid", "bat_spawn_x", "bat_spawn_y",
                    "bat_spawn_z", "bat_head_yaw",
                    "bat_render_yaw_offset",
                    "bat_body_rotation_tick_counter",
                    "bat_body_prev_head_yaw", "bat_move_forward",
                    "bat_move_strafing", "bat_entity_age",
                    "bat_persistence_required",
                    "squid_active_exact", "squid_pitch",
                    "squid_prev_pitch", "squid_yaw", "squid_prev_yaw",
                    "squid_rotation", "squid_prev_rotation",
                    "squid_tentacle_angle", "squid_last_tentacle_angle",
                    ...
                    "horse_exact", "horse_subtype", "horse_growing_age",
                    "horse_forced_age", "horse_forced_age_timer",
                    "horse_in_love", "horse_tame", "horse_saddled",
                    "horse_bred", "horse_eating", "horse_rearing",
                    "horse_mouth_open", "horse_temper",
                    "horse_owner_present", "horse_owner_uuid_most",
                    "horse_owner_uuid_least", "horse_variant",
                    "horse_armor", "horse_chested", "horse_trap",
                    "horse_trap_time", "horse_jump_strength",
                    ...
                    "horse_jumping", "horse_jump_power",
                    "horse_allow_stand_sliding", "horse_head_lean",
                    ...
                    "horse_inventory", "llama_strength", "llama_decor",
                    "llama_did_spit", "llama_leashed",
                    "llama_leash_holder_kind", "llama_leash_holder_eid",
                    "llama_leash_holder_uuid_most",
                    "llama_leash_holder_uuid_least",
                    "llama_leash_pending", "llama_leash_pending_x",
                    "llama_leash_pending_y", "llama_leash_pending_z",
                    "llama_caravan_head_eid", "llama_caravan_tail_eid",
                    "llama_caravan_speed", "llama_caravan_dist_counter",
                )} if entity.get("kind") == "mob" else {}),
            }
        )
        ...
        (
            {
                ...
                **({"stack_payload": {
                    "kind": "item_tag",
                    "nbt": nbt_codec.canonical_hex(
                        entry["stack_payload"]["nbt"]),
                }}
                   if isinstance(entry.get("stack_payload"), dict)
                   and entry["stack_payload"].get("kind") == "item_tag"
                   else {}),
            }
            for entry in raw.get("item_frames", [])
        ),
        key=lambda entry: (
    ...
    return {
        ...
        "default_game_mode": raw.get("default_game_mode", 0),
        "difficulty": raw.get("difficulty", 2),
        "world_border": {
            "center_x": raw.get("border_center_x", 0.0),
            "center_z": raw.get("border_center_z", 0.0),
            "diameter": raw.get("border_diameter", 60000000.0),
            "target_diameter": raw.get(
                "border_target_diameter", 60000000.0),
            "time_until_target": raw.get("border_time_until_target", 0),
            "damage_amount": raw.get("border_damage_amount", 0.2),
            "damage_buffer": raw.get("border_damage_buffer", 5.0),
            "warning_time": raw.get("border_warning_time", 15),
            "warning_distance": raw.get("border_warning_distance", 5),
        },
        "world_spawn": {
            "x": raw.get("world_spawn_x", 0),
            "y": raw.get("world_spawn_y", 64),
            "z": raw.get("world_spawn_z", 0),
        },
        ...
        "world_rng": {
            ...
            **({"collections_seed48": raw.get(
                "collections_rand_seed48")}
               if raw.get("collections_rand_seed48") is not None else {}),
        ...
        "controlled_input": (
            {
                ...
                "world_rng": {
                    ...
                    "block_seed48": raw["controlled_input"].get(
                        "block_rand_seed48"),
                    "inventory_helper_seed48": raw[
                        "controlled_input"].get(
                            "inventory_helper_rand_seed48"),
                    "inventory_helper_have_gaussian": raw[
                        "controlled_input"].get(
                            "inventory_helper_rand_have_gaussian", False),
                    "inventory_helper_gaussian": raw[
                        "controlled_input"].get(
        ...
        "inventory": inventory,
        "entities": entities,
        "weather_effects": raw.get("weather_effects", []),
        ...
        "village_collection_tick": raw.get(
            "village_collection", {}).get("tick", 0),
        "villages": raw.get(
            "village_collection", {}).get("villages", []),
...
def parse_args():
    ...
    parser.add_argument(
        "--trigger-command-block-from-capsule", action="store_true",
        help="trigger the capsule's sole command block before tape tick 0",
    )
    parser.add_argument(
        "--command-trigger-clock-file",
        help="Java output-clock JSON for deterministic deferred triggering",
    )
    parser.add_argument(
        "--xp-fixture",
    ...
    if args.trigger_command_block_from_capsule and not args.capsule:
        parser.error("--trigger-command-block-from-capsule requires --capsule")
    if args.command_trigger_clock_file \
            and not args.trigger_command_block_from_capsule:
        parser.error("--command-trigger-clock-file requires a deferred trigger")
...
def main():
    ...
    command_trigger_clock = None
    if args.command_trigger_clock_file:
        command_trigger_clock = json.loads(pathlib.Path(
            args.command_trigger_clock_file).read_text(encoding="utf-8"))
        if command_trigger_clock and (
                set(command_trigger_clock) not in (
                    {"hour", "minute", "second"},
                    {"hour", "minute", "second",
                     "weather_duration_ticks"},
                    {"hour", "minute", "second",
                     "teleport_client_delay_ticks",
                     "teleport_client_pre_tick"})
                or not 0 <= command_trigger_clock["hour"] <= 23
                or not 0 <= command_trigger_clock["minute"] <= 59
                or not 0 <= command_trigger_clock["second"] <= 59
                or "weather_duration_ticks" in command_trigger_clock and (
                    not 6000 <= command_trigger_clock[
                        "weather_duration_ticks"] <= 17980
                    or command_trigger_clock["weather_duration_ticks"] % 20
                )
                or "teleport_client_delay_ticks" in command_trigger_clock
                and not 1 <= command_trigger_clock[
                    "teleport_client_delay_ticks"] <= 100
                or "teleport_client_pre_tick" in command_trigger_clock
                and command_trigger_clock[
                    "teleport_client_pre_tick"] not in (0, 1)):
            raise SystemExit("invalid command trigger clock sidecar")
    write_script(
        args.script_out,
        tape,
        spawn,
        ...
        args.trigger_command_block_from_capsule,
        command_trigger_clock,
    )
