...
MC_HD static inline void elb_move_with_heading_contacts(
        EbLiving *e, float strafe, float forward, float ground_slip,
...
MC_HD static inline void elb_on_living_update_contacts(
        EbLiving *e, float ground_slip, int isMovementBlocked,
        const PcfBlock *blocks, int nblocks,
        const int (*contact_cells)[4], int ncontacts,
        const McSinTable *st) {
...
MC_HD static inline void eb_tick_living_contacts(
        EbLiving *e, float ground_slip, int isMovementBlocked,
        const PcfBlock *blocks, int nblocks,
        const int (*contact_cells)[4], int ncontacts,
        const McSinTable *st) {
    eb_on_entity_update(&e->base);
    elb_on_living_update_contacts(
