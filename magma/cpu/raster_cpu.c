...
static inline float cr_depth24(float z, const CrShadeCtx *sh) {
    return floorf(z * 16777215.0f + 0.5f)
        - (float)sh->depth_bias_d24;
}

static inline float cr_subpixel(float v, const CrShadeCtx *sh) {
...
void cr_raster_cpu(CrFramebuffer *fb, const CrScreenTri *tris, int ntris,
    ...
    const char *probe = cr_cfg()->raster_probe;
    int probe_all = !strcmp(probe, "all");
    if (probe[0]) (void)sscanf(probe, "%d,%d", &probe_x, &probe_y);
    ...
    for (int t = 0; t < ntris; t++) {
        ...
        for (int py = miny; py < maxy; py++) {
            for (int px = minx; px < maxx; px++) {
                ...
                else if (sh->depth_d24) {
                    /* The Java client explicitly requests a 24-bit depth
                     * buffer.  Apply LESS/LEQUAL after fixed-point conversion. */
                    float z24 = cr_depth24(z, sh);
                    float d24 = floorf(
                        fb->depth[idx] * 16777215.0f + 0.5f);
                    depth_ok = z24 < d24
                        || (sh->depth_lequal && z24 == d24);
                }
                else
                ...
                if (probe_all || (px == probe_x && py == probe_y)) {
                    CrRgba texel = cr_atlas_sample(sh->atlas, frag.uv.x, frag.uv.y);
                    ...
                    if (sh->lightmap && frag.light >= 0.0f) {
                        ...
                        int s1 = s0 < 15 ? s0 + 1 : 15;
                        int b1 = b0 < 15 ? b0 + 1 : 15;
                        float fs = s - (float)s0, fb = b - (float)b0;
                        float w00 = (1.0f - fs) * (1.0f - fb);
                        float w01 = (1.0f - fs) * fb;
                        float w10 = fs * (1.0f - fb), w11 = fs * fb;
                        const CrRgba *lm = sh->lightmap;
#define LMC(C) (((float)lm[s0*16+b0].C*w00 \
                            + (float)lm[s0*16+b1].C*w01 \
                            + (float)lm[s1*16+b0].C*w10 \
                            + (float)lm[s1*16+b1].C*w11) * (1.0f/255.0f))
                        lmr = LMC(r); lmg = LMC(g); lmb = LMC(b);
#undef LMC
                    }
                    fprintf(stderr,
                            "raster_probe x=%d y=%d tri=%d uv=%.9g,%.9g "
                            "texel=%u,%u,%u,%u light=%.9g ao=%.9g blk=%.9g "
                            "tint=%u,%u,%u,%u lm=%.9g,%.9g,%.9g "
                            "out=%u,%u,%u,%u z=%.9g "
                            "v0=%.9g,%.9g,%.9g v1=%.9g,%.9g,%.9g "
                            "v2=%.9g,%.9g,%.9g raw_y=%.9g,%.9g,%.9g\n",
                            px, py, t, frag.uv.x, frag.uv.y,
                            texel.r, texel.g, texel.b, texel.a,
                            frag.light, frag.ao, frag.blk,
                            frag.tint.r, frag.tint.g, frag.tint.b, frag.tint.a,
                            lmr, lmg, lmb,
                            c.r, c.g, c.b, c.a, z,
                            x0, y0, v0->spos.z,
                            x1, y1, v1->spos.z,
                ...
                if (sh->blend == 1 || sh->blend == 4 || sh->blend == 6) {
                    ...
                    if (sh->blend == 6)
                        fb->color[idx].a = (u8)(fminf(255.0f,
                            c.a * a + d.a * ia) + 0.5f);
                    else
                        fb->color[idx].a = 255;
                    if (sh->blend == 4 || sh->blend == 6)
                        fb->depth[idx] = sh->depth_d24
                            ? cr_depth24(z, sh) / 16777215.0f
                            : z;
                } else if (sh->blend == 2) {
