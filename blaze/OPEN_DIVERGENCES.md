...
Last documented: 2026-08-25 spawn->dragon policy vs magma (section below). No new M1/M2.
...
- Native magma eval is wired (`eval --backend magma --transfer closed|replay`)
  and the 32-tick BOLR harness is `make -C blaze/rl test-eval-magma`. The
  13-seed ladder is not yet measured. Camera stays compile-time 64x36
  (`oc_pixel`); `--rl-bin` is not a resolution flag. GATES row 2.
