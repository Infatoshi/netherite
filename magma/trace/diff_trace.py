...
PLAYER_FEATURES = [
    ...
    ("player.spawn",        ["spawn_present", "spawn_x", "spawn_y",
                              "spawn_z", "spawn_forced"]),
]
...
def inv_map(row):
    """slot -> complete represented ItemStack tuple for all player slots."""
    d = {}
    for it in (row.get("inventory") or []):
        ...
        d[it["slot"]] = (
            ...
            it.get("repair_cost", 0), it.get("custom_name", ""))
    return d
...
def diff_inventory(ja, cc, n):
    ...
    if first_full is None:
        r.status = "match"
        r.note = ("all 41 slots, counts, ids, metadata, enchantments, "
                  "repair costs, and custom names identical")
    else:
        r.status = "diverge"
...
def diff_time(ja, cc, n):
    ...
    for i in range(n):
        ...
        for k in (
            ...
            jv, cv = jt.get(k), ct.get(k)
            strength = k in (
                "prev_rain_strength", "rain_strength",
                "prev_thunder_strength", "thunder_strength")
            equal = (
                struct.pack(">f", float(jv))
                == struct.pack(">f", float(cv))
                if strength and jv is not None and cv is not None
                else jv == cv)
            if not equal:
...
def diff_world_spawn(ja, cc, n):
    r = FeatureResult("world_spawn")
    for i in range(n):
        java_spawn = ja[i].get("world_spawn")
        magma_spawn = cc[i].get("world_spawn")
        if java_spawn != magma_spawn:
            r.status = "diverge"
            r.first_tick = ja[i]["tick"]
            r.first_field = "world_spawn"
            r.first_delta = float("inf")
            r.note = f"Java {java_spawn!r} vs C {magma_spawn!r}"
            return r
    return r


def diff_weather_effects(ja, cc, n):
    r = FeatureResult("entities.weather_effects")
    for i in range(n):
        java_effects = ja[i].get("weather_effects", [])
        magma_effects = cc[i].get("weather_effects", [])
        if java_effects != magma_effects:
            r.status = "diverge"
            r.first_tick = ja[i]["tick"]
            r.first_field = "weather_effects"
            r.first_delta = float("inf")
            r.note = f"Java {java_effects!r} vs C {magma_effects!r}"
            return r
    return r
...
def controlled_input_transition(bundle):
    ...
    if any(key in before_rng or key in after_rng
           for key in inventory_fields):
        steps = java_random_transition_count(
            before_rng.get("inventory_helper_seed48"),
            after_rng.get("inventory_helper_seed48"))
        before_have = before_rng.get("inventory_helper_have_gaussian")
        after_have = after_rng.get("inventory_helper_have_gaussian")
        if steps is None or not isinstance(before_have, bool) \
                or not isinstance(after_have, bool):
            return None
        transitions["inventory_helper_seed48"] = steps
        transitions["inventory_helper_gaussian_cache"] = (
            before_have, after_have)
...
def diff_note_blocks(ja, cc, n):
    r = FeatureResult("tile_entities.note_blocks")
    for i in range(n):
        java_complete = ja[i].get("note_blocks_complete") is True
        magma_complete = cc[i].get("note_blocks_complete") is True
        jt = ja[i].get("note_blocks", [])
        ct = cc[i].get("note_blocks", [])
        if not java_complete or not magma_complete or jt != ct:
            r.status = "diverge"
            r.first_tick = ja[i]["tick"]
            r.first_field = (
                "completeness" if not java_complete or not magma_complete
                else "note-powered-state-list"
            )
            r.first_delta = float("inf")
            r.note = f"Java {jt!r} vs C {ct!r}"
            return r
    counts = [len(row.get("note_blocks", [])) for row in ja[:n]]
    r.note = (
        "exact note-block tile state, max="
        f"{max(counts) if counts else 0}"
    )
    return r
...
def diff_decorative_tiles(ja, cc, n):
    r = FeatureResult("tile_entities.decorative")
    for i in range(n):
        java_complete = ja[i].get("decorative_tiles_complete") is True
        magma_complete = cc[i].get("decorative_tiles_complete") is True
        try:
            jt = [
                {
                    **entry,
                    "tile_nbt": nbt_codec.canonical_hex(entry["tile_nbt"]),
                    **({"drop_nbt": nbt_codec.canonical_hex(
                        entry["drop_nbt"])}
                       if "drop_nbt" in entry else {}),
                }
                for entry in ja[i].get("decorative_tiles", [])
            ]
            ct = [
                {
                    **entry,
                ...
                for entry in cc[i].get("decorative_tiles", [])
            ]
        except (KeyError, nbt_codec.NbtError) as exc:
            r.status = "diverge"
            r.first_tick = ja[i]["tick"]
            r.first_field = "nbt"
            r.first_delta = float("inf")
            r.note = f"invalid decorative tile/drop NBT: {exc}"
            return r
        if not java_complete or not magma_complete or jt != ct:
            r.status = "diverge"
            r.first_tick = ja[i]["tick"]
            r.first_field = (
                "completeness" if not java_complete or not magma_complete
                else "tile-and-drop-nbt-list"
            )
            r.first_delta = float("inf")
            r.note = f"Java {jt!r} vs C {ct!r}"
            return r
    counts = [len(row.get("decorative_tiles", [])) for row in ja[:n]]
    r.note = (
        "exact sign/banner tile and drop NBT, max="
        f"{max(counts) if counts else 0}"
    )
    return r
...
def diff_item_frames(ja, cc, n):
    ...
    r.note = (
        "exact bounded item-frame lifecycle state, max="
        f"{max(counts) if counts else 0}"
    )
    return r


def diff_hanging_entities(ja, cc, n):
    r = FeatureResult("entities.hanging")
    for i in range(n):
        for key in ("paintings", "leash_knots"):
            java_complete = ja[i].get(f"{key}_complete") is True
            magma_complete = cc[i].get(f"{key}_complete") is True
            java_values = ja[i].get(key, [])
            magma_values = cc[i].get(key, [])
            if not java_complete or not magma_complete \
                    or java_values != magma_values:
                r.status = "diverge"
                r.first_tick = ja[i]["tick"]
                r.first_field = (
                    "completeness"
                    if not java_complete or not magma_complete else key)
                r.first_delta = float("inf")
                r.note = f"Java {java_values!r} vs C {magma_values!r}"
                return r
    painting_counts = [len(row.get("paintings", [])) for row in ja[:n]]
    knot_counts = [len(row.get("leash_knots", [])) for row in ja[:n]]
    r.note = (
        "exact painting/knot live state, max="
        f"{max(painting_counts) if painting_counts else 0}/"
        f"{max(knot_counts) if knot_counts else 0}"
    )
    return r
...
def run_state(args):
    ...
    results.append(diff_note_blocks(ja, cc, n))
    results.append(diff_skulls(ja, cc, n))
    results.append(diff_decorative_tiles(ja, cc, n))
    results.append(diff_item_frames(ja, cc, n))
    results.append(diff_hanging_entities(ja, cc, n))
    ...
    results.append(diff_world_spawn(ja, cc, n))
    results.append(diff_weather_effects(ja, cc, n))
...
def selftest_weather_float_diff():
    java = [{"tick": 0, "time": {
        "rain_strength": 0.01, "prev_rain_strength": 0.0,
        "thunder_strength": 0.01, "prev_thunder_strength": 0.0,
    }}]
    same_bits = [{"tick": 0, "time": {
        "rain_strength": 0.00999999978, "prev_rain_strength": 0,
        "thunder_strength": 0.00999999978, "prev_thunder_strength": 0,
    }}]
    if diff_time(java, same_bits, 1).status != "match":
        raise AssertionError("equivalent float32 weather spellings diverged")
    next_bits = json.loads(json.dumps(same_bits))
    raw = struct.unpack(">I", struct.pack(">f", 0.01))[0] + 1
    next_bits[0]["time"]["rain_strength"] = struct.unpack(
        ">f", struct.pack(">I", raw))[0]
    result = diff_time(java, next_bits, 1)
    if result.status != "diverge" or result.first_field != "rain_strength":
        raise AssertionError("one-ULP weather mutation did not diverge")
    print("diff_trace weather-float selftest PASS "
          "(same bits match, one ULP diverges)")
    return 0
...
def main():
    ...
    ap.add_argument("--selftest-weather-float", action="store_true")
    ...
    if args.selftest_weather_float:
        return selftest_weather_float_diff()
    if not args.java or not args.c:
        ap.error("--java and --c are required unless a selftest is used")
