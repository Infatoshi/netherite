...
int main(void)
{
    ...
    {
        GmPlayerView v; gm_runtime_view(&r, &v);
        /* Keep this single-chest fixture one block clear of the earlier
         * player chest. Adjacent ordinary chests form a 54-slot container,
         * whose north/west half is intentionally active regardless of which
         * half was clicked. */
        int bx = (int)v.x + 3, by = (int)v.y, bz = (int)v.z;
        if (by < 1) by = 1;
        gm_world_ensure(r.world, bx >> 4, bz >> 4, 0);
    ...
    /* Full 27-slot chest break: no silent item loss (GM_LIVE_MAX + overflow). */
    {
              ...
              "full chest break does not hard-fail spawn (no silent loss)");
        {
            ...
            for (int i = 0; i < r.entities.n_overflow; ++i) {
                if (r.entities.overflow[i].entity.item == 403 &&
                    r.entities.overflow[i].entity.n_enchants == 1)
                    book_ok = 1;
            }
            CHECK(book_ok, "full-chest break keeps enchanted-book payload");
        }
        (void)overflow_before;
        /* Cold entities are active entities, not a lossy deferred-spawn
         * queue. They remain cold and receive normal item ticks. */
        {
            int cold_before = gm_live_overflow_count(&r.entities);
            int age_before = cold_before > 0
                ? r.entities.overflow[0].entity.age : 0;
            GmAction idle; memset(&idle, 0, sizeof idle); idle.hotbar_sel = -1;
            for (int t = 0; t < 5; ++t) gm_runtime_tick(&r, idle);
            CHECK(cold_before == 0
                  || gm_live_overflow_count(&r.entities) <= cold_before,
                  "cold item tick does not duplicate overflow entities");
            CHECK(cold_before == 0
                  || gm_live_overflow_count(&r.entities) < cold_before
                  || r.entities.overflow[0].entity.age == age_before + 5,
                  "occupied cold item receives every normal entity tick");
        }
    }

    /* The old 32-entry overflow boundary is cold-growable. */
    {
        enum { COLD_EXTRA = GM_LIVE_OVERFLOW_INITIAL * 8 + 1 };
        const char *checkpoint = "game/.overflow-capacity-checkpoint.bin";
        int fails0, held = 0;
        for (int i = 0; i < GM_LIVE_MAX; ++i) r.entities.ents[i].active = 0;
        r.entities.n_active = 0;
        r.entities.n_overflow = 0;
        ...
        for (int i = 0; i < GM_LIVE_MAX + COLD_EXTRA; ++i) {
            ICStack st = ic_mk(4 /* cobble */, 1, 0);
            if (gm_live_spawn_stack(&r.entities, 0.5, 64.0, 0.5, st, 10))
                held++;
        }
        for (int i = 0; i < gm_live_entity_slot_count(&r.entities); ++i) {
            GmLiveEnt *entity = gm_live_entity_mut(&r.entities, i);
            if (!entity || !entity->active) continue;
            entity->x = 1000.5 + (double)i * 2.0;
            entity->y = 240.0;
            entity->z = 1000.5;
            entity->mx = entity->my = entity->mz = 0.0;
            entity->controlled_stationary = 1;
            entity->no_gravity = 1;
        }
        CHECK(held == GM_LIVE_MAX + COLD_EXTRA
              && gm_live_overflow_count(&r.entities) == COLD_EXTRA
              && r.entities.overflow_cap >= COLD_EXTRA,
              "opening >old overflow capacity succeeds via cold growth");
        CHECK(gm_live_spawn_fail_count(&r.entities) == fails0,
              "cold growth does not report a false spawn rejection");
        CHECK(r.entities.overflow[0].entity.item == 4
              && r.entities.overflow[0].entity.pickup_delay == 10
              && r.entities.overflow[COLD_EXTRA - 1].entity.item == 4,
              "cold-grown overflow retains first/last FIFO payload");
        CHECK(gm_runtime_write_checkpoint(&r, checkpoint)
              && gm_runtime_load_checkpoint(&r, checkpoint)
              && gm_live_overflow_count(&r.entities) == COLD_EXTRA
              && r.entities.overflow_cap >= COLD_EXTRA
              && r.entities.overflow[0].entity.item == 4
              && r.entities.overflow[0].entity.pickup_delay == 10
              && r.entities.overflow[COLD_EXTRA - 1].entity.item == 4,
              "checkpoint restores >old-capacity overflow FIFO exactly");
        {
            GmLiveEnt before = r.entities.overflow[0].entity;
            GmAction idle;
            memset(&idle, 0, sizeof idle);
            idle.hotbar_sel = -1;
            gm_runtime_tick(&r, idle);
            CHECK(r.entities.overflow[0].occupied
                      && r.entities.overflow[0].entity.age == before.age + 1
                      && r.entities.overflow[0].entity.ticks_existed
                          == before.ticks_existed + 1
                      && r.entities.overflow[0].entity.pickup_delay
                          == before.pickup_delay - 1
                      && r.entities.overflow[0].entity.x == before.x
                      && r.entities.overflow[0].entity.y == before.y
                      && r.entities.overflow[0].entity.z == before.z,
                  "checkpoint-restored cold item continues one exact active tick");
        }
        (void)remove(checkpoint);
        (void)remove("game/.overflow-capacity-checkpoint.bin.tmp");
    ...
    /* Growable TE: opening many unique chests retains live TEs (no eviction). */
    {
        ...
        for (int n = 0; n < n_open; ++n) {
            ...
            gm_world_ensure(r.world, bx >> 4, bz >> 4, 0);
            CHECK(gm_runtime_set_block(&r, bx, by, bz, 54, 3)
                      && gm_runtime_set_block(&r, bx, by + 1, bz, 0, 0),
                  "place growable-table chest with explicit lid headroom");
            gm_runtime_set_pose(&r, bx + 0.5, by, bz + 0.5, 0.0f, 0.0f);
