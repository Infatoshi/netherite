...
frame grabbed every --cadence ticks. craster side: the same actions as a
...
yaw/pitch), pinned on the oracle with a tp before tick 0 and set at craster
tick 0 with set_pose. Same seed-0 worldgen -> same terrain contact.

Outputs (--out): oracle_trace.jsonl, craster state, divergence.csv
...
def run_oracle(start, acts, cadence, outdir, seed):
    ...
    # craster renders no entities; remove worldgen animals so frame diffs are clean
...
def run_craster(start, acts, outdir, seed):
    scr = os.path.join(outdir, "craster_script.jsonl")
    with open(scr, "w") as f:
        ...
        for t, a in enumerate(acts):
            ev = to_craster_event(t, a)
            if len(ev) > 2:
                f.write(json.dumps(ev) + "\n")
    frames = os.path.join(outdir, "craster_frames")
    os.makedirs(frames, exist_ok=True)
    state = os.path.join(outdir, "craster_state.jsonl")
    t0 = time.time()
    ol.run_craster_script(scr, len(acts), frames, state, w=W, h=H, seed=seed,
                          timeout=3600)
    print(f"[traj] craster ran {len(acts)} ticks in {time.time()-t0:.1f}s")
    rows = []
...
def main():
    ...
    c_rows, c_frames_dir = run_craster(start, acts, args.out, args.seed)
    ...
    a1.set_title("Oracle (Java MC 1.11.2) vs craster: position divergence, fixed 200-tick trajectory")
    ...
    for t, mc_png in sorted(j_frames.items()):
        ...
        if not os.path.exists(ppm):
            continue
        cpng = os.path.join(args.out, f"craster_t{t:04d}.png")
    ...
    with open(md, "w") as f:
        ...
        f.write("Both games start pinned at the same pose "
                ...
                "craster --state-out per tick. Frames diffed every "
    ...
    # keep out/ lean: drop the full craster frame dump (cadence PNGs are kept)
