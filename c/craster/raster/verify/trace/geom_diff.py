"""Geometry oracle differ: vanilla ModelRenderer part poses vs craster's.
...
Craster side (CRASTER_GEOM_DUMP=<path> on the replay binary): text lines
  D <tick> <label> rpx rpy rpz rx ry rz
with the exact args craster passes to er_dragon_part (same units: rotation
...
so vanilla 'spine' compares against craster 'tail11'. Craster's neck0-4 /
...
Warmup: craster seeds the 64-entry trail ring flat at tape start while the
...
PART_MAP = {  # vanilla field -> craster label
...
def load_java(path, cls):
    out = defaultdict(dict)          # tick -> {craster_label: [6 floats]}
...
def compare(jv, cr, offset, warmup=WARMUP, verbose=False, max_report=40):
    ...
    if verbose:
        for _, t, lbl, pe, ae, jvals, cvals in worst[:max_report]:
            ...
            print(f"    craster {['%.5f' % v for v in cvals]}")
...
def main():
    ...
    ap.add_argument("--craster", required=True, help="CRASTER_GEOM_DUMP file")
    ...
    cr = load_craster(args.craster)
    print(f"[geom] java ticks={len(jv)} craster ticks={len(cr)} cls={args.cls}")
    if not jv or not cr:
