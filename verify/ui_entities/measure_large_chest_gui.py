#!/usr/bin/env python3
"""Strict real-Java pixel gate for the 54-slot Large Chest panel."""
...
# GuiChest at 854x480, GUI scale 2. ySize=222 centers at framebuffer y=18;
# the generic_54 composite is 221 source pixels high. As in the established
# single-chest gate, the transparent first source row and corner cutouts are
# not panel-owned.
PANEL = (250, 20, 602, 460)
...
def mutation_selftest() -> None:
    ...
    if result["different_px"] != 1 or result["max_channel"] != 1:
        raise RuntimeError("Large Chest GUI negative control was not detected")
...
def main() -> int:
    ...
    java_a = load(goldens / "gui_large_chest_a.png")
    java_b = load(goldens / "gui_large_chest_b.png")
    native = load(Path(args.c_frame))
    if java_a.shape != java_b.shape or java_a.shape != native.shape:
        raise RuntimeError("Large Chest GUI frame dimensions differ")
    ...
    report = {
        ...
        "excluded": "transparent first source row and four 6x6 corners",
        "java_ab": java_ab,
    ...
    if not stable:
        raise RuntimeError("real-Java Large Chest GUI capture is unstable")
    if not exact:
        raise RuntimeError("native Large Chest GUI panel differs from Java")
    print("PASS Large Chest GUI: owned pixels exact vs real Java")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL Large Chest GUI: {error}")
        raise SystemExit(1)
