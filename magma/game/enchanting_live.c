...
{
    if (slot == 0)
        /* ContainerEnchantment's item Slot explicitly accepts every stack;
         * non-enchantable items simply produce three empty offers. */
        return !stack || stack->count <= 0 || stack->item != 0;
    if (slot == 1)
