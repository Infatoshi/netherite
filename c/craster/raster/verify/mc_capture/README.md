...
> Verification entry point: `c/craster/VERIFY.md`. This dir holds the legacy
...
the coarse whole-frame golden that craster's full seed->raster path is diffed
against.

## Run
    bash capture.sh          # capture the golden (needs the live game on :1)
    make -C ../../.. rung4-verify        # single-shot craster vs golden
    make -C ../../.. hard-scene-verify   # hard leaf-canopy scene + ablations
Outputs (this dir): `mc_frame.png` (854x480 window content), `pose.json`,
`craster_frame.png` (craster's render of the same scene/pose, written by
`run_rung4.sh`), `hard_scene_craster.png` (baseline from hard-scene).

### Hard scene verifier
See `hard_scene.json` + `run_hard_scene.sh`. Full-stack integration golden on the
seed-0 elevated forest pose (the known crop~27 case). Ablates `CRASTER_SMOOTH` /
`CRASTER_FOG` / `CRASTER_GAMMA` and prints a scoreboard. Report lands in
`/tmp/hard_scene_seed0/` (override with `HARD_SCENE_OUT=`).

## Pose (MATCHED to craster's rung-3/rung-4 ChunkScene camera)
Both sides render the seed-0 3x3-chunk-around-origin scene from the SAME camera:
world eye ~(8.3, 95, 40), FOV 70, 854x480. craster uses yaw 0 / pitch -35deg
...
camera holds altitude (no gravity, and no hand/hotbar drawn - closer to craster),
...
- This is a COARSE whole-frame tolerance target, NOT the tight OSMesa noise floor
  ...
  gradient + clouds (craster fills a flat sky-blue), full smooth per-vertex lighting
  (craster folds MC's directional per-FACE shade, flatter), a runtime-stitched atlas
  (craster uses its own stitched 128x128 atlas), and the FULL view-distance terrain
  where craster only meshes the 3x3-chunk island around origin (sky-void beyond ~48
...
rendered by craster; the number is printed, not gated, per PRODUCT.md honesty rules).
