...
typedef struct {
    float x, y, z;              /* hanging BlockPos centre */
    int facing;                 /* EnumFacing 2..5 */
    int item, meta, rotation;   /* displayed stack; count is always one */
    float lm_light, lm_blk;
    const unsigned char *map_colors; /* exact 128x128 MapData.colors plane */
    int map_decoration_present;
    int map_decoration_type;
    int map_decoration_x, map_decoration_z, map_decoration_rotation;
} GmItemFrameRenderView;

/* RenderItemFrame's baked frame model and displayed FIXED item.  The frame
 * and block items bind the terrain atlas; generated items bind item_atlas. */
...
int gm_item_frame_flat_items_emit(const GmItemFrameRenderView *views, int n,
                                  CrVertex *out, int max);
/* Filled-map dynamic plane and its frame marker. The caller binds a 128x128
 * texture made by gm_item_frame_map_rgba for the plane, then entity atlas for
 * the optional map-icons quad. */
int gm_item_frame_map_plane_emit(const GmItemFrameRenderView *view,
                                 CrVertex *out, int max);
int gm_item_frame_map_icon_emit(const GmItemFrameRenderView *view,
                                CrVertex *out, int max);
...
/* Armor-stand held block models. Kept in the terrain-atlas pass so block-face
 * texels and tinting stay identical to world/drop block rendering. */
int gm_held_blocks_emit(const GmEntityView *ents, int n,
                        CrVertex *out, int max);

/* LayerMooshroomMushroom: three red-mushroom cross models attached to an
 * adult visible Mooshroom. Terrain atlas; 72 verts per entity. */
