...
int gm_mobs_det_place(GmMobLive *m, int eid, int type,
    ...
    int slot;
    EwStore *s;
    if (!m || !(gm_passive(type) || hai_ok(type))) return -1;
...
static void move_mob(GmWorld *w, const McSinTable *st, GmMobLive *m, EwStore *s,
    ...
    if ((gm_passive(s->type[i]) || (pai_det() && hai_ok(s->type[i]))) && moving) {
        if (pai_det()) {
...
void gm_mobs_tick(GmMobLive *m, GmWorld *w, const struct McSinTable *st_,
    ...
    for(int i=1;i<EW_MAX_ENTITIES;++i)
        ...
        if(moving&&type!=EW_TYPE_GHAST && !(pai_det() && (passive || hai_ok(type)))){
        ...
        if(hai_ok(type) && pai_det()) pai_body_update(m, nx, now, i);
