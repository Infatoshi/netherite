...
static void s_boss(GmPlayerView *pv, CrFramebuffer *fb) {
    (void)fb; (void)pv;
    gm_hud_set_boss(1, 0.5f, "Ender Dragon");
}

static void s_wither_boss(GmPlayerView *pv, CrFramebuffer *fb) {
    (void)fb; (void)pv;
    gm_hud_set_boss(1, 0.5f, "Wither");
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < n; ++i) {
        fill_gray(&fb);
        gm_hud_set_boss(0, 1.0f, "Ender Dragon");
