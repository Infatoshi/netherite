...
## 1. Tick 9811: oracle world has flowing water craster's pristine worldgen lacks (save-state provenance)
...
(0,-1)) from TWO flowing-water cells at (41, 64-65, 238) that craster does not
have - CRASTER_DUMP at t9811 shows its live pond == pristine worldgen (all
meta-0 sources, nothing at y>=64 there; the fluid CA is mark-driven and was
never poked). Craster's worldgen block extent is verified against the real
...
The diverged trajectory later kills the craster player by fall damage
...
(delete the save before the session - first-session worldgen == craster
worldgen), or (b) snapshot the oracle's loaded-region block/meta state at
recstart and seed craster's world from it. (a) is free and is now the VERIFY.md
rule; (b) is what full save-replay eventually needs.

Repro: `cd raster/verify/trace && uv run --no-project --with numpy --with pillow python replay_tape.py /tmp/play_tape.jsonl` -> FIRST DIVERGENCE tick 9811 field vx. `CRASTER_DUMP="9811,38,44,62,66,236,241" ./craster_game --headless ... --script out/tape_play_tape/craster_script.jsonl` dumps craster's live cells; compare against the live game's (41,64-65,238).
...
craster matches it exactly, so the old "oracle 0.098 vs craster 0.0196"
bridge-stepped measurement was purely the one-tick input-alignment artifact
documented in VERIFY.md. No craster change needed.
...
header's world_time) while craster's clock free-ran into dusk. craster now
...
- Missing grass_side_overlay (the "phase" clusters on grass walls):
  ...
  grass side face; craster drew only the untinted grass_side base (baked-in
...
- Latent: CUTOUT_MIPPED sampled craster's mip chain while both oracle
...
1. FIRST-PERSON ARM (A: all 5 remaining clusters; B: ~1.0/ch of the 2.54):
   craster's gm_hand_draw arm has visibly different silhouette/pose vs
   vanilla ItemRenderer.renderArmFirstPerson. Biggest single residual now.
2. Selection box: the oracle draws the targeted-block wireframe; craster's
   CRASTER_OVERLAY box does NOT match it (enabling it made pose B WORSE,
...
the settled pose (-102.8,73,83.0), craster went 20.2 -> 19.3/ch, i.e. the
...
2935/14007 cells (qrl `sample_light` vs CRASTER_DUMP_LIGHT over
(-112..-92, 60..88, 72..94)), java DARKER by up to 14 at y 60-72, and java
equals craster's pure column ladder in 2899 of them. Vanilla's end state is
...
gated isAreaLoaded(16), Chunk.recheckGaps gated 33x33); craster floods
everywhere (compute_skylight_spread). But this is PIXEL-NEUTRAL: with the
in-binary A/B gate verified live (CRASTER_NO_SKYSPREAD=1 changes 17k cells),
...
cluster decomposition = scipy.ndimage.label on |mc-craster|>16 per pose frame.
...
orchid, fossil gate), but craster's decorate dispatch only matched id 6, so 134
...
craster_game link line) - a stale binary reproduces the OLD divergence.
...
enters that tick with motionX/Z == 0 while craster (and vanilla formulas)
...
from zero (post vz 0.053508 = 0.098*0.546); craster displacement 0.261807 =
...
Cross-check: craster frames rendered at every tick around a moving capture
...
are horizon cells where the oracle shows sky (136,176,250) and craster draws
...
chunk (1,8); craster's replay worldgen grows a height-4 small oak whose
...
by craster's lazy player-path chunk gen. Same family as #1 (world provenance),
...
world snapshot, #1 fix (b)) into the tape and drive craster's worldgen with
it; until then each such site is one sidecar entry, built by diffing
CRASTER_DUMP against the session save region NBT.

Repro: remove the sidecar
raster/verify/tapes/20260712T055346Z_..._77b5b462.jsonl.worldpatch.jsonl and
replay -> FIRST DIVERGENCE t1798 x (craster clamps at 28.3 on phantom leaves);
restore it -> t2291.
...
none of it visible to the replay, so craster's slot 6 stayed empty while the
...
on dirt is what let the oracle out-dig craster's bare hand, clamping craster
...
before the use press, so craster's OWN rightClickMouse/place path runs) and
slot 6 = wooden shovel x1 at t2320 (after the oracle's table-GUI session, so
craster's own dig-speed/wear model runs). With both, plus the shovel
...
compare oracle keyframe `<tape>_frames/f_%06d.png` (tick i*20) against craster
frame i in craster_frames.npy. Frame indices below are that pairing.
...
predates the freeze or is server-side). Craster stays bright blue. Recorder
...
matches craster; t1140 both_sky after fogColor1 fix is 3/ch).
...
by default (opt out CRASTER_NO_OVERLAY / CRASTER_NO_CRACK). Selection is
...
Swim-path frames t840/860/880 showed oracle grass/dirt steps vs craster sand
(oracle-green & craster-sand tens of thousands of pixels). Session save has
...
the top band; craster has zero - solid foliage silhouettes. both_sky empty;
...
[163,196,255] vs craster [166,197,253] (|d|~2-3/ch) - full day blue. Rain
...
craster brighter R/G by ~13-15 (not geometry). Solid-only save-derived
...
is the ease during approach/intermittent contact, which craster's sprint
machine already reproduces (physics bit-exact on the probe tape). Verified
pixel-exact: planks-wall span identical oracle vs craster at every sampled
...
partialTicks between prev/current fovModifierHand while craster steps
...
- t435 vx: craster applied the 1.3x sprint air accel the tick sprint turned
...
- t451 y: with jump held, craster re-jumped the tick it landed; the oracle
...
- Hand camera fov: renderHand projects with getFOVModifier(pt,false) - the
  eye-in-water 60/70 squeeze applies (never the sprint ease). Craster
  hardcoded 70: underwater the oracle's arm sits mostly off-screen while
  craster's stayed centered (whole bottom-right quadrant diffing 17-25/ch).
...
x*y quarter combos), waterlily (y quarters). craster only randomized fire.
...
partial-tick pose; craster renders the exact tick pose. At walk speed the
...
an opaque block (suffocation view); craster draws the scene as if the
...
frames near-black, craster showed the cave - 7.5-8.4/ch over 96% of the
...
craster has already switched to the fully-lit overworld palette; by t584
...
ordering, not a craster scene bug; deprioritized. Fix if ever needed:
...
oracle's bow reads slightly larger/left of craster's (whole-frame 3.9/ch
...
X-cross at (427,258), craster fins at (427,190), t120). Recorder now
...
The oracle consumes an arrow per shot (63,62,...); craster's replay
...
- ita_tool_dig_speed had no axe branch (hand speed on logs; oracle chopped in
  15 ticks, craster never finished). Fixed: ita_is_axe + ItemAxe
...
before craster (x 49.798 vs 49.7196, |d|=0.078, heals at the next pin; end
...
shows a dark right-edge silhouette; craster's plate is flat-lit). Same
...
ended 22 blocks apart (craster stayed on the pillar, oracle inched to the lip
...
craster's own correctly-timed heal (e.g. re-anchor 1.83 at t630, own heal
...
near-black with distance; craster kept the overworld linear fog (start 96).
...
image pixels = model texels * 2. craster sampled the raw texel coords ->
...
craster has no particle system. Accept scattered speck diffs; revisit if a
...
(GuiIngame heart flash); craster draws the plain post-damage hearts. Small
fixed-position HUD diff during damage windows.

## 42. OPEN: entity-driven world edits do not replay (crystal explosion fire)

The tape carries no block-change records; craster re-simulates PLAYER dig and
...
fire keeps burning in craster after t276. Residual ~30px orange at the
...
craster rebuilds it from tape rows, seeding flat on first sight. On a tape
...
pitch = ring y-slope * 10 deg) while craster renders it level. Fix in data,
...
first-person model); craster draws the plain item. Visible as a persistent
...
exists in the tape. craster draws one bar from recorded state - arguably more
...
and the final smoke burst. craster renders none (no beam/orb/particle
...
deathTicks/200 (alpha-test pass + depth-EQUAL repaint). craster drops whole
...
ground oracle [50,42,37] vs craster [54,50,38], far ground [18,14,25] vs
...
11.7/ch: the oracle wing filled the screen while craster's dragon had swung
off-frame). Vanilla pushes (rotationYaw, posY) into the 64-entry
getMovementOffsets ring every onLivingUpdate; craster pushed only inside
...
did run - "Scanning for legacy world dragon fight" fired post-entry). craster
...
near-opaque deep blue (dense water fog) while craster renders clear water with
...
holes while craster shows a cleanly formed window in the same wall - the
...
haze (fog toward the horizon/lower view rows) while craster renders the same
