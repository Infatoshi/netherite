 ...
 * 12, full-column AABB frustum test, CRASTER_NO_CULL escape hatch, znear/zfar) is
 ...
 *
 * Camera convention is craster's (core/math.c): yaw about +Y, pitch about +X,
 ...
 * into this convention via craster_yaw = 180 - MC_yaw, craster_pitch = -MC_pitch
 ...
 */
#ifndef CRASTER_VERIFY_POSE_SCENE_H
#define CRASTER_VERIFY_POSE_SCENE_H
 ...
 * get evicted and rebuild without cascade. Escape: CRASTER_NO_PREP=1.
 * Spawn: CRASTER_SPAWN_CX/CZ, else seed0 qrl_0 (2,11). Optional exact list:
 * CRASTER_PREP_LIST=path ("bcx bcz" per line, recorded genprobe order). */
static inline void pscn__prepare_populate(long long seed, int ccx, int ccz,
                                          int ensure_r) {
    if (getenv("CRASTER_NO_PREP")) {
        fprintf(stderr, "posescene: CRASTER_NO_PREP set, skipping cumulative prepare\n");
        return;
    }

    int scx = 0, scz = 0;
    const char *esx = getenv("CRASTER_SPAWN_CX");
    const char *esz = getenv("CRASTER_SPAWN_CZ");
    if (esx) scx = atoi(esx);
    if (esz) scz = atoi(esz);
    ...
    const char *prepl = getenv("CRASTER_PREP_LIST");
    if (prepl) {
        ...
        if (pf) {
            ...
            fprintf(stderr, "posescene: cannot open CRASTER_PREP_LIST=%s\n", prepl);
        }
    }
...
static inline void posescene_init_seed(PoseScene *s, const CrCamera *cam, int W, int H,
    ...
    const int cull_off = getenv("CRASTER_NO_CULL") != NULL;
...
#endif /* CRASTER_VERIFY_POSE_SCENE_H */
