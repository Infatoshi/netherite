...
TWO modes, auto-selected by file extension:
  ...
  STATE mode (*.jsonl): the FULL per-tick state-vector diff. Reads java_state.jsonl (ground
    truth) vs c_state.jsonl (craster) and reports, for EACH feature/category:
      ...
      - UNSIMULATED on the C side (C emits null for the whole category -- a critical finding:
        the craster game does not simulate that feature at all).
...
def main():
    ...
    ap.add_argument("--c", required=True, help="craster trace (same kind as --java)")
    ...
    ap.add_argument("--tracer",
                    default="/home/infatoshi/dev/minecraft/mc-1.11.2-env/c/craster/trace_game")
