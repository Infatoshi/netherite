...
printf '%s\n' \
	'{"tick":0,"type":"spawn_potion_fixture","eid":701,"potion_item":438,"potion_type":24,"x":12.5,"y":100,"z":8.5,"vx":0.25,"vy":0.5,"vz":-0.125,"age":7,"player_thrower":1,"ignore_player":1,"ignore_player_time":2}' \
	>"$script"
...
assert potion["age"] == 8
assert potion["player_thrower"] == 1
assert potion["ignore_player"] == 1
assert potion["ignore_player_time"] == 1
...
printf '%s\n' \
	...
	'{"tick":0,"type":"spawn_area_effect_cloud_fixture","eid":703,"potion_type":16,"x":8.5,"y":5,"z":8.5,"age":9,"duration":600,"wait_time":10,"reapplication_delay":20,"radius":3,"radius_on_use":-0.5,"radius_per_tick":-0.005,"next_application":0,"entity_seed48":38780996791245}' \
	>"$script"
./magma_game --world superflat --headless --ticks 1 --mobs off \
	--script "$script" --state-out "$state" --render off --pace unlimited
uv run --no-project python - "$state" <<'PY'
...
assert cloud["entity_seed48"] == 191187885815570, cloud
particles = row["particle_events"]
assert len(particles) == 2, particles
assert all((p["kind"], p["count"], p["entity_eid"]) == (15, 0, -1)
