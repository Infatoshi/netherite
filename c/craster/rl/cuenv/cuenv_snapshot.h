/* cuenv_snapshot.h - the .bsnp state-snapshot format, shared between the
 * writer (game/rl_mode.c, "snapshot":"<path>" action key + --snapshot-in
 * restore) and the batched-env reader (rl/cuenv). The CODE is the canonical
...
#ifndef CUENV_SNAPSHOT_H
#define CUENV_SNAPSHOT_H

#ifdef __cplusplus
extern "C" {
#endif

#define CUENV_SNAP_MAX_ITEMS 16   /* == GM_LIVE_MAX (game/live_sim.h) */
#define CUENV_SNAP_MAX_CONT  64   /* container-list cap (ids 58/61/62); more
...
/* ---- host-side loader (rl/cuenv/cuenv_snapshot.c; NOT linked into the
...
typedef struct {
    ...
    int            ncont;          /* -1 = > CUENV_SNAP_MAX_CONT (fallback) */
} CuSnapshot;

/* Load a .bsnp into *out (mallocs cells/coal; cuenv_snapshot_free releases).
 * Returns 1 on success, else 0 with a message in err. */
int  cuenv_snapshot_load(const char *path, CuSnapshot *out,
                         char *err, int err_cap);
void cuenv_snapshot_free(CuSnapshot *s);
...
int  cuenv_build_ore_xy(const int *ore, int nore,
 ...
 * init-time only. */
int  cuenv_build_containers(const unsigned short *cells,
                            int rx0, int ry0, int rz0,
