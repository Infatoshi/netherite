...
#include "inventory_stack_rules.h" /* complete vanilla stack limits */
#include "mc_blocks.h"
...
typedef struct {
    ...
    long long tick, seed;
    /* Standalone magma's deterministic World.rand and Math.random cursors.
     * The compact BSNP v1 contract does not persist them, so both rl_mode and
     * Blaze reconstruct these exact initial cursors from the world seed. */
    u64 world_random_seed48, math_random_seed48;
    int dead, deaths;
    int player_air;              /* Entity air supply, 300..-19 */
...
MC_HD static inline void blaze_player_tick(Blaze *env, const McSinTable *st,
    ...
    a.attack   = act.attack;
    {
        ICStack feet = isr_get_stack(&pl->inv, ISR_ARMOR_FEET);
        double ai_speed = 0.10000000149011612
            * pl->movement_speed_multiplier;
        if (pl->sprinting)
            ai_speed *= 1.0 + 0.30000001192092896;
        a.depth_strider = ic_enchantment_level(&feet, 8);
        a.water_ai_speed = (float)ai_speed;
    }
    ...
    if (act.attack) {
        ...
        if (r >= 0) {
            ...
            if (bid != BLK_AIR && bp.hardness >= 0.0f) {
                ...
                {
                    ICStack head = isr_get_stack(&pl->inv, ISR_ARMOR_HEAD);
                    pin.aqua_affinity = ic_enchantment_level(&head, 6) > 0;
                }
    ...
    if (env->rc_delay > 0) --env->rc_delay;
    {
        ...
        if (use_fire) {
            ...
            int done_use = 0;
            if (isr_right_click_equip(&pl->inv, pl->inv.current_item)) {
                done_use = 1;
            } else if (held0.item == 325) {
                int bx, by, bz;
            ...
            if (!done_use) {
                ...
                if (r >= 0) {
                    ...
                    ICStack slab_held = isr_get_stack(
                        &pl->inv, pl->inv.current_item);
                    int slab_double = slab_held.item == 44 ? 43
                        : slab_held.item == 126 ? 125
                        : slab_held.item == 182 ? 181
                        : slab_held.item == 205 ? 204 : 0;
                    int slab_face = cu_face_from_adj(
                        hx, hy, hz, ax, ay, az);
                    int slab_target = slab_double
                        && hit_id == slab_held.item
                        && (hit_meta & 7) == (slab_held.meta & 7)
                        && ((slab_face == IBP_UP && (hit_meta & 8) == 0)
                            || (slab_face == IBP_DOWN
                                && (hit_meta & 8) != 0));
                    int slab_adjacent = slab_double && r == 1
                        && psv_get_block(window, ax, ay, az)
                            == slab_held.item
                        && (psv_get_meta(window, ax, ay, az) & 7)
                            == (slab_held.meta & 7);
                    int snow_target = slab_held.item == 78 && hit_id == 78
                        && slab_face == IBP_UP && hit_meta < 7;
                    int snow_adjacent = slab_held.item == 78 && r == 1
                        && psv_get_block(window, ax, ay, az) == 78
                        && psv_get_meta(window, ax, ay, az) < 7;
                    int door_block = slab_held.item == 324 ? 64
                        : slab_held.item == 330 ? 71
                        : slab_held.item >= 427 && slab_held.item <= 431
                        ? 193 + slab_held.item - 427 : 0;
                    int crop_block = slab_held.item == 295 ? 59
                        : slab_held.item == 361 ? 104
                        : slab_held.item == 362 ? 105
                        : slab_held.item == 372 ? 115
                        : slab_held.item == 391 ? 141
                        : slab_held.item == 392 ? 142
                        : slab_held.item == 435 ? 207 : 0;
                    int crop_support = slab_held.item == 372 ? 88 : 60;
                    int hoe = slab_held.item >= 290 && slab_held.item <= 294;
                    int shovel = ita_is_shovel(slab_held.item);
                    if (snow_target || snow_adjacent) {
                        int sx = snow_target ? hx : ax;
                        int sy = snow_target ? hy : ay;
                        int sz = snow_target ? hz : az;
                        int sm = psv_get_meta(window, sx, sy, sz) + 1;
                        double top = (double)sy
                            + (double)(sm + 1) / 8.0;
                        const McAABB *pb = &pl->ent.box;
                        if (!(pb->minX < (double)sx + 1.0
                                && pb->maxX > (double)sx
                                && pb->minY < top
                                && pb->maxY > (double)sy
                                && pb->minZ < (double)sz + 1.0
                                && pb->maxZ > (double)sz)) {
                            cu_win_set_state(window, sx, sy, sz, 78, sm);
                            (void)isr_decr_stack_size(
                                &pl->inv, pl->inv.current_item, 1);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, sx, sy, sz, 78, sm,
                                0, 0, 0);
                            done_use = 1;
                        }
                    }
                    if (done_use) {
                        /* ItemSnow stacking consumed the action. */
                    } else if (slab_held.item == 331) {
                        if (slab_face == IBP_UP && psv_solid(hit_id)
                                && cu_replaceable(psv_get_block(
                                    window, ax, ay, az))) {
                            cu_win_set_state(window, ax, ay, az, 55, 0);
                            (void)isr_decr_stack_size(
                                &pl->inv, pl->inv.current_item, 1);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, ax, ay, az, 55, 0, 0, 0, 0);
                        }
                        done_use = 1;
                    } else if (hoe || shovel) {
                        int transform = 0;
                        if (slab_face != IBP_DOWN
                                && psv_get_block(window, hx, hy + 1, hz)
                                    == BLK_AIR) {
                            if (shovel && hit_id == 2)
                                transform = 208;
                            else if (hoe && (hit_id == 2 || hit_id == 208
                                    || (hit_id == 3
                                        && (hit_meta & 3) == 0)))
                                transform = 60;
                            else if (hoe && hit_id == 3
                                    && (hit_meta & 3) == 1)
                                transform = 3;
                        }
                        if (transform) {
                            ITAStack tool = ita_mk(
                                slab_held.item, slab_held.meta);
                            int max_damage;
                            cu_win_set_state(
                                window, hx, hy, hz, transform, 0);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, hx, hy, hz,
                                transform, 0, 0, 0, 0);
                            tool.damage++;
                            max_damage = ita_stack_max_damage(&tool);
                            if (max_damage > 0 && tool.damage > max_damage)
                                (void)isr_decr_stack_size(
                                    &pl->inv, pl->inv.current_item, 1);
                            else {
                                slab_held.meta = tool.damage;
                                isr_set_stack(&pl->inv,
                                    pl->inv.current_item, slab_held);
                            }
                        }
                        done_use = 1;
                    } else if (crop_block) {
                        if (slab_face == IBP_UP
                                && cu_replaceable(psv_get_block(
                                    window, ax, ay, az))
                                && psv_get_block(window, hx, hy, hz)
                                    == crop_support) {
                            cu_win_set_state(
                                window, ax, ay, az, crop_block, 0);
                            (void)isr_decr_stack_size(
                                &pl->inv, pl->inv.current_item, 1);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, ax, ay, az,
                        ...
                        if (slab_face == IBP_UP && ay < 255
                                && cu_replaceable(psv_get_block(
                                    window, ax, ay, az))
                                && cu_replaceable(psv_get_block(
                                    window, ax, ay + 1, az))
                                && psv_solid(psv_get_block(
                                    window, ax, ay - 1, az))) {
                            int yq = cu_yaw_to_quad(pl->yaw);
                            int facing = ibp_horizontal_facing(yq);
                            int fx = facing == IBP_EAST ? 1
                                : facing == IBP_WEST ? -1 : 0;
                            int fz = facing == IBP_SOUTH ? 1
                                : facing == IBP_NORTH ? -1 : 0;
                            int rx = -fz, rz = fx;
                            float lf = mc_cos(st,
                                -pl->yaw * 0.017453292f - 3.1415927f);
                            float lf1 = mc_sin(st,
                                -pl->yaw * 0.017453292f - 3.1415927f);
                            float lf2 = -mc_cos(st,
                                -pl->pitch * 0.017453292f);
                            double hit_x = pl->ent.posX
                                + (double)(lf1 * lf2) * hit_t - (double)hx;
                            double hit_z = pl->ent.posZ
                                + (double)(lf * lf2) * hit_t - (double)hz;
                            int right_hinge = (fx < 0 && hit_z < 0.5)
                                || (fx > 0 && hit_z > 0.5)
                                || (fz < 0 && hit_x > 0.5)
                                || (fz > 0 && hit_x < 0.5);
                            int left_solids = psv_solid(psv_get_block(
                                    window, ax - rx, ay, az - rz))
                                + psv_solid(psv_get_block(
                                    window, ax - rx, ay + 1, az - rz));
                            int right_solids = psv_solid(psv_get_block(
                                    window, ax + rx, ay, az + rz))
                                + psv_solid(psv_get_block(
                                    window, ax + rx, ay + 1, az + rz));
                            int left_door = psv_get_block(
                                    window, ax - rx, ay, az - rz) == door_block
                                || psv_get_block(window, ax - rx, ay + 1,
                                    az - rz) == door_block;
                            int right_door = psv_get_block(
                                    window, ax + rx, ay, az + rz) == door_block
                                || psv_get_block(window, ax + rx, ay + 1,
                                    az + rz) == door_block;
                            if ((!left_door || right_door)
                                    && right_solids <= left_solids) {
                            ...
                            cu_win_set_state(window, ax, ay, az,
                                door_block, (yq + 1) & 3);
                            cu_win_set_state(window, ax, ay + 1, az,
                                door_block, 8 | right_hinge);
                            (void)isr_decr_stack_size(
                                &pl->inv, pl->inv.current_item, 1);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, ax, ay, az, door_block,
                                (yq + 1) & 3, 0, 0, 0);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, ax, ay + 1, az, door_block,
                                8 | right_hinge, 0, 0, 0);
                        }
                        done_use = 1;
                    } else if (slab_target || slab_adjacent) {
                        int mx = slab_target ? hx : ax;
                        int my = slab_target ? hy : ay;
                        int mz = slab_target ? hz : az;
                        const McAABB *pb = &pl->ent.box;
                        if (!(pb->minX < (double)mx + 1.0
                                && pb->maxX > (double)mx
                                && pb->minY < (double)my + 1.0
                                && pb->maxY > (double)my
                                && pb->minZ < (double)mz + 1.0
                                && pb->maxZ > (double)mz)) {
                            cu_win_set_state(window, mx, my, mz,
                                slab_double, slab_held.meta & 7);
                            (void)isr_decr_stack_size(
                                &pl->inv, pl->inv.current_item, 1);
                            cu_emit_edit(edits, &ne, max_edits,
                                ox, oy, oz, mx, my, mz, slab_double,
                                slab_held.meta & 7, 0, 0, 0);
                        }
                        done_use = 1;
                        ...
                        if ((held.item == 326 || held.item == 327) &&
                            ...
                            cu_emit_edit(edits, &ne, max_edits, ox, oy, oz,
                                   ...
                                   cu_replaceable(psv_get_block(window, ax, ay,
                                                                az))) {
                            int place_id = ibp_item_placed_block(held.item);
                            ...
                            if (place_id > 0 && place_id < 256 &&
                                ...
                                int face = cu_face_from_adj(hx, hy, hz,
                                                            ax, ay, az);
                                int yq = cu_yaw_to_quad(pl->yaw);
                                float ray_dy = mc_sin(st,
                                    -pl->pitch * 0.017453292f);
                                double hit_local_y = pl->ent.posY
                                    + PSV_EYE_HEIGHT
                                    + (double)ray_dy * hit_t - (double)hy;
                                int hit_y_high = hit_local_y > 0.5;
                                int entity_dir = ibp_opposite(
                                    ibp_horizontal_facing(yq));
                                if (fabs(pl->ent.posX
                                            - ((double)ax + 0.5)) < 2.0
                                        && fabs(pl->ent.posZ
                                            - ((double)az + 0.5)) < 2.0) {
                                    double eye_y = pl->ent.posY
                                        + PSV_EYE_HEIGHT;
                                    if (eye_y - (double)ay > 2.0)
                                        entity_dir = IBP_UP;
                                    else if ((double)ay - eye_y > 0.0)
                                        entity_dir = IBP_DOWN;
                                }
                                int pmeta = ibp_placed_meta_exact(
                                    place_id, face, yq, hit_y_high,
                                    entity_dir, held.meta) & 15;
...
/* Block.dropBlockAsItemWithChance -> Block.spawnAsEntity -> EntityItem ctor.
 * This is the exact standalone-magma constructor prefix relevant to Blaze:
 * World.rand owns the chance/offset floats and Math.random owns the four
 * hover/yaw/motion doubles.  Render-only hover/yaw and entity UUID/RNG draws
 * are intentionally absent from CuItem, but their independent cursors cannot
 * affect later World.rand/Math.random values. */
MC_HD static inline int cu_spawn_drop_at_block(Blaze *env, int wx, int wy,
                                               int wz, int item, int meta) {
    JavaRandom world_random, math_random;
    double x, y, z, mx, mz;
    int slot;
    jrand_set_seed48(&world_random, env->world_random_seed48);
    jrand_set_seed48(&math_random, env->math_random_seed48);
    (void)jrand_float(&world_random); /* dropBlockAsItemWithChance(1) */
    x = (double)wx + (double)(jrand_float(&world_random) * 0.5f) + 0.25;
    y = (double)wy + (double)(jrand_float(&world_random) * 0.5f) + 0.25;
    z = (double)wz + (double)(jrand_float(&world_random) * 0.5f) + 0.25;
    (void)jrand_double(&math_random); /* hoverStart */
    (void)jrand_double(&math_random); /* yaw */
    mx = (double)(float)(jrand_double(&math_random)
        * 0.20000000298023224 - 0.10000000149011612);
    mz = (double)(float)(jrand_double(&math_random)
        * 0.20000000298023224 - 0.10000000149011612);
    env->world_random_seed48 = world_random.seed;
    env->math_random_seed48 = math_random.seed;
    if (!cu_spawn_item(env, x, y, z, item, 1, meta, 10)) return 0;
    for (slot = 0; slot < CU_MAX_ITEMS; ++slot) {
        CuItem *e = &env->items[slot];
        if (!e->active || e->age != 0 || e->item != item || e->x != x ||
            e->y != y || e->z != z) continue;
        e->mx = mx;
...
MC_HD static inline double cu_item_cube_axis_offset(
        const McAABB *entity, int x, int y, int z,
        int axis, double offset) {
    double min_x = (double)x, min_y = (double)y, min_z = (double)z;
    double max_x = min_x + 1.0, max_y = min_y + 1.0, max_z = min_z + 1.0;
    if (axis == 0) {
        if (entity->maxY <= min_y || entity->minY >= max_y ||
            entity->maxZ <= min_z || entity->minZ >= max_z) return offset;
        if (offset > 0.0 && entity->maxX <= min_x) {
            double gap = min_x - entity->maxX;
            if (gap < offset) offset = gap;
        } else if (offset < 0.0 && entity->minX >= max_x) {
            double gap = max_x - entity->minX;
            if (gap > offset) offset = gap;
        }
    } else if (axis == 1) {
        if (entity->maxX <= min_x || entity->minX >= max_x ||
            entity->maxZ <= min_z || entity->minZ >= max_z) return offset;
        if (offset > 0.0 && entity->maxY <= min_y) {
            double gap = min_y - entity->maxY;
            if (gap < offset) offset = gap;
        } else if (offset < 0.0 && entity->minY >= max_y) {
            double gap = max_y - entity->minY;
            if (gap > offset) offset = gap;
        }
    } else {
        if (entity->maxX <= min_x || entity->minX >= max_x ||
            entity->maxY <= min_y || entity->minY >= max_y) return offset;
        if (offset > 0.0 && entity->maxZ <= min_z) {
            double gap = min_z - entity->maxZ;
            if (gap < offset) offset = gap;
        } else if (offset < 0.0 && entity->minZ >= max_z) {
            double gap = max_z - entity->minZ;
            if (gap > offset) offset = gap;
        }
    }
    return offset;
...
/* runtime_resolve_item_self_motion's static full-cube slice. The production
 * runtime additionally supplies partial and moving-piston shapes; the Blaze
 * curriculum snapshots contain ordinary terrain on the item flight paths.
 * Resolve Y, X, Z in Java's Entity.move order. */
MC_HD static inline void cu_item_resolve_self_motion(
        Blaze *env, CuItem *e, double bx, double by, double bz) {
    double req[3] = {e->x - bx, e->y - by, e->z - bz};
    double act[3] = {req[0], req[1], req[2]};
    double amin_x = fmin(bx - 0.125, e->x - 0.125);
    double amin_y = fmin(by, e->y);
    double amin_z = fmin(bz - 0.125, e->z - 0.125);
    double amax_x = fmax(bx + 0.125, e->x + 0.125);
    double amax_y = fmax(by + 0.25, e->y + 0.25);
    double amax_z = fmax(bz + 0.125, e->z + 0.125);
    int x0 = (int)floor(amin_x), y0 = (int)floor(amin_y);
    int z0 = (int)floor(amin_z);
    int x1 = (int)floor(amax_x - 1.0e-9);
    int y1 = (int)floor(amax_y - 1.0e-9);
    int z1 = (int)floor(amax_z - 1.0e-9);
    McAABB box = mc_aabb_make(
        bx - 0.125, by, bz - 0.125, bx + 0.125, by + 0.25, bz + 0.125);
    int x, y, z;
    for (y = y0; y <= y1; ++y)
        for (z = z0; z <= z1; ++z)
            for (x = x0; x <= x1; ++x)
                if (cu_item_solid_at(env, x, y, z))
                    act[1] = cu_item_cube_axis_offset(
                        &box, x, y, z, 1, act[1]);
    box = mc_aabb_offset(&box, 0.0, act[1], 0.0);
    for (y = y0; y <= y1; ++y)
        for (z = z0; z <= z1; ++z)
            for (x = x0; x <= x1; ++x)
                if (cu_item_solid_at(env, x, y, z))
                    act[0] = cu_item_cube_axis_offset(
                        &box, x, y, z, 0, act[0]);
    box = mc_aabb_offset(&box, act[0], 0.0, 0.0);
    for (y = y0; y <= y1; ++y)
        for (z = z0; z <= z1; ++z)
            for (x = x0; x <= x1; ++x)
                if (cu_item_solid_at(env, x, y, z))
                    act[2] = cu_item_cube_axis_offset(
                        &box, x, y, z, 2, act[2]);
    e->x = bx + act[0]; e->y = by + act[1]; e->z = bz + act[2];
    if (act[0] != req[0]) e->mx = 0.0;
    if (act[1] != req[1]) {
        e->my = 0.0;
        if (req[1] < 0.0) e->on_ground = 1;
    }
    if (e->on_ground && e->my == 0.0) e->my = -0.0;
    if (act[2] != req[2]) e->mz = 0.0;
}

MC_HD static inline int cu_item_combine(Blaze *env, int ai, int bi) {
    CuItem *a, *b;
    int limit;
    if (ai == bi || ai < 0 || bi < 0 || ai >= CU_MAX_ITEMS ||
        bi >= CU_MAX_ITEMS) return 0;
    a = &env->items[ai]; b = &env->items[bi];
    if (!a->active || !b->active || a->item <= 0 || b->item != a->item ||
        a->meta != b->meta) return 0;
    if (b->count < a->count) return cu_item_combine(env, bi, ai);
    limit = isr_max_stack_size(b->item, b->meta);
    if (a->count <= 0 || b->count <= 0 || b->count > limit - a->count)
        return 0;
    b->count += a->count;
    if (a->pickup_delay > b->pickup_delay) b->pickup_delay = a->pickup_delay;
    if (a->age < b->age) b->age = a->age;
    a->active = 0; a->count = 0;
    if (env->n_items > 0) --env->n_items;
    return 1;
}

MC_HD static inline void cu_item_search_nearby(Blaze *env, int index) {
    CuItem *e = &env->items[index];
    int j;
    if (!e->active) return;
    for (j = 0; j < CU_MAX_ITEMS; ++j) {
        CuItem *other = &env->items[j];
        if (!other->active || fabs(other->x - e->x) >= 0.75 ||
            other->y >= e->y + 0.25 || other->y + 0.25 <= e->y ||
            fabs(other->z - e->z) >= 0.75) continue;
        (void)cu_item_combine(env, index, j);
        if (!e->active) return;
    }
}
...
MC_HD static inline void cu_live_tick_player(Blaze *env) {
    ...
    for (i = 0; i < CU_MAX_ITEMS; ++i) {
        ...
        before_x = e->x; before_y = e->y; before_z = e->z;
        ...
        if ((int)e->x != (int)before_x || (int)e->y != (int)before_y ||
            (int)e->z != (int)before_z || (e->age + 1) % 25 == 0) {
            cu_item_search_nearby(env, i);
            if (!e->active) continue;
        }
        slip = 0.6f;
        ...
        if (e->active)
            cu_item_resolve_self_motion(
                env, e, before_x, before_y, before_z);
    }
    ...
    for (i = 0; i < CU_MAX_ITEMS; ++i) {
         ...
         * by (1.0, 0.5, 1.0), then EntityItem contributes its 0.25 cube.
         * Keep the strict inequalities and asymmetric Y bounds identical to
         * gm_live_tick_player/World#getEntitiesWithinAABBExcludingEntity. */
        if (fabs(e->x - px) >= 1.425 || fabs(e->z - pz) >= 1.425 ||
            e->y >= py + 2.3 || e->y + 0.25 <= py - 0.5) continue;
...
MC_HD static inline void cu_break_unsupported_plants(Blaze *env, int wx, int wy, int wz) {
    ...
    for (y = wy + 1; y < 255; ++y) {
        ...
        if (drop > 0)
            (void)cu_spawn_drop_at_block(env, wx, y, wz, drop, 0);
    }
}

/* BlockLog/BlockLeaves.breakBlock CHECK_DECAY propagation. The standalone
 * runtime applies this after replacing the center and before random ticks. */
MC_HD static inline void cu_mark_leaf_decay_after_break(
        Blaze *env, int wx, int wy, int wz, int old_id) {
    int radius, ox, oy, oz;
    if (old_id != 17 && old_id != 162 && old_id != 18 && old_id != 161)
        return;
    radius = (old_id == 17 || old_id == 162) ? 4 : 1;
    for (ox = -radius; ox <= radius; ++ox)
        for (oy = -radius; oy <= radius; ++oy)
            for (oz = -radius; oz <= radius; ++oz) {
                int x = wx + ox, y = wy + oy, z = wz + oz;
                int id = cu_world_block(env, x, y, z);
                int meta;
                if (id != 18 && id != 161) continue;
                meta = cu_world_meta(env, x, y, z);
                if (!(meta & 8))
                    cu_world_set_state(env, x, y, z, id, meta | 8);
            }
}
...
/* verbatim furnace_live_tick (FftFurnace kernel round-trip). */
...
MC_HD static inline void blaze_runtime_tick_nr(Blaze *env, const McSinTable *st,
    ...
    /* EntityLivingBase.onEntityUpdate runs the eye-height water test before
     * player travel. With no Respiration or Water Breathing in the compact
     * curriculum inventory, air decrements once per submerged tick; -20
     * resets to zero and deals two points of drowning damage. */
    {
        double eye_y = env->pl.ent.posY + PSV_EYE_HEIGHT;
        int wx = mc_floor(env->pl.ent.posX + (double)env->ox);
        int wy = mc_floor(eye_y);
        int wz = mc_floor(env->pl.ent.posZ + (double)env->oz);
        int id = cu_world_block(env, wx, wy, wz);
        if (id != 8 && id != 9) {
            env->player_air = 300;
        } else if (--env->player_air == -20) {
            env->player_air = 0;
            pv_attack(&env->vit, 2.0F);
    ...
    for (i = 0; i < n; ++i) {
        int old_id = cu_world_block(
            env, edits[i].wx, edits[i].wy, edits[i].wz);
        int harvest_xp = 0;
        /* Forge BreakEvent asks the ore for XP before getDrops.  Both the XP
         * quantity and the later orb constructor advance cursors shared by
         * subsequent item drops, even though the compact training state does
         * not retain XP orbs themselves. */
        if (edits[i].id == 0 && edits[i].drop_id > 0 &&
            pb_can_harvest(edits[i].harvest_tool, old_id)) {
            JavaRandom random;
            jrand_set_seed48(&random, env->world_random_seed48);
            if (old_id == 16)
                harvest_xp = jrand_int_bound(&random, 3);
            else if (old_id == 56 || old_id == 129)
                harvest_xp = 3 + jrand_int_bound(&random, 5);
            else if (old_id == 21 || old_id == 153)
                harvest_xp = 2 + jrand_int_bound(&random, 4);
            env->world_random_seed48 = random.seed;
        }
        cu_world_set_state(env, edits[i].wx, edits[i].wy, edits[i].wz,
                           edits[i].id, edits[i].meta);
        if (old_id != edits[i].id)
            cu_mark_leaf_decay_after_break(
                env, edits[i].wx, edits[i].wy, edits[i].wz, old_id);
        ...
        if (edits[i].drop_id > 0)
            for (int drop = 0; drop < edits[i].drop_count; ++drop)
                (void)cu_spawn_drop_at_block(
                    env, edits[i].wx, edits[i].wy, edits[i].wz,
                    edits[i].drop_id, edits[i].drop_meta);
        if (harvest_xp > 0) {
            JavaRandom math_random;
            int remaining = harvest_xp;
            jrand_set_seed48(&math_random, env->math_random_seed48);
            while (remaining > 0) {
                ...
                remaining -= value;
                (void)jrand_double(&math_random); /* yaw */
                (void)jrand_double(&math_random); /* motion X */
                (void)jrand_double(&math_random); /* motion Y */
                (void)jrand_double(&math_random); /* motion Z */
            }
            env->math_random_seed48 = math_random.seed;
        }
    }
    ...
    if (env->vit.health <= 0.0f) {
        /* PlayerMP closes its container and drops/clears the complete player
         * inventory before the terminal observation is emitted. Blaze does
         * not advance post-terminal item entities, but the terminal inventory
         * itself remains part of the byte-exact RL observation contract. */
        int selected = env->pl.inv.current_item;
        isr_init(&env->pl.inv);
        env->pl.inv.current_item = selected;
        env->dead = 1;
        env->deaths++;
    }
...
MC_HD static inline void blaze_reset_scalar(Blaze *env, const RlSnapHead *h,
    ...
    env->seed = h->seed;
    env->tick = h->tick;
    env->world_random_seed48 =
        ((u64)h->seed ^ MC_JR_MULT) & MC_JR_MASK;
    env->math_random_seed48 =
        ((u64)h->seed ^ UINT64_C(0x9E3779B97F4A) ^ MC_JR_MULT)
            & MC_JR_MASK;
    env->dead = 0;
    env->deaths = 0;
    env->player_air = 300;
