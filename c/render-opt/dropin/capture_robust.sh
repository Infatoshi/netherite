#!/usr/bin/env bash
set -u
MODE="$1"; ROOT=/home/infatoshi/dev/minecraft/mc-1.11.2-env; DROP="$ROOT/c/render-opt/dropin"
...
for i in $(seq 1 240); do grep -q "\[qrl\] listening" runclient.log 2>/dev/null && { echo "qrl up ${i}s"; break; }; sleep 1; done
