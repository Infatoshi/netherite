...
## Same-input command fork (2026-08-22)

The bounded command/time/weather matrix now forks an unexecuted Java pre-tick
capsule and sends the trigger independently to Java and native. Java's local
output clock is a recorded external input. This replaced the weaker
post-trigger continuation comparison and exposed two real defects: vanilla's
empty `{"text":""}` output needed canonical empty-state normalization, and
native time commands needed to synchronize the private weather-clock kernel or
their effect vanished on the following tick. All eleven direct cases pass 30
state families plus raw blocks and light. The aggregate also exposed a Rabbit
test that compared uninitialized struct padding; it now compares every semantic
field exactly.
...
## 2026-08-08 (player attack particles)

- Extended the locked attack fixture from 60 to 67 real-Java cases. It now
  captures critical, enchantment-critical, sweep, and damage-indicator events
  at their source hooks and matches native identity, order, count, target
  geometry, and every raw spawn argument bit. A lethal one-health target pins
  damage-indicator count to clamped pre/post health loss and proves the packet
  is suppressed. Rejected and ordinary strong attacks prove the empty-stream
  cases.
- Added fixed-capacity combat dispatch and rendering. Critical and magic
  particles use attached three-update emitters, damage packets expand through
  the deterministic visual RNG, and sweep renders its byte-exact texture with
  the vanilla layer-3 fullbright quad. The 1,024-particle and 16-emitter caps
  add no idle scan or allocation. Unsaved Java constructor entropy remains an
  explicit per-particle/pixel residual.
- Focused Java/native, runtime-order, lifecycle, asset, render, and product
  gates pass. The aggregate is 6:45.89 at 450,356 KiB peak RSS with no major
  faults or swap. CPU throughput is 4,004 steps/s against the 3,858.9 floor
  and 1.4% below the 4,062 reference baseline. All 13 locally available
  quick-sweep stages pass; two missing-snapshot Blaze stages skip. Pinning the
  last unpinned
  villager/bootstrap Gradle calls to `run/gradle` also reduced the parity
  bundle from a failed remote-download retry to a 40-second pass. GPU 1 was
  untouched.

## 2026-08-08 (player target hurt/death audio)

- Expanded the locked attack fixture from 67 to 79 real-Java cases. Seeded
  pig, cow, sheep, and chicken targets now pin hurt/death sound identity,
  coordinates, NEUTRAL category, exact private-RNG pitch bits, cow's 0.4
  volume, sprint and sweep-neighbor ordering, plus rejected and accepted
  hurt-resistance-delta silence.
- Fresh represented passive hits append status and sound at the existing
  bounded mob-event seam. The runtime drains that attack-local audio between
  player knockback and the sweep/critical/strong/weak follow-up, matching
  Java without allocation or idle work. The focused native gate passes 15
  hurt/death/rejected/order cases; adjacent player-effect, combat-particle,
  product, Java, and OpenAL gates pass.
- The full native aggregate passes in 5:34.62 at 450,204 KiB peak RSS with
  zero major faults and zero swap. Stopped-oracle CPU throughput is 4,420
  steps/s against the 3,858.9 floor and above the 4,062 baseline. The quick
  sweep passes all 13 available stages and skips only the two absent-snapshot
  Blaze stages. GPU 1 was untouched. Hostile and other target sound families,
  Thorns, and player-target velocity acknowledgement remain open.

## 2026-08-08 (all represented living target audio)

- Extended the locked player-attack fixture from 79 to 115 real-Java cases.
  Thirty-six new rows pin hurt/death audio for zombies, pigmen, skeletons,
  wither skeletons, creepers, spiders, cave spiders, endermen, blazes, ghasts,
  silverfish, villagers, and slime/magma sizes 1, 2, and 4. Identity, source
  position, HOSTILE/NEUTRAL category, pitch bits, ghast volume 10.0,
  slime/magma `0.4 * size` volume and small/big selection all match.
- The oracle exposed pigman's two `becomeAngryAt` draws before ordinary
  damage. Native primary and sweep hits now consume anger level and angry
  sound-delay RNG before hurt processing, preserving the exact later pitch
  cursor. The represented spider max-health constant is also corrected from
  20 to Java's 16.
- The fixed mob/runtime stream now resolves all represented living target
  families. The focused native gate passes 50 hurt/death/rejected/order cases,
  and adjacent player-effect, combat-particle, Java, product, and OpenAL gates
  pass. The owned manifest grows from 146 events/469 variants to 176
  events/557 variants without allocation or idle-path work.
- The full native aggregate passes in 5:31.17 at 450,612 KiB peak RSS with
  zero major faults and zero swap. Stopped-oracle CPU throughput is 4,635
  steps/s against the 3,858.9 floor and above the 4,062 baseline. The quick
  sweep passes all 13 available stages and skips only the two absent-snapshot
  Blaze stages. GPU 1 was untouched. Thorns, player-target velocity
  acknowledgement, statistics, and criteria remain open.

## 2026-08-08 (returning splash potion player collision)

- Added the missing player collision candidate to the shared throwable tick.
  Player-thrown potions now suppress the nearby thrower on their first update,
  retain Java's two-tick ignore countdown while separated, and can directly
  collide after release. The closest player/mob segment hit wins before block
  impact handling, and direct splash-player impact uses factor 1.0.
- Added a locked real-Java/native five-transition oracle. It matches ignore
  identity, countdown 2, 1, 0, release at -1, returning impact removal,
  Strong Harming health bits, `hurtTime=10`, and `hurtResistantTime=20`.
  The focused native brewing/runtime suite passes 389 checks. Broader
  multi-entity throwable ignore ordering remains explicit open work.
- Java and product builds pass. The full native aggregate passes in 5:21.18 at
  450,764 KiB peak RSS with zero major faults and zero swap. Stopped-oracle CPU
  throughput is 4,750 steps/s against the 3,858.9 floor and above the 4,062
  baseline. The active-projectile path remains fixed-size and allocation-free;
  all 13 available quick-sweep stages pass and only the two absent-snapshot
  Blaze stages skip. GPU 1 was untouched.

## 2026-08-08 (automatic potion/cloud state capsules)

- Extended Java authoritative capture with conservative exact predicates for
  default thrown splash/lingering potions and default area-effect clouds.
  Potion item/type, age, motion, player thrower, ignored player, and the private
  ignore countdown are captured. Cloud potion/lifecycle/radius values, the
  bounded player reapplication deadline, owner, entity ID, and loaded-list
  rank are captured; custom effects, colors, particles, gravity, in-ground
  projectiles, and unrepresented owners remain outside this promotion.
- The version-2 neutral capsule validates and emits both subsets. Native
  restores the hidden thrower-ignore state and explicit cloud owner, preserves
  old fixture defaults, and exposes the fields in raw state. Negative controls
  reject an ignored potion without its player thrower and a cloud below the
  vanilla minimum radius.
- A locked real-Java gate captures both entities at a parked pre-tick boundary,
  runs the production capsule serializer/emitter, and matches the next native
  tick at double/float bit precision. The capsule self-test, 389-check brewing
  suite, product build, and JDK 8 build pass.
- The full native aggregate passes in 5:40.59 at 450,756 KiB peak RSS with no
  major faults or swap. Stopped-oracle CPU throughput is 4,903 steps/s against
  the 3,858.9 floor. All 13 available quick-sweep stages pass; only the two
  absent-snapshot Blaze stages skip. The state additions are fixed-capacity and
  add no idle scan or allocation. GPU 1 was untouched.

## 2026-08-08 (lingering-cloud particle stream)

- Added a parked-server comparator for the real
  `EntityAreaEffectCloud.onUpdate` client branch. The default active cloud
  matches all 29 `SPELL_MOB` spawn calls bit for bit; the waiting branch
  matches its Boolean gate, two calls, per-particle white/color choice, and
  final 48-bit entity RNG cursor.
- Added a fixed 1,024-event runtime seam and deterministic `ParticleSpell`
  consumer with the vanilla texture strip, movement, tint, light, lifetime,
  collision, and translucent layer-0 formulas. Java's class-static Random,
  per-particle Random, and Math.random constructor streams are unsaved client
  entropy, so individual particle pixels remain an explicit residual.
- Repaired stale standalone harness link closures exposed by the uninterrupted
  aggregate. The world-mesh lock now compares exact opaque/cutout triangle
  multisets and byte-identical translucent order with both visibility filters
  disabled; the mob harness uses the current blaze-shot and sheep-graze APIs
  without preserving a false exact panic-speed claim.
- Focused Java/native and brewing gates pass. The full native aggregate passes
  in 7:17.20 at 1,100,916 KiB peak RSS. Stopped-oracle CPU throughput is 4,873
  steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass; the two snapshot-backed Blaze stages skip
  because their `.bsnp` inputs are absent. GPU 1 was untouched.

## 2026-08-08 (brewing-stand hopper sidedness)

- Promoted the existing live brewing-stand automation path against the real
  1.11.2 `TileEntityHopper` and `TileEntityBrewingStand`. Six discriminating
  fixtures cover top-only ingredient insertion, side potion and fuel insertion,
  bottom potion extraction, ordinary ingredient rejection, and the ingredient
  slot's glass-bottle extraction exception.
- The side-fuel case executes the full same-tick hopper-then-stand boundary:
  blaze powder enters slot 4, is consumed, and produces fuel 20 while the
  source hopper receives cooldown 8. All six Java/native state rows match.
- The expanded automation oracle, native checks, and Java build pass. The
  implementation was already fixed-capacity and work-proportional, so this
  promotion adds no product-path code, allocation, or idle tick cost.

## 2026-08-08 (multi-slot dropper/dispenser selection)

- Replaced the one-occupied-slot restriction with the exact
  `TileEntityDispenser.getDispenseSlot` reservoir sampler. Droppers and
  dispensers share one runtime-local 48-bit cursor, including Java's otherwise
  surprising `nextInt(1)` draw for the first occupied slot.
- Two seven-callback fixtures use three noncontiguous occupied slots. Every
  decremented slot and the final cursor match the real Java static RNG for both
  dropper insertion and default dispenser ejection. Raw state exposes the
  cursor, and scripts can inject it for deterministic comparison.
- Java's cursor is process-global and absent from world NBT. Standalone native
  environments seed it deterministically and independently; automatic capsule
  transport remains open. Selection scans only the fixed nine-slot inventory,
  with no idle-world work or allocation.
- The focused automation and 389-check brewing gates pass. The uninterrupted
  native aggregate passes in 6:57.96 at 1,100,896 KiB peak RSS. Stopped-oracle
  CPU throughput is 4,126 steps/s against the 3,858.9 floor and 4,062 baseline.
  All 13 locally available quick-sweep stages pass; the two snapshot-backed
  Blaze stages skip because their `.bsnp` inputs are absent. GPU 1 was
  untouched.

## 2026-08-08 (empty-bucket dispenser behavior)

- Added Forge 1.11.2's `DispenseFluidContainer` empty-bucket path. A singleton
  bucket now removes an exact water or lava source and becomes the matching
  filled bucket. A non-fluid target delegates to default item ejection.
- Five real-Java/native rows match source and inventory mutation, emitted item,
  world RNG/Gaussian cursor, and world events. Stacked empty buckets put the
  filled result in the first empty dispenser slot; when all nine slots are
  occupied, the filled bucket is ejected with the nested and outer `1000/2000`
  event pairs in exact order. The standalone Java oracle now
  recreates Forge's normal pre-init fluid capability wiring before invoking the
  production dispenser registry.
- This is a fixed firing-time branch with no allocation or idle-world scan.

## 2026-08-08 (furnace and double-chest hopper boundaries)

- Promoted the existing sided-furnace automation path with five real-Java/native
  rows. Top hoppers insert iron ore only into input, side hoppers insert coal
  only into fuel, and bottom hoppers extract output before the fuel-slot
  water-bucket exception while rejecting ordinary fuel.
- Five more rows lock ordinary double-chest ordering and access. In a
  west/east pair, insertion fills the west half before continuing east, and a
  hopper beneath the east half still extracts canonical west slot zero first.
  Solid blocks above either the addressed or adjacent half do not obstruct the
  Forge hopper lookup, distinguishing automation from player/comparator access.
- All inventory fields and successful cooldowns match the production 1.11.2
  tile entities. The focused automation gate passes. The product path was
  already fixed-capacity and work-proportional, so this promotion adds no idle
  scan or allocation.

## 2026-08-08 (trapped-chest and shulker hopper boundaries)

- Added five production Java/native rows for two represented container
  families. Trapped double chests use the same canonical west-first insertion
  and extraction as ordinary pairs.
- White shulker boxes accept ordinary side insertion and bottom extraction,
  while a shulker-box item is rejected without changing either inventory or
  starting hopper cooldown.
- Every inventory count and successful cooldown 8 matches. The existing
  fixed-size container paths require no new allocation or idle scan.
- The uninterrupted native aggregate passes in 7:16.75 at 1,101,120 KiB peak
  RSS with zero major faults and zero swap. CPU throughput is 4,844 steps/s
  against the 3,858.9 floor and 4,062 baseline. All 13 locally available quick
  stages pass; only the two absent-snapshot Blaze stages skip. GPU 1 was
  untouched.

## 2026-08-08 (broad default dispenser ejection)

- Generalized the exact stone ejection path to ordinary untagged vanilla
  items while preserving item metadata and tool damage. Six production
  Java/native rows cover cobblestone, planks metadata 5, charcoal through a
  solid-facing target, a damaged iron pickaxe, non-white dye, and a milk
  bucket.
- Source count, emitted item/metadata, target block, final world RNG/Gaussian
  state, and the `1000/2000` event pair match exactly in every row.
- Audited all 1.11.2 registrations in `Bootstrap`, `ItemArmor`, `ItemMinecart`,
  `ItemShield`, and `ItemElytra`. Native guards prevent unimplemented armor,
  equipment, minecart, and shulker-placement behaviors from silently falling
  through as ordinary ejection. This firing-time path scans only nine slots
  and adds no allocation or idle work.

## 2026-08-08 (minecart and shulker dispenser behaviors)

- Added the registered `ItemMinecart` behavior for rideable, chest, furnace,
  TNT, hopper, and command carts. Nine production Java/native rows match every
  kind on flat rails, ascending rails, the air-over-lower-rail branch, and the
  blocked-target nested default ejection, including exact position, motion,
  inventory, RNG/Gaussian state, and ordered world events.
- The ascending lower-rail row exposed and fixed an existing cart miss:
  chest/hopper carts use Java's inventory-fullness-dependent drag. An empty
  container cart applies the exact float-derived `0.995` factor rather than an
  ordinary cart's `0.96`. The independent rail oracle remains green.
- Added the registered shulker-box behavior for all 16 colors. Eighteen
  Java/native rows lock dispenser-facing placement over air, upward placement
  over support, blocked-target item preservation, all block metadata, the
  empty 27-slot tile, and exact `1000/1001` plus `2000` events.
- Both behaviors run only for a scheduled dispenser callback. Carts and tiles
  reuse fixed/bounded live stores; no idle scan was added.
- The uninterrupted native aggregate passes in 7:17.41 at 1,100,908 KiB peak
  RSS with zero major faults and zero swap. The stopped-oracle CPU guard passes
  at 4,913 steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass; the two absent-snapshot Blaze stages
  skip. GPU 1 was untouched.

## 2026-08-08 (all boat dispenser variants)

- Expanded the oak-only water fixture to all six registered boat items. Eight
  production Java/native rows match wood-type ordinal, exact float-derived
  spawn position/yaw, source mutation, and events for direct water,
  air-over-water, and blocked-target nested default ejection.
- Boat type is retained explicitly in the bounded live mob state and exposed
  in entity views. The broader mob/boat battery passes. Variant texture-atlas
  promotion and automatic capsule transport remain separate visual/state work.
- The added field is 96 bytes per runtime and the branch executes only on a
  scheduled dispenser callback; no per-tick scan or allocation was added.

## 2026-08-08 (equipment dispenser fallback and player targets)

- Added the no-living-target branch of `ItemArmor.DISPENSER_BEHAVIOR` for all
  20 armor IDs, shield, and elytra. Twenty-two production Java/native rows
  match source mutation, emitted item/damage, final world RNG/Gaussian state,
  and the single `1000/2000` event pair.
- Added all 22 represented-player target branches. Production Java/native rows
  match the selected head/chest/legs/feet/offhand slot, item damage, source,
  RNG/Gaussian state, and events. They also preserve a 1.11.2 edge in which
  successful equipment delegates the now-empty source to default ejection;
  the empty `EntityItem` is created and retires on its first update.
- Six occupied-slot rows match default ejection followed by same-tick player
  pickup into the first inventory slot. Represented mob and mixed-target
  equipment remains fail-closed until mob equipment/order is modeled. The
  target scan is bounded and firing-only; no idle-world work or allocation was
  added.
- The uninterrupted aggregate gameplay gate passes in 7:11.57 at 1,101,168
  KiB maximum RSS, zero major faults, and no swap. The isolated CPU guard passes
  at 4,933 steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass; the two absent-snapshot Blaze stages skip.
  GPU 1 was untouched.

## 2026-08-08 (skull and pumpkin dispenser headwear)

- Added the non-summoning branches of both registered optional behaviors.
  Thirteen production Java/native rows cover all six skull metadata values and
  pumpkins against an empty player head slot, an occupied head slot, and no
  living target, including singleton and stacked equip. Source item/metadata,
  equipped headwear, unchanged RNG state, and the exact optional behavior
  events match.
- The rows lock a 1.11.2 edge: a successful singleton head equip returns an
  empty stack, so the optional behavior emits its failure sound even though
  equipment changed; a stacked equip leaves the source nonempty and emits the
  normal success sound. Possible wither/snow-golem/iron-golem structures and
  represented mob targets fail closed until summon/equipment state exists.
- Pattern detection is a bounded firing-time neighborhood scan. No idle work,
  allocation, or rendering path changed.
- The uninterrupted aggregate gameplay gate passes in 7:09.29 at 1,100,292
  KiB maximum RSS, zero major faults, and no swap. The isolated CPU guard passes
  at 4,139 steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass; the two absent-snapshot Blaze stages skip.
  GPU 1 was untouched.

## 2026-08-08 (player chest lid obstruction)

- Added exact player-use obstruction for ordinary and trapped single/double
  chests. Twelve production Java/native rows cover air, stone, lower and upper
  stone slabs, plus a solid block above either double-chest half. A cover blocks
  the chest only when its lower face is side-solid, matching Forge 1.11.2.
- The check runs only on player use. Existing exact covered-double-chest hopper
  rows continue to prove automation ignores both halves' lid obstruction.
  Sitting-ocelot obstruction remains open with the unrepresented ocelot.
- The check is a bounded five-cell query with no allocation or idle-tick work.
- The first aggregate run correctly exposed a capacity-test fixture that placed
  chests into generated terrain without clearing lid headroom. The fixture now
  declares that precondition; its table-growth and retained-content checks pass.
- The corrected uninterrupted aggregate gameplay gate passes in 7:20.59 at
  1,100,924 KiB maximum RSS, zero major faults, and no swap. The isolated CPU
  guard passes at 4,632 steps/s against the 3,858.9 floor and 4,062 baseline.
  All 13 locally available quick-sweep stages pass; the two absent-snapshot
  Blaze stages skip. GPU 1 was untouched.

## 2026-08-08 (bounded plant bonemeal dispenser behavior)

- Added the registered white-dye behavior for wheat, carrots, potatoes,
  beetroot, pumpkin/melon stems, cocoa, tall grass, and ferns. Crops and stems
  consume one dye, use Java's exact `2 + nextInt(4)` draw, and clamp at maximum
  age; beetroot integer-divides that increase by three. Cocoa preserves its
  facing and advances one stage without RNG. Grass and fern become exact lower
  and default-north upper double-plant states; dead bushes fail unchanged.
- Sixty-seven locked production Java/native rows cover seed-zero and high-draw
  RNG branches, initial/near-mature/mature ages, three valid cocoa facings,
  two-block plant state, both target halves of sunflower/lilac/rose/peony,
  exact source, final world/Math/EID cursors, and ordered event position/data.
  Clone rows also lock exact item metadata, position, motion, yaw, hover phase,
  age, pickup delay, and same-boundary update. The seed-zero beetroot row locks
  Java's successful consume and particles despite zero integer growth. The
  same cases run as assertion-only coverage in the broad native automation
  battery.
- All six stage-zero sapling types consume one dye and exactly one
  `World.rand.nextFloat()`. Values below 0.45 promote only the stage bit; values
  at or above 0.45 preserve stage zero while still reporting success, consuming
  dye, and emitting particles. The 12 added rows lock both branches and exact
  post-call cursors. Stage-one tree generation remains fail-closed.
- Brown and red mushrooms now consume the exact 0.4 probability draw and, on
  acceptance, run the fixed-color big-mushroom generator with exact ordinary
  or doubled height, clearance, soil validation, obstruction rollback, and all
  cap/stem metadata. Eight live rows compare every state in a 735-cell raw
  cuboid; independent broad-native FNV constants are measured from those Java
  cuboids rather than from the C implementation.
- Flat/plains grass bonemeal now runs the exact 128-attempt `BlockGrass.grow`
  random walk, including depth-dependent offsets, early support/normal-cube
  rejection, tall-grass placement, and Forge's weighted plains flower table.
  Three live rows compare the complete 450-state platform and plant layer,
  biome, final RNG cursor, and events. Non-plains weighted flower tables remain
  outside the promoted flat-world boundary.
- The clone rows exposed two pre-existing item seams. Live item physics had
  mistaken the double plant's full selection box for an entity collision box,
  and the shared block-drop helper discarded the constructor hover phase after
  consuming it. Double plants are now non-colliding for item motion and exact
  hover state is retained for rendering. The Java fixture captures the real
  grow payload server-locally, then constructs the real untracked `EntityItem`
  at pinned cursors so integrated-client mirror work cannot contaminate the
  measured boundary.
- Fixed the isolated-oracle launcher's existing config wiring: each generated
  config is copied into the selected run directory, and unique qrl port and
  username values are passed through the documented Gradle properties. A
  clean instance now creates its world and binds its assigned port in about
  nine seconds.
- The behavior is a fixed firing-time branch with no idle-world scan. Clone
  item construction uses the bounded live store only when the dispenser fires.
  The expanded live real-Java/native comparator passes all 67 rows. The
  uninterrupted native aggregate passes in 7:43.25 at 1,101,168 KiB peak RSS,
  zero major faults, and zero swap. CPU throughput passes at 4,391
  steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass in 49.85 seconds; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Stage-one sapling
  tree generation and non-plains grass-bonemeal flower weighting remain open.

## 2026-08-08 (stage-one sapling dispenser generation)

- Promoted stage-one dispenser bonemeal through the real `BlockSapling`
  dispatch: the unconditional `nextInt(10)` oak-candidate draw, ordinary and
  fresh-generator big oak, taiga spruce, birch, variable-height vine-free
  jungle, acacia, required 2x2 dark oak, and exact single-dark-oak return. The
  existing verified terrain generators remain the geometry source; sapling big
  oak retains the constructor-default four-layer leaf distance rather than the
  decoration singleton's five-layer setting.
- The live bridge snapshots a bounded 32x256x32 canonical-state window into
  lazily allocated fixed scratch, runs the exact Java 48-bit stream, and applies
  only changed raw states back through a bulk edit path. Successful generation
  performs one relight/version finish. Failure restores the exact initiating
  stage/type state to one or all four saplings. There is no allocation or scan
  on idle ticks; first use retains about 1.6 MiB of fixed process-local scratch.
- Added a scoped host write observer to the shared population primitive for
  notification-only effects. Java installs the replacement state before a
  removed leaf's `breakBlock` marks the currently present 3x3x3 neighboring
  leaves for decay. Recording that actual write order produces the exact mixed
  legacy metadata for ordinary crowns, big-oak limbs, and dark-oak hanging
  branches without inferring it from final geometry. Decoration and device
  paths leave the observer unset.
- Eleven new locked rows compare every state in a 34,596-cell raw volume:
  all six single stage-one types at seed zero, a seed selecting big oak,
  2x2 spruce/jungle/dark-oak layouts, and a blocked oak. The adjacent dispenser
  is inside the huge spruce/jungle base-clearance radius, so those two correctly
  fail and restore all four saplings while dark oak grows. Source state, ordered
  events, and final RNG cursor match in every row. The complete plant gate now
  passes 78 production Java/native rows.
- Java-derived FNV constants for all eleven volumes are assertion-only coverage
  in the broad native automation binary. That focused binary passes in 9.44
  seconds at 172,760 KiB peak RSS with zero major faults. The Java build and the
  final exact-source native aggregate pass in 7:41.49 at 1,101,144 KiB peak
  RSS, zero major faults, and zero swap. CPU throughput passes at 4,556
  steps/s against the 3,858.9 floor and 4,062 baseline. The shared worldgen
  wrapper census remains exact for seeds 0, 7, 9, and 19, and all 13 locally
  available quick-sweep stages pass; only the two absent-snapshot Blaze stages
  skip. The CUDA population targets compile without running GPU work. The
  wrapper rerun also repaired its stale `CpScratch` caller and linked the
  already-required village object into the offline world dump. GPU 1 was
  untouched.

## 2026-08-08 (all-biome grass bonemeal oracle promotion)

- Closed the remaining grass-bonemeal coverage gap with a temporary uniform
  biome fixture on both real Java chunks and native loaded columns. The fixture
  restores every Java biome byte and is exposed in C only as a test hook; it
  adds no product-tick branch or scan.
- Eleven new rows exercise default forest, plains mutation, swamp and its
  mutation, and flower forest across multiple 48-bit seeds. The full live gate
  now passes 89 rows with exact source state, events, final RNG cursor, selected
  biome, and every state in each 450-cell grass result volume.
- Fourteen independent Java-derived grass-volume hashes now run in the broad
  native automation battery. It passes at 172,156 KiB peak RSS with zero major
  faults and no swap. The final native aggregate passes in 8:00.76 at
  1,100,524 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,474 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass; only the two absent-snapshot Blaze
  stages skip. GPU 1 was untouched.

## 2026-08-08 (player-applied sapling bonemeal)

- Refactored the exact IGrowable body behind the dispenser path into one shared
  runtime operation and added the playable main-hand white-dye front end. It
  raycasts at normal reach, consumes one item outside creative mode, emits the
  exact 2005 event, and performs the client swing only after success. The
  direct operation stays at Java's `ItemDye.onItemUse` server-call boundary.
- Added 23 locked real-Java/native player rows: seed-zero and high-draw
  stage-zero outcomes for every sapling type, plus the existing eleven
  stage-one generator cases. Unlike an adjacent dispenser, the unobstructed
  player fixtures prove successful 2x2 spruce and jungle generation as well as
  dark oak; big oak, single-dark-oak failure, and blocked-oak rollback also
  retain exact raw volumes and final world-RNG cursors.
- The direct cases are promoted into the broad native automation battery, and
  a full runtime action test proves the actual main-hand ray/use route. The
  live plant comparator passes all 112 rows in 16.41 seconds at 30,252 KiB
  peak RSS. The broad battery passes in 17.88 seconds at 171,752 KiB peak RSS,
  zero major faults, and no swap. The full native aggregate passes in 7:47.51
  at 1,101,000 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,446 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass; only the two absent-snapshot
  Blaze stages skip. Growth remains firing-time work with no additional idle
  scan or allocation. Offhand routing and natural random-tick sapling growth
  remain open.

## 2026-08-08 (natural sapling random ticks)

- Routed canonical supported saplings through an exact natural-random-tick
  front end: Java's light-at-above threshold and `nextInt(7)` acceptance feed
  the same stage-one promotion and ordinary/big/2x2 generator operation used by
  player/dispenser bonemeal. No idle scan or allocation was added.
- Added 24 real-Java/native natural rows: both stage-zero RNG outcomes for all
  six species, a low-light rejection, and all eleven stage-one generator
  cases. The combined plant gate passes 136/136 in 17.31 seconds at 30,252 KiB
  peak RSS with exact source state, events, raw volume, world hash, and RNG
  cursor.
- Added an isolated real `WorldServer` rank-zero selector fixture. Java and C
  select the same cell, advance `updateLCG` and the world RNG identically, and
  promote oak `6:0` to `6:8`; the strict matrix case passes in 18.2 seconds.
  General loaded-world section/chunk iteration and invalid-support sapling
  drops remain open.
- Re-synchronized the standalone trace runner's mirrored game-object and audio
  link lists after that gate exposed stale omissions of particle, audio, and
  window-composition objects.
- The broad automation battery passes in 16.92 seconds at 172,728 KiB peak RSS
  with zero major faults. The full native aggregate passes in 7:45.53 at
  1,101,160 KiB peak RSS. CPU throughput passes at 4,553 steps/s against the
  3,858.9 floor and 4,062 baseline. All 13 locally available quick-sweep stages
  pass in 54.66 seconds; only the two absent-snapshot Blaze stages skip. GPU 1
  was untouched.

## 2026-08-08 (offhand player bonemeal)

- Extended the exact player `ItemDye.onItemUse` boundary to the offhand and
  added the playable empty-main hand-order route. Main-hand white dye still
  wins; an empty main hand falls through to offhand white dye, consumes from
  slot 40 outside creative mode, emits event 2005, and swings only on success.
- Added 23 live real-Java/native offhand rows mirroring every sapling stage-zero
  RNG outcome and stage-one ordinary/big/2x2 generator case. The combined gate
  passes 159/159 in 24.03 seconds at 30,252 KiB peak RSS with exact stack,
  event, raw-volume, hash, and world-RNG results.
- The offline Java-derived automation battery now executes both hands at the
  direct callback and both actual ray-use paths. It passes in 16.75 seconds at
  172,184 KiB peak RSS with zero major faults. General populated-main-hand PASS
  arbitration before an offhand attempt remains open.
- The full native aggregate passes in 7:42.46 at 1,100,540 KiB peak RSS with
  zero major faults. CPU throughput passes at 4,624 steps/s against the 3,858.9
  floor and 4,062 baseline. All 13 locally available quick-sweep stages pass in
  54.97 seconds; only the two absent-snapshot Blaze stages skip. GPU 1 was
  untouched.

## 2026-08-08 (crop-family natural random ticks)

- Promoted controlled natural callbacks for wheat, carrots, potatoes,
  beetroot, pumpkin/melon stems, and cocoa on canonical valid support. The
  shared crop path preserves Java float truncation, hydrated-farmland and crop
  layout weighting, light/age exits, and exact `nextInt` bounds.
- Beetroot retains its independent `nextInt(3)` throttle before delegating to
  ordinary crop growth. Cocoa retains facing/support metadata and its
  `nextInt(5)` age roll. Mature stems preserve accepted-growth ordering,
  existing-adjacent-fruit short circuit, `N,E,S,W` selection, target air/soil
  checks, selected-target obstruction, and pumpkin/melon default metadata.
- Added 63 real-Java/native rows, including all four stem directions and the
  two-draw beetroot growth branch. The combined plant gate passes 222/222 in
  21.62 seconds at 30,252 KiB peak RSS with exact raw state and world-RNG
  cursors. General loaded-world selector ordering and invalid-support drop/item
  edges remain open.
- The broad automation regression passes in 17.56 seconds at 172,204 KiB peak
  RSS with zero major faults. The full native aggregate passes in 7:46.58 at
  1,101,100 KiB peak RSS. CPU throughput passes at 4,657 steps/s against the
  3,858.9 floor and 4,062 baseline. All 13 locally available quick-sweep stages
  pass in 54.96 seconds; only the two absent-snapshot Blaze stages skip. GPU 1
  was untouched.

## 2026-08-08 (nether wart, cactus, and sugar-cane natural ticks)

- Promoted controlled natural callbacks for nether wart, cactus, and sugar
  cane on canonical valid support. Immature wart consumes the exact
  `nextInt(10)` draw and advances only on zero; mature wart exits without RNG.
  Cactus and cane age deterministically, extend and reset at age 15, preserve
  default metadata on the new block, stop under a ceiling, and enforce the
  three-block height cap. Java's cactus callback does not itself revalidate an
  already-loaded side-invalid state, so native retains that source ordering
  instead of inventing an early support check.
- Added 17 locked real-Java/native rows spanning ages 0, 14, and 15, one-,
  two-, and three-block columns, blocked ceilings, wart grow/reject/mature
  seeds, exact block outcomes, and final RNG cursors. The combined plant gate
  passes 239/239 in 21.74 seconds at 30,252 KiB peak RSS with zero major
  faults. Ordinary loaded-chunk section membership/iteration and
  invalid-support reed/wart drop-item edges remain open.
- The broad automation battery passes in 17.18 seconds at 172,740 KiB peak RSS
  with zero major faults. The uninterrupted native aggregate passes in
  7:36.70 at 1,101,160 KiB peak RSS, three major faults, and zero swap. CPU
  throughput passes at 4,816 steps/s against the 3,858.9 floor and 4,062
  baseline. All 13 locally available quick-sweep stages pass in 52.76 seconds;
  only the two absent-snapshot Blaze stages skip. GPU 1 was untouched.

## 2026-08-08 (natural mushroom spreading)

- Promoted controlled random-tick spreading for brown and red mushrooms on a
  canonical mycelium platform. The callback retains Java's outer
  `nextInt(25)`, the fixed 9x3x9 same-color density census and five-mushroom
  cap, four rounds of candidate acceptance/rebasing, final support check, and
  exact 21-draw accepted cursor. The represented support predicate also admits
  podzol and low-light full cubes.
- Added eight locked real-Java/native rows. For both colors, seed zero executes
  the complete walk and places exactly three blocks east, two independent
  seeds reject after the outer draw, and a five-mushroom fixture exits after
  the accepted outer draw without placing a sixth. All 507 platform/result
  states and final RNG cursors match. The combined plant gate passes 247/247 in
  25.77 seconds at 30,252 KiB peak RSS with zero major faults.
- Broad automation passes in 17.17 seconds at 172,752 KiB peak RSS with zero
  ...
  1,100,904 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,784 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass in 54.23 seconds; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Ordinary loaded-world
  random-tick membership/iteration remains open.

## 2026-08-08 (controlled grass decay and spread)

- Promoted the complete controlled `BlockGrass.updateTick` body. A dark opaque
  cap decays grass to default dirt without RNG. Bright grass executes four
  fixed attempts with Java's ordered `nextInt(3), nextInt(5), nextInt(3)`
  triples, requires canonical dirt, candidate light at least four, and
  candidate-above opacity at most two before placing default grass.
- Added four locked real-Java/native rows over a 450-state dirt platform and
  plant layer. Seeds zero and one select two different exact spread cells, the
  high seed consumes all twelve draws without placement, and the stone-cap row
  locks no-RNG decay. The combined plant gate passes 251/251 in 28.37 seconds
  at 30,252 KiB peak RSS with zero major faults.
- Broad automation passes in 17.24 seconds at 171,848 KiB peak RSS with zero
  major faults. The uninterrupted native aggregate passes in 7:10.07 at
  1,101,180 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,776 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass in 54.51 seconds; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Ordinary loaded-world
  section/chunk random-tick ordering remains open.

## 2026-08-08 (controlled mycelium decay and spread)

- Promoted the complete controlled `BlockMycelium.updateTick` body through the
  shared exact surface-spread operation. A dark opaque cap converts mycelium
  to default dirt without RNG. Bright mycelium executes four ordered
  `nextInt(3), nextInt(5), nextInt(3)` triples and converts only canonical
  dirt under the exact candidate light/opacity gates.
- Added four locked real-Java/native rows over a 450-state dirt platform and
  air layer. Seeds zero and one select two different exact mycelium cells, the
  high seed consumes twelve draws without placement, and a stone cap locks
  no-RNG decay. The combined plant gate passes 255/255 in 30.79 seconds at
  30,252 KiB peak RSS with zero major faults.
- Broad automation passes in 16.70 seconds at 172,748 KiB peak RSS with zero
  major faults. The uninterrupted native aggregate passes in 7:06.80 at
  1,100,148 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,984 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass in 52.15 seconds; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Ordinary loaded-world
  section/chunk random-tick ordering remains open.

## 2026-08-08 (controlled farmland moisture and hydration)

- Promoted the no-entity `BlockFarmland.updateTick` callback. It scans the
  exact 9x2x9 water volume, hydrates from water or the verified open-sky rain
  predicate, decrements dry moisture, preserves moisture-zero farmland under
  a supported vanilla plant, and otherwise converts it to default dirt. Every
  path leaves the world RNG cursor unchanged.
- Added seven locked real-Java/native rows over the complete 162-state search
  and result volume. They cover moisture 7-to-6 and 1-to-0, terminal dirt,
  wheat-covered preservation, already-wet and newly hydrated static-water
  paths, and rain hydration. The combined plant gate passes 262/262 in 32.17
  seconds at 30,252 KiB peak RSS with zero major faults.
- Broad automation passes in 17.45 seconds at 172,196 KiB peak RSS with zero
  major faults. The uninterrupted native aggregate passes in 7:46.86 at
  1,101,160 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,307 steps/s against the 3,858.9 floor and 4,062 baseline while
  unrelated training jobs use the host. All 13 locally available quick-sweep
  stages pass in 56.86 seconds; only the two absent-snapshot Blaze stages skip.
  GPU 1 was untouched. Ordinary loaded-world random-tick selection and entity
  lifting during terminal dirt conversion remain open.

## 2026-08-08 (controlled snow-layer light melting)

- Promoted the complete controlled `BlockSnow.updateTick` body for all eight
  metadata states. Block light at most 11 retains the layer unchanged; block
  light above 11 removes it. Neither path consumes world RNG.
- Added 16 locked real-Java/native rows for every layer metadata in dark and
  adjacent-glowstone fixtures. The comparator now locks the measured
  target block-light value as well as the exact block result and cursor. The
  combined gate passes 278/278 in 32.74 seconds at 30,252 KiB peak RSS with
  zero major faults.
- Broad automation passes in 17.10 seconds at 170,972 KiB peak RSS with zero
  major faults. The uninterrupted native aggregate passes in 7:05.58 at
  1,100,952 KiB peak RSS, zero major faults, and zero swap. CPU throughput
  passes at 4,515 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass in 53.56 seconds; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Ordinary loaded-world
  random-tick selection remains open.

## 2026-08-08 (controlled Overworld ice melting)

- Promoted the controlled `BlockIce.updateTick` body on the verified
  flat-stone water basin. Ice opacity three makes the exact melt threshold
  block light greater than eight. A retained block consumes no RNG; a melt
  replaces static ice with flowing water 8:0 and appends the priority-zero
  water callback due five ticks later, also without RNG.
- Added dark and lit locked real-Java/native rows. They compare measured block
  light, exact target and support state, events, every RNG cursor, and the
  normalized scheduled tuple including relative position, block ID, delay,
  priority, and queue rank. The combined plant gate passes 280/280 in 32.53
  seconds at 30,252 KiB peak RSS with zero major faults.
- The new queue assertion exposed setup-owned sand callbacks left by prior
  cactus cases. Every parked plant fixture now begins with an empty local
  scheduled queue and removes only its own queued work during teardown, so
  later rows cannot inherit fixture contamination.
- Broad automation passes in 33.85 seconds at 172,056 KiB peak RSS with zero
  major faults while unrelated CPU experiments are active. The uninterrupted
  native aggregate passes in 7:27.23 at 1,100,880 KiB peak RSS, zero major
  faults, and zero swap. CPU throughput passes at 4,472 steps/s against the
  3,858.9 floor and 4,062 baseline. All 13 locally available quick-sweep
  stages pass in 54.45 seconds; only the two absent-snapshot Blaze stages skip.
  GPU 1 was untouched. Nether vaporization and general loaded-world
  random-tick selection remain open.

## 2026-08-08 (controlled full snow-block melting)

- Promoted the complete controlled `BlockSnowBlock.updateTick` body. A full
  snow block retains through stored block light 11. At 12 it becomes air and
  emits four separate snowball stacks. Each stack consumes its independent
  chance and three offset draws, then its EntityItem constructor consumes four
  Math.random doubles and one entity ID.
- Added a test-only saved block-light nibble hook that mirrors Java
  `World.setLightFor(EnumSkyBlock.BLOCK, ...)` without running a relight flood.
  The two locked rows inject 11 and 12 only after the fixture is complete and
  compare the measured value, exact block result, four item positions,
  velocities, stack/lifecycle fields, yaw and hover float bits, world and Math
  RNG cursors, causal entity IDs, and events. The combined plant gate passes
  282/282 in 32.07 seconds at 30,252 KiB peak RSS with zero major faults.
- Broad automation passes in 34.43 seconds at 172,740 KiB peak RSS, zero major
  faults, and zero swaps. The uninterrupted native aggregate passes in 9:07.07
  at 1,101,172 KiB peak RSS, zero major faults, and zero swaps while unrelated
  CPU experiments are active. CPU throughput passes at 4,019 steps/s against
  the 3,858.9 floor and 4,062 baseline. All 13 locally available quick-sweep
  stages pass in 54.20 seconds at 715,400 KiB peak RSS; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Ordinary loaded-world
  random-tick selection remains open.

## 2026-08-08 (controlled frosted-ice aging and collapse)

- Promoted the controlled `BlockFrostedIce.updateTick` body for ages zero
  through three. The callback consumes `nextInt(3)`, compares the target's
  combined light against `8-age` when fewer than four faces are frosted, and
  schedules its priority-zero successor at `20+nextInt(21)` whether it retains
  or advances. Source inspection corrected an early harness assumption:
  frosted ice does not use neighbor brightness, so the measured value comes
  from the target cell rather than its six adjacent cells.
- Age three now follows inherited `BlockIce.turnIntoWater` semantics exactly.
  Static water first notifies WEST/EAST/DOWN/UP/NORTH/SOUTH using the removed
  frosted block as the source. Sparse frosted neighbors can recursively
  collapse during those notifications. The target then becomes flowing water,
  queues its +5 liquid update, and the original callback separately visits
  DOWN/UP/NORTH/SOUTH/WEST/EAST to age or melt surviving frosted neighbors.
- Added eight age/light rows, a sparse five-neighbor star, and a supported
  dense ring. The combined real-Java/native gate passes 292/292 in 33.23
  seconds at 30,252 KiB peak RSS with zero major faults and zero swap. It locks
  combined light, all 11 local states, every local scheduled tuple and queue
  rank, events, and final RNG cursors. The Java oracle jar must be rebuilt, not
  merely `compileJava`-updated, because the launcher consumes
  `build/libs/MalmoMod-0.37.0.jar`.
- Broad automation passes in 39.79 seconds at 172,032 KiB peak RSS with one
  major fault and zero swap. The uninterrupted native aggregate passes in
  8:11.20 at 1,100,992 KiB peak RSS, zero major faults, and zero swap. GPU 1
  was untouched. CPU throughput passes at 4,722 steps/s against the 3,858.9
  floor and 4,062 baseline. All 13 locally available quick-sweep stages pass
  in 59.37 seconds at 716,824 KiB peak RSS; only the two absent-snapshot Blaze
  stages skip. Nether vaporization and general loaded-world random-tick
  selection remain open.

## 2026-08-08 (controlled chorus-flower growth and death)

- Promoted the controlled `BlockChorusFlower.updateTick` body for represented
  air, end-stone, chorus-plant, and stone-obstruction neighborhoods. It retains
  Forge's default `onCropsGrowPre` path and the easily missed `nextInt(1)` RNG
  advance, scans up to four stem cells, applies the rooted depth gate, and
  reproduces vertical continuation, randomized lateral branching, parent
  conversion to chorus plant, terminal death, and ages zero through five.
- Added 40 locked rows: all ages on direct end stone, three seeds for every
  obstructed growable age, four seeds at ages zero/three/four on a rooted
  three-deep stem, and the unconditional two-deep branch. Each compares a
  5x6x5 volume, World RNG cursor, and ordered 1033/1034 events. The final live
  Java/native gate passes 332/332 in 34.42 seconds at 30,252 KiB peak RSS with
  zero major faults and zero swap.
- The post-pass branch audit caught a vacuous first version of the rooted
  fixture: it had placed flowers rather than plants under the target, so both
  sides destroyed the unsupported flower. The corrected bottom-up native
  setup and Java flags-two setup use plant 199. Seed 2 now proves the intended
  depth rejection, exact south-then-north branches, and age-four death.
- Permanent native assertions cover vertical growth, two ordered lateral
  branches, depth rejection, death, and dead-state inertness. The focused
  aggregate passed before the final scheduler audit. Support-loss entries now
  dispatch the full callback, so restoring support before the due +1 boundary
  produces exact growth rather than the old no-op shortcut. The final
  scheduler-integrated runtime passes in 4:47.74 at 257,036 KiB peak RSS with
  zero major faults and zero swap. CPU throughput passes at 4,663 steps/s
  against the 3,858.9 floor and 4,062 baseline. All 13 locally available
  quick-sweep stages pass in 55.48 seconds at 716,788 KiB peak RSS; only the two
  absent-snapshot Blaze stages skip. GPU 1 was untouched. Forge event
  overrides, general loaded-chunk selection, and broader mixed-material growth
  neighborhoods remain open.

## 2026-08-08 (controlled static-lava fire ignition)

- Promoted `BlockStaticLiquid.updateTick` for static source lava inside a
  strict bounded proof region. The live block-state census now captures
  `Material.getCanBurn` and `Material.blocksMovement` for all 4,096 legacy
  ID/metadata inputs and emits provenance-locked native masks. The positive
  outer branch follows the cumulative upward coordinate walk and returns after
  ignition or its first movement-blocking material. The zero branch performs
  all three independent floor probes. Every created fire runs the already
  exact `BlockFire.onBlockAdded` path and retains its successor schedule draw;
  `doFireTick=false` exits before RNG.
- Added eight original and 12 material-expansion locked real-Java/native rows.
  Seeds 1 and 2 distinguish outer
  values two and one under an overhead plank layer, seeds 0 and 4 create three
  differently ordered floor fires each, the stone layer proves exact abort
  cursors, and two disabled rows prove both outer classes remain untouched. Six
  representative fuels cover wood, leaves, cloth, vines, carpet, and TNT.
  Supported vine and carpet fixtures prove the callback walks through their
  non-moving materials before igniting above them. The comparator locks the
  7x5x7 volume and every local fire schedule. The combined callback gate passes
  352/352 in 36.26 seconds at 30,252 KiB peak RSS with zero major faults and
  zero swap.
- Permanent native assertions pin the disabled cursor, upward ignition,
  three-fire floor outcome, solid abort, all six burning material categories,
  and non-moving vine/carpet continuation. The full native runtime battery
  passes in 5:57.02 at 469,864 KiB peak RSS with zero major faults and zero
  swap. CPU throughput passes at 4,666 steps/s against the 3,858.9 floor and
  4,062 baseline. All 13 locally available quick-sweep stages pass in 54.40
  seconds at 716,808 KiB
  peak RSS; only the two absent-snapshot Blaze stages skip. GPU 1 was
  untouched. Unknown non-burning neighborhood states, unloaded-world
  boundaries, and general loaded-chunk selection remain open.

## 2026-08-08 (controlled vine random spreading)

- Promoted the complete `BlockVine.updateTick` body for canonical block states.
  It retains the one-in-four gate, the x/z/y-ordered 9x3x9 density scan and
  five-vine cutoff, `EnumFacing.values()` direction draw, NORTH/EAST/SOUTH/WEST
  face-filter order, both rotated lateral attachment paths, diagonal and
  ceiling-supported fallbacks, opaque full-cube face acquisition, downward
  filtering, and lower-vine face union. The generated registry header now also
  emits the already captured `Material.isOpaque` mask used by the solid-target
  branch.
- Added 17 locked real-Java/native rows. They cover the no-roll cursor, all six
  direction classes, upward metadata 12, two downward constructions, east/west
  diagonals, three solid-face acquisitions, both rotated direct attachments,
  metadata-zero ceiling fallback, lower-vine merge to metadata 15, horizontal
  and upward density suppression, and the density-independent downward branch.
  The comparator locks all 243 local states and the final World RNG cursor. The
  combined callback gate passes 369/369 in 36.30 seconds at 30,252 KiB peak RSS
  with zero major faults and zero swap.
- Permanent native assertions pin upward filtering, diagonal spread,
  solid-face acquisition, lower merge, and the exact density cutoff. The native
  runtime passes in 5:10.16 at 257,560 KiB peak RSS with zero major faults and
  zero swap. CPU throughput passes at 4,951 steps/s against the 3,858.9 floor
  and 4,062 baseline. All 13 locally available quick-sweep stages pass in 52.78
  seconds at 716,748 KiB peak RSS; only the two absent-snapshot Blaze stages
  skip. GPU 1 was untouched. Loaded-chunk selection and neighbor-driven
  invalid-support teardown remain open.

## 2026-08-08 (vine neighbor support and inherited-face teardown)

- Promoted `BlockVine.neighborChanged` through the ordinary synchronous block
  notification path. The callback now rechecks stored NORTH/EAST/SOUTH/WEST
  faces against the exact full-cube plus movement-blocking attachment
  predicate, retains a missing face when the vine immediately above carries
  the same bit, writes reduced metadata with flag-two semantics, and performs
  the zero-drop flag-three air transition when no stored face remains.
  Metadata-zero ceiling vines follow Java's stored-state behavior and break on
  notification even though their rendered actual state can expose `UP`.
- Added an eight-row parked real-Java/native gate covering single-face removal,
  partial pruning, inherited survival, mismatched inheritance, mixed direct and
  inherited support, metadata-zero removal, and synchronous two- and
  three-vine collapse. It compares 800 raw states plus World RNG, Math RNG, and
  entity-ID cursors. All rows pass in 1.26 seconds at 30,252 KiB peak RSS with
  zero major faults and zero swap; the cursors remain unchanged. The broader
  plant/random-tick gate remains 369/369 in 36.85 seconds.
- Permanent native assertions cover zero-drop break, partial pruning,
  inherited survival and collapse, mixed-face reduction, and metadata-zero
  removal. The full native runtime battery passes in 5:43.71 at 471,812 KiB
  peak RSS with zero major faults and zero swap. CPU throughput passes at 4,905
  steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass in 56.62 seconds at 716,824 KiB peak RSS;
  only the two absent-snapshot Blaze stages skip. GPU 1 was untouched.
  Loaded-chunk random-tick selection and other plant-family support/drop edges
  remain open.

## 2026-08-08 (leaf decay and ordinary break lifecycle)

- Closed the remaining ordinary leaf lifecycle seams around the existing
  canonical decay callback. Removing either log family now marks old and new
  leaves throughout the exact surrounding 9-cube; removing either leaf family
  marks the surrounding 3-cube. Both paths include diagonals and player-placed
  leaves. A successful decay now performs the flag-three neighbor
  notifications, fluid wakeup, observer update, and plant support teardown
  associated with Java's air transition.
- Added a 13-row parked real-Java/native gate. Its support cases distinguish a
  face-connected distance-four path through mixed old/new leaves from a
  distance-five log and a diagonal-only log. Drop rows pin oak sapling plus
  apple, jungle no-drop, acacia sapling metadata, and dark-oak apple outcomes.
  Break rows pin both marking radii, and a mixed decay row proves adjacent vine
  support loss without contaminating the RNG result. All 17,303 raw states,
  complete spawned item state, and World, Math, Block, and entity-ID cursors
  match. The gate passes 13/13 in 22.15 seconds at 30,252 KiB peak RSS with
  zero major faults or swap; the broader plant gate remains 369/369 in 36.74
  seconds.
- Permanent native assertions pin exact log radius, diagonal nondecayable
  marking, radius-five exclusion, and decay-driven vine teardown. The final
  native runtime passes in 5:08.58 at 256,056 KiB peak RSS with zero major
  faults or swap, and the Java build passes. CPU throughput passes at 4,727
  steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass in 1:30.55 at 715,388 KiB peak RSS; only
  the two absent-snapshot Blaze stages skip. GPU 1 was untouched. General
  loaded-chunk random-tick selection and unloaded-boundary behavior remain
  open.

## 2026-08-08 (mixed-opacity sky columns and static-water wakeup)

- Added a 13-row parked real-Java/native sky-column gate. It starts from an
  exact all-air, sky-light-15 boundary and covers stone, old leaves, static
  water, and glass addition/removal; sequential stacked-water edits; leaf
  placement under stone and water; and stone/water edits at local x=15. Exact
  transition tables pin every affected vertical light cell and reject
  contaminated fixtures.
- The comparator matches 75,504 raw block/skylight scalar values, exact
  pre/post delayed queues, and World, Math, Block, and entity-ID cursors. All
  13 rows pass in 0.52 seconds at 30,252 KiB peak RSS with zero major faults
  and zero swap. The static-water rows found a missing ordinary neighbor
  callback: still water now changes to flowing water with retained level and
  schedules its priority-zero callback five ticks later, matching Java.
- The first full aggregate correctly rejected an old waterlily assertion. A
  clean cold-state reproducer and the 1.11.2 `BlockStaticLiquid` body showed
  that piston destruction wakes the static source below the lily. The older
  piston oracle fixture was not evidence to the contrary because placing its
  fixture had already converted the water to flowing before the measured
  tick. The corrected all-16-metadata assertion now pins the flowing state and
  exact +5 queue entry, and a sub-second focused native target preserves the
  distinction.
- The Java build passes. The broad 369-row plant gate remains exact. The full
  native runtime battery passes in 6:02.64 at 478,256 KiB peak RSS with zero
  major faults and zero swap. CPU throughput passes at 4,874 steps/s against
  the 3,858.9 floor and 4,062 baseline. All 13 locally available quick-sweep
  stages pass in 55.01 seconds at 716,876 KiB peak RSS; only the two absent-
  snapshot Blaze stages skip. GPU 1 was untouched. General multi-column
  overhang shapes, unloaded boundaries, and deferred gap relighting remain
  open.

## 2026-08-08 (static-lava neighbor mixing and live effects)

- Promoted `BlockStaticLiquid.neighborChanged` for lava through the ordinary
  synchronous notification path. Nonmixing states retain metadata, become
  flowing lava, and schedule the exact priority-zero callback 30 ticks later
  in the Overworld/End or 10 in the Nether. Water on every face except DOWN
  forms obsidian from level zero
  and cobblestone from levels one through four. The flag-three solid write
  preserves nested notification behavior, including static-water conversion
  and +5 schedules.
- Added a 58-row parked real-Java/native gate covering four nonmixing levels,
  static and flowing water on multiple faces, DOWN exclusion, levels above the
  mixing threshold, both solid outcomes, two simultaneous water wakeups, and
  exact Nether/End cadence. Nine direct dynamic rows cover Nether and
  End decay one/two and cadence 10/30, plus a shaped-floor discriminator for
  the Nether four-step versus End two-step slope search. Two rows lock the
  exact `nextInt(4)` slowdown and +40/+120 schedules; another proves Forge's
  default refusal to create a lava source from two neighbors. Nether/End
  downward flow into air matches falling level eight and exact child/source
  queue order. Two more rows replace horizontal fire with flowing lava in the
  Nether and End, including child/source queues and extinguish effects. The
  next six cover horizontal static and flowing water replacement plus downward
  source-water reaction in both dimensions. Horizontal water first makes the
  source static, reenters mixing to form obsidian, wakes static water onto a
  stale +5 queue, and then continues the original callback to replace the
  target, producing two ordered extinguish streams. Downward contact forms
  stone, emits one stream, and requeues the source at +10/+30. Exact scheduled
  fluid callbacks retire only overlapping approximate-CA regions so the live
  CA cannot rewrite their result; a focused regression preserves a remote
  region. The Java fixture restores the injected save-state RNG after setup
  because `BlockFire.onBlockAdded` consumes world RNG while creating work that
  the controlled boundary discards. Six more Nether rows replace valid sapling,
  web, floor torch, snow layer, wall vine, and carpet states with exact
  effects/queues, while a supported ladder stays unchanged with no RNG use,
  proving Java's explicit non-flow exception. Nine more rows cover ordinary
  and powered rail, redstone wire, tall grass, wheat, dead bush, brown
  mushroom, lit redstone torch, and supported reeds. They exposed the powered
  rail break-hook ordering and sand's unconditional two-tick falling callback.
  Ten common-plant rows add both flowers, red mushroom, pumpkin/melon stems,
  waterlily, nether wart, carrots, potatoes, and beetroot. Waterlily also locks
  the support-water +5 callback ahead of child/source lava. The gate compares
  14,500 raw states, exact queues, one or two sound descriptors, eight or sixteen
  particle descriptors, all sound/particle floating-point bits, and World,
  Math, Block, and entity-ID cursors. All rows pass in 3.59 seconds at 46,160
  KiB peak RSS with zero major faults and zero swap.
- Added `block.lava.extinguish` to the represented sound manifest and routed
  the exact runtime SMOKE_LARGE stream into a fixed-pool implementation of the
  1.11.2 `ParticleSmokeLarge` constructor, update, texture, scale-ramp,
  lightmap, and billboard rules. Java's constructor-private wall-clock entropy
  is intentionally deterministic in the visual pool and does not feed world
  state. The focused particle and offline 58-row gates pass.
- Installing general flag-three notifications exposed a test-isolation bug:
  a cactus fixture's now-correct sand callback survived a raw-block reset into
  the following chorus fixture. The fixture boundary now resets the delayed
  queue and order cursor, matching a loaded saved-state boundary. The first
  aggregate rejected four contaminated assertions; the clean rerun passed.
- The fluid gate passes in 3.70 seconds at 41,768 KiB peak RSS. The full native
  runtime battery passes in 7:41.70 at 1,101,108 KiB peak RSS with zero major
  faults and zero swap. The Java build passes. CPU throughput passes at 4,730
  steps/s against the 3,858.9 floor and 4,062 baseline. All 13 locally
  available quick-sweep stages pass in 57.77 seconds at 714,032 KiB peak RSS
  with zero major faults and zero swap; only the two absent-snapshot Blaze
  stages skip. GPU 1 was untouched. Remaining metadata/material families,
  broader topology, and unloaded-boundary behavior remain open.

## 2026-08-08 (plant and attachment support teardown)

- Removed the approximate vertical plant scan from ordinary block edits,
  harvesting, leaf decay, and generated edit application. Unsupported bush
  states now enter their own synchronous neighbor callback, evaluate the
  1.11.2 support predicate, run the registry-derived destroy payload before
  the air write, and then perform the nested flag-three neighbor and observer
  traversal. Existing reed, cactus, double-plant, vine, and chorus callbacks
  remain separate and retain their Java-specific cascade rules.
- Added a 65-row parked real-Java/native gate. Nineteen rows cover saplings,
  tall grass, dead bush, flowers, mushrooms, crops, stems, waterlily, nether
  wart, reeds, cactus, and double plants. Thirty-six exhaustive metadata rows
  cover all snow layers, carpet colors, and canonical cocoa states. Ten
  controls verify accepted and rejected snow supports, carpet on static water,
  and cocoa on jungle versus oak logs. It compares 4,680 pre/post raw states,
  complete EntityItem descriptors, and World, Math, Block, and entity-ID
  cursors. All rows pass in 12.64 seconds at 30,252 KiB peak RSS with zero
  major faults and zero swap.
- Snow removal consumes no item or drop RNG. Carpet drops its preserved color
  before the air transition. Cocoa validates its facing-dependent horizontal
  jungle log, removes itself before spawning one or three brown-dye stacks,
  and rejects metadata 12 through 15 atomically. The broad aggregate caught
  the initial noncanonical metadata mistake before that guard was corrected.
- The integrated client constructs replicated EntityItems asynchronously and
  shares process-global `Entity.nextEntityID` and `Math.random` with its
  server. A short drain boundary between real-oracle rows prevents a previous
  drop replication from contaminating the next parked fixture's saved cursor.
  Two uninterrupted full real-oracle runs passed after adding that boundary.
- The quick sweep exposed an old bonemeal fixture that placed a mushroom on
  exposed bright-world dirt and then edited the neighboring dispenser. The
  new callback correctly removed that invalid state. Staging the deliberately
  cold mushroom last keeps the fixture on its intended bonemeal boundary; the
  broad automation oracle then passes unchanged.
- Focused plant, random-tick, automation, native oracle, and Java build gates
  pass. The focused runtime passes in 5:40.84 at 477,476 KiB peak RSS with zero
  major faults and zero swap. The complete native aggregate passes in 7:45.49
  at 1,101,128 KiB peak RSS with zero major faults and zero swap. CPU throughput
  passes at 4,733 steps/s against the 3,858.9 floor and 4,062 baseline. All 13
  locally available quick-sweep stages pass in 55.15 seconds at 716,924 KiB
  peak RSS with four major faults and zero swap; only the two absent-snapshot
  Blaze stages skip. GPU 1 was untouched. General loaded-chunk selection,
  unloaded boundaries, and unrepresented support families remain open.

## 2026-08-08 (unsupported cocoa random-tick teardown)

- Added a one-tick real-Java/native matrix case for mature cocoa without its
  facing-dependent jungle-log support. Before the fix, Java removed the pod
  and native rejected the controlled random-tick command as unrepresented.
- Implemented `BlockCocoa.updateTick`'s support-failure branch in the existing
  allocation-free callback path. It writes and synchronously notifies air
  first, then uses the retained old age state to emit three separate brown-dye
  items. It skips the growth `nextInt(5)` branch while retaining exact item
  offsets, Math.random constructor fields, entity IDs, and Block/World cursors.
- The unchanged focused case passes all 27 saved-state features, exact behavior,
  and all 10,625 raw block cells in 17.87 seconds. The focused runtime passes in
  6:33.87 at 477,688 KiB peak RSS with zero major faults and zero swap. The
  65-row plant/attachment support gate and random-tick battery pass. The
  complete native aggregate passes in 9:09.90 at 1,101,184 KiB peak RSS with
  zero major faults and zero swap. CPU throughput passes at 4,067 steps/s
  against the 3,858.9 floor and 4,062 baseline under concurrent host load. All
  13 locally available quick-sweep stages pass in 56.80 seconds at 716,892 KiB
  peak RSS with zero major faults and zero swap; the two absent snapshot stages
  skip. GPU 1 was untouched. Invalid-support random-tick teardown outside cocoa
  remains open.

## 2026-08-09 (note-block behavior, sound, particles, and state)

- Added exact note-block tile state and redstone-edge behavior. A bounded cold
  pool retains pitch and prior power, rising edges play, stable and falling
  edges do not, and an occupied ceiling suppresses effects. Right-click tunes
  modulo 25 before playback and left-click plays the retained pitch.
- Extended the live Java block registry to schema v3 with rock, sand, glass,
  and wood material masks. The native callback uses that authority to select
  bass drum, snare, hat, bass, or default harp without hand-maintained block
  lists. The sound manifest now owns 182 events and 563 variants.
- Added a fixed-pool NOTE particle using the 1.11.2 constructor formula, color,
  texture 64, six-age lifecycle, drag, scale ramp, and billboard path. Its
  private constructor entropy is deterministic visual state and never advances
  a world cursor.
- Added Java and native capture/restore support, script mutation/state output,
  strict capsule validation, and trace normalization for note tiles. The live
  Java restore hook was verified against an actual placed note block.
- The 12-row parked real-Java/native comparator passes in 2.57 seconds at
  30,252 KiB peak RSS. It matches tile state, sound/category/volume/pitch bits,
  particle payload bits, and unchanged World/Math RNG cursors. Audio, particle,
  script, capsule, Java-build, and exhaustive native gates pass. The native
  aggregate takes 7:01.99 at 506,256 KiB peak RSS with zero major faults and
  zero swap. CPU throughput passes at 4,043 steps/s against the 3,858.9 floor.
  The quick pyramid passes in 71.33 seconds at 716,848 KiB peak RSS; only two
  undistributed snapshot stages skip. The full `make -C magma test-game`
  aggregate passes in 575.93 seconds at 1,100,912 KiB peak RSS with zero major
  faults and zero swap, including the fresh-spawn-through-credits route. GPU 1
  was untouched. Pixel-scene promotion remains open.

## 2026-08-09 (igloo generation, loot, and resident sites)

- Added an asset bootstrap generator for the owned top, middle, and bottom
  1.11.2 igloo templates. The generated pointer-free table is consumed by one
  shared CPU/CUDA placement core with exact rotation and block-state transforms.
- Added a parked real-Java structure oracle. Ten fixtures cover four rotations,
  surface/basement selection, shaft depths 4 and 11, 41,875 packed cells per
  fixture, tile families, chest facing/seed, resident positions/types, and final
  RNG. Four more seeds match every field of the deferred 27-slot igloo loot.
- Integrated igloos into ice plains and cold taiga natural generation with
  complete rotated cross-chunk spill. The fixed populate cache retains chest
  metadata and one-time resident sites without allocation or live-tick scans.
- Added cold runtime synchronization for the priest and zombie-villager site.
  Both are persistence-required. The priest retains profession 2, wealth 15,
  and the template's two fixed offers. Full zombie-villager data/render and
  broader template entity NBT remain explicitly open.
- Focused Java, natural, and runtime gates pass in 15.27, 0.34, and 0.64
  seconds at 30,252, 25,920, and 82,088 KiB peak RSS. The shared CUDA worldgen
  translation unit compiles for `sm_120 --fmad=false` after replacing generated
  host pointers with indexed local constant accessors.
- Village runtime, deferred chest loot, mob runtime, the 40.34-second parity-60
  bundle, and the Java 8 build pass. CPU throughput passes at 4,962 steps/s
  against the 3,858.9 floor and 4,062 baseline. The quick sweep passes every
  locally runnable stage in 53.94 seconds at 716,788 KiB peak RSS; only the two
  undistributed Blaze snapshot stages skip. The full product aggregate passes
  through fresh spawn and credits in 458.33 seconds at 1,101,000 KiB peak RSS
  with zero major faults and zero swap. GPU 1 was not executed.

## 2026-08-09 (igloo resident saved state and zombie-villager rendering)

- Extended template extraction, generation sites, populate caches, and runtime
  materialization with both residents' exact position, motion, health,
  yaw/pitch, fire, air, profession, persistence, ground, type, and conversion
  fields. The parked Java oracle compares float fields at their stored float32
  boundary and all other fields exactly across every structure rotation.
- Added a distinct zombie-villager simulation type and routed its hostile,
  undead, melee, drop, sound, daylight, armor, and potion behavior through the
  existing bounded mob core. The igloo resident retains its inactive saved
  conversion timer without pretending the cure lifecycle is complete.
- Transcribed the exact eight-box 1.11.2 `ModelZombieVillager(false)` geometry
  and added all six profession textures to the generated atlas. Focused tests
  lock part count, UVs, winding, type and eye-height routing, head pose,
  profession selection, and byte identity against the owned jar.
- The Java/shared-CPU oracle passes in 13.41 seconds at 30,252 KiB peak RSS;
  the natural/runtime scattered gate passes in 2.58 seconds at 115,200 KiB.
  Java 8, entity render, mob-live, atlas, and CUDA compile gates pass. CPU
  throughput is 4,958 steps/s against the 3,858.9 floor and 4,062 baseline.
  The quick sweep passes in 53.20 seconds at 716,788 KiB. The full product
  aggregate reaches credits in 455.85 seconds at 1,100,964 KiB with zero major
  faults and zero swap. GPU 1 was compiled for but not executed. Active cure
  conversion, arbitrary entity NBT/attributes, and a stable Java/native pixel
  scene remain open.

## 2026-08-09 (swamp-hut witch runtime materialization)

- Added a retained-site enumerator and cold runtime synchronizer for generated
  swamp-hut witches. Cross-window sites deduplicate, materialize once into the
  bounded mob store, and preserve the exact represented centered position,
  motion, health, rotation, Fire, Air, persistence-required, and ground state.
  The scan is keyed by center chunk and populate build count and performs no
  unchanged-tick world traversal or allocation.
- Added a distinct live witch type with 26 health, correct dimensions/eye
  height/movement speed, hostile routing, and the existing exact witch
  renderer. Initial spawn consumes Java's follow-range Gaussian and subsequent
  five-percent left-hand roll. The close-range branch cannot fall through to
  the generic melee approximation while potion launch remains unimplemented.
- Added a six-seed parked real-Java/shared-CPU oracle. It compares position,
  motion, health, dimensions, movement/follow attributes, Fire, Air,
  persistence, ground state, raw 48-bit private cursor, Gaussian cache, and
  both hand outcomes. The comparator passes in 0.16 seconds at 30,252 KiB.
- Structure/runtime, entity-render, mob-live, Java 8, and shared CUDA compile
  gates pass. The CUDA compile takes 82.07 seconds at 3,185,280 KiB; GPU 1 was
  not executed. CPU throughput is 4,921 steps/s against the 3,858.9 floor and
  4,062 baseline. The quick sweep passes in 53.10 seconds at 716,776 KiB with
  only the two undistributed snapshot skips. The exhaustive product aggregate
  reaches credits in 476.13 seconds at 1,100,928 KiB with one major fault and
  zero swap. Exact potion selection/drinking/launch, witch loot/sounds,
  persistence serialization, and pixels remain open.

## 2026-08-09 (Witch self-potion selection and drinking)

- Added a pointer-free CPU/CUDA Witch transition matching the exact
  `EntityWitch.onLivingUpdate` branch order, active-effect guards, timer edge,
  potion outcomes, sound-pitch draw, and final status-15 random roll.
- Integrated it into the ordinary live Witch tick. Drinking suppresses movement
  input, retains authoritative held state for entity views, and completion
  queues effects after the general aging pass. Duration-1 Instant Health now
  applies and expires on the following represented mob tick.
- Added ten parked real-Java/shared-CPU cases and a live regression covering a
  production tick, four starts, four completions, exact cursors, held state,
  the rare status event, and next-tick healing. The Java comparator passes in
  0.16 seconds at 30,252 KiB.
- The scattered/runtime, mob-live, 389-check brewing, Java 8, and
  `sm_120 --fmad=false` compile gates pass. The CUDA probe compiles in 0.50
  seconds at 188,640 KiB without GPU execution. CPU throughput passes at 4,682
  steps/s against the 3,858.9 floor and 4,062 baseline.
- Ranged potion selection/launch, audio playback, actual held-potion and status
  particles, loot, persistence, and Witch pixel promotion remain open.

## 2026-08-09 (Witch ranged splash-potion combat)

- Added a pointer-free CPU/CUDA Witch ranged transition matching the real
  moving-target distance, Slowness/Poison/Weakness guards, health threshold,
  Weakness RNG, Harming fallback, spawn/aim geometry, and throw-pitch draw.
- Integrated the 60-tick ranged task, independent visibility counter,
  20-visible-tick hold condition, cooldown-zero occlusion wait, and drinking
  suppression. A fixed three-entry per-Witch queue feeds the existing fixed
  splash-projectile flight, collision, impact/effect, and rendering path.
- Materialization now preserves a living shooter and reconciled entity ID,
  uses exact private three-Gaussian heading plus MathHelper rotation, and emits
  `entity.witch.throw` through the HOSTILE fixed sound stream.
- Nine parked real-Java/shared-CPU cases compare the actual selected potion,
  moving-distance boundary, active effects, relative spawn/aim bits, every
  direct Witch draw, sound bits, and a seeded real EntityPotion's exact motion,
  rotation, cursor, and Gaussian cache. The comparator passes in 0.19 seconds
  at 30,252 KiB. The 65-check live regression covers all outcomes, cooldown,
  drinking and visibility suppression, queue transport, runtime construction,
  first flight tick, and sound delivery.
- Scattered/runtime passes in 3.31 seconds at 115,200 KiB, mob-live in 6.82
  seconds at 172,320 KiB, and the 389-check brewing gate passes. Java 8 builds
  in 3.64 seconds at 112,328 KiB. The shared CUDA probe compiles in 0.55
  seconds at 188,640 KiB without executing GPU 1. CPU throughput passes at
  4,752 steps/s against the 3,858.9 floor and 4,062 baseline.
- Audio-live passes for 182 represented events and 563 loaded variants. The
  full native aggregate reaches credits in 7:47.04 at 1,101,244 KiB with zero
  major faults or swap. The clean quick pyramid passes in 58.52 seconds at
  714,016 KiB with zero major faults or swap; only the two undistributed
  `.bsnp` stages skip.
- Ordinary projectile constructor entropy remains deterministic unless an
  oracle cursor is injected. Fixed-capacity overflow, arbitrary NBT resume,
  drink/hurt/death audio, loot, held/status particles, and pixel promotion
  remain open.

## 2026-08-09 (Witch lifecycle audio and exact player loot)

- Added exact Witch drink, hurt, and death events to the fixed mob/runtime
  sound streams and owned manifest. The Java self-potion oracle now includes
  drink sound bits; the 117-case player-attack oracle includes Witch hurt and
  death. Audio-live passes with 186 represented events and 576 variants.
- Added a pointer-free CPU/CUDA implementation of the real seven-entry
  `entities/witch` table, including weighted stick selection, one-to-three
  rolls, `SetCount`, Looting bonus, zero-stack discard, and exact private RNG.
  Thirteen parked Java/native rows cover every item family, Looting 0 through
  3, `doMobLoot=false`, separate stacks, EntityItem constructor fields, Math
  cursor, and global entity IDs. The comparison passes in 0.17 seconds at
  30,252 KiB peak RSS.
- Integrated the verified table into ordinary player-credited Witch deaths.
  The held enchant supplies Looting, synchronous loot precedes the player
  attack tail, accepted deaths remain loaded for their update window, and
  fixed item capacity is preflighted atomically. The focused live gate covers
  a three-stack exact result, the product player route, gamerule suppression,
  status/sound order, and a full-store rejection.
- Scattered/runtime, mob-live, player-audio, audio-live, Java 8, and
  `sm_120 --fmad=false` compile gates pass without GPU execution. CPU
  throughput is 4,842 steps/s against the 3,858.9 floor and 4,062 baseline.
  The full native aggregate reaches credits in 480.04 seconds at 1,100,120
  KiB with zero major faults and zero swap. The quick pyramid passes in 55.78
  seconds at 716,824 KiB with one major fault and zero swap; only the two
  undistributed `.bsnp` stages skip. At this checkpoint terminal Witch XP was
  still open; the next entry closes the ordinary non-drinking player-credit
  case. Non-player lethal routes, drinking/equipped XP, arbitrary NBT resume,
  held/status rendering, and pixel promotion remain open.

## 2026-08-09 (ordinary Witch terminal XP)

- Added the exact ordinary non-drinking player-credit tick-20 XP tail. Witch
  base experience five splits into independent `3,1,1` orbs with exact global
  IDs and position, motion, yaw, health, age, pickup, color, and target-color
  constructor state. The three constructors consume the shared Math cursor
  before the exact 20-particle Witch RNG tail; `World.rand` remains unchanged.
- Added a fixture-only `EntityLivingBase.onDeathUpdate` spawn capture and a
  seven-case real-Java/shared-CPU comparator. It checks raw Witch, Math, and
  World cursors, all orb fields, Gaussian cache, global IDs, removal state,
  and `doMobLoot=false`.
- Integrated the split into both controlled and ordinary live Witch death
  updates with all-three-slot preflight. The live gate locks dynamic loaded
  order, same-boundary orb ticks, terminal particles, retirement, product
  routing, and gamerule suppression.
- Scattered/runtime passes in 3.49 seconds at 115,200 KiB and mob-live passes
  in 6.89 seconds at 172,748 KiB. CPU throughput is 4,794 steps/s against the
  frozen 3,858.9 floor. The full native aggregate reaches credits in 492.81
  seconds at 1,101,196 KiB with zero major faults or swap. The quick pyramid
  passes in 53.70 seconds at 713,996 KiB with two major faults, zero swap, and
  only the two undistributed `.bsnp` skips. Java 8 and the shared
...
## 2026-08-09 (represented equipped Witch death)

- Added shared CPU/CUDA helpers for the exact main-hand equipment-drop float
  comparison and equipped-stack XP bonus. Player-credited drinking-Witch death
  now runs loot selection, optional held-potion EntityItem construction,
  tick-20 `1 + nextInt(3)` XP bonus, greedy split-orb construction, and the
  terminal particle tail in Java order. The four represented self-potion kinds
  map to canonical PotionType metadata in the compact item schema.
- Added an eight-case parked real-Java/shared-CPU comparator. It covers potion
  drop/no-drop, XP totals 6/7/8, splits `3,3`/`7`/`7,1`, Looting zero through
  three, zero through three ordinary loot stacks, gamerule suppression,
  EntityItem and EntityXPOrb fields, global IDs, raw Witch/Math/World cursors,
  and the final Gaussian cache. A fixture setup check caught and corrected the
  distinction between Looting read from the attacking player's held enchant
  and the modifier passed to `dropEquipment` before product code was changed.
- Added a live lethal-to-terminal regression. Its potion-drop branch verifies
  healing-potion metadata and constructor state, XP eight, two same-tick orbs,
  loaded order, and 20 exact particles. A separate no-drop branch proves that
  XP seven is independent of drop success through the ordinary product tick.
  A full item store rejects the potion-only lethal boundary atomically.
- Scattered/runtime passes in 1.86 seconds at 82,120 KiB, mob-live in 6.98
  seconds at 172,728 KiB, and CPU throughput at 4,615 steps/s against the
  3,858.9 floor. Java 8 passes. The shared `sm_120 --fmad=false` compile probe
  passes in 0.53 seconds at 188,640 KiB without GPU execution. The quick
  pyramid passes in 55.91 seconds at 716,844 KiB with zero major faults or
  swap and only the two undistributed `.bsnp` skips.
- Non-player Witch lethal/loot paths, arbitrary injected equipment and
  Witch NBT/load resume, held/status rendering, and Witch/potion pixels remain
  open. Fixed item/orb capacity remains an explicit atomic boundary.

## 2026-08-09 (ordinary drowning-killed Witch death)

- Promoted the first non-player Witch lethal route through the ordinary live
  tick. The drowning pulse now preserves the 48 bubble floats, fresh status,
  skipped `setBeenAttacked`, no-attacker Math yaw, death-sound pitch, Looting-0
  table, status 3, and immediate `deathTime=1` ordering.
- Kept Java's attribution distinction explicit: drowning leaves
  `recentlyHit=0`, so ordinary table loot still runs but a represented held
  potion never enters `dropEquipment` and tick 20 emits no XP. Terminal
  particles retain the exact Gaussian/float tail.
- Added a seven-case parked real-Java/shared-CPU comparator over zero through
  three stacks, gamerule suppression, exact item constructors, all cursors,
  global IDs, death fields, no equipment/orbs, and Gaussian cache. It passes
  in 0.19 seconds at 30,252 KiB.
- Added a focused product regression using real water and air state. It locks
  the ordinary death window, exact drops/events, no potion/XP, 20 particles,
  `doMobLoot=false`, and atomic fixed-item-capacity rejection.
- Scattered/runtime passes in 3.41 seconds at 115,200 KiB with three major
  faults and zero swap. Mob-live passes in 6.86 seconds at 172,860 KiB with no
  major faults or swap. CPU throughput is 5,009 steps/s against the frozen
  3,858.9 floor and 4,062 baseline. Java 8 builds. The quick pyramid passes in
  53.37 seconds at 716,872 KiB with two major faults, zero swap, and only the
  two undistributed `.bsnp` stages skipped.
- Other non-player Witch lethal sources and attribution, arbitrary equipment
  and NBT resume, held/status rendering, and Witch/potion pixels remain open.

## 2026-08-09 (ordinary falling-anvil-killed Witch death)

- Extended the actual falling-block collision callback from controlled
  passives to ordinary Witches. Lethal anvil damage now preserves the
  `setBeenAttacked` private draw, global Math yaw, death-sound pitch, Looting-0
  table, exact item constructors, status order, and following product mob tick
  to `deathTime=1`.
- Kept no-attacker credit exact: a drinking Witch does not drop its held potion
  and creates no terminal XP, while ordinary table loot still obeys
  `doMobLoot`. Fixed item capacity rejects the complete lethal boundary before
  mutating mob state or any RNG cursor.
- Added a seven-case parked real-Java/shared-CPU comparator over zero through
  three table stacks, gamerule suppression, every item/event field, all
  cursors and global IDs, death fields, no equipment/orbs, and the terminal
  Gaussian cache. It passes in 0.20 seconds at 30,252 KiB.
- Added a focused live regression through the real collision function and
  production tick order. It passes in 0.02 seconds at 18,744 KiB. The complete
  header-rebuilt runtime regression, including existing controlled passive
  anvil cases, passes in 329.56 seconds at 505,780 KiB with zero major faults.
- Scattered/runtime passes in 3.38 seconds at 115,200 KiB and mob-live in 6.76
  seconds at 172,688 KiB. CPU throughput remains green at 4,883 steps/s
  against the frozen 3,858.9 floor and 4,062 baseline.
- Java 8 builds. The quick pyramid passes in 52.45 seconds at 717,012 KiB with
  two major faults, zero swap, and only the two undistributed `.bsnp` stages
  skipped.
- Other non-player lethal sources, broader anvil multi-target behavior,
  arbitrary equipment/NBT resume, held/status rendering, and Witch/potion
  pixels remain open.

## 2026-08-09 (ordinary periodic-ON_FIRE Witch death)

- Factored falling-anvil and periodic ON_FIRE Witch damage through one exact
  source-less damage body. The product fire phase now precedes air and living
  work, preserves fire 20-to-19, ages fresh hurt timers to 9/19, and reaches
  `deathTime=1` in the same tick.
- Preserved source attribution: Looting-0 table loot still runs, but a drinking
  Witch cannot drop its held potion and creates no terminal XP without attacker
  credit. Fire Resistance suppresses the attack while fire continues aging.
- Expanded the parked comparator to 14 real-Java/shared-CPU rows, seven each
  for falling-anvil and ON_FIRE deaths. It checks zero through three stacks,
  gamerule suppression, all item/event fields, fire/death state, every cursor
  and ID, no equipment/orbs, and terminal particles. It passes in 0.20 seconds
  at 30,252 KiB.
- Added a focused live gate for product order, Fire Resistance, the terminal
  tail, and atomic item-capacity retry. It passes in 0.01 seconds at 18,728
  KiB. Scattered/runtime passes in 3.37 seconds at 115,200 KiB and mob-live in
  6.75 seconds at 173,232 KiB. CPU throughput remains green at 4,741 steps/s
  against the 3,858.9 floor and 4,062 baseline.
- The header-rebuilt full runtime gate, including all prior anvil/passive
  fixtures, passes in 327.96 seconds at 506,204 KiB with zero major faults.
- The quick pyramid passes in 53.00 seconds at 716,904 KiB with one major
  fault and only the two undistributed `.bsnp` stages skipped.
- Other non-player lethal sources, arbitrary equipment/NBT resume,
  held/status rendering, and Witch/potion pixels remain open.

## 2026-08-09 (ordinary lava-killed Witch death)

- Added exact ordinary-Witch `isInLava` AABB probing immediately after the
  periodic fire phase. LAVA applies raw damage four, a 300-tick fire floor, and
  fall-distance halving before air, timer aging, and death progress.
- Expanded the parked source comparator to 28 real-Java/shared-CPU rows: seven
  each for falling anvil, ON_FIRE, direct lava, and ON_FIRE-before-LAVA. The
  combined case caught the exact hurt-sound ordering: a nonlethal fresh fire
  pulse sounds once, then LAVA kills through the immunity-window difference
  without a second sound and restores fire from 19 to 300.
- Extended the focused live gate through actual world lava with Fire
  Resistance, exact no-credit loot, direct and combined phase order, and
  full-item-pool atomic retry. The comparator passes in 0.46 seconds at 34,560
  KiB and the live gate in 0.02 seconds at 18,728 KiB.
- Scattered/runtime passes in 5.22 seconds at 180,272 KiB, mob-live in 6.78
  seconds at 172,128 KiB, and CPU throughput in 5,024 steps/s against the
  3,858.9 floor and 4,062 baseline. The exact lava scan is bounded and
  allocation-free inside the existing mob visit.
- The full runtime gate passes in 325.65 seconds at 506,028 KiB with zero major
  faults. Java 8 builds. The quick pyramid passes in 56.66 seconds at 716,928
  KiB with five major faults and only the two undistributed `.bsnp` stages
  skipped.
- Block-collision IN_FIRE and other non-player lethal sources, arbitrary
  equipment/NBT resume, held/status rendering, and Witch/potion pixels remain
  open.

## 2026-08-09 (ordinary block-fire-contact Witch death)

- Added the exact contracted FIRE/LAVA callback probe to the ordinary Witch
  move path. Cold FIRE contact now applies raw-one source-less damage and sets
  fire -1 to 160, with exact Fire Resistance, immunity-window, repeat-contact,
  wet-extinguish RNG, and fixed-capacity behavior.
- Restored the surrounding base-entity composition needed by a true full-tick
  oracle: `ticksExisted`, previous positions, Witch's ambient `nextInt(1000)`,
  two-float pitch, hostile sound routing, and lethal short-circuit ordering.
  The generated sound manifest now owns `entity.witch.ambient`.
- Expanded the parked comparator from 28 to 42 real-Java/shared-CPU rows with
  direct and full-tick IN_FIRE families. Every row locks loot, constructors,
  events, damage/death fields, terminal particles, IDs, and all RNG cursors.
  It passes in 0.61 seconds at 30,252 KiB. The focused actual-block gate passes
  in 0.03 seconds at 18,728 KiB.
- Scattered/runtime passes in 1.72 seconds at 82,112 KiB, mob-live in 6.94
  seconds at 172,168 KiB, and CPU throughput at 4,937 steps/s against the
  frozen 3,858.9 floor and 4,062 baseline. Full runtime passes in 346.11
  seconds at 505,888 KiB with zero major faults. Java 8 builds.
- The quick pyramid passes in 53.55 seconds at 716,900 KiB, with every
  available step green and only the two undistributed `.bsnp` stages skipped.
  Rain/mixed-water contact, other non-player sources, arbitrary equipment/NBT,
  held/status rendering, and Witch/potion pixels remain open.

## 2026-08-09 (ordinary cactus-contact Witch death)

- Added the ordinary Witch `doBlockCollisions` cactus callback before the
  flammable move tail. The coordinate scan uses the exact 0.001-inset entity
  AABB independently of cactus's 15/16-high collision shape.
- Expanded the parked comparator from 42 to 56 real-Java/shared-CPU rows with
  direct and full-tick CACTUS families. The new rows preserve the exact
  ambient/status prefix, post-move death state, no-credit loot, feedback,
  particles, constructors, IDs, and all RNG cursors. It passes in 0.56 seconds
  at 30,252 KiB.
- The actual-world focused gate stages cactus over sand at the collision top,
  locks three exact loot stacks and fixed-capacity rejection, and passes in
  0.03 seconds at 18,752 KiB. Scattered/runtime passes in 3.39 seconds at
  115,200 KiB and mob-live in 6.82 seconds at 173,124 KiB.
- CPU throughput passes at 5,000 steps/s against the frozen 3,858.9 floor and
  4,062 baseline. Java 8 builds. The quick pyramid passes in 55.11 seconds at
  716,848 KiB, with every available step green and only the two undistributed
  `.bsnp` stages skipped.

## 2026-08-09 (ordinary still-water plus block-fire Witch death)

- Added the ordinary Witch base-phase still-water probe before periodic fire
  and lava. Water overlap now latches `inWater`, clears fall distance, and
  extinguishes before the later move callbacks.
- Closed the static mixed WATER/FIRE composition: a Witch spanning adjacent
  water and supported fire still receives raw-one IN_FIRE damage, while wetness
  suppresses ignition and leaves the final fire counter at zero.
- Expanded the parked source comparator from 56 to 63 real-Java/shared-CPU
  rows. The seven new full-tick cases lock ambient/status prefix ordering,
  Looting-0 drops, constructors, events, particles, IDs, fire/fall/water state,
  and every RNG cursor. It passes in 0.70 seconds at 30,252 KiB; the focused
  actual-world gate passes in 0.04 seconds at 18,764 KiB.
- Scattered/runtime passes in 3.46 seconds at 115,200 KiB, mob-live in 6.91
  seconds at 171,816 KiB, and CPU throughput at 4,955 steps/s against the
  3,858.9 floor and 4,062 baseline. Java 8 builds.
- The quick pyramid passes in 54.34 seconds at 716,928 KiB with zero failures,
  five major faults, no swap, and only the two undistributed `.bsnp` stages
  skipped. Rain wetness, flowing-water acceleration/entry effects, other
  non-player sources, persistence, held/status rendering, and pixels remain
  open.

## 2026-08-09 (ordinary rain-wet block-fire Witch death)

- Added ordinary-Witch rain wetness through Java's feet and full-height
  precipitation checks using the live world clock, dimension, and open-sky
  precipitation height. The base living phase extinguishes rain-exposed FIRE
  contact after damage; a solid roof rejects wetness and retains fire 160.
- Expanded the parked source comparator from 63 to 77 real-Java/shared-CPU
  rows with seven open-rain and seven roofed-rain cases. They lock exact
  damage/death state, Looting-0 drops, constructors, events, particles, IDs,
  fire state, and every Witch/Math/World cursor. It passes in 0.73 seconds at
  30,252 KiB; the focused actual-world gate passes in 0.06 seconds at 18,728
  KiB.
- Scattered/runtime passes in 3.43 seconds at 115,200 KiB, mob-live in 6.71
  seconds at 171,880 KiB, and CPU throughput at 4,960 steps/s against the
  3,858.9 floor and 4,062 baseline. Java 8 builds.
- A clean header-rebuilt `test-game` passes every game suite through the
  fresh-spawn-to-credits route in 481.99 seconds at 1,100,948 KiB with one
  major fault and no swap. The quick pyramid passes in 55.30 seconds at
  716,840 KiB with zero failures, two major faults, no swap, and only the two
  undistributed `.bsnp` stages skipped.
- Flowing-water acceleration and entry splash effects, broader
  precipitation-height/biome edges, other non-player sources, persistence,
  held/status rendering, and pixels remain open.

## 2026-08-09 (ordinary Witch flowing-water and dry-to-water entry)

- Reused the shared Java-verified `handleMaterialAcceleration` kernel in the
  ordinary Witch base phase. A flowing-water cell beside a west source now
  adds the exact normalized +0.014 X motion before an overlapping upper-body
  lava hit; the earlier water phase clears fall distance before lava halves
  it and applies damage/fire.
- Expanded the source comparator from 77 to 84 real-Java/shared-CPU rows. The
  seven new complete ticks lock motion, wet/fire/fall and death state,
  Looting-0 drops, constructors, feedback, terminal particles, IDs, and every
  Witch/Math/World cursor. It passes in 0.99 seconds at 34,560 KiB.
- Added an isolated seven-seed real-Java/shared-CPU dry-to-water gate. It
  bit-matches the non-first-update HOSTILE splash event and position,
  motion-scaled volume, two-float pitch, unchanged motion, wet/fire/fall state,
  26 ordered particle calls, and final entity RNG cursor. It passes in 0.29
  seconds at 31,680 KiB. The 188-event sound manifest and HOSTILE runtime drain
  now own `entity.hostile.splash`.
- The product retains the exact mob splash payload in four bounded batches,
  drains all 26 descriptors into the current-tick runtime seam, and invokes
  the existing water-particle constructor, lifecycle, and renderer. The
  focused gate checks every coordinate, velocity, kind, entity, dimension,
  and order. Constructor-private visual entropy stays deterministic rather
  than being mislabeled Java-pixel exact.
- Focused live passes in 0.11 seconds at 20,216 KiB, the water renderer gate
  in 0.11 seconds at 36,000 KiB, player movement runtime in 0.18 seconds at
  20,176 KiB, audio in 0.51 seconds at 63,360 KiB, scattered/runtime in 3.46
  seconds at 115,200 KiB, and mob-live in 6.84 seconds at 172,964 KiB. Java 8
  builds.
- A clean header-rebuilt `test-game` passes every game suite through the
  fresh-spawn-to-credits route in 484.66 seconds at 1,101,224 KiB with ten
  major faults and zero swap. CPU throughput passes at 5,030 steps/s against
  the frozen 3,858.9 floor and 4,062 baseline. The quick pyramid passes in
  55.36 seconds at 715,468 KiB with zero failures, zero major faults, zero
  swap, and only the two undistributed `.bsnp` stages skipped.
- Broader precipitation-height/biome edges, mob splash pixel promotion, other
  non-player sources, persistence, held/status rendering, and pixels remain
  open.

## 2026-08-09 (ordinary Witch in-wall suffocation)

- Extended the real-Java blockstate registry capture to schema v4 with exact
  `Block.causesSuffocation` results for all 4,096 legacy states. The generated
  predicate now drives both the in-wall overlay and ordinary Witch damage;
  the capsule pins digest
  `57911f7d3a42bd1e585d3cca75283fe500fbfcb2308c3f15127837a2c56433f6`.
- Added Java's exact eight width-scaled eye samples in base-phase order and
  expanded the source comparator from 84 to 91 real-Java/shared-CPU rows. It
  passes in 0.85 seconds at 30,252 KiB with exact loot, constructors, events,
  terminal particles, IDs, and all Witch/Math/World cursors.
- Corrected a neighboring death-tick edge discovered by the oracle: a Witch
  killed in the base phase still executes its subclass living-status step
  after death progress. Drowning, anvil, fire, lava, and in-wall focused gates
  now pin that cursor and drinking-timer behavior.
- Focused live passes in 0.14 seconds at 20,180 KiB, scattered/runtime in 1.96
  seconds at 82,104 KiB, mob-live in 6.73 seconds at 172,924 KiB, particles in
  0.13 seconds at 36,000 KiB, and Java 8 builds. A clean rebuilt `test-game`
  passes through the final route in about 489 seconds.
- CPU throughput remains above the frozen gate at 4,748 steps/s versus the
  3,858.9 floor and 4,062 baseline. The quick pyramid passes in 56.78 seconds
  at 717,036 KiB with zero failures, zero major faults, zero swap, and only
  the two undistributed `.bsnp` stages skipped. Fall, general-mob suffocation,
  persistence, held/status rendering, and pixels remain open.

## 2026-08-09 (ordinary Witch stone/hay landing)

- Added the ordinary Witch's late FALL landing boundary to the product move
  path. It preserves the pre-move fall distance, applies Jump Boost deduction
  and the support block's fall multiplier, and keeps death time at zero for a
  lethal hit during `Entity.move`. Stone uses full damage; hay uses one-fifth.
- Expanded the source comparator from 91 to 105 real-Java/shared-CPU rows.
  Seven stone and seven hay complete ticks lock exact final position/motion,
  ground/fall and combat timers, Looting-0 drops, constructors, IDs, feedback,
  and every Witch/Math/World cursor. It passes in 0.88 seconds at 30,252 KiB.
- Added packet-shaped BLOCK_DUST export to the bounded mob/runtime particle
  seam. The fixtures match Java's counts 40/80 and encoded block-state
  parameters 1/170. Hostile small/big fall and all represented support-block
  fall families now drain into the audio stream; the owned manifest contains
  190 represented events and 586 variants.
- Focused live passes in 0.12 seconds at 20,160 KiB, audio in 0.37 seconds at
  63,420 KiB, and mob-live in 6.45 seconds at 172,336 KiB, all without major
  faults or swap. Java 8 builds.
- CPU throughput passes at 4,942 steps/s against the 3,858.9 floor and 4,062
  baseline. The full game aggregate passes through fresh spawn to credits in
  478.45 seconds at 1,101,084 KiB with one major fault and zero swap. The
  quick pyramid passes in 54.57 seconds at 716,848 KiB with zero failures,
  zero major faults, zero swap, and only the two undistributed `.bsnp` stages
  skipped.
- Other non-player sources, general-mob suffocation/landing callbacks,
  arbitrary Witch NBT resume, held/status rendering, landing-particle pixel
  promotion, and Witch/potion pixels remain open.

## 2026-08-09 (ordinary Witch landing completion)

- Added exact big-stone fall coverage to the lethal source matrix. The parked
  real-Java/shared-CPU comparator now passes 112 rows across sixteen source
  scenarios and seven seeds in 1.01 seconds at 30,252 KiB.
- Added a separate 28-row landing comparator for nonlethal Jump Boost II,
  non-sneaking living slime bounce, seeded farmland trample, and
  `mobGriefing=false`. It locks position and motion bits, health/timers,
  effects, BLOCK_DUST descriptors, support state, and Witch/World RNG, and
  passes in 0.22 seconds at 30,252 KiB.
- Promoted farmland's exact 15/16 collision height and staged World.rand/block
  mutation, plus the ordinary Witch slime landing callback. An oracle
  comparison exposed and fixed an ordering bug: visible potion effects consume
  server Entity.rand before Witch ambient logic even though the base server
  particle method emits no packet.
- Focused live passes in 0.19 seconds at 20,200 KiB, mob-live in 8.09 seconds
  at 172,256 KiB, scattered/runtime in 4.40 seconds at 115,200 KiB, particles
  in 2.55 seconds at 289,544 KiB, and audio in 0.82 seconds at 63,396 KiB.
  Java 8 builds.
- CPU throughput passes at 4,104 steps/s against the frozen 3,858.9 floor and
  4,062 baseline. A clean full `test-game` passes through fresh spawn to
  credits in 484.76 seconds at 1,100,928 KiB with no major faults or swap.
  The quick pyramid passes in 65.98 seconds at 715,480 KiB with zero failures
  and only the two undistributed `.bsnp` stages skipped. GPU 1 was left
  untouched while shared.
- General-mob landing callbacks, sneaking/nonliving slime variants, arbitrary
  Witch NBT resume, held/status rendering, landing-particle pixel promotion,
  and Witch/potion pixels remain open.

## 2026-08-09 (shared ordinary-living slime landing)

- Replaced the Witch-only ordinary slime contact scan with a shared bounded
  living path. It reuses the collision query's resident block list, performs
  no extra world reads or heap allocation, and caps landing contacts at 32.
- Added a six-row parked real-Java/shared-CPU travel comparator. Zombie,
  sheep, and Witch match raw position/motion, collision/ground/fall flags, and
  support state for both the full bounce and low-speed horizontal damping.
  Every row also locks Java's exact 80-count BLOCK_DUST descriptor.
  The coordinate is passed through explicitly so IEEE-754 subtraction is
  compared at the same absolute world cell instead of hidden by tolerance.
- Added actual product-tick regressions for hostile zombie and passive sheep;
  both finish at Java's exact `0.01960000038146973` Y-motion tail and publish
  the same generic landing particle batch. The prior 28-row Witch landing
  matrix remains exact.
- The new comparator passes in 0.15 seconds at 30,252 KiB, Witch landing in
  0.20 seconds at 30,252 KiB, mob-live in 6.74 seconds at 173,188 KiB,
  scattered/runtime in 3.57 seconds at 115,200 KiB, and particles in 0.13
  seconds at 37,440 KiB. Java 8 builds. CPU throughput passes at 4,991 steps/s
  against the 3,858.9 floor and 4,062 baseline. The quick pyramid passes in
  54.00 seconds at 716,896 KiB with zero
  failures and only the two undistributed `.bsnp` stages skipped. GPU 1 was
  not executed.
- General-mob fall damage/audio, landing-particle pixels, sneaking/nonliving
  slime, mixed-contact cases, general-mob suffocation, and other landing
  callbacks remain open.

## 2026-08-09 (ordinary-living nonlethal landings)

- Promoted ordinary nonlethal small/big stone falls, hay reduction, and Jump
  Boost II avoidance through the common non-ridden living path for zombie,
  zombie villager, skeleton, wither skeleton, creeper, spider, cave spider,
  pigman, silverfish, sheep, pig, cow, and villager. The boundary preserves
  exact hostile/generic fall audio, type hurt feedback, support audio,
  health/immunity/effect state, entity and Math RNG, landing dust, and creeper
  fuse advancement.
- Expanded the real-Java/native living-landing comparator from six to 58 exact
  rows. It exposed zombie villager's distinct hurt event, now represented, and
  Enderman's immediate random teleport override, which remains an explicit
  special-case backlog item rather than an approximate promotion.
- Added actual product-tick coverage for all 13 types on small stone plus
  representative big-stone, hay, and Jump Boost branches. The regressions also
  pin source-less FALL semantics: pigman anger and passive panic stay zero.
  The audio gate drains a passive landing as generic fall, type hurt, then
  neutral stone support. The sound manifest owns 194 events and 591 variants.
- The comparator passes in 0.33 seconds at 30,252 KiB, Witch landing in 0.17
  seconds at 30,252 KiB, mob-live in 8.73 seconds at 172,004 KiB, particles in
  0.16 seconds at 37,440 KiB, audio in 0.52 seconds at 65,772 KiB, and
  scattered/runtime in 4.49 seconds at 115,200 KiB. Java 8 builds.
- CPU throughput passes at 4,946 steps/s against the 3,858.9 floor and 4,062
  baseline. The quick pyramid passes in 54.69 seconds at 714,340 KiB with zero
  failures, seven major faults, no swap, and only the two undistributed `.bsnp`
  stages skipped. GPU 1 was not executed.
- Lethal ordinary falls, Enderman teleport-on-damage, slime's generic fall
  callback, special no-fall classes, other block callbacks, landing-particle
  pixels, and general-mob suffocation remain open.

## 2026-08-10 (ordinary item environment continuation)

- Added a 39-row real-Java/native `EntityItem` environment comparator covering
  flowing/still water, lava, periodic/contact fire, wet extinguish, cactus,
  expiry, void retirement, and five-direction full-cube push-out across four
  private-RNG seeds.
- Extended the version-2 capsule with fire, water, first-update, and private
  RNG state. The continuation gate now covers air, ground, landing, water,
  fire, cactus, push-out, and loaded-order merge behind conservative proof
  fences.
- The full aggregate caught and drove fixes for soul sand's 7/8 collision box
  and the controlled stationary-item fixture contract. The repaired aggregate
  passes in 7:42.22 at 505,560 KiB peak RSS with no major faults or swap.
- The quick pyramid passes all 13 locally runnable stages in 1:07.91 at
  715,596 KiB. Stopped-oracle CPU throughput passes at 4,983 steps/s against
  the 3,858.9 floor and 4,062 baseline. GPU 1 was not executed while shared.

## 2026-08-10 woodland mansion checkpoint

- Ported the complete 1.11.2 woodland mansion grid, room assignment, wall and
  roof traversal, and piece graph. Five seed/rotation fixtures match real Java
  piece count, bounds, and whole-graph hash exactly.
- Generated one compact owned table from all 73 real templates: 66,331 block
  cells, 156 legacy states with exact mirror/rotation metadata, and 34 data
  markers. Runtime placement includes piece overwrite order, foundations,
  exact-facing deferred-loot chests, and idempotent chunk discovery.
- Mage and Warrior markers materialize as persistent Evokers and Vindicators.
  Base health, dimensions, speed, hostile lifecycle, Vindicator iron-axe
  damage, Evoker close-player avoidance, loot, hurt/death audio, skins, and
  the exact idle crossed-arm model are live. The real-Java loot gate matches
  all 30 item/count and final entity-RNG rows.
- Mansion graph/template/runtime/loot, broad mob lifecycle, renderer
  geometry/UV/winding, and product builds pass. CPU throughput is 4,270
  steps/s against the 4,062 baseline and 3,858.9 floor. GPU 1 was not used.
- Honest effort-weighted parity is approximately 85%. Ocean monuments and
  guardians, Evoker fangs/Vex/Wololo and casting poses, general task/NBT
  continuation, remaining mob families, exhaustive redstone/rail edges, and
  global pixel equivalence remain open.

## 2026-08-10 Evoker spell and Vex checkpoint

- Added a direct real-1.11.2 Java oracle for Evoker attack, summon, and Wololo
  task boundaries. Native output matches all 16 far-attack fang coordinates
  and warmups by raw bits, fang damage at tick 8 and retirement at tick 23,
  three seeded Vex positions/bounds/lifetimes, and blue-to-red sheep conversion.
- The live scheduler owns vanilla spell priority, warmup, casting, and cooldown
  counters. Attack fangs search downward for support, exclude their caster and
  Illagers, publish status/audio, damage at the exact boundary, and retire.
  Summoned Vexes retain owner, bound, limited lifespan, starvation pulses,
  no-clip/no-gravity flight, charge attacks, and bounded random movement.
- Added exact owned Vex idle/charging and fang textures, Vex and fang models,
  Evoker casting arms, Vindicator aggressive arms and axe, Vex sword/wing
  poses, plus the represented 1.11.2 spell, fang, Vex, and illager sound set.
- The direct Java/native comparator, automatic scheduler/view regression,
  entity geometry/UV/winding, audio manifest, mansion, mob, item-render, and
  runtime gates pass. The automatic regression uses five accelerated
  state-boundary ticks instead of 180 idle ticks while retaining every branch.
  The quick pyramid passes every locally runnable stage, with only the two
  undistributed `.bsnp` stages skipped.
  The stopped-oracle CPU guard passes at 4,566 steps/s against the 4,062
  baseline and 3,858.9 floor. GPU 1 was not used.
- Honest effort-weighted parity is approximately 87%. Ocean monuments and
  guardians are the next large generation/entity bundle. General task/NBT
  continuation, remaining mob families, exhaustive rail/redstone edges, and
  global pixel equivalence remain open.

## 2026-08-10 Guardian and ocean-monument checkpoint

- Added distinct Guardian and Elder Guardian live types with exact dimensions,
  health, movement/attack attributes, 80/60-tick laser damage boundaries,
  stationary thorns, mining-fatigue delivery, water/land audio, and seeded
  loot. The direct Java/native attribute, combat, and loot comparator passes.
- Added animated Guardian and Elder geometry, skins, spikes, tail and eye
  steering, plus the textured laser beam in both capture and window-compose
  paths. Geometry, UV/winding, texture ownership, audio ownership, and runtime
  regressions pass; this is not yet a global Java pixel-tape claim.
- Ported the complete 1.11.2 ocean-monument room graph and piece placement,
  including all four facings, chunk clips, shell/interior variants, sea
  lanterns, treasure core, sponge rooms, foundation, water clearing, and three
  elder sites. Five real-Java/native fixtures match candidate chunks, complete
  volume hashes/censuses, and elder positions exactly.
- Wired bounded nearby candidate discovery and idempotent live placement. The
  seed-0 runtime fixture finds `(-75,-368)`, places 15,678 prismarine, 126 sea
  lanterns and eight gold blocks, and retains exactly three persistent elders.
  ASAN found and drove closure of the generator's temporary room-graph leaks;
  the final five-fixture run is clean with leak detection enabled.
- The focused runtime, Guardian/monument gates, Java 8 build, and all thirteen
  locally runnable quick-sweep stages pass. Only the two undistributed `.bsnp`
  stages skip. CPU throughput is 4,438 steps/s against the 4,062 baseline and
  3,858.9 floor. GPU 1 was not used while shared.
- The broad aggregate passes every new gate and all unchanged gates through
  `dragon_live`. Its final survival route reproduces at the pre-bundle
  `35aa47c` commit: the harness guesses a gravel drop from coordinates while
  the live drop follows the World.rand cursor, then mistakes the valid
  iron-to-nuggets recipe for flint-and-steel crafting. This is retained as a
  known pre-existing harness failure, not hidden as a product pass.
- Honest effort-weighted parity is approximately 89%, with high uncertainty.
  Ordinary Guardian natural-spawn/loaded ordering, monument locate and
  structure persistence, general task/entity NBT continuation, villager AI,
  remaining mob families, exhaustive rail/redstone edges, and global pixel
  equivalence remain open.

## 2026-08-10 (fresh-object NBT continuation and legal survival route)

- Added a real Java save/load fence for parked NoAI mobs: write atomic NBT,
  remove the old entity, construct a fresh object through `EntityList`, retain
  its ID/order, then capture after the fresh-tick fence.
- Promoted all 23 plain native living classes through Java NBT, capsule import,
  and 20 exact native ticks. The comparator includes raw AABBs, private RNG,
  sheep/chicken fields, and Slime/Magma size and squish history. Specialized
  exact schemas continue to cover pig, villager, wolf, and ocelot.
- Expanded the direct source-derived NoAI comparator to 19 classes and closed
  Ghast drag, Witch self-potion RNG, and Slime/Magma null-ambient ordering.
- Replaced the route's coordinate-derived fake flint with actual live gravel
  drop attempts and retained a completely legal represented survival path.
- The focused gates and quick pyramid pass. CPU throughput is 4,035 steps/s
  against the frozen 3,858.9 floor and 4,062 baseline under shared-host load;
  no hot allocation or scan was added and GPU 1 was not used.
- A clean full `make -C magma test-game` passes through the legal fresh-spawn-
  to-credits route in 10:15.22 at 1,101,200 KiB peak RSS with zero swap.
- Honest effort-weighted parity is approximately 95%, with high uncertainty.
  Arbitrary task/path continuation, villager AI, unsupported Java mob species,
  Guardian pack/load ordering, exhaustive rail/redstone edges, custom NBT, and
  global pixels remain open.

## 2026-08-14 Glowing outline checkpoint

- Promoted active effect 24 and saved glowing flags into represented live
  render views, including invisible entities restored only for the outline.
- Implemented the real-client entity mask, two Sobel passes, and final
  source-alpha composition in capture and interactive renderers. Living-model
  culling is disabled for this pass, and Slime alone emits its gel layer.
- Added byte-stable Java A/B Magma and Slime control/Glowing fixtures. Magma's
  final affected pixels are channel-exact. Slime's exact outline bounds and
  classified fixed-function shading-offset tail are mutation-gated.
- The dedicated oracle gate, focused render/overlay/Armor Stand tests, and full
  `make -C magma test-game` aggregate pass through credits. CPU throughput is
  3,961 steps/s against the 3,858.9 floor and 4,062 baseline. GPU 1 was not
  used.

## 2026-08-14 Luck loot-context checkpoint

- Traced every 1.11.2 `getLuck`, `withLuck`, and deferred-loot call from the
  local oracle source. Single chests pass the player; double chests first touch
  each half through a null-player slot read, which native now preserves.
- Generalized the native table fill boundary to accept `LootContext` and wired
  live player Luck into single generated-chest opening. Null access,
  automation, and unopened-container break continue to use zero Luck.
- Added a direct Java/native synthetic oracle that exercises nonzero entry
  quality and bonus rolls through generated lists and the final shuffled
  27-slot inventory over 64 seeds. A Java reflection census checks all 81
  built-in tables and confirms that the 80 non-fishing rows are Luck-invariant.
- The new gate and existing generated-chest, unopened-break, and automation
  regressions pass. The change adds no tick scan or allocation; deferred fill
  remains one-shot. GPU 1 was not used.

## 2026-08-14 dispenser/dropper/hopper playable containers

- Added the exact real-jar `dispenser.png` and `hopper.png` panels to the
  generated GUI atlas. Dispenser/dropper use Java's 3x3 tile grid, player-main
  row at y=84 and hotbar at y=142; hopper uses its five-slot row, y=51 main
  inventory, y=109 hotbar, and 133-pixel panel height. Live titles distinguish
  Dispenser, Dropper, and Item Hopper.
- Connected the existing nine-slot dispenser/dropper and five-slot hopper
  inventories to the shared container click path. Pickup, half-pickup, deposit,
  merge, quick-move, throw, distance close, rich ItemStack state, comparator
  notification, and native checkpoint continuation are live. Opening a jungle
  temple's deferred dispenser passes player Luck; automation retains the
  required null-player, zero-Luck boundary.
- Added `qrl.StaticContainerGolden`, built from the actual 1.11.2
  `ContainerDispenser` and `ContainerHopper`. The direct Java/native gate
  compares every slot coordinate and five discriminating click-state rows for
  both layouts. It also verifies an open checkpoint/reload and forced distance
  close.
- The new notification assertion found that hopper, brewing stand, and Shulker
  Box fullness calculations existed but were rejected by comparator override
  admission because the blocks are not normal cubes. The backend now admits
  all three, and the focused gate retains a click case for each family.
- Focused container, 448-case brewing, hopper automation, Shulker Box runtime,
  chest loot, Luck loot, and screen tests pass. The full game aggregate passes
  through credits in 14:47.68 at 2,341,936 KiB peak RSS, with zero major faults
  and zero swap. CPU throughput is 4,059 steps/s against the 4,062 baseline and
  3,858.9 floor. The implementation adds no per-tick scan or idle allocation;
  GPU 1 was not used.
- Added deterministic populated real-client fixtures for Dispenser, Dropper,
  and Item Hopper. Every Java A/B pixel is identical, and native matches all
  116,792, 116,792, and 93,560 owned pixels respectively with maximum channel
  delta zero. The dedicated exact mask gate includes a negative mutation
  control.

## 2026-08-14 Furnace/Brewing populated screen checkpoint

- Added deterministic populated fixtures built from the real 1.11.2
  `GuiFurnace` and `GuiBrewingStand`. The recorder exposes the live tile fields,
  and capture refuses Furnace values other than `[800,1600,100,200]` or Brewing
  Stand values other than `[200,10]`.
- Added the two native candidate states and a strict owned-panel mask with a
  negative mutation control. Both Java A/B pairs are byte-identical. Native
  matches all 116,792 owned pixels for both screens, with maximum channel delta
  zero.
- The first strict comparison exposed that setting Furnace fields before its
  inventory reset erased the oracle arrow state. It also isolated the Brewing
  Stand residual to Java's separately tinted `potion_overlay` layer. The
  fixture order is corrected, the generated atlas includes that layer, and
  native uses Java's exact rounded texture modulation.
- `test_screen`, all 1,452 furnace cases, all 448 brewing cases, and the prior
  dispenser/dropper/hopper zero-delta gate pass. CPU throughput is 4,015
  steps/s against the 4,062 baseline and 3,858.9 floor. No idle scan or
  allocation was added; GPU 1 was not used.

## 2026-08-14 Crafting/Anvil/Merchant populated screens

- Added deterministic real-client fixtures for actual `GuiCrafting`,
  `GuiRepair`, and `GuiMerchant` containers. The recorder exposes strict
  crafting-slot, anvil cost/name/result, and merchant selection/recipe/result
  state, so a visually plausible but behaviorally false frame cannot pass.
- The Crafting Table computes two Blaze Powder from a three-count Blaze Rod
  stack. The Villager has two actual `MerchantRecipe` rows and populated
  Emerald/Diamond inputs that compute its Blaze Rod output. Both Java A/B
  pairs and both native frames were zero-delta on the first strict run.
- The first Anvil run found 8,332 wrong owned pixels. Java's active 110x16
  name-field slice and invalid-result overlay were absent, the cost used one
  generic shadow instead of four explicit font passes, and native drew an
  Inventory label that `GuiRepair` does not. After those fixes the remaining
  1,256 pixels isolated the exact label as `Enchantment Cost: 1`, not
  `Cost: 1`. The final Anvil frame is zero-delta.
- All three screens now match all 116,792 owned pixels against byte-exact Java
  A/B controls. Their common mutation-tested lane is
  `ENTITY_GATE_STANDARD_CONTAINER_GUI_ONLY=1`. Screen, container, Anvil live,
  and full Villager trade behavior pass; prior processing/static screen lanes
  remain zero-delta. CPU throughput is 4,068 steps/s against the 4,062
  baseline and 3,858.9 floor. No tick scan or idle allocation was added, and
  GPU 1 was not used.

## 2026-08-14 Enchanting populated screen

- Added a deterministic real-client fixture using the actual
  `GuiEnchantment` and `ContainerEnchantment`. It asserts the item and lapis
  stacks, seed-zero 15-bookcase offer levels `[8,13,30]`, clue IDs
  `[34,17,21]`, clue levels `[1,2,2]`, and exact generated runic line layout.
- Added the complete Enchanting panel, option states, icons, Standard Galactic
  Alphabet glyphs, and book texture to the generated atlas. Native renders the
  real seven-part `ModelBook` with Java's transforms, perspective, texture,
  depth interaction, and measured llvmpipe fixed-function lighting response.
- The first golden was stable at the state-query boundary but not at the actual
  framebuffer boundary: the free-running client advanced the book before
  `frame_pair`. The recorder now re-pins it inside that action and reports the
  captured open/flip values. This removed a false geometry target. The real
  panel's depth value of 0.5 then explained the final three coverage pixels.
- Java A/B is byte-identical. Native matches all 116,792 owned pixels exactly,
  and the combined Crafting/Anvil/Merchant/Enchanting lane remains zero-delta.
  Enchanting live and general screen behavior pass. CPU throughput is 4,056
  steps/s against the 4,062 baseline and 3,858.9 floor, with no tick-path scan
  or allocation added. GPU 1 was not used.

## 2026-08-14 Llama carpet item-model correction

- The tightened horse-family GUI run caught a stale two-pixel budget overrun
  in the llama panel. Pixel isolation showed the error was not the 3D llama
  preview: item 171 rendered all six carpet faces with the full wool texture.
  The real `block/carpet.json` maps each one-pixel side wall to rows 15..16.
- Native now uses those explicit side-wall UVs. The llama panel improves from
  642 differing pixels, 32 above channel delta 1, one above 25, and maximum 26
  to 622, four, zero, and 10 respectively. The strict budget was tightened to
  those measured values. Java A/B remains byte-identical; ordinary Horse and
  chested Donkey results are unchanged.

## 2026-08-14 Complete furnace recipe and fuel registries

- Replaced the 17-recipe subset and obsolete shifted item-id shim with the
  exact 51 rows exported from the initialized Java 1.11.2 game. The native
  table and constant-time live lookup match every row, including exact-meta
  sponge, stone-brick, and fish recipes plus iron/gold equipment recycling.
- Added a read-only Java fuel export and checked manifest for every one of the
  392 initialized item rows. Native matches metadata 0, 1, and 15 for every
  row: 65 fuels across all ten burn durations. The gate also locks negative
  items and mutation-tests both the table and hot lookup comparators.
- Removed the 51-row recipe array and per-tick table construction from each
  scalar/CUDA furnace kernel. Live insertion now uses the existing complete
  item stack-limit census. Lava fuel returns its bucket and a wet sponge with
  a bucket in the fuel slot produces the water bucket exactly as Java does.
- `test_furnace_live` passes 1,470 checks. Six fuel seed classes each match
  Java over 2,000 tick fields; recipe and full-tick CPU/CUDA outputs are
  bit-identical; the Furnace/Brewing screen lane remains zero-delta. CPU
  throughput is 4,221 steps/s against the 4,062 baseline and 3,858.9 floor.

## 2026-08-15 Furnace output XP

- Extended the read-only real-client furnace census from fuel values to
  `FurnaceRecipes.getSmeltingExperience` for every one of 392 registered items
  and all 16 metadata values. The first strict run caught and corrected the
  stone-brick XP key: only cracked metadata 2 earns 0.1 XP.
- Player pickup, throw, quick-move, scripted extraction, and the Magma RL
  extraction primitive now pass output takes through `SlotFurnaceOutput`'s
  fractional float calculation and stochastic rounding. The shared Math RNG,
  entity seed generator, server UUID cursor, logical entity IDs, split values,
  and loaded append order feed the existing exact XP-orb constructor path.
  Hopper extraction remains a direct inventory operation and awards no XP.
- The focused container regression covers both fractional branches, exact RNG
  cursor endpoints, multiple split orbs, and the no-XP/no-RNG branch. The
  392-row fuel/XP census, 51-row recipe census, all 1,470 furnace assertions,
  container integration, hopper automation, full route, and long runtime tests
  pass. CPU throughput is 4,123 steps/s against the 4,062 baseline and 3,858.9
  floor; output XP adds no idle-tick work.

## 2026-08-15 Furnace names and rich break payloads

- Furnace tiles now retain a custom name from named-item placement and use it
  as the centered GUI title. Java authoritative capture, exact capsule schema,
  Anvil `CustomName` admission, native state export, checkpoints, and Structure
  template save/load all carry the same value.
- An isolated real 1.11.2 save was staged with `CustomName = Oracle Furnace`.
  The fail-closed Anvil importer reported zero limitations and native restore
  reproduced the name and all four furnace timers. The capsule selftest, Java
  build, player placement test, container test, and exact checkpoint fork pass.
- Furnace destruction now drops the full represented `ICStack`, retaining
  metadata, repair cost, display-name handle, arbitrary tag handle, and all
  represented enchantments.

## 2026-08-15 Exact InventoryHelper drops and continuation

- Ported the actual 1.11.2 `InventoryHelper.spawnItemStack` draw sequence:
  three shared float offsets, `nextInt(21)+10` splits, four EntityItem
  constructor `Math.random` draws, and three Gaussian motion values. Block
  inventory drops preserve rich stack payloads, pickup/age/first-update state,
  UUID construction, entity IDs, and loaded-entity append order. Minecart
  block/cart drops remain on their distinct `Entity.entityDropItem` path.
- Added a direct Java/native raw-bit oracle for empty and preloaded Gaussian
  caches. It locks every split count, position and motion component, Math and
  InventoryHelper terminal cursors, and the 27-slot count-64 capacity case.
  All 1,728 items survive the strict maximum of 189 spawned entities.
- Captured the helper's private static seed and Gaussian cache through the
  authoritative recorder and isolated server shadow. Hidden Java state,
  capsules, native state output, and native checkpoints now restore it. A real
  isolated save imported with zero limitations and replayed the same native
  seed/cache values.
- Expanded the live item table to 256 entries. Matched 200,000-tick headless
  runs both completed in 3.80 seconds at 33,120/33,124 KiB RSS; CPU throughput
  remains above its strict floor at 4,014 steps/s. The direct oracle,
  container, static-container, minecart, script-route, Java build, capsule,
  Anvil, and checkpoint gates pass.

## 2026-08-15 Furnace output events, statistics, and save boundaries

- Added a direct real `SlotFurnaceOutput` comparator. Native matches the exact
  craft-stat, XP-orb construction, Forge smelt-event, and achievement-attempt
  order for iron with and without its prerequisite, cooked fish, and stone.
  `acquireIron` and `cookFish` persist only when `buildFurnace` is already set.
- Generalized imported Java statistics handling to the three furnace
  achievements and all 392 per-item craft counters. Existing values are
  replaced in source order, new values append deterministically, and unknown
  nested JSON stays byte-identical. Checkpoints retain counters and the bounded
  rich smelt-event stream.
- Added eight direct real `TileEntityFurnace` NBT-reload pairs. Ignition, burn
  1 refueling, completion, blocked output, idle cooling, invalid-input reset,
  extinguishing, wet-sponge bucket conversion, lit block state, and the
  unsaved current-burn reconstruction all match one tick later.
- Controlled input now captures the InventoryHelper seed and Gaussian cache on
  both sides of every edit. Equal starts require exact cursor equality; ambient
  starts compare exact transition counts. The negative cache mutation is
  retained in the script gate.
- All focused neighboring gates and Java build pass. The CPU guard is 4,028
  steps/s against the frozen 3,858.9 floor, with no new idle scan or allocation.

## 2026-08-15 Container.slotClick promotion

- Promoted the one shipped container path to all seven 1.11.2 ClickTypes over
  player, crafting, furnace, chest-family, brewing, enchanting, anvil,
  merchant, horse, beacon, armor, and offhand slots. The direct actual-Java
  comparator now passes 149 rows, including a generated 98-state ordinary
  PICKUP matrix and discriminating rich-NBT, output-slot, cap, and event cases.
- Added exact furnace and brewing output `onTake` behavior, ordered rich event
  records, prerequisite-gated achievements/statistics, unknown-stat JSON
  preservation, and checkpoint continuation. Take-only slots now reproduce
  Java's subtype-aware loose-metadata merge rule.
- Matched the windowed client's drag, ordinary double-click, and
  shift-double-click sequencing. The latter performs Java's second QUICK_MOVE
  on press, then scans takeable matching stacks from the clicked slot's backing
  inventory on release in `Container.inventorySlots` order.
- Closing or switching containers now tosses the cursor and transient inputs in
  Java order with exact forward drop motion. The focused container, screen,
  brewing, enchanting, furnace, merchant, script, static-container, and native
  save/checkpoint gates pass. The large container regression fixtures now use
  static/heap runtime storage instead of consuming 8,334,720 bytes of an 8 MiB
  process stack.
- CPU throughput passes at 3,935 steps/s against the frozen 3,858.9 floor while
  the host load average was above 77. The benchmarked Blaze kernel is unchanged;
  container additions add no idle simulation scan or allocation.
## 2026-08-16: Skeleton-trap cull-disabled render parity

- The horse-family pixel gate isolated a real model-ownership error in the
  skeleton trap rather than absorbing it into the fixed-function budget.
  `RenderLivingBase` disables `GL_CULL_FACE` for both the base model and armor
  layers; native skeletons and their iron helmets had retained one-sided
  geometry. Emitting both windings removes the missing rear helmet and body
  faces visible through transparent texels.
- The isolated rider's hard residual fell from 538 to 77 pixels and the
  four-rider scene from 3,574 to 2,989. Ordinary horse states did not regress.
  The gate ceilings and mutation control now lock those smaller measured
  values. `test_entity_render.sh` also verifies the cull-disabled skeleton and
  helmet vertex capacities.

## 2026-08-16: Live caravan block-update replanning

- Added a bounded ordered gameplay block-mutation journal to the live world.
  Llama navigators consume it independently and mirror 1.11.2
  `PathWorldListener`: collision-box equality, the final-path midpoint distance
  test, and `PathNavigate`'s 20-tick deferred-update clock. Snapshot bulk loads
  stay outside the journal, and mutation-free ticks do no scan or allocation.
- Promoted an eight-tick real-Java/native dynamic terrain fixture. A distant
  stone insertion retains the materialized path; opening the upper center of
  the nearby wall replaces it. Path points, index, position, velocity, and
  on-ground state match through the post-edit continuation.
- The new one-block-rise path exposed an inherited physics constant: every
  `AbstractHorse` has step height 1.0, while native only applied 1.0 to ridden
  horses and also ran a generic 0.6-height jump heuristic. The shared horse
  step height and heuristic boundary now match Java. Existing llama runtime
  tests and all 28 prior real-Java caravan ticks remain green.

## 2026-08-16: Follow-parent and probabilistic llama goals

- Implemented `EntityAIFollowParent` with its zero mutex, nearest-adult search,
  3-to-16-block continuation window, ten-tick request clock, and child-sized
  navigator. The native navigator now distinguishes a fresh XYZ request from
  a persistent path: an airborne XYZ request clears the path, while an
  existing path remains active and continues feeding `MoveHelper`, as Java
  does.
- A 16-tick real-Java/native child fixture crosses the first airborne boundary
  and the first navigator refresh. Every materialized path point and index,
  position, velocity, task mask, and entity `java.util.Random` cursor matches.
  Four direct conflict rows also lock caravan suppression of ranged and mating
  goals plus concurrent swimming and follow-parent execution.
- Llama wander, watch-player, and idle-look setup now consume the real entity
  RNG. Three accepted-start fixtures include the preceding ambient, horse-tail,
  and wolf-defense draws; task masks, terminal cursors, and all six motion
  doubles match the real 1.11.2 client. The completeness gate now records
  twelve direct AI boundary rows and narrows the remaining AI work to panic,
  mating, and ranged navigator edge conflicts.

## 2026-08-16: Llama panic and mating navigation

- Replaced the llama panic shortcut with the exact nearest-water search and
  random-position fallback, then routed both through the shared A* navigator
  at speed 1.2. The passive scheduler now preserves panic's priority and
  mutex behavior without consuming target-selection RNG on the water branch.
- Routed the active llama mating task through the same navigator at speed 1.0.
  A direct fixture exposed a distinct Java lifecycle boundary:
  `tryMoveToEntityLiving` returns early while airborne and retains the old
  path, whereas `tryMoveToXYZ` reaches `setPath(null)` and clears it. Native
  now represents that request kind explicitly.
- Two 12-tick real-Java panic scenarios and one 16-tick mating scenario match
  every materialized path point and index, task mask, entity RNG cursor,
  on-ground flag, position, and velocity bit-for-bit. Reproducing the last
  motion bits also required Java's `MathHelper.atan2` turn and persistent
  horse AABB accumulation. All prior caravan, follow-parent, conflict, and
  lower-task rows remain green.

## 2026-08-16: Llama ranged navigator closure

- Routed the ordinary llama ranged goal through the shared exact A* navigator
  at speed 1.25. Entity-target requests retain the current path while
  airborne, and twenty consecutive visible ticks clear both the path and its
  cached navigation state before the attack clock advances.
- Added a 41-tick direct real-Java/native fixture. A naturally acquired wild
  wolf and a forced floor-gap detour exercise target selection, path
  materialization, airborne continuation, the visibility stop, and the first
  spit. Path nodes and index, task mask, attack and visibility clocks, target,
  spit flag, on-ground state, all six motion values, and entity RNG match at
  every tick.
- The trace also exposed a separate ambient boundary: `EntityLlama` overrides
  `getAmbientSound` without calling `AbstractHorse`, so it consumes only the
  two sound-pitch floats and not the horse mouth/rearing roll. Native now
  preserves that concrete-class distinction. Prior llama fixtures remain
  green.

## 2026-08-16: Active Bat AI, lifetime, and continuation

- Ported `EntityBat`'s hanging and flight state machine, including normal-cube
  ceiling support, survival versus creative player wake, world event 1025,
  private target invalidation and retargeting, motion, yaw, look/body helpers,
  hanging anchoring, flight damping, and exact entity RNG order.
- Added the exact `EntityLiving` lifetime boundary for active Bats. Entity age,
  persistence reset, the strict 32-block reset, strict 128-block hard removal,
  and the ordered 1-in-800 soft lottery now match Java. The lottery remains
  after the ambient `nextInt(1000)` draw, including the near-player branch
  that consumes the draw before resetting age.
- The authoritative recorder, script restore, state capsule, native state,
  and checkpoint path carry Bat age, persistence, target, body-helper, AABB,
  and RNG state. A direct real 1.11.2 comparator passes 101 exact ticks across
  eleven hanging, flight, retarget, soft/hard despawn, and persistence cases.
  A warmed Java capsule continues for 16 exact native ticks after a nine-tick
  checkpoint; the native checkpoint fork is exact through twelve ticks.
- The full native aggregate exposed two independent harness issues while this
  slice was promoted. Merchant close now correctly tests Java's dropped input
  rather than the obsolete inventory-refund expectation. Chest-loot and broad
  mob fixtures no longer place their expanded 8+ MiB runtimes on the process
  stack, and the player-preview target links its enchanting-book texture
  provider explicitly. Their focused gates and the aggregate tail pass.
- CPU throughput passes at 3,923 steps/s against the 4,062 baseline and
  3,858.9 floor. The Bat work adds no allocation or global entity scan.

## 2026-08-16: Bat render promotion

- Replaced the flattened Bat approximation with the exact nine-box
  `ModelBat` hierarchy. The native path now carries Java's parent-child
  transforms, flying wing phase, hanging head/body pose, `RenderBat` scale
  and offsets, two-sided faces, and the singleton model's retained wing-X
  state.
- The recorder and tape bridge now carry the Bat hanging bit and exact
  `ticksExisted`. Three pinned real-Java states cover flying, hanging, and a
  subject-free background, with byte-identical Java A/B captures and a
  mutation-tested native subject mask.
- Flying has identical 1,064-pixel ownership and only 55 one-channel-value
  residual pixels. Hanging has identical 852-pixel ownership; one pixel is
  above max-channel 25, measured at the body-back/tail-top 24-bit depth tie.
  That single fixed-function boundary sample has an explicit one-pixel
  budget. All other hanging residuals are at or below the hard threshold.
- The focused mesh, live-view, tape-parser, pixel, and Bat completeness gates
  pass. Bat's remaining family-level gap is natural world-spawner pack and
  eligibility ordering, not active behavior, persistence, or model pose.
- The complete native aggregate passes in 15:14.08 at 2,347,260 KiB peak RSS,
  zero major faults, and zero swap. The broad completeness registry and final
  route-to-credits test pass, and CPU throughput remains 3,923 steps/s against
  the frozen 3,858.9 floor.

## 2026-08-16: Active Squid and render promotion

- Ported the active `EntitySquid` random-swim task, exact Java RNG order,
  water and dry motion, rotation, pitch/yaw/tentacle animation, cycle refresh,
  age stop, persistence, base living tick, and AABB evolution. The native
  `atan2` path uses Mojang's exact `FRAC_BIAS` bit constant; replacing it with
  the visually similar numeric value had caused an out-of-range table index.
- Five direct fixtures pass 76 bit-exact real-Java ticks, including motion,
  private animation fields, render yaw, AABB, age, persistence, and RNG. A
  native twelve-tick save fork and a warmed Java capsule followed by sixteen
  native ticks both continue exactly.
- Replaced the static Squid render pose with the complete `RenderSquid`
  transform chain and live tentacle hinges. A dedicated Java render pin
  freezes every interpolation endpoint at `frame_pair`, and the native live
  view carries the same four pose inputs without conflating them with the
  generic living head state.
- The swimming and dry same-scene Java A/B pairs have zero oracle noise. Dry
  has no hard residual and maximum channel delta 1. Swimming has one measured
  internal-tentacle fixed-function depth tie above channel 25, maximum delta
  32, under a one-pixel budget with a negative control. Squid is promoted to
  `live_bounded`; natural world-spawner pack/eligibility ordering remains.
- The broad completeness gate and the full native aggregate pass. The latter
  reaches fresh-spawn-to-credits in 16:11.06 at 2,347,268 KiB peak RSS, one
  major fault, and zero swap. The top-level aggregate now invokes the focused
  Squid runtime directly as well; its runtime and route tail pass after that
  wiring change. CPU throughput is 3,945 steps/s against the frozen 3,858.9
  floor.

## 2026-08-17: Snow Golem live-bounded promotion

- Implemented Forge-compatible pumpkin shearing, exact tool/Unbreaking RNG,
  active hostile selection, unseen-target memory, ranged stop/cooldown logic,
  and Snow Golem-owned snowball launch, collision, and impact routing.
- Direct real-Java gates pass three shearing cases, three exact ranged
  launches, five owned-impact cases inside the fifteen-case snowball matrix,
  and fifteen Snow Golem loot rows inside the 345-row hostile aggregate.
- Runtime save/load now resumes both a pending shear interaction and an
  in-flight owned snowball exactly. The first active shot occurs on Java's
  tick-21 boundary.
- Added wet and hot-biome damage, mobGriefing-guarded snow trails, complete
  Snow Golem audio, the exact five-box model, conditional baked pumpkin layer,
  jar skin, and generated pumpkin atlas texture. Model and atlas gates pass;
  same-scene Java/native pixels remain an explicit open item.
- Added a checked family manifest and promoted EntitySnowman from
  `live_partial` to `live_bounded`. The entity registry is now 63 bounded and
  18 partial. CPU throughput is 3,890 steps/s against the frozen 3,858.9
  floor, with the trajectory hash unchanged and no GPU-path modification.

## 2026-08-17: Endermite live-bounded promotion

- Promoted Endermite only after direct real-Java gates covered the Ender Pearl
  spawn branch, fifteen empty-loot rows, nine composed lethal hits, and a
  20-tick Java/NBT/capsule/native continuation of lifetime, player origin,
  persistence, RNG, and update order.
- Corrected the terminal death tail from the generic hostile five XP to
  Endermite's exact three XP. The existing hostile terminal regression now
  includes Endermite and locks split-orb construction, first tick, retirement
  order, entity RNG, and all twenty death particles.
- Added inherited hostile fall handling, ambient/hurt/death/step resources,
  exact jar texture, four-box `ModelEnderMite` animation, dimensions, eye
  height, and 180-degree death rotation. Step resources are present, while
  type-specific locomotion event production remains explicitly open.
- Added a checked family manifest and focused native save regression for the
  exact lifetime 2399/2400 boundary. The entity registry is now 64 bounded
  and 17 partial; natural spawning, Enderman retaliation, full active AI,
  portal particles, and same-scene pixels remain under `AI-01`.
- A clean rebuild exposed two harness assumptions. The completeness gate now
  builds `magma_game` before chunk-bundle checks, and the audio test stores
  its three large runtime fixtures statically instead of overflowing the
  8 MiB process stack. The clean broad gate passes, and CPU throughput is
  4,089 steps/s against the unchanged 3,858.9 floor with no GPU changes.

## 2026-08-21: Spectral Arrow live-bounded promotion

- Audited the only projectile registry class still marked partial against the
  already-implemented runtime and direct oracle suite. Eight Java/native
  payload cases pass, including configured Glowing duration, while the live
  test covers launch/ammo, impacts, pickup, render-state handoff, and native
  checkpoint continuation.
- The real-Java neutral capsule includes normal, tipped, and spectral arrows
  together and continues every motion, collision, payload, UUID, RNG, pickup,
  and semantic NBT field exactly for the next native tick.
- Added a checked family manifest and promoted EntitySpectralArrow from
  `live_partial` to `live_bounded`. The registry is now 65 bounded and 16
  partial; mixed collision ordering, foreign-dimension terminal delivery,
  and strict projectile pixels remain open under `ENT-06`.

## 2026-08-21: Giant live-bounded promotion

- Audited `EntityGiantZombie` against the 1.11.2 source and found the major
  divergence was invented generic hostile AI: Java installs neither tasks
  nor target tasks. The native active path now idles without targeting,
  wandering, navigating, or attacking while retaining shared living updates.
- Added Giant to the real-Java loot and composed-death matrices. All 345 loot
  rows and 243 death rows pass exactly, including its empty loot table, no
  hurt/death sound events, private RNG cursor, and five-XP terminal path.
- Added a 40-tick active native test with a tick-20 save fork. Uninterrupted
  and restored states match exactly. The scale-six zombie model, dimensions,
  eye height, and mapping are now explicit entity-render assertions.
- Added a checked family manifest and promoted `EntityGiantZombie` from
  `live_partial` to `live_bounded`. The registry now contains 66 bounded and
  15 partial entities.

## 2026-08-21: Chest and Furnace Minecart live-bounded promotion

- Re-ran the real 1.11.2 minecart golden across all rail shapes, powered and
  activator callbacks, collisions, riders, damage, inventory destruction,
  and Furnace fuel/push/smoke behavior. The complete comparison passes.
- Re-ran the direct Structure oracle, which preserves all seven concrete cart
  classes, subtype payloads, constructor RNG, fresh UUID allocation, and
  transforms. Chest arbitrary tagged inventory and Furnace fuel/push state
  also cross capsule and checkpoint boundaries.
- Added a checked variant-family manifest and promoted Chest and Furnace
  Minecarts to `live_bounded`. Spawner and Command Minecarts deliberately
  remain partial for arbitrary spawn EntityTag families and general command
  dispatch. The registry now contains 68 bounded and 13 partial entities.

## 2026-08-21: Husk live-bounded promotion

- Corrected the inherited Zombie daylight branch so Husk remains unlit while
  Zombie, Skeleton, and Stray controls ignite. Added its exact ambient, hurt,
  death, and step assets and distinct sound-event routing.
- Added empty-hand Hunger melee with the Java Normal-difficulty world-age and
  moon cast, including both 140- and 280-tick oracle boundaries, the delayed
  first 0.005 exhaustion pulse, exported potion state, and native save-fork
  continuation.
- Added Husk to the exact loot and composed-death matrices and to the native
  terminal suite, now 23 attack/death families and 460 terminal particles.
  All 345 loot rows and 243 death rows remain exact against real 1.11.2.
- Added a checked family manifest and promoted `EntityHusk` from
  `live_partial` to `live_bounded`. The registry now contains 69 bounded and
  12 partial entities; the remaining bounded residuals are recorded in the
  family manifest.

## 2026-08-21: Stray live-bounded promotion

- Added Stray to the active Skeleton ranged path while preserving its exact
  40-tick attack cadence, daylight burning, dimensions, eye height, skin, and
  four owned sound resources.
- A parked real 1.11.2 reflection oracle proves that `EntityStray.getArrow`
  creates one visible, non-ambient Slowness-I custom effect with duration 600.
  Native launch, impact application, and in-flight save continuation pass.
- Added all fifteen Stray loot rows and nine composed lethal-hit rows. The
  345-row loot and 243-row death matrices match real Java exactly, including
  player-only tipped-arrow loot with canonical slowness potion NBT. The native
  terminal suite now covers 23 families and 460 death particles.
- Native save/reload retains both the flying custom projectile and the tagged
  tipped-arrow item. A checked family manifest promotes `EntityStray` to
  `live_bounded`; the registry now contains 70 bounded and 11 partial rows.

## 2026-08-21: Equid and llama ledger correction

- Re-audited all five equid rows and both llama rows against the registry's
  `live_bounded` definition. Their checked manifests already cover playable
  behavior, persistence, direct Java state/event oracles, containers, audio,
  particles, and bounded model/UI pixels.
- Their remaining items are measured fixed-function pixel tails or terminal
  details explicitly owned by other family TODOs, not absent major behavior
  families. Promoted the seven stale rows without removing those residuals.
  The entity registry is now 77 bounded and 4 partial.

## 2026-08-22: Rabbit, Polar Bear, and entity-registry closure

- Added a private real-1.11.2 Rabbit probe for ordinary and Killer Bunny
  combat, armor, jump thresholds, jump duration, and saved RabbitType and
  MoreCarrotTicks. Native now implements hopping, Killer Bunny targeting,
  carrot raids, breeding input/type state, exact loot/death, audio, all coat
  variants, child pose, and native save continuation.
- Rabbit expands the exact Java/native aggregates to 375 loot rows and 261
  composed deaths. The native terminal suite now covers 25 families and 500
  death particles. Added a checked Rabbit family boundary and refreshed every
  family gate that deliberately pins those aggregate counts.
- Re-audited Spawner and Command Minecarts. Their direct Java/native golden
  already locks spawner activation/countdown/reset RNG and command activator
  cooldown, payload, built-in Searge output, plus checkpoint continuation.
  Structure, capsule, rendering, movement, collisions, and destruction cover
  the shared cart lifecycle. General commands and arbitrary unsupported spawn
  tags remain owned by their broader MODE-02 and WORLD-06 rows.
- Promoted Rabbit and the last two minecart rows. The conservative registry
  gate now reports all 81 Java registry entities as `live_bounded`.
# 2026-08-22: strict gap audit and exact boat status boundary

- Added `verify/completeness/gap_audit.py` and a checked immediate-priority
  manifest. The gate now distinguishes exhaustive registry identity coverage
  from strict row closure: 81/81 entities and 24/24 tiles have live-bounded
  identities, while 20/92 strict rows are closed.
- Promoted the first `ENT-08` slice through a direct real-Java/native oracle.
  Six cases cover air, land, surface water, submerged source/flowing water,
  and the air-to-water transition with raw private and public numeric bits.
- The comparison exposed four real native differences: C `DBL_MIN` was not
  Java `Double.MIN_VALUE`, the transition branch damped motion, direct center
  addition skipped Java's AABB recentering, and boat fall distance did not
  advance. All four are corrected and the focused oracle is byte-exact.
- Extended that oracle with four remote-client rider cases: forward,
  turn-only, reverse-turn, and forward-turn. Native now matches raw Java bits
  for control thrust, yaw and delta rotation, both paddle states and phases,
  boat pose/motion, and the passenger pose after movement.
- Added the server-side submerged boundary: the rider remains through tick 59
  and is removed at tick 60 with exact private timer bits. A separate mixed
  steering run forks at tick 7 and proves byte-identical boat and passenger
  continuation through tick 16 after native checkpoint reload.
- Post-change regression results: the monolithic runtime passes in 6:16.12 at
  520,160 KiB peak RSS; the complete ledger passes in 1:20.19 at 774,596 KiB;
  and scalar CPU throughput passes at 4,749 steps/s against the 3,858.9 floor.
  Both runs used zero swap and GPU 1 remained untouched.
