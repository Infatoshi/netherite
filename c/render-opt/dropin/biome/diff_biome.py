# Pixel-diff the biome-blend drop-in frames vs the vanilla (off) baseline, over the
# auto-detected rendered-window bbox. native-vs-off should be ~noise (kernel 18 is
# bit-exact); sabotage-vs-off should be large (grass forced magenta).
import numpy as np
from PIL import Image
D = "/home/infatoshi/dev/minecraft/mc-1.11.2-env/c/render-opt/dropin/biome/"
def load(n): return np.asarray(Image.open(D + n).convert("RGB")).astype(np.int16)
off = load("frame_off.png")
mask = off.max(axis=2) > 8
ys, xs = np.where(mask)
y0, y1, x0, x1 = ys.min(), ys.max()+1, xs.min(), xs.max()+1
win = (slice(y0, y1), slice(x0, x1))
print(f"window bbox rows[{y0}:{y1}] cols[{x0}:{x1}]")
def diff(n):
    a = load(n)
    if a.shape != off.shape: return f"{n}: shape {a.shape} != {off.shape}"
    d = np.abs(a - off)[win]; per = d.max(axis=2); differ = int((per>0).sum())
    return (f"{n:20s} max/ch={int(d.max()):3d} mean={d.mean():6.3f} "
            f"differ={differ}/{per.size} ({100.0*differ/per.size:5.2f}%)")
print(diff("frame_native.png"))
print(diff("frame_sabotage.png"))
