...
#      ON and with it fully OFF (CRASTER_NO_CULL=1, meshes every chunk in radius).
...
echo "  -- culling OFF (CRASTER_NO_CULL=1) --"
CRASTER_NO_CULL=1 "$T/rung4" "$T/cull_off.ppm" | sed -n 's/^view-distance/  &/p'
