...
static int neighbor_light_query(int neighbors, int up, int east, int west,
                                int south, int north, int own) {
    if (!neighbors) return own;
    int light = up;
    if (east > light) light = east;
    if (west > light) light = west;
    if (south > light) light = south;
    if (north > light) light = north;
    return light;
}
...
void gm_uw_eval(const GmWorld *w, int dim, const GmPlayerView *pv,
                float fog_c1, float night_vision, float boss_color_modifier,
                int blindness_duration,
    ...
    out->fog01.x *= 1.0f - 0.3f * boss_color_modifier;
    out->fog01.y *= 1.0f - 0.4f * boss_color_modifier;
    out->fog01.z *= 1.0f - 0.4f * boss_color_modifier;
    {
