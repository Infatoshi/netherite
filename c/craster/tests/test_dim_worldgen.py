...
1) mc-sim Java golden == C provideChunk for multiple seeds (bitwise hex).
2) REAL Anvil DIM-1 / DIM1 (qrl_0 seed 0) vs C multi-chunk dump after free-pass
   of known populate/structures only.

Run:
  uv run --no-project --with numpy --with nbt pytest c/craster/tests/test_dim_worldgen.py -q
"""
...
# Multi-seed matrix for provideChunk Java golden (mc-sim oracle runner).
...
def test_mc_sim_java_golden_cpu(name, seed):
    ...
    r = _run(
        [sys.executable, str(MCSIM / "oracle" / "runner.py"), "--cpu-only", name, seed],
        cwd=str(MCSIM),
        timeout=180,
    )
