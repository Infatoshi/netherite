...
MAGMA = HERE.parents[2]  # c/magma
MC_CAPTURE = MAGMA / "raster" / "verify" / "mc_capture"
REPO = MAGMA.parents[1]  # fogcurve or netherite monorepo root
...
DEFAULT_SLIME_GOLDEN_DIR = Path(
    "/home/infatoshi/dev/netherite/c/magma/raster/verify/tapes/"
...
def ensure_game_candidate(bin_path: Path) -> Path:
    if bin_path.exists() and bin_path.stat().st_mtime > (
        ...
        if os.environ.get("FOGCURVE_FORCE_BUILD") != "1":
            return bin_path
    mcsim = MAGMA.parent / "mc-sim" / "core"
    flags = [
        "-O2",
        "-ffp-contract=off",
        "-Wall",
