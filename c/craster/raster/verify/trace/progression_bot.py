...
each as a tape (tape.py), then replay through craster (replay_tape.py --cuda
...
Usage (game live with the qrl bridge up):
  ...
  ... --replay   also replay+report through craster afterwards (GPU1)
"""
import argparse
import math
import os
...
def seg_dragon(env, bow_slot=0):
    ...
    def setup():
        ...
        # its header starts in dim 1 and craster replays from the header.
        ensure_dim(env, 1)
        cmds(env, [
...
def main():
    ...
    ap.add_argument("--replay", action="store_true",
                    help="replay+report through craster (CUDA, GPU1) afterwards")
