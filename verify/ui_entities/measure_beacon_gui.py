#!/usr/bin/env python3
"""Strict real-Java pixel gate for the Minecraft 1.11.2 Beacon panel."""
...
# GuiBeacon at 854x480, GUI scale 2: xSize=230, ySize=219.
PANEL = (196, 20, 656, 458)
...
def panel_mask(shape: tuple[int, ...]) -> np.ndarray:
    ...
    # The source texture has only 18 transparent corner texels. Excluding a
    # 6x6 framebuffer square at each corner removes every unowned underlay
    # pixel while retaining the complete panel, controls, labels, and slots.
...
def mutation_selftest() -> None:
    ...
    if result["different_px"] != 1 or result["max_channel"] != 1:
        raise RuntimeError("Beacon GUI negative control was not detected")
...
def main() -> int:
    ...
    java_a = load(goldens / "gui_beacon_a.png")
    java_b = load(goldens / "gui_beacon_b.png")
    native = load(Path(args.c_frame))
    if java_a.shape != java_b.shape or java_a.shape != native.shape:
        raise RuntimeError("Beacon GUI frame dimensions differ")
    ...
    report = {
        ...
        "excluded": "transparent four 6x6 corners",
        "java_ab": java_ab,
    ...
    if not stable:
        raise RuntimeError("real-Java Beacon GUI capture is unstable")
    if not exact:
        raise RuntimeError("native Beacon GUI panel differs from Java")
    print("PASS Beacon GUI: owned pixels exact vs real Java")
    return 0
