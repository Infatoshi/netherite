...
/^COHORT / { cohorts++; count=split($0,parts," ");for(i=2;i<=count;i++){split(parts[i],kv,"=");if(kv[1]=="actual_ticks")cohort_ticks+=kv[2]} }
...
END {
 if (!("total_ms" in r) && "elapsed_s" in r) {r["total_ms"]=r["elapsed_s"]*1000;r["actual_ticks_per_s"]=(cohorts==2&&r["elapsed_s"]>0?cohort_ticks/r["elapsed_s"]:"NA")}
 if ("reset_batches" in r) r["reset_count"]=r["reset_batches"];
