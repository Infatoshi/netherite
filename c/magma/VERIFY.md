...
| Death GuiGameOver (title, score, buttons, tint) | `game/hud.c:hud_death_draw` | `run_ui_hud_gates.sh` hard title/score/buttons + soft full-frame residual + live respawn | **Partial**: opaque chrome (title/score body+shadow, full button rects) bit-exact vs A/B goldens; translucent gradient/world tint soft/open | High (tint) | Closed chrome: genuine `hud_death_{a,b}.png`; Respawn/Title hit + timer. Tint residual remains open. |
