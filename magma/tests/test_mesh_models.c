...
int main(void) {
    ...
    CHECK(bm_block(T_BEACON)->kind == BM_KIND_BEACON,
          "beacon: specialized model is not reachable");
    CHECK(bm_block(T_BEACON)->layer == CR_LAYER_CUTOUT,
          "beacon: wrong render layer");
    ...
    int xf   = BASE + 6,  zf  = BASE + 6;   /* isolated fence */
    int xfc  = BASE + 15, zfc = BASE + 6;   /* east-connected boundary fence */
    int yfc  = TY + 10;
    ...
    int xbe  = BASE + 9,  zbe = BASE + 10, ybe = TY + 70; /* beacon */
    ...
    light_debug_set_block(L, xfc, yfc, zfc, T_FENCE);
    light_debug_set_block(L, xfc + 1, yfc, zfc, T_STONE);
    ...
    light_debug_set_block(L, xbe, ybe, zbe, T_BEACON);
    ...
    /* --- BEACON: exact beacon.json glass/base/inner boxes and explicit UVs. --- */
    {
        GList g = {0};
        const float from[3][3] = {
            {0,0,0}, {2,0.1f,2}, {3,3,3}
        };
        const float to[3][3] = {
            {16,16,16}, {14,3,14}, {13,14,13}
        };
        const float uv[3][2][4] = {
            {{0,0,16,16},{0,0,16,16}},
            {{2,2,14,14},{2,13,14,16}},
            {{3,3,13,13},{3,2,13,13}},
        };
        const int sprites[3] = {
            CR_SPRITE_GLASS, CR_SPRITE_OBSIDIAN, CR_SPRITE_BEACON
        };
        for (int element = 0; element < 3; ++element)
            for (int face = 0; face < 6; ++face)
                golden_face_custom(
                    &g, xbe, ybe, zbe, from[element], to[element], face,
                    uv[element][face >= BM_NORTH], 0, 3, 0.0f, NULL, 0,
                    sprites[element]);
        int n = collect_cell_quads(
            &m, CR_LAYER_CUTOUT, xbe, ybe, zbe, got, 512);
        CHECK(n == 108, "beacon: %d verts (want 108)", n);
        CHECK(multiset_eq(got, n, &g, 1),
              "beacon: positions/UV differ from beacon.json");
        CHECK(collect_cell_quads(
                  &m, CR_LAYER_SOLID, xbe, ybe, zbe, got, 512) == 0,
              "beacon: leaked onto SOLID");
        free(g.v);
    }
    ...
    /* --- FENCE: an isolated actual-state fence is only its centre post. --- */
    {
        GList g = {0};
        float pf[3]={6,0,6}, pt[3]={10,16,10};
        golden_box(&g, xf, TY, zf, pf, pt, planks);
        int n = collect(&m, CR_LAYER_SOLID, xf, zf, ylo, yhi, got, 512);
        CHECK(n == 36, "isolated fence: %d verts (want 36)", n);
        CHECK(multiset_eq(got, n, &g, 1), "fence: positions/uv do not match facebakery golden");
        free(g.v);
    }
    /* A full cube just across the chunk edge connects only the east rails;
     * the neighbor is visible to actual-state lookup but is not meshed here. */
    {
        GList g = {0};
        float pf[3]={6,0,6}, pt[3]={10,16,10};
        float lo[3]={7,6,7}, lt[3]={16,9,9};
        float hi[3]={7,12,7}, ht[3]={16,15,9};
        golden_box(&g, xfc, yfc, zfc, pf, pt, planks);
        golden_box(&g, xfc, yfc, zfc, lo, lt, planks);
        golden_box(&g, xfc, yfc, zfc, hi, ht, planks);
        int n = collect(&m, CR_LAYER_SOLID, xfc, zfc,
                        (float)yfc - .01f, (float)yfc + 1.01f, got, 512);
        CHECK(n == 108, "east-connected fence: %d verts (want 108)", n);
        CHECK(multiset_eq(got, n, &g, 1),
              "east-connected fence positions/uv do not match golden");
        free(g.v);
    }
