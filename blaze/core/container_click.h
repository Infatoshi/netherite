 ...
 *
 * Boundaries of this synthetic CPU/CUDA kernel (the shipped runtime implements
 * these in magma/game/container_live.c):
...
enum {
    ...
    CC_CLICK_CLONE    = 3,
    CC_CLICK_THROW    = 4,
    CC_CLICK_QUICK_CRAFT = 5,
    CC_CLICK_PICKUP_ALL = 6,
...
MC_HD static inline i32 cc_max_stack_size(i32 item, i32 meta) {
    return isr_max_stack_size(item, meta);
}
                ...
                if (cc_is_empty(cursor)) {
                    ...
                    *cursor = cc_decr_slot(slot, k2);
                    ...
                    if (cc_stack_match(slot, cursor)) {
                        ...
                        /* Container.slotClick does not clamp this at zero;
                         * an invalid overstack transfers excess to cursor. */
                        cc_shrink(cursor, j2);
                        cc_grow(slot, j2);
                    } else {
