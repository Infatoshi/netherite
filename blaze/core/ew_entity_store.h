...
#define EW_SPAWN_LIMIT EW_MAX_ENTITIES
#endif
...
/* Copy one represented entity without disturbing unrelated writes already
 * made to the destination half of a live double-buffered tick. */
MC_HD static inline void ew_store_copy_slot(
        EwStore *dst, const EwStore *src, int i) {
    if (!dst || !src || i < 0 || i >= EW_MAX_ENTITIES) return;
    if (src->count > dst->count) dst->count = src->count;
    dst->type[i] = src->type[i];
    dst->alive[i] = src->alive[i];
    dst->id[i] = src->id[i];
    dst->x[i] = src->x[i]; dst->y[i] = src->y[i]; dst->z[i] = src->z[i];
