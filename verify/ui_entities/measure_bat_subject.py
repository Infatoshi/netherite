#!/usr/bin/env python3
"""Strict same-scene Bat model gate for flying and hanging poses."""
...
STATES = ("bat_flying", "bat_hanging")
OWNERSHIP_THRESHOLD = 25
# Hanging has one measured fixed-function boundary sample where the Java GL
# depth tie selects the tail top and the software 24-bit path selects the body
# back (same geometry and coverage, 33 max-channel delta). Keep it explicit.
HARD_BUDGET = {"bat_flying": 0, "bat_hanging": 1}
...
def main():
    ...
    java_bg_a = load(goldens / "bat_background_a.png")
    java_bg_b = load(goldens / "bat_background_b.png")
    native_bg = load(c_frames / "bat_background.ppm")
    if not (java_bg_a.shape == java_bg_b.shape == native_bg.shape):
        raise SystemExit("FAIL: Bat background dimensions differ")
    ...
    for state in STATES:
        ...
        if args.selftest and state == "bat_flying":
            points = np.argwhere(owned)
            if not len(points):
                raise SystemExit("FAIL: Bat mutation has no owned pixel")
            y, x = points[0]
            mutated = native_diff.copy()
            mutated[y, x] = OWNERSHIP_THRESHOLD + 1
            mutation_rejected = np.count_nonzero(
                mutated > OWNERSHIP_THRESHOLD) > 0

    if args.selftest:
        if not mutation_rejected:
            raise SystemExit("FAIL: Bat hard-pixel mutation was accepted")
    ...
    if failed:
        raise SystemExit("FAIL: Bat subject contract")
    print("PASS: Bat subject contract")
