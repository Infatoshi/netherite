/* CUDA: same core/tree_gen_birch_real.h as CPU; single-thread worldgen on device. */
#include <cstdio>
#include <cstdlib>
#include "../core/tree_gen_birch_real.h"

__global__ void run_tgbr(i64 seed, ChunkPrimer *primer, CpScratch *sc) {
    if (threadIdx.x || blockIdx.x) return;
    tgbr_run(primer, sc, seed);
}
...
int main(int argc, char **argv) {
    ...
    run_tgbr<<<1, 1>>>(seed, d_primer, d_sc);
