...
int main(void)
{
...

    {
        ...
        cold_before = gm_live_overflow_count(&r.entities);
        CHECK(gm_runtime_use_block(&r, 10, y, 8)
                  && gm_world_meta(r.world, 10, y, 8) == 0
                  && gm_runtime_static_container_get(&r, 0, &jukebox)
                  && jukebox.slots[0].item == 0
                  && gm_live_overflow_count(&r.entities) == cold_before + 1
                  && r.entities.overflow[r.entities.overflow_slots - 1]
                      .entity.item == 2256
                  && r.world_random_seed48 != world_random_seed48
                  && r.math_random_seed48 != math_random_seed48
                  && r.next_entity_id == next_entity_id + 1,
              "full hot item pool ejects jukebox record into active cold storage");
    }
