...
\item \textbf{A verified engine with two explicit fidelity tiers} (Sections~\ref{sec:design}--\ref{sec:impl}). \texttt{magma}, the replay/playable tier, targets the real game: same worldgen from the same seeds, physics checked at $10^{-9}$ over full sessions, and a software rasterizer gated pixel-by-pixel against real-client screenshots, with bitwise CPU$=$CUDA parity including the depth buffer. \texttt{blaze}, the batched simulation tier, holds a different but equally mechanical contract: the CPU scalar path and the CUDA batch path are compiled from the same source and must agree bitwise on every tick.
...
\item \textbf{Measured results} (Section~\ref{sec:eval}): 3.02M env-ticks/s at batch 8{,}192 on one RTX~PRO~6000 (vs.\ 0.29M for the same loop on the whole 32-thread Ryzen 9950X3D); a pinned 48-tape verification suite with 42 pixel-gate rc~0 replays and one deliberately visible baseline regression; and a PPO proof-of-life that learns a 2{,}058-action tool-progression chain with zero scripted stages and transfers to the real Java client on 12 of 13 evaluation seeds.
\end{enumerate}
