...
cleanup() {
    ...
    if [ "$KEY_USE_EXISTED" -eq 1 ]; then
        sed -i "s/^key_key.use:.*/key_key.use:${ORIG_USE}/" "$OPTS" 2>/dev/null
    else
        sed -i '/^key_key.use:/d' "$OPTS" 2>/dev/null
    fi
}
trap cleanup EXIT
sed -i 's/^key_key.use:.*/key_key.use:19/' "$OPTS"
log "key.use rebound: ${ORIG_USE:-(default)} -> 19 (R); will restore on exit"
...
# Held-out pose3 exercises a different pair of body/head pitch rotations. It is
# used by the preview color diagnostic, not by the two-pose formula fit.
log "inventory preview pose3 (mouse fb 390,210) ..."
xdotool mousemove $((AX + 390)) $((AY + 210)); sleep 0.4
ffmpeg -hide_banner -loglevel error -y -f x11grab -draw_mouse 0 \
    -video_size "${W}x${H}" -i ":1.0+${AX},${AY}" -frames:v 1 \
    "$OUTDIR/mc_gui_inventory_pose3_a.png" || fail "grab inventory_pose3_a"
sleep 0.5
ffmpeg -hide_banner -loglevel error -y -f x11grab -draw_mouse 0 \
    -video_size "${W}x${H}" -i ":1.0+${AX},${AY}" -frames:v 1 \
    "$OUTDIR/mc_gui_inventory_pose3_b.png" || fail "grab inventory_pose3_b"
[ -s "$OUTDIR/mc_gui_inventory_pose3_a.png" ] || fail "mc_gui_inventory_pose3_a.png empty"
for pose_spec in 'pose4 330 300' 'pose5 500 150'; do
    read -r pose mx my <<<"$pose_spec"
    log "inventory preview $pose (mouse fb $mx,$my) ..."
    xdotool mousemove $((AX + mx)) $((AY + my)); sleep 0.4
    for ab in a b; do
        ffmpeg -hide_banner -loglevel error -y -f x11grab -draw_mouse 0 \
            -video_size "${W}x${H}" -i ":1.0+${AX},${AY}" -frames:v 1 \
            "$OUTDIR/mc_gui_inventory_${pose}_${ab}.png" || fail "grab ${pose}_${ab}"
        sleep 0.5
    done
    [ -s "$OUTDIR/mc_gui_inventory_${pose}_a.png" ] || fail "mc_gui_inventory_${pose}_a.png empty"
done
pin_preview 0
xdotool key --window "$WIN" Escape; sleep 1
...
scene.update({"width": int(sys.argv[2]), "height": int(sys.argv[3]),
              ...
              "screens": ["table", "furnace", "inventory", "chest",
                          "inventory_pose2", "inventory_pose3",
                          "inventory_pose4", "inventory_pose5"],
              ...
              "notes": ("Empty survival inventory, frozen clear noon. Inventory "
                        ...
                        "held-out poses 3-5 at fb (390,210), (330,300), and (500,150). "
