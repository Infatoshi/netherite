...
typedef struct {
    ...
    int    exact_randticks;  /* lockstep-oracle only; never a batched hot path */
    int    success_item;     /* +10/done=1 item; 263 default (exact ppo_coal),
                              * 0 = disabled. Applied at reset. */
} CuVec;

/* CPU lockstep-only copy of the standalone magma hash selector and the leaf
 * callback. Production batched CPU/CUDA keeps this disabled: training owns a
 * quiescent snapshot world, while verify_cpu opts in to compare the complete
 * native action-line environment, where random block ticks remain live. */
static int blaze_cpu_is_leaves(int id) { return id == 18 || id == 161; }
static int blaze_cpu_is_log(int id) { return id == 17 || id == 162; }

static int blaze_cpu_light_at(const Blaze *e, int wx, int wy, int wz) {
    /* Snapshot cells do not carry light nibbles. The standalone curriculum
     * worlds use clear daylight. Preserve the discriminating immediate-cover
     * branch; side-propagated skylight keeps exposed cells at the growth
     * threshold even underneath a sparse canopy. */
    int id = cu_world_block(e, wx, wy, wz);
    return id && mc_bpt_props(id).light_opacity > 2 ? 0 : 15;
}

static void blaze_cpu_randtick_grass(
        Blaze *e, int wx, int wy, int wz, long long tick) {
    int above = cu_world_block(e, wx, wy + 1, wz);
    int light = blaze_cpu_light_at(e, wx, wy + 1, wz);
    int i;
    if (light < 4 && mc_bpt_props(above).light_opacity > 2) {
        cu_world_set_state(e, wx, wy, wz, 3, 0);
        return;
    }
    if (light < 9) return;
    for (i = 0; i < 4; ++i) {
        u64 h = mc_hash_seed(
            (u64)e->seed, tick, wx, wy, wz, 0x52544752u);
        int dx, dy, dz, x, y, z, above_id;
        h = mc_hash64(h ^ (u64)i);
        ...
        x = wx + dx; y = wy + dy; z = wz + dz;
        if (cu_world_block(e, x, y, z) != 3 ||
            (cu_world_meta(e, x, y, z) & 15) != 0) continue;
        above_id = cu_world_block(e, x, y + 1, z);
        if (blaze_cpu_light_at(e, x, y + 1, z) >= 4 &&
            mc_bpt_props(above_id).light_opacity <= 2)
            cu_world_set_state(e, x, y, z, 2, 0);
    }
}

static void blaze_cpu_randtick_leaf(Blaze *e, int wx, int wy, int wz) {
    int surroundings[32 * 32 * 32];
    int id = cu_world_block(e, wx, wy, wz);
    int meta = cu_world_meta(e, wx, wy, wz) & 15;
    int dx, dy, dz, distance;
    if (!blaze_cpu_is_leaves(id) || !(meta & 8) || (meta & 4)) return;
    memset(surroundings, 0, sizeof surroundings);
    for (dx = -4; dx <= 4; ++dx)
        for (dy = -4; dy <= 4; ++dy)
            for (dz = -4; dz <= 4; ++dz) {
                int block = cu_world_block(e, wx + dx, wy + dy, wz + dz);
                int index = (dx + 16) * 1024 + (dy + 16) * 32 + dz + 16;
                surroundings[index] = blaze_cpu_is_log(block) ? 0
                    : blaze_cpu_is_leaves(block) ? -2 : -1;
            }
    for (distance = 1; distance <= 4; ++distance)
        for (dx = -4; dx <= 4; ++dx)
            for (dy = -4; dy <= 4; ++dy)
                for (dz = -4; dz <= 4; ++dz) {
                    int base = (dx + 16) * 1024 + (dy + 16) * 32 + dz + 16;
                    if (surroundings[base] != distance - 1) continue;
#define BLAZE_LEAF_PROPAGATE(index_) \
    do { if (surroundings[(index_)] == -2) \
        surroundings[(index_)] = distance; } while (0)
                    BLAZE_LEAF_PROPAGATE(base - 1024);
                    BLAZE_LEAF_PROPAGATE(base + 1024);
                    BLAZE_LEAF_PROPAGATE(base - 32);
                    BLAZE_LEAF_PROPAGATE(base + 32);
                    BLAZE_LEAF_PROPAGATE(base - 1);
                    BLAZE_LEAF_PROPAGATE(base + 1);
#undef BLAZE_LEAF_PROPAGATE
                }
    if (surroundings[16912] >= 0)
        cu_world_set_state(e, wx, wy, wz, id, meta & ~8);
    else
        cu_world_set_state(e, wx, wy, wz, 0, 0);
}

static void blaze_cpu_randtick_pass(Blaze *e, long long tick) {
    const unsigned purpose = 0x5254504Fu; /* randtick.c RT_PURPOSE_POS */
    int cx, cz, section, attempt;
    for (cz = e->ccz - 2; cz <= e->ccz + 2; ++cz)
        for (cx = e->ccx - 2; cx <= e->ccx + 2; ++cx)
            for (section = 0; section < 16; ++section)
                for (attempt = 0; attempt < 3; ++attempt) {
                    u64 h = mc_hash_seed(
                        (u64)e->seed, tick, cx, section, cz,
                        purpose ^ (unsigned)attempt);
                    int x = cx * 16 + mc_hash_bound(h, 16);
                    int y, z;
                    h = mc_hash64(h + 1ULL);
                    y = section * 16 + mc_hash_bound(h, 16);
                    h = mc_hash64(h + 2ULL);
                    z = cz * 16 + mc_hash_bound(h, 16);
                    int id = cu_world_block(e, x, y, z);
                    if (blaze_cpu_is_leaves(id))
                        blaze_cpu_randtick_leaf(e, x, y, z);
                    else if (id == 2)
                        blaze_cpu_randtick_grass(e, x, y, z, tick);
                }
}

int blaze_set_exact_randticks(void *vh, int enabled) {
    CuVec *v = (CuVec *)vh;
    if (!v) return -1;
    v->exact_randticks = enabled ? 1 : 0;
    return 0;
}

int blaze_debug_randtick_pass(void *vh, int env, long long tick) {
    CuVec *v = (CuVec *)vh;
    if (!v || env < 0 || env >= v->n) return -1;
    blaze_cpu_randtick_pass(&v->envs[env], tick);
    return 0;
}

int blaze_debug_chunk_center(void *vh, int env, int *ccx, int *ccz) {
    CuVec *v = (CuVec *)vh;
    if (!v || env < 0 || env >= v->n || !ccx || !ccz) return -1;
    *ccx = v->envs[env].ccx; *ccz = v->envs[env].ccz;
    return 0;
}

int blaze_debug_randtick_grass(
        void *vh, int env, int x, int y, int z, long long tick) {
    CuVec *v = (CuVec *)vh;
    if (!v || env < 0 || env >= v->n) return -1;
    blaze_cpu_randtick_grass(&v->envs[env], x, y, z, tick);
    return 0;
}
...
#pragma omp parallel for schedule(static) reduction(|:rc) if(v->n > 1)
    ...
    if (v->exact_randticks)
        blaze_cpu_randtick_pass(&v->envs[env], v->envs[env].tick - 1);
...
/* Read one live region cell for cross-runtime divergence diagnostics.  This
 * is deliberately outside the step/observation ABI and never enters a hot
 * path; it lets the lockstep verifier distinguish world-state drift from a
 * camera or pose error without exposing CuVec's private layout to Python. */
int blaze_debug_cell(void *vh, int env, int wx, int wy, int wz) {
    CuVec *v = (CuVec *)vh;
    long ri;
    if (!v || env < 0 || env >= v->n) return -1;
    ri = cu_region_idx(&v->envs[env], wx, wy, wz);
    if (ri < 0) return -1;
    return (int)v->envs[env].cells[ri];
}

/* Packed rows: active,x,y,z,mx,my,mz,age,item,count,pickup_delay. */
int blaze_debug_items(void *vh, int env, double *out, int cap) {
    CuVec *v = (CuVec *)vh;
    int i, k = 0;
    if (!v || env < 0 || env >= v->n || !out || cap < CU_MAX_ITEMS * 11)
        return -1;
    for (i = 0; i < CU_MAX_ITEMS; ++i) {
        const CuItem *it = &v->envs[env].items[i];
        out[k++] = (double)it->active;
        out[k++] = it->x; out[k++] = it->y; out[k++] = it->z;
        out[k++] = it->mx; out[k++] = it->my; out[k++] = it->mz;
        out[k++] = (double)it->age; out[k++] = (double)it->item;
        out[k++] = (double)it->count; out[k++] = (double)it->pickup_delay;
    }
    return CU_MAX_ITEMS;
}
