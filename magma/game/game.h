...
typedef struct {
    ...
    /* PotionEffect.getIsAmbient(). This is independent of particle
     * visibility and participates in PotionEffect.combine. */
    int ambient;
...
typedef struct {
    ...
    int   creative;
    int   game_mode;
    int   adventure_can_destroy;
    int   adventure_can_place;
    int   attack_entity; /* getMouseOver selected an entity, not a block */
    /* Harness-only parked-server step.  The integrated Java client's world
     * thread does not advance during server_tick_locked, so it emits no
     * movement packet and performs no local player physics. */
    int   server_only;
} GmAction;
...
typedef struct {
    ...
    float portal_prev;          /* EntityPlayerSP.prevTimeInPortal */
} GmPlayerView;
...
#define GM_VIEW_SHULKER 42 /* anchored EntityShulker model */
#define GM_VIEW_SHULKER_BULLET 43 /* fullbright guided projectile model */
#define GM_VIEW_EXPLOSION_LARGE 50 /* replay-only ParticleExplosionLarge */
#define GM_VIEW_EVOKER_FANGS 54 /* live/tape EntityEvokerFangs model */
...
#define GM_VIEW_WITHER 66
#define GM_VIEW_WITHER_SKULL 67
#define GM_VIEW_MINECART_SPAWNER 74
#define GM_VIEW_MINECART_COMMAND 75
typedef struct {
    ...
    int   flags;             /* 1 burn, 2 sneak, 4 invisible, 8 child,
                              * 16 sheep graze, 32 Witch drinking,
                              * 64 pet sitting, 128 pet tamed,
                              * 256 Vindicator aggressive, 512 Evoker casting,
                              * 1024 Vex charging, 2048 Guardian moving,
                              * 4096 Guardian beam target,
                              * 131072 Skeleton bow-and-arrow arm pose,
                              * 262144 Entity.isGlowing */
    ...
    int   ticks_existed;
    /* EntityIronGolem private animation timers. */
    int   golem_attack_timer;
    int   golem_rose_timer;
    /* EntityArmorStand saved render state. Equipment holds vanilla item ids.
     * stand_flags: 1 ShowArms, 2 NoBasePlate, 4 Small. stand_pose follows
     * EntityArmorStand's head/body/left-arm/right-arm/left-leg/right-leg
     * Rotations order. Zeroed legacy callers retain the baked default pose. */
    int   armor_feet, armor_legs, armor_chest, armor_head;
    int   armor_feet_meta, armor_legs_meta, armor_chest_meta, armor_head_meta;
    int   armor_color[4];
    int   armor_color_valid;
    int   stand_mainhand, stand_offhand;
    int   stand_mainhand_meta, stand_offhand_meta;
    int   stand_flags;
    int   stand_pose_valid;
    float stand_pose[6][3];
    int   stand_punch_time_valid;
    float stand_punch_time;
    float boat_paddle[2];
    /* EntityWither render state. Side-head rotations are absolute degrees,
     * matching EntityWither.getHead[YX]Rotation; the model subtracts yaw. */
    int   wither_invul_time;
    float wither_head_yaw[2], wither_head_pitch[2];
    int   wither_skull_invulnerable;
    /* EntityMinecart display state. Zero means subtype default. */
    int   minecart_custom_display;
    int   minecart_display_block, minecart_display_meta;
    int   minecart_display_offset;
    int   minecart_powered, minecart_tnt_fuse;
    int   minecart_tnt_fuse_valid;
} GmEntityView;

#ifndef GM_ENTITY_FLAG_GLOWING
#define GM_ENTITY_FLAG_GLOWING 262144
#endif
#ifndef GM_ENTITY_FLAG_BAT_HANGING
#define GM_ENTITY_FLAG_BAT_HANGING 524288
#endif
#ifndef GM_ENTITY_FLAG_SNOWMAN_PUMPKIN
#define GM_ENTITY_FLAG_SNOWMAN_PUMPKIN 1048576
#endif
...
typedef struct {
    ...
    int harvest_silk; /* held Silk Touch level at the authoritative break */
    int harvest_fortune; /* held Fortune level at the authoritative break */
    int harvest_tool_slot; /* InventoryPlayer slot used by authoritative wear */
    int harvest_tool_meta; /* damage before ItemStack.onBlockDestroyed */
    int harvest_tool_wear; /* Item callback damage amount, before Unbreaking */
    int harvest_tool_unbreaking;
    ...
    /* Runtime-local display-name handle copied from the consumed ItemStack.
     * BlockFurnace.onBlockPlacedBy transfers it to the new tile. */
    int place_custom_name;
} GmBlockEdit;
...
/* world_type: 0 Overworld, 1 superflat RL arena, 2 Nether, 3 End. */
...
/* Attach an immutable, indexed Anvil chunk store. Saved chunks page into the
 * existing toroidal hot pool on demand; absent chunks keep vanilla worldgen. */
int       gm_world_attach_chunk_store(
              GmWorld *w, const char *path, int dimension);
/* Atomically write the attached store plus live sparse mutations. */
...
/* Test-only biome override for a loaded column. */
int       gm_world_debug_set_biome(
              GmWorld *w, int wx, int wz, int biome);
/* Test-only saved block-light nibble override. */
...
/* Finish a related batch of load_block_meta gameplay writes with one relight and
 * one authoritative generation bump. The batch must stay inside the ensured
 * chunk square supplied here. */
...
/* Ordered gameplay block writes for PathWorldListener parity. Snapshot/bulk
 * loads are deliberately excluded: Java registers a living navigator only
 * after its entity has been added to the world. The bounded log lets each
 * navigator consume writes independently without putting observer pointers
 * in save state. */
typedef struct {
    long long sequence;
    int x, y, z;
 ...
 * until the next call; do NOT free). Every layer is section-cull aware, and
 * translucent follows Java's reverse visible-section submission order.
...
/* Device-resident variant. Same walk and SAME SUBMISSION ORDER as
 * gm_world_mesh_view - column frustum cull, remesh, per-16-block-section
 * frustum cull, and reverse visible-section translucent order - but instead of memcpy-concat it
 ...
 * a layer emits at most GM_MESH_SECTIONS runs per kept chunk. Feeding these to the backend gather reproduces the host
...
int       gm_crystal_beams_emit(const GmEntityView *ents, int n,
                                CrVertex *out, int max);
int       gm_guardian_beams_emit(const GmEntityView *ents, int n,
                                 CrVertex *out, int max);
