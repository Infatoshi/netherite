...
GUI_CX = (W + S - 1) // S // 2
SH = (H + S - 1) // S
HORSE_GUI_MOUSE_X = 176
HORSE_GUI_MOUSE_Y = 62
HB_X = (GUI_CX - 91) * S
J1 = (SH - 39) * S
...
def grab_pair(e, path_a, path_b, expect_use_branch=None, gmx=None, gmy=None,
              pin_enchant_book=False):
...
def roi_for(state_id):
    if state_id in ("gui_crafting_table", "gui_anvil", "gui_merchant",
                    "gui_enchanting",
                    "gui_furnace",
                    "gui_brewing_stand"):
        return (250, 74, 602, 406)
    if state_id in ("gui_dispenser", "gui_dropper"):
        return (250, 74, 602, 406)
    if state_id == "gui_hopper":
        return (250, 106, 602, 372)
    if state_id == "gui_ender_chest":
        return (250, 72, 602, 408)
    if state_id == "gui_large_chest":
        return (250, 18, 602, 462)
    if state_id == "gui_shulker_box":
        return (250, 72, 602, 406)
    if state_id == "gui_beacon":
        return (196, 20, 656, 458)
    if state_id.startswith("gui_horse_"):
        return (250, 74, 602, 406)
    ...
    if state_id in ("hud_boss_half", "hud_wither_half"):
        bb_x = (GUI_CX - 91) * S
        bb_y = 12 * S
...
def assert_feature_presence(state_id, path, pin_reply):
    ...
    if state_id in (
            "gui_crafting_table", "gui_anvil", "gui_merchant",
            "gui_enchanting", "gui_furnace",
            "gui_brewing_stand",
            "gui_dispenser", "gui_dropper", "gui_hopper"):
        want = "GuiCrafting" if state_id == "gui_crafting_table" \
            else "GuiRepair" if state_id == "gui_anvil" \
            else "GuiMerchant" if state_id == "gui_merchant" \
            else "GuiEnchantment" if state_id == "gui_enchanting" \
            else "GuiFurnace" if state_id == "gui_furnace" \
            else "GuiBrewingStand" if state_id == "gui_brewing_stand" \
            else "GuiHopper" if state_id == "gui_hopper" \
            else "GuiDispenser"
        if pin_reply.get("screen") != want:
            raise RuntimeError(
                "%s: %s screen absent: %s"
                % (state_id, want, pin_reply))
        if std < 20.0:
            raise RuntimeError(
                "%s: static-container panel nearly flat (std=%.2f)"
                % (state_id, std))
        if state_id == "gui_furnace" \
                and pin_reply.get("container_fields") != [800, 1600, 100, 200]:
            raise RuntimeError(
                "gui_furnace: progress fields absent/wrong: %s" % pin_reply)
        if state_id == "gui_brewing_stand" \
                and pin_reply.get("container_fields") != [200, 10]:
            raise RuntimeError(
                "gui_brewing_stand: progress fields absent/wrong: %s"
                % pin_reply)
        if state_id == "gui_crafting_table":
            want_slots = [[377, 2, 0], [369, 3, 0]] + [[0, 0, 0]] * 8
            if pin_reply.get("crafting_slots") != want_slots:
                raise RuntimeError(
                    "gui_crafting_table: real recipe slots absent/wrong: %s"
                    % pin_reply)
        if state_id == "gui_anvil":
            want_anvil = {
                "maximum_cost": 1, "material_cost": 0, "name": "Oracle",
                "slots": [[369, 1, 0], [0, 0, 0], [369, 1, 0]],
            }
            if pin_reply.get("anvil_state") != want_anvil:
                raise RuntimeError(
                    "gui_anvil: real rename result/cost absent/wrong: %s"
                    % pin_reply)
        if state_id == "gui_merchant":
            want_merchant = {
                "selected": 0, "recipe_count": 2,
                "slots": [[388, 3, 0], [264, 1, 0], [369, 2, 0]],
            }
            if pin_reply.get("merchant_state") != want_merchant:
                raise RuntimeError(
                    "gui_merchant: selected trade/result absent/wrong: %s"
                    % pin_reply)
        if state_id == "gui_enchanting":
            want_enchanting = {
                "xp_seed": 0,
                "levels": [8, 13, 30],
                "clue_id": [34, 17, 21],
                "clue_level": [1, 2, 2],
                "names": ["ignite enchant dry self", "enchant fiddle imbue",
                          "snuff embiggen shorten earth"],
                "name_layout": [
                    {"width": 80, "string_width": 108,
                     "lines": ["ignite enchant", "dry self"]},
                    {"width": 74, "string_width": 101,
                     "lines": ["enchant fiddle", "imbue"]},
                    {"width": 74, "string_width": 142,
                     "lines": ["snuff embiggen", "shorten earth"]},
                ],
                "slots": [[276, 1, 0], [351, 12, 4]],
            }
            if pin_reply.get("enchanting_state") != want_enchanting:
                raise RuntimeError(
                    "gui_enchanting: verified offers/slots absent or wrong: %s"
                    % pin_reply)
    elif state_id in ("gui_ender_chest", "gui_large_chest"):
        if pin_reply.get("screen") != "GuiChest":
            raise RuntimeError(
                "%s: Chest screen absent: %s"
                % (state_id, pin_reply))
        if std < 20.0:
            raise RuntimeError(
                "%s: Chest panel nearly flat (std=%.2f)"
                % (state_id, std))
    elif state_id == "gui_shulker_box":
        if pin_reply.get("screen") != "GuiShulkerBox":
            raise RuntimeError(
                "%s: Shulker Box screen absent: %s"
                % (state_id, pin_reply))
        if std < 20.0:
            raise RuntimeError(
                "%s: Shulker Box panel nearly flat (std=%.2f)"
                % (state_id, std))
    elif state_id.startswith("gui_horse_"):
        if pin_reply.get("screen") != "GuiScreenHorseInventory":
            raise RuntimeError(
                "%s: horse inventory screen absent: %s"
                % (state_id, pin_reply))
        if std < 20.0:
            raise RuntimeError(
                "%s: horse panel/preview nearly flat (std=%.2f)"
                % (state_id, std))

    elif state_id == "hud_death":
    ...
    elif state_id in ("hud_boss_half", "hud_wither_half"):
        # A stone wall is already textured enough to satisfy a generic ROI
        # variance check. Require the PINK bar's measured purple/dark signal,
        # so strip.overlays=true cannot create a vacuous boss-bar golden.
        if purple_bias < 8.0 or dark_frac < 0.08:
            raise RuntimeError(
                "%s: boss bar absent (B-G=%.1f dark_frac=%.3f); "
                "capture oracle with strip.overlays=false"
                % (state_id, purple_bias, dark_frac))
...
def capture_pair(e, outdir, state_id, pin_kwargs, meta_extra=None, settle_n=2):
    ...
    # Every frame owns its health-bookkeeping phase. Without these defaults,
    # a prior hurt-state capture leaves GuiIngame.healthUpdateCounter active
    # and later overlay goldens inherit flashing white hearts.
    health_i = int(math.ceil(float(pin.get("health", 20.0))))
    pin.setdefault("hud_flash", False)
    pin.setdefault("hud_health", health_i)
    pin.setdefault("hud_last_health", health_i)
    pin.setdefault("hud_update_counter", 1000)
    ...
    use_pair = (
        ...
        or state_id in (
            ...
            "gui_brewing_stand",
            "gui_dispenser", "gui_dropper", "gui_hopper",
            "gui_ender_chest", "gui_large_chest", "gui_shulker_box",
            "gui_beacon")
        or state_id.startswith("gui_horse_")
        or state_id in (
            "hud_hurt_flash_on", "hud_hurt_flash_off",
    ...
    if use_pair:
        r2 = r1
        pair = grab_pair(
            e, path_a, path_b, expect_use_branch=expect_branch,
            pin_enchant_book=(state_id == "gui_enchanting"),
            gmx=110 if state_id in (
                "gui_beacon", "gui_crafting_table", "gui_anvil",
                "gui_merchant", "gui_enchanting")
                else HORSE_GUI_MOUSE_X
                if state_id in (
                    "gui_crafting_table", "gui_anvil", "gui_merchant",
                    "gui_enchanting",
                    "gui_furnace",
                    ...
                    "gui_ender_chest", "gui_large_chest", "gui_shulker_box",
                    "gui_beacon")
                    or state_id.startswith("gui_horse_") else None,
            gmy=100 if state_id in (
                "gui_beacon", "gui_crafting_table", "gui_anvil",
                "gui_merchant", "gui_enchanting")
                else HORSE_GUI_MOUSE_Y
                if state_id in (
                    "gui_crafting_table", "gui_anvil", "gui_merchant",
                    "gui_enchanting",
                    "gui_furnace",
                    ...
                    or state_id.startswith("gui_horse_") else None)
        if state_id == "gui_enchanting":
            if int(pair.get("enchant_book_pin", 0) or 0) != 1:
                raise RuntimeError(
                    "gui_enchanting: render-boundary book pin missing: %s"
                    % pair)
            if (float(pair.get("enchant_book_open", -1)) != 1.0
                    or float(pair.get("enchant_book_flip", -1)) != 0.0):
                raise RuntimeError(
                    "gui_enchanting: wrong render-boundary book state: %s"
                    % pair)
        fa = dict(pair)
        fa["file"] = path_a
        fb = dict(pair)
...
def main():
    ...
    args = ap.parse_args()
    # The Java client runs from java/Minecraft, not the driver's cwd.  qrl
    # receives these paths over a socket, so always send canonical paths.
    ...
    if want("hud_air_partial"):
        ...
        add(capture_pair(e, outdir, "hud_air_partial", {
            # Sticky hud_pin re-applies 121 at frame render: 4 full + 1 partial.
    ...
    if want("hud_wither_half"):
        begin_state(e, "hud_wither_half", rebuild_scene=False)
        add(capture_pair(e, outdir, "hud_wither_half", {
            "health": 20.0, "food": 20, "air": 300, "absorption": 0.0,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": True, "frac": 0.5, "name": "Wither"},
            "clear_effects": True,
        }, {"roi": "Wither boss bar top center"}))

    # ---- Ender Chest inventory (fixed mouse, stable client-only inventory) ----
    if want("gui_ender_chest"):
        begin_state(e, "gui_ender_chest", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_ender_chest", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "ender_chest_gui": {
                "storage": [[264, 5, 0], [297, 2, 0]],
            },
        }, {"roi": "full populated Ender Chest container panel",
             "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    # ---- Large Chest inventory (54 slots, both physical halves populated) ----
    if want("gui_large_chest"):
        begin_state(e, "gui_large_chest", rebuild_scene=False)
        storage = [[264, 5, 0]] + [[0, 0, 0]] * 26 + [[297, 2, 0]]
        add(capture_pair(e, outdir, "gui_large_chest", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "large_chest_gui": {"storage": storage},
        }, {"roi": "full populated 54-slot Large Chest container panel",
             "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    # ---- Dispenser/dropper/hopper inventories (fixed mouse, client-only) ----
    for state_id, kind in (
            ("gui_dispenser", "dispenser"),
            ("gui_dropper", "dropper"),
            ("gui_hopper", "hopper")):
        if not want(state_id):
            continue
        begin_state(e, state_id, rebuild_scene=False)
        add(capture_pair(e, outdir, state_id, {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "static_container_gui": {
                "kind": kind,
                "storage": [[264, 5, 0], [297, 2, 0]],
            },
        }, {"roi": "full populated %s container panel" % kind,
             "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    # ---- Furnace/brewing inventories with visible progress (client-only) ----
    for state_id, kind, storage, progress in (
            ("gui_furnace", "furnace",
             [[15, 5, 0], [263, 2, 0], [265, 3, 0]],
             {"burn_time": 800, "current_burn_time": 1600,
              "cook_time": 100, "total_cook_time": 200}),
            ("gui_brewing_stand", "brewing",
             [[373, 1, 0], [373, 1, 0], [373, 1, 0],
              [372, 2, 0], [377, 3, 0]],
             {"brew_time": 200, "fuel": 10})):
        if not want(state_id):
            continue
        begin_state(e, state_id, rebuild_scene=False)
        fixture = {"kind": kind, "storage": storage}
        fixture.update(progress)
        add(capture_pair(e, outdir, state_id, {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "static_container_gui": fixture,
        }, {"roi": "full populated %s progress panel" % kind,
             "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    # ---- Crafting Table with a real computed recipe result (client-only) ----
    if want("gui_crafting_table"):
        begin_state(e, "gui_crafting_table", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_crafting_table", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "crafting_table_gui": {
                "grid": [[369, 3, 0]] + [[0, 0, 0]] * 8,
            },
        }, {"roi": "full populated Crafting Table panel with real recipe",
             "gmx": 110, "gmy": 100}))

    # ---- Anvil with real rename output/cost and visible name field ----
    if want("gui_anvil"):
        begin_state(e, "gui_anvil", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_anvil", {
            "health": 20.0, "food": 20, "air": 300,
            "xp_level": 30, "xp_frac": 0.0,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "anvil_gui": {
                "item": 369, "count": 1, "meta": 0, "name": "Oracle",
            },
        }, {"roi": "full populated Anvil rename/result/cost panel",
             "gmx": 110, "gmy": 100}))

    # ---- Villager with two real offers and a populated selected trade ----
    if want("gui_merchant"):
        begin_state(e, "gui_merchant", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_merchant", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "merchant_gui": {},
        }, {"roi": "full Villager selected-offer/result panel",
             "gmx": 110, "gmy": 100}))

    # ---- Enchanting with verified seed-0/power-15 sword offers ----
    if want("gui_enchanting"):
        begin_state(e, "gui_enchanting", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_enchanting", {
            "health": 20.0, "food": 20, "air": 300,
            "xp_level": 30, "xp_frac": 0.0,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            ...
            "boss": {"show": False}, "clear_effects": True,
            "enchanting_gui": {},
        }, {"roi": "full populated Enchant panel with verified offers",
             "gmx": 110, "gmy": 100}))

    # ---- Shulker Box inventory (fixed mouse, stable client-only inventory) ----
    if want("gui_shulker_box"):
        begin_state(e, "gui_shulker_box", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_shulker_box", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
        ...
        }, {"roi": "full populated Shulker Box container panel",
             "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    # ---- Beacon controls (all tiers, payment, level-II selected) ----
    if want("gui_beacon"):
        begin_state(e, "gui_beacon", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_beacon", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "beacon_gui": {
                "levels": 4, "primary": 5, "secondary": 5,
                "payment": 264,
            },
        }, {"roi": "full Beacon panel and controls",
             "gmx": 110, "gmy": 100}))

    # ---- Horse inventory screens (fixed scaled mouse, stable client-only horse) ----
    if want("gui_horse_armor"):
        begin_state(e, "gui_horse_armor", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_horse_armor", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "horse_gui": {
                "kind": "horse", "variant": 6 | (4 << 8),
                "armor": 3, "saddled": True, "ticks_existed": 40,
            },
        }, {"roi": "full horse container panel",
             "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    if want("gui_horse_donkey_chest"):
        begin_state(e, "gui_horse_donkey_chest", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_horse_donkey_chest", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "horse_gui": {
                "kind": "donkey", "chested": True, "saddled": True,
                "ticks_existed": 40,
                "storage": [[264, 3, 0], [297, 5, 0]],
            },
        }, {"roi": "full chested-donkey container panel",
            "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))

    if want("gui_horse_llama_chest"):
        begin_state(e, "gui_horse_llama_chest", rebuild_scene=False)
        add(capture_pair(e, outdir, "gui_horse_llama_chest", {
            "health": 20.0, "food": 20, "air": 300,
            "armor": [0, 0, 0, 0],
            "hotbar": [[0, 0, 0]] * 9,
            "use_action": 0, "fire": 0, "portal": 0.0,
            "boss": {"show": False}, "clear_effects": True,
            "horse_gui": {
                "kind": "llama", "variant": 2, "decor": 4,
                "chested": True, "strength": 3,
                "ticks_existed": 40,
                "storage": [[264, 3, 0], [297, 5, 0]],
            },
        }, {"roi": "full chested-llama container panel",
            "gmx": HORSE_GUI_MOUSE_X, "gmy": HORSE_GUI_MOUSE_Y}))
    ...
    preferred = [
        ...
        "hud_durability_half", "hud_boss_half", "hud_wither_half",
        "gui_crafting_table", "gui_anvil", "gui_merchant",
        "gui_enchanting", "gui_furnace",
        "gui_brewing_stand",
        "gui_dispenser", "gui_dropper", "gui_hopper",
