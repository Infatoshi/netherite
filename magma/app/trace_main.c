...
int main(int argc, char **argv) {
    ...
    int         daylight = 0;
    FixtureBlock *fixtures = NULL;
    int         nfixtures = 0, fixtures_cap = 0;
    ...
    for (int i = 1; i < argc; ++i) {
        if      (!strcmp(argv[i], "--seed")     && i+1 < argc) seed = atoll(argv[++i]);
            ...
            if (nfixtures == fixtures_cap) {
                int cap = fixtures_cap > 0 ? fixtures_cap * 2 : 64;
                FixtureBlock *grown = (FixtureBlock *)realloc(
                    fixtures, (size_t)cap * sizeof *grown);
                if (!grown) {
                    fprintf(stderr, "trace: set-block allocation failed\n");
                    free(fixtures);
                    return 2;
                }
                fixtures = grown;
    ...
    free(fixtures); free(tape); free(tris); free(win);
