...
Explicit non-vacuous controls (always run):
  ...
  portal_real_ab_control - noisy twins force CAPTURE_BLOCKED; bit-exact twins
                           let exact C pass while Java_a+1 stays RESIDUAL
...
def _portal_real_ab_controls(goldens):
    """Portal controls adapt to the measured A/B capture integrity."""
    ...
    for label, c in controls:
        r = evaluate_fullscreen_replace(PORTAL_SID, ja, jb, c)
        if n_ab > 0:
            ok = (r["verdict"] == "CAPTURE_BLOCKED"
                  and r.get("noise_max", 0) > 0)
            want = "CAPTURE_BLOCKED"
        elif label == "portal_C_Java_a_plus1":
            ok = r["verdict"] == "RESIDUAL" and (r.get("hard_px") or 0) > 0
            want = "RESIDUAL"
        else:
            ok = r["verdict"] == "PASS" and r.get("hard_px") == 0
            want = "PASS"
        if not ok:
            print("CONTROL FAIL: %s verdict=%s (want %s) "
                  "hard_px=%s thr=%s noise_max=%s" % (
                      label, r.get("verdict"), want, r.get("hard_px"),
...
def main():
    ...
    for sid in IDS:
        ...
        # No product PASS when the capture twins themselves disagree.
        if (sid == PORTAL_SID and r0["verdict"] == "PASS"
                and (r0.get("n_ab_maxch_ge1") or 0) > 0):
            n_fail += 1
