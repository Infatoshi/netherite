...
static void test_capacity_fallbacks(void) {
    ...
    CHECK(born.result == GM_SHEEP_MATE_BORN
              && born.child_eid == 20000
              && born.child_slot == GM_MOB_LIVING_STAGE_SLOT
              && mobs.sheep_birth_dropped == 0
              && mobs.living_cold_count == 1
              && sheep_count(&mobs) == 3
          ...
          "full hot living store grows a cold child and preserves birth effects");
    ...
    delay = 59;
    born = update(&mobs, &delay, 0, 1, &world, &math, &next_id, 1);
    const McOrb *grown_xp = gm_mobs_xp_orb_ref(&mobs, born.xp_slot);
    CHECK(born.result == GM_SHEEP_MATE_BORN
              && born.child_eid == 30000 && born.child_slot >= 0
              && born.xp_eid == 30001 && born.xp_slot == GM_XP_ORBS
              && born.xp_value >= 1 && born.xp_value <= 7
              && grown_xp && grown_xp->eid == 30001
              && grown_xp->xpValue == born.xp_value
              && mobs.sheep_breed_xp_dropped == 0
...
static void test_particle_batch_capacity(void) {
    ...
    CHECK(gm_mobs_particle_batch_count(&mobs)
              ...
              && mobs.particle_batches_cap > GM_MOB_PARTICLE_BATCH_CAPACITY
              && mobs.particle_batch_dropped == 0
              && gm_mobs_particle_batch_get(&mobs, 0, &oldest)
              && gm_mobs_particle_batch_get(
                  &mobs, GM_MOB_PARTICLE_BATCH_CAPACITY, &newest)
              && oldest.seq == 0
...
static void test_chicken_egg_threshold(void) {
    for (int full = 0; full <= 1; ++full) {
        ...
        for (int slot = 0;
                slot < gm_live_entity_slot_count(&runtime.entities); ++slot) {
            const GmLiveEnt *candidate =
                gm_live_entity_ref(&runtime.entities, slot);
            if (candidate && candidate->active
                    && candidate->eid == base_eid + 3)
                egg = candidate;
        }
        GmMobEvent event;
        ...
        CHECK(egg && egg->item == 344 && egg->count == 1
                  ...
                  && egg->lifespan == 6000
                  && (!full || runtime.entities.n_overflow == 1)
                  && runtime.entities.spawn_fail_count == 0,
              full
                  ? "full hot store ticks the new egg in active cold storage"
...
static void test_simultaneous_birth_order(void) {
    ...
    runtime.mobs.sheep_mate_eid[parent[0]] = runtime.mobs.a.id[parent[1]];
    runtime.mobs.sheep_mate_delay[parent[0]] = 59;
    runtime.mobs.sheep_mate_active[parent[2]] = 1;
    runtime.mobs.sheep_mate_eid[parent[2]] = runtime.mobs.a.id[parent[3]];
