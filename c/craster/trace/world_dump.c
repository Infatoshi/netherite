/* world_dump.c - dump craster's decorated world blocks + biome for a chunk range,
 * for the worldgen INTEGRATION verifier (trace/world_diff.py). Ground truth is the
 * REAL Java game's saved .mca; this emits craster's side the way craster RENDERS it:
 * mc-sim cp_provide_chunk base terrain + owr_run decoration (world/light.c gen_chunk
...
/* RNG-cursor probe sink (CRASTER_GENPROBE=path): same line format as the live-game
...
int main(int argc, char **argv) {
    ...
    const char *out = "trace/out/craster_world.bin";
    ...
    const char *probe_path = getenv("CRASTER_GENPROBE");
    if (probe_path) {
