"""ctypes wrapper for the batched mine-coal env (rl/cuenv).

VecCuEnv(n, device=0, so_path=None) auto-detects the backend: cuenv_cuda.so
(if present and torch sees a GPU) else cuenv_cpu.so - both export the same C
ABI (cuenv_step takes double actions[N][13] since the iron extension).
...
Action tensor: EITHER
  ...
  - full float64 [N,13] raw action rows in cuenv_tick_raw order:
...
Chain extras (cuenv_step_full / capture / success-item, 2026-07-15):
  ...
  env.capture(env_idx, slot): snapshot a live env into snapshot slot
    ...
    overwrites - keep a fixed slot->seed discipline (see cuenv_capture).

Usage:
    env = VecCuEnv(1024, device=0)
...
CUDA_SO = os.path.join(HERE, "cuenv_cuda.so")
CPU_SO = os.path.join(HERE, "cuenv_cpu.so")

CAM_H, CAM_W, NPIX = 36, 64, 36 * 64
NSCAL, NPOSE, NHEADS = 6, 5, 5   # NHEADS = legacy trainer head count
NACT = 13                        # full raw action row (cuenv_tick_raw order)
NACT_LEGACY = 12                 # pre-iron rows, zero-padded (smelt=0)
NSTATUS = 17                     # CU_STATUS_K (cuenv_fill_status layout)
YAW_STEPS = (-15.0, 0.0, 15.0)   # act_dict head decode (cu_yaw_step)
PITCH_STEPS = (-10.0, 0.0, 10.0)
OP_NAMES = (                     # cuenv_core.h CU_OP_* order (op_trace cols)
...
class VecCuEnv:
    def __init__(self, n, device=0, so_path=None):
        ...
        self.lib.cuenv_create.restype = ctypes.c_void_p
        self.lib.cuenv_create.argtypes = [ctypes.c_int, ctypes.c_int]
        self.lib.cuenv_destroy.argtypes = [ctypes.c_void_p]
        self.lib.cuenv_load_snapshots.argtypes = [
            ctypes.c_void_p, ctypes.POINTER(ctypes.c_char_p), ctypes.c_int,
            ctypes.c_char_p, ctypes.c_int]
        self.lib.cuenv_snapshot_has_liquid.argtypes = [ctypes.c_void_p,
                                                       ctypes.c_int]
        self.lib.cuenv_assign.argtypes = [ctypes.c_void_p,
                                          ctypes.POINTER(ctypes.c_int)]
        self.lib.cuenv_set_reward_gate.argtypes = [ctypes.c_void_p,
                                                   ctypes.c_double]
        self.lib.cuenv_reset.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        self.lib.cuenv_step.argtypes = [ctypes.c_void_p] + \
            [ctypes.c_void_p, ctypes.c_int] + [ctypes.c_void_p] * 7
        # step arg order: (h, actions, repeat, cam, depth, edge, scal, rew,
        #                  done, pose)
        self.lib.cuenv_step.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p,
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
            ctypes.c_void_p, ctypes.c_void_p]
        self.lib.cuenv_step_full.argtypes = \
            self.lib.cuenv_step.argtypes + [ctypes.c_void_p]
        self.lib.cuenv_set_success_item.argtypes = [ctypes.c_void_p,
                                                    ctypes.c_int]
        self.lib.cuenv_capture.argtypes = [ctypes.c_void_p, ctypes.c_int,
                                           ctypes.c_int]
        self.lib.cuenv_op_count.restype = ctypes.c_int
        self.lib.cuenv_op_trace.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        self.h = self.lib.cuenv_create(device, n)
        if not self.h:
            raise RuntimeError(
                f"cuenv_create(device={device}, n={n}) failed [{so_path}]")
    ...
    def load_snapshots(self, paths):
        ...
        r = self.lib.cuenv_load_snapshots(self.h, arr, len(paths), err, 512)
        if r < 0:
            raise RuntimeError(f"cuenv_load_snapshots: {err.value.decode()}")
        self.n_snaps = r
        return r

    def snapshot_has_liquid(self, i):
        return self.lib.cuenv_snapshot_has_liquid(self.h, i)
    ...
    def set_success_item(self, item):
        ...
        if self.lib.cuenv_set_success_item(self.h, int(item)) != 0:
            raise RuntimeError("cuenv_set_success_item failed")

    def capture(self, env_idx, slot):
        """Snapshot live env `env_idx` into snapshot slot `slot` (appends at
        n_snaps or overwrites; see cuenv_capture's slot discipline note)."""
        r = self.lib.cuenv_capture(self.h, int(env_idx), int(slot))
        if r != 0:
            raise RuntimeError(f"cuenv_capture({env_idx}, {slot}) failed")
        if slot == self.n_snaps:
            self.n_snaps += 1

    def op_trace(self):
        """Cumulative per-env op-trace counters as uint64 [N, CU_OP_N]
        (see cuenv_core.h CU_OP_* order). Requires CUENV_OP_TRACE=1 in the
        environment BEFORE this VecCuEnv was created; returns None when
        tracing is off. Counters accumulate across steps AND resets."""
        import numpy as np
        nop = self.lib.cuenv_op_count()
        out = np.zeros((self.n, nop), dtype=np.uint64)
        r = self.lib.cuenv_op_trace(self.h,
    ...
    def set_reward_gate(self, dist):
        ...
        if self.lib.cuenv_set_reward_gate(self.h, float(dist)) != 0:
            raise RuntimeError("cuenv_set_reward_gate failed")

    def assign(self, snap_idx):
        arr = (ctypes.c_int * self.n)(*list(snap_idx))
        if self.lib.cuenv_assign(self.h, arr) != 0:
            raise RuntimeError("cuenv_assign failed")
    ...
    def reset(self, mask=None):
        ...
        if mask is not None:
            ...
            m = ctypes.c_char_p(b)
        if self.lib.cuenv_reset(self.h, m) != 0:
            raise RuntimeError("cuenv_reset failed")
    ...
    def step(self, actions, repeat=4):
        ...
        if r != 0:
            raise RuntimeError("cuenv_step failed")
