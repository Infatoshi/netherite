#!/usr/bin/env python3
"""Side-by-side oracle|craster MP4 from a replayed tape.

Usage:
  uv run --no-project --with numpy --with pillow python make_sbs_mp4.py \
      ../tapes/<TAPE>.jsonl [--t0 N] [--t1 N] [--fps 20] [--out X.mp4]

Reads the oracle frames (../tapes/<name>_frames/f_%06d.png) and the craster
replay frames (out/tape_<name>/craster_frames.npy + .ticks.npy), pairs them by
...
def main():
    ...
    fr = np.load(os.path.join(cdir, "craster_frames.npy"))
    tk = np.load(os.path.join(cdir, "craster_frames.ticks.npy"))
    ...
    for i, t in enumerate(tk):
        ...
        if os.path.exists(p):
            pairs.append((t, i, p))
    if not pairs:
        sys.exit("no overlapping oracle/craster ticks in range")
