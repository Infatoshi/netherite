 ...
 * game.h's full definition (CRASTER_GAME_H) or the types conflict. */
...
/* CRASTER_STATE_PROF=1: batched-env sizing census, printed to stderr at run
...
int gm_script_run(const GmConfig *cfg) {
    ...
    /* CRASTER_STATE_PROF: per-tick world-edit rate. gm_world_block_gen counts
    ...
    int prof_on = getenv("CRASTER_STATE_PROF") != NULL;
    ...
    for (int tick = 0; tick < cfg->ticks && !r.dead && !r.won; ++tick) {
        ...
        for (;;) {
            ...
            if (!strcmp(type,"action")) {
                 ...
                 * mid-walk 15-degree turn accelerated craster along the new yaw
                 * one tick early (accel fit: oracle 0.070711 = yaw 0, craster
                 * 0.086603 = yaw -15). */
                 ...
                 * no craster model are skipped (logged once per type). */
...
#undef OPT_DBL
        ...
        /* Flywheel probe: CRASTER_DUMP="tick,x0,x1,y0,y1,z0,z1" dumps id/meta of
         * a world region to stderr at that tick - the way to see craster's LIVE
         * world state mid-replay (fluid CA etc.), which no state-out field has. */
        {
            const char *dbg = getenv("CRASTER_DUMP");
            if (dbg) {
        ...
        /* Same, for light: CRASTER_DUMP_LIGHT="tick,x0,x1,y0,y1,z0,z1" dumps
         * "wx wy wz sky blk" lines (matches the qrl sample_light CSV columns)
         * so live-game light can be diffed cell-for-cell against craster's. */
        {
            const char *dbg = getenv("CRASTER_DUMP_LIGHT");
            if (dbg) {
                int dt,dx0,dx1,dy0,dy1,dz0,dz1;
                if (sscanf(dbg,"%d,%d,%d,%d,%d,%d,%d",&dt,&dx0,&dx1,&dy0,&dy1,&dz0,&dz1)==7 &&
                    (long long)dt==r.tick) {
