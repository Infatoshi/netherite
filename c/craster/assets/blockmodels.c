...
/* mc-sim CB_ / PB_ block-state ids (the set our worldgen + populate emit). The
 * feature ids 21+ come from mc-sim populate.h (PB_ ids); the CBX_ ids at 200+ are
 * craster-local synthetic ids used only by the non-cube mesh unit test (no
 * worldgen emits them). Kept mc-sim-header-free on purpose. */
enum {
    ...
    /* craster-local synthetic ids (mesh unit test only) */
