...
Run (anvil): cd c/craster && uv run --no-project --with numpy,pillow python rl/make_walk_videos.py
"""
import json
import os
import subprocess
...
from make_videos import (CRASTER, OUT, COLORS, CAM_W, CAM_H, SCALE,
...
def game_frames(script, ticks, tag, seed):
    ...
    cmd = [os.path.join(CRASTER, "craster_game"),
...
def cam_frames_from_actions(actions, seed, end):
    ...
    proc = subprocess.Popen(
        [os.path.join(CRASTER, "craster_game"), "--rl", "--render", "off",
