/* app/main.c - playable walkable/flyable voxel world for craster.
 *
...
static void chunk_load(CrWorld *w, int cx, int cz)
{
    ...
    if (!slot) {
        fprintf(stderr, "craster: chunk cache full, dropping (%d,%d)\n", cx, cz);
        return;
    }
    ...
    if (n > 0) {
        ...
        if (!slot->verts) {
            fprintf(stderr, "craster: OOM caching chunk (%d,%d)\n", cx, cz);
            slot->nverts = 0;
        } else {
...
static int write_ppm(const char *path, const CrFramebuffer *fb)
{
    ...
    if (!f) {
        fprintf(stderr, "craster: cannot open %s for writing\n", path);
        return -1;
    }
    fprintf(f, "P6\n%d %d\n255\n", fb->w, fb->h);
    ...
    fclose(f);
    if (wrote != n) {
        fprintf(stderr, "craster: short write to %s\n", path);
        return -1;
    }
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (fb_w < 1 || fb_h < 1 || radius < 1) {
        fprintf(stderr, "craster: bad dimensions/radius\n");
        return 2;
    }
    ...
    if (!fb.color || !fb.depth) {
        fprintf(stderr, "craster: cr_fb_alloc failed\n");
        return 1;
    }
    ...
    if (!g_mesh_scratch || !tris) {
        fprintf(stderr, "craster: scratch allocation failed\n");
        return 1;
    }
    ...
    if (!world) {
        fprintf(stderr, "craster: world_create failed\n");
        return 1;
    }
    ...
    CrWindow *win = cr_window_open(fb_w, fb_h, "craster - world");
    if (!win) {
        fprintf(stderr, "craster: cr_window_open failed\n");
        return 1;
    }
