...
static CrCamera camera_for(const GmPlayerView *v, int w, int h) {
    ...
    /* MC-degrees -> craster camera radians: the ONE pixel-verified mapping
...
static void terrain_shades(const CrTexture *atlas, CrRgba fog, int dimension,
     ...
     * (no mip chain). Sampling craster's mip chain would shift texel
...
GmFrameCapture *gm_frame_capture_open(const GmConfig *cfg, char *err, int err_cap) {
    ...
    if(c->use_cuda){
         ...
         * CRASTER_NO_DEVMESH is set. */
        if(cr_raster_cuda_slab_pool&&cr_raster_cuda_slab_sync&&
           cr_raster_cuda_render_gather&&!getenv("CRASTER_NO_DEVMESH")&&
        ...
        if(cr_raster_cuda_pin){
            ...
            /* deferred-frame readback target (CRASTER_NO_DEFER=1 disables) */
            if(cr_raster_cuda_frame_end_async&&cr_raster_cuda_frame_wait&&
               !getenv("CRASTER_NO_DEFER")){
...
static int finish_pending(GmFrameCapture *c) {
    ...
    if (!c->pend_v.dead && !getenv("CRASTER_NO_HAND"))
...
int gm_frame_capture_write(GmFrameCapture *c, GmRuntime *r,
    ...
    if(r->dimension==0&&c->use_cuda&&cr_raster_cuda_sky&&!getenv("CRASTER_CPU_SKY")){
     ...
     * CRASTER_NO_OVERLAY=1 for goldens that predate the overlay pass. */
    if(!getenv("CRASTER_NO_OVERLAY")&&!v.dead){
        ...
        if(have_dig && dmg>0.0f && !getenv("CRASTER_NO_CRACK")){
    ...
    if(v.loading==2){
        ...
        if(!v.dead&&!getenv("CRASTER_NO_HAND"))gm_hand_draw(&c->fb,&v,c->hand_bob);
        /* ItemRenderer.renderOverlays runs with the hand, BEFORE the HUD. */
