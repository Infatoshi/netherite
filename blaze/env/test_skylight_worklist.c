...
static int setup_env(Blaze *e, u16 **cells, u8 **light, Chunk **window,
                     int **light_q, int **sky_q, u8 **sky_clean, int rnx,
                     int rny, int rnz) {
    ...
    *sky_q = (int *)malloc((size_t)CU_SKY_Q * sizeof(int));
    nch = (long)CU_SEC_SPAN(rnx) * CU_SEC_SPAN(rnz);
    *sky_clean = (u8 *)malloc((size_t)nch);
...
static void apply_opacity(Blaze *live, Blaze *ref, int x, int y, int z,
    ...
    if (old_op != new_op) {
        cu_light_after_opacity(live, x, y, z, new_op < old_op);
...
int main(void) {
    ...
    static const int k_border[] = {0, 15, 16, 31, 32, 47};

    if (!setup_env(&live, &cells_l, &light_l, &win_l, &lq_l, &sq_l, &sc_l,
                   rnx, rny, rnz) ||
    ...
    memset(live.sky_clean, 1, (size_t)live.sky_cnx * (size_t)live.sky_cnz);
    copy_world(&ref, &live);
    ...
    /* chunk-border break/place: Fable item 3 invariant. */
    for (k = 0; k < 200; ++k) {
        int x = k_border[rng_next() % 6u];
        int z = k_border[rng_next() % 6u];
        int y = 8 + (int)(rng_next() % (unsigned)(rny - 16));
        int id = k_ids[rng_next() % (unsigned)(sizeof k_ids / sizeof k_ids[0])];
        char buf[64];
        apply_opacity(&live, &ref, x, y, z, id, 0);
        snprintf(buf, sizeof buf, "border %d (%d,%d,%d id=%d)", k, x, y, z, id);
        if (!light_eq(&live, &ref, buf))
            break;
    }
    if (k == 200)
        expect(1, "200 chunk-border break/place vs 15-pass");
    ...
    live.sky_clean = NULL;
    copy_world(&live, &ref);
    apply_opacity(&live, &ref, 16, 50, 16, BLK_AIR, 0);
    expect(light_eq(&live, &ref, "NULL sky_clean full path"),
           "NULL sky_clean vs 15-pass");
    live.sky_clean = sc_l;
