...
typedef struct {
    ...
    int   do_place;
    int   attack;
    /* Equipment-derived EntityLivingBase travel inputs. Keeping these on the
     * action avoids widening the hot PsvPlayer state shared by every batched
     * environment. water_ai_speed is read only when depth_strider > 0. */
    int   depth_strider;
    float water_ai_speed;
} PsvAction;
        ...
        for (int y = y0; y <= y1; ++y)
            for (int z = z0; z <= z1; ++z) {
                ...
                if (id == 78) {
                    ...
                    /* One layer returns a zero-height, full-XZ AABB rather
                     * than NULL_AABB. It can still clamp a downward move at
                     * exact face contact before horizontal resolution. */
                    psv_add_collision_box(blocks, &n, maxblocks,
                ...
                if (id == 198) {
                    int rod_meta = psv_get_meta(chunks, x, y, z) % 6;
...
MC_HD static inline void psv_physics_tick_effects(
    ...
    if (in_water || in_lava) {
        double d0 = e->posY;
        float water_drag = 0.8f;
        float liquid_accel = 0.02f;
        if (in_water && act->depth_strider > 0) {
            float depth = (float)act->depth_strider;
            if (depth > 3.0f) depth = 3.0f;
            if (!e->onGround) depth *= 0.5f;
            water_drag += (0.54600006f - water_drag) * depth / 3.0f;
            liquid_accel += (act->water_ai_speed - liquid_accel) * depth / 3.0f;
        }
        ppw_move_flying(st, e, act->yaw, strafing, forward, liquid_accel);
        ...
        e->motionX *= in_water ? (double)water_drag : 0.5;
        e->motionY *= in_water ? 0.800000011920929 : 0.5;
        e->motionZ *= in_water ? (double)water_drag : 0.5;
        e->motionY -= 0.02;
        if (e->collidedHorizontally &&
