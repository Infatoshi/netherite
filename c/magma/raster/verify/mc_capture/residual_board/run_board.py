...
Usage (repo root):
  uv run --no-project --with numpy --with pillow --with scipy \\
    python c/magma/raster/verify/mc_capture/residual_board/run_board.py
...
REPO = BOARD_DIR.parents[4]  # residual_board -> mc_capture -> verify -> raster -> magma -> c -> REPO? 
# Path: .../mc-1.11.2-env/c/magma/raster/verify/mc_capture/residual_board
# parents[0]=residual_board, [1]=mc_capture, [2]=verify, [3]=raster, [4]=magma, [5]=c, [6]=repo
REPO = BOARD_DIR.parents[6] if (BOARD_DIR.parents[5] / "c").exists() else BOARD_DIR.parents[5]
# Fix: board is under c/magma/... so parents[5] = c, parents[6] = repo
for p in BOARD_DIR.parents:
    if (p / "c" / "magma").is_dir() and (p / "java").is_dir():
        REPO = p
        break
