...
(`mc-1.11.2-env/c/render-opt/kernels/<NN_name>/candidate.c`). Each kernel's compute path is
...
(Run from the repo root `craster/` so `-I.` finds nothing else; `rk_*.c` include `"rk.h"`
