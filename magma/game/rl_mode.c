...
static int rl_probe_valid, rl_probe_x, rl_probe_y, rl_probe_z;
static int rl_gui_open;
...
#define RL_NATIVE_ITEMS_MAGIC 0x314d5449u /* "ITM1" little-endian */
typedef struct {
    int slot;
    GmLiveEnt entity;
} RlNativeItem;

static int rl_native_transient_path(
        char *out, size_t cap, const char *root, const char *slot) {
    int n = snprintf(out, cap, "%s/%s/rl_transient.bin", root, slot);
    return n > 0 && (size_t)n < cap;
}

static int rl_native_transient_write(
        const GmRuntime *r, const char *root, const char *slot) {
    char path[1024];
    RlNativeTransient value;
    FILE *stream;
    if (!rl_native_transient_path(path, sizeof path, root, slot)) return 0;
    memset(&value, 0, sizeof value);
    memcpy(value.magic, "NTHRRL01", 8);
    value.tick = r->tick;
    gm_player_ctl_dig_export(&value.ctl);
    ...
    if (fputc(rl_gui_open ? 1 : 0, stream) == EOF) {
        fclose(stream);
        return 0;
    }
    {
        unsigned magic = RL_NATIVE_ITEMS_MAGIC, count = 0;
        int i;
        ...
        for (i = 0; i < GM_LIVE_MAX; ++i) {
            ...
            if (!r->entities.ents[i].active ||
                ...
                continue;
            item.slot = i;
            item.entity = r->entities.ents[i];
            if (fwrite(&item, sizeof item, 1, stream) != 1) {
                fclose(stream);
                return 0;
            }
        }
...
static int rl_native_transient_load(
        GmRuntime *r, const char *root, const char *slot) {
    char path[1024];
    RlNativeTransient value;
    FILE *stream;
    if (!rl_native_transient_path(path, sizeof path, root, slot)
            || !(stream = fopen(path, "rb")))
        return 0;
    if (fread(&value, sizeof value, 1, stream) != 1
            || memcmp(value.magic, "NTHRRL01", 8) != 0
            || value.tick != r->tick)
        { fclose(stream); return 0; }
    {
        int gui = fgetc(stream);
        if (gui == EOF) {
            rl_gui_open = 0; /* backward-compatible pre-GUI checkpoint */
        } else {
            unsigned magic, count, i;
            rl_gui_open = gui != 0;
            if (fread(&magic, sizeof magic, 1, stream) != 1) {
                if (feof(stream)) goto transient_done; /* GUI-only v1 */
                fclose(stream); return 0;
            }
            if (magic != RL_NATIVE_ITEMS_MAGIC ||
                    fread(&count, sizeof count, 1, stream) != 1 ||
                    count > GM_LIVE_MAX) {
                fclose(stream); return 0;
            }
            for (i = 0; i < GM_LIVE_MAX; ++i) {
            ...
            for (i = 0; i < count; ++i) {
                RlNativeItem item;
                GmLiveEnt *entity;
                if (fread(&item, sizeof item, 1, stream) != 1 ||
                        item.slot < 0 || item.slot >= GM_LIVE_MAX ||
                        !item.entity.active || item.entity.type != 0) {
                    fclose(stream); return 0;
                }
                entity = &r->entities.ents[item.slot];
                if (entity->active) { fclose(stream); return 0; }
                *entity = item.entity;
                ++r->entities.n_active;
            }
            if (fgetc(stream) != EOF) { fclose(stream); return 0; }
        }
    }
transient_done:
    if (fclose(stream) != 0) return 0;
    gm_player_ctl_dig_import(&value.ctl);
    return 1;
}
...
static int rl_snapshot_write(GmRuntime *r, const char *path, int radius) {
    ...
    h.n_items = 0;
    for (i = 0; i < gm_live_entity_slot_count(&r->entities); ++i) {
        const GmLiveEnt *e = gm_live_entity_ref(&r->entities, i);
        RlSnapItem *it;
        if (!e) continue;
        if (!e->active || e->type != 0) continue;
        if (h.n_items >= BLAZE_SNAP_MAX_ITEMS) {
            fprintf(stderr,
                    "[rl] snapshot WRITE FAILED: %u+ item entities exceed "
                    "the shared blaze-v1 limit %d\n",
                    h.n_items + 1, BLAZE_SNAP_MAX_ITEMS);
            return 0;
        }
        it = &items[h.n_items];
...
static void rl_emit_obs(GmRuntime *r, int bin, int want_cam) {
    ...
    if (bin) {
        ...
        int dig_x, dig_y, dig_z;
        int ray_hx = 0, ray_hy = 0, ray_hz = 0;
        int ray_ax = 0, ray_ay = 0, ray_az = 0;
        float dig_progress;
        int dig_hitting = gm_player_dig_state(
            &dig_x, &dig_y, &dig_z, &dig_progress);
        int ray_hit = gm_raycast_sel_reach(
            r->window, &r->sin_table, &r->player, PSV_REACH,
            &ray_hx, &ray_hy, &ray_hz, &ray_ax, &ray_ay, &ray_az) >= 0;
        printf("{\"t\":%lld,\"world_time\":%lld,\"x\":%.6f,\"y\":%.6f,\"z\":%.6f,"
               "\"yaw\":%.4f,\"pitch\":%.4f,\"dead\":%d,"
               "\"dimension\":%d,\"health\":%.6f,\"food\":%d,"
               "\"fall_distance\":%.6f,\"fire_ticks\":%d,\"air\":%d,"
               "\"last_damage_source\":%d,\"on_ground\":%d,"
               "\"portal_time\":%d,\"portal_cooldown\":%d,"
               "\"portal_last_valid\":%d,"
               "\"portal_cache\":[%d,%d,%d,%d,%d,%d],"
               "\"dig\":[%d,%.9g,%d,%d,%d],"
               "\"ray\":[%d,%d,%d,%d,%d,%d,%d],",
               r->tick, (long long)r->clock.world_time,
               (double)v.x, (double)v.y, (double)v.z,
               (double)v.yaw, (double)v.pitch, v.dead, r->dimension,
               (double)r->vitals.health, r->vitals.foodLevel,
               (double)r->player.fall_distance, r->player_fire_ticks,
               r->player_air, gm_mobs_last_player_damage_source(),
               r->player.ent.onGround, r->portal_time,
               r->portal_cooldown, r->player_last_portal_pos_valid,
               r->portal_caches[0].count,
               r->portal_caches[0].count ?
                   r->portal_caches[0].entries[0].key_x : 0,
               r->portal_caches[0].count ?
                   r->portal_caches[0].entries[0].key_z : 0,
               r->portal_caches[0].count ?
                   r->portal_caches[0].entries[0].portal_x : 0,
               r->portal_caches[0].count ?
                   r->portal_caches[0].entries[0].portal_y : 0,
               r->portal_caches[0].count ?
                   r->portal_caches[0].entries[0].portal_z : 0,
               dig_hitting,
               (double)dig_progress, dig_x, dig_y, dig_z,
               ray_hit ? gm_world_block(
                   r->world, ray_hx + r->ox, ray_hy, ray_hz + r->oz) : 0,
               ray_hx + r->ox, ray_hy, ray_hz + r->oz,
               ray_ax + r->ox, ray_ay, ray_az + r->oz);
        if (rl_probe_valid) {
            printf("\"probe\":[%d,%d,%d,%d,%d],",
                   gm_world_block(r->world, rl_probe_x, rl_probe_y, rl_probe_z),
                   gm_world_meta(r->world, rl_probe_x, rl_probe_y, rl_probe_z),
                   rl_probe_x, rl_probe_y, rl_probe_z);
        } else {
            printf("\"probe\":[0,0,0,0,0],");
        }
        printf("\"hotbar_ids\":[");
        ...
        for (i = 0; i < 9; ++i)
            printf("%s%d", i ? "," : "",
                   isr_get_stack(&r->player.inv, i).meta);
        printf("],\"hotbar_sel\":%d,\"container\":%d,\"gui\":%d,"
               "\"inv_counts\":[",
               v.hotbar_sel, r->container, rl_gui_open);
        for (i = 0; i < 9; ++i)
        ...
        printf("],\"inventory_ids\":[");
        for (i = 0; i < ISR_MAIN_SLOTS; ++i) {
            ICStack stack = isr_get_stack(&r->player.inv, i);
            printf("%s%d", i ? "," : "", isr_is_empty(&stack) ? 0 : stack.item);
        }
        printf("],\"inventory_counts\":[");
        for (i = 0; i < ISR_MAIN_SLOTS; ++i) {
            ICStack stack = isr_get_stack(&r->player.inv, i);
            printf("%s%d", i ? "," : "", isr_is_empty(&stack) ? 0 : stack.count);
        }
        {
            ICStack cursor = gm_player_cursor();
            printf("],\"cursor\":[%d,%d,%d],\"craft_grid\":[",
                   isr_is_empty(&cursor) ? 0 : cursor.item,
                   isr_is_empty(&cursor) ? 0 : cursor.count,
                   isr_is_empty(&cursor) ? 0 : cursor.meta);
        }
        for (i = 0; i < 9; ++i)
            printf("%s[%d,%d,%d]", i ? "," : "",
                   isr_is_empty(&r->craft_grid[i]) ? 0 : r->craft_grid[i].item,
                   isr_is_empty(&r->craft_grid[i]) ? 0 : r->craft_grid[i].count,
                   isr_is_empty(&r->craft_grid[i]) ? 0 : r->craft_grid[i].meta);
        {
            ICStack result = gm_container_result(r);
            printf("],\"craft_result\":[%d,%d,%d]",
                   isr_is_empty(&result) ? 0 : result.item,
                   isr_is_empty(&result) ? 0 : result.count,
                   isr_is_empty(&result) ? 0 : result.meta);
        }
        printf(",\"living_count\":%d,\"hostile_count\":%d,\"mobs\":[",
               gm_mobs_living_count(&r->mobs), gm_mobs_alive(&r->mobs));
        {
            const EwStore *store = r->mobs.current ? &r->mobs.b : &r->mobs.a;
            int first = 1;
            for (i = 1; i < store->count; ++i) {
                if (!store->alive[i]) continue;
                printf("%s[%d,%d,%.6f,%.6f,%.6f,%.6f]", first ? "" : ",",
                       (int)store->type[i], store->id[i], store->x[i], store->y[i],
                       store->z[i], (double)store->health[i]);
                first = 0;
            }
            for (i = 0; i < r->mobs.living_cold_count; ++i) {
                const GmLivingColdSlot *cold = &r->mobs.living_cold[i];
                if (!cold->store_alive
                        || cold->entity_dimension != r->dimension)
                    continue;
                printf("%s[%d,%d,%.6f,%.6f,%.6f,%.6f]",
                       first ? "" : ",", (int)cold->store_type,
                       cold->store_id, cold->store_x, cold->store_y,
                       cold->store_z, (double)cold->store_health);
                first = 0;
            }
        }
        printf("],\"items\":[");
        ...
        {
            int first = 1;
            for (i = 0; i < r->projectiles_cap; ++i) {
                const GmRuntimeProjectile *p = &r->projectiles[i];
                if (!p->active || p->dimension != r->dimension) continue;
        ...
        printf("],\"lava_sources\":[");
        {
            ...
            for (i = 0; i < nblk && emitted < 64; ++i) {
                if ((blk[i].id != 10 && blk[i].id != 11)
                        || gm_world_meta(
                            r->world, blk[i].x, blk[i].y, blk[i].z) != 0)
                    continue;
                printf("%s[%d,%d,%d]", first ? "" : ",",
                       blk[i].x, blk[i].y, blk[i].z);
                first = 0;
                ++emitted;
            }
        }
        printf("],\"water_sources\":[");
        {
            int first = 1, emitted = 0;
            for (i = 0; i < nblk && emitted < 64; ++i) {
                if ((blk[i].id != 8 && blk[i].id != 9)
                        || gm_world_meta(
                            r->world, blk[i].x, blk[i].y, blk[i].z) != 0)
                    continue;
                printf("%s[%d,%d,%d]", first ? "" : ",",
...
int gm_rl_run(const GmConfig *cfg) {
    ...
    /* Full native saves are the restart boundary for long, stateful recovery
     * jobs. Unlike the compact .bsnp search snapshots, these preserve every
     * runtime subsystem (containers/furnaces, entities, dimensions and world
     * clock), so splitting a run across processes does not weaken the oracle.
     * The action tape remains ordinary input: save_slot is a wire-only harness
     * command and is deliberately applied after that action's tick. */
    {
        const char *load_slot = getenv("MAGMA_RL_LOAD_SLOT");
        const char *save_root = getenv("MAGMA_NATIVE_WORLD_ROOT");
        if (load_slot && *load_slot) {
            if (!save_root || !*save_root) save_root = "saves";
            if (!gm_native_save_load(&r, cfg, save_root, load_slot,
                                     err, sizeof err)) {
                fprintf(stderr, "native-save-in: %s\n", err);
                gm_runtime_destroy(&r);
                return 1;
            }
            if (!rl_native_transient_load(&r, save_root, load_slot)) {
                fprintf(stderr, "native-save-in: missing or stale RL transient\n");
                gm_runtime_destroy(&r);
                return 1;
            }
        }
    ...
    for (t = 0; cfg->ticks < 0 || t < cfg->ticks; ++t) {
        ...
        a.do_break = (int)rl_num(line, "do_break", 0);
        a.do_place = (int)rl_num(line, "do_place", 0);
        a.close_container = (int)rl_num(line, "close_container", 0);
        if ((int)rl_num(line, "open_inventory", 0)) rl_gui_open = 1;
        if (a.close_container) rl_gui_open = 0;
        a.hotbar_sel = (int)rl_num(line, "hotbar", -1);
        idle_ticks = (int)rl_num(line, "idle_ticks", 0);
        if (idle_ticks < 0 || idle_ticks > 100000) idle_ticks = 0;
        {
            int inv_slot = (int)rl_num(line, "inv_slot", -10000);
            if (inv_slot != -10000) {
                a.inv_click = 1;
                a.inv_slot = inv_slot;
        ...
        a.death_click = (int)rl_num(line, "death_click", 0);
        a.death_button = (int)rl_num(line, "death_button", 0);
        {
            int probe_x = (int)rl_num(line, "probe_x", -10000000);
            rl_probe_valid = probe_x != -10000000;
            if (rl_probe_valid) {
                rl_probe_x = probe_x;
                rl_probe_y = (int)rl_num(line, "probe_y", 0);
                rl_probe_z = (int)rl_num(line, "probe_z", 0);
            }
        }
        if (a.attack || a.use)
        ...
        }
        {
            ...
            if ((int)rl_num(line, "interact", 0) && rl_do_interact(&r))
                rl_gui_open = 1;
        ...
        }
        {
            ...
            if (idle_ticks > 0) {
                ...
                idle.hotbar_sel = -1;
                for (repeat = 0; repeat < idle_ticks; ++repeat)
                    gm_runtime_tick(&r, idle);
            } else {
                gm_runtime_tick(&r, a);
            }
            if (r.container != 0) rl_gui_open = 1;
            rl_prof[0] += rl_now() - t0;
        }
        {
            char save_slot[GM_NATIVE_SAVE_SLOT_MAX + 1];
            const char *save_root = getenv("MAGMA_NATIVE_WORLD_ROOT");
            if (!save_root || !*save_root) save_root = "saves";
            if (rl_str(line, "save_slot", save_slot, sizeof save_slot)) {
                err[0] = '\0';
                if (!gm_native_save_write(&r, save_root, save_slot,
                                          err, sizeof err)
                        || !rl_native_transient_write(
                            &r, save_root, save_slot)) {
                    fprintf(stderr, "native-save-out: %s\n",
                            err[0] ? err : "RL transient write failed");
                    break;
                }
            }
        }
        if (frames) {
            if (rl_gui_open)
                gm_runtime_gui_view(&r, r.container, 0, 0);
            else
