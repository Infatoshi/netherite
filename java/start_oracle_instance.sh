...
POOL_OUT_ROOT="${ORACLE_POOL_OUT_ROOT:-$REPO/magma/trace/out/oracle_pool}"
OUT="$POOL_OUT_ROOT/instance_$INSTANCE"
...
case "$ACTION" in
    status)
        if instance_alive; then
            if port_ready && port_owned_by_instance; then
                state=ready
...
if port_ready; then
    owner="$(port_owner_pid)"
    echo "refusing to start instance $INSTANCE: port $PORT is owned by pid ${owner:-unknown}, not this instance" >&2
    exit 1
fi
mkdir -p "$OUT/run"
...
jq --argjson port "$PORT" --argjson seed "$SEED" \
   ...
   --arg strip_menus "${ORACLE_POOL_STRIP_MENUS:-}" \
   --arg strip_overlays "${ORACLE_POOL_STRIP_OVERLAYS:-}" \
   --arg mob_spawning "${ORACLE_POOL_MOB_SPAWNING:-}" \
   --arg save_source "${ORACLE_POOL_SAVE_SOURCE:-}" \
   '.port=$port | .world.seed=$seed | .profile="oracle-pool"
    ...
    | if $strip_menus != "" then
        .strip.menus=($strip_menus == "1" or $strip_menus == "true")
      else . end
    | if $strip_overlays != "" then
        .strip.overlays=($strip_overlays == "1" or $strip_overlays == "true")
      else . end
    | if $mob_spawning != "" then
        .world.gamerules.doMobSpawning=
          (if $mob_spawning == "1" or $mob_spawning == "true"
           then "true" else "false" end)
      else . end
    | if $save_source != "" then
        .world.preserve_loaded_state=true | .world.start_locked=true
        | .world.pin_reload_rng=true
      else . end' \
    "$BASE_CONFIG" >"$CONFIG"
cp "$CONFIG" "$OUT/run/qrl_launch.json"

if [ -n "${ORACLE_POOL_SAVE_SOURCE:-}" ]; then
    SAVE_SOURCE="$(readlink -f "$ORACLE_POOL_SAVE_SOURCE")"
    [ -f "$SAVE_SOURCE/level.dat" ] || {
        echo "oracle save source has no level.dat: $SAVE_SOURCE" >&2
        exit 1
    }
    if find "$SAVE_SOURCE" -type l -print -quit | grep -q .; then
        echo "oracle save source contains a symlink: $SAVE_SOURCE" >&2
        exit 1
    fi
    SAVE_SUFFIX=""
    [ "$(jq -r '.world.type // ""' "$CONFIG")" = flat ] && SAVE_SUFFIX="_flat"
    SAVE_DEST="$OUT/run/saves/qrl_${SEED}${SAVE_SUFFIX}"
    if [ -e "$SAVE_DEST" ]; then
        echo "refusing to replace existing oracle save: $SAVE_DEST" >&2
        exit 1
    fi
    mkdir -p "$SAVE_DEST"
    cp -a "$SAVE_SOURCE/." "$SAVE_DEST/"
fi
...
export QRL_AUDIO_SELECTOR_SEED="${QRL_AUDIO_SELECTOR_SEED:-0x4e65746865726974}"
ORACLE_USERNAME="${ORACLE_POOL_USERNAME:-PoolPlayer$INSTANCE}"
export MC_USERNAME="$ORACLE_USERNAME"
...
(
    ...
    setsid nohup ./gradlew "${gradle_args[@]}" \
        -PrunDir="$OUT/run" -PqrlPort="$PORT" \
        -PmcUsername="$ORACLE_USERNAME" runClient --stacktrace \
...
if [ "${ORACLE_POOL_WAIT:-1}" = 1 ]; then
    for _ in $(seq 1 420); do
        if port_ready; then
            if port_owned_by_instance; then
                echo "instance=$INSTANCE ready port=$PORT"
                exit 0
            fi
            owner="$(port_owner_pid)"
            stop_instance
            echo "instance $INSTANCE found foreign listener pid ${owner:-unknown} on port $PORT" >&2
            exit 1
        fi
