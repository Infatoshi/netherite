...
static ICStack tec_to_ic(TecStack t)
{
    ...
    s.repair_cost = t.repair_cost;
    s.custom_name = t.custom_name;
    s.tag_id = t.tag_id;
    s.n_enchants = n;
    for (i = 0; i < n; ++i) {
...
static TecStack ic_to_tec(ICStack s)
{
    ...
    t.repair_cost = s.repair_cost;
    t.custom_name = s.custom_name;
    t.tag_id = s.tag_id;
    t.n_enchants = n;
    for (i = 0; i < n; ++i) {
...
void chest_live_ensure_loot_luck(ChestLive *c, float luck)
{
    ...
    if (c->loot_table >= 0)
        shl_fill_chest_luck(
            &c->te, c->loot_table, (i64)c->loot_seed, luck);
    c->loot_filled = 1;
}

void chest_live_ensure_loot(ChestLive *c)
{
    chest_live_ensure_loot_luck(c, 0.0f);
}
...
{
    if (!c) return;
    chest_live_ensure_loot_luck(c, luck);
    tec_open(&c->te);
}
