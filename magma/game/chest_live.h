 ...
 * Ender Chest reuses the slot arithmetic for its player-owned inventory; its
 * tile animation is separate. Lid angle feeds the per-frame chest TESR path. */
...
enum {
    ...
    /* represented vanilla generated-container loot tables */
    CHEST_LOOT_NONE     = -1,
    ...
    CHEST_LOOT_IGLOO = 10,
    CHEST_LOOT_WOODLAND_MANSION = 11
};
...
/* Player-aware TileEntityLockableLoot.fillWithLoot boundary. */
void chest_live_ensure_loot_luck(ChestLive *c, float luck);
...
void chest_live_open_luck(ChestLive *c, float luck);
