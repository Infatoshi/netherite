...
/* Warp-cooperative randtick. nlanes==1 is the serial pass. nlanes==32:
 * lane 0 still consumes World.rand (prefix is sequential), then the 16
 * sections of each in-region chunk ballot the live census, skip-ahead
 * updateLCG in closed form, and load the three picked cells in parallel.
 * Lane 0 applies handlers in Java section order, re-reading a pick only
 * when an earlier handler in this chunk could have written it. A census
 * flip vs the ballot (grass spreading into an empty later section) falls
 * back to serial LCG+tick for the rest of that chunk. */
MC_HD MC_NOINLINE static inline void cu_randtick_pass_lanes(Blaze *e, int lane,
                                                            int nlanes) {
    if (!e || !e->light_valid) return;
    if (nlanes <= 1) {
        if (lane == 0) cu_randtick_pass(e);
        return;
    }
#if !defined(__CUDA_ARCH__)
    (void)lane;
...
#else
    {
        ...
        int radius = 8;
        int cx, cz, ccx, ccz, raining, thundering;
        McGameRules gr;
        gr = mc_gamerules_default();
        raining = e->weather_enabled ? e->ww.raining : 0;
        thundering = e->weather_enabled ? e->ww.thundering : 0;
        ...
        for (cx = ccx - radius; cx <= ccx + radius; ++cx) {
            for (cz = ccz - radius; cz <= ccz + radius; ++cz) {
                int my_need = 0;
                int my_wx[3], my_wy[3], my_wz[3], my_id[3];
                int att, sec, serial, nseen;
                unsigned needmask;
                unsigned long long t0 = 0;
                u32 chunk_lcg;

                my_wx[0] = my_wx[1] = my_wx[2] = 0;
                my_wy[0] = my_wy[1] = my_wy[2] = 0;
                my_wz[0] = my_wz[1] = my_wz[2] = 0;
                my_id[0] = my_id[1] = my_id[2] = 0;

                if (lane == 0) {
                    CU_OP(e, CU_OP_RT_CHUNK);
                    {
                        CU_PHASE_T0(e);
                        rt_live_chunk_worldrand_prefix(
                            e, &e->world_rand, &e->update_lcg, raining,
                            thundering, cx * 16, cz * 16);
                        CU_PHASE_END(e, CU_PHASE_RT_PREFIX);
                    }
                }
                __syncwarp();

                if (!cu_rt_chunk_may_need(e, cx, cz)) continue;

                if (lane == 0 && e->phase) t0 = cu_clk64();
                if (lane < 16)
                    my_need = cu_rt_section_needs(e, cx, lane, cz) ? 1 : 0;
                needmask = __ballot_sync(FULL, my_need);
                if (lane == 0) {
                    CU_OP_ADD(e, CU_OP_RT_SEC_CHECK, 16);
                    CU_OP_ADD(e, CU_OP_RT_SEC_NEED, __popc(needmask));
                    if (e->phase)
                        e->phase[CU_PHASE_RT_SEC] += cu_clk64() - t0;
                }

                chunk_lcg = (u32)e->update_lcg;
                if (lane == 0 && e->phase) t0 = cu_clk64();
                if (my_need) {
                    int before = (int)__popc(needmask & ((1u << lane) - 1u));
                    int base_y = lane * 16;
                    for (att = 0; att < 3; ++att) {
                        u32 stt = rt_live_lcg_skip(
                            chunk_lcg, (u32)(3 * before + att + 1));
                        i32 j1 = (i32)stt >> 2;
                        long ri;
                        my_wx[att] = cx * 16 + (j1 & 15);
                        my_wz[att] = cz * 16 + ((j1 >> 8) & 15);
                        my_wy[att] = base_y + ((j1 >> 16) & 15);
                        ri = cu_region_idx(e, my_wx[att], my_wy[att],
                                           my_wz[att]);
                        my_id[att] = ri < 0 ? 0 : mc_state_id(e->cells[ri]);
                    }
                }
                __syncwarp();
                if (lane == 0 && e->phase)
                    e->phase[CU_PHASE_RT_PICK] += cu_clk64() - t0;

                serial = 0;
                nseen = 0;
                {
                    int have_box = 0, bx0 = 0, bx1 = 0, by0 = 0, by1 = 0,
                        bz0 = 0, bz1 = 0;
                    if (lane == 0 && e->phase) t0 = cu_clk64();
                    for (sec = 0; sec < 16; ++sec) {
                        int live = 0, snap, diverge;
                        if (lane == 0)
                            live = cu_rt_section_needs(e, cx, sec, cz) ? 1 : 0;
                        live = __shfl_sync(FULL, live, 0);
                        snap = (int)((needmask >> sec) & 1u);
                        diverge = serial || (live != snap);
                        serial = diverge;
                        if (diverge) {
                            if (lane == 0 && live) {
                                int base_y = sec * 16;
                                for (att = 0; att < 3; ++att) {
                                    i32 j1 = rt_live_step_lcg(&e->update_lcg);
                                    int wx = cx * 16 + (j1 & 15);
                                    int wz = cz * 16 + ((j1 >> 8) & 15);
                                    int wy = base_y + ((j1 >> 16) & 15);
                                    CU_OP(e, CU_OP_RT_LCG);
                                    rt_live_tick_block(e, wx, wy, wz,
                                                       &e->world_rand, &gr,
                                                       e->rt_leaf, raining);
                                }
                            }
                        ...
                        if (!snap) continue;
                        for (att = 0; att < 3; ++att) {
                            int wx = __shfl_sync(FULL, my_wx[att], sec);
                            int wy = __shfl_sync(FULL, my_wy[att], sec);
                            int wz = __shfl_sync(FULL, my_wz[att], sec);
                            int id = __shfl_sync(FULL, my_id[att], sec);
                            if (lane == 0) {
                                int in_box = have_box && wx >= bx0 &&
                                             wx <= bx1 && wy >= by0 &&
                                             wy <= by1 && wz >= bz0 &&
                                             wz <= bz1;
                                (void)rt_live_step_lcg(&e->update_lcg);
                                CU_OP(e, CU_OP_RT_LCG);
                                if (in_box)
                                    id = cu_world_block(e, wx, wy, wz);
                                else
                                    CU_OP(e, CU_OP_RT_LOOKUP);
                                if (bp_is_randtick_id(id)) {
                                    rt_live_tick_block(e, wx, wy, wz,
                                                       &e->world_rand, &gr,
                                                       e->rt_leaf, raining);
                                    if (!have_box) {
                                    ...
                                    if (id == BLK_GRASS ||
                                        id == RT_BLK_MYCELIUM) {
                                        if (wx - 1 < bx0) bx0 = wx - 1;
                                        if (wx + 1 > bx1) bx1 = wx + 1;
                                        if (wy - 3 < by0) by0 = wy - 3;
                                        if (wy + 1 > by1) by1 = wy + 1;
                                        if (wz - 1 < bz0) bz0 = wz - 1;
                                        if (wz + 1 > bz1) bz1 = wz + 1;
                                    } else if (id == RT_BLK_FIRE) {
                                        bx0 = e->rx0;
                                        bx1 = e->rx0 + e->rnx;
                                        by0 = 0;
                                        by1 = 255;
                    ...
                    (void)nseen;
                    if (lane == 0 && e->phase)
                        e->phase[CU_PHASE_RT_HANDLER] += cu_clk64() - t0;
                }
            }
        }
    }
