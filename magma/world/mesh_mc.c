...
static unsigned char g_source_section;
...
static int fancy_leaf_key(int cb) {
    return cr_cfg()->fancy_leaves
...
static void push_face(CrChunkMeshMC *out, int *cap, int layer, const CrVertex *quad) {
    ...
    if (out->quad_sections[layer])
        out->quad_sections[layer][out->nverts[layer] / 6] = g_source_section;
...
/* models/block/beacon.json. No face carries cullface, so all 18 quads remain
 * even beside an opaque neighbor. ambientocclusion=false disables only AO;
 * vanilla still applies the ordinary directional face shade. */
static void emit_beacon(CrChunkMeshMC *out, int *cap, const BmBlock *m,
                        int wx, int wy, int wz, CrRgba tint, float base01) {
    static const float from[3][3] = {
        {0.0f, 0.0f, 0.0f},
        {2.0f, 0.1f, 2.0f},
        {3.0f, 3.0f, 3.0f},
    };
    static const float to[3][3] = {
        {16.0f, 16.0f, 16.0f},
        {14.0f, 3.0f, 14.0f},
        {13.0f, 14.0f, 13.0f},
    };
    static const float uv[3][2][4] = {
        {{0.0f, 0.0f, 16.0f, 16.0f}, {0.0f, 0.0f, 16.0f, 16.0f}},
        {{2.0f, 2.0f, 14.0f, 14.0f}, {2.0f, 13.0f, 14.0f, 16.0f}},
        {{3.0f, 3.0f, 13.0f, 13.0f}, {3.0f, 2.0f, 13.0f, 13.0f}},
    };
    const int sprite[3] = {
        m->face[BM_DOWN].sprite,
        m->face[BM_UP].sprite,
    ...
    for (int element = 0; element < 3; ++element)
        for (int face = 0; face < 6; ++face)
            emit_model_face(
                out, cap, m->layer, wx, wy, wz,
                from[element], to[element], face,
                uv[element][face >= BM_NORTH], 0, 3, 0.0f, NULL, 0,
                sprite[element], tint, base01, 1, 0);
}
...
static int fence_connects(const CrLight *L, int wx, int wy, int wz,
                          int fence_cb) {
    int cb = light_block(L, wx, wy, wz);
    const BmBlock *m = bm_block(cb);
    if (m->kind == BM_KIND_FENCE)
        return (fence_cb == 232) == (cb == 232);
    /* BlockFence.canConnectTo excludes pumpkin/melon despite their opaque,
     * full-cube state.  Model keys 67..70 are the four pumpkin facings; 87 is
     * melon. */
    if ((cb >= 67 && cb <= 70) || cb == 87) return 0;
    return m->is_full_cube && m->layer == CR_LAYER_SOLID;
}
...
static void emit_noncube(CrChunkMeshMC *out, int *cap, const CrLight *L,
    ...
    float base01 = m->kind == BM_KIND_FLUID ? 1.0f :
        (((m->kind == BM_KIND_CROSS && cb != CR_CB_WEB) ||
         ...
         m->kind == BM_KIND_TRAPDOOR || m->kind == BM_KIND_LADDER ||
         m->kind == BM_KIND_CACTUS || m->kind == BM_KIND_BEACON)
    ...
    switch (m->kind) {
        ...
        case BM_KIND_FENCE: {
            ...
            int connected[4] = {
                fence_connects(L, wx, wy, wz - 1, cb),
                fence_connects(L, wx, wy, wz + 1, cb),
                fence_connects(L, wx - 1, wy, wz, cb),
                fence_connects(L, wx + 1, wy, wz, cb),
            };
        ...
        case BM_KIND_LADDER:
            ...
            break;
        case BM_KIND_BEACON:
            emit_beacon(out, cap, m, wx, wy, wz, tint, base01);
            break;
    ...
    for (int lx = 0; lx < 16; ++lx) {
        for (int lz = 0; lz < 16; ++lz) {
            ...
            for (int wy = 0; wy < 256; ++wy) {
                g_source_section = (unsigned char)(wy >> 4);
                ...
                int render_layer = fancy_leaf_key(cb)
                    ? CR_LAYER_CUTOUT_MIPPED : m->layer;
                int self_translucent = (render_layer == CR_LAYER_TRANSLUCENT);
                ...
                for (int f = 0; f < 6; ++f) {
                    ...
                    int neigh_opaque = nm->is_full_cube
                        && nm->layer == CR_LAYER_SOLID
                        && !fancy_leaf_key(ncb);
                    ...
                    push_face(out, cap, render_layer, quad);
