...
enum {
    POPMC_IGLOO_RESIDENT_VILLAGER = 1,
    POPMC_IGLOO_RESIDENT_ZOMBIE_VILLAGER = 2
};
typedef struct {
    double x, y, z, vx, vy, vz;
    float health, yaw, pitch;
    int conversion_time, fire, air, profession;
    int kind, persistence, on_ground;
} PopmcIglooResident;
...
typedef struct {
    double x, y, z, vx, vy, vz;
    float health, yaw, pitch;
    int fire, air, persistence, on_ground;
} PopmcSwampWitch;
/* Copy unique generated hut witches in the inclusive X/Z bounds. Static
 * constructor/structure fields are returned at the pre-first-tick boundary. */
...
/* Decoration's population Random cursor immediately before
 * WorldEntitySpawner.performWorldGenSpawning. Runtime consumes these batches
 * in build order so initial animals are entities, not a block-only worldgen
 * cut. `after_seq` is exclusive; returned rows are ascending by seq. */
typedef struct {
    long seq;
    int bcx, bcz, biome;
    uint64_t population_seed48;
} PopmcWorldgenSpawnBatch;
int popmc_worldgen_spawn_batches(
    long long seed, long after_seq, PopmcWorldgenSpawnBatch *out, int capacity);
...
/* Live stage-one sapling generation reuses the verified population tree
 * kernels through a canonical-state callback seam. The caller ensures the
 * surrounding chunks first and finishes the returned bulk writes with one
 * relight. Returns the number of final raw states changed, or -1 on setup
 * failure. `seed48` is java.util.Random's internal 48-bit cursor.
 * `source_state` is Java's stale BlockSapling callback state; the live source
 * cell may already be air after unsupported-bush teardown. */
typedef uint16_t (*PopmcSaplingGetState)(
    void *opaque, int x, int y, int z);
typedef void (*PopmcSaplingSetState)(
