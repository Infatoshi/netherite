# diffbreak/breakdown.py - decompose the remaining craster-vs-real-MC terrain-crop
# error into measurable causes. Reads the checked-in goldens and the HEAD craster
...
def region_stats(mask, g, c, top=5):
    ...
    for pix in regions[:top]:
        ...
        out.append({
            ...
            "craster_rgb": tuple(np.round(c[sel].mean(axis=0)).astype(int)),
        })
    return out
...
for i in range(4):
    ...
    c = crop(load(f"{CAP}/craster_pose_{i}.png"))
    d = np.abs(g - c)
    dmean = d.mean()
    ...
    # luma vs chroma: chroma = residual after scaling craster pixel to golden luma
    lg, lc = luma(g), luma(c)
    ...
    for r in region_stats(mask, g, c):
        print(f"    area {r['area']:6d}  bbox {r['bbox']}  golden {r['golden_rgb']}  craster {r['craster_rgb']}")
