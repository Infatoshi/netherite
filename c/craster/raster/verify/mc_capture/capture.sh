...
# fixed camera pose) to serve as the rung-4 whole-frame golden that craster's
...
# --- FIXED aerial pose, MATCHED to the rung-3/rung-4 craster ChunkScene camera ---
# craster renders the seed-0 3x3-chunk-around-origin scene from world eye
# (POS_X, POS_EYE_Y, POS_Z) with craster yaw 0 / pitch -35deg (toward -Z, tilted
# down). See rung4_candidate.c: it PRINTS this exact resolved pose. The MC camera
# convention that reproduces craster forward is yaw 180 / pitch +35 (MC: yaw 180
...
YAW=180           # MC heading for craster forward -Z
PITCH=35          # MC pitch (down) for craster pitch -35
...
# hand/hotbar drawn - closer to craster which draws neither). Absolute tp to the
# ChunkScene eye. Do NOT flood water (craster's world is clean).
setup = [
...
p.update({
    ...
    "craster_yaw_deg": cam.get("craster_yaw_deg"),
    "craster_pitch_deg": cam.get("craster_pitch_deg"),
