 ...
 * ground) so the craster frame and the MC frame are directly comparable. The one
 ...
 *
 * MC vs craster camera convention (see core/math.c header + capture.sh):
 *   craster forward = (-sin(yaw)cos(pitch), sin(pitch), -cos(yaw)cos(pitch));
...
int main(int argc, char **argv) {
    ...
    float mc_pitch = -scn.cam.pitch * (180.0f/pi);  /* craster -35 -> MC +35 (down) */
