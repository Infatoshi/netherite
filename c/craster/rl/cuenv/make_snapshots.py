#!/usr/bin/env python3
"""Bake .bsnp training snapshots for the batched env (cuenv).
...
FLAGGED - cuenv does not simulate the fluids CA, so flagged (seed,stage)
...
Run (anvil):
  cd c/craster && uv run --no-project --with numpy,torch,matplotlib \
      python rl/cuenv/make_snapshots.py
  cd c/craster && T0=1 uv run --no-project --with numpy \
      python rl/cuenv/make_snapshots.py
...
RL = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # c/craster/rl
...
GAME_BIN = os.path.join(CRASTER, "craster_game")

# BOLR record size (packed, see game/rl_mode.c RlBinObs) - t0 mode reads raw
# records from craster_game --rl-bin without importing verify_cpu.
BIN_SIZE = sum(sz for sz in (4, 8, 8, 8, 8, 4, 4, 4, 36, 36, 4, 4, 36,
                             256 * 16, 64 * 12, 32 * 12,
                             2304 * 2, 2304, 2304))

# .bsnp header layout (rl/cuenv/cuenv_snapshot.h): packed, little-endian.
HEAD_SIZE = 752
N_ITEMS_OFF = 724          # u32
