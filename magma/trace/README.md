...
generates canonical-metadata, `Material.isOpaque`, `Material.isSolid`,
`getCanBurn`, `blocksMovement`, `causesSuffocation`, and note-block
instrument-material masks from the live Java registry; the capsule pins
capture digest
...
The focused vine neighbor gate drives real flag-3 support edits while the
server is parked, then compares the complete 5x4x5 result volume and all global
cursors against the native runtime:

```bash
make -C magma game/test_vine_neighbor_oracle
uv run --no-project python magma/trace/test_vine_neighbor.py \
  --port 25600 --native magma/game/test_vine_neighbor_oracle
```

Its eight rows cover direct support loss, partial metadata pruning, inherited
faces, mismatched inheritance, metadata-zero removal, and recursive columns.
The fixture uses no world tick, so unrelated loaded-chunk work cannot make a
support callback appear to consume RNG or entity IDs.

The focused leaf gate drives direct `BlockLeaves.updateTick` callbacks and
ordinary flag-3 log/leaf removals at the same parked boundary:

```bash
make -C magma game/test_leaf_decay_oracle
uv run --no-project python magma/trace/test_leaf_decay.py \
  --port 25600 --native magma/game/test_leaf_decay_oracle
```

Its 13 rows compare an 11x11x11 volume, complete spawned item state, and all
World, Math, Block, and entity-ID cursors. They distinguish distance-four
face connectivity from distance-five and diagonal-only rejection, pin each
represented sapling/apple drop branch, retain nondecayable leaves, exercise
exact log/leaf break-mark radii, and prove decay-driven neighbor notification.

The focused sky-column gate drives ordinary edits from an exact all-15 saved
sky-light boundary and compares blocks, sky light, delayed queues, and global
cursors without advancing the server tick:

```bash
make -C magma game/test_sky_light_column_oracle
uv run --no-project python magma/trace/test_sky_light_column.py \
  --port 25600 --native magma/game/test_sky_light_column_oracle
```

Its 13 rows cover opaque addition/removal, leaf and water attenuation,
transparent glass, stacked sequential water edits, stone/water overhangs, and
stone/water changes at local x=15. Locked transition tables reject vacuous or
contaminated fixtures, including missing static-to-flowing water schedules.

The focused static-lava gate stages a cold neighbor boundary and compares all
125 raw states, exact delayed queues, sound float bits, eight smoke-position
double payloads, and World, Math, Block, and entity-ID cursors:

```bash
make -C magma game/test_static_lava_neighbor_oracle
uv run --no-project python magma/trace/test_static_lava_neighbor.py \
  --port 25600 --native magma/game/test_static_lava_neighbor_oracle
```

Its 58 rows cover nonmixing levels 0, 4, 5, and 15; static and flowing water;
water above, north, west, and exclusively below; both solid outcomes; the
level-greater-than-four fallback; two simultaneous static-water wakeups; and
the exact Nether 10 versus Overworld/End 30 tick cadence. Nine direct dynamic
rows lock Nether/End decay one/two, cadence 10/30, queue order, a shaped-floor
discriminator for the four-step/two-step slope-search split, exact
`nextInt(4)` +40/+120 slowdowns, and Forge's default no-source result with two
lava-source neighbors. Nether/End downward air-flow rows lock falling level
eight and exact child/source queue order. Two horizontal fire-replacement rows
lock the resulting lava level, exact extinguish effects, queues, and RNG state.
Six water rows cover horizontal static/flowing replacement and downward stone
reaction in both dimensions, including source-to-obsidian reentrancy, two
effect streams, static water's stale +5 queue, and overlapping-CA retirement.
Six Nether rows replace valid sapling, web, floor torch, snow layer, wall vine,
and carpet states; a supported ladder row locks the explicit blocked exception.
Nine more rows cover ordinary rail, powered rail, redstone wire, tall grass,
wheat, dead bush, brown mushroom, lit redstone torch, and valid supported
reeds. They distinguish powered rail's center-before-child queue order, the
dead-bush support's two-tick falling-sand callback, and reeds' explicit blocked
exception. Ten common-plant rows add both flowers, red mushroom, pumpkin/melon
stems, waterlily, nether wart, carrots, potatoes, and beetroot. The waterlily
row also locks support water's static-to-flowing conversion and +5 queue ahead
of the child/source lava callbacks.
Use `--native-only` for the fast offline regression embedded in `test-game`.

The focused plant-and-attachment support gate removes each support block
through the real flag-three neighbor path and compares a 3x4x3 volume,
complete EntityItem descriptors, and all controlled global cursors without
advancing a world tick:

```bash
make -C magma game/test_plant_support_neighbor_oracle
uv run --no-project python magma/trace/test_plant_support_neighbor.py \
  --port 25600 --native magma/game/test_plant_support_neighbor_oracle
```

Its 65 rows cover the ordinary plants, every snow-layer metadata, every carpet
color, every canonical cocoa state, and ten support-preservation/removal
controls. The comparator locks exact drop metadata, item
position/motion/pickup delay, synchronous collapse order, World/Math/Block
RNG, and entity-ID consumption. Real mode leaves a short client-drain boundary
between item-producing rows because the integrated client and server share
`Entity.nextEntityID` and `Math.random`; otherwise a replicated client item can
contaminate the next parked fixture's cursors. Use `--native-only` for the fast
offline regression embedded in `test-game`.

The `cocoa_invalid_support_random_tick_seed_0` matrix case invokes the real
`BlockCocoa.randomTick` server-thread callback on a mature unsupported pod. It
locks the air-first raw mutation, three independent brown-dye EntityItems,
their complete trajectories, and World/Math/Block/entity-ID cursor transitions:

```bash
uv run --no-project python magma/trace/run_oracle_matrix.py \
  --instances 1 --case cocoa_invalid_support_random_tick_seed_0
```
