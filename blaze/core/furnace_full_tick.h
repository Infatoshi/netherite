/* furnace_full_tick: TileEntityFurnace.update over N ticks with the full smelt registry.
 *
 ...
 * FFT_NUM_TICKS=400. CUT: world.isRemote gate, BlockFurnace.setState,
 * markDirty/NBT and output-slot experience. CPU==CUDA. */
...
MC_HD static inline int fft_can_smelt(const FftFurnace *f) {
    ...
    result = sr_getSmeltingResultBuiltin(f->slot0);
...
MC_HD static inline void fft_smelt(FftFurnace *f) {
    ...
    if (f->slot0.item == 19 && f->slot0.meta == 1
...
MC_HD static inline void fft_tick(FftFurnace *f) {
    ...
    if (fft_is_burning(f) || (!fft_is_empty(&f->slot1) && !fft_is_empty(&f->slot0))) {
        if (!fft_is_burning(f) && fft_can_smelt(f)) {
            ...
            if (fft_is_burning(f)) {
                i32 fuel_item = f->slot1.item;
                f->slot1.count--;
                if (f->slot1.count <= 0) {
                    f->slot1 = fuel_item == SR_LAVA_BUCKET
                        ? sr_mk(SR_BUCKET, 1, 0) : sr_empty();
                }
            }
        }
        if (fft_is_burning(f) && fft_can_smelt(f)) {
