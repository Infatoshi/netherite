...
int main(int argc,char**argv){
 int n=8,steps=32,warm=4,repeat=4,threads=8,split=0,policy=0,reset_every=32,op=0;
 ...
 for(int i=1;i<argc;i++){if(i+1>=argc)fail("arguments require values");const char*k=argv[i],*v=argv[++i];
  ...
  else if(!strcmp(k,"--n"))n=atoi(v);else if(!strcmp(k,"--steps"))steps=atoi(v);else if(!strcmp(k,"--warmup"))warm=atoi(v);else if(!strcmp(k,"--repeat"))repeat=atoi(v);else if(!strcmp(k,"--threads"))threads=atoi(v);else if(!strcmp(k,"--split"))split=atoi(v);else if(!strcmp(k,"--policy"))policy=atoi(v);else if(!strcmp(k,"--reset-every"))reset_every=atoi(v);else if(!strcmp(k,"--op"))op=atoi(v);else fail("unknown option");
 }
 ...
 int*assignment=alloc(n,sizeof(int));unsigned char*mask=alloc(n,1);memset(mask,1,n);if(assign(env,assignment)||reset(env,mask)||set_success_item(env,0))fail("initial reset");
 ...
 printf("step,env_ms,render_ms,pack_ms,policy_ms,reset_ms,total_ms,terminal_lanes,h2d_bytes,d2h_bytes\n");fflush(stdout);
 ...
 for(int t=0;t<steps+warm;t++){
  ...
  int any=0;for(int i=0;i<n;i++){mask[i]=(t==warm||t%reset_every==0||done[i]);any|=mask[i];}
  if(any){double a=now();if(reset(env,mask))fail("reset");for(int i=0;i<n;i++)if(mask[i]){have[i]=0;epdec[i]=0;}if(hybrid){if(render(obs,env,1,cam,depth,edge,&stats)||commit(env,cam,depth,edge))fail("reset render");}resetms=(now()-a)*1000;}
  ...
  double total=(now()-t0)*1000;int term=0;for(int i=0;i<n;i++)term+=done[i]!=0;
 ...
 printf("RESULT total_ms=%.6f nominal_subticks=%llu terminal_lanes=%llu decisions_per_s=%.6f nominal_ticks_per_s=%.6f maxrss_kib=%ld digest=%016llx compared=%d\n",sum,(unsigned long long)subticks,(unsigned long long)terminals,(double)n*steps*1000/sum,(double)subticks*1000/sum,ru.ru_maxrss,(unsigned long long)whole,reference!=NULL);
 if(record&&fclose(record))fail("record close");if(reference)fclose(reference);
 if(nn)nn_destroy(nn);if(obs)obs_destroy(obs);if(gpu)env_cuda_stage_destroy(&stage);destroy(env);return 0;
}
