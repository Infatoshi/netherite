...
> Verification entry point: `c/magma/VERIFY.md`. Day-to-day input replay is
...
**`null` = UNSIMULATED on C (a SENTINEL, not a value).** The magma game path does not
...
- **UNSIMULATED on C -> `null`**: `air`, `fire`, `xp_level`/`xp_frac`, `potions`,
  `attack_cooldown`, `hurt_time`, `death_time`, the whole **`entities`** array (the magma
...
The C tracer spawns at the magma worldgen origin column; Java spawns at the REAL world
...
```bash
cd c/magma && bash trace/run_oracle.sh          # TICKS=300 SEED=0 by default
```
...
- **Inventory/held id namespaces differ** (Java vanilla registry vs C mc-sim `IC_*`), and the C
  magma player is pre-loaded with 64 cobblestone while a fresh Java survival player has an
...
- `../raster/verify/mc_capture/capture_at_poses.sh` - the Java-side engine: reads a poses
  ...
  divergence magma lacks); `--gamemode spectator` gives a clean camera hold instead.
