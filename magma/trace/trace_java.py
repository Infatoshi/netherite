...
def canonical_container(entry):
    value = {
        **entry,
        "items": sorted(({
            **item,
            "stack_payload": canonical_stack_payload(
                item.get("stack_payload")),
        } if "stack_payload" in item else dict(item)
            for item in entry.get("items", [])),
            key=lambda item: item.get("slot")),
    }
    ...
    if value.get("type") in (
            ...
            "chain_command_block") \
            and value.get("last_output") == '{"text":""}':
        value["last_output"] = ""
    return value
...
def canonicalize(tick, ob, scheduled_box=None, scheduled_blocks=(1,)):
    ...
    server_position = os.environ.get("SERVER_POSITION_CANONICAL") == "1"
    p = {
        "eid": authoritative.get("player_eid", 1),
        "name": authoritative.get("player_name", "Player"),
        "x": authoritative.get("x") if server_position else _g(ob, "x"),
        "y": authoritative.get("y") if server_position else _g(ob, "y"),
        "z": authoritative.get("z") if server_position else _g(ob, "z"),
        "yaw": _g(ob, "yaw"), "pitch": _g(ob, "pitch"),
        "vx": authoritative.get("vx") if server_position else _g(ob, "vx"),
        "vy": authoritative.get("vy") if server_position else _g(ob, "vy"),
        "vz": authoritative.get("vz") if server_position else _g(ob, "vz"),
        ...
        "dead": 1 if authoritative.get("dead", _g(ob, "dead")) else 0,
        "creative": bool(authoritative.get("creative", False)),
        "game_mode": authoritative.get("game_mode", 0),
        "deaths": authoritative.get("deaths", _g(ob, "deaths")),
        "dim": authoritative.get("dim", _g(ob, "dim")),
        "uuid_most_hex": authoritative.get(
            "player_uuid_most_hex", "a01e3843e5213998"),
        "uuid_least_hex": authoritative.get(
            "player_uuid_least_hex", "958af459800e4d11"),
        "riding_eid": authoritative.get("player_riding_eid", -1),
        "spawn_present": bool(authoritative.get("player_spawn_present", False)),
        "spawn_x": authoritative.get("player_spawn_x", 0),
        "spawn_y": authoritative.get("player_spawn_y", 0),
        "spawn_z": authoritative.get("player_spawn_z", 0),
        "spawn_forced": bool(authoritative.get("player_spawn_forced", False)),
        "trigger_qrl_present": bool(authoritative.get(
            "trigger_qrl_present", False)),
        "trigger_qrl_score": authoritative.get("trigger_qrl_score", 0),
        "trigger_qrl_locked": bool(authoritative.get(
            "trigger_qrl_locked", False)),
        "achievement_open_inventory": bool(authoritative.get(
            "achievement_open_inventory", False)),
    ...
    inv = [
        ...
        } if "stack_payload" in stack else dict(stack))
        for stack in authoritative.get(
            "inventory", _g(ob, "inventory", []))
    ]
    ents = [
        dict(entity)
    ...
    if ents and all("loaded_order" in entity for entity in ents):
        relative_order = {
            entity["eid"]: rank for rank, entity in enumerate(
                sorted(ents, key=lambda value: value["loaded_order"]))
        }
        for entity in ents:
            entity["loaded_order"] = relative_order[entity["eid"]]
    for entity in ents:
        entity["stack_payload"] = canonical_stack_payload(
            entity.get("stack_payload"))
        if isinstance(entity.get("items"), list):
            entity["items"] = [
                ({
                    **item,
                    "stack_payload": canonical_stack_payload(
    ...
    loaded_tile_values = [
        ...
        for entry in authoritative.get("loaded_tiles", [])
    ]
    if loaded_tile_values and all(
            "loaded_order" in entry for entry in loaded_tile_values):
        loaded_tile_values.sort(key=lambda value: value["loaded_order"])
        for rank, entry in enumerate(loaded_tile_values):
            entry["loaded_order"] = rank
    if loaded_tile_values:
        tickable = [
            entry for entry in loaded_tile_values if entry["tickable"]
        ]
        if all(isinstance(entry.get("update_order"), int)
                and entry["update_order"] >= 0 for entry in tickable):
            tickable.sort(key=lambda value: value["update_order"])
        else:
            tickable.sort(key=lambda value: value["loaded_order"])
        for rank, entry in enumerate(tickable):
            entry["update_order"] = rank
        for entry in loaded_tile_values:
            if not entry["tickable"]:
                entry["update_order"] = -1
    loaded_tile_order_by_position = {
        (entry["x"], entry["y"], entry["z"]): entry["loaded_order"]
        for entry in loaded_tile_values
    }
    spawners = sorted(
        (
            {
                ...
                "min_delay": entry.get("min_delay"),
                "max_delay": entry.get("max_delay"),
                "spawn_count": entry.get("spawn_count"),
                "max_nearby": entry.get("max_nearby"),
                "activate_range": entry.get("activate_range"),
                "spawn_range": entry.get("spawn_range"),
                "entity_id": entry.get("entity_id"),
                "spawn_data_nbt": entry.get("spawn_data_nbt"),
                "default_entity_nbt": entry.get("default_entity_nbt"),
                "potentials": [
                    {
                        "weight": potential.get("weight"),
                        "entity_id": potential.get("entity_id"),
                        "entity_nbt": potential.get("entity_nbt"),
                        "default_entity_nbt": potential.get(
                            "default_entity_nbt"),
                    }
                    for potential in entry.get("potentials", [])
                ],
            }
            for entry in authoritative.get("spawners", [])
        ),
        key=lambda entry: (entry["x"], entry["y"], entry["z"]),
    )
    if container_values and all(
            "loaded_order" in entry for entry in container_values):
        relative_order = loaded_tile_order_by_position or {
            (entry["x"], entry["y"], entry["z"]): rank
            for rank, entry in enumerate(sorted(
                container_values,
                key=lambda value: value["loaded_order"]))
        }
        for entry in container_values:
            entry["loaded_order"] = relative_order[
    ...
    note_blocks = sorted(
        (
            {
                field: entry.get(field)
                for field in ("x", "y", "z", "note", "powered")
            }
            for entry in authoritative.get("note_blocks", [])
        ),
        key=lambda entry: (
            entry.get("x"), entry.get("y"), entry.get("z")
        ),
    ...
    decorative_tiles = sorted(
        (
            {
                field: entry.get(field)
                for field in (
                    "x", "y", "z", "block", "tile_nbt",
                    "drop_item", "drop_meta",
                )
            } | ({"drop_nbt": entry.get("drop_nbt")}
                 if "drop_nbt" in entry else {})
            for entry in authoritative.get("decorative_tiles", [])
        ),
        key=lambda entry: (
            entry.get("x"), entry.get("y"), entry.get("z")
        ),
        ...
        (
            {
                ...
                for field in (
                    ...
                    "entity_gaussian", "uuid_most", "uuid_least",
                    "loaded_order", "repair_cost", "custom_name",
                    "enchants", "tracker_update_counter",
                    "map_data_present", "map_colors_b64", "map_dimension",
                    ...
                    entry.get("stack_payload"))}
                 if "stack_payload" in entry else {})
    ...
    paintings = sorted(
        (
            ...
            for entry in authoritative.get("paintings", [])
        ),
        key=lambda entry: (
            entry.get("hanging_x"), entry.get("hanging_y"),
            entry.get("hanging_z"), entry.get("eid"),
    ...
    leash_knots = sorted(
        (
            ...
            for entry in authoritative.get("leash_knots", [])
        ),
        key=lambda entry: (
            entry.get("hanging_x"), entry.get("hanging_y"),
            entry.get("hanging_z"), entry.get("eid"),
        ...
        (
            {
                ...
                for field in (
                    ...
                    "holder_uuid_least", "pending", "pending_x",
                    "pending_y", "pending_z", "wolf_angry",
                )
            }
            for entry in authoritative.get("living_leashes", [])
        ),
        key=lambda entry: entry.get("eid"),
    )
    hanging_entities = [*item_frames, *paintings, *leash_knots]
    if hanging_entities:
        relative_order = {
            entity["eid"]: rank for rank, entity in enumerate(sorted(
                hanging_entities, key=lambda value: value["loaded_order"]))
        }
        for entity in hanging_entities:
            entity["loaded_order"] = relative_order[entity["eid"]]
    villages = []
    for source_village in authoritative.get("villages", []):
        doors = [
            {
                "x": door.get("X"),
                "y": door.get("Y"),
                "z": door.get("Z"),
                "inside_dx": door.get("IDX"),
                "inside_dz": door.get("IDZ"),
                "timestamp": door.get("TS"),
            }
            for door in source_village.get("Doors", [])
        ]
        reputations = sorted(
            (
                {
                    "uuid_most_hex": str(
                        reputation.get("uuid_most_hex", "")).lower(),
                    "uuid_least_hex": str(
                        reputation.get("uuid_least_hex", "")).lower(),
                    "score": reputation.get("score"),
                }
                for reputation in source_village.get("Players", [])
            ),
            key=lambda reputation: (
                reputation["uuid_most_hex"],
                reputation["uuid_least_hex"],
            ),
        )
        villages.append({
            "population": source_village.get("PopSize"),
            "radius": source_village.get("Radius"),
            "golems": source_village.get("Golems"),
            "stable": source_village.get("Stable"),
            "state_tick": source_village.get("Tick"),
            "mating_tick": source_village.get("MTick"),
            "center_x": source_village.get("CX"),
            "center_y": source_village.get("CY"),
            "center_z": source_village.get("CZ"),
            "helper_x": source_village.get("ACX"),
            "helper_y": source_village.get("ACY"),
            "helper_z": source_village.get("ACZ"),
            "doors": doors,
    ...
    return {
        ...
        "do_mob_spawning": authoritative.get("do_mob_spawning", True),
        "random_tick_speed": authoritative.get("random_tick_speed", 3),
        "default_game_mode": authoritative.get("default_game_mode", 0),
        "difficulty": authoritative.get("difficulty", 2),
        "world_border": {
            "center_x": authoritative.get("border_center_x", 0.0),
            "center_z": authoritative.get("border_center_z", 0.0),
            "diameter": authoritative.get("border_diameter", 60000000.0),
            "target_diameter": authoritative.get(
                "border_target_diameter", 60000000.0),
            "time_until_target": authoritative.get(
                "border_time_until_target", 0),
            "damage_amount": authoritative.get("border_damage_amount", 0.2),
            "damage_buffer": authoritative.get("border_damage_buffer", 5.0),
            "warning_time": authoritative.get("border_warning_time", 15),
            "warning_distance": authoritative.get(
                "border_warning_distance", 5),
        },
        "world_spawn": {
            "x": authoritative.get("world_spawn_x", 0),
            "y": authoritative.get("world_spawn_y", 64),
            "z": authoritative.get("world_spawn_z", 0),
        },
        "ticking_chunks": [
            {
                ...
                "x": value.get("x"),
                "z": value.get("z"),
                "random_tick_mask": value.get("random_tick_mask"),
            }
            for value in authoritative.get("ticking_chunks", [])
        ],
        "ticking_chunks_complete": authoritative.get(
            "ticking_chunks_complete", False),
        ...
        "world_rng": {
            ...
            **({"collections_seed48": authoritative.get(
                "collections_seed48")}
               if authoritative.get("collections_seed48") is not None
               else {}),
            **({"server_uuid_seed48": authoritative.get(
                "server_uuid_seed48")}
               if authoritative.get("server_uuid_seed48") is not None
               else {}),
            **({"entity_seed_generator_seed48": authoritative.get(
                "entity_seed_generator_seed48")}
               if authoritative.get("entity_seed_generator_seed48")
               is not None else {}),
            "block_seed48": authoritative.get("block_rand_seed48"),
            **({
                "inventory_helper_seed48": authoritative.get(
                    "inventory_helper_rand_seed48"),
                "inventory_helper_have_gaussian": authoritative.get(
                    "inventory_helper_rand_have_gaussian", False),
                "inventory_helper_gaussian": authoritative.get(
                    "inventory_helper_rand_gaussian", 0.0),
            } if authoritative.get("inventory_helper_rand_seed48")
                is not None else {}),
        ...
        "controlled_input": (
            {
                "before": {
                    ...
                    "world_rng": {
                        ...
                        "inventory_helper_seed48": (
                            authoritative["controlled_input"].get("before")
                            or {}).get("inventory_helper_rand_seed48"),
                        ...
                        "inventory_helper_gaussian": (
                            authoritative["controlled_input"].get("before")
                            or {}).get("inventory_helper_rand_gaussian", 0.0),
                        "update_lcg": (
                            authoritative["controlled_input"].get("before")
                ...
                "world_rng": {
                    ...
                    "block_seed48": authoritative["controlled_input"].get(
                        "block_rand_seed48"),
                    "inventory_helper_seed48": authoritative[
                        "controlled_input"].get(
                            "inventory_helper_rand_seed48"),
                    "inventory_helper_have_gaussian": authoritative[
                        "controlled_input"].get(
                            "inventory_helper_rand_have_gaussian", False),
                    "inventory_helper_gaussian": authoritative[
                        "controlled_input"].get(
                            "inventory_helper_rand_gaussian", 0.0),
        ...
        "weather_effects": [
            dict(effect)
            for effect in authoritative.get("weather_effects", [])
        ],
        ...
        "loaded_tiles": loaded_tile_values,
        "loaded_tiles_complete": authoritative.get(
            "loaded_tiles_complete", False),
        "spawners": spawners,
        "spawners_complete": authoritative.get(
            "spawners_complete", False),
        "flower_pots": flower_pots,
        "flower_pots_complete": authoritative.get(
            "flower_pots_complete", False),
        "note_blocks": note_blocks,
        "note_blocks_complete": authoritative.get(
            "note_blocks_complete", False),
        "skulls": skulls,
        "skulls_complete": authoritative.get(
            "skulls_complete", False),
        "decorative_tiles": decorative_tiles,
        "decorative_tiles_complete": authoritative.get(
            "decorative_tiles_complete", False),
        ...
        "paintings": paintings,
        "paintings_complete": authoritative.get(
            "paintings_complete", False),
        "leash_knots": leash_knots,
        "leash_knots_complete": authoritative.get(
            "leash_knots_complete", False),
        "living_leashes": living_leashes,
        "living_leashes_complete": authoritative.get(
            "living_leashes_complete", False),
        "village_collection_tick": authoritative.get(
            "village_collection_tick", 0),
        "villages": villages,
        "villages_complete": authoritative.get(
            "villages_complete", False),
...
def main():
    ...
    ap.add_argument(
        "--decorative-nbt-offset",
        nargs=4,
        metavar=("DX", "DY", "DZ", "NBT_JSON"),
        help="restore canonical coordinate-free sign/banner NBT from typed "
             "JSON at a player-relative parked pre-tick position",
    )
    ap.add_argument(
        "--shulker-transient",
        type=int,
        nargs=4,
        metavar=("STATUS", "OPEN_COUNT", "PROGRESS_OLD_BITS", "PROGRESS_BITS"),
        help="with --shulker-nbt-offset, restore exact AnimationStatus ordinal, "
             "openCount, progressOld bits, and progress bits",
    )
    ap.add_argument(
        "--flower-pot-offset",
        type=int,
    ...
    ap.add_argument(
        ...
        type=int,
        nargs=5,
        metavar=("DX", "DY", "DZ", "NOTE", "POWERED"),
        help="restore one note tile at a player-relative parked pre-tick "
             "position",
    )
    ap.add_argument(
        "--skull-offset",
    ...
    ap.add_argument(
        "--command-searge-trigger", action="store_true",
        help="set the bounded Searge command and execute it while parked",
    )
    ap.add_argument(
        "--command-trigger-text", default="",
        help="set and execute one bounded command while parked",
    )
    ap.add_argument(
        "--command-trigger-clock-out",
        help="write the Java command-block output clock used by a deferred "
             "trigger",
    )
    ap.add_argument(
        "--trigger-score",
        type=int,
        nargs=2,
        metavar=("SCORE", "LOCKED"),
        help="restore the bounded qrl trigger score at the parked boundary",
    )
    ap.add_argument(
        "--item-frame-offset",
        type=int,
    ...
    deferred_command_fixture = None

    # The integrated client and server otherwise share Malmo's static random
    # allocators. Client-only entity construction can then advance the next
    # server Entity seed between this parked snapshot and the permitted tick.
    # Fork those globals at the boundary we serialize, exactly as the cold
    # save/reload continuation harness does, so the capsule describes the
    # causal server stream rather than client scheduling noise.
    hidden = env._cmd({"cmd": "hidden_state_locked", "action": {}})
    if not hidden.get("ok"):
        print("oracle hidden-state capture FAILED:", hidden, file=sys.stderr)
        release_server_gate()
        env.close()
        return 1
    isolated = env._cmd({
        "cmd": "isolate_server_globals_locked",
        "action": hidden,
    })
    if not isolated.get("ok"):
        print("oracle server-global isolation FAILED:", isolated,
              file=sys.stderr)
        release_server_gate()
        env.close()
        return 1
    refreshed = env._cmd({
        "cmd": "authoritative_state_locked",
        "action": {},
    })
    if not refreshed.get("ok") or not refreshed.get("authoritative"):
        print("oracle isolated-state refresh FAILED:", refreshed,
              file=sys.stderr)
        release_server_gate()
        env.close()
        return 1
    initial_observation["authoritative"] = refreshed["authoritative"]
    ...
    if args.clean_entities:
        # Fixture staging can itself wake remote falling blocks or portals
        # after the earlier unlocked cleanup.  Remove that setup fallout at
        # the final parked boundary, before any explicit controlled entity
        # fixtures below are constructed.  This also makes the captured EID,
        # UUID, and entity-list cursors causal to the tested transition.
        cleared = env._cmd({"cmd": "clear_entities_locked", "action": {}})
        if (not cleared.get("ok")
                or not cleared.get("authoritative")):
            print("oracle final locked entity cleanup FAILED:", cleared,
                  file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        initial_observation["authoritative"] = cleared["authoritative"]
        print("removed " + str(cleared.get("removed"))
              + " fixture-staging entity/entities at the final parked "
              "boundary")
        x0, y0, z0, x1, y1, z1 = args.blocks_box
        pruned = env._cmd({
            "cmd": "prune_scheduled_ticks_locked",
            "action": {
                "x0": x0, "y0": y0, "z0": z0,
                "x1": x1, "y1": y1, "z1": z1,
            },
        ...
        if (not pruned.get("ok")
                or not pruned.get("authoritative")):
            print("oracle final scheduled-tick prune FAILED:", pruned,
                  file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        initial_observation["authoritative"] = pruned["authoritative"]
        print("removed " + str(pruned.get("removed_references"))
              + " out-of-box scheduled-tick reference(s) at the final "
              "parked boundary")
    if args.potion_fixture:
    ...
    if args.trigger_score:
        score_value, locked = args.trigger_score
        if locked not in (0, 1):
            print("trigger score LOCKED must be 0 or 1", file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        seeded = env._cmd({
            "cmd": "set_trigger_qrl_score_locked",
            "action": {"score": score_value, "locked": bool(locked)},
        })
        if not seeded.get("ok") or not seeded.get("authoritative"):
            print("oracle trigger score fixture FAILED:", seeded,
                  file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        initial_observation["authoritative"] = seeded["authoritative"]
        print(f"seeded qrl trigger score={score_value} locked={locked}")
    if args.command_success_offset:
        ...
        command_text = args.command_trigger_text
        if args.command_searge_trigger:
            if command_text and command_text.lower() != "searge":
                print("conflicting bounded command trigger options",
                      file=sys.stderr)
                release_server_gate()
                env.close()
                return 1
            command_text = "Searge"
        if command_text:
            fixture["command"] = command_text
            deferred_command_fixture = dict(fixture)
        seeded = env._cmd({
    ...
    if args.shulker_nbt_offset:
        ...
        if args.shulker_transient:
            status, open_count, progress_old_bits, progress_bits = \
                args.shulker_transient
            if not (0 <= status <= 3
                    and -(1 << 31) <= open_count < (1 << 31)
                    and 0 <= progress_old_bits <= 0xFFFFFFFF
                    and 0 <= progress_bits <= 0xFFFFFFFF):
                print("invalid shulker transient fixture", file=sys.stderr)
                release_server_gate()
                env.close()
                return 1
            fixture.update({
    ...
    elif args.shulker_transient:
        print("--shulker-transient requires --shulker-nbt-offset",
              file=sys.stderr)
        release_server_gate()
        env.close()
        return 1
    if args.decorative_nbt_offset:
        raw_dx, raw_dy, raw_dz, nbt_json_path = \
            args.decorative_nbt_offset
        try:
            dx, dy, dz = int(raw_dx), int(raw_dy), int(raw_dz)
            document = json.loads(
                pathlib.Path(nbt_json_path).read_text(encoding="utf-8"))
            nbt_hex = nbt_codec.encode_hex(document)
                ...
                nbt_codec.NbtError) as exc:
            print(f"invalid decorative NBT fixture: {exc}", file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        fixture = {
        ...
        seeded = env._cmd({
            "cmd": "set_decorative_tile_locked",
            "action": fixture,
        })
        if not seeded.get("ok") or not seeded.get("authoritative"):
            print("oracle decorative NBT fixture FAILED:", seeded,
                  file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        initial_observation["authoritative"] = seeded["authoritative"]
        print(
            "seeded locked decorative NBT at "
            f"({fixture['x']},{fixture['y']},{fixture['z']})"
        )
    ...
    if args.note_block_offset:
        dx, dy, dz, note, powered = args.note_block_offset
        if not (0 <= note <= 24 and powered in (0, 1)):
            print(
                "note-block fixture requires NOTE 0..24 and POWERED 0 or 1",
                file=sys.stderr,
            )
            release_server_gate()
            env.close()
        ...
        seeded = env._cmd({
            "cmd": "set_note_block_locked",
            "action": fixture,
        })
        if not seeded.get("ok") or not seeded.get("authoritative"):
            print(
                "oracle note-block fixture FAILED:",
                seeded,
                file=sys.stderr,
            )
            release_server_gate()
        ...
        initial_observation["authoritative"] = seeded["authoritative"]
        print(
            f"seeded locked note block pitch={note} powered={powered} at "
            f"({fixture['x']},{fixture['y']},{fixture['z']})"
        )
    ...
    command_clock = None
    command_clock_path = None
    teleport_server_target = None
    teleport_client_start = None
    teleport_client_correction_tick = None
    teleport_client_correction_pre_tick = False
    if deferred_command_fixture is not None:
        trigger_fixture = dict(deferred_command_fixture)
        trigger_fixture["trigger"] = True
        triggered = env._cmd({
            "cmd": "set_command_success_locked",
            "action": trigger_fixture,
        })
        if not triggered.get("ok") or not triggered.get("authoritative"):
            print("oracle deferred command trigger FAILED:", triggered,
                  file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        initial_observation["authoritative"] = triggered["authoritative"]
        post_trigger_state = canonicalize(
            -1, initial_observation, args.blocks_box, scheduled_blocks
        )
        command_container = next((
            container for container in post_trigger_state["containers"]
            if container.get("x") == deferred_command_fixture["x"]
            and container.get("y") == deferred_command_fixture["y"]
            and container.get("z") == deferred_command_fixture["z"]
            and container.get("type") in (
                "command_block", "repeating_command_block",
                "chain_command_block")
        ), None)
        if command_container is None:
            print("oracle deferred command result is missing its tile state",
                  file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        ...
        match = re.search(
            r'\[(\d{2}):(\d{2}):(\d{2})\]',
            command_container.get("last_output", ""),
        )
        if match:
            clock = dict(zip(
                ("hour", "minute", "second"), map(int, match.groups())
            ))
        command_text = deferred_command_fixture.get("command", "").lower()
        if re.fullmatch(r"(?:tp|teleport) @p .+", command_text):
            diagnostics = post_trigger_state.get("diagnostics", {})
            teleport_server_target = tuple(
                diagnostics.get(field)
                for field in ("server_x", "server_y", "server_z")
            )
            if any(value is None for value in teleport_server_target):
                teleport_server_target = None
            else:
                player = post_trigger_state.get("player", {})
                teleport_client_start = tuple(
                    player.get(field) for field in ("x", "y", "z"))
        weather_default = re.fullmatch(
            r"weather (clear|rain|thunder)", command_text
        )
        if weather_default:
            timer_field = (
                "clean_weather_time"
                if weather_default.group(1) == "clear" else "rain_time"
            )
            clock["weather_duration_ticks"] = int(
                post_trigger_state["time"][timer_field]
            )
        if args.command_trigger_clock_out:
            command_clock_path = os.path.abspath(
                args.command_trigger_clock_out)
            os.makedirs(os.path.dirname(command_clock_path), exist_ok=True)
            with open(command_clock_path, "w", encoding="utf-8") as clock_file:
                json.dump(clock, clock_file, sort_keys=True)
                clock_file.write("\n")
        command_clock = clock
        print("executed deferred command at the canonical pre-tick fork")
    ...
    for t, v in enumerate(tape):
        ...
        if teleport_server_target is not None \
                and teleport_client_correction_tick is None:
            player = canonical.get("player", {})
            current = tuple(player.get(field) for field in ("x", "y", "z"))
            if teleport_client_start == teleport_server_target or any(
                    not math.isclose(
                        float(value), float(start),
                        rel_tol=0.0, abs_tol=1.0e-12)
                    for value, start in zip(current, teleport_client_start)):
                teleport_client_correction_tick = t
                teleport_client_correction_pre_tick = not math.isclose(
                    float(current[1]), float(teleport_server_target[1]),
                    rel_tol=0.0, abs_tol=1.0e-12)
    ...
    st_f.close()
    if teleport_server_target is not None:
        if teleport_client_correction_tick is None:
            print(
                "oracle teleport client correction did not arrive within "
                f"{len(tape)} tick(s)", file=sys.stderr)
            release_server_gate()
            env.close()
            return 1
        if command_clock is not None and command_clock_path is not None:
            command_clock["teleport_client_delay_ticks"] = (
                teleport_client_correction_tick + 1)
            command_clock["teleport_client_pre_tick"] = int(
                teleport_client_correction_pre_tick)
            with open(
                    command_clock_path, "w", encoding="utf-8") as clock_file:
                json.dump(command_clock, clock_file, sort_keys=True)
                clock_file.write("\n")
