...
ANIMAL_CASES = (
    ...
    ("horse_horse_live_birth", {
        "species": "horse", "first_kind": "horse",
        "second_kind": "horse", "distance": 0.25,
    }),
    ("horse_donkey_live_birth", {
        "species": "horse", "first_kind": "horse",
        "second_kind": "donkey", "distance": 0.25,
    }),
    ("donkey_horse_live_birth", {
        "species": "horse", "first_kind": "donkey",
        "second_kind": "horse", "distance": 0.25,
    }),
    ("donkey_donkey_live_birth", {
        "species": "horse", "first_kind": "donkey",
        "second_kind": "donkey", "distance": 0.25,
    }),
)
...
HORSE_KEYS = ANIMAL_KEYS | {
    "horse_kind", "tame", "max_health_bits", "speed_bits", "jump_bits",
    "variant",
}
XP_KEYS = {
...
def animal_native(case, java=None):
    ...
    if case["species"] == "horse":
        args.extend([case["first_kind"], case["second_kind"]])
    return json.loads(subprocess.check_output(
        [str(value) for value in args], text=True))
...
def validate_animal(value, case):
    ...
    expected_keys = (CHICKEN_KEYS if species == "chicken"
                     else HORSE_KEYS if species == "horse"
    ...
    for index, row in enumerate(value["animals"]):
        ...
        if species == "horse":
            expected_kinds = [case["first_kind"], case["second_kind"],
                              ("horse" if case["first_kind"] == "horse"
                               and case["second_kind"] == "horse"
                               else "donkey"
                               if case["first_kind"] == "donkey"
                               and case["second_kind"] == "donkey"
                               else "mule")]
            if row["horse_kind"] != expected_kinds[index]:
                raise AssertionError(
                    f"{case}: animal {index} wrong horse kind")
            if not isinstance(row["tame"], bool):
                raise AssertionError(
                    f"{case}: animal {index} malformed tame state")
            for key in ("max_health_bits", "speed_bits", "jump_bits"):
                if not isinstance(row[key], str) \
                        or not HEX16.fullmatch(row[key]):
                    raise AssertionError(
                        f"{case}: animal {index} malformed {key}")
            if not isinstance(row["variant"], int):
                raise AssertionError(
                    f"{case}: animal {index} malformed variant")
