...
int nn_layout_kcrs_to_krsc(const float *kcrs, __half *krsc, int k, int c, int r,
                           int s, int c_pad) {
...
int nn_layout_krsc_to_kcrs(const __half *krsc, float *kcrs, int k, int c, int r,
                           int s, int c_pad) {
...
int nn_layout_f32_to_f16(const float *src, __half *dst, size_t n) {
  if (!src || !dst || n == 0)
    return -1;
  const int grid = layout_grid(n);
  k_f32_to_f16<<<grid, kThr>>>(src, dst, n);
  return cudaGetLastError() == cudaSuccess ? 0 : -1;
}
...
int nn_layout_f16_to_f32(const __half *src, float *dst, size_t n) {
  if (!src || !dst || n == 0)
    return -1;
  const int grid = layout_grid(n);
  k_f16_to_f32<<<grid, kThr>>>(src, dst, n);
  return cudaGetLastError() == cudaSuccess ? 0 : -1;
}
