...
craft torches. Every stage talks to craster --rl-bin through CrasterEnv and
...
Run (anvil): cd c/craster && uv run --no-project --with numpy python rl/chain_probe.py
"""
import json
import math
import os
...
from look_at_tree import CrasterEnv, INV_IDS, EYE, log_dirs, wrap180
...
def load_iron_oracle(seed):
    ...
    # rnx rny rnz (cuenv_snapshot.h); items are 76 bytes each; region cells
...
def run_seed(seed, verbose=True):
    ...
    env = CrasterEnv(seed, bin=not os.environ.get("IRON"))
    results = []
    try:
