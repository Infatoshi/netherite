...
CR_HD CrRgba cr_shade(const CrShadeCtx *sh, const CrFragment *frag) {
    ...
    float frag_light = frag->light;
    float dissolve_threshold = 0.0f;
    int dissolve = sh->alpha_mask && frag_light < 0.0f;

    if (dissolve) {
        /* emit_dragon packs -(light+1+threshold/32). Both values survive
         * perspective interpolation because every vertex in a draw carries
         * the same light and threshold. */
        float packed = -frag_light;
        float whole = floorf(packed);
        frag_light = whole - 1.0f;
        dissolve_threshold = (packed - whole) * 32.0f;
    }

    /* RenderDragon death dissolve: sample dragon_exploding at uv+mask_off,
     * matching GL_GREATER against deathTicks/200. */
    if (dissolve && sh->atlas) {
        ...
        if ((float)mask.a * inv255 <= dissolve_threshold) {
    ...
    float lscalar = frag_light;
    float ao_mul = frag->ao;
    if (sh->lightmap && frag_light >= 0.0f) {
        float s = fmaxf(0.0f, fminf(15.0f, frag_light));
    ...
    if (sh->entity_brightness) {
        /* Mesa fixed-function entity path: lighting*material*lightmap reaches
         * the texture combiner as unorm8 primary colour, then MODULATE uses an
         * exact normalized-byte multiply. Keeping this as one float product
         * until framebuffer conversion is observably one high at selected
         * horse-skin channels. */
        int lmr8 = (int)(fmaxf(0.0f, fminf(1.0f, lmr)) * 255.0f + 0.5f);
        int lmg8 = (int)(fmaxf(0.0f, fminf(1.0f, lmg)) * 255.0f + 0.5f);
        int lmb8 = (int)(fmaxf(0.0f, fminf(1.0f, lmb)) * 255.0f + 0.5f);
        int lr = (int)(fmaxf(0.0f, fminf(1.0f, tr * la)) * (float)lmr8);
        int lg = (int)(fmaxf(0.0f, fminf(1.0f, tg * la)) * (float)lmg8);
        int lb = (int)(fmaxf(0.0f, fminf(1.0f, tb * la)) * (float)lmb8);
