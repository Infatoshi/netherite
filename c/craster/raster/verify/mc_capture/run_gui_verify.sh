...
#     translucent corners composite over the live 3D scene (which craster's
...
#     3D player-model preview, which craster does not render (documented gap
...
echo "== render craster screens =="
# capture_gui.sh grabs the 854x480 window; container ids: 0 player, 1 table, 2 furnace
"$OUT/gui_candidate" --container 1 --w 854 --h 480 --ppm "$OUT/craster_gui_table.ppm"
"$OUT/gui_candidate" --container 2 --w 854 --h 480 --ppm "$OUT/craster_gui_furnace.ppm"
"$OUT/gui_candidate" --container 0 --w 854 --h 480 --ppm "$OUT/craster_gui_inventory.ppm"
...
fail = 0
print(f"{'screen':<10} {'noise(J-vs-J)':>13} {'craster-vs-J':>13} {'gate':>8}  verdict")
for name, hard in (("table", True), ("furnace", True), ("inventory", False)):
    ...
    c  = panel_crop(Image.open(f"{out}/craster_gui_{name}.ppm"))
