#!/usr/bin/env python3
"""frame_oracle.py - POSE-FORCED FIRST-MINUTE FRAME ORACLE for the craster C game.

North-star integration test: run a FIXED action tape through BOTH the real Java
Minecraft 1.11.2 game and the craster C game and pixel-diff FRAMES over the first
...
POSE-FORCED diff. At CHECKPOINT ticks over the first minute we take craster's exact
player pose (from c_phys.csv), TELEPORT the Java player there, grab the Java frame,
render the craster frame at the SAME pose, and pixel-diff. Both cameras are then
...
PIPELINE (one command, --run-java to include the live Java half):
  1. run trace_game on the tape  -> c_phys.csv (craster per-tick physics + pose)
  2. pick checkpoints (every --cadence ticks) and read craster's pose at each
  3. craster frame: build + run game_candidate at each checkpoint pose  -> PNG
  ...
  6. emit a per-checkpoint table + aggregate + ranked worst offenders, and
     materialize the MC frame / craster frame / diff heat-map ONLY for checkpoints
...
craster draws no hand/HUD-items); that is the POINT - the oracle makes each
divergence concrete and localized. This is the measuring stick to drive down.

Usage:
  # craster-side only (no live game): renders craster checkpoints + validates the
...
MCSIM_CORE = "/home/infatoshi/dev/minecraft/mc-1.11.2-env/c/mc-sim/core"
DIFF = "/home/infatoshi/dev/minecraft/mc-1.11.2-env/c/render-opt/wholeframe/diff_frame.py"
TRACE_GAME = os.path.join(CRASTER, "trace_game")
GAME_CANDIDATE = "/tmp/game_candidate_oracle"
CAPTURE_SH = os.path.join(CRASTER, "raster/verify/mc_capture/capture_at_poses.sh")
AERIAL_GOLDEN = os.path.join(CRASTER, "raster/verify/mc_capture/mc_frame.png")

EYE_HEIGHT = 1.62      # PSV_EYE_HEIGHT (mc-sim player_survival.h); MC + craster agree
FOV = 70               # MC "Normal" default == craster fov_deg
...
GC_FLAGS = ["-O2", "-ffp-contract=off", "-Wall", "-Icore", "-I.", "-I" + MCSIM_CORE]


def sh(cmd, **kw):
    return subprocess.run(cmd, cwd=CRASTER, **kw)


def build_craster_tools(force=False):
    ...
    r = sh(["gcc", *GC_FLAGS, "raster/verify/mc_capture/game_candidate.c",
...
def render_craster(ck, w, h, out_ppm):
    """Render craster at a checkpoint pose. eye = feet + EYE_HEIGHT; MC->craster
    convention: craster_yaw = 180 - mc_yaw, craster_pitch = -mc_pitch."""
...
def main():
    ...
    ap.add_argument("--tape", default=os.path.join(CRASTER, "trace/out/tape.txt"))
    ap.add_argument("--csv", default=os.path.join(CRASTER, "trace/out/c_phys.csv"))
    ap.add_argument("--outdir", default=os.path.join(CRASTER, "trace/out/frame_oracle"))
    ...
    craster_dir = os.path.join(args.outdir, "craster")
    os.makedirs(craster_dir, exist_ok=True)
    ...
    # ---- 1. tape + trace_game (craster physics/pose) ----
    ...
    # ---- 2. render craster frame at each checkpoint pose ----
    for ck in cks:
        ppm = os.path.join(craster_dir, f"craster_ck_{ck['tick']}.ppm")
        png = os.path.join(craster_dir, f"craster_ck_{ck['tick']}.png")
        render_craster(ck, args.w, args.h, ppm)
        ppm_to_png(ppm, png)
        ck["craster_png"] = png
    print(f"[oracle] rendered {len(cks)} craster checkpoint frames -> {craster_dir}")
    ...
    if have_java:
        for ck in cks:
            if not ck.get("mc_png"):
                ...
                continue
            stats = diff_regions(ck["mc_png"], ck["craster_png"], args.w, args.h, crops)
            results.append((ck, stats))
    else:
        # No live game: prove the render+diff wiring end-to-end by rendering craster
        ...
        ppm = os.path.join(craster_dir, "craster_aerial.ppm")
        png = os.path.join(craster_dir, "craster_aerial.png")
        render_craster(aerial, args.w, args.h, ppm)
        ppm_to_png(ppm, png)
        aerial["craster_png"] = png
        ...
        if aerial["mc_png"]:
            ...
            print("[oracle] aerial golden not found; craster frames rendered only.")
    ...
    if scored:
        ...
        for ck, stats, where in worst:
            ...
            heatmap(ck["mc_png"], ck["craster_png"], hp)
            ...
            print(f"           craster={ck['craster_png']}")
            print(f"           heatmap={hp}")
    if not have_java:
