...
#include "core/config.h"   /* audio / asset_objects registry knobs */
...
static int find_objects(char *out, int cap) {
    const char *override = cr_cfg()->asset_objects;
    static const char *const candidates[] = {
        "java/Minecraft/run/gradle/caches/minecraft/assets/objects",
        "../../java/Minecraft/run/gradle/caches/minecraft/assets/objects",
        "../java/Minecraft/run/gradle/caches/minecraft/assets/objects"
    };
    if (override[0] && directory_exists(override)) {
...
int gm_audio_live_init(GmAudioLive *audio, char *err, int err_cap) {
    ...
    /* audio=0: clean skip (no device/resource work), same as prior MAGMA_AUDIO=0. */
    if (!cr_cfg()->audio) return 1;
