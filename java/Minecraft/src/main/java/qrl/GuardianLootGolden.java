...
/** Exact built-in 1.11.2 Guardian loot from a raw entity RNG cursor. */
public final class GuardianLootGolden {
    ...
    private static final class FixedContext extends LootContext {
        ...
        @Override public int getLootingModifier() { return looting; }
        @Override public Entity getKiller() { return killer; }
    }

    private static EntityPlayer fakePlayer() throws Exception {
        Field field = Unsafe.class.getDeclaredField("theUnsafe");
        field.setAccessible(true);
        return (EntityPlayer)((Unsafe)field.get(null))
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] names = {"guardian", "elder_guardian"};
        LootTable[] tables = {
            manager.getLootTableFromLocation(LootTableList.ENTITIES_GUARDIAN),
        ...
        for (int type = 0; type < names.length; ++type) {
            for (long seed : SEEDS) {
                for (int fixture = 0; fixture < 3; ++fixture) {
                    ...
                    System.out.printf("%s %d %d %d %d", names[type], seed,
                        looting, killed ? 1 : 0, stacks.size());
