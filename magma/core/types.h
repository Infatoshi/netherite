...
typedef struct {
     ...
     * 5 = additive + depth write. This is SRC_ALPHA,ONE for the untextured
     * TNT flash; LayerWitherAura's ONE,ONE is equivalent because its
     * alpha-tested texture is binary-alpha;
     * 6 = src-over RGBA + depth write for the entity-outline Slime gel mask. */
    int    blend;
     ...
     * a negative fragment light packs the original sky/light scalar plus the
     * death threshold. The shader samples atlas at uv+(mask_u_off,mask_v_off)
     * and discards when mask.a/255 <= that threshold. ao remains the model's
     * directional face light. Zero-init keeps existing callers unchanged. */
    int    alpha_mask;
    ...
    int    repeat_uv;
    /* Match a requested 24-bit OpenGL depth buffer for entity passes.  The
     * software framebuffer remains float; comparisons and writes quantize at
     * this boundary only, avoiding a cost on terrain. */
    int    depth_d24;
    /* Signed fixed-point offset applied after D24 quantization. Positive
     * values pull a layer toward the camera. Zero preserves normal GL depth. */
    int    depth_bias_d24;
    /* Fixed-function raster subpixel grid. 8 snaps window-space endpoints to
     * 1/256 pixel before coverage; zero keeps full-float software coverage. */
    int    subpixel_bits;
    /* 1 = GL_ALWAYS. RenderGlobal uses this while building entity outlines. */
    int    depth_always;
} CrShadeCtx;
...
typedef struct {
    ...
    int  mouse_left, mouse_right, mouse_middle; /* 1 = button currently held */
    ...
    int  click_left, click_right, click_middle; /* button PRESS edges */
    int  release_left, release_right, release_middle; /* button RELEASE edges */
    int  mouse_moved;             /* at least one absolute motion event */
    uint32_t time_ms;             /* SDL monotonic milliseconds at poll end */
    int  key_esc;                 /* ESC keydown edge; app owns the screen/menu stack */
    int  key_tab;                 /* hotbar cycle (held; edge handled in input_map) */
    /* UTF-8 text committed this poll plus a backspace edge. The anvil rename
     * field consumes these only while GuiRepair owns the cursor. */
    char text[32];
    int  text_len;
    int  key_backspace;
    int  key_enter;               /* Return/keypad Enter keydown edge */
} CrInput;
