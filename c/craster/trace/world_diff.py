#!/usr/bin/env python3
"""world_diff.py - WORLDGEN INTEGRATION VERIFIER for craster vs the REAL Minecraft.

Ground truth = the REAL Java Minecraft 1.11.2 game's SAVED world (seed 0, default
generator), parsed BLOCK-FOR-BLOCK from its Anvil .mca region files. NOT a mc-sim
self-golden. Craster's side = trace/world_dump.c output (mc-sim cp_provide_chunk base
...
For chunks (cx,cz) in a range around origin it reports, per chunk:
  ...
  - biome-per-column agreement (craster voronoi vs the game's stored Biomes),
  - TREE-block counts (logs=17, leaves=18/161) MC vs craster.
...
Usage (uv brings numpy + nbt):
  uv run --no-project --with numpy --with nbt python3 trace/world_diff.py \
      --region <save>/region --craster trace/out/craster_world_s0.bin \
...
# ---- craster binary reader -------------------------------------------------
def read_craster(path):
...
def main():
    ...
    ap.add_argument("--craster", required=True, help="world_dump.c .bin output")
    ...
    seed, cr_chunks = read_craster(args.craster)
    print(f"craster seed={seed}, {len(cr_chunks)} chunks loaded")
    ...
    for cz in range(args.cz0, args.cz0 + args.ncz):
        for cx in range(args.cx0, args.cx0 + args.ncx):
            ...
            if (cx, cz) not in cr_chunks:
                print(f"  chunk ({cx},{cz}): craster missing")
                continue
            ...
            print(f"  chunk ({cx},{cz}): exact-match {pct:6.2f}%  "
                  f"trees MC={tree_mc:5d} craster={tree_cr:5d}  "
                  f"biome cols match={bmatch}/256 "
                  f"MC_biomes={bio_mc_set} craster_biomes={bio_cr_set}")
            if cat:
