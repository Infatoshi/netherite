 ...
 * Java game consumes) through the VERIFIED mc-sim player tick inside the craster world,
 ...
 * craster game does NOT simulate emit JSON `null` (a SENTINEL) so the diff tool can report
 ...
 * effects, attack cooldown, hurt/death timers, ENTITIES (craster game wires nents=0), and
...
int main(int argc, char **argv) {
    ...
    /* MC yaw 180 -> craster looks -Z. Use the WRAPPED form -180 (identical direction):
    ...
    for (int t = 0; t < nticks; ++t) {
        ...
        fprintf(state, "],");
        /* entities + time: UNSIMULATED on the craster game path (nents=0, no world clock) */
