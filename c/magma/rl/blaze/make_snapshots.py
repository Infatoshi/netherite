...
Run (anvil):
  cd c/magma && uv run --no-project --with numpy,torch,matplotlib \
      python rl/blaze/make_snapshots.py
  cd c/magma && T0=1 uv run --no-project --with numpy \
      python rl/blaze/make_snapshots.py
...
RL = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # c/magma/rl
...
# .bsnp header layout (rl/blaze/blaze_snapshot.h): packed, little-endian.
HEAD_SIZE = 752
N_ITEMS_OFF = 724          # u32
