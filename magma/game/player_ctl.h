...
/* Synthetic hotbar-only Container.slotClick helper used by narrow core tests.
 * The shipped full-inventory/container path is gm_container_click. */
...
/* Consume one ItemFood stack which completed its 32-tick EAT action. */
...
/* EntityTracker's self velocity packet from an authoritative server vector.
 * Quantize the client components while retaining the exact server X/Z shadow
 * used by later damage acknowledgement. */
void gm_player_apply_tracked_velocity(
    struct PsvPlayer *pl, double x, double y, double z);
 ...
 * (checkpointed by GmRuntime beside the container drag state). dig_hx/hy/hz are window-LOCAL;
...
typedef struct {
    ...
    float  step_distance;
    int    step_next_distance;
    int    water_initialized;
    int    player_in_water;
} GmPlayerCtlSnap;
...
/* Consume the authoritative once-per-20-flight-ticks chest durability call. */
int  gm_player_take_elytra_damage(void);
/* Consume this tick's server-side DamageSource.FLY_INTO_WALL amount. */
