...
SPRITE_NAMES += ["ladder"]
# EntityItemFrame's two baked block models.  These still bind TextureMap just
# like ordinary block models, so keep their sprites in the terrain atlas.
# Append only: established sprite ids are consumed by generated fixtures.
SPRITE_NAMES += ["planks_birch", "itemframe_background"]
...
# Beacon's static block model uses this alongside the established glass and
# obsidian sprites. Append-only keeps every existing generated sprite id stable.
SPRITE_NAMES += ["beacon"]
# Canonical storage blocks are ordinary one-texture cubes. Append them as one
# family so gameplay state never falls through to the explicit stone model.
SPRITE_NAMES += [
    "coal_block",
    "diamond_block",
    "emerald_block",
...
# Stained glass uses one authored texture per EnumDyeColor metadata. Magenta
# is already present in the End City append above; keep this list duplicate-free.
SPRITE_NAMES += [
    "glass_white", "glass_orange", "glass_light_blue", "glass_yellow",
    "glass_lime", "glass_pink", "glass_gray", "glass_silver",
    "glass_cyan", "glass_purple", "glass_blue", "glass_brown",
    "glass_green", "glass_red", "glass_black",
]
# Structure Block has one cube-all texture for each legacy MODE metadata.
# Append-only keeps every established sprite id stable.
SPRITE_NAMES += [
    "structure_block_save", "structure_block_load",
    "structure_block_corner", "structure_block_data",
]
# EntityMinecartCommandBlock's default north-facing display model. Append-only
# preserves every established atlas sprite id.
SPRITE_NAMES += [
    "command_block_back", "command_block_conditional",
    "command_block_front", "command_block_side", "furnace_front_on",
]
