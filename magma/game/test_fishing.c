...
int main(void) {
...

    {
        ...
        for (int cycle = 0; cycle < 15; ++cycle) {
            CHECK(gm_runtime_cast_fishing_rod(&r, 0, 0) >= 0,
                  "dense fishing event fixture casts");
            CHECK(gm_runtime_retract_fishing_rod(&r) == 0,
                  "dense fishing event fixture retracts");
        }
        CHECK(r.fish_event_count == GM_RUNTIME_FISH_EVENTS + 1
                  && r.fish_events_cap > GM_RUNTIME_FISH_EVENTS
                  && r.fish_event_dropped == 0
                  && gm_runtime_fish_event_get(
                      &r, GM_RUNTIME_FISH_EVENTS, &fish_event)
                  && fish_event.kind == GM_FISH_EVENT_RETRACT
                  && fish_event.seq == GM_RUNTIME_FISH_EVENTS,
              "fish-event storage grows without dropping dense cast order");
        CHECK(gm_runtime_write_checkpoint(
                  &r, "test_fishing_event_capacity.checkpoint")
                  && gm_runtime_load_checkpoint(
                      &r, "test_fishing_event_capacity.checkpoint")
                  && r.fish_event_count == GM_RUNTIME_FISH_EVENTS + 1
                  && r.fish_events_cap > GM_RUNTIME_FISH_EVENTS
                  && r.fish_event_dropped == 0,
              "grown fish-event store survives checkpoint reload");
        (void)remove("test_fishing_event_capacity.checkpoint");
    ...
    {
        GmRuntime r;
        CHECK(init_flat(&r), "initialize collision/unload fishing fixture");
        CHECK(gm_runtime_set_inventory(&r, 0, 346, 1, 0)
                  && gm_runtime_set_block(&r, 9, 30, 8, 1, 0)
                  && gm_runtime_spawn_fish_hook_fixture(
                      &r, 93, 8.5, 30.5, 8.5, 2.0, 0.0, 0.0,
                      0.0F, 0.0F, 0, 0, 0, 0, 0, 0, 0,
                      0.0F, 0, 0, 0, raw, 0, 0.0),
              "restore flying hook against a solid block");
        CHECK(gm_world_block(r.world, 9, 30, 8) == 1,
              "solid collision block remains loaded");
        gm_runtime_tick_fishing(&r);
        CHECK(r.fish_hook.active && r.fish_hook.in_ground
                  && r.fish_hook.vx == 0.0 && r.fish_hook.vz == 0.0
                  && r.fish_hook.ticks_in_air == 0,
              "nearest block collision parks the hook");
        {
            GmRuntimeFishHook parked = r.fish_hook;
            CHECK(gm_runtime_set_dimension(&r, -1),
                  "unload hook owner dimension");
            for (int tick = 0; tick < 20; ++tick)
                gm_runtime_tick_fishing(&r);
            CHECK(!memcmp(&parked, &r.fish_hook, sizeof parked),
                  "unloaded hook state and RNG do not advance");
            CHECK(gm_runtime_set_dimension(&r, 0),
                  "return to hook owner dimension");
        }
        gm_runtime_destroy(&r);
    }
    {
        GmRuntime r;
        CHECK(init_flat(&r), "initialize entity-collision fishing fixture");
        CHECK(gm_runtime_set_inventory(&r, 0, 346, 1, 0)
                  && gm_runtime_spawn_mob_fixture(
                      &r, GM_MOB_PIG, 94, 10.0, 30.0, 8.5,
                      0.0, 0.0, 0.0, 0.0F, 10.0F, 1, 0, 0, 0)
                  && gm_runtime_spawn_fish_hook_fixture(
                      &r, 95, 8.5, 30.5, 8.5, 2.0, 0.0, 0.0,
                      0.0F, 0.0F, 0, 0, 0, 0, 0, 0, 0,
                      0.0F, 0, 0, 0, raw, 0, 0.0),
              "restore flying hook against a living target");
        gm_runtime_tick_fishing(&r);
        CHECK(r.fish_hook.active
                  && r.fish_hook.state == GM_FISH_STATE_HOOKED
                  && r.fish_hook.caught_kind == 1,
              "nearest living collision hooks the entity");
        gm_runtime_destroy(&r);
    }
    puts("fishing: PASS (timers, loot, hook/retract/events, collisions, unload, persistence)");
    return 0;
}
