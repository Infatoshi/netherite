...
/* craster-local dimension id from assets/blockmodels.c. Vanilla BlockMagma
...
/* Opt-in via CRASTER_SMOOTH=1. Maps to Java ao:2 smooth path. Default OFF
 * matches hard-scene / options ao:0 (BlockModelRenderer.renderModelFlat). */
static int smooth_light_enabled(void) {
    static int cached = -1;
    if (cached < 0) {
        const char *s = getenv("CRASTER_SMOOTH");
 ...
 * Legacy craster always applied 3-neighbour ao_level even with smooth off,
 * which systematically darkened canopy corners vs Fast goldens.
 * CRASTER_AO=1 re-enables vertex AO without smooth light averaging. */
static int vertex_ao_enabled(void) {
    if (smooth_light_enabled()) return 1;
    static int cached = -1;
    if (cached < 0) {
        const char *s = getenv("CRASTER_AO");
        cached = (s && atoi(s) != 0) ? 1 : 0;
    }
    return cached;
}
