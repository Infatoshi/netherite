...
__global__ void sps_run_one_dev(Env *e, TcfAux *aux, u64 seed, ChunkPrimer *primer, CpScratch *sc,
...
int main(int argc, char **argv) {
    ...
    for (round = 0; round < SPS_ROUNDS; ++round) {
        for (env = 0; env < n_envs; ++env) {
            sps_run_one_dev<<<1, 1>>>(d_e, d_aux, CBT_SEEDS[env], d_primer, d_sc, d_st,
    ...
    cudaMemcpy(&h_last, d_lines + (SPS_NTICKS - 1), sizeof(CbtEmitLine), cudaMemcpyDeviceToHost);
    ...
    fprintf(stderr, "sps_benchmark cuda: envs=%d ticks=%d rounds=%d steps=%llu elapsed=%.3fs sps=%.0f\n",
            n_envs, SPS_NTICKS, SPS_ROUNDS, (unsigned long long)total_steps, elapsed, sps);
