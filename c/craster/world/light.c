/* world/light.c - LIGHT module for craster (see world/light.h).
 *
 * Sky light, block light, and biome tint over the mc-sim voxel world.
 *
 * Blocks come from mc-sim's cp_provide_chunk as compact renderer model keys. A
 ...
 *
 * Biome tint  : real per-column biome id from the mc-sim GenLayer stack
...
static LChunk *gen_chunk(CrLight *L, int cx, int cz) {
     ...
     * at ANY (cx,cz) - not just the origin 2x2. popmc_decorate_chunk runs mc-sim's
    ...
    if (L->world_type == 0 && !getenv("CRASTER_NO_DECOR"))
...
static void compute_skylight_spread(CrLight *L) {
     ...
     * pixel diff is the horizontal flood over-brightening vs the real game. */
    {
        static int off = -1;
        if (off < 0) {
            const char *s = getenv("CRASTER_NO_SKYSPREAD");
            off = (s && atoi(s) != 0) ? 1 : 0;
        }
        if (off) return;
    }
 ...
 * is not loaded. Same value craster renders from (world_diff verifier hook). */
