...
#include "../../blaze/core/block_props_table.h"
...
static void test_collision_barrier(void) {
    ...
    CHECK(gm_state_to_model_key(gm_pack_state(138, 0)) == GM_MODEL_BEACON,
          "beacon reverse map");
    for (int meta = 0; meta < 4; ++meta) {
        uint16_t st = 0;
        int key = GM_MODEL_STRUCTURE_BLOCK_BASE + meta;
        CHECK(gm_state_to_model_key(gm_pack_state(255, meta)) == key,
              "structure block reverse map preserves mode");
        CHECK(gm_model_key_to_state(key, 0, &st) == GM_MAP_EXACT,
              "structure block model key supported");
        CHECK(gm_state_id(st) == 255 && gm_state_meta(st) == meta,
              "structure block model key round-trips mode");
    }
    for (int meta = 0; meta < 16; ++meta) {
        uint16_t st = 0;
        int key = GM_MODEL_STAINED_GLASS_BASE + meta;
        CHECK(gm_state_to_model_key(gm_pack_state(95, meta)) == key,
              "stained glass reverse map preserves color");
        CHECK(gm_model_key_to_state(key, 0, &st) == GM_MAP_EXACT,
              "stained glass model key supported");
        CHECK(gm_state_id(st) == 95 && gm_state_meta(st) == meta,
              "stained glass model key round-trips color");
    }
    {
        static const int ids[] = {42, 41, 22, 57, 133, 152, 173};
        static const int keys[] = {
            GM_MODEL_IRON_BLOCK, GM_MODEL_GOLD_BLOCK,
            GM_MODEL_LAPIS_BLOCK, GM_MODEL_DIAMOND_BLOCK,
            GM_MODEL_EMERALD_BLOCK, GM_MODEL_REDSTONE_BLOCK,
            GM_MODEL_COAL_BLOCK,
        };
        for (unsigned i = 0; i < sizeof ids / sizeof ids[0]; ++i) {
            uint16_t st = 0;
            CHECK(gm_state_to_model_key(gm_pack_state(ids[i], 0)) == keys[i],
                  "storage block reverse map");
            CHECK(gm_model_key_to_state(keys[i], 0, &st) == GM_MAP_EXACT,
                  "storage block model key supported");
            CHECK(gm_state_id(st) == ids[i] && gm_state_meta(st) == 0,
                  "storage block model key round-trips");
        }
    }
          ...
          "dead chorus flower reverse map");
    {
        ...
        CHECK(gm_model_key_to_state(GM_MODEL_BEACON, 0, &st) == GM_MAP_EXACT,
              "beacon key supported");
        CHECK(gm_state_id(st) == 138 && gm_state_meta(st) == 0,
              "beacon key -> id 138");
...
static void test_causes_suffocation(void) {
    CHECK(gm_block_causes_suffocation_1_11_2(1, 0),
          "stone causes suffocation");
    CHECK(!gm_block_causes_suffocation_1_11_2(18, 0),
          "leaves override causesSuffocation false");
    CHECK(!gm_block_causes_suffocation_1_11_2(20, 0),
          "glass default state is not a full cube");
    CHECK(gm_block_causes_suffocation_1_11_2(219, 0),
          "shulker box override causesSuffocation true");
    CHECK(gm_block_causes_suffocation_1_11_2(33, 0),
          "retracted piston causes suffocation");
    CHECK(!gm_block_causes_suffocation_1_11_2(33, 8),
          "extended piston does not cause suffocation");
    CHECK(!gm_block_causes_suffocation_1_11_2(44, 8),
          "upper slab does not cause suffocation");
    CHECK(mc_bpt_props(219).hardness == 2.0F
              && mc_bpt_props(219).light_opacity == 0
              && (mc_bpt_props(219).flags & BF_SOLID),
          "Shulker Box keeps rock collision but is non-opaque to light");
    CHECK(mc_bpt_props(138).hardness == 3.0F
              && mc_bpt_props(138).light_emit == 15
              && mc_bpt_props(138).light_opacity == 0
              && (mc_bpt_props(138).flags & BF_SOLID),
          "Beacon emits level 15 and is non-opaque to light");
    CHECK(mc_bpt_props(52).hardness == 5.0F
              && mc_bpt_props(52).light_opacity == 0
              && (mc_bpt_props(52).flags & BF_SOLID),
          "mob spawner keeps rock collision but is non-opaque to light");
}
