...
static void emit_drop(const GmLiveEnt *item) {
    printf("{\"eid\":%d,\"item\":%d,\"count\":%d,\"meta\":%d,"
        ...
        item->x, item->y, item->z, item->mx, item->my, item->mz,
...
int main(int argc, char **argv) {
    ...
    if (strcmp(mode, "count1") && strcmp(mode, "count3")
            ...
            && strcmp(mode, "mooshroom")
            && strcmp(mode, "mooshroom_child")
            && strcmp(mode, "mooshroom_unbreaking")
            && strcmp(mode, "snowman")
            && strcmp(mode, "snowman_removed")
            && strcmp(mode, "snowman_unbreaking")) {
    ...
    int mooshroom = !strncmp(mode, "mooshroom", 9);
    int snowman = !strncmp(mode, "snowman", 7);
    int hand_slot = !strcmp(mode, "adult_offhand")
        ? ISR_OFFHAND_SLOT : 0;
    int initially_sheared = !strcmp(mode, "already_sheared");
    int child = !strcmp(mode, "child") || !strcmp(mode, "mooshroom_child");
    int held_item = !strcmp(mode, "non_shears") ? 280 : 359;
    int tool_meta = 0;
    int unbreaking = (!strcmp(mode, "unbreaking")
        || !strcmp(mode, "mooshroom_unbreaking")
        || !strcmp(mode, "snowman_unbreaking")) ? 3 : 0;
    ...
    int slot = gm_mobs_spawn_exact(
        &mobs, mooshroom ? EW_TYPE_MOOSHROOM
            : snowman ? EW_TYPE_SNOWMAN : EW_TYPE_SHEEP, sheep_eid,
        x, y, z, 0.0, 0.0, 0.0, mooshroom ? 37.25F : 0.0F,
        mooshroom ? 7.5F : snowman ? 4.0F : 8.0F, 1, 0, 0, 0);
    int random_ok = gm_mobs_set_entity_random_state(
        &mobs, sheep_eid, entity_seed, 0, 0.0);
    int sheep_ok = mooshroom || snowman || gm_mobs_set_sheep_state(
        &mobs, sheep_eid, 14, initially_sheared);
    int snowman_ok = !snowman || gm_mobs_restore_snowman_state(
        &mobs, sheep_eid, strcmp(mode, "snowman_removed") != 0);
    int age_ok = snowman || gm_mobs_set_growing_age(
        &mobs, sheep_eid, child ? -100 : 0);
    if (slot < 0 || !random_ok || !sheep_ok || !snowman_ok || !age_ok) {
        fprintf(stderr, "fixture setup failed: slot=%d random=%d sheep=%d "
                        "snowman=%d age=%d\n",
                slot, random_ok, sheep_ok, snowman_ok, age_ok);
        return 1;
    }
    if (mooshroom) {
        mobs.entity_pitch[slot] = -12.5F;
        mobs.squid_render_yaw_offset[slot] = -21.75F;
    }
    ...
    if (snowman) {
        ...
        printf("{\"ok\":true,\"result_code\":%d,\"eid\":%d,"
               "\"x\":%.17g,\"y\":%.17g,\"z\":%.17g,"
               "\"pumpkin\":%s,\"entity_seed48\":%" PRIu64 ","
               "\"math_seed48\":%" PRIu64 ","
               "\"shear_random_constructed\":%s,"
               "\"shear_seed48\":%" PRIu64 ",\"next_entity_id\":%d,"
               "\"tool_item\":%d,\"tool_count\":%d,"
               "\"tool_meta\":%d,\"tool_unbreaking\":%d,"
               "\"drops\":[],\"events\":[]}\n",
            result, sheep_eid, x, y, z,
            mobs.snowman_pumpkin[slot] ? "true" : "false",
            mobs.entity_random[slot].random.seed, math_seed,
            result == 2 ? "true" : "false", shear_seed, next_id,
            after.item, after.count, after.meta,
            enchantment_level(&after, 34));
        return 0;
    }
    if (mooshroom) {
        int cow_slot = sheep_slot(&mobs, sheep_eid + 1);
        ICStack after = isr_get_stack(&inventory, hand_slot);
        const EwStore *s = mobs.current ? &mobs.b : &mobs.a;
        printf("{\"ok\":true,\"result_code\":%d,\"eid\":%d,"
               "\"x\":%.17g,\"y\":%.17g,\"z\":%.17g,"
               "\"source_dead\":%s,\"cow_present\":%s,",
            result, sheep_eid, x, y, z,
            sheep_slot(&mobs, sheep_eid) < 0 ? "true" : "false",
            cow_slot > 0 ? "true" : "false");
        if (cow_slot > 0)
            printf("\"cow_eid\":%d,\"cow_x\":%.17g,"
                   "\"cow_y\":%.17g,\"cow_z\":%.17g,"
                   "\"cow_yaw\":%.9g,\"cow_pitch\":%.9g,"
                   "\"cow_render_yaw\":%.9g,\"cow_health\":%.9g,"
                   "\"cow_growing_age\":%d,",
                s->id[cow_slot], s->x[cow_slot], s->y[cow_slot],
                s->z[cow_slot], (double)s->yaw[cow_slot],
                (double)mobs.entity_pitch[cow_slot],
                (double)mobs.squid_render_yaw_offset[cow_slot],
                (double)s->health[cow_slot], mobs.growing_age[cow_slot]);
        printf("\"growing_age\":%d,\"entity_seed48\":%" PRIu64 ","
               "\"math_seed48\":%" PRIu64 ","
               "\"shear_random_constructed\":%s,"
               "\"shear_seed48\":%" PRIu64 ",\"next_entity_id\":%d,"
               "\"tool_item\":%d,\"tool_count\":%d,"
               "\"tool_meta\":%d,\"tool_unbreaking\":%d,\"drops\":[",
            child ? -100 : 0, entity_seed, math_seed,
            result == 2 ? "true" : "false", shear_seed, next_id,
            after.item, after.count, after.meta,
            enchantment_level(&after, 34));
        int first = 1;
        ...
        for (int i = 0; i < gm_mobs_event_count(&mobs); ++i) {
            ...
            printf("{\"kind\":\"sound\",\"eid\":%d,"
                   "\"sound\":\"minecraft:entity.mooshroom.shear\","
                   "\"category\":\"neutral\","
                   "\"x\":%.17g,\"y\":%.17g,\"z\":%.17g,"
                   "\"volume\":%.9g,\"pitch\":%.9g}",
