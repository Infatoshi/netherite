...
static int animal_type(const char *species) {
    ...
    if (!strcmp(species, "horse")) return EW_TYPE_HORSE;
    return EW_TYPE_NONE;
}

static int horse_type(const char *kind) {
    if (!strcmp(kind, "horse")) return EW_TYPE_HORSE;
    if (!strcmp(kind, "donkey")) return EW_TYPE_DONKEY;
    return EW_TYPE_NONE;
}

static const char *horse_kind(int type) {
    if (type == EW_TYPE_HORSE) return "horse";
    if (type == EW_TYPE_DONKEY) return "donkey";
    if (type == EW_TYPE_MULE) return "mule";
    return "unknown";
}
...
static void print_animal(
    ...
    if (gm_mobs_horse_type(s->type[slot]))
        printf(",\"horse_kind\":\"%s\",\"tame\":%s,"
               "\"max_health_bits\":\"%016" PRIx64 "\","
               "\"speed_bits\":\"%016" PRIx64 "\","
               "\"jump_bits\":\"%016" PRIx64 "\",\"variant\":%d",
            horse_kind(s->type[slot]),
            (m->horse_status[slot] & GM_HORSE_TAME) ? "true" : "false",
            double_bits(m->horse_max_health[slot]),
            double_bits(m->horse_movement_speed[slot]),
            double_bits(m->horse_jump_strength[slot]),
...
int main(int argc, char **argv) {
    int generic = argc == 35 || argc == 37;
    int horse_args = argc == 37;
    int shift = generic ? 1 : 0;
    const char *species = generic ? argv[1] : "sheep";
    const char *first_kind = horse_args ? argv[35] : "";
    const char *second_kind = horse_args ? argv[36] : "";
    ...
    if ((argc != 28 && argc != 35 && argc != 37)
            ...
            || (!strcmp(species, "horse")
                ? (!horse_args || horse_type(first_kind) == EW_TYPE_NONE
                    || horse_type(second_kind) == EW_TYPE_NONE
                    || pair_count != 1)
                : horse_args)
        ...
        fprintf(stderr, "usage: %s [SPECIES] FIRST_COLOR SECOND_COLOR DISTANCE "
            ...
            "[FIRST_HORSE_KIND SECOND_HORSE_KIND]]\n",
            argv[0]);
        return 2;
    }
    ...
    int first = horse_args
        ? gm_mobs_spawn_horse_exact(
            &runtime.mobs, horse_type(first_kind), parent_base_id,
            x, y, z, 0.0, 0.0, 0.0, 0.0F, 25.0F, 0,
            25.0, 0.25, 0.75, 0, GM_HORSE_TAME, 0,
            horse_type(first_kind) == EW_TYPE_HORSE ? 0x304 : 0,
            0, 0, 0, 0)
        : gm_mobs_spawn(&runtime.mobs, type, x, y, z);
    int second = horse_args
        ? gm_mobs_spawn_horse_exact(
            &runtime.mobs, horse_type(second_kind), parent_base_id + 1,
            x + distance, y, z + z_offset,
            0.0, 0.0, 0.0, 0.0F, 20.0F, 0,
            20.0, 0.20, 0.60, 0, GM_HORSE_TAME, 0,
            horse_type(second_kind) == EW_TYPE_HORSE ? 0x102 : 0,
            0, 0, 0, 0)
        : gm_mobs_spawn(
    ...
    runtime.mobs.tick = 1;
    if (horse_args && first > 0 && second > 0) {
        runtime.mobs.controlled_no_ai[first] = 0;
        runtime.mobs.controlled_no_ai[second] = 0;
        runtime.mobs.controlled_block_collisions[first] = 0;
        runtime.mobs.controlled_block_collisions[second] = 0;
    }
    if (first < 0 || second < 0
    ...
    runtime.mobs.sheep_mate_eid[first] = runtime.mobs.a.id[second];
    runtime.mobs.sheep_mate_delay[first] = 59;
    if (pair_count == 2) {
        runtime.mobs.sheep_mate_active[third] = 1;
        runtime.mobs.sheep_mate_eid[third] = runtime.mobs.a.id[fourth];
