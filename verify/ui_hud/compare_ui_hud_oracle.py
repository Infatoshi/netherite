...
def roi_rect(name):
    ...
    if name in ("hud_boss_half", "hud_wither_half"):
...
def evaluate_hand_exact(sid, ja_full, jb_full, c_full):
    ...
    if _HAND_EMPTY_FULL is not None:
        empty = crop(_HAND_EMPTY_FULL, rect)[:h, :w]
        # Same-scene empty-hand Java capture removes wall texture from subject
        # ownership. The border-median heuristic classified interior wall
        # texels as viewmodel and priced backdrop differences as hand errors.
        j_subj = np.abs(ja.astype(np.int16) - empty.astype(np.int16)).max(
            axis=2) > HAND_SUBJECT_THR
    else:
    ...
    return {
        ...
        "reason": reason,
        "java_subject": "empty_hand_differential" if _HAND_EMPTY_FULL is not None
                        else "border_backdrop",
...
def oracle_core_feature(img, sid):
    ...
    elif sid in ("hud_boss_half", "hud_wither_half"):
...
# Cached RGBA sprites from the generated HUD atlas. Core state masks use the
# source alpha at the exact Java draw coordinates; deriving ownership from the
# live-world RGB can accidentally classify stone/water pixels as icon texels.
_HUD_SPRITE_RGBA = None


def _load_hud_sprite_rgba():
    global _HUD_SPRITE_RGBA
    if _HUD_SPRITE_RGBA is not None:
        return _HUD_SPRITE_RGBA

    import re
    atlas_path = os.path.abspath(os.path.join(
        os.path.dirname(__file__), "..", "..", "magma", "assets",
        "hud_atlas.h"))
    text = open(atlas_path, "r").read()
    rows = re.findall(
        r'\{\s*"([A-Z0-9_]+)",\s*(\d+),\s*(\d+),\s*(\d+)\s*\}', text)
    start = text.index("static const unsigned char HUD_RGBA[")
    brace = text.index("{", start)
    end = text.index("};", brace)
    blob = np.array([int(v) for v in re.findall(r"\d+", text[brace:end])],
                    dtype=np.uint8)
    sprites = {}
    for name, sw, sh, off in rows:
        sw, sh, off = int(sw), int(sh), int(off)
        size = sw * sh * 4
        sprites[name] = blob[off:off + size].reshape(sh, sw, 4)
    _HUD_SPRITE_RGBA = sprites
    return sprites


def _place_hud_alpha(mask, name, x, y):
    alpha = _load_hud_sprite_rgba()[name][:, :, 3] > 0
    scaled = np.repeat(np.repeat(alpha, S, axis=0), S, axis=1)
    sh, sw = scaled.shape
    mask[y:y + sh, x:x + sw] |= scaled


def pinned_core_source_mask(sid):
    """Exact source ownership for deterministic hurt, food, and air pins."""
    m = np.zeros((H, W), dtype=bool)
    if sid in ("hud_hurt_flash_on", "hud_hurt_flash_off"):
        bg = "HEART_BG_FLASH" if sid.endswith("_on") else "HEART_BG"
        for i in range(10):
            x = HB_X + i * 8 * S
            _place_hud_alpha(m, bg, x, J1)
            if sid.endswith("_on"):
                _place_hud_alpha(m, "HEART_FLASH_FULL", x, J1)
            if i < 7:  # health=14
                _place_hud_alpha(m, "HEART_FULL", x, J1)
        return m
    if sid == "hud_hunger_poison":
        right = HB_X + 182 * S
        for i in range(10):
            x = right - (i * 8 + 9) * S
            _place_hud_alpha(m, "HUNGER_POISON_BG", x, J1)
            if i < 4:  # food=8
                _place_hud_alpha(m, "HUNGER_POISON_FULL", x, J1)
        return m
    if sid == "hud_air_partial":
        right = (CX + 91) * S
        y = (SH - 49) * S
        for i in range(5):  # air=121: 4 full + 1 partial
            x = right - (i * 8 + 9) * S
            _place_hud_alpha(m, "AIR_FULL" if i < 4 else "AIR_PARTIAL", x, y)
        return m
    return None
...
def core_compare_mask(sid, ja_full, c_full, painted_full):
    ...
    pinned = pinned_core_source_mask(sid)
    if pinned is not None:
        # Source ownership catches missing Java/C pixels. Candidate paint
        # outside it catches shifted or extra output without importing the
        # unrelated Java world underlay.
        return pinned | painted_full
...
def evaluate_core(sid, ja_full, jb_full, c_full, margin):
    ...
    if n_hard_px > 0:
        ...
        step = max(1, n_hard_px // RESIDUAL_LOC_SAMPLES)
        for i in range(0, n_hard_px, step):
            if len(residual_locs) >= RESIDUAL_LOC_SAMPLES:
                break
            yi, xi = int(ys[i]), int(xs[i])
            residual_locs.append({
...
def run_compare(goldens, cframes, margin, report_path=""):
    global _HAND_EMPTY_FULL
    empty_path = os.path.join(goldens, "_hand_empty_baseline.png")
    _HAND_EMPTY_FULL = load_rgb(empty_path) if os.path.isfile(empty_path) else None
    ids = [
