#!/usr/bin/env python3
"""Test skip_render mode to find throughput ceiling."""
...
def main():
    ...
    print("=" * 60, flush=True)
    print("SKIP RENDER TEST - Finding throughput ceiling", flush=True)
    print("=" * 60, flush=True)
    ...
    cfg = NetheriteConfig(
        ...
        max_fps=260,
        obs_mode="voxels",
        skip_render=True,  # Skip rendering entirely!
        java_home="/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home",
    )

    print(f"obs_mode='{cfg.obs_mode}', skip_render={cfg.skip_render}", flush=True)
    ...
    print("=" * 60, flush=True)
    print(
        f"RESULT: skip_render=True achieves {sps_step:.0f}-{sps_state:.0f} sps",
        flush=True,
    )
    print("=" * 60, flush=True)
