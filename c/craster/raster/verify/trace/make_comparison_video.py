...
def main():
    ...
    with args.tape.open() as stream:
        for line in stream:
            ...
            if ("frame" in row and tick >= args.start_tick
                ...
                rows.append(row)
    c_frames = np.load(args.replay_dir / "craster_frames.npy", mmap_mode="r")
    c_ticks = np.load(args.replay_dir / "craster_frames.ticks.npy")
    ...
    try:
        for row in rows:
            ...
            craster = np.asarray(c_frames[c_index[tick]])
            if oracle.shape != craster.shape or oracle.shape[:2] != (height, width):
                raise RuntimeError(f"frame shape mismatch at tick {tick}: "
                                   f"{oracle.shape} vs {craster.shape}")
            canvas = Image.new("RGB", (out_width, out_height), "black")
            canvas.paste(Image.fromarray(oracle), (0, header_height))
            canvas.paste(Image.fromarray(craster), (width, header_height))
