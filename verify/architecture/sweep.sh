...
usage: bash verify/architecture/sweep.sh [--full] [--run] [--out DIR]
  ...
  --variants CSV        cpu,hybrid_dense,hybrid_delta,cuda,fine1_g0,fine1_g1,fine32_g0,fine32_g1
...
full=0; run=0; out=; batches=; workers=; reps=; steps=; warm=; variants=; workloads=; policy=1; checkpoint=out/verify/architecture/inputs/policy.bin; timeout_s=300; budget=; retries=1; foreign_cpu=20; gpu_index=0
while (($#)); do
 case $1 in
 --help|-h) usage; exit 0;; --full) full=1; shift;; --run) run=1; shift;;
 --out|--batches|--workers|--repeats|--steps|--warmup|--variants|--workloads|--policy|--checkpoint|--timeout|--max-seconds|--retries|--foreign-cpu|--gpu-index)
...
: "${variants:=cpu,hybrid_dense,hybrid_delta,cuda,fine1_g0,fine1_g1,fine32_g0,fine32_g1}"
for v in "$reps" "$steps" "$warm" "$timeout_s" "$budget" "$retries" "$foreign_cpu" "$gpu_index" "$policy"; do [[ $v =~ ^[0-9]+$ ]] || { echo "invalid unsigned integer: $v" >&2; exit 2; }; done
((reps>0 && steps>0 && timeout_s>0 && budget>0 && policy<=1)) || exit 2
...
for v in "${variant_list[@]}"; do case $v in cpu|hybrid_dense|hybrid_delta|cuda|fine1_g0|fine1_g1|fine32_g0|fine32_g1);; *) echo "bad variant $v" >&2; exit 2;; esac; done
...
for ((r=0;r<reps;r++)); do for w in "${work_list[@]}"; do for n in "${batch_list[@]}"; do
 ...
 for ((k=0;k<${#variant_list[@]};k++)); do v=${variant_list[$(((k+r)%${#variant_list[@]}))]}
  [[ $w != det_ai || $v == cpu || $v == hybrid_* ]] || continue
  active_workers=(1); [[ $v != cpu && $v != hybrid_* ]] || active_workers=("${worker_list[@]}")
...
{ printf '%s\n' "$measure" "$0"; for row in "${rows[@]}"; do read -r r w n threads v <<< "$row"; build_cmd "$v" "$w" "$n" "$threads"; for ((i=1;i<${#cmd[@]};i++)); do case ${cmd[$i]} in --fixture) tr ',' '\n' <<< "${cmd[$((i+1))]}";; --lib|--checkpoint|--cubin) printf '%s\n' "${cmd[$((i+1))]}";; --fine) printf '%s\n' "$base/fine/pre_actions.cubin" "$base/fine/pure_player.cubin" "$base/fine/after_player.cubin" "$base/driver_dispatch.so";; esac; done; [[ $v != hybrid_* ]] || echo "$base/env_cuda_obs.so"; done; [[ -z $checkpoint || ! -f $checkpoint.policy.conf ]] || echo "$checkpoint.policy.conf"; } | sort -u > "$out/input-files.txt"
...
classify_load() {
 ...
 getline line<c1;split(line,a," ");getline line<c2;split(line,b," ");busy=0;total=0;for(i=2;i<=9;i++){d=b[i]-a[i];total+=d;if(i!=5&&i!=6)busy+=d}}
 FNR==NR {old[$1]=$3;parent[$1]=$2;next} {parent[$1]=$2;now[$1]=$3}
 END {dt=(t2-t1)/1e9;used=0;for(p in now){q=p;for(j=0;j<100&&q>1;j++){if(q in own){own[p]=1;break}q=parent[q]}if(p in own){delta=now[p]-(p in old?old[p]:0);if(delta>0)used+=delta}}
 ...
 awk -F, -v owner="$owner" -v shell="$$" -v known="$owned" 'BEGIN{while((getline p<known)>0)owned[p]=1;close(known)}NR==FNR{split($0,a," ");parent[a[1]]=a[2];next}$1~/^[ ]*[0-9]+[ ]*$/{p=$1+0;q=p;own=0;for(j=0;j<100&&q>1;j++){if(q==owner||q==shell||q in owned){own=1;break}q=parent[q]}if(!own)print "CONTENTION gpu_pid="p}' "$b.proc" "$b.gpu-processes" >> "$result"
}
for row in "${rows[@]}"; do
 ...
 for ((attempt=0;attempt<=retries;attempt++)); do
  ...
  if grep -Eq 'CONTENTION|INVALID' "$d/load.txt"; then classification=invalid_preload
   ...
   set +e; wait "$child"; rc=$?; wait "$monitor"; set -e; child=; monitor=
   snapshot "$d/post"
   ...
   NR==FNR&&$1=="cpu"{for(i=2;i<=9;i++)a[i]=$i;next}$1=="cpu"{busy=0;for(i=2;i<=9;i++)if(i!=5&&i!=6)busy+=$i-a[i];seconds=(ns2-ns1)/1e9;residual=busy/hz-t["user_s"]-t["sys_s"];if(residual<0)residual=0;pct=seconds>0?100*residual/seconds:99999;printf "whole_run_residual_cpu_pct=%.3f includes_monitor_overhead=1\n",pct;if(!("user_s" in t))print "INVALID process_timing";else if(pct>limit)print "CONTENTION whole_run_cpu"}' "$d/pre1.cpu" "$d/post.cpu" >> "$d/load.txt"
