#!/usr/bin/env bash
# Play craster_game over VNC on anvil (headless). Starts Xvfb :1 + openbox + x11vnc
...
#
# Env: CRASTER_VIEW_RADIUS (default 6; higher = longer view, lower FPS. 12 = full/verified).
...
VNCPW="redstone"
W=1280; H=720
RADIUS="${CRASTER_VIEW_RADIUS:-6}"

echo "=== building craster_game ==="
...
pkill -f "craster_game" 2>/dev/null
sleep 0.5
...
echo "=== launching craster_game (ESC to quit) ==="
exec env CRASTER_VIEW_RADIUS="$RADIUS" "$DIR/craster_game" --seed 0 --w "$W" --h "$H"
