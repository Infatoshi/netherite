...
static float  g_boss_color_modifier = 0.0f;
...
void gm_sky_set_boss_color_modifier(float amount) {
#if !defined(__CUDA_ARCH__)
    g_boss_color_modifier = clamp01(amount);
#else
    (void)amount;
#endif
}
...
static CR_HD GmSkyCtx gm_sky_ctx(float time_of_day) {
    ...
    c.fog.x *= 1.0f - 0.3f * g_boss_color_modifier;
    c.fog.y *= 1.0f - 0.4f * g_boss_color_modifier;
    c.fog.z *= 1.0f - 0.4f * g_boss_color_modifier;
    {
...
CrRgba gm_terrain_fog_color(float time_of_day) {
    ...
    gm_void_blindness_rgb(
        ...
        g_fog_feet_y, g_void_fog_y_factor);
    fog.x *= 1.0f - 0.3f * g_boss_color_modifier;
    fog.y *= 1.0f - 0.4f * g_boss_color_modifier;
    fog.z *= 1.0f - 0.4f * g_boss_color_modifier;
    {
        CrLightmapRgb nv = cr_night_vision_rgb(
