/* CUDA driver for combat_knockback_resist - same core as CPU path. */
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
__global__ void run_ckr(u64 *out, int *count) {
    if (threadIdx.x || blockIdx.x) return;
    EmitCtx ctx;
    ctx.buf = out;
    ctx.k = 0;
    mc_ckr_run_battery(d_emit, &ctx);
    *count = ctx.k;
}
...
int main(int argc, char **argv) {
    ...
    u64 *d_out;
    int *d_count;
    cudaMalloc(&d_out, sizeof(u64) * MC_CKR_NOUT);
    cudaMalloc(&d_count, sizeof(int));
    run_ckr<<<1, 1>>>(d_out, d_count);
