/* CUDA: same core/tree_gen_taiga.h as CPU; single-thread worldgen on device. */
#include <cstdio>
#include <cstdlib>
#include "../core/tree_gen_taiga.h"

__global__ void run_tgt(i64 seed, ChunkPrimer *primer, CpScratch *sc) {
    if (threadIdx.x || blockIdx.x) return;
    tgt_run(primer, sc, seed);
}
...
int main(int argc, char **argv) {
    ...
    run_tgt<<<1, 1>>>(seed, d_primer, d_sc);
