/* craster - shared per-fragment shader (core/shade.c).
 *
...
CR_HD CrRgba cr_atlas_sample(const CrTexture *tex, float u, float v) {
     ...
     * Optional CRASTER_TEXEL_BIAS{,_U,_V} (host-only) shifts nearest phase in
...
#if !defined(__CUDA_ARCH__)
    {
        ...
        if (!init) {
            const char *s = getenv("CRASTER_TEXEL_BIAS");
            if (s && *s) sbu = sbv = (float)atof(s);
            s = getenv("CRASTER_TEXEL_BIAS_U");
            if (s && *s) sbu = (float)atof(s);
            s = getenv("CRASTER_TEXEL_BIAS_V");
...
#if !defined(__CUDA_ARCH__)
    {
        static int sm = -2;
        if (sm == -2) {
            const char *s = getenv("CRASTER_SAMPLE_MODE");
            sm = (s && *s) ? atoi(s) : 0;
        }
        mode = sm;
    }
...
CR_HD CrRgba cr_shade(const CrShadeCtx *sh, const CrFragment *frag) {
    ...
    if (!alpha_test && sh->layer == CR_LAYER_SOLID) {
        static int sa = -1;
        if (sa < 0) {
            const char *s = getenv("CRASTER_SOLID_ALPHA");
            sa = (s && atoi(s) != 0) ? 1 : 0;
        }
        if (sa) alpha_test = 1;
    }
...
#if !defined(__CUDA_ARCH__)
    {
        static int flat = -1;
        if (flat < 0) {
            const char *s = getenv("CRASTER_FLAT_SHADE");
            flat = (s && atoi(s) != 0) ? 1 : 0;
        }
