...
int main(void) {
    ...
    /* Dragon dissolve markers (per-texel path uses light/ao; not box drop). */
    {
        ...
        for (int i = 0; i < mid; ++i) {
            ...
            float threshold = (packed - floorf(packed)) * 32.0f;
            if (!(out[i].light < 0.0f &&
                  fabsf(threshold - 0.5f) < 1e-4f && out[i].ao > 0.0f))
                ok = 0;
        }
        CHECK(ok, "dissolve verts pack deathTicks/200 and retain face AO");
        d.death_ticks = 200;
     ...
     * Not a Java pixel gate — proves the interactive shade path works. */
    {
        ...
        frag.light = -1.03125f; /* packed original light 0, threshold 1 */
