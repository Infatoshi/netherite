...
#include "game/hud.h"
#include "obs_camera.h"   /* mc-sim verified semantic camera (LUT trig) */
 ...
 * The camera is the mc-sim verified obs_camera.h source (LUT trig, DDA), so
 * the real env and the batched cuenv are bit-identical. oc_pixel reads an
 ...
 * rl/cuenv/cuenv_snapshot.h, SHARED with the batched-env reader - this file
...
#include "rl/cuenv/cuenv_snapshot.h"
...
static void rl_emit_obs(GmRuntime *r, int bin, int want_cam) {
     ...
     * what the batched cuenv computes from its CuPlayer doubles. */
    if (want_cam) {
