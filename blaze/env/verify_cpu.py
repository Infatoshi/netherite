...
class Blaze1:
    def __init__(self, snap):
        ...
        # The native --rl reference leaves standalone random block ticks live.
        # Batched training intentionally owns quiescent snapshot worlds, so
        # this exact hash-selector path is an explicit lockstep-only opt-in.
        self.lib.blaze_set_exact_randticks.argtypes = [ctypes.c_void_p,
                                                       ctypes.c_int]
        assert self.lib.blaze_set_exact_randticks(
            ctypes.c_void_p(self.h), 1) == 0
...
def run_seed(seed, snap, actions, label, show_final_inv=False):
    ...
    try:
        ...
        cu.close()
    if ok:
        suffix = (f"terminal matched at action {terminal_tick}"
                  if terminal_tick is not None
                  else f"{len(actions)} ticks")
        print(f"  seed {seed} [{label}]: {suffix}, ZERO diffs")
    return ok
