...
block format). Craster side = trace/world_dump (cp_provide_chunk terrain + owr_run
...
is legitimately missing decoration and the diff would lie), and emits:
  ...
  1. trace/out/wv_mismatch_s<seed>.csv  -- EVERY wrong cell:
       wx,wy,wz,java_id,craster_id      (world block coords, vanilla numeric ids)
  2. trace/out/wv_blobs_s<seed>.txt     -- mismatch cells clustered into connected
       ...
       count, and the dominant (java,craster) id pair. A misplaced tree = one blob.
...
read_craster = world_diff.read_craster
...
def dump_tiles(seed, chunks, tile, outdir, preplist=None, cascade=None):
    ...
    dump_bin = os.path.join(CRASTER, "trace", "world_dump")
    ...
    if preplist:
        ...
        r = subprocess.run(cmd, cwd=CRASTER, capture_output=True, text=True)
        if r.returncode != 0:
            sys.exit(f"world_dump failed:\n{r.stderr}")
        for line in r.stderr.splitlines():
            print("  " + line)
        _, ch = read_craster(out)
        os.remove(out)
        return ch
    tiles = {}
    for (cx, cz) in chunks:
...
def main():
    ...
    ap.add_argument("--out-dir", default=os.path.join(CRASTER, "trace", "out"))
    ...
    print(f"[2/4] craster dump ({len(chunks)} chunks, tile={args.tile})")
    ...
    with open(csv_path, "w") as f:
        f.write("wx,wy,wz,java_id,craster_id\n")
    ...
    with open(blob_path, "w") as f:
        for b in blobs:
            ...
            f.write(f"{len(b):5d} cells  bbox x[{min(xs)},{max(xs)}] y[{min(ys)},{max(ys)}] "
                    f"z[{min(zs)},{max(zs)}]  dominant java={bname(dom[0])} craster={bname(dom[1])}  "
    ...
    print("\ntop id-pair confusion (java -> craster):")
