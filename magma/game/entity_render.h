...
#endif
#ifndef GM_VIEW_EVOKER_FANGS
#define GM_VIEW_EVOKER_FANGS 54
#endif
#ifndef GM_VIEW_WITHER
#define GM_VIEW_WITHER 66
#endif
#ifndef GM_VIEW_WITHER_SKULL
...
/* Cold hanging-entity render views.  Simulation owns the authoritative stores;
 * frame_capture materializes these only on rendered ticks. */
typedef struct {
    float x, y, z;
    int facing;                 /* EnumFacing D-U-N-S-W-E, horizontal only */
    int art;                    /* EntityPainting.EnumArt ordinal 0..25 */
    float lm_light[16], lm_blk[16]; /* one pair per 16x16 art tile */
} GmPaintingView;

typedef struct {
    float x, y, z;
    float lm_light, lm_blk;     /* owning entity lightmap coordinates */
} GmLeashKnotView;

typedef struct {
    float x, y, z;              /* leashed EntityLiving feet */
    float yaw, width, height;   /* renderYawOffset + setSize dimensions */
    float holder_x, holder_y, holder_z;
    float holder_yaw, holder_pitch, holder_eye;
    int holder_hanging;         /* EntityHanging anchor branch */
} GmLeashRenderView;

typedef struct {
    int x, y, z;
    int meta;                    /* BlockEnderChest.FACING metadata */
    float prev_lid, lid;
    float light, blk;
} GmEnderChestRenderView;

typedef struct {
    int x, y, z;
    int meta;                    /* BlockChest horizontal metadata */
    float prev_lid, lid;
    float light, blk;
    int sprite;                  /* GM_CHEST_TEXTURE_* */
    int large;                   /* ModelLargeChest 30x14 footprint */
    int shift_x, shift_z;        /* Java double-chest pre-rotation translate */
} GmChestRenderView;

typedef struct {
    int x, y, z;
    int facing;                  /* EnumFacing D-U-N-S-W-E */
    int color;                   /* EnumDyeColor metadata 0..15 */
    float prev_progress, progress;
    float light, blk;
} GmShulkerBoxRenderView;

typedef struct {
    float red, green, blue;
    int height;
} GmBeaconBeamSegmentView;

typedef struct {
    int x, y, z;
    float texture_scale;         /* TileEntityBeacon.shouldBeamRender */
    double total_world_time;
    int segment_count;
    const GmBeaconBeamSegmentView *segments;
} GmBeaconRenderView;

enum {
    GM_CHEST_TEXTURE_ENDER = 0,
    GM_CHEST_TEXTURE_NORMAL,
    GM_CHEST_TEXTURE_TRAPPED,
    GM_CHEST_TEXTURE_NORMAL_DOUBLE,
    GM_CHEST_TEXTURE_TRAPPED_DOUBLE,
};
...
/* RenderGlobal's entity-outline source pass. Only glowing views are emitted;
 * invisible living models are intentionally restored because
 * RenderLivingBase renders them while renderOutlines is true. */
int       gm_entities_glowing_emit(const GmEntityView *ents, int n,
                                    CrVertex *out, int max);
int       gm_entities_glowing_slime_gel_emit(
                                    const GmEntityView *ents, int n,
                                    CrVertex *out, int max);

/* RenderPainting, RenderLeashKnot, and RenderLiving.renderLeash, transcribed
 * from the 1.11.2 client.  Painting light is sampled independently per 16x16
 * art tile, exactly like RenderPainting.setLightmap. */
int gm_painting_light_cells(const GmPaintingView *view, int out[][3], int max);
...
int gm_shulker_boxes_emit(const GmShulkerBoxRenderView *views, int n,
                          float partial_ticks, CrVertex *out, int max);
/* TileEntityBeaconRenderer's opaque rotating core and translucent outer glow
 * are separate outputs because their blend/depth-mask states differ. */
...
int       gm_minecart_contents_emit(const GmEntityView *ents, int n,
                                    float partial_ticks,
                                    CrVertex *out, int max);
/* RenderTntMinecart's second untextured white pass. Caller renders with
 * SRC_ALPHA,ONE-equivalent blend, depth LEQUAL, and depth write enabled. */
...
int gm_block_break_particles_emit(int wx, int wy, int wz, int block_id,
                                  ...
                                  CrVertex *out, int max);
/* Exact oracle seam for ParticleDigging rows observed after their settle
 * ticks. A process-local fixture imports current position, scale, tint, and
 * constructor-private UV jitter, then frame_capture uses this in preference
 * to the live deterministic reconstruction above. */
 ...
 *
 * Runtime block-spawner state constructs this view directly from the exact
 * saved SpawnData choice. The transform remains independently verified by
 * game/test_entity_render.c. */
typedef struct {
    int   wx, wy, wz;
    int   type;
    float mob_rotation;
    /* Cached entities never tick, so RenderLivingBase ageInTicks is exactly
     * the frame partial tick (handleRotationFloat = ticksExisted + partial). */
    float partial_ticks;
} GmSpawnerView;
