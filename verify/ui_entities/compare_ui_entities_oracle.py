...
def roi_rect(state_id):
    ...
    if state_id.startswith("wither_skull"):
        return (W // 2 - 100, 80, W // 2 + 100, H - 100)
    if state_id.startswith("wither"):
        return (W // 2 - 220, 20, W // 2 + 220, H - 40)
...
def family(state_id):
    ...
    if state_id.startswith("wither"):
        return "wither"
    if (state_id.startswith("horse_") or state_id.startswith("donkey_")
...
def particle_cli_args(meta_path):
    with open(meta_path, encoding="utf-8") as stream:
        meta = json.load(stream)
    result = []
    for row in meta.get("particle_state", []):
        if not isinstance(row, list) or len(row) != 8:
            raise ValueError("invalid particle_state row")
        particle_id = int(row[0])
        state = row[7]
        if particle_id == 9004:
            packed = int(state["lightmap"])
            values = [
                *state["pos"], state["scale"], *state["rgba"][:3],
                *state["jitter"],
                ((packed >> 16) & 65535) / 16.0,
                (packed & 65535) / 16.0,
            ]
            result.extend([
                "--dig-particle", ",".join(str(value) for value in values)])
            continue
        if particle_id == 1:
            age, max_age, scale = (
                state["life"], state["life_time"], state["size"])
        elif particle_id == 2:
            age, max_age, scale = state["time"], 8, state["scale"]
        else:
            age, max_age, scale = (
                state["age"], state["max_age"], state["scale"])
        values = [
            particle_id,
            *state["prev"], *state["pos"], *state["motion"],
            age, max_age, 1 if state["ground"] else 0, scale,
            *state["rgba"][:3],
            int(state["tex"][1]) * 16 + int(state["tex"][0]),
            int(state.get("base_tex", 0)),
        ]
        result.extend(["--particle", ",".join(str(value) for value in values)])
    return result
...
def main():
    ...
    for sid in states:
        ...
        if need_run:
            ...
            try:
                cmd.extend(particle_cli_args(meta))
            except (KeyError, TypeError, ValueError) as ex:
                print("FAIL %s particle metadata: %s" % (sid, ex))
                failed += 1
                results.append({"id": sid, "verdict": "FAIL",
                                "reason": "particle_meta"})
                continue
