...
craster headless game (--script set_time+set_pose), pixel-diff the pair over
...
def capture_oracle(env, cks, outdir):
    ...
    # Remove worldgen passive mobs: craster renders no entities, and a wandering
    ...
    for ck in cks:
        ...
        # craster must render from where the oracle camera ACTUALLY was.
    ...
    # craster from the pose the cached mc_<name>.png was actually grabbed at,
...
def render_craster(ck, outdir):
    ...
    t0 = time.time()
    ol.run_craster_script(scr, 2, frames, state, w=W, h=H)
    png = os.path.join(outdir, f"craster_{name}.png")
    ol.ppm_to_png(os.path.join(frames, "frame_000001.ppm"), png)
    shutil.rmtree(frames)
    ck["craster_png"] = png
    print(f"[ck] craster {name}: rendered in {time.time()-t0:.1f}s -> {png}")
...
def main():
    ...
    results = []
    for ck in cks:
        render_craster(ck, args.out)
        stats = ol.diff_regions(ck["mc_png"], ck["craster_png"], W, H)
    ...
    with open(md, "w") as f:
        ...
        f.write("Same seed-0 world, same pose (feet + MC yaw/pitch), same 854x480 "
                ...
                "craster = headless `--script` render, frame after 2 ticks. Diff = "
