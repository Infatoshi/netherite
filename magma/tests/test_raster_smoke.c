/* Integration smoke test of the whole magma raster spine.
 *
...
/* Rotate a world position about the Y axis by `ang` radians. Done in the test
...
static int write_ppm(const char *path, const CrFramebuffer *fb)
{
    ...
    if (!f) {
        fprintf(stderr, "raster smoke: cannot open %s for writing\n", path);
        return -1;
    }
    fprintf(f, "P6\n%d %d\n255\n", fb->w, fb->h);
    ...
    fclose(f);
    if (wrote != n) {
        fprintf(stderr, "raster smoke: short write to %s\n", path);
        return -1;
    }
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (!fb.color || !fb.depth) {
        fprintf(stderr, "raster smoke: cr_fb_alloc failed\n");
        return 1;
    }
    ...
    if (nverts != 36) {
        fprintf(stderr, "raster smoke: scene_cube returned %d (expected 36)\n", nverts);
        cr_fb_free(&fb);
        return 1;
    }
    ...
    if (!win) {
        fprintf(stderr, "raster smoke: cr_window_open failed\n");
        cr_fb_free(&fb);
        return 1;
    }
