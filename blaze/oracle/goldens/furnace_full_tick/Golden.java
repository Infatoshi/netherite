...
// Item/Block objects use the live Java 1.11.2 registry ids. The complete 51-row registry is
// separately locked by verify/completeness/smelting_registry_gate.py.
//
// CUT (matches core/furnace_full_tick.h): world.isRemote / BlockFurnace.setState,
// markDirty/NBT and output-slot experience. getCookTime is a
...
public class Golden {
    ...
    // ---- live item registry ids (core/smelting_recipes.h) ----
    static final int PLANKS = 5, LOG = 17, COAL = 263, DIAMOND = 264, IRON_INGOT = 265,
        GOLD_INGOT = 266, STICK = 280, BUCKET = 325, WATER_BUCKET = 326,
        REDSTONE = 331, LAVA_BUCKET = 327,
        PORKCHOP = 319, COOKED_PORKCHOP = 320, FISH = 349, COOKED_FISH = 350, DYE = 351,
        BEEF = 363, COOKED_BEEF = 364, CHICKEN = 365, COOKED_CHICKEN = 366,
        MUTTON = 423, COOKED_MUTTON = 424, RABBIT = 411, COOKED_RABBIT = 412,
        ROTTEN_FLESH = 367, BLAZE_ROD = 369, EMERALD = 388, POTATO = 392, BAKED_POTATO = 393,
        QUARTZ = 406, CHORUS_FRUIT = 432, CHORUS_FRUIT_POPPED = 433, DYE_BLUE = 4;
    ...
    void smelt() {
        ...
        if (slot0.item == 19 && slot0.meta == 1 && !isEmpty(slot1) && slot1.item == BUCKET)
            slot1 = mk(WATER_BUCKET, 1, 0);
        slot0.count--;
    ...
    void tick() {
        ...
        if (isBurning() || (!isEmpty(slot1) && !isEmpty(slot0))) {
            if (!isBurning() && canSmelt()) {
                ...
                if (isBurning()) {
                    int fuelItem = slot1.item;
                    slot1.count--;
                    if (slot1.count <= 0)
                        slot1 = fuelItem == LAVA_BUCKET ? mk(BUCKET, 1, 0) : empty();
                }
            }
            if (isBurning() && canSmelt()) {
                cookTime++;
