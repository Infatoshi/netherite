...
/* World creation's pre-populate getGroundAboveSeaLevel result. */
int      light_spawn_ground_id(const CrLight *, int wx, int wz);
int      light_biome(const CrLight *, int wx, int wz);         /* voronoi biome id, -1 unloaded */
/* Exact live Chunk.heightMap value (one above highest nonzero-opacity block). */
int      light_height(const CrLight *, int wx, int wz);        /* -1 unloaded */

/* DEBUG/CAPS: number of chunks currently loaded (LChunk store size). */
int      light_loaded_chunks(const CrLight *);
/* Monotonic resident-slot identity. Zero means absent; a recycled chunk that
 * returns to the same coordinates receives a different epoch. */
unsigned long long light_chunk_epoch(const CrLight *, int cx, int cz);
...
/* Test-only column override for an already-ensured chunk. Returns one when the
 * requested biome was installed. */
int      light_debug_set_biome(CrLight *, int wx, int wz, int biome);
/* Test-only saved block-light nibble override for an already-ensured cell.
 * Mirrors World.setLightFor(BLOCK) without running the relight flood. */
int      light_debug_set_block_light(
             CrLight *, int wx, int wy, int wz, int value);
...
int      light_load_block_snapshot(CrLight *, int wx, int wy, int wz, int value);
void     light_finalize_block_snapshot(CrLight *);
/* Exact persisted Chunk.heightMap column restore after block relight. */
int      light_load_height_snapshot(CrLight *, int wx, int wz, int value);
