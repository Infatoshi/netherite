 ...
 * sample_mode 88, entity_brightness 92, repeat_uv 96, depth_d24 100,
 * depth_bias_d24 104, subpixel_bits 108. size 112. */
typedef struct {
...
/* GmSkyCtx (game/sky.h): 100 bytes, align 4.
 * offsets: sky_top 0, fog 12, sunset_active 24, sunset 28, sun_h 44,
 * starB 56, cA 60, sA 64, celestial_alpha 68, uw 72, uw_fog 76,
 * uw_density 88, linear_fog_end 92, plane_y 96. */
typedef struct {
    CrVec3M sky_top, fog;
    int     sunset_active;
    float   sunset[4];
...
static_assert(sizeof(CrShadeCtxM)   == 112, "CrShadeCtx layout");
static_assert(sizeof(GmSkyCtxM)     == 100, "GmSkyCtx layout");
...
static inline float cr_depth24(float z, device const CrShadeCtxM *sh) {
    return floor(z * 16777215.0f + 0.5f)
        - (float)sh->depth_bias_d24;
}

static inline float cr_subpixel(float v,
                                device const CrShadeCtxM *sh) {
    int bits = sh->subpixel_bits;
    if (bits <= 0) return v;
    if (bits > 16) bits = 16;
    float scale = (float)(1u << (uint)bits);
...
static CrRgbaM cr_shade(device const CrShadeCtxM *sh,
    ...
    float cr, cg, cb;
    if (sh->entity_brightness) {
        int lmr8 = (int)(fmax(0.0f, fmin(1.0f, lmr)) * 255.0f + 0.5f);
        int lmg8 = (int)(fmax(0.0f, fmin(1.0f, lmg)) * 255.0f + 0.5f);
        int lmb8 = (int)(fmax(0.0f, fmin(1.0f, lmb)) * 255.0f + 0.5f);
        int lr = (int)(fmax(0.0f, fmin(1.0f, tr * la)) * (float)lmr8);
        int lg = (int)(fmax(0.0f, fmin(1.0f, tg * la)) * (float)lmg8);
        int lb = (int)(fmax(0.0f, fmin(1.0f, tb * la)) * (float)lmb8);
        cr = (float)(((int)texel.r * lr + 127) / 255) * inv255;
        cg = (float)(((int)texel.g * lg + 127) / 255) * inv255;
        cb = (float)(((int)texel.b * lb + 127) / 255) * inv255;
    } else {
        cr = (texel.r * inv255) * tr * la * lmr;
        cg = (texel.g * inv255) * tg * la * lmg;
        cb = (texel.b * inv255) * tb * la * lmb;
    }
...
kernel void cr_raster_tri_kernel(device CrRgbaM *color        [[buffer(0)]],
    ...
    if (sh->depth_d24) {
        ...
        float d24 = floor(depth[idx] * 16777215.0f + 0.5f);
    ...
    for (int batch = 0; batch < nbatches; ++batch) {
        ...
        for (int k = 0; k < total; k++) {
            ...
            if (shl->depth_d24) {
                ...
                float d24 = floor(curz * 16777215.0f + 0.5f);
                if (!(z24 < d24
                        || (shl->depth_lequal && z24 == d24))) continue;
            } else if (!(z < curz
...
static CrRgbaM gm_sky_ray_color_ctx(thread const GmSkyCtxM *sc, V3 dir_in,
    ...
    V3 col = (ey >= 0.0f) ? mc_sky_plane_fog(
