...
Inputs: two checkpoint logs with lines "TAG cx cz TYPE cursor":
  ...
  - craster side: trace/world_dump with CRASTER_GENPROBE=path (MC_PROBE checkpoints)
...
Usage:
  uv run --no-project python3 trace/genprobe_diff.py \
      --java trace/out/java_genprobe.log --craster trace/out/craster_genprobe.log
"""
import argparse
...
def main():
    ...
    ap.add_argument("--craster", required=True)
    ...
    c, _ = parse(args.craster)

    if args.chunk:
        key = tuple(args.chunk)
        js, cs = j.get(key, []), c.get(key, [])
        print(f"chunk {key}: java={len(js)} craster={len(cs)} checkpoints")
    ...
    print(f"chunks: java={len(j)} craster={len(c)} common={len(both)}")
    ...
    for (key, (i, label, jc, cc)) in first_div[: args.limit]:
        print(f"  chunk {key}  step {i:2d}  {label:20s} java={jc} craster={cc}")
