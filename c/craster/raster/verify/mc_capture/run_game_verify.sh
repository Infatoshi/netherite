...
# POSES list, render the seed-0 view-distance world through craster (game_candidate
...
#
# POSES here MUST mirror capture_poses.sh's POSES (same craster EYE + MC yaw/pitch)
# so each craster render registers to its golden. The craster camera convention is
# derived from the MC pose: craster_yaw = 180 - MC_yaw, craster_pitch = -MC_pitch
...
FB_H=480
FOV=70
# Lower-center terrain band where craster + MC both show ground (excludes the sky
...
for ((i=0; i<NP; i++)); do
  ...
  CAND_PNG="$OUT/craster_pose_$i.png"
  echo "== pose $i: eye($EX,$EY,$EZ) MC yaw=$MYAW pitch=$MPITCH -> craster yaw=$CYAW pitch=$CPITCH =="
...
echo "craster: $OUT/craster_pose_<i>.png"
exit $GATE
