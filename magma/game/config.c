...
void gm_config_defaults(GmConfig *cfg) {
    ...
    cfg->dump_config = 0;
    cfg->mob_griefing = 1;
    cfg->game_mode = GM_MODE_SURVIVAL;
}
...
int gm_config_parse(GmConfig *cfg, int argc, char **argv, char *err, int err_cap) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (!strcmp(a, "--gamemode")) {
            if (mark_once(&seen, SEEN_GAME_MODE, a, err, err_cap)) return 2;
            if (!(v = need_value(&i, argc, argv, err, err_cap))) return 2;
            if (!strcmp(v, "survival") || !strcmp(v, "0"))
                cfg->game_mode = GM_MODE_SURVIVAL;
            else if (!strcmp(v, "creative") || !strcmp(v, "1"))
                cfg->game_mode = GM_MODE_CREATIVE;
            else if (!strcmp(v, "adventure") || !strcmp(v, "2"))
                cfg->game_mode = GM_MODE_ADVENTURE;
            else if (!strcmp(v, "spectator") || !strcmp(v, "3"))
                cfg->game_mode = GM_MODE_SPECTATOR;
            else return fail(err, err_cap, "invalid gamemode: %s", v);
...
void gm_config_print(FILE *out, const GmConfig *c) {
    fprintf(out,
        ...
        "mobs=%s daylight=%s stats=%s mob_griefing=%s gamemode=%s "
        ...
        c->game_mode == GM_MODE_SURVIVAL ? "survival" :
        c->game_mode == GM_MODE_CREATIVE ? "creative" :
        c->game_mode == GM_MODE_ADVENTURE ? "adventure" : "spectator",
