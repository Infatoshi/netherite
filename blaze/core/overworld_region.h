...
static void (*owr_igloo_hook)(int, int, const IgIgloo *);
#endif
...
MC_HD static inline int owr_sd_contains_y(void *opaque, int x, int y, int z) {
    ...
    for (int scx = bcx; scx <= bcx + 1; ++scx)
        for (int scz = bcz; scz <= bcz + 1; ++scz) {
            ...
            if (biome != B_DESERT && biome != B_DESERT_HILLS
                    ...
                    && biome != B_COLD_TAIGA) continue;
            facing = owr_sd_facing(seed, scx, scz);
            if (biome == B_SWAMP) {
                local_x = 7;
                local_z = 9;
            } else if (biome == B_ICE_PLAINS || biome == B_COLD_TAIGA) {
                local_x = 7;
                local_z = 8;
            ...
            if (biome == B_SWAMP) {
                ...
                igloo.origin_x = scx * 16;
                igloo.origin_z = scz * 16;
                igloo.base_y = hpos - 1;
                {
                    SdAccess igloo_access = access;
                    /* Template.addBlocksToWorldChunk clears the component box
                     * when no chunk is attached, so 1.11.2 writes the complete
                     * rotated template, including neighboring chunks. */
                    igloo_access.contains = owr_sd_contains_y;
                    ig_igloo_generate(&igloo_access, &igloo, population_random);
                }
#ifndef __CUDA_ARCH__
                if (owr_igloo_hook) owr_igloo_hook(bcx, bcz, &igloo);
#endif
            } else if (biome == B_JUNGLE || biome == B_JUNGLE_HILLS) {
