...
struct GmWindowCompose {
    ...
    CrVertex *structure_verts;
    size_t structure_verts_cap;
    int max_tris;
...
static int window_transform_entity_nocull(
...
static int build_entity_outline(GmWindowCompose *c, const CrCamera *cam,
                                const GmEntityView *ents, int n) {
    CrFramebuffer mask;
    CrTexture atlas;
    CrShadeCtx shade = {0};
    int nv, nt;
    if (!c || !cam || !ents || n <= 0) return 0;
    ...
    nt = window_transform_entity_nocull(
...
#undef TSH
    ...
    sh_solid.use_mips = cr_cfg()->terrain_mips;
    sh_cmip.use_mips = cr_cfg()->terrain_mips;
    sh_cut.use_mips = cr_cfg()->terrain_mips;
    sh_trans.use_mips = cr_cfg()->terrain_mips;
    if (uw && uw->fluid) {
...
static void render_structures(GmWindowCompose *c, const CrCamera *cam,
                              CrRgba fog) {
    size_t required = gm_overlay_structure_vertices_required(
        c->runtime, cam);
    int nv;
    if (!required || required > (size_t)INT_MAX) return;
    if (required > c->structure_verts_cap) {
        CrVertex *grown = realloc(
            c->structure_verts, required * sizeof *grown);
        if (!grown) return;
        c->structure_verts = grown;
        c->structure_verts_cap = required;
    }
    nv = gm_overlay_emit_structures(
        c->structure_verts, (int)c->structure_verts_cap, c->runtime,
        cam, c->fb.w, c->fb.h);
    if (nv > 0) {
        CrShadeCtx shade = {0};
        shade.atlas = &c->atlas;
        ...
        shade.depth_lequal = 1;
        shade.depth_d24 = 1;
        render_layer(c, cam, c->structure_verts, nv, &shade);
    }
}
...
static int collect_entities(
        GmWindowCompose *c, GmEntityView *ents, int capacity) {
    GmRuntime *r = c->runtime;
    int n = gm_dragon_fill_views(&r->dragon, ents, capacity);
    n += gm_runtime_end_crystal_views(r, ents + n, capacity - n);
    n += gm_mobs_fill_views(&r->mobs, ents + n, capacity - n);
    n += gm_runtime_shulker_views(r, ents + n, capacity - n);
    n += gm_runtime_wither_views(r, ents + n, capacity - n);
    n += gm_runtime_armor_stand_views(r, ents + n, capacity - n);
    int projectile0 = n;
    n += gm_runtime_projectile_views(r, ents + n, capacity - n);
    {
        int *types = c->projectile_types;
        int nt = 0;
    ...
    n += gm_runtime_falling_block_views(r, ents + n, capacity - n);
    {
        int tape_falling = 0;
        ...
        n += gm_live_fill_views_filtered(&r->entities, ents + n, capacity - n,
                                         tape_falling);
    }
    n += gm_runtime_ghost_views(r, ents + n, capacity - n);
...
static void update_boss_state(GmWindowCompose *c,
    ...
    for (int i = 0; i < n; ++i) {
        if (ents[i].type != GM_VIEW_WITHER) continue;
        wither_seen = 1;
...
static int window_wither_boss_active(const GmRuntime *r) {
...
static void advance_boss_color_state(GmWindowCompose *c, int nticks) {
    int active = window_wither_boss_active(c->runtime);
    for (int i = 0; i < nticks; ++i) {
        c->boss_color_modifier_prev = c->boss_color_modifier;
        c->boss_color_modifier += active ? 0.05f : -0.0125f;
...
GmWindowCompose *gm_window_compose_open(const GmConfig *cfg,
    ...
    c->post_scratch = malloc(
        (size_t)c->fb.w * (size_t)c->fb.h * sizeof *c->post_scratch);
    c->outline_verts = malloc(
        (size_t)c->max_entity_verts * sizeof *c->outline_verts);
    c->outline_work = malloc(
        (size_t)c->fb.w * (size_t)c->fb.h * sizeof *c->outline_work);
    c->outline_depth = malloc(
        (size_t)c->fb.w * (size_t)c->fb.h * sizeof *c->outline_depth);
    if (!c->fb.color || !c->tris || !c->entity_verts || !c->post_scratch
            || !c->outline_verts || !c->outline_work
            || !c->outline_depth) {
        set_error(err, err_cap, "window compose allocation failed");
...
void gm_window_compose_advance(GmWindowCompose *c, GmPlayerView *view,
    ...
    advance_boss_color_state(c, nticks);
...
int gm_window_compose_draw(GmWindowCompose *c,
    ...
    double void_fog_y_factor = r->world_type == GM_WORLD_SUPERFLAT
        ? 1.0 : 0.03125;
    GmUnderwater uw;
    float boss_color_modifier = c->boss_color_modifier_prev
        + (c->boss_color_modifier - c->boss_color_modifier_prev)
            * frame->partial_ticks;
    ...
    gm_sky_set_boss_color_modifier(boss_color_modifier);
    gm_sky_set_weather(
    ...
    CrRgba clear = gm_frame_clear_color(
        day, r->dimension, fog_c1, night_vision, boss_color_modifier,
        blindness, (double)cpv->y, void_fog_y_factor, &uw);
...
#endif
    stamp(frame, 5);
    enum { WC_FIXED_ENTS = GM_RUNTIME_GHOST_VIEWS
        + GM_MOB_CAPACITY + 1 };
    int needed = gm_live_entity_slot_count(&r->entities)
        + r->end_crystals_cap + r->falling_blocks_cap
        + r->fireworks_cap + r->projectiles_cap + r->withers_cap
        + r->shulkers_cap + r->armor_stands_cap
        + WC_FIXED_ENTS;
    if (r->projectiles_cap > c->projectile_types_cap) {
        int cap = c->projectile_types_cap > 0
            ? c->projectile_types_cap : 32;
        int *grown;
        while (cap < r->projectiles_cap) {
            if (cap > INT_MAX / 2) return 0;
            cap *= 2;
        }
        grown = (int *)realloc(
            c->projectile_types, (size_t)cap * sizeof *grown);
        if (!grown) return 0;
        c->projectile_types = grown;
        c->projectile_types_cap = cap;
    }
    if (r->lightning_cap > c->lightning_views_cap) {
        int cap = c->lightning_views_cap > 0 ? c->lightning_views_cap : 8;
        GmLightningView *grown;
        while (cap < r->lightning_cap) {
            if (cap > INT_MAX / 2) return 0;
            cap *= 2;
        }
    ...
    if (needed > c->entity_views_cap) {
        int cap = c->entity_views_cap > 0 ? c->entity_views_cap : 512;
        GmEntityView *grown;
        while (cap < needed) {
            if (cap > INT_MAX / 2) return 0;
            cap *= 2;
    ...
    int nents = collect_entities(c, ents, c->entity_views_cap);
    ...
    if (worldmc_lightmap_mode() && r->dimension == 0) {
        gm_frame_lightmap_fill_view_boss(
            ...
            gm_world_rain_strength(&r->clock, frame->partial_ticks),
            gm_world_thunder_strength(&r->clock, frame->partial_ticks),
            night_vision, boss_color_modifier, c->lm_lut);
        lm = c->lm_lut;
    }
    ...
    gm_frame_entities_light_boss(
        ents, nents, r->world, r->dimension, boss_color_modifier, lm);
    int nchests = 0;
    for (int i = 0; i < r->chests_cap; ++i) {
        ...
        if (gm_world_block(r->world, tile->wx - 1, tile->wy, tile->wz)
                    == block
                || gm_world_block(
    ...
    for (int i = 0; i < r->ender_chests_cap; ++i)
        if (r->ender_chests[i].active
                ...
                && gm_world_block(
                    r->world, r->ender_chests[i].wx,
                    r->ender_chests[i].wy, r->ender_chests[i].wz) == 130)
            ++nender_chests;
    int nshulker_boxes = 0;
    ...
    if (nents > 0 || nchests > 0 || nender_chests > 0
            || nshulker_boxes > 0 || nspawners > 0) {
        ...
        for (int i = 0; i < r->mobs.spawners_cap; ++i) {
            ...
            if (!tile->active || tile->dimension != r->dimension
                    || gm_world_block(
                        r->world, tile->x, tile->y, tile->z) != 52)
                continue;
            double rotation = tile->prev_mob_rotation
                + (tile->mob_rotation - tile->prev_mob_rotation)
                    * (double)frame->partial_ticks;
            GmSpawnerView spawner = {
                tile->x, tile->y, tile->z, tile->entity_type,
                (float)rotation, frame->partial_ticks
            };
            nv += gm_spawner_miniatures_emit(
                &spawner, 1, c->entity_verts + nv,
        ...
        for (int i = 0; i < r->chests_cap; ++i) {
            ...
            GmChestRenderView chest = {
                ...
                large,
                large && xpos && meta == 2 ? 1 : 0,
                large && zpos && meta == 5 ? 1 : 0
            };
            nv += gm_chests_emit(
                &chest, 1, frame->partial_ticks,
        ...
        for (int i = 0; i < r->ender_chests_cap; ++i) {
            ...
            if (!tile->active || tile->dimension != r->dimension
                    || gm_world_block(
                        r->world, tile->wx, tile->wy, tile->wz) != 130)
                continue;
        ...
        for (int i = 0; i < r->static_containers_cap; ++i) {
            const GmRuntimeStaticContainer *tile =
            ...
            GmShulkerBoxRenderView box = {
                tile->wx, tile->wy, tile->wz,
                gm_world_meta(
                    r->world, tile->wx, tile->wy, tile->wz) % 6,
                tile->block - 219,
        ...
        nv = gm_wither_aura_emit(ents, nents, c->entity_verts,
                                 c->max_entity_verts);
        if (nv > 0) {
            CrTexture wither_texture = gm_wither_armor_texture();
            CrShadeCtx aura = {0};
            aura.atlas = &wither_texture;
            aura.fog_color = (CrRgba){0, 0, 0, 255};
            aura.alpha_test = 1;
            aura.alpha_ref = 0.1f;
            aura.layer = CR_LAYER_TRANSLUCENT;
            aura.blend = 5;
            aura.depth_lequal = 1;
            aura.repeat_uv = 1;
            aura.sample_mode = 1;
            ...
            apply_fluid_fog(&aura, &uw);
            render_layer(c, &cam, c->entity_verts, nv, &aura);
        }
        {
            int nl = gm_fishing_line_emit(
                r, frame->partial_ticks, c->swing_progress,
                cr_cfg()->fov_setting,
                cam.pos.x, cam.pos.y, cam.pos.z,
                cam.fov_deg, c->fb.h,
                c->entity_verts + nv, c->max_entity_verts - nv);
        ...
        nv = gm_guardian_beams_emit(ents, nents, c->entity_verts,
                                    c->max_entity_verts);
        if (nv > 0) {
            CrTexture beam_texture = gm_guardian_beam_texture();
            CrShadeCtx beam = {0};
            beam.atlas = &beam_texture;
            beam.fog_color = fog;
            beam.alpha_test = 1;
        ...
        nv = gm_items_emit(ents, nents, c->entity_verts,
                           c->max_entity_verts);
        nv += gm_held_blocks_emit(ents, nents, c->entity_verts + nv,
                                  c->max_entity_verts - nv);
        nv += gm_mooshroom_mushrooms_emit(
            ents, nents, c->entity_verts + nv, c->max_entity_verts - nv);
        nv += gm_falling_blocks_emit(ents, nents, c->entity_verts + nv,
                                     c->max_entity_verts - nv);
        nv += gm_minecart_contents_emit(
            ents, nents, frame->partial_ticks, c->entity_verts + nv,
            c->max_entity_verts - nv);
        if (nv > 0) {
            CrShadeCtx ish = {0};
            ish.atlas = &c->atlas;
        ...
        nv = gm_minecart_tnt_flash_emit(
            ents, nents, frame->partial_ticks, c->entity_verts,
            c->max_entity_verts);
        if (nv > 0) {
            CrShadeCtx flash = {0};
            flash.untextured = 1;
            flash.fog_color = fog;
            flash.layer = CR_LAYER_TRANSLUCENT;
            flash.blend = 5;
            flash.depth_lequal = 1;
            apply_fluid_fog(&flash, &uw);
            render_layer(c, &cam, c->entity_verts, nv, &flash);
        }
    ...
    int outline_active = build_entity_outline(c, &cam, ents, nents);
    stamp(frame, 9);
    if (c->backend == GM_BACKEND_CUDA) {
...
#endif
    if (outline_active)
        gm_overlay_entity_outline_composite(&c->fb, c->outline_work);
    stamp(frame, 10);

    {
        ...
        {
            int nausea = gm_potion_view_duration(pv, 9) > 0;
            float portal = pv->portal_prev
                + (pv->portal - pv->portal_prev) * frame->partial_ticks;
            gm_overlay_portal_warp(
                &c->fb, c->post_scratch, portal, pv->portal_phase,
                frame->partial_ticks, nausea, cam.fov_deg);
        }
        if (!pv->dead)
    ...
    if (pv->portal > 0.0f && gm_potion_view_duration(pv, 9) <= 0)
...
void gm_window_compose_close(GmWindowCompose *c) {
    ...
    free(c->entity_verts);
    free(c->outline_verts);
    free(c->structure_verts);
    free(c->post_scratch);
    free(c->outline_work);
    free(c->outline_depth);
    free(c->ppm_buf);
    free(c->chunks);
    free(c->runs);
    free(c->gsrc);
