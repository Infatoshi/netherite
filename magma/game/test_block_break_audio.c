/* Complete block-id break/place/hit sound map, compared against real Java
 * by trace/test_block_break_audio.py. Prints one B/P/H row per registered
 * non-air id with raw float bits so the comparison is exact. */
...
int main(void) {
...

    {
        ...
        CHECK(gm_runtime_sound_event_count(&runtime) == 5,
              "sound ring retains the five break events");
        for (int i = 0; i < 5; ++i) {
        ...
        CHECK(gm_runtime_sound_event_count(&runtime) == 10,
              "placement appends its sound after the five breaks");
        for (int i = 0; i < 5; ++i) {
