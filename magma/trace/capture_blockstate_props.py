...
def validate(response):
    if response.get("schema") != "qrl.blockstate_props.v4":
    ...
    for row in response.get("blocks", []):
        ...
        for field in (
                ...
                "causes_suffocation_mask",
                "opaque_material_mask", "solid_material_mask",
                "burning_material_mask", "blocks_movement_mask",
                "note_rock_material_mask", "note_sand_material_mask",
                "note_glass_material_mask", "note_wood_material_mask",
        ...
        for meta, value in enumerate(canonical):
            ...
            for field in (
                    ...
                    "fully_opaque_mask", "causes_suffocation_mask",
                    "opaque_material_mask", "solid_material_mask",
                    "burning_material_mask", "blocks_movement_mask",
                    "note_rock_material_mask", "note_sand_material_mask",
                    "note_glass_material_mask", "note_wood_material_mask",
...
def header_text(response, digest):
    ...
    suffocation_masks = [0] * 256
    opaque_material_masks = [0] * 256
    solid_material_masks = [0] * 256
    burning_material_masks = [0] * 256
    blocks_movement_masks = [0] * 256
    note_rock_masks = [0] * 256
    note_sand_masks = [0] * 256
    note_glass_masks = [0] * 256
    note_wood_masks = [0] * 256
    power_masks = [0] * 256
    ...
    for row in response["blocks"]:
        ...
        suffocation_masks[row["id"]] = row["causes_suffocation_mask"]
        opaque_material_masks[row["id"]] = row["opaque_material_mask"]
        solid_material_masks[row["id"]] = row["solid_material_mask"]
        burning_material_masks[row["id"]] = row["burning_material_mask"]
        blocks_movement_masks[row["id"]] = row["blocks_movement_mask"]
        note_rock_masks[row["id"]] = row["note_rock_material_mask"]
        note_sand_masks[row["id"]] = row["note_sand_material_mask"]
        note_glass_masks[row["id"]] = row["note_glass_material_mask"]
        note_wood_masks[row["id"]] = row["note_wood_material_mask"]
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}"
            for value in suffocation_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}"
            for value in opaque_material_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}"
            for value in burning_material_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}"
            for value in blocks_movement_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}" for value in note_rock_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}" for value in note_sand_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}" for value in note_glass_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
    ...
    for start in range(0, 256, 8):
        chunk = ", ".join(
            f"0x{value:04x}" for value in note_wood_masks[start:start + 8])
        lines.append(f"    {chunk},")
    lines.extend([
        "};",
        "",
