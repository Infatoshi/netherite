...
if [ -s "$OUT/camera.json" ]; then
	...
	SEED0_YAW=$(jq -r '.craster_yaw_deg // 0.0' "$OUT/camera.json")
	SEED0_PITCH=$(jq -r '.craster_pitch_deg // -35.0' "$OUT/camera.json")
	SEED0_FOV=$(jq -r '.fov_effective // 77.0' "$OUT/camera.json")
fi
SEED0_SPAWN_CX="${CRASTER_SPAWN_CX:-2}"
SEED0_SPAWN_CZ="${CRASTER_SPAWN_CZ:-11}"
SEED0_PREP_LIST="${CRASTER_PREP_LIST:-}"
...
if [ -s "$OUT/camera_seed7.json" ]; then
	...
	SEED7_YAW=$(jq -r '.craster_yaw_deg' "$OUT/camera_seed7.json")
	SEED7_PITCH=$(jq -r '.craster_pitch_deg' "$OUT/camera_seed7.json")
...
measure() {
	...
	local cand_png="${OUT}/craster_${name}.png"
	local -a render_env=()
	if [ ! -s "$golden" ]; then
	...
	if [ "$name" = seed0 ]; then
		render_env+=("CRASTER_SPAWN_CX=$SEED0_SPAWN_CX")
		render_env+=("CRASTER_SPAWN_CZ=$SEED0_SPAWN_CZ")
		if [ -n "$SEED0_PREP_LIST" ]; then
			render_env+=("CRASTER_PREP_LIST=$SEED0_PREP_LIST")
		fi
	elif [ "$name" = seed7 ]; then
		render_env+=("CRASTER_SPAWN_CX=$SEED7_SPAWN_CX")
		render_env+=("CRASTER_SPAWN_CZ=$SEED7_SPAWN_CZ")
	fi
