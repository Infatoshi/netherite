"""Convert a recstart Anvil snapshot into the minimal craster state patch.
...
against craster's generated canonical state. Mismatches become dimension-tagged
...
def _read_craster_states(path: Path) -> dict[tuple[int, int], np.ndarray]:
    chunks = {}
...
def _ensure_world_dump() -> Path:
    ...
    if not binary.exists() or binary.stat().st_mtime < source.stat().st_mtime:
        subprocess.run(["bash", str(TRACE / "build_world_dump.sh")], check=True, cwd=CRASTER)
    return binary
...
def ensure_snapshot_patch(tape_path: str, header: dict, ticks: list[dict]) -> Path | None:
    ...
    with cache.open("w") as out:
        for dimension, tx, tz in sorted(tiles):
            ...
            ], check=True, cwd=CRASTER, stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            craster = _read_craster_states(temp)
            ...
            for cx, cz in tiles[(dimension, tx, tz)]:
                jstate, cstate = java[(dimension, cx, cz)], craster[(cx, cz)]
