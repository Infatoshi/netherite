...
2. Start taping (any time, mid-session is fine - the header records the full
   start state):
       ...
       cd c/magma/raster/verify/trace
...
fixed-scene CPU gate:
    ...
    bash raster/verify/mc_capture/run_anim_verify.sh
...
- Container screens: `raster/verify/mc_capture/run_gui_verify.sh` covers all
  three implemented magma screens (inventory, crafting table, furnace).
  `capture_gui_actions.sh` + `run_gui_actions_verify.sh` verify inventory
  PICKUP, split/deposit, QUICK_MOVE, hotbar swap, THROW, cursor, counts, hover,
  and close against the real GUI after each operation.
...
`particles`, and `transit` classes in `raster/verify/trace/pixel_gate.py` are
...
| Player inventory panel | `game/screen.c:gm_screen_draw`; `game/screen.c:gm_screen_layout` | `game/test_screen.sh` | **Partial**: informational GUI gate; player-model preview is absent | High | Open a populated inventory, move cursor across armor/craft/hotbar slots, then close. |
...
| Empty-hand Steve/Alex arm, swing/equip | `game/hand.c:gm_hand_draw`; `game/hand.c:gm_hand_set_skin` | `game/test_hand.sh` | **Partial**: canonical tape exercises empty arm/swing; skin variants are not isolated | Medium | Switch Steve/Alex skin setting and punch a block through a full swing. |
...
| Eating, drinking, and blocking use transforms | `game/hand.c:gm_hand_emit_held` (generic fallback; only bow has a use branch) | No; partial/stubbed | **No** | High | Eat bread, drink a potion, and block with a sword in one fixed-camera tape. |
...
| Particles: block break, explosion, portal, Enderman, dragon | `game/frame_capture.c:gm_frame_capture_write` (no particle emit pass) | No; explicitly unimplemented in divergences #14/#40/#46 | **No** | High | Break blocks, detonate creeper, enter portal, hit enderman, and kill dragon in focused tapes. |
