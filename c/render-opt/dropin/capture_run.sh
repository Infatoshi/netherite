...
echo "[driver] launched mode=$MODE, waiting for qrl listening..."

# wait up to 240s for the bridge
for i in $(seq 1 240); do
  if grep -q "\[qrl\] listening" runclient.log 2>/dev/null; then echo "[driver] qrl up after ${i}s"; break; fi
  sleep 1
done
grep -q "\[qrl\] listening" runclient.log 2>/dev/null || { echo "[driver] ERROR qrl never came up"; tail -20 runclient.log; exit 2; }
