...
typedef struct {
    ...
    uint64_t epoch;   /* changes on every toroidal-slot recycle */
    ...
    u16 spawn_ground[256]; /* pre-populate getGroundAboveSeaLevel state */
} LChunk;
...
static LChunk *gen_chunk(CrLight *L, int cx, int cz) {
    ...
    /* WorldServer.createSpawnPosition asks WorldProvider.canCoordinateBeSpawn
     * after base terrain/structures but before population adds trees, plants,
     * snow, and lakes. Preserve that sea-level upward probe before decoration. */
    for (int x = 0; x < 16; ++x) {
        for (int z = 0; z < 16; ++z) {
            int y = 63;
            while (y < 255 && primer->data[CB_INDEX(x, y + 1, z)] != 0)
                ++y;
            uint16_t state = 0;
            if (gm_model_key_to_state(
                    (int)primer->data[CB_INDEX(x, y, z)], 0, &state)
                    == GM_MAP_UNSUPPORTED) {
                fprintf(stderr,
                    "[light] FATAL: unsupported pre-populate spawn key %u "
                    "in chunk (%d,%d)\n",
                    (unsigned)primer->data[CB_INDEX(x, y, z)], cx, cz);
                abort();
            }
            c->spawn_ground[x + z * 16] = state;
        }
    }
...
unsigned long long light_chunk_epoch(const CrLight *L, int cx, int cz) {
    LChunk *chunk = L ? find_chunk(L, cx, cz) : NULL;
    return chunk ? (unsigned long long)chunk->epoch : 0;
}
...
int light_spawn_ground_id(const CrLight *L, int wx, int wz) {
    LChunk *c = L ? find_chunk(L, wx >> 4, wz >> 4) : NULL;
    if (!c) return -1;
    return gm_state_id(c->spawn_ground[(wx & 15) + (wz & 15) * 16]);
}
...
int light_debug_set_biome(CrLight *L, int wx, int wz, int biome) {
    LChunk *c;
    if (!L || biome < 0 || biome >= 255) return 0;
...
int light_debug_set_block_light(
        CrLight *L, int wx, int wy, int wz, int value) {
    LChunk *c;
    if (!L || wy < 0 || wy >= WY || value < 0 || value > 15) return 0;
    c = find_chunk(L, wx >> 4, wz >> 4);
    if (!c) return 0;
    c->blk[CB_INDEX(wx & 15, wy, wz & 15)] = (u8)value;
    return 1;
}
...
int light_load_block_snapshot(CrLight *L, int wx, int wy, int wz, int value) {
    if (!L || wy < 0 || wy >= WY || value < 0 || value > 15) return 0;
    LChunk *c = find_chunk(L, wx >> 4, wz >> 4);
    if (!c) return 0;
    c->blk[CB_INDEX(wx & 15, wy, wz & 15)] = (u8)value;
...
void light_finalize_block_snapshot(CrLight *L) {
    if (L) L->blocklight_dirty = 0;
}

int light_load_height_snapshot(
        CrLight *L, int wx, int wz, int value) {
    if (!L || value < 0 || value > 256) return 0;
    LChunk *c = find_chunk(L, wx >> 4, wz >> 4);
    if (!c) return 0;
    c->height[((wz & 15) << 4) | (wx & 15)] = (u16)value;
    return 1;
}
...
int light_height(const CrLight *L, int wx, int wz) {
    if (!L) return -1;
    LChunk *c = find_chunk((CrLight *)L, wx >> 4, wz >> 4);
    return c ? c->height[((wz & 15) << 4) | (wx & 15)] : -1;
}
