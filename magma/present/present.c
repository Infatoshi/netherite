...
void cr_window_poll(CrWindow *win, CrInput *out)
{
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_KEYDOWN:
            if (e.key.keysym.sym == SDLK_ESCAPE) {
                /* The app owns Minecraft's ESC stack: container -> pause ->
                 * resume/title. Release relative capture immediately so the
                 * first press can open a clickable pause screen. */
                out->key_esc = 1;
                if (win->captured)
                    present_set_capture(win, 0);
            }
            ...
            break;
        case SDL_TEXTINPUT: {
            size_t have = (size_t)out->text_len;
            size_t add = strlen(e.text.text);
            size_t room = sizeof(out->text) - 1 - have;
            if (add > room) add = room;
            if (add > 0) {
                memcpy(out->text + have, e.text.text, add);
                out->text_len += (int)add;
    ...
    out->mouse_middle = (mb & SDL_BUTTON(SDL_BUTTON_MIDDLE)) ? 1 : 0;
    out->mouse_x = mx;
    out->mouse_y = my;

    out->key_num = key_num;
    out->wheel   = wheel;
    out->time_ms = SDL_GetTicks();
