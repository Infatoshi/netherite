/* game/player_ctl.c - PLAYER-CTL module: live composition of verified mc-sim kernels.
 *
                            ...
                            int in_water_pre, int eye_water_post, int land_jump)
{
     ...
     * 20260712T055346Z t1488: craster's food hit 19 early because bouncing on
                    ...
                    GmBlockEdit *edits, int *nedits, int max_edits)
{
     ...
     * which is exactly the vanilla FOV pumping artifact. */
    {
         ...
         * craster only mirrors the hotbar, so trust the recorded inputs
     ...
     * the oracle's t712 move is accel-only (dx=0.084872) while craster kept
     * vx=0.157733; craster_dx - oracle_dx equalled the retained motion
     * exactly. */
    if (s_hurt_vel_reset) {
     ...
     * 20260712T055346Z t438-562 hold-dig: without the delay craster broke a 6th
...
use_done:
     ...
     * at hp 20 while craster had accumulated the whole ~13-block drop into the
