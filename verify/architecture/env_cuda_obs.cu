...
struct EnvCudaObs {
 int device,n; size_t cap; HybridCamInputsFn inputs; HybridCamFreshFn fresh;
...
extern "C" int env_cuda_obs_render(EnvCudaObs *s,void *env,int force,u16 *cam,u8 *depth,u8 *edge,EnvCudaObsStats *stats) {
 if(!s||!env)return -1; EnvCudaObsStats z={};double start=now_ms();size_t used=0;
 ...
 for(int e=0;e<s->n;e++) {
  ...
  memcpy(s->hcells+used,cells,count*sizeof(u16));r->cells=s->dcells+used;used+=count;
 }
 z.pack_ms=now_ms()-start; z.h2d_bytes=used*sizeof(u16)+s->n*sizeof(Camera);
 CK(cudaEventRecord(s->ev[0],s->stream));
 if(used)CK(cudaMemcpyAsync(s->dcells,s->hcells,used*sizeof(u16),cudaMemcpyHostToDevice,s->stream));
