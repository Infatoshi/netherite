...
static void test_component_controls(void) {
    ...
    for (int i = 0; i < GM_LIVE_MAX; ++i)
        CHECK(gm_live_spawn_item_exact(
                  &drops, 710000 + i, 10.0 + (double)i, 64.0, 10.0,
                  0.0, 0.0, 0.0, 0.0F,
                  1, 1, 0, 0, 10, 1),
              "growable capacity prefill succeeds");
    shear_seed = UINT64_C(0x3456789ABCDE);
    math_seed = UINT64_C(0x123456789ABC);
    next_id = 640001;
    CHECK(gm_mobs_shear_sheep(
              ...
              && drops.n_overflow == next_id - 640001
              && drops.spawn_fail_count == 0,
          "production shearing grows into active cold item storage");
    for (int slot = GM_LIVE_MAX;
            slot < gm_live_entity_slot_count(&drops); ++slot) {
        const GmLiveEnt *drop = gm_live_entity_ref(&drops, slot);
        CHECK(drop && drop->active && drop->eid == 640001 + slot - GM_LIVE_MAX
                  && drop->item == 35 && drop->meta == 14,
              "cold shearing drop preserves ordered wool identity");
    }
    gm_live_destroy(&drops);
