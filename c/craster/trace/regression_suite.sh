...
# structures-off ground-truth save, print one line per seed. Run from c/craster.
...
export CRASTER_GENPROBE=$PWD/trace/out/craster_genprobe.log
SEEDS=("$@")
...
for S in "${SEEDS[@]}"; do
    ...
    D=$(uv run --no-project python3 trace/genprobe_diff.py --java "$JLOG" \
        --craster trace/out/craster_genprobe.log 2>&1)
