/* CUDA driver for mob_ai_skeleton - SAME core/mob_ai_skeleton.h as CPU.
 * PfWork on device heap; A* out-of-line to avoid nvcc compile hang. */
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include "../core/mob_ai_skeleton.h"
...
__global__ void run_mob_ai_skeleton(i64 seed, int nticks, MasTickOut *out, PfWork *work) {
...
int main(int argc, char **argv) {
    ...
    MasTickOut *d_out = NULL;
    PfWork *d_work = NULL;
    MasTickOut *host = (MasTickOut *)malloc(sizeof(MasTickOut) * (size_t)nticks);

    cudaDeviceSetLimit(cudaLimitStackSize, (size_t)128 * 1024);

    if (cudaMalloc(&d_out, sizeof(MasTickOut) * (size_t)nticks) != cudaSuccess ||
    ...
    run_mob_ai_skeleton<<<1, 1>>>(seed, nticks, d_out, d_work);
    ...
    cudaMemcpy(host, d_out, sizeof(MasTickOut) * (size_t)nticks, cudaMemcpyDeviceToHost);

    for (int t = 0; t < nticks; ++t) {
        const MasTickOut *o = &host[t];
        printf("%08x\n", (unsigned)o->state);
        for (int i = 0; i < 4; ++i) {
