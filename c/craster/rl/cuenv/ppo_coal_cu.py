"""Batched-GPU PPO for the mine-coal skill (M4): VecCuEnv on the 3090.
...
Run (anvil, GPU1/3090 - check nvidia-smi first):
  cd c/craster && uv run --no-project --with numpy,torch,matplotlib \
      python rl/cuenv/ppo_coal_cu.py
"""
import os
import sys
import time
...
DEVICE = int(os.environ.get("CUENV_DEV", 1))       # 1=3090, 0=PRO 6000
# OPT-IN training-reward gate (cuenv_set_reward_gate): nearest-coal dist
...
def main():
    ...
    def save_png():
        ...
        ax.legend()
        ax.set_title("batched PPO mine-coal (cuenv, 3090): success by stage")
        fig.tight_layout()
