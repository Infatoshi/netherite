...
echo "== assets/build_igloo_templates.py =="
uv run --no-project --with nbtlib python assets/build_igloo_templates.py
echo "== assets/build_village_templates.py =="
VILLAGE_OUT="$REPO/.tmp/village-templates"
mkdir -p "$VILLAGE_OUT"
(cd "$REPO/java/Minecraft" && ./gradlew -g run/gradle -q villageTemplateGolden) \
