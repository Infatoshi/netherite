...
if [ ! -f assets/mob_atlas.h ]; then
  need_atlas=1
     ...
     ! grep -q 'CR_MOB_SWEEP' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_WOLF_TAME' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_CAT_SIAMESE' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_SHULKER_SPARK' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_ELYTRA' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_STEVE' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_LLAMA_GRAY' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_LLAMA_DECOR_BLACK' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_LLAMA_SPIT' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_LEAD_KNOT' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_PAINTINGS' assets/mob_atlas.h || \
     ! grep -q 'CR_MOB_SNOWMAN_PUMPKIN' assets/mob_atlas.h || \
     ! grep -q 'CR_ENDERCRYSTAL_BEAM_RGBA' assets/mob_atlas.h || \
     ! grep -q 'CR_BEACON_BEAM_RGBA' assets/mob_atlas.h; then
  need_atlas=1
fi
