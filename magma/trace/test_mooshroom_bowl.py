#!/usr/bin/env python3
"""Bit-compare exact Mooshroom bowl inventory and EntityItem toss state."""
...
def native(case, java):
    raw = subprocess.check_output([
        ...
        "bowl_" + case["mode"], str(case["math_seed48"]),
...
def normalize_float_fields(result):
    for drop in result.get("drops", ()):
        for key in ("yaw", "hover_start"):
            drop[key] = struct.unpack(">I", struct.pack(">f", drop[key]))[0]
    return result
    ...
    try:
        ...
        for name, overrides in cases:
            ...
            java = normalize_float_fields(
                request(args.port, "mooshroom_bowl_locked", case))
            magma = normalize_float_fields(native(case, java))
            if java != magma:
                raise AssertionError(f"{name}: java={java!r} magma={magma!r}")
        print(f"PASS java==magma: {len(cases)} exact Mooshroom bowl "
