...
run_step() {
	...
	local t0 t1 rc
	t0=$(date +%s)
	(cd "$dir" && timeout -k 15 "$tmo" "$@") >"$log" 2>&1 </dev/null
	rc=$?
	t1=$(date +%s)
	if [ "$rc" -eq 0 ]; then
...
run_step completeness-local 1200 "$ROOT" \
	bash verify/completeness/gate.sh
...
if [ -z "$FIRST_SNAP" ]; then
	...
	if [ "$MODE" = full ]; then
		...
		# Seeds 14/16 enter known liquid/item-pickup strict tails in the
		# freshly baked d6 fixtures. Keep the quick lane on two 500-tick
		# zero-diff trajectories; the full census remains a separate gate.
		run_step blaze-cpu-gate 600 "$ROOT" \
			uv run --no-project --with numpy python blaze/env/verify_cpu.py --seeds 11,20 --ticks 500
	fi
fi
