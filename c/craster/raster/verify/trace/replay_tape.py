#!/usr/bin/env python3
"""replay_tape.py - replay a human-play tape through craster and find the first
divergence. THE human-verification flywheel step (see c/craster/VERIFY.md).
...
Replay: the same inputs through craster (set_pose at t0 from the header,
...
Phase 2 diffs craster frames against the tape's sparse oracle frames.
...
# Ghost pushers: recorded oracle entities become ent_box events so craster can
...
def gui_slot_id(gui, index):
    """Map vanilla Container slot-list indexes to craster's unified slots."""
    if gui == "GuiInventory":
        if index == 0:
            return 45
        if 1 <= index <= 4:
...
def tape_to_script(header, ticks, script_path, tape_path=None):
    """Emit the craster JSONL event script for this tape.
    ...
    with open(script_path, "w") as f:
        ...
        # craster worldgen, then apply it after set_pose has generated the
        ...
        for row in ticks:
            ...
            # use-item 0.2 folded in); craster applies its own scaling from the
            # flag inputs, so replay the raw key sign.
            if sgn(i["f"]):
                ev["forward"] = sgn(i["f"])
            if sgn(i["s"]):
                # tape s is vanilla moveStrafe (+1 = LEFT); GmAction.strafe is
                # craster's input convention (+1 = D/right, negated internally
            ...
            # HUD truth which craster already knows how to draw. Absent air
            ...
            # 4 gives slack for craster-vs-oracle drift within tolerance)
            ...
            # entity also becomes an ent_view so craster DRAWS it at this
            ...
            # mouse), emit a render-only gui_view. craster maps the class to a
            ...
            if "gui" in row:
                ...
                # differ by vanilla screen, so translate them to craster's
...
def first_divergence(ticks, c_rows):
    """Return (tick, field, tape_val, craster_val, |d|) of the first per-field
    ...
    # the death screen or respawn the Oracle, while craster correctly stops the
...
def main():
    ...
    ap.add_argument("--cuda", action="store_true",
                    help="(default) render on the GPU (craster_game_cuda,"
    ...
    # ---- ONE craster run: state + (if the tape has golden frames) pixels.
    ...
    scr = os.path.join(out, "craster_script.jsonl")
    tape_to_script(header, ticks, scr, tape_path=args.tape)
    state = os.path.join(out, "craster_state.jsonl")
    frame_ticks = [(row["t"], row["frame"]) for row in ticks if "frame" in row]
    frames_npy = None
    every = 1
    offset = 0
    if frame_ticks:
        # npy-direct: craster appends each rendered frame to ONE uint8
        # [N,H,W,3] file (no 500+ PPM writes then re-reads); this file IS
        # the craster_frames.npy record. Frame i is tick offset + i*every.
        frames_npy = os.path.join(out, "craster_frames.npy")
        ...
        offset = fts[0]
    # mobs=False: craster's spawn RNG cannot match the oracle session's, so a
    ...
    if first is None:
        ...
        print(f"[tape] FIRST DIVERGENCE tick {t} field {k}: "
              f"oracle={jv!r} craster={cv!r} |d|={d:.3g}")
    ...
    if frame_ticks:
        ...
        # immutable); craster side: the run's npy, memory-mapped (no PPM/PNG
        ...
        # craster against a frame of the pre-tape scene is pure noise.
        stale = 0
        ...
        for r_ in results:
            ...
            if first is not None and abs(t - first[0]) <= 40:
                cpng = os.path.join(out, f"craster_t{t:06d}.png")
        ...
        # craster_frames.npy is written by the run itself; record its ticks
        np.save(os.path.join(out, "craster_frames.ticks.npy"),
        ...
        with open(rp, "w") as f:
            ...
            if first is None:
                ...
                f.write(f"**FIRST DIVERGENCE: tick {t}, field `{k}`** "
                        f"oracle={jv!r} craster={cv!r} |d|={d:.3g}; inputs "
