#ifndef CRASTER_GAME_CONFIG_H
#define CRASTER_GAME_CONFIG_H
...
typedef struct {
     ...
     * determinism - the oracle session craster replays against never
     * leaves its start time-of-day). */
    int daylight;
    /* Mob simulation (spawning + AI). Default on. Tape replay passes off when
     * the recorded oracle session ran with doMobSpawning=false: craster's spawn
