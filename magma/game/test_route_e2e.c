...
static int recover_health(GmRuntime *r){
    GmPlayerView anchor;gm_runtime_view(r,&anchor);
    gm_runtime_set_pose(r,anchor.x,anchor.y,anchor.z,anchor.yaw,anchor.pitch);
    for(int pass=0;pass<64&&r->vitals.health<17.0f&&!r->dead;++pass){
        if(r->vitals.foodLevel<20){int food=slot_item(r,424,1);
            if(food<0)food=slot_item(r,423,1);
        ...
        GmAction wait;memset(&wait,0,sizeof wait);wait.hotbar_sel=-1;
        for(int t=0;t<200&&!r->dead;++t)gm_runtime_tick(r,wait);
    }
    gm_runtime_set_pose(r,anchor.x,anchor.y,anchor.z,anchor.yaw,anchor.pitch);
    gm_runtime_set_velocity(r,0,0,0);
    int ok=!r->dead&&r->vitals.health>=17.0f;
    if(!ok)fprintf(stderr,
        "recover_health: dead=%d hp=%.1f/%.1f food=%d saturation=%.1f raw_mutton=%d cooked_mutton=%d chicken=%d fire=%d wither=%d effects=%d\n",
        r->dead,r->vitals.health,r->vitals.maxHealth,r->vitals.foodLevel,r->vitals.saturation,
        total_item(r,423),total_item(r,424),total_item(r,365),r->player_fire_ticks,
        r->mobs.player_wither_ticks,r->potion_count);
    return ok;
}
...
static int find_mine_pose(GmRuntime *r,int id,int *tx,int *ty,int *tz,
...
static int mine_one(GmRuntime *r,int block,int tool_slot){
    int x,y,z;double px,py,pz;float yaw,pitch;
    if(!find_mine_pose(r,block,&x,&y,&z,&px,&py,&pz,&yaw,&pitch))return 0;
...
static int mine_until_item(GmRuntime *r,int block,int tool_slot,int item,int attempts){
    int before=total_item(r,item);
    for(int i=0;i<attempts&&total_item(r,item)==before;++i)
        if(!mine_one(r,block,tool_slot))return 0;
    return total_item(r,item)>before;
}
...
static int cast_obsidian(GmRuntime *r,int x,int y,int z){
    ...
    int bucket=selectable_slot(r,slot_item(r,325,1));if(bucket<0||
       !find_mine_pose(r,11,&wx,&wy,&wz,&px,&py,&pz,&yaw,&pitch)){
    ...
    /* A source left by an earlier cast can convert the newly placed lava in
     * the same server tick.  Observe the result before requiring another
     * water-bucket action; Java bucket casting has the same legal shortcut. */
    if(gm_world_block(r->world,x,y,z)==49)return 1;
    bucket=selectable_slot(r,slot_item(r,325,1));if(bucket<0){
        fprintf(stderr,"cast (%d,%d,%d): lava placement did not return bucket\n",x,y,z);return 0;}
    if(!find_mine_pose(r,9,&wx,&wy,&wz,&px,&py,&pz,&yaw,&pitch)){
    ...
    int water=slot_item(r,326,1);int placed=water>=0&&place_north(r,water,x,y,z);
    if(!placed&&gm_world_block(r->world,x,y,z)!=49){
...
int main(void){
    ...
    int wood_pick=slot_item(&r,270,1);for(int i=0;i<11;++i)CHECK(mine_one(&r,1,wood_pick),"mine natural stone");
    ...
    STOP_IF_FAILED(&r);
    int stone_pick=slot_item(&r,274,1);for(int i=0;i<8;++i)CHECK(mine_one(&r,15,stone_pick),"mine natural iron ore");
    CHECK(mine_one(&r,16,stone_pick),"mine first natural coal fuel");
    CHECK(mine_one(&r,16,stone_pick),"mine second natural coal fuel");
    ...
    CHECK(mine_until_item(&r,13,-1,318,64),"mine natural gravel until World.rand drops flint");
    ...
    stone_pick=slot_item(&r,274,1);CHECK(mine_one(&r,1,stone_pick),"mine backup sword cobblestone one");
    CHECK(mine_one(&r,1,stone_pick),"mine backup sword cobblestone two");
    ...
    for(int i=0;i<26;++i)CHECK(mine_one(&r,3,-1),"mine natural dirt scaffolding");
    ...
    STOP_IF_FAILED(&r);
    for(int i=0;i<2;++i)CHECK(mine_one(&r,17,-1),"mine extra oak for bed and bow planks");
    ...
    while((total_item(&r,35)<3||
           total_item(&r,423)+total_item(&r,424)<32)&&sheep_attempts++<64){
    ...
    CHECK(total_item(&r,423)+total_item(&r,424)>=32,
    ...
    while(total_item(&r,288)<8&&feather_attempts++<30){
    ...
    CHECK(total_item(&r,288)>=8,"collect eight natural feather drops");
    for(int batch=0;total_item(&r,318)<8&&batch<8;++batch)
        CHECK(mine_until_item(&r,13,-1,318,64),
              "mine natural gravel until World.rand drops arrow flint");
    CHECK(total_item(&r,318)>=8,"collect eight RNG-driven flint drops");
    gm_runtime_set_pose(&r,0.5,72,2.5,0,0);CHECK(gm_runtime_use_block(&r,0,72,3),
        "open table for bow and arrows");
    for(int batch=0;total_item(&r,280)<12&&batch<3;++batch){
        planks=slot_item(&r,5,2);empty_grid(g);g[0]=g[3]=planks;CHECK(craft(&r,2,g),
            "craft sticks for bow and arrows");}
    CHECK(total_item(&r,280)>=12,"twelve sticks reach route inventory");
    ...
    for(int batch=0;total_item(&r,262)<32&&batch<8;++batch){
        int flint2=slot_item(&r,318,1);sticks=slot_item(&r,280,1);
        int feather=slot_item(&r,288,1);empty_grid(g);g[0]=flint2;g[3]=sticks;g[6]=feather;
        CHECK(flint2>=0&&sticks>=0&&feather>=0&&craft(&r,3,g),
              "craft arrows from flint, stick, and feather");}
    CHECK(total_item(&r,262)>=32,"thirty-two crafted arrows reach inventory");
    STOP_IF_FAILED(&r);
    ...
    if(slot_item(&r,267,1)<0&&slot_item(&r,272,1)<0){
        if(total_item(&r,4)<3){int pick=slot_item(&r,257,1);
            if(pick<0)pick=slot_item(&r,274,1);
            for(int n=total_item(&r,4);n<3;++n)
                CHECK(pick>=0&&mine_one(&r,1,pick),
                      "mine replacement sword and blast-cover cobblestone");}
        gm_runtime_set_pose(&r,0.5,72,2.5,0,0);CHECK(gm_runtime_use_block(&r,0,72,3),
    ...
    while(total_item(&r,368)<13&&ender_attempts++<80){
    ...
    /* Eye insertion leaves the travel hook beside whichever frame was visited
     * last, which can be inside the generated portal-room wall.  Recover on
     * the known Overworld work pad before returning to the live portal. */
    gm_runtime_set_pose(&r,0.5,73.0,3.5,0,0);
    ...
    for(int i=0;i<ED_NUM_CRYSTALS;++i){EdCrystal *c=&r.dragon.state.arena.crystals[i];
        /* Guarded spikes have an iron-bar roof at crystal Y+2. Remove only
         * its center block, then shoot vertically from outside the twelve-
         * block crystal blast radius. */
        int roof_x=(int)floor(c->x),roof_y=(int)c->y+2,roof_z=(int)floor(c->z);
        if(gm_world_block(r.world,roof_x,roof_y,roof_z)==101)
            for(int dx=-1;dx<=1;++dx)for(int dz=-1;dz<=1;++dz)
                if(gm_world_block(r.world,roof_x+dx,roof_y,roof_z+dz)==101)
                    CHECK(mine_fixed_hover(&r,roof_x+dx,roof_y,roof_z+dz,
                              c->x+dx,c->y+4.0,c->z+dz,0,90),
                          "mine guarded spike roof through survival action");
        for(int attempt=0;attempt<3&&c->alive&&!r.dead;++attempt){
            int bow=selectable_slot(&r,slot_item(&r,261,1));
            int arrows_before=total_item(&r,262);
            CHECK(bow>=0&&arrows_before>=1,"crystal shot has crafted bow and arrow");
            gm_runtime_set_pose(&r,c->x,c->y+20.0,c->z,0,90);
            GmAction draw;memset(&draw,0,sizeof draw);draw.use=1;draw.hotbar_sel=bow;
            for(int t=0;t<22;++t){gm_runtime_set_velocity(&r,0,0,0);
                gm_runtime_tick(&r,draw);}
            GmAction release;memset(&release,0,sizeof release);release.hotbar_sel=bow;
            gm_runtime_tick(&r,release);
            for(int t=0;t<40&&c->alive&&!r.dead;++t)idle_hover(&r,1);
            CHECK(total_item(&r,262)<arrows_before,"crystal shot consumes crafted arrow");}
        CHECK(!c->alive,"destroy End crystal with survival bow projectile");}
