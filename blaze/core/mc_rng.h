...
MC_HD static inline i32 jrand_int_bound(JavaRandom *r, i32 bound) {
    ...
    for (;;) {
        i32 bits = jrand_next(r, 31);
        i32 val = bits % bound;
        /* Random.nextInt tests a Java int expression. Its overflow is
         * specified two's-complement wrap; signed C overflow is undefined
         * and O2 used to optimize this rare rejection away. */
        i32 java_sum = (i32)((u32)bits - (u32)val
                           + (u32)(bound - 1));
        if (java_sum >= 0) return val;
    }
}
