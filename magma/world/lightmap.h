 ...
 * Lightning is intentionally absent. Night Vision is supplied as
 * EntityRenderer.getNightVisionBrightness for the rendered frame. */
static inline CrLightmapRgb cr_lightmap_rgb_boss_night_vision(
        int dimension, int sky, int block, float sun_brightness,
        float torch_flicker_x, float gamma, float boss_color_modifier,
        float night_vision)
{
    ...
    /* EntityRenderer.updateLightmap: a BossInfo whose darkenSky bit is set
     * desaturates/dims the generic lightmap before the End replacement and
     * before Night Vision/gamma. EntityWither always publishes that bit. */
    if (boss_color_modifier > 0.0f) {
        float f8 = cr_lm_clamp01(boss_color_modifier);
        r = r * (1.0f - f8) + r * 0.7f * f8;
        g = g * (1.0f - f8) + g * 0.6f * f8;
        b = b * (1.0f - f8) + b * 0.6f * f8;
    }
...
{
    return cr_lightmap_rgb_boss_night_vision(
        dimension, sky, block, sun_brightness, torch_flicker_x, gamma,
        0.0f, night_vision);
}
