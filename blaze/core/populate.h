...
/* Sapling growth uses the decoration generators with block notifications on.
 * The live host bridge installs this observer to retain effects which depend on
 * generator write order (notably BlockLeaves.breakBlock decay marking). */
static void (*g_w_before_write)(World *w, int x, int y, int z,
...
#ifndef __CUDA_ARCH__
        if (g_w_before_write)
            g_w_before_write(w, x, y, z, old, v);
#endif
        w->blocks[idx] = (u16)v;
#ifndef __CUDA_ARCH__
