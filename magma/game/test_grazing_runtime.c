...
int main(void) {
    ...
    int grass_events = 0, grass_sounds = 0;
    for (int index = 0; index < gm_runtime_world_event_count(&r); ++index)
        if (gm_runtime_world_event_get(&r, index, &event)
                && event.id == 2001 && event.dimension == r.dimension
                && event.x == x && event.y == y - 1 && event.z == z
                && event.data == 2)
            ++grass_events;
    for (int index = 0; index < gm_runtime_sound_event_count(&r); ++index)
        if (gm_runtime_sound_event_get(&r, index, &sound)
