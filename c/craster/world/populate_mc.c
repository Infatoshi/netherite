...
/* stale-skylight mode: default ON; CRASTER_SHROOMLIGHT=ca falls back to the fixpoint CA */
static int popmc_stale_light_enabled(void) {
    static int mode = -1;
    if (mode < 0) {
        const char *s = getenv("CRASTER_SHROOMLIGHT");
 ...
 * fluid CA is off (default); the CA distinction only matters with CRASTER_FLUID_CA. */
...
static void record_window_cells(Window *win, const u16 *owr, const u16 *stb,
    ...
    if (n > cap) {
        fprintf(stderr,
            ...
            "(raise owr_cells_max in craster.conf / caps.h)\n",
...
static Window *build_window(long long seed, int bcx, int bcz) {
    ...
    JavaRandom r;
    {
        ...
        if (g_oob_n && getenv("CRASTER_DEBUG_TREES"))
    ...
    win->seed = seed; win->bcx = bcx; win->bcz = bcz; win->valid = 1; win->seq = g_builds;
    {
        ...
        { static int g_maxpop = 0;
          if (getenv("CRASTER_DEBUG_CAPS") && npop > g_maxpop) {
              g_maxpop = npop;
     ...
     * java WATER -> craster FLOWING_WATER; disabling: 157k -> 14.6k mismatches, seed 0
     * roughly neutral at +712). Opt back in with CRASTER_FLUID_CA=1. */
    if (getenv("CRASTER_FLUID_CA"))
    ...
    { static int g_maxdec = 0; if (getenv("CRASTER_DEBUG_CAPS") && ndec > g_maxdec) {
