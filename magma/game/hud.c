...
_Static_assert(GM_GUI_INV_PANEL == GUI_INV_PANEL &&
               ...
               GM_GUI_LARGE_CHEST_PANEL == GUI_LARGE_CHEST_PANEL &&
               ...
               GM_GUI_ENCHANTING_OPTION_NORMAL == GUI_ENCHANTING_OPTION_NORMAL &&
               GM_GUI_ENCHANTING_OPTION_DISABLED == GUI_ENCHANTING_OPTION_DISABLED &&
               GM_GUI_ENCHANTING_OPTION_HOVER == GUI_ENCHANTING_OPTION_HOVER &&
               GM_GUI_ENCHANTING_ICON0 == GUI_ENCHANTING_ICON0 &&
               GM_GUI_ENCHANTING_ICON1 == GUI_ENCHANTING_ICON1 &&
               GM_GUI_ENCHANTING_ICON2 == GUI_ENCHANTING_ICON2 &&
               GM_GUI_ENCHANTING_ICON0_DISABLED == GUI_ENCHANTING_ICON0_DISABLED &&
               GM_GUI_ENCHANTING_ICON1_DISABLED == GUI_ENCHANTING_ICON1_DISABLED &&
               GM_GUI_ENCHANTING_ICON2_DISABLED == GUI_ENCHANTING_ICON2_DISABLED &&
               GM_GUI_ENCHANTING_SGA_FONT == GUI_ENCHANTING_SGA_FONT &&
               GM_GUI_ENCHANTING_BOOK == GUI_ENCHANTING_BOOK &&
               GM_GUI_ANVIL_PANEL == GUI_ANVIL_PANEL &&
               GM_GUI_ANVIL_NAME_ACTIVE == GUI_ANVIL_NAME_ACTIVE &&
               GM_GUI_ANVIL_NAME_INACTIVE == GUI_ANVIL_NAME_INACTIVE &&
               GM_GUI_ANVIL_INVALID == GUI_ANVIL_INVALID &&
               GM_GUI_MERCHANT_PANEL == GUI_MERCHANT_PANEL &&
               GM_GUI_MERCHANT_NEXT_NORMAL == GUI_MERCHANT_NEXT_NORMAL &&
               GM_GUI_MERCHANT_NEXT_HOVER == GUI_MERCHANT_NEXT_HOVER &&
               GM_GUI_MERCHANT_NEXT_DISABLED == GUI_MERCHANT_NEXT_DISABLED &&
               GM_GUI_MERCHANT_PREV_NORMAL == GUI_MERCHANT_PREV_NORMAL &&
               GM_GUI_MERCHANT_PREV_HOVER == GUI_MERCHANT_PREV_HOVER &&
               GM_GUI_MERCHANT_PREV_DISABLED == GUI_MERCHANT_PREV_DISABLED &&
               GM_GUI_MERCHANT_DISABLED == GUI_MERCHANT_DISABLED &&
               GM_GUI_HORSE_PANEL == GUI_HORSE_PANEL &&
               GM_GUI_HORSE_CHEST == GUI_HORSE_CHEST &&
               GM_GUI_HORSE_SADDLE == GUI_HORSE_SADDLE &&
               GM_GUI_HORSE_ARMOR == GUI_HORSE_ARMOR &&
               GM_GUI_HORSE_LLAMA_DECOR == GUI_HORSE_LLAMA_DECOR &&
               GM_GUI_SHULKER_PANEL == GUI_SHULKER_PANEL &&
               GM_GUI_BEACON_PANEL == GUI_BEACON_PANEL &&
               GM_GUI_BEACON_BUTTON_NORMAL == GUI_BEACON_BUTTON_NORMAL &&
               GM_GUI_BEACON_BUTTON_SELECTED == GUI_BEACON_BUTTON_SELECTED &&
               GM_GUI_BEACON_BUTTON_DISABLED == GUI_BEACON_BUTTON_DISABLED &&
               GM_GUI_BEACON_BUTTON_HOVER == GUI_BEACON_BUTTON_HOVER &&
               GM_GUI_BEACON_CONFIRM == GUI_BEACON_CONFIRM &&
               GM_GUI_BEACON_CANCEL == GUI_BEACON_CANCEL &&
               GM_GUI_BEACON_SPEED == GUI_BEACON_SPEED &&
               GM_GUI_BEACON_HASTE == GUI_BEACON_HASTE &&
               GM_GUI_BEACON_RESISTANCE == GUI_BEACON_RESISTANCE &&
               GM_GUI_BEACON_JUMP == GUI_BEACON_JUMP &&
               GM_GUI_BEACON_STRENGTH == GUI_BEACON_STRENGTH &&
               GM_GUI_BEACON_REGENERATION == GUI_BEACON_REGENERATION &&
               GM_GUI_DISPENSER_PANEL == GUI_DISPENSER_PANEL &&
               GM_GUI_HOPPER_PANEL == GUI_HOPPER_PANEL &&
               GM_GUI_POTION_OVERLAY == GUI_POTION_OVERLAY,
...
static void gui_blit_tinted(CrFramebuffer *fb, int idx, int dx, int dy,
                            int scale, unsigned rgb) {
    if (idx < 0 || idx >= GUI_SPRITE_COUNT || scale < 1) return;
    const GuiSprite *sprite = &GUI_SPRITES[idx];
    int tr = (rgb >> 16) & 255, tg = (rgb >> 8) & 255, tb = rgb & 255;
    for (int sy = 0; sy < sprite->h; ++sy) {
        for (int sx = 0; sx < sprite->w; ++sx) {
            const unsigned char *p =
                &GUI_RGBA[sprite->off + (sy * sprite->w + sx) * 4];
            CrRgba pixel = {
                (u8)((p[0] * tr + 127) / 255),
                (u8)((p[1] * tg + 127) / 255),
                (u8)((p[2] * tb + 127) / 255), p[3]
            };
            if (pixel.a == 0) continue;
            int px0 = dx + sx * scale, py0 = dy + sy * scale;
            for (int yy = 0; yy < scale; ++yy)
                for (int xx = 0; xx < scale; ++xx)
                    hud_blend_px_tex(fb, px0 + xx, py0 + yy, pixel);
        }
    }
}
...
int gm_gui_texture_get(int idx, CrTexture *out) {
    if (!out || idx < 0 || idx >= GUI_SPRITE_COUNT) return 0;
    const GuiSprite *sprite = &GUI_SPRITES[idx];
    *out = (CrTexture){0};
    out->w = sprite->w;
    out->h = sprite->h;
    out->texels = (const CrRgba *)(const void *)(GUI_RGBA + sprite->off);
    return 1;
}

int gm_gui_enchant_book_texture_get(CrTexture *out) {
    return gm_gui_texture_get(GUI_ENCHANTING_BOOK, out);
}
...
int gm_gui_item_icon(CrFramebuffer *fb, int item_id, int item_meta,
    ...
    /* Generated potion models layer a tinted potion_overlay below the bottle.
     * Meta alone does not identify a 1.11.2 potion type, so this boundary uses
     * PotionTypes.EMPTY's exact fallback color. Rich-stack NBT tint transport
     * remains a separate registry-wide item-render concern. */
    if (item_id == 373 || item_id == 438 || item_id == 441)
        gui_blit_tinted(
            fb, GM_GUI_POTION_OVERLAY, dx, dy, scale, 0xF800F8u);
...
static uint8_t g_sga_font_width[256];
static int g_font_ready = 0;
static int g_sga_font_ready = 0;

static void font_init_one(int sprite, uint8_t widths[256]) {
    const GuiSprite *fs = &GUI_SPRITES[sprite];
    for (int c = 0; c < 256; c++) {
        if (c == 32) { widths[c] = 4; continue; }
        ...
        widths[c] = (uint8_t)(col + 2);
    }
}

static void font_init(void) {
    font_init_one(GUI_FONT, g_font_width);
    g_font_ready = 1;
}

static void sga_font_init(void) {
    font_init_one(GUI_ENCHANTING_SGA_FONT, g_sga_font_width);
    g_sga_font_ready = 1;
}
...
int gm_sga_font_width(const char *s) {
    if (!g_sga_font_ready) sga_font_init();
    int w = 0;
    for (; s && *s; ++s) w += g_sga_font_width[(unsigned char)*s];
    return w;
}

void gm_sga_font_draw(CrFramebuffer *fb, const char *s,
                      int dx, int dy, int scale, unsigned rgb) {
    if (!fb || !fb->color || scale < 1) return;
    if (!g_sga_font_ready) sga_font_init();
    const GuiSprite *fs = &GUI_SPRITES[GUI_ENCHANTING_SGA_FONT];
    CrRgba color = {(u8)(rgb >> 16), (u8)(rgb >> 8), (u8)rgb, 255};
    int x = dx;
    for (; s && *s; ++s) {
        int ch = (unsigned char)*s;
        int w = g_sga_font_width[ch];
        int cx = (ch % 16) * 8, cy = (ch / 16) * 8;
        for (int row = 0; row < 8; ++row)
            for (int col = 0; col < w - 1 && col < 8; ++col) {
                if (GUI_RGBA[fs->off
                        + ((cy + row) * fs->w + cx + col) * 4 + 3] == 0)
                    continue;
                for (int yy = 0; yy < scale; ++yy)
                    for (int xx = 0; xx < scale; ++xx)
                        hud_blend_px(
                            fb, x + col * scale + xx,
                            dy + row * scale + yy, color);
            }
        x += w * scale;
    }
}
...
void gm_hud_set_boss(int show, float frac, const char *name) {
    g_boss_show = show;
    if (frac < 0.0f) frac = 0.0f;
    if (frac > 1.0f) frac = 1.0f;
    g_boss_frac = frac;
    g_boss_name = name && name[0] ? name : "Ender Dragon";
}
...
void gm_hud_gui_button_draw(
        CrFramebuffer *fb, int x, int y, int w, int h, int scale,
        const char *label, int hovered, int enabled) {
    int sprite = !enabled ? HUD_BUTTON_DISABLED
        : hovered ? HUD_BUTTON_HOVER : HUD_BUTTON_ENABLED;
    int half;
    unsigned color;
    if (!fb || !label || scale < 1 || w <= 0 || h <= 0) return;
    half = w / (2 * scale);
    if (half > 100) half = 100;
    if (half > 0) {
        hud_blit_sub_alpha(
            fb, sprite, 0, 0, half, h / scale,
            x, y, scale, 1.0F);
        hud_blit_sub_alpha(
            fb, sprite, 200 - half, 0, half, h / scale,
            x + half * scale, y, scale, 1.0F);
    }
    color = !enabled ? 0xa0a0a0u
        : hovered ? 0xffffa0u : 0xe0e0e0u;
    gm_font_draw(
        fb, label,
        (x / scale + (w / scale) / 2
            - gm_font_width(label) / 2) * scale,
        y + (h - 8 * scale) / 2,
        scale, color, 1);
}
