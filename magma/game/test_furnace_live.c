...
static void test_complete_fuel_limits_and_bucket_edges(void) {
    FurnaceLive furnace;

    furnace_live_init(&furnace);
    CHECK(furnace_live_insert(&furnace, FURNACE_LIVE_SLOT_FUEL,
                              sr_mk(268, 4, 0)) == 1); /* wooden sword */
    CHECK(stack_is(furnace.fuel, 268, 1, 0));
    furnace_live_init(&furnace);
    CHECK(furnace_live_insert(&furnace, FURNACE_LIVE_SLOT_FUEL,
                              sr_mk(323, 32, 0)) == 16); /* sign */
    furnace_live_init(&furnace);
    CHECK(furnace_live_insert(&furnace, FURNACE_LIVE_SLOT_FUEL,
                              sr_mk(171, 80, 12)) == 64); /* carpet */
    CHECK(sr_getItemBurnTime(sr_mk(171, 1, 12)) == 67);
    CHECK(sr_getItemBurnTime(sr_mk(173, 1, 0)) == 16000);
    CHECK(sr_getItemBurnTime(sr_mk(448, 1, 0)) == 400);

    furnace_live_init(&furnace);
    CHECK(furnace_live_insert(&furnace, FURNACE_LIVE_SLOT_INPUT,
                              sr_mk(BLK_IRON_ORE, 1, 0)) == 1);
    CHECK(furnace_live_insert(&furnace, FURNACE_LIVE_SLOT_FUEL,
                              sr_mk(SR_LAVA_BUCKET, 2, 0)) == 1);
    furnace_live_tick(&furnace);
    CHECK(stack_is(furnace.fuel, SR_BUCKET, 1, 0));
    CHECK(furnace.burn_time == 20000);
    CHECK(furnace.current_burn_time == 20000);
    CHECK(furnace.cook_time == 1);

    furnace_live_init(&furnace);
    furnace_live_set_ic(&furnace, FURNACE_LIVE_SLOT_INPUT,
                        ic_mk(19, 1, 1));
    furnace_live_set_ic(&furnace, FURNACE_LIVE_SLOT_FUEL,
                        ic_mk(SR_BUCKET, 1, 0));
    furnace.burn_time = 2;
    furnace.current_burn_time = 1600;
    furnace.cook_time = TE_COOK_TICKS - 1;
    furnace_live_tick(&furnace);
    CHECK(stack_is(furnace.input, SR_AIR, 0, 0));
    CHECK(stack_is(furnace.fuel, SR_WATER_BUCKET, 1, 0));
    CHECK(stack_is(furnace.output, 19, 1, 0));
    CHECK(furnace.burn_time == 1);
    CHECK(furnace.cook_time == 0);
}
...
int main(void) {
    ...
    test_complete_fuel_limits_and_bucket_edges();
