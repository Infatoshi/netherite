...
REQUIRED_SPRITES = {
    ...
    "slime", "dragon", "dragon_exploding", "particles", "explosion", "sweep",
    "villager", "villager_farmer", "villager_librarian",
    "villager_priest", "villager_smith", "villager_butcher",
    "zombie_villager", "zombie_farmer", "zombie_librarian",
    "zombie_priest", "zombie_smith", "zombie_butcher",
    "snowman", "snowman_pumpkin", "endermite",
}
...
def builder_contract():
    """Evaluate only the builder's declarations, including generated ranges."""
    sys.path.insert(0, str(BUILDER.parent))
    try:
        contract = runpy.run_path(str(BUILDER), run_name="_mob_atlas_contract")
    finally:
        sys.path.pop(0)
    return contract, dict(contract["MOB_SPRITES"])
...
def golden_sprite(zf: zipfile.ZipFile, member: str, contract) -> Image.Image:
    if member == "@snowman_pumpkin":
        def block(name: str) -> Image.Image:
        ...
        img.paste(top, (16, 0))
        img.paste(top.transpose(Image.Transpose.FLIP_TOP_BOTTOM), (32, 0))
        return img
    if not member.startswith("@horse:"):
        return Image.open(io.BytesIO(zf.read(jar_path_for_member(member)))).convert("RGBA")
    color, marking, armor = map(int, member.split(":")[1:])

    def load(path: str) -> Image.Image:
        return Image.open(io.BytesIO(zf.read(ENTITY + path))).convert("RGBA")

    img = load("horse/" + contract["HORSE_BASE"][color])
    marking_name = contract["HORSE_MARKING"][marking]
    armor_name = contract["HORSE_ARMOR"][armor]
    if marking_name is not None:
        img = Image.alpha_composite(img, load("horse/" + marking_name))
    if armor_name is not None:
        img = Image.alpha_composite(img, load("horse/armor/" + armor_name))
    return img
...
def main() -> int:
    ...
    contract, members = builder_contract()
    ...
    with zipfile.ZipFile(jar) as zf:
        for name, x0, y0, x1, y1, nw, nh in sprites:
            ...
            if member is None:
                ...
                continue
            try:
                img = golden_sprite(zf, member, contract)
            except KeyError:
                path = jar_path_for_member(member)
                print(f"FAIL: {name}: jar missing {path}")
                bad += 1
                continue
            path = member if member.startswith("@") else jar_path_for_member(member)
