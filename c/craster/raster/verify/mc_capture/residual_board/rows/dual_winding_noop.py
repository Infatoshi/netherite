...
showed crop delta ~0.001 (dual vs CRASTER_NO_DUAL_WIND). This row locks that:

  1) game_candidate defaults dual OFF (opt-in CRASTER_DUAL_WIND=1 only)
...
def _default_dual_off(gc_src: str) -> bool:
    """Default dual must be off; only CRASTER_DUAL_WIND=1 enables it."""
    if "CRASTER_DUAL_WIND" not in gc_src:
        return False
...
def main():
    ...
    gc_path = repo / "c/craster/raster/verify/mc_capture/game_candidate.c"
