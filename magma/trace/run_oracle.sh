...
DECORATIVE_NBT_FIXTURE="${DECORATIVE_NBT_FIXTURE:-}"
...
COMMAND_SEARGE_TRIGGER="${COMMAND_SEARGE_TRIGGER:-0}"
COMMAND_TRIGGER_TEXT="${COMMAND_TRIGGER_TEXT:-}"
TRIGGER_SCORE_FIXTURE="${TRIGGER_SCORE_FIXTURE:-}"
...
DECORATIVE_NBT_JAVA_ARGS=()
FLOWER_POT_JAVA_ARGS=()
SKULL_JAVA_ARGS=()
COMMAND_SUCCESS_JAVA_ARGS=()
TRIGGER_SCORE_JAVA_ARGS=()
COMMAND_SUCCESS_RUNTIME_ARGS=()
...
if [ -n "$DECORATIVE_NBT_FIXTURE" ]; then
    read -r -a DECORATIVE_NBT_VALUES <<<"$DECORATIVE_NBT_FIXTURE"
    if [ "${#DECORATIVE_NBT_VALUES[@]}" -ne 4 ] ||
       ! [[ "${DECORATIVE_NBT_VALUES[0]}" =~ ^-?[0-9]+$ &&
             "${DECORATIVE_NBT_VALUES[1]}" =~ ^-?[0-9]+$ &&
             "${DECORATIVE_NBT_VALUES[2]}" =~ ^-?[0-9]+$ ]] ||
       [ ! -f "${DECORATIVE_NBT_VALUES[3]}" ]; then
        echo "DECORATIVE_NBT_FIXTURE must contain DX DY DZ NBT_JSON" >&2
        exit 2
    fi
    DECORATIVE_NBT_JAVA_ARGS=(
        --decorative-nbt-offset "${DECORATIVE_NBT_VALUES[@]}")
fi
...
if [ -n "$TRIGGER_SCORE_FIXTURE" ]; then
    read -r -a TRIGGER_SCORE_VALUES <<<"$TRIGGER_SCORE_FIXTURE"
    if [ "${#TRIGGER_SCORE_VALUES[@]}" -ne 2 ] ||
       ! [[ "${TRIGGER_SCORE_VALUES[0]}" =~ ^-?[0-9]+$ &&
             "${TRIGGER_SCORE_VALUES[1]}" =~ ^[01]$ ]]; then
        echo "TRIGGER_SCORE_FIXTURE must contain SCORE LOCKED(0|1)" >&2
        exit 2
    fi
    TRIGGER_SCORE_JAVA_ARGS=(--trigger-score "${TRIGGER_SCORE_VALUES[@]}")
fi
if [ -n "$COMMAND_SUCCESS_FIXTURE" ]; then
    ...
    if [ "$COMMAND_SEARGE_TRIGGER" = 1 ]; then
        COMMAND_SUCCESS_JAVA_ARGS+=(--command-searge-trigger)
    elif [ "$COMMAND_SEARGE_TRIGGER" != 0 ]; then
        echo "COMMAND_SEARGE_TRIGGER must be 0 or 1" >&2
        exit 2
    fi
    if [ -n "$COMMAND_TRIGGER_TEXT" ]; then
        COMMAND_SUCCESS_JAVA_ARGS+=(
            --command-trigger-text "$COMMAND_TRIGGER_TEXT")
    fi
    if [ "$COMMAND_SEARGE_TRIGGER" = 1 ] || [ -n "$COMMAND_TRIGGER_TEXT" ]; then
        COMMAND_SUCCESS_JAVA_ARGS+=(
            --command-trigger-clock-out "$OUT/java_command_trigger_clock.json")
        COMMAND_SUCCESS_RUNTIME_ARGS=(
            --trigger-command-block-from-capsule
            --command-trigger-clock-file "$OUT/java_command_trigger_clock.json")
    fi
fi
