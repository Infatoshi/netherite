#!/usr/bin/env python3
"""Strict same-scene Mooshroom model and mushroom-layer gate."""
...
STATES = (
    "mooshroom_adult_idle",
    "mooshroom_adult_head_pose",
    "mooshroom_child",
)
OWNERSHIP_THRESHOLD = 25
# Stable fixed-function/software boundary samples after the Java baked quad,
# UV orientation, transforms, and lighting all match. pxdiff finds no entity
# component above its reporting floor; keep the scattered per-pixel ceilings
# explicit so any growth is still a regression.
HARD_BUDGET = {
    "mooshroom_adult_idle": 41,
    "mooshroom_adult_head_pose": 80,
    "mooshroom_child": 49,
}
...
def main():
    ...
    java_bg_a = load(goldens / "mooshroom_background_a.png")
    java_bg_b = load(goldens / "mooshroom_background_b.png")
    native_bg = load(c_frames / "mooshroom_background.ppm")
    if not (java_bg_a.shape == java_bg_b.shape == native_bg.shape):
        raise SystemExit("FAIL: Mooshroom background dimensions differ")

    background_ab = np.max(np.abs(java_bg_a - java_bg_b), axis=2)
    report = {
        "rule": "same_scene_union_owned_max_channel_25",
    ...
    for state in STATES:
        ...
        entry = {
            ...
            "ownership_xor_px": int(np.count_nonzero(
                java_owned ^ native_owned)),
        ...
        if args.selftest and state == STATES[0]:
            points = np.argwhere(owned & ~hard)
            if not len(points):
                raise SystemExit("FAIL: Mooshroom mutation has no owned pixel")
            y, x = points[0]
    ...
    if args.selftest:
        if not mutation_rejected:
            raise SystemExit("FAIL: Mooshroom hard-pixel mutation was accepted")
    ...
    if failed:
        raise SystemExit("FAIL: Mooshroom subject contract")
    print("PASS: Mooshroom subject contract")
