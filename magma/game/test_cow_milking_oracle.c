...
int main(int argc, char **argv) {
    const char *mode, *base_mode;
    int mooshroom_bowl;
    uint64_t math_seed, player_seed;
    int cow_eid;
    float yaw, pitch;
    double player_x, player_z;
    if (argc != 9 || ((mode = argv[1]),
                (mooshroom_bowl = !strncmp(mode, "bowl_", 5)),
                (base_mode = mooshroom_bowl ? mode + 5 : mode),
                strcmp(base_mode, "insert") && strcmp(base_mode, "drop")
                && strcmp(base_mode, "replace_full"))
        ...
        fprintf(stderr, "usage: %s [bowl_]insert|drop|replace_full "
    ...
    printf("{\"ok\":true,\"mode\":\"%s\",\"result\":\"success\","
        ...
        base_mode, cow_eid, player_x, player_z);
