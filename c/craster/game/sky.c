/* game/sky.c - MC 1.11.2 sky for the craster software rasterizer. See sky.h.
 *
...
#include <math.h>
#include <stddef.h>
#include <stdlib.h>   /* getenv/atoi for the CRASTER_FOG gate */
...
/* Default ON (Java always runs setupFog(0) on terrain). CRASTER_FOG=0 disables.
 * Same cached-getenv pattern as CRASTER_SMOOTH. */
int gm_terrain_fog_enabled(void) {
    static int cached = -1;
    if (cached < 0) {
        const char *s = getenv("CRASTER_FOG");
