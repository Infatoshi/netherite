...
| Death GuiGameOver (title, score, buttons, tint) | `game/hud.c:hud_death_draw` | `run_ui_hud_gates.sh` hard `hud_death` + live respawn | **Yes** (owned chrome bit-exact vs A/B goldens) | — | Closed: genuine `hud_death_{a,b}.png`; Respawn/Title hit + timer. |
