...
| Zombie villager variant | `game/entity_render.c:gm_entity_type_for_name`; `game/entity_render.c:gm_entity_skin_for_name`; `game/entity_render.c:gm_entities_emit` | Exact eight-box part/UV/winding gate, distinct live type, and all six jar-identical profession skins in `game/test_entity_render.sh` | **No**; no stable real-Java pixel scene yet | High | Summon each profession beside an ordinary zombie for close face, nose, robe, and headwear comparisons. |
...
| Squid model and tentacle animation | `game/entity_render.c:emit_squid`; `game/mob_live.c:gm_mobs_fill_views` | Full model/pose checks in `game/test_entity_render.sh`; exact live handoff in `game/test_squid_runtime`; `ENTITY_GATE_SQUID_ONLY=1` zero-noise Java A/B subject gate | **Bounded**: dry pose has no hard residual; swimming has one measured internal-tentacle depth-tie pixel | Low | Extend the promoted keyframes into a close live swimming tape; static transform, tentacle angle, and subject ownership are locked. |
| Witch model, nose, and held item | `game/entity_render.c:gm_entities_emit`; `game/item_render.c:gm_held_items_emit`; `game/runtime.c:gm_runtime_sync_swamp_witches` | Type/part checks in `game/test_entity_render.sh`; natural one-time materialization in `game/test_scattered_live.sh`; six-seed constructor/RNG comparison in `trace/test_swamp_witch.py` | **No**: base state and initial-spawn RNG are oracle-locked, but potion combat and pixels are not | High | Record one hut witch through idle, self-potion drinking, target selection, held-potion pose, throw, hurt, and death. |
...
| Magma-cube model and squash/stretch | `game/entity_render.c:gm_entities_emit` | Type/part checks in `game/test_entity_render.sh`; `ui_entities` sizes/squish | **Open**: four stable Java pairs, four strict C residuals | High | Summon magma cube on a marked floor and record several complete jumps. |
...
| XP-orb model/color animation | `game/entity_render.c:gm_entities_emit` | Marker/part count in `game/test_entity_render.sh`; stable visible `ui_entities` pair | **Open**: C ROI is `2.263/ch`, `hard_px=11963` | High | Spawn a spread of differently valued XP orbs and record attraction/pickup. |
...
| Slime model + LayerSlimeGel | `game/entity_render.c` (EntitySlime map, size/squish, `gm_slime_gel_emit`) | Size scale + gel α=0.1 geometry gates (`test_entity_render`, `ui_entities`) | **Open**: stable real-Java A/B goldens exist for sizes 1/2/4 and squish, but all four C ROIs are strict residuals | High | Summon slime sizes 1, 2, and 4; record bounce squish and outer gel pass. |
...
| Durability bar | `game/hud.c:hud_item_durability` | `test_hud`; focused stable Java pair | **Yes**: half-durability ownership is bit-exact | Low | Damage wood and diamond tools to several values and scroll between them. |
| XP bar and level number | `game/hud.c:gm_hud_draw` | XP totals plus focused stable Java pair | **Yes**: half-fill and level are bit-exact | Low | Collect orbs across a level boundary and pause at several fractional fills. |
| Hearts, armor, absorption, hurt flash | `game/hud.c:gm_hud_draw` | `test_hud`; focused stable Java pairs | **Yes** for iron armor, absorption+armor, and both flash phases; broader values remain to census | Medium | Take one-half-heart steps, heal, and include low-health heart flash. |
| Hunger and poison state | `game/hud.c:gm_hud_draw` | `test_hud`; focused stable Java pair | **Yes** for the promoted poison/partial state | Medium | Sprint until hunger falls, eat, and hold on odd and even food values. |
| Air bubbles and partial bubble | `game/hud.c:gm_hud_draw` | Tape plumbing plus sticky-air focused Java pair | **Yes** for the promoted partial state | Medium | Submerge until air drains and record full/partial bubbles through first damage. |
| Dragon boss bar fill and label | `game/hud.c:gm_hud_set_boss`; `game/hud.c:gm_hud_draw` | Dragon sim/geometry gates plus focused stable Java pair | **Yes** for half fill and label | Low | Fight dragon from full to half health and keep the bar unobstructed. |
...
| Portal full-screen swirl | `game/overlay.c:gm_overlay_portal_screen` | Tape field plumbing; focused stable Java pair and phase mutation controls | **Open**: live-scene composition remains `46.887/ch`, `hard_px=391116` | High | Stand in portal from first contact through maximum opacity without leaving. |
| Portal/Nausea camera warp | `game/overlay.c:gm_overlay_portal_warp`; capture and window callers | Nine-case real-client/native float-state gate; projection mutation unit test | **Partial**: client ramp, cutoff, reset, phase rate, interpolation state, overlay suppression, and checkpoint continuation are exact; stable projection pixels remain open | High | Same portal hold plus Nausea onset/removal, with straight grid lines to expose projection residuals. |
...
| Underwater fog, FOV, tint, and texture overlay | `game/underwater.c:gm_uw_eval`; `game/underwater.c:gm_uw_overlay_draw` | Focused stable Java pair plus strict full-frame gate | **Open**: `6.083/ch`, `hard_px=388619`; eye-at-surface fog remains weak | High | Walk from shore to eye-at-surface to fully submerged, then look in four directions. |
